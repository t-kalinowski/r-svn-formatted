...
int F77_SYMBOL(dsvdc)(double *x, int *ldx, int *n, int *p, double *s, double *e, double *u, int *ldu, double *v,
                      int *ldv, double *work, int *job, int *info)
{
    double b, c;
    double cs;
    ...
    *info = 0;
    ...
    L20:
        ...
        for (j = lp1; j <= i__2; ++j)
        {
            ...
            i__3 = *n - l + 1;
            t = -F77_SYMBOL(ddot)(&i__3, &x[l + l * x_dim1], &c__1, &x[l + j * x_dim1], &c__1) / x[l + l * x_dim1];
            i__3 = *n - l + 1;
            F77_SYMBOL(daxpy)(&i__3, &t, &x[l + l * x_dim1], &c__1, &x[l + j * x_dim1], &c__1);
        L30:
    ...
    L80:
        ...
        {
            i__3 = *n - l;
            F77_SYMBOL(daxpy)(&i__3, &e[j], &x[lp1 + j * x_dim1], &c__1, &work[lp1], &c__1);
        }
        i__2 = *p;
        for (j = lp1; j <= i__2; ++j)
        {
            i__3 = *n - l;
            d__1 = -e[j] / e[lp1];
            F77_SYMBOL(daxpy)(&i__3, &d__1, &work[lp1], &c__1, &x[lp1 + j * x_dim1], &c__1);
        }
    L120:
        if (!wantv)
        {
...
L200:
    ...
    for (ll = 1; ll <= i__1; ++ll)
    {
        ...
        {
            i__3 = *n - l + 1;
            t = -F77_SYMBOL(ddot)(&i__3, &u[l + l * u_dim1], &c__1, &u[l + j * u_dim1], &c__1) / u[l + l * u_dim1];
            i__3 = *n - l + 1;
            F77_SYMBOL(daxpy)(&i__3, &t, &u[l + l * u_dim1], &c__1, &u[l + j * u_dim1], &c__1);
        }
    L220:
        i__2 = *n - l + 1;
...
L300:
    ...
    for (ll = 1; ll <= i__1; ++ll)
    {
        ...
        {
            i__3 = *p - l;
            t = -F77_SYMBOL(ddot)(&i__3, &v[lp1 + l * v_dim1], &c__1, &v[lp1 + j * v_dim1], &c__1) /
                v[lp1 + l * v_dim1];
            i__3 = *p - l;
            F77_SYMBOL(daxpy)(&i__3, &t, &v[lp1 + l * v_dim1], &c__1, &v[lp1 + j * v_dim1], &c__1);
        }
    L320:
        i__2 = *p;
        for (i = 1; i <= i__2; ++i)
...
L490:
    ...
    L500:
        if (wantv)
        {
            F77_SYMBOL(drot)(p, &v[k * v_dim1 + 1], &c__1, &v[m * v_dim1 + 1], &c__1, &cs, &sn);
        }
    }
    goto L610;
...
L520:
    ...
    for (k = l; k <= i__1; ++k)
    {
        ...
        if (wantu)
        {
            F77_SYMBOL(drot)(n, &u[k * u_dim1 + 1], &c__1, &u[(l - 1) * u_dim1 + 1], &c__1, &cs, &sn);
        }
    }
    goto L610;
...
L540:
    ...
    d__6 = (d__1 = s[m], abs(d__1)), d__7 = (d__2 = s[m - 1], abs(d__2)), d__6 = max(d__6, d__7),
    d__7 = (d__3 = e[m - 1], abs(d__3)), d__6 = max(d__6, d__7), d__7 = (d__4 = s[l], abs(d__4)),
...
L550:
    ...
    for (k = l; k <= i__1; ++k)
    {
        ...
        if (wantv)
        {
            F77_SYMBOL(drot)(p, &v[k * v_dim1 + 1], &c__1, &v[(k + 1) * v_dim1 + 1], &c__1, &cs, &sn);
        }
        F77_SYMBOL(drotg)(&f, &g, &cs, &sn);
        s[k] = f;
        f = cs * e[k] + sn * s[k + 1];
        ...
        if (wantu && k < *n)
        {
            F77_SYMBOL(drot)(n, &u[k * u_dim1 + 1], &c__1, &u[(k + 1) * u_dim1 + 1], &c__1, &cs, &sn);
        }
    }
    e[m - 1] = f;
    ++iter;
