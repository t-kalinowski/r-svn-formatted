...
int lminfl_(double *x, int *ldx, int *n, int *k, double *qraux, double *b, double *resid, double *hat, double *coef,
            double *sigma)
{
    double denom;
    double dummy, sum;
    ...
    */
    ...
    for (j = 1; j <= i__1; ++j)
    {
        ...
        sigma[j] = 1.;
        F77_SYMBOL(dqrsl)
        (&x[x_offset], ldx, n, k, &qraux[1], &sigma[1], &sigma[1], &dummy, &dummy, &dummy, &dummy, &c__10000, &info);
        i__2 = *n;
        for (i = 1; i <= i__2; ++i)
        {
    ...
    for (i = 1; i <= i__1; ++i)
    {
        ...
        F77_SYMBOL(dqrsl)
        (&x[x_offset], ldx, n, k, &qraux[1], &sigma[1], &dummy, &sigma[1], &dummy, &dummy, &dummy, &c__1000, &info);
