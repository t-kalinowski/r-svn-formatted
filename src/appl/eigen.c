...
int F77_SYMBOL(cg)(int *nm, int *n, double *ar, double *ai, double *wr, double *wi, int *matz, double *zr, double *zi,
                   double *fv1, double *fv2, double *fv3, int *ierr);
int F77_SYMBOL(ch)(int *nm, int *n, double *ar, double *ai, double *w, int *matz, double *zr, double *zi, double *fv1,
                   double *fv2, double *fm1, int *ierr);
...
static int comqr2_(int *nm, int *n, int *low, int *igh, double *ortr, double *orti, double *hr, double *hi, double *wr,
...
L190:
    ...
    for (i = k; i <= i__1; ++i)
    {
        ...
        for (j = k; j <= i__2; ++j)
        {
            ...
            c = c + (d__1 = ar[j + i * ar_dim1], abs(d__1)) + (d__2 = ai[j + i * ai_dim1], abs(d__2));
            r = r + (d__1 = ar[i + j * ar_dim1], abs(d__1)) + (d__2 = ai[i + j * ai_dim1], abs(d__2));
        L200:;
        }
        /*     .......... guard against zero c or r due to underflow .........
        . */
...
{
    /* System generated locals */
    int ar_dim1, ar_offset, ai_dim1, ai_offset, zr_dim1, zr_offset, zi_dim1, zi_offset;

    /* Local variables */
    extern int cbal_(), corth_(), comqr_(), cbabk2_(), comqr2_();
    int is1, is2;
...
L10:
    ...
    comqr_(nm, n, &is1, &is2, &ar[ar_offset], &ai[ai_offset], &wr[1], &wi[1], ierr);
    goto L50;
/*     .......... find both eigenvalues and eigenvectors .......... */
L20:
    comqr2_(nm, n, &is1, &is2, &fv2[1], &fv3[1], &ar[ar_offset], &ai[ai_offset], &wr[1], &wi[1], &zr[zr_offset],
            &zi[zi_offset], ierr);
    if (*ierr != 0)
    {
        goto L50;
...
{
    /* System generated locals */
    int ar_dim1, ar_offset, ai_dim1, ai_offset, zr_dim1, zr_offset, zi_dim1, zi_offset, i__1, i__2;
...
L10:
    htridi_(nm, n, &ar[ar_offset], &ai[ai_offset], &w[1], &fv1[1], &fv2[1], &fm1[3]);
    if (*matz != 0)
    {
        goto L20;
    }
...
L20:
    ...
    htribk_(nm, n, &ar[ar_offset], &ai[ai_offset], &fm1[3], n, &zr[zr_offset], &zi[zi_offset]);
L50:
    return 0;
}
    ...
    *ierr = 0;
    ...
    for (i = l; i <= i__1; ++i)
    {
        ...
        for (j = i; j <= i__2; ++j)
        {
            ...
            hr[i + j * hr_dim1] = yr * hr[i + j * hr_dim1] + yi * hi[i + j * hi_dim1];
            hi[i + j * hi_dim1] = si;
            /* L155: */
        }
        ...
        for (j = *low; j <= i__2; ++j)
        {
            ...
            hr[j + i * hr_dim1] = yr * hr[j + i * hr_dim1] - yi * hi[j + i * hi_dim1];
            hi[j + i * hi_dim1] = si;
            /* L160: */
        }
...
L240:
    ...
    for (ll = *low; ll <= i__1; ++ll)
    {
        ...
        tst1 = (d__1 = hr[l - 1 + (l - 1) * hr_dim1], abs(d__1)) + (d__2 = hi[l - 1 + (l - 1) * hi_dim1], abs(d__2)) +
               (d__3 = hr[l + l * hr_dim1], abs(d__3)) + (d__4 = hi[l + l * hi_dim1], abs(d__4));
...
L320:
    sr = (d__1 = hr[en + enm1 * hr_dim1], abs(d__1)) + (d__2 = hr[enm1 + (en - 2) * hr_dim1], abs(d__2));
    si = 0.;
...
L340:
    ...
    for (i = lp1; i <= i__1; ++i)
    {
        sr = hr[i + (i - 1) * hr_dim1];
        hr[i + (i - 1) * hr_dim1] = 0.;
        d__1 = pythag_(&hr[i - 1 + (i - 1) * hr_dim1], &hi[i - 1 + (i - 1) * hi_dim1]);
        ...
        for (j = i; j <= i__2; ++j)
        {
            ...
            zzr = hr[i + j * hr_dim1];
            zzi = hi[i + j * hi_dim1];
            hr[i - 1 + j * hr_dim1] = xr * yr + xi * yi + hi[i + (i - 1) * hi_dim1] * zzr;
            hi[i - 1 + j * hi_dim1] = xr * yi - xi * yr + hi[i + (i - 1) * hi_dim1] * zzi;
            hr[i + j * hr_dim1] = xr * zzr - xi * zzi - hi[i + (i - 1) * hi_dim1] * yr;
            hi[i + j * hi_dim1] = xr * zzi + xi * zzr - hi[i + (i - 1) * hi_dim1] * yi;
            /* L490: */
        }
...
L540:
    ...
    for (j = lp1; j <= i__1; ++j)
    {
        ...
        for (i = l; i <= i__2; ++i)
        {
            ...
            yi = hi[i + (j - 1) * hi_dim1];
            hi[i + (j - 1) * hi_dim1] = xr * yi + xi * yr + hi[j + (j - 1) * hi_dim1] * zzi;
        L560:
            hr[i + (j - 1) * hr_dim1] = xr * yr - xi * yi + hi[j + (j - 1) * hi_dim1] * zzr;
            hr[i + j * hr_dim1] = xr * zzr + xi * zzi - hi[j + (j - 1) * hi_dim1] * yr;
            hi[i + j * hi_dim1] = xr * zzi - xi * zzr - hi[j + (j - 1) * hi_dim1] * yi;
            /* L580: */
        }
...
L105:
    ...
    for (ii = 1; ii <= i__1; ++ii)
    {
        ...
        if (hr[i + (i - 1) * hr_dim1] == 0. && hi[i + (i - 1) * hi_dim1] == 0.)
        {
            goto L140;
        }
        /*     .......... norm below is negative of h formed in corth ........
        .. */
        norm = hr[i + (i - 1) * hr_dim1] * ortr[i] + hi[i + (i - 1) * hi_dim1] * orti[i];
        ip1 = i + 1;
        ...
        for (j = i; j <= i__2; ++j)
        {
            ...
            for (k = i; k <= i__3; ++k)
            {
                sr = sr + ortr[k] * zr[k + j * zr_dim1] + orti[k] * zi[k + j * zi_dim1];
                si = si + ortr[k] * zi[k + j * zi_dim1] - orti[k] * zr[k + j * zr_dim1];
                /* L115: */
            }
            ...
            for (k = i; k <= i__3; ++k)
            {
                zr[k + j * zr_dim1] = zr[k + j * zr_dim1] + sr * ortr[k] - si * orti[k];
                zi[k + j * zi_dim1] = zi[k + j * zi_dim1] + sr * orti[k] + si * ortr[k];
                /* L120: */
            }
...
L150:
    ...
    for (i = l; i <= i__1; ++i)
    {
        ...
        for (j = *low; j <= i__2; ++j)
        {
            ...
            zr[j + i * zr_dim1] = yr * zr[j + i * zr_dim1] - yi * zi[j + i * zi_dim1];
            zi[j + i * zi_dim1] = si;
            /* L165: */
        }
...
L540:
    ...
    for (j = lp1; j <= i__1; ++j)
    {
        ...
        for (i = *low; i <= i__2; ++i)
        {
            ...
            zzr = zr[i + j * zr_dim1];
            zzi = zi[i + j * zi_dim1];
            zr[i + (j - 1) * zr_dim1] = xr * yr - xi * yi + hi[j + (j - 1) * hi_dim1] * zzr;
            zi[i + (j - 1) * zi_dim1] = xr * yi + xi * yr + hi[j + (j - 1) * hi_dim1] * zzi;
            zr[i + j * zr_dim1] = xr * zzr + xi * zzi - hi[j + (j - 1) * hi_dim1] * yr;
            zi[i + j * zi_dim1] = xr * zzi - xi * zzr - hi[j + (j - 1) * hi_dim1] * yi;
            /* L590: */
        }
...
L680:
    ...
    for (i = 1; i <= i__1; ++i)
    {
        ...
        for (j = i; j <= i__2; ++j)
        {
            tr = (d__1 = hr[i + j * hr_dim1], abs(d__1)) + (d__2 = hi[i + j * hi_dim1], abs(d__2));
            if (tr > norm)
            {
                norm = tr;
            }
    ...
    for (nn = 2; nn <= i__2; ++nn)
    {
        ...
        for (ii = 1; ii <= i__1; ++ii)
        {
            ...
            for (j = ip1; j <= i__3; ++j)
            {
                zzr = zzr + hr[i + j * hr_dim1] * hr[j + en * hr_dim1] - hi[i + j * hi_dim1] * hi[j + en * hi_dim1];
                zzi = zzi + hr[i + j * hr_dim1] * hi[j + en * hi_dim1] + hi[i + j * hi_dim1] * hr[j + en * hr_dim1];
                /* L740: */
            }
        ...
        L765:
            cdiv_(&zzr, &zzi, &yr, &yi, &hr[i + en * hr_dim1], &hi[i + en * hi_dim1]);
            /*     .......... overflow control .......... */
            tr = (d__1 = hr[i + en * hr_dim1], abs(d__1)) + (d__2 = hi[i + en * hi_dim1], abs(d__2));
            if (tr == 0.)
            {
                goto L780;
            }
    ...
    for (jj = *low; jj <= i__2; ++jj)
    {
        ...
        for (i = *low; i <= i__1; ++i)
        {
            ...
            for (k = *low; k <= i__3; ++k)
            {
                zzr = zzr + zr[i + k * zr_dim1] * hr[k + j * hr_dim1] - zi[i + k * zi_dim1] * hi[k + j * hi_dim1];
                zzi = zzi + zr[i + k * zr_dim1] * hi[k + j * hi_dim1] + zi[i + k * zi_dim1] * hr[k + j * hr_dim1];
                /* L860: */
            }
...
static int corth_(int *nm, int *n, int *low, int *igh, double *ar, double *ai, double *ortr, double *orti)
{
    ...
    for (m = kp1; m <= i__1; ++m)
    {
        ...
        for (i = m; i <= i__2; ++i)
        {
            /* L90: */
            scale =
                scale + (d__1 = ar[i + (m - 1) * ar_dim1], abs(d__1)) + (d__2 = ai[i + (m - 1) * ai_dim1], abs(d__2));
        }
    ...
    L105:
        ...
        for (j = m; j <= i__2; ++j)
        {
            ...
            {
                i = mp - ii;
                fr = fr + ortr[i] * ar[i + j * ar_dim1] + orti[i] * ai[i + j * ai_dim1];
                fi = fi + ortr[i] * ai[i + j * ai_dim1] - orti[i] * ar[i + j * ar_dim1];
                /* L110: */
            }
            ...
            for (i = m; i <= i__3; ++i)
            {
                ar[i + j * ar_dim1] = ar[i + j * ar_dim1] - fr * ortr[i] + fi * orti[i];
                ai[i + j * ai_dim1] = ai[i + j * ai_dim1] - fr * orti[i] - fi * ortr[i];
                /* L120: */
            }
        ...
        for (i = 1; i <= i__2; ++i)
        {
            ...
            {
                j = mp - jj;
                fr = fr + ortr[j] * ar[i + j * ar_dim1] - orti[j] * ai[i + j * ai_dim1];
                fi = fi + ortr[j] * ai[i + j * ai_dim1] + orti[j] * ar[i + j * ar_dim1];
                /* L140: */
            }
            ...
            for (j = m; j <= i__3; ++j)
            {
                ar[i + j * ar_dim1] = ar[i + j * ar_dim1] - fr * ortr[j] - fi * orti[j];
                ai[i + j * ai_dim1] = ai[i + j * ai_dim1] + fr * orti[j] - fi * ortr[j];
                /* L150: */
            }
...
L70:
    ...
    for (ll = *low; ll <= i__1; ++ll)
    {
        ...
        s = (d__1 = h[l - 1 + (l - 1) * h_dim1], abs(d__1)) + (d__2 = h[l + l * h_dim1], abs(d__2));
        if (s == 0.)
        {
            s = norm;
        }
...
L100:
    ...
    s = (d__1 = h[en + na * h_dim1], abs(d__1)) + (d__2 = h[na + enm2 * h_dim1], abs(d__2));
    x = s * .75;
    y = x;
    w = s * -.4375 * s;
L130:
...
L150:
    ...
    L225:
        ...
        for (j = k; j <= i__2; ++j)
        {
            p = h[k + j * h_dim1] + q * h[k + 1 + j * h_dim1] + r * h[k + 2 + j * h_dim1];
            h[k + j * h_dim1] -= p * x;
            h[k + 1 + j * h_dim1] -= p * y;
            h[k + 2 + j * h_dim1] -= p * zz;
            /* L230: */
...
L150:
    ...
    L225:
        ...
        for (i = *low; i <= i__2; ++i)
        {
            p = x * z[i + k * z_dim1] + y * z[i + (k + 1) * z_dim1] + zz * z[i + (k + 2) * z_dim1];
            z[i + k * z_dim1] -= p;
            z[i + (k + 1) * z_dim1] -= p * q;
            z[i + (k + 2) * z_dim1] -= p * r;
            /* L250: */
...
L340:
    ...
    L720:
        ...
        cdiv_(&c_b256, &d__1, &d__2, &q, &h[na + na * h_dim1], &h[na + en * h_dim1]);
    L730:
        h[en + na * h_dim1] = 0.;
        h[en + en * h_dim1] = 1.;
        enm2 = na - 1;
...
static int htribk_(int *nm, int *n, double *ar, double *ai, double *tau, int *m, double *zr, double *zi)
{
    ...
    for (i = 2; i <= i__2; ++i)
    {
        ...
        for (j = 1; j <= i__1; ++j)
        {
            ...
            for (k = 1; k <= i__3; ++k)
            {
                s = s + ar[i + k * ar_dim1] * zr[k + j * zr_dim1] - ai[i + k * ai_dim1] * zi[k + j * zi_dim1];
...
static int htridi_(int *nm, int *n, double *ar, double *ai, double *d, double *e, double *e2, double *tau)
{
    ...
    L140:
        ...
        tau[(l << 1) + 1] = (ai[i + l * ai_dim1] * tau[(i << 1) + 2] - ar[i + l * ar_dim1] * tau[(i << 1) + 1]) / f;
        si = (ar[i + l * ar_dim1] * tau[(i << 1) + 2] + ai[i + l * ai_dim1] * tau[(i << 1) + 1]) / f;
        h += f * g;
        g = g / f + 1.;
    ...
    L170:
        ...
        for (j = 1; j <= i__2; ++j)
        {
            ...
            for (k = 1; k <= i__3; ++k)
            {
                g = g + ar[j + k * ar_dim1] * ar[i + k * ar_dim1] + ai[j + k * ai_dim1] * ai[i + k * ai_dim1];
                gi = gi - ar[j + k * ar_dim1] * ai[i + k * ai_dim1] + ai[j + k * ai_dim1] * ar[i + k * ar_dim1];
                /* L180: */
            }
