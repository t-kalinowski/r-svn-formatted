...
static void Rdqags(IntStruct IS, double *lower, double *upper, double *epsabs, double *epsrel, double *result,
                   double *abserr, int *neval, int *ier, int *limit, int *lenw, int *last, int *iwork, double *work);

static void Rdqagi(IntStruct IS, double *bound, int *inf, double *epsabs, double *epsrel, double *result,
                   double *abserr, int *neval, int *ier, int *limit, int *lenw, int *last, int *iwork, double *work);
...
SEXP call_dqags(SEXP args)
{
    ...
    Rdqags(&is, &lower, &upper, &epsabs, &epsrel, &result, &abserr, &neval, &ier, &limit, &lenw, &last, iwork, work);
...
SEXP call_dqagi(SEXP args)
{
    ...
    Rdqagi(&is, &bound, &inf, &epsabs, &epsrel, &result, &abserr, &neval, &ier, &limit, &lenw, &last, iwork, work);
...
static void rdqagie(IntStruct IS, double *, int *, double *, double *, int *, double *, double *, int *, int *,
                    double *, double *, double *, double *, int *, int *);

static void rdqk15i(IntStruct IS, double *, int *, double *, double *, double *, double *, double *, double *);

static void rdqelg(int *, double *, double *, double *, double *, int *);

static void rdqpsrt(int *, int *, int *, double *, double *, int *, int *);

static void rdqagse(IntStruct IS, double *, double *, double *, double *, int *, double *, double *, int *, int *,
                    double *, double *, double *, double *, int *, int *);
                   ...
                   double *abserr, int *neval, int *ier, int *limit, int *lenw, int *last, int *iwork, double *work)
{
    ...
    *abserr = 0.;
    ...
    rdqagie(IS, bound, inf, epsabs, epsrel, limit, result, abserr, neval, ier, &work[1], &work[l1], &work[l2],
            &work[l3], &iwork[1], last);

    return;
} /* Rdqagi */

static void rdqagie(IntStruct IS, double *bound, int *inf, double *epsabs, double *epsrel, int *limit, double *result,
                    double *abserr, int *neval, int *ier, double *alist__, double *blist, double *rlist, double *elist,
                    int *iord, int *last)
{
    ...
    *abserr = oflow;
    ...
    for (*last = 2; *last <= i__1; ++(*last))
    {
        ...
        if ((d__1 = rlist[maxerr] - area12, fabs(d__1)) > fabs(area12) * 1e-5 || erro12 < errmax * .99)
        {
            goto L10;
        }
        if (extrap)
...
static void Rdqags(IntStruct IS, double *a, double *b, double *epsabs, double *epsrel, double *result, double *abserr,
                   int *neval, int *ier, int *limit, int *lenw, int *last, int *iwork, double *work)
{
    ...
    *abserr = 0.;
    ...
    rdqagse(IS, a, b, epsabs, epsrel, limit, result, abserr, neval, ier, &work[1], &work[l1], &work[l2], &work[l3],
            &iwork[1], last);

    return;
} /* rdqags_ */

static void rdqagse(IntStruct IS, double *a, double *b, double *epsabs, double *epsrel, int *limit, double *result,
                    double *abserr, int *neval, int *ier, double *alist__, double *blist, double *rlist, double *elist,
                    int *iord, int *last)
{
    /* System generated locals */
...
static void rdqk15i(IntStruct IS, double *boun, int *inf, double *a, double *b, double *result, double *abserr,
                    double *resabs, double *resasc)
{
    /* Initialized data */

    static double wg[8] = {0., .129484966168869693270611432679082, 0., .27970539148927666790146777142378,
                           0., .381830050505118944950369775488975, 0., .417959183673469387755102040816327};
    static double xgk[8] = {.991455371120812639206854697526329, .949107912342758524526189684047851,
    ...
    static double wgk[8] = {.02293532201052922496373200805897,  .063092092629978553290700663189204,
    ...
    *resasc = wgk[7] * (d__1 = fc - reskh, fabs(d__1));
    for (j = 1; j <= 7; ++j)
    {
        *resasc += wgk[j - 1] * ((d__1 = fv1[j - 1] - reskh, fabs(d__1)) + (d__2 = fv2[j - 1] - reskh, fabs(d__2)));
        /* L20: */
    }
    *result = resk * hlgth;
    *resasc *= hlgth;
...
L90:
    *abserr = (d__1 = *result - res3la[3], fabs(d__1)) + (d__2 = *result - res3la[2], fabs(d__2)) +
              (d__3 = *result - res3la[1], fabs(d__3));
    res3la[1] = res3la[2];
    res3la[2] = res3la[3];
    res3la[3] = *result;
L100:
...
static void rdqk21(IntStruct IS, double *a, double *b, double *result, double *abserr, double *resabs, double *resasc)
{
    /* Initialized data */

    static double wg[5] = {.066671344308688137593568809893332, .149451349150580593145776339657697,
    ...
    static double wgk[11] = {
        .011694638867371874278064396062192, .03255816230796472747881897245939,  .05475589657435199603138130024458,
        .07503967481091995276704314091619,  .093125454583697605535065465083366, .109387158802297641899210590325805,
        .123491976262065851077958109831074, .134709217311473325928054001771707, .142775938577060080797094273138717,
        .147739104901338491374841515972068, .149445554002916905664936468389821};
