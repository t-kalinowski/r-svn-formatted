...
static void mainlb(int, int, double *, double *, double *, int *, double *, double *, double, double *, double *,
                   ...
                   double *, int *, int *, int *, char *, int, char *, int *);
...
static void prn1lb(int n, int m, double *l, double *u, double *x, int iprint, double epsmch);
static void prn2lb(int n, double *x, double *f, double *g, int iprint, int iter, int nfgv, int nact, double sbgnrm,
                   int nint, char *word, int iword, int iback, double stp, double xstep);
static void prn3lb(int n, double *x, double *f, char *task, int iprint, int info, int iter, int nfgv, int nintol,
                   int nskip, int nact, double sbgnrm, int nint, char *word, int iback, double stp, double xstep,
                   int k);

/* ================    L-BFGS-B (version 2.3)	========================== */
static void setulb(int n, int m, double *x, double *l, double *u, int *nbd, double *f, double *g, double factr,
                   double *pgtol, double *wa, int *iwa, char *task, int iprint, int *isave)
{
    /*     ************
    ...
    */
    ...
    mainlb(n, m, x, l, u, nbd, f, g, factr, pgtol, &wa[lws], &wa[lwy], &wa[lsy], &wa[lss], &wa[lwt], &wa[lwn],
           &wa[lsnd], &wa[lz], &wa[lr], &wa[ld], &wa[lt], &wa[lwa], iwa, &iwa[n], &iwa[n << 1], task, iprint, csave,
           isave);
    return;
} /* setulb */
/* ======================= The end of setulb ============================= */
...
static void mainlb(int n, int m, double *x, double *l, double *u, int *nbd, double *f, double *g, double factr,
                   ...
                   double *z, double *r, double *d, double *t, double *wa, int *indx, int *iwhere, int *indx2,
                   char *task, int iprint, char *csave, int *isave)
{
    /*     ************
           Subroutine mainlb
    ...
    */
    ...
    int ws_offset = 0, wy_offset = 0, sy_offset = 0, ss_offset = 0, wt_offset = 0, wn_offset = 0, snd_offset = 0;
    double d__1, d__2;
    ...
    // in isave
    static int nintol, iback, nskip, head, col, itail, iter, iupdat, nint, nfgv, info, ifun, iword, nfree, nact, ileave,
        nenter;
    // formerly dsave
    static double theta, fold, tol, dnorm, epsmch, gd, stpmx, sbgnrm, stp, gdold, dtd;
    ...
    if (strncmp(task, "START", 5) == 0)
    {
        ...
        {
            prn3lb(n, x + 1, f, task, iprint, info, iter, nfgv, nintol, nskip, nact, sbgnrm, nint, word, iback, stp,
                   xstep, k);
            return;
        }
...
L222:
    ...
    /* ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc */
    cauchy(n, &x[1], &l[1], &u[1], &nbd[1], &g[1], &indx2[1], &iwhere[1], &t[1], &d[1], &z[1], m, &wy[wy_offset],
    ...
    freev(n, &nfree, &indx[1], &nenter, &ileave, &indx2[1], &iwhere[1], &wrk, &updatd, &cnstnd, iprint, &iter);
    nact = n - nfree;
L333:
    ...
    /*		       [ 0  I] */
    if (wrk)
        formk(n, &nfree, &indx[1], &nenter, &ileave, &indx2[1], &iupdat, &updatd, &wn[wn_offset], &snd[snd_offset], m,
    ...
    /*						     from 'cauchy'). */
    cmprlb(n, m, &x[1], &g[1], &ws[ws_offset], &wy[wy_offset], &sy[sy_offset], &wt[wt_offset], &z[1], &r[1], &wa[1],
           &indx[1], &theta, &col, &head, &nfree, &cnstnd, &info);
    if (info != 0)
        goto L444;
    /*	 call the direct method. */
    subsm(n, m, &nfree, &indx[1], &l[1], &u[1], &nbd[1], &z[1], &r[1], &ws[ws_offset], &wy[wy_offset], &theta, &col,
...
L666:
    ...
    else
    {
        ...
        prn2lb(n, x + 1, f, g + 1, iprint, iter, nfgv, nact, sbgnrm, nint, word, iword, iback, stp, xstep);
        goto L1000;
    }
L777:
    ...
    matupd(n, m, &ws[ws_offset], &wy[wy_offset], &sy[sy_offset], &ss[ss_offset], &d[1], &r[1], &itail, &iupdat, &col,
...
L1000:
    ...
    prn3lb(n, x + 1, f, task, iprint, info, iter, nfgv, nintol, nskip, nact, sbgnrm, nint, word, iback, stp, xstep, k);
    return;
} /* mainlb */
/* ======================= The end of mainlb ============================= */
                   ...
                   int *boxed)
{
    ...
    *boxed = TRUE_;
    ...
    if (iprint >= 0)
    {
        ...
        if (!*cnstnd)
            Rprintf("This problem is unconstrained.\n");
    }
    if (iprint > 0)
...
static void cauchy(int n, double *x, double *l, double *u, int *nbd, double *g, int *iorder, int *iwhere, double *t,
                   double *d, double *xcp, int m, double *wy, double *ws, double *sy, double *wt, double *theta,
                   int *col, int *head, double *p, double *c, double *wbp, double *v, int *nint, int iprint,
                   double *sbgnrm, int *info, double *epsmch)
{
    ...
    *nint = 1;
    if (iprint >= 99)
        Rprintf("There are %d  breakpoints\n", nbreak);
...
static void cmprlb(int n, int m, double *x, double *g, double *ws, double *wy, double *sy, double *wt, double *z,
                   double *r, double *wa, int *indx, double *theta, int *col, int *head, int *nfree, int *cnstnd,
                   int *info)
{
    /*	************
    ...
    */
    ...
    /* System generated locals */
    int ws_dim1, ws_offset, wy_dim1, wy_offset, sy_dim1, sy_offset, wt_dim1, wt_offset, Col, n_f;
    ...
    else
    {
        ...
        for (int j = 1; j <= Col; ++j)
        {
            ...
            {
                k = indx[i];
                r[i] += wy[k + pointr * wy_dim1] * a1 + ws[k + pointr * ws_dim1] * a2;
            }
            pointr = pointr % m + 1;
        }
    }
                  ...
                  int *head, int *info)
{
    ...
    */
    ...
    /* System generated locals */
    int wn_dim1, wn_offset, wn1_dim1, wn1_offset, ws_dim1, ws_offset, wy_dim1, wy_offset, sy_dim1, sy_offset;
    ...
    //    int i, k;
    int ipntr, jpntr, k1, m2, dbegin, is, js, iy, pbegin, is1, js1, col2;
    ...
    */
    ...
    for (int i = 2; i <= *col; ++i)
    {
        for (int j = i; j <= *col; ++j)
        {
            ...
            for (k = 1; k <= k1; ++k)
            {
                ddum += sy[i + k * sy_dim1] * sy[j + k * sy_dim1] / sy[k + k * sy_dim1];
            }
...
static void freev(int n, int *nfree, int *indx, int *nenter, int *ileave, int *indx2, int *iwhere, int *wrk,
...
static void matupd(int n, int m, double *ws, double *wy, double *sy, double *ss, double *d, double *r, int *itail,
                   int *iupdat, int *col, int *head, double *theta, double *rr, double *dr, double *stp, double *dtd)
{
    ...
    */
    ...
    /* System generated locals */
    int ws_dim1, ws_offset, wy_dim1, wy_offset, sy_dim1, sy_offset, ss_dim1, ss_offset;
    ...
    *theta = *rr / *dr;
    ...
    for (int j = 1; j <= *col - 1; ++j)
    {
        sy[*col + j * sy_dim1] = F77_CALL(ddot)(&n, &d[1], &c__1, &wy[pointr * wy_dim1 + 1], &c__1);
        ss[j + *col * ss_dim1] = F77_CALL(ddot)(&n, &ws[pointr * ws_dim1 + 1], &c__1, &d[1], &c__1);
        pointr = pointr % m + 1;
    }
    if (*stp == 1.)
    {
...
static void projgr(int n, double *l, double *u, int *nbd, double *x, double *g, double *sbgnrm)
{
    ...
    *sbgnrm = 0.;
    for (int i = 0; i < n; ++i)
    {
        ...
        if (*sbgnrm < (d__1 = fabs(gi)))
            *sbgnrm = d__1;
    }
    return;
} /* projgr */

static void subsm(int n, int m, int *nsub, int *ind, double *l, double *u, int *nbd, double *x, double *d, double *ws,
                  ...
                  int *info)
{
    ...
    */
    ...
    for (int jy = 1; jy <= *col; ++jy)
    {
        ...
        {
            k = ind[i];
            d[i] += (wy[k + pointr * n] * wv[jy] / *theta + ws[k + pointr * n] * wv[js]);
        }
        pointr = pointr % m + 1;
    }
...
static void prn3lb(int n, double *x, double *f, char *task, int iprint, int info, int iter, int nfgv, int nintol,
                   int nskip, int nact, double sbgnrm, int nint, char *word, int iback, double stp, double xstep, int k)
{
    ...
    if (iprint >= 0)
    {
        ...
        {
        case -1:
            Rprintf("Matrix in 1st Cholesky factorization in formk is not Pos. Def.");
            break;
        case -2:
            Rprintf("Matrix in 2st Cholesky factorization in formk is not Pos. Def.");
            break;
        case -3:
            Rprintf("Matrix in the Cholesky factorization in formt is not Pos. Def.");
            break;
        case -4:
            Rprintf("Derivative >= 0, backtracking line search impossible.");
            break;
        case -5:
            Rprintf("l(%d) > u(%d).  No feasible solution", k, k);
            break;
        case -6:
            Rprintf("Input nbd(%d) is invalid", k);
            break;
        case -7:
            Rprintf("Warning:  more than 10 function and gradient evaluations\n   in the last line search\n");
            break;
        case -8:
            Rprintf("The triangular system is singular.");
            break;
        case -9:
