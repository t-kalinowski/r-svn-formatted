...
#define h5 high_u_fact[1]
    ...
    REprintf("pretty(lo=%g,up=%g,ndiv=%d,min_n=%d,shrink=%g,high_u=(%g,%g),eps=%d)"
             ...
             *lo, *up, *ndiv, min_n, shrink_sml, h, h5, eps_correction, dx, (int)i_small, cell);
#endif
    ...
    else
    {
        ...
        { // up - lo = +Inf (overflow; both are finite)
            if (*ndiv < 2)
            {
                warning(_("Internal(pretty()): infinite range; *ndiv=%d, should have ndiv >= 2"), *ndiv);
            }
            else
            {
    ...
    if (cell < 2 * DBL_MIN)
    {
        warning(_("Internal(pretty()): very small range 'cell'=%g, corrected to %g"), cell, 2 * DBL_MIN);
        cell = 2 * DBL_MIN;
    }
    else if (cell > DBL_MAX / 8)
    {
        warning(_("Internal(pretty()): very large range 'cell'=%g, corrected to %g"), cell, DBL_MAX / 8);
        cell = DBL_MAX / 8.;
    }
    ...
    double ns = floor(*lo / unit + rounding_eps), nu = ceil(*up / unit - rounding_eps);
#ifdef DEBUGpr
