...
static long handle_message(HWND hwnd, UINT message, WPARAM wParam, LONG lParam, int *pass)
{
    ...
    case WM_KEYDOWN: /* record state of shift and control keys */
        ...
        if ((obj->flags & UseUnicode) && LOWORD(wParam) == VK_PACKET)
        {
            ...
            result = ToUnicodeEx(VK_PACKET, HIWORD(lParam), sta, wcs, /* 3 */ sizeof(wcs) / sizeof(wchar_t), 1, 0);
            if (result == 1)
            {
        ...
        else if (obj->flags & UseUnicode)
        {
            ...
            GetKeyboardState(sta);
            result = ToUnicodeEx(wParam, lParam, sta, wcs, /* 3 */ sizeof(wcs) / sizeof(wchar_t), 0, dwhkl);
            if (result == 1)
            {
                if (deadkey != L'\0')
                {
                    ...
                    int nchars = FoldStringW(MAP_PRECOMPOSED, wcs_in, 3, wcs_out, 3);
                    if (nchars == 2)
                        handle_char(obj, wcs_out[0]);
                    else if (nchars == 3 && wcs_in[0] == L' ' && wcs_out[0] == wcs_in[0] && wcs_out[1] == wcs_in[1])
        ...
        { /* is fixed multiGAbyte string */
            ...
            if (obj->flags & UseUnicode)
            {
                ...
                for (i = 0; i < (len / sizeof(wchar_t)); i++)
                    if (IsSurrogatePairsHi(p[i]) && i + 1 < (len / sizeof(wchar_t)) && IsSurrogatePairsLo(p[i + 1]))
                    {
                        handle_char(obj, L'?');
                        handle_char(obj, L'?');
                        i++;
 ...
 *  belong to us).
 */
LRESULT WINAPI app_win_proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    long result;
    int pass = 0;
...
LRESULT WINAPI app_doc_proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    long result;
    int pass = 0;
    object obj;
...
LRESULT WINAPI app_work_proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    long result;
    int pass = 0;
    result = handle_message(hwnd, message, wParam, lParam, &pass);
 ...
 */
UINT WINAPI app_timer_procedure(HWND hwnd, UINT message, UINT tid, DWORD time)
{
    object obj;
    UINT id = LOWORD(tid);
...
static int TranslateMenuKeys(MSG *msg)
{
    ...
    if ((GetKeyState(VK_CONTROL) & 0x8000) && (msg->message == WM_KEYDOWN) && !(GetKeyState(VK_RMENU) & 0x8000))
    {
