...
void gcopyalpha(drawing d, drawing d2, rect r, int alpha)
{
        ...
        return;
    {
        ...
        bl.AlphaFormat = 0;
        AlphaBlend(GETHDC(d), r.x, r.y, r.width, r.height, GETHDC(d2), r.x, r.y, r.width, r.height, bl);
    }
}
...
void gcopyalpha2(drawing d, image src, rect r)
{
    ...
    AlphaBlend(GETHDC(d), r.x, r.y, r.width, r.height, GETHDC(bm), 0, 0, r.width, r.height, bl);
    del(bm);
}
...
void gdrawimage(drawing d, image img, rect dr, rect sr)
{
    ...
    BitBlt(dc, dr.x, dr.y, dr.width, dr.height, bc, sr.x, sr.y, SRCCOPY);
...
void gmaskimage(drawing d, image img, rect dr, rect sr, image mask)
{
    ...
    BitBlt(mbwc, sr.x, sr.y, sr.width, sr.height, mbc, sr.x, sr.y, SRCCOPY);

    MaskBlt(dc, dr.x, dr.y, dr.width, dr.height, bc, sr.x, sr.y, (HBITMAP)mbw->handle, 0, 0, MAKEROP4(SRCCOPY, SRCAND));
