...
void gdrawimage(drawing d, image img, rect dr, rect sr)
{
    ...
    BitBlt(dc, dr.x, dr.y, dr.width, dr.height, get_context(b), sr.x, sr.y, SRCCOPY);
...
void gmaskimage(drawing d, image img, rect dr, rect sr, image mask)
{
    ...
    BitBlt(get_context(mbw), sr.x, sr.y, sr.width, sr.height, get_context(mb), sr.x, sr.y, SRCCOPY);

    MaskBlt(dc, dr.x, dr.y, dr.width, dr.height, get_context(b), sr.x, sr.y, (HBITMAP)mbw->handle, 0, 0,
