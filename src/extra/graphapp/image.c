...
static char *base_file_name(const char *filename)
{
    ...
    {
        if ((filename[start] == '\\') || (filename[start] == '/') || (filename[start] == ':'))
        {
            start++;
            break;
        }
...
void drawmonochrome(image src, rect dr, rect sr)
{
    ...
    for (y = 0; y < dest->height; y++)
        for (x = 0; x < dest->width; x++)
            dest->pixels[y * dest->width + x] = get_mono_pixval(src, dest, x, y);
...
void drawgreyscale(image src, rect dr, rect sr)
{
    ...
    for (y = 0; y < dest->height; y++)
        for (x = 0; x < dest->width; x++)
            dest->pixels[y * dest->width + x] = get_grey_pixval(src, dest, x, y);
    drawimage(dest, dr, sr);
    del(dest);
}
...
void drawdarker(image src, rect dr, rect sr)
{
    ...
    else if (src->depth == 32)
    {
        ...
        if (dest)
        {
            ...
            for (y = 0; y < dest->height; y++)
                for (x = 0; x < dest->width; x++)
                    pixels[y * dest->width + x] = darker(get_image_pixel(src, x, y));
        }
        else
        {
            dest = src;
...
void drawbrighter(image src, rect dr, rect sr)
{
    ...
    else if (src->depth == 32)
    {
        ...
        if (dest)
        {
            ...
            for (y = 0; y < dest->height; y++)
                for (x = 0; x < dest->width; x++)
                    pixels[y * dest->width + x] = brighter(get_image_pixel(src, x, y));
        }
        else
        {
            dest = src;
