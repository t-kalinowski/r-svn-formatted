...
static void private_delmenu(menu m)
{
    ...
    {
        DeleteMenu(m->parent->handle, find_menu_position(m->parent, m->text), MF_BYPOSITION);
    }
    else
...
static void set_search_string(char *search, const char *name, int key)
{
    ...
    if (!string_diff(name, "Cut"))
        search[dest++] = 't';
    if (!string_diff(name, "Exit"))
        search[dest++] = 'x';
    if (!string_diff(name, "?"))
        search[dest++] = '?';
