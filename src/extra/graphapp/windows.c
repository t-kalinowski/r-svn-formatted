...
static void fix_sys_menu(HWND hwnd, unsigned long win_style)
{
    ...
    if ((!(win_style & WS_MINIMIZEBOX)) && (!(win_style & WS_MAXIMIZEBOX)))
...
static void make_client_window(HWND hwnd)
{
    ...
    hwndClient = CreateWindow(
        "MDICLIENT", NULL,
        WS_CHILD | WS_CLIPCHILDREN | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL | WS_CLIPSIBLINGS | MDIS_ALLCHILDSTYLES,
        CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, hwnd, 0, this_instance, (void *)&clientcreate);
}
...
static HWND new_mdi_window(const char *name, rect r, unsigned long sty)
{
    ...
    hwnd = (HWND)sendmessage(hwndClient, WM_MDICREATE, 0, (intptr_t)(LPMDICREATESTRUCT)&mdi);
 ...
 */
static void fix_win_style(long *flags_ptr, long *state_ptr, unsigned long *style_ptr)
{
...
static object new_window_object(HWND hwnd, const char *name, rect r, long flags, long state)
{
    ...
    obj = new_object(WindowObject, hwnd, (flags & ChildWindow) ? current_window : NULL);
    if (obj)
    {
        obj->rect = r;
        obj->flags = flags;
...
object find_valid_sibling(object obj)
{
    ...
    do
    {
        if ((obj->kind & ControlObject) && (isenabled(obj)) && (isvisible(obj)))
        {
...
static void select_sibling(object obj)
{
    ...
    {
        if (obj->flags & Document)
            sendmessage(hwndClient, WM_MDIACTIVATE, obj->handle, 0L);
        else
            SetFocus(obj->handle);
    }
}
...
{
    if (w->kind != WindowObject)
        return 0;
    if (w->flags & Document)
        return 0;
    if (w->flags & ChildWindow)
        return 0;
    return 1;
}
