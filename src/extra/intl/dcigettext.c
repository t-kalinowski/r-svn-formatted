...
static char *plural_lookup(struct loaded_l10nfile *domain, unsigned long int n, const char *translation,
                           size_t translation_len) internal_function;

#ifdef IN_LIBGLOCALE
static const char *guess_category_value(int category, const char *categoryname,
                                        const char *localename) internal_function;
#else
...
#define category_to_name(category) _nl_category_names.str + _nl_category_name_idxs[category]
#else
...
#ifdef IN_LIBGLOCALE
    char *gl_dcigettext(const char *domainname, const char *msgid1, const char *msgid2, int plural, unsigned long int n,
                        int category, const char *localename, const char *encoding)
#else
char *DCIGETTEXT(const char *domainname, const char *msgid1, const char *msgid2, int plural, unsigned long int n,
                 int category)
#endif
{
#ifndef HAVE_ALLOCA
           ...
           starting at RETVAL, RETLEN bytes.  */
            ...
            if (foundp == NULL)
            {
                ...
                if (newp != NULL)
                {
                    ...
                    new_domainname = (char *)mempcpy(newp->msgid, msgid1, msgid_len);
...
#ifdef IN_LIBGLOCALE
_nl_find_msg(struct loaded_l10nfile *domain_file, struct binding *domainbinding, const char *encoding,
             const char *msgid, size_t *lengthp)
#else
    _nl_find_msg(struct loaded_l10nfile * domain_file, struct binding * domainbinding, const char *msgid, int convert,
                 size_t *lengthp)
#endif
{
        ...
        while (1)
        {
            ...
            if (nstr < nstrings
                    ? W(domain->must_swap, domain->orig_tab[nstr].length) >= len &&
                          (strcmp(msgid, domain->data + W(domain->must_swap, domain->orig_tab[nstr].offset)) == 0)
       ...
       encoding has already been allocated.  */
        ...
        if (convd == NULL)
        {
...

            {
                ...
                struct converted_domain *new_conversions =
                    (struct converted_domain *)(domain->conversions != NULL
                    ...
                    if (nullentry != NULL)
                    {
                        ...
                        if (charsetstr != NULL)
                        {
                            ...
                            *((char *)mempcpy(charset, charsetstr, len)) = '\0';
                            ...
                            int r = __gconv_open(outcharset, charset, &convd->conv, GCONV_AVOID_NOCONV);
        ...
        )
        {
            ...
            if (convd->conv_tab == NULL &&
                ((convd->conv_tab = (char **)calloc(nstrings + domain->n_sysdep_strings, sizeof(char *))) == NULL))
...
#define INITIAL_BLOCK_SIZE 4080
                ...
                while (1)
                {
                    ...
                    res = __gconv(convd->conv, &inbuf, inbuf + resultlen, &outbuf,
...
static char *internal_function plural_lookup(struct loaded_l10nfile *domain, unsigned long int n,
...
static const char *internal_function category_to_name(int category)
{
    const char *retval;
...
#endif
{
    ...
    const static struct
    {
        const char *const win;
        const char *const unix;
    } Abb2Names[] = {{"ch", "zh_TW"}, {"cz", "cs_CZ"}, {"de", "de_DE"}, {"fr", "fr_FR"},
                     {"ge", "de_DE"}, {"gr", "el_GR"}, {"ic", "is_IS"}, {"it", "it_IT"},
                     {"ja", "ja_JP"}, {"ko", "ko_KO"}, {"no", "nb_NO"}, {"po", "pl_PL"},
                     {"sl", "sk_SK"}, {"sp", "es_ES"}, {"sw", "sv_SE"}, {NULL, ""}};
...
static const char *internal_function get_output_charset(struct binding *domainbinding)
{
