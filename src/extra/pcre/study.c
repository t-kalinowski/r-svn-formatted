...
pcre_extra *pcre_study(const pcre *external_re, int options, const char **errorptr)
{
    ...
    uschar *code = (uschar *)re + sizeof(real_pcre) + (re->name_count * re->name_entry_size);
    ...
    *errorptr = NULL;
    ...
    if (!set_start_bits(code, start_bits, (re->options & PCRE_CASELESS) != 0, (re->options & PCRE_UTF8) != 0,
                        &compile_block))
        return NULL;
