...
/* Fills the POSIX.2 regmatch_t array according to the TNFA tag and match
   endpoint values. */
void tre_fill_pmatch(size_t nmatch, regmatch_t pmatch[], int cflags, const tre_tnfa_t *tnfa, int *tags, int match_eo)
{
    ...
    if (match_eo >= 0 && !(cflags & REG_NOSUB))
    {
        ...
        while (i < tnfa->num_submatches && i < nmatch)
        {
            ...
            DPRINT(("pmatch[%d] = {t%d = %d, t%d = %d}\n", i, submatch_data[i].so_tag, pmatch[i].rm_so,
...
static int tre_match(const tre_tnfa_t *tnfa, const void *string, size_t len, tre_str_type_t type, size_t nmatch,
                     regmatch_t pmatch[], int eflags)
{
    ...
    if (tnfa->have_backrefs || eflags & REG_BACKTRACKING_MATCHER)
    {
        ...
        status = tre_tnfa_run_backtrack(tnfa, string, (int)len, type, tags, eflags, &eo);
    }
#ifdef TRE_APPROX
    ...
    else
    {
        ...
        status = tre_tnfa_run_parallel(tnfa, string, (int)len, type, tags, eflags, &eo);
    }
...
static int tre_match_approx(const tre_tnfa_t *tnfa, const void *string, size_t len, tre_str_type_t type,
