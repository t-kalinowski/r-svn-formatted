...
static void tre_print_reach(const tre_tnfa_t *tnfa, tre_tnfa_approx_reach_t *reach, int pos, int num_tags)
{
    int id;
...
/* Sets the matching parameters in `reach' to the ones defined in the `pa'
   ...
   `default_params'. */
inline static void tre_set_params(tre_tnfa_approx_reach_t *reach, int *pa, regaparams_t default_params)
{
    int value;
...
reg_errcode_t tre_tnfa_run_approx(const tre_tnfa_t *tnfa, const void *string, int len, tre_str_type_t type,
                                  int *match_tags, regamatch_t *match, regaparams_t default_params, int eflags,
                                  int *match_end_ofs)
{
    ...
    DPRINT(("max cost %d, ins %d, del %d, subst %d\n", default_params.max_cost, default_params.cost_ins,
            default_params.cost_del, default_params.cost_subst));
    ...
    while (/*CONSTCOND*/ 1)
    {
       ...
       give a better match than what we already have. */
        for (id = 0; id < tnfa->num_states; id++)
        {
            ...
            DPRINT(("  insert: from %d to %d, cost %d: ", id, id, reach[id].costs[depth][TRE_M_COST]));
       ...
       no more states can be reached with better costs. */
        {
            ...
            while (deque_end != deque_start)
            {
                ...
                for (trans = reach_p->state; trans->state; trans++)
                {
                    ...
                    DPRINT(
                        ("  delete: from %03d to %03d, cost %d (%d): ", id, dest_id, cost0, reach_p->params.max_cost));
                    ...
                    if (reach_next[dest_id].pos == pos &&
                        (cost0 > reach_next[dest_id].costs[0][TRE_M_COST] ||
                         (cost0 == reach_next[dest_id].costs[0][TRE_M_COST] &&
                          (!match_tags ||
                           !tre_tag_order(num_tags, tnfa->tag_directions, tmp_tags, reach_next[dest_id].tags)))))
                    {
                    ...
                    if (trans->state == tnfa->final &&
                        (match_eo < 0 || match_costs[TRE_M_COST] > cost0 ||
                         (match_costs[TRE_M_COST] == cost0 && (num_tags > 0 && tmp_tags[0] <= match_tags[0]))))
                    {
                        DPRINT(("	 setting new match at %d, cost %d\n", pos, cost0));
                        match_eo = pos;
                        memcpy(match_costs, reach_next[dest_id].costs[0], sizeof(match_costs[0]) * TRE_M_LAST);
                        for (i = 0; i < num_tags; i++)
        ...
        for (id = 0; id < tnfa->num_states; id++)
        {
            ...
            for (trans = reach[id].state; trans->state; trans++)
            {
                ...
                if (trans->assertions &&
                    (CHECK_ASSERTIONS(trans->assertions) || CHECK_CHAR_CLASSES(trans, tnfa, eflags)))
                {
               ...
               one with the smallest cost or best tags if costs are equal. */
                ...
                DPRINT(("win %d %d\n", reach_next[dest_id].pos, reach_next[dest_id].costs[0][TRE_M_COST]));
                ...
                if (trans->state == tnfa->final &&
                    (match_eo < 0 || cost0 < match_costs[TRE_M_COST] ||
                     (cost0 == match_costs[TRE_M_COST] && num_tags > 0 && tmp_tags[0] <= match_tags[0])))
                {
                    DPRINT(("    setting new match at %d, cost %d\n", pos, cost0));
                    match_eo = pos;
                    for (i = 0; i < TRE_M_LAST; i++)
