...
reg_errcode_t tre_tnfa_run_backtrack(const tre_tnfa_t *tnfa, const void *string, int len, tre_str_type_t type,
...
retry : {
    ...
    for (trans_i = tnfa->initial; trans_i->state; trans_i++)
    {
        ...
        else
        {
            ...
            BT_STACK_PUSH(pos, str_byte, str_wide, trans_i->state, trans_i->state_id, next_c, tags, mbstate);
            {
    ...
    while (/*CONSTCOND*/ 1)
    {
...
#ifdef TRE_DEBUG
        DPRINT(("%3d:%2lc/%05d | %p ", pos, (tre_cint_t)next_c, (int)next_c, state));
        {
            int i;
            for (i = 0; i < tnfa->num_tags; i++)
        ...
        if (trans_i->state && trans_i->assertions & ASSERT_BACKREF)
        {
            ...
            {
                if (type == STR_USER)
                    result = str_source->compare((unsigned)so, (unsigned)pos, (unsigned)bt_len, str_source->context);
#ifdef TRE_WCHAR
                else if (type == STR_WIDE)
        ...
        {
            DPRINT(("  transition %d-%d (%c-%c) %d to %d\n", trans_i->code_min, trans_i->code_max, trans_i->code_min,
                    trans_i->code_max, trans_i->assertions, trans_i->state_id));
            if (trans_i->code_min <= (tre_cint_t)prev_c && trans_i->code_max >= (tre_cint_t)prev_c)
            {
                if (trans_i->assertions &&
                    (CHECK_ASSERTIONS(trans_i->assertions) || CHECK_CHAR_CLASSES(trans_i, tnfa, eflags)))
                {
