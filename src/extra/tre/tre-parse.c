...
static reg_errcode_t tre_expand_ctype(tre_mem_t mem, tre_ctype_t class, tre_ast_node_t ***items, int *i, int *max_i,
                                      int cflags)
{
    ...
    for (j = 0; (j < 256) && (status == REG_OK); j++)
    {
        ...
        if (tre_isctype(c, class) ||
            ((cflags & REG_ICASE) && (tre_isctype(tre_tolower(c), class) || tre_isctype(tre_toupper(c), class))))
        {
            if (min < 0)
                min = c;
            max = c;
...
static reg_errcode_t tre_parse_bracket(tre_parse_ctx_t *ctx, tre_ast_node_t **result)
{
    ...
    for (j = 0; j < i && status == REG_OK; j++)
    {
        ...
        DPRINT(("item: %d - %d, class %ld, curr_max = %d\n", (int)l->code_min, (int)l->code_max, (long)l->u.class,
                curr_max));
