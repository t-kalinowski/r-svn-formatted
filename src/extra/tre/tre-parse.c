...
static void tre_expand_macro(const tre_char_t *regex, const tre_char_t *regex_end, tre_char_t *buf, size_t buf_len)
{
    int i;
...
static reg_errcode_t tre_new_item(tre_mem_t mem, int min, int max, int *i, int *max_i, tre_ast_node_t ***items)
{
...
static reg_errcode_t tre_expand_ctype(tre_mem_t mem, tre_ctype_t classt, tre_ast_node_t ***items, int *i, int *max_i,
                                      int cflags)
{
    ...
    for (j = 0; (j < 256) && (status == REG_OK); j++)
    {
        ...
        if (tre_isctype(c, classt) ||
            ((cflags & REG_ICASE) && (tre_isctype(tre_tolower(c), classt) || tre_isctype(tre_toupper(c), classt))))
        {
            if (min < 0)
                min = c;
            max = c;
...
static reg_errcode_t tre_parse_bracket_items(tre_parse_ctx_t *ctx, int negate, tre_ctype_t neg_classes[],
                                             int *num_neg_classes, tre_ast_node_t ***items, int *num_items,
                                             int *items_size)
{
        ...
        else
        {
            ...
            else if (re + 1 < ctx->re_end && *re == CHAR_LBRACKET && *(re + 1) == CHAR_COLON)
            {
                ...
                if (endptr != ctx->re_end)
                {
                    ...
                    {
                        status = tre_expand_ctype(ctx->mem, classt, items, &i, &max_i, ctx->cflags);
                        classt = (tre_ctype_t)0;
                        skip = 1;
                    }
                    re = endptr + 2;
...
static reg_errcode_t tre_parse_bound(tre_parse_ctx_t *ctx, tre_ast_node_t **result)
{
    ...
    */
    ...
    DPRINT(("tre_parse_bound: min %d, max %d, costs [%d,%d,%d, total %d], "
            ...
            min, max, cost_ins, cost_del, cost_subst, cost_max, limit_ins, limit_del, limit_subst, limit_err));
    ...
    while (tre_stack_num_objects(stack) > bottom && status == REG_OK)
    {
            ...
            /* If the expression has not ended, parse another piece. */
            {
                ...
                if (!(ctx->cflags & REG_LITERAL))
                {
                    ...
                    if ((ctx->cflags & REG_EXTENDED && c == CHAR_RPAREN && depth > 0) ||
                        (!(ctx->cflags & REG_EXTENDED) && (c == CHAR_BACKSLASH && *(ctx->re + 1) == CHAR_RPAREN)))
                    {
        ...
        case PARSE_POSTFIX:
            ...
            case CHAR_BACKSLASH:
                ...
                if (!(ctx->cflags & REG_EXTENDED) && ctx->re + 1 < ctx->re_end && *(ctx->re + 1) == CHAR_LBRACE)
                {
                    ctx->re++;
                    goto parse_brace;
                }
        ...
        case PARSE_ATOM:
            ...
            case CHAR_LPAREN: /* parenthesized subexpression */
               ...
               to Perls corresponding extensions. */
                ...
                if (ctx->cflags & REG_EXTENDED || (ctx->re > ctx->re_start && *(ctx->re - 1) == CHAR_BACKSLASH))
                {
                    depth++;
            ...
            case CHAR_DOLLAR: /* end of line assertion. */
               ...
               string and before \) is BREs. */
                if (ctx->cflags & REG_EXTENDED ||
                    (ctx->re + 2 < ctx->re_end && *(ctx->re + 1) == CHAR_BACKSLASH && *(ctx->re + 2) == CHAR_RPAREN) ||
                    ctx->re + 1 == ctx->re_end)
                {
               ...
               (which matches an empty string).  */
                if (
                    ...
                    (ctx->re >= ctx->re_end || *ctx->re == CHAR_STAR ||
                     (ctx->cflags & REG_EXTENDED && (*ctx->re == CHAR_PIPE || *ctx->re == CHAR_LBRACE ||
                                                     *ctx->re == CHAR_PLUS || *ctx->re == CHAR_QUESTIONMARK))
                     /* Test for "\)" in BRE mode. */
                     || (!(ctx->cflags & REG_EXTENDED) && ctx->re + 1 < ctx->re_end && *ctx->re == CHAR_BACKSLASH &&
