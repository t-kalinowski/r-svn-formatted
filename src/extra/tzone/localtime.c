...
static time_t time2sub(struct tm *tmp, struct tm *(*funcp)(const time_t *, long, struct tm *), long offset, int *okayp,
                       int do_norm_secs);
...
static int tzload(const char *name, struct state *const sp, const int doextend)
{
    ...
    sp->goback = sp->goback && typesequiv(sp, sp->types[i], sp->types[0]) && differ_by_repeat(sp->ats[i], sp->ats[0]);
    sp->goahead = sp->goahead && typesequiv(sp, sp->types[sp->timecnt - 1], sp->types[sp->timecnt - 1 - i]) &&
                  differ_by_repeat(sp->ats[sp->timecnt - 1], sp->ats[sp->timecnt - 1 - i]);
    return 0;
}
...
static struct tm *timesub(const time_t *const timep, const long offset, const struct state *const sp,
                          struct tm *const tmp)
{
...
static int long_normalize_overflow(long *const tensptr, int *const unitsptr, const int base)
{
    int tensdelta;
...
static time_t time2sub(struct tm *const tmp, struct tm *(*const funcp)(const time_t *, long, struct tm *),
...
static time_t time2(struct tm *const tmp, struct tm *(*const funcp)(const time_t *, long, struct tm *),
...
static time_t time1(struct tm *const tmp, struct tm *(*const funcp)(const time_t *, long, struct tm *),
                    const long offset)
{
    time_t t;
    const struct state *sp;
