...
} TZtable[] = {{"Afghanistan Standard Time", "Asia/Kabul"},
...
static const char *reg2Olson(const char *s)
{
    ...
    {
        size = 1000;
        rc = RegQueryValueEx(hkey, "TimeZoneKeyName", NULL, &type, regname, &size);
        RegCloseKey(hkey);
        if (rc == ERROR_SUCCESS)
        {
            for (i = 0; TZtable[i].reg; i++)
        ...
        */
        for (i = 0; TZtable[i].reg; i++)
        {
            ...
            */
            ...
            rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, keyname, 0, KEY_QUERY_VALUE, &hkey);
            if (rc != ERROR_SUCCESS)
                continue;
            size = 1000;
