...
const static struct
{
    ...
    const char *const Olson;
} TZtable[] = {{L"Afghanistan Standard Time", "Asia/Kabul"},
               ...
               {L"Argentina Standard Time", "America/Buenos_Aires"} {L"Bahia Standard Time", "America/Bahia"} {
                   L"Belarus Standard Time", "Europe/Minsk"} {L"Kaliningrad Standard Time", "Europe/Kaliningrad"},
               {L"Kamchatka Standard Time", "Asia/Kamchatka"} {L"Libya Standard Time", "Africa/Tripoli"} {
                   L"Morocco Standard Time", "Africa/Casablanca"} {L"Syria Standard Time", "Asia/Damascus"} {
                   L"Ulaanbaatar Standard Time", "Asia/Ulaanbaatar"} {L"Russia Time Zone 3", "Europe/Samara"} {
                   L"Russia Time Zone 10", "Asia/Srednekolymsk"} {L"Russia Time Zone 11", "Asia/Kamchatka"}
...
static wchar_t basekey[] = L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Time Zones\\";
...
static const char *reg2Olson(const wchar_t *s)
{
    ...
    for (i = 0; TZtable[i].reg; i++)
        if (!wcscmp(s, TZtable[i].reg))
            return TZtable[i].Olson;

    /* On Vista, the English name of the current timezone is in the registry */
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, tzikey, 0, KEY_QUERY_VALUE, &hkey) == ERROR_SUCCESS)
    {
        size = 1000;
        rc = RegQueryValueExW(hkey, L"TimeZoneKeyName", NULL, &type, regdata, &size);
        RegCloseKey(hkey);
        if (rc == ERROR_SUCCESS)
        {
            for (i = 0; TZtable[i].reg; i++)
        ...
        */
        for (i = 0; TZtable[i].reg; i++)
        {
            ...
            */
            ...
            rc = RegOpenKeyExW(HKEY_LOCAL_MACHINE, keyname, 0, KEY_QUERY_VALUE, &hkey);
            if (rc != ERROR_SUCCESS)
                continue;
            size = 1000;
