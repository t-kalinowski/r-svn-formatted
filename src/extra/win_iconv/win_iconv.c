...
static int load_mlang(void)
{
    ...
    ConvertINetMultiByteToUnicode =
        (CONVERTINETMULTIBYTETOUNICODE)GetProcAddress(h, TEXT("ConvertINetMultiByteToUnicode"));
    ConvertINetUnicodeToMultiByte =
        (CONVERTINETUNICODETOMULTIBYTE)GetProcAddress(h, TEXT("ConvertINetUnicodeToMultiByte"));
    IsConvertINetStringAvailable =
        (ISCONVERTINETSTRINGAVAILABLE)GetProcAddress(h, TEXT("IsConvertINetStringAvailable"));
...
static int make_csconv(const char *_name, csconv_t *cv)
{
    ...
    else if (IsValidCodePage(cv->codepage) && GetCPInfo(cv->codepage, &cpinfo) != 0)
    {
        cv->mbtowc = kernel_mbtowc;
        cv->wctomb = kernel_wctomb;
