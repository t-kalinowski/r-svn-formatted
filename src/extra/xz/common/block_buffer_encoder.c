...
static lzma_ret block_encode_uncompressed(lzma_block *block, const uint8_t *in, size_t in_size, uint8_t *out,
...
static lzma_ret block_encode_normal(lzma_block *block, lzma_allocator *allocator, const uint8_t *in, size_t in_size,
...
extern LZMA_API(lzma_ret) lzma_block_buffer_encode(lzma_block *block, lzma_allocator *allocator, const uint8_t *in,
                                                   size_t in_size, uint8_t *out, size_t *out_pos, size_t out_size)
{
    // Sanity checks
    if (block == NULL || block->filters == NULL || (in == NULL && in_size != 0) || out == NULL || out_pos == NULL ||
        *out_pos > out_size)
        return LZMA_PROG_ERROR;
    ...
    const lzma_ret ret = block_encode_normal(block, allocator, in, in_size, out, out_pos, out_size);
    if (ret != LZMA_OK)
    {
