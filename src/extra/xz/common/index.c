...
extern LZMA_API(lzma_vli) lzma_index_memusage(lzma_vli count)
{
...
extern LZMA_API(lzma_index *) lzma_index_init(lzma_index *i, lzma_allocator *allocator)
{
    if (i == NULL)
    {
...
extern LZMA_API(lzma_vli) lzma_index_stream_size(const lzma_index *i)
{
    ...
    return LZMA_STREAM_HEADER_SIZE + i->total_size + index_size(i->count, i->index_list_size) + LZMA_STREAM_HEADER_SIZE;
}
...
extern LZMA_API(lzma_vli) lzma_index_file_size(const lzma_index *i)
{
    ...
    return i->old.streams_size + LZMA_STREAM_HEADER_SIZE + i->total_size +
           index_size(i->count - i->old.count, i->index_list_size - i->old.index_list_size) + LZMA_STREAM_HEADER_SIZE;
}
...
{
    return (LZMA_VLI_C(4) - index_size_unpadded(i->count, i->index_list_size)) & 3;
}

/// Appends a new Record to the Index. If needed, this allocates a new
/// Record group.
static lzma_ret index_append_real(lzma_index *i, lzma_allocator *allocator, lzma_vli unpadded_size,
                                  lzma_vli uncompressed_size, bool is_padding)
{
    ...
    if (i->tail == NULL || i->tail->last == INDEX_GROUP_SIZE - 1)
    {
        ...
        lzma_index_group *g = lzma_alloc(sizeof(lzma_index_group), allocator);
        if (g == NULL)
    ...
    else
    {
        ...
        i->tail->unpadded_sums[i->tail->last + 1] = unpadded_size + vli_ceil4(i->tail->unpadded_sums[i->tail->last]);
        i->tail->uncompressed_sums[i->tail->last + 1] = i->tail->uncompressed_sums[i->tail->last] + uncompressed_size;
...
{
    if (unpadded_size < UNPADDED_SIZE_MIN || unpadded_size > UNPADDED_SIZE_MAX || uncompressed_size > LZMA_VLI_MAX)
    ...
    const lzma_vli index_list_size_add = lzma_vli_size(unpadded_size) + lzma_vli_size(uncompressed_size);
    ...
    if (i->total_size > LZMA_VLI_MAX || i->uncompressed_size > LZMA_VLI_MAX ||
        lzma_index_size(i) > LZMA_BACKWARD_SIZE_MAX || lzma_index_file_size(i) > LZMA_VLI_MAX)
        ret = LZMA_DATA_ERROR; // Would grow past the limits.
    else
        ret = index_append_real(i, allocator, unpadded_size, uncompressed_size, false);
...
static void previous_group(lzma_index *i)
{
    ...
    i->current.stream_offset -= vli_ceil4(i->current.group->unpadded_sums[i->current.group->last]);
    i->current.uncompressed_offset -= i->current.group->uncompressed_sums[i->current.group->last];
...
static void set_info(const lzma_index *i, lzma_index_record *info)
{
    ...
    // current group.
    info->unpadded_size = i->current.group->unpadded_sums[i->current.record];
    info->total_size = vli_ceil4(info->unpadded_size);
    info->uncompressed_size = i->current.group->uncompressed_sums[i->current.record];
    ...
    if (i->current.record > 0)
    {
        ...
        const lzma_vli total_sum = vli_ceil4(i->current.group->unpadded_sums[i->current.record - 1]);

        const lzma_vli uncompressed_sum = i->current.group->uncompressed_sums[i->current.record - 1];
...
extern LZMA_API(lzma_bool) lzma_index_read(lzma_index *i, lzma_index_record *info)
{
...
extern LZMA_API(lzma_bool) lzma_index_locate(lzma_index *i, lzma_index_record *info, lzma_vli target)
{
    ...
    while (i->current.uncompressed_offset <= target)
    {
        ...
        if (i->current.uncompressed_offset + i->current.group->uncompressed_sums[i->current.group->last] > target)
            break;
    ...
    if (i->current.record == 0)
        ...
        assert(i->current.group->uncompressed_sums[i->current.record] -
                   i->current.group->uncompressed_sums[i->current.record - 1] >
               0);
#endif
...
extern LZMA_API(lzma_ret)
    lzma_index_cat(lzma_index *restrict dest, lzma_index *restrict src, lzma_allocator *allocator, lzma_vli padding)
{
    if (dest == NULL || src == NULL || dest == src || padding > LZMA_VLI_MAX)
        return LZMA_PROG_ERROR;

    // Check that the combined size of the Indexes stays within limits.
    {
        const lzma_vli dest_size = index_size_unpadded(dest->count, dest->index_list_size);
        const lzma_vli src_size = index_size_unpadded(src->count, src->index_list_size);
    ...
    // encoded sizes) stays within limits.
    {
        ...
        if (dest_size + src_size > LZMA_VLI_MAX || dest_size + src_size + padding > LZMA_VLI_MAX)
            return LZMA_DATA_ERROR;
    }
    ...
    padding += index_size(dest->count - dest->old.count, dest->index_list_size - dest->old.index_list_size) +
    ...
    // Add the padding Record.
    {
        ...
        if (dest->old.streams_size > LZMA_VLI_MAX || lzma_index_file_size(dest) > LZMA_VLI_MAX)
            ret = LZMA_DATA_ERROR; // Would grow past the limits.
        else
            ret = index_append_real(dest, allocator, padding, 0, true);
    ...
    if (src->head != NULL && src->head->last + 1 <= INDEX_GROUP_SIZE - dest->tail->last - 1)
    {
        // Copy the first Record.
        dest->tail->unpadded_sums[dest->tail->last + 1] =
            vli_ceil4(dest->tail->unpadded_sums[dest->tail->last]) + src->head->unpadded_sums[0];

        dest->tail->uncompressed_sums[dest->tail->last + 1] =
            dest->tail->uncompressed_sums[dest->tail->last] + src->head->uncompressed_sums[0];

        dest->tail->paddings[dest->tail->last + 1] = src->head->paddings[0];
        ...
        {
            dest->tail->unpadded_sums[dest->tail->last + 1] = vli_ceil4(dest->tail->unpadded_sums[dest->tail->last]) +
                                                              src->head->unpadded_sums[i + 1] -
                                                              src->head->unpadded_sums[i];

            dest->tail->uncompressed_sums[dest->tail->last + 1] = dest->tail->uncompressed_sums[dest->tail->last] +
                                                                  src->head->uncompressed_sums[i + 1] -
                                                                  src->head->uncompressed_sums[i];

            dest->tail->paddings[dest->tail->last + 1] = src->head->paddings[i + 1];
    ...
    dest->old.index_list_size = dest->index_list_size + src->old.index_list_size;
...
extern LZMA_API(lzma_index *) lzma_index_dup(const lzma_index *src, lzma_allocator *allocator)
{
    ...
    *dest = *src;
    ...
    {
        // Allocate a new group.
        lzma_index_group *dest_group = lzma_alloc(sizeof(lzma_index_group), allocator);
        if (dest_group == NULL)
        {
        ...
        memcpy(dest_group->unpadded_sums, src_group->unpadded_sums, sizeof(lzma_vli) * count);
        memcpy(dest_group->uncompressed_sums, src_group->uncompressed_sums, sizeof(lzma_vli) * count);
        memcpy(dest_group->paddings, src_group->paddings, sizeof(bool) * count);
...
extern LZMA_API(lzma_bool) lzma_index_equal(const lzma_index *a, const lzma_index *b)
{
    ...
    if (a->total_size != b->total_size || a->uncompressed_size != b->uncompressed_size ||
        a->index_list_size != b->index_list_size || a->count != b->count)
        return false;
    ...
    while (ag != NULL && bg != NULL)
    {
        ...
        if (ag->last != bg->last || memcmp(ag->unpadded_sums, bg->unpadded_sums, sizeof(lzma_vli) * count) != 0 ||
            memcmp(ag->uncompressed_sums, bg->uncompressed_sums, sizeof(lzma_vli) * count) != 0 ||
            memcmp(ag->paddings, bg->paddings, sizeof(bool) * count) != 0)
            return false;
