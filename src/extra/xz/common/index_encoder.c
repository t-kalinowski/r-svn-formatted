                             ...
                             lzma_action action lzma_attribute((unused)))
{
    ...
    while (*out_pos < out_size)
        ...
        case SEQ_COUNT: {
            ...
            ret = lzma_vli_encode(index_count, &coder->pos, out, out_pos, out_size);
        ...
        case SEQ_NEXT:
            ...
            if (coder->record.unpadded_size < UNPADDED_SIZE_MIN || coder->record.unpadded_size > UNPADDED_SIZE_MAX)
        ...
        case SEQ_UNCOMPRESSED: {
            const lzma_vli size =
                coder->sequence == SEQ_UNPADDED ? coder->record.unpadded_size : coder->record.uncompressed_size;
...
extern lzma_ret lzma_index_encoder_init(lzma_next_coder *next, lzma_allocator *allocator, lzma_index *i)
{
...
extern LZMA_API(lzma_ret) lzma_index_buffer_encode(lzma_index *i, uint8_t *out, size_t *out_pos, size_t out_size)
{
