...
extern LZMA_API(lzma_index_hash *) lzma_index_hash_init(lzma_index_hash *index_hash, lzma_allocator *allocator)
{
    if (index_hash == NULL)
    {
...
extern LZMA_API(void) lzma_index_hash_end(lzma_index_hash *index_hash, lzma_allocator *allocator)
{
    ...
    lzma_index_hash_append(lzma_index_hash *index_hash, lzma_vli unpadded_size, lzma_vli uncompressed_size)
{
    ...
    if (index_hash->blocks.blocks_size > LZMA_VLI_MAX || index_hash->blocks.uncompressed_size > LZMA_VLI_MAX ||
        ...
        index_stream_size(index_hash->blocks.blocks_size, index_hash->blocks.count,
    ...
    lzma_index_hash_decode(lzma_index_hash *index_hash, const uint8_t *in, size_t *in_pos, size_t in_size)
{
    ...
    while (*in_pos < in_size)
        ...
        case SEQ_PADDING_INIT:
            index_hash->pos =
                (LZMA_VLI_C(4) - index_size_unpadded(index_hash->records.count, index_hash->records.index_list_size)) &
                3;
        ...
        case SEQ_PADDING:
            ...
            if (memcmp(index_hash->blocks.check.buffer.u8, index_hash->records.check.buffer.u8,
