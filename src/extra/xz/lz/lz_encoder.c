...
///
static lzma_ret fill_window(lzma_coder *coder, lzma_allocator *allocator, const uint8_t *in, size_t *in_pos,
                            size_t in_size, lzma_action action)
{
    ...
    if (coder->next.code == NULL)
    {
        ...
        in_used = lzma_bufcpy(in, in_pos, in_size, coder->mf.buffer, &write_pos, coder->mf.size);
    ...
    else
    {
        ...
        ret = coder->next.code(coder->next.coder, allocator, in, in_pos, in_size, coder->mf.buffer, &write_pos,
...
static lzma_ret lz_encode(lzma_coder *coder, lzma_allocator *allocator, const uint8_t *restrict in,
                          size_t *restrict in_pos, size_t in_size, uint8_t *restrict out, size_t *restrict out_pos,
...
static bool lz_encoder_init(lzma_mf *mf, lzma_allocator *allocator, const lzma_lz_options *lz_options)
{
    ...
    if (mf->hash == NULL)
    {
        mf->hash = lzma_alloc(alloc_count * sizeof(uint32_t), allocator);
        if (mf->hash == NULL)
            return true;
    }
...
extern uint64_t lzma_lz_encoder_memusage(const lzma_lz_options *lz_options)
{
    ...
    return (uint64_t)(mf.hash_size_sum + mf.sons_count) * sizeof(uint32_t) + (uint64_t)(mf.size) + sizeof(lzma_coder);
}
...
extern lzma_ret lzma_lz_encoder_init(lzma_next_coder *next, lzma_allocator *allocator, const lzma_filter_info *filters,
                                     lzma_ret (*lz_init)(lzma_lz_encoder *lz, lzma_allocator *allocator,
                                                         const void *options, lzma_lz_options *lz_options))
{
    ...
    return_if_error(lz_init(&next->coder->lz, allocator, filters[0].options, &lz_options));
