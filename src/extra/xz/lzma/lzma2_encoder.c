...
static void lzma2_header_lzma(lzma_coder *coder)
{
...
static void lzma2_header_uncompressed(lzma_coder *coder)
{
    assert(coder->uncompressed_size > 0);
...
static lzma_ret lzma2_encode(lzma_coder *restrict coder, lzma_mf *restrict mf, uint8_t *restrict out,
                             size_t *restrict out_pos, size_t out_size)
{
    while (*out_pos < out_size)
        ...
        case SEQ_LZMA_ENCODE: {
            ...
            const lzma_ret ret = lzma_lzma_encode(coder->lzma, mf, coder->buf + LZMA2_HEADER_MAX,
...
static void lzma2_encoder_end(lzma_coder *coder, lzma_allocator *allocator)
{
...
static lzma_ret lzma2_encoder_options_update(lzma_coder *coder, const lzma_filter *filter)
{
    ...
    if (coder->opt_cur.lc != opt->lc || coder->opt_cur.lp != opt->lp || coder->opt_cur.pb != opt->pb)
    {
        ...
        if (opt->lc > LZMA_LCLP_MAX || opt->lp > LZMA_LCLP_MAX || opt->lc + opt->lp > LZMA_LCLP_MAX ||
            opt->pb > LZMA_PB_MAX)
...
static lzma_ret lzma2_encoder_init(lzma_lz_encoder *lz, lzma_allocator *allocator, const void *options,
                                   lzma_lz_options *lz_options)
{
    ...
    lz->coder->need_dictionary_reset =
        lz->coder->opt_cur.preset_dict == NULL || lz->coder->opt_cur.preset_dict_size == 0;

    // Initialize LZMA encoder
    return_if_error(lzma_lzma_encoder_create(&lz->coder->lzma, allocator, &lz->coder->opt_cur, lz_options));
...
extern lzma_ret lzma_lzma2_encoder_init(lzma_next_coder *next, lzma_allocator *allocator,
