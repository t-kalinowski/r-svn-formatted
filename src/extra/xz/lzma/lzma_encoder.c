...
static inline void literal(lzma_coder *coder, lzma_mf *mf, uint32_t position)
{
    // Locate the literal byte to be encoded and the subcoder.
    const uint8_t cur_byte = mf->buffer[mf->read_pos - mf->read_ahead];
    probability *subcoder = literal_subcoder(coder->literal, coder->literal_context_bits, coder->literal_pos_mask,
...
static inline void length(lzma_range_encoder *rc, lzma_length_encoder *lc, const uint32_t pos_state, uint32_t len,
...
static inline void match(lzma_coder *coder, const uint32_t pos_state, const uint32_t distance, const uint32_t len)
{
    ...
    rc_bittree(&coder->rc, coder->pos_slot[len_to_pos_state], POS_SLOT_BITS, pos_slot);
    ...
    if (pos_slot >= START_POS_MODEL_INDEX)
    {
        ...
        if (pos_slot < END_POS_MODEL_INDEX)
        {
            ...
            rc_bittree_reverse(&coder->rc, coder->pos_special + base - pos_slot - 1, footer_bits, pos_reduced);
        }
        else
        {
            rc_direct(&coder->rc, pos_reduced >> ALIGN_BITS, footer_bits - ALIGN_BITS);
            rc_bittree_reverse(&coder->rc, coder->pos_align, ALIGN_BITS, pos_reduced & ALIGN_MASK);
...
static inline void rep_match(lzma_coder *coder, const uint32_t pos_state, const uint32_t rep, const uint32_t len)
{
    if (rep == 0)
    {
...
extern lzma_ret lzma_lzma_encode(lzma_coder *restrict coder, lzma_mf *restrict mf, uint8_t *restrict out,
                                 size_t *restrict out_pos, size_t out_size, uint32_t limit)
{
...
static lzma_ret lzma_encode(lzma_coder *restrict coder, lzma_mf *restrict mf, uint8_t *restrict out,
...
extern lzma_ret lzma_lzma_encoder_reset(lzma_coder *coder, const lzma_options_lzma *options)
{
...
extern lzma_ret lzma_lzma_encoder_create(lzma_coder **coder_ptr, lzma_allocator *allocator,
...
static lzma_ret lzma_encoder_init(lzma_lz_encoder *lz, lzma_allocator *allocator, const void *options,
...
extern lzma_ret lzma_lzma_encoder_init(lzma_next_coder *next, lzma_allocator *allocator,
