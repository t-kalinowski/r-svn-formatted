...
extern void lzma_lzma_optimum_fast(lzma_coder *restrict coder, lzma_mf *restrict mf, uint32_t *restrict back_res,
                                   uint32_t *restrict len_res)
{
    ...
    for (uint32_t i = 0; i < REP_DISTANCES; ++i)
    {
        ...
        uint32_t len;
        for (len = 2; len < buf_avail && buf[len] == buf_back[len]; ++len)
            ;
    ...
    for (uint32_t i = 0; i < REP_DISTANCES; ++i)
    {
        ...
        uint32_t len;
        for (len = 2; len < limit && buf[len] == buf_back[len]; ++len)
            ;
