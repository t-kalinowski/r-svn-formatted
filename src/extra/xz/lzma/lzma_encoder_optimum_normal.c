...
static uint32_t get_literal_price(const lzma_coder *const coder, const uint32_t pos, const uint32_t prev_byte,
...
static inline uint32_t get_len_price(const lzma_length_encoder *const lencoder, const uint32_t len,
...
static inline uint32_t get_short_rep_price(const lzma_coder *const coder, const lzma_lzma_state state,
                                           const uint32_t pos_state)
{
    return rc_bit_0_price(coder->is_rep0[state]) + rc_bit_0_price(coder->is_rep0_long[state][pos_state]);
}

static inline uint32_t get_pure_rep_price(const lzma_coder *const coder, const uint32_t rep_index,
...
static inline uint32_t get_rep_price(const lzma_coder *const coder, const uint32_t rep_index, const uint32_t len,
...
static inline uint32_t get_pos_len_price(const lzma_coder *const coder, const uint32_t pos, const uint32_t len,
                                         const uint32_t pos_state)
{
    ...
    else
    {
        ...
        price = coder->pos_slot_prices[len_to_pos_state][pos_slot] + coder->align_prices[pos & ALIGN_MASK];
    }
...
{
    for (uint32_t len_to_pos_state = 0; len_to_pos_state < LEN_TO_POS_STATES; ++len_to_pos_state)
    {

        uint32_t *const pos_slot_prices = coder->pos_slot_prices[len_to_pos_state];

        // Price to encode the pos_slot.
        for (uint32_t pos_slot = 0; pos_slot < coder->dist_table_size; ++pos_slot)
            pos_slot_prices[pos_slot] = rc_bittree_price(coder->pos_slot[len_to_pos_state], POS_SLOT_BITS, pos_slot);

        // For matches with distance >= FULL_DISTANCES, add the price
        // of the direct bits part of the match distance. (Align bits
        // are handled by fill_align_prices()).
        for (uint32_t pos_slot = END_POS_MODEL_INDEX; pos_slot < coder->dist_table_size; ++pos_slot)
            pos_slot_prices[pos_slot] += rc_direct_price(((pos_slot >> 1) - 1) - ALIGN_BITS);

        // Distances in the range [0, 3] are fully encoded with
        // pos_slot, so they are used for coder->distances_prices
        // as is.
        for (uint32_t i = 0; i < START_POS_MODEL_INDEX; ++i)
            coder->distances_prices[len_to_pos_state][i] = pos_slot_prices[i];
    }
    ...
    for (uint32_t i = START_POS_MODEL_INDEX; i < FULL_DISTANCES; ++i)
    {
        ...
        const uint32_t price =
            rc_bittree_reverse_price(coder->pos_special + base - pos_slot - 1, footer_bits, i - base);

        for (uint32_t len_to_pos_state = 0; len_to_pos_state < LEN_TO_POS_STATES; ++len_to_pos_state)
            coder->distances_prices[len_to_pos_state][i] = price + coder->pos_slot_prices[len_to_pos_state][pos_slot];
    }
...
static void fill_align_prices(lzma_coder *coder)
{
    for (uint32_t i = 0; i < ALIGN_TABLE_SIZE; ++i)
        coder->align_prices[i] = rc_bittree_reverse_price(coder->pos_align, ALIGN_BITS, i);
...
static inline uint32_t helper1(lzma_coder *restrict coder, lzma_mf *restrict mf, uint32_t *restrict back_res,
                               uint32_t *restrict len_res, uint32_t position)
{
    ...
    {
        *back_res = coder->matches[matches_count - 1].dist + REP_DISTANCES;
        *len_res = len_main;
    ...
    if (len <= len_main)
    {
        ...
        for (;; ++len)
        {
            ...
            const uint32_t cur_and_len_price = normal_match_price + get_pos_len_price(coder, dist, len, pos_state);

            if (cur_and_len_price < coder->opts[len].price)
            {
                coder->opts[len].price = cur_and_len_price;
                coder->opts[len].pos_prev = 0;
                coder->opts[len].back_prev = dist + REP_DISTANCES;
...
static inline uint32_t helper2(lzma_coder *coder, uint32_t *reps, const uint8_t *buf, uint32_t len_end,
                               uint32_t position, const uint32_t cur, const uint32_t nice_len,
                               const uint32_t buf_avail_full)
{
    ...
    if (new_len >= start_len)
    {
        ...
        for (uint32_t len_test = start_len;; ++len_test)
        {
            ...
            uint32_t cur_and_len_price = normal_match_price + get_pos_len_price(coder, cur_back, len_test, pos_state);

            if (cur_and_len_price < coder->opts[cur + len_test].price)
            {
                coder->opts[cur + len_test].price = cur_and_len_price;
                coder->opts[cur + len_test].pos_prev = cur;
                coder->opts[cur + len_test].back_prev = cur_back + REP_DISTANCES;
                coder->opts[cur + len_test].prev_1_is_literal = false;
            }
            ...
            if (len_test == coder->matches[i].len)
            {
                ...
                if (len_test_2 >= 2)
                {
                    ...
                    if (cur_and_len_price < coder->opts[offset].price)
                    {
                        ...
                        coder->opts[offset].prev_2 = true;
                        coder->opts[offset].pos_prev_2 = cur;
                        coder->opts[offset].back_prev_2 = cur_back + REP_DISTANCES;
                    }
                    //}
                }
...
extern void lzma_lzma_optimum_normal(lzma_coder *restrict coder, lzma_mf *restrict mf, uint32_t *restrict back_res,
                                     uint32_t *restrict len_res, uint32_t position)
{
