...
static lzma_ret copy_or_code(lzma_coder *coder, lzma_allocator *allocator, const uint8_t *restrict in,
...
static lzma_ret simple_code(lzma_coder *coder, lzma_allocator *allocator, const uint8_t *restrict in,
                            size_t *restrict in_pos, size_t in_size, uint8_t *restrict out, size_t *restrict out_pos,
                            size_t out_size, lzma_action action)
{
    ...
    {
        {
            const lzma_ret ret = copy_or_code(coder, allocator, in, in_pos, in_size, coder->buffer, &coder->size,
                                       ...
                                       size_t simple_size, size_t unfiltered_max, uint32_t alignment, bool is_encoder)
{
    ...
    if (next->coder == NULL)
    {
        ...
        next->coder = lzma_alloc(sizeof(lzma_coder) + 2 * unfiltered_max, allocator);
        if (next->coder == NULL)
        ...
        if (simple_size > 0)
        {
            next->coder->simple = lzma_alloc(simple_size, allocator);
