...
static bool subblock_align(lzma_coder *coder, uint8_t *restrict out, size_t *restrict out_pos, size_t out_size,
                           size_t chunk_size, uint32_t skew)
{
    ...
    {
        const uint32_t target = coder->alignment.in_pos % coder->alignment.multiple;

        while ((coder->alignment.out_pos + skew) % coder->alignment.multiple != target)
        {
...
///
static bool is_repeating(const uint8_t *restrict needle, size_t needle_size, const uint8_t *restrict buf,
                         size_t buf_chunks)
{
    while (buf_chunks-- != 0)
    {
...
static lzma_ret subblock_data_size(lzma_coder *coder, lzma_allocator *allocator, size_t new_limit)
{
    ...
    if (new_limit < LZMA_SUBBLOCK_DATA_SIZE_MIN || new_limit > LZMA_SUBBLOCK_DATA_SIZE_MAX)
...
static lzma_ret subblock_buffer(lzma_coder *coder, lzma_allocator *allocator, const uint8_t *restrict in,
                                size_t *restrict in_pos, size_t in_size, uint8_t *restrict out,
                                size_t *restrict out_pos, size_t out_size, lzma_action action)
{
    // Changing allow_subfilter is not allowed.
    if (coder->options != NULL && coder->subfilter.allow != coder->options->allow_subfilters)
    ...
    if (coder->subfilter.allow)
    {
        ...
        case LZMA_SUBFILTER_SET:
            if (coder->subfilter.mode_locked || coder->subfilter.mode != SUB_NONE)
                return LZMA_PROG_ERROR;
    ...
    while (*out_pos < out_size)
        ...
        case SEQ_FILL:
            ...
            if (coder->subblock.size == 0 && coder->options != NULL &&
                coder->options->subblock_data_size != coder->subblock.limit)
                return_if_error(subblock_data_size(coder, allocator, coder->options->subblock_data_size));
            ...
            if (coder->subfilter.mode == SUB_NONE)
            {
                ...
                coder->subblock.in_pending += lzma_bufcpy(in, in_pos, in_size, coder->subblock.data,
                                                          &coder->subblock.size, coder->subblock.limit);

                // If we ran out of input before the whole buffer
                // was filled, return to application.
                if (coder->subblock.size < coder->subblock.limit && action == LZMA_RUN)
                    return LZMA_OK;
            }
            else
            {
                assert(coder->options->subfilter_mode != LZMA_SUBFILTER_SET);
                ...
                {
                    coder->options->subfilter_mode = LZMA_SUBFILTER_FINISH;
                    coder->subfilter.mode = SUB_FINISH;
                }
                ...
                const lzma_ret ret =
                    coder->subfilter.subcoder.code(coder->subfilter.subcoder.coder, allocator, in, in_pos, in_size,
                                                   coder->subblock.data, &coder->subblock.size, coder->subblock.limit,
                                                   coder->subfilter.mode == SUB_FINISH ? LZMA_FINISH : action);
                ...
                if (ret == LZMA_STREAM_END)
                {
                    ...
                    assert(coder->subfilter.mode == SUB_FINISH || action == LZMA_FINISH);
        ...
        case SEQ_FLUSH:
            if (coder->options != NULL)
            {
                ...
                if (coder->alignment.multiple < LZMA_SUBBLOCK_ALIGNMENT_MIN ||
                    coder->alignment.multiple > LZMA_SUBBLOCK_ALIGNMENT_MAX)
                    return LZMA_OPTIONS_ERROR;
                ...
                if (coder->rle.count > 0 &&
                    (coder->rle.size != coder->options->rle || coder->subblock.size % coder->rle.size))
                {
                ...
                if (coder->subblock.size != 0 && coder->rle.size != LZMA_SUBBLOCK_RLE_OFF &&
                    coder->subblock.size % coder->rle.size == 0)
                {

                    // Initialize coder->rle.buffer if we don't
                    // have RLE already running.
                    if (coder->rle.count == 0)
                        memcpy(coder->rle.buffer, coder->subblock.data, coder->rle.size);
                    ...
                    const size_t count = coder->subblock.size / coder->rle.size;
                    if (UINT64_MAX - count > coder->rle.count &&
                        is_repeating(coder->rle.buffer, coder->rle.size, coder->subblock.data, count))
                    {
                        coder->rle.count += count;
                        coder->rle.in_pending += coder->subblock.in_pending;
            ...
            if (action != LZMA_RUN && *in_pos == in_size &&
                (coder->subfilter.mode == SUB_NONE || coder->subfilter.mode == SUB_FLUSH))
            {
                if (coder->rle.count > 0)
                {
                    subblock_rle_flush(coder);
        ...
        case SEQ_RLE_COUNT_0:
            ...
            if (coder->rle.count == 1)
            {
                ...
                // Subblock Type `Data'.
                if (subblock_align(coder, out, out_pos, out_size, coder->rle.size, ALIGN_SKEW_DATA))
                    return LZMA_OK;
        ...
        case SEQ_RLE_DATA:
            lzma_bufcpy(coder->rle.buffer, &coder->pos, coder->rle.size, out, out_pos, out_size);
        ...
        case SEQ_DATA_SIZE_0:
            ...
            if (subblock_align(coder, out, out_pos, out_size, coder->subblock.size, ALIGN_SKEW_DATA))
                return LZMA_OK;
        ...
        case SEQ_DATA:
            lzma_bufcpy(coder->subblock.data, &coder->pos, coder->subblock.size, out, out_pos, out_size);
        ...
        case SEQ_SUBFILTER_INIT: {
            ...
            return_if_error(lzma_raw_encoder_init(&coder->subfilter.subcoder, allocator, options));
            ...
            lzma_ret ret = lzma_filter_flags_size(&coder->subfilter.flags_size, options);
            assert(ret == LZMA_OK);
            if (ret != LZMA_OK)
                return LZMA_PROG_ERROR;

            coder->subfilter.flags = lzma_alloc(coder->subfilter.flags_size, allocator);
            ...
            size_t dummy = 0;
            ret = lzma_filter_flags_encode(options, coder->subfilter.flags, &dummy, coder->subfilter.flags_size);
            assert(ret == LZMA_OK);
        ...
        case SEQ_SUBFILTER_FLAGS:
            ...
            lzma_bufcpy(coder->subfilter.flags, &coder->pos, coder->subfilter.flags_size, out, out_pos, out_size);
...
static lzma_ret subblock_encode(lzma_coder *coder, lzma_allocator *allocator, const uint8_t *restrict in,
                                size_t *restrict in_pos, size_t in_size, uint8_t *restrict out,
                                size_t *restrict out_pos, size_t out_size, lzma_action action)
{
    if (coder->next.code == NULL)
        return subblock_buffer(coder, allocator, in, in_pos, in_size, out, out_pos, out_size, action);
    ...
    while (*out_pos < out_size && (*in_pos < in_size || action != LZMA_RUN))
    {
        if (!coder->next_finished && coder->temp.pos == coder->temp.size)
        {
            ...
            const lzma_ret ret = coder->next.code(coder->next.coder, allocator, in, in_pos, in_size, coder->temp.buffer,
                                                  &coder->temp.size, LZMA_BUFFER_SIZE, action);
            if (ret == LZMA_STREAM_END)
            {
        ...
        const lzma_ret ret = subblock_buffer(coder, allocator, coder->temp.buffer, &coder->temp.pos, coder->temp.size,
                                             out, out_pos, out_size, coder->next_finished ? LZMA_FINISH : LZMA_RUN);
        if (ret == LZMA_STREAM_END)
        {
            assert(action != LZMA_RUN);
...
extern lzma_ret lzma_subblock_encoder_init(lzma_next_coder *next, lzma_allocator *allocator,
                                           const lzma_filter_info *filters)
{
    ...
    {
        lzma_next_end(&next->coder->subfilter.subcoder, allocator);
    ...
    {
        if (next->coder->options->alignment < LZMA_SUBBLOCK_ALIGNMENT_MIN ||
            next->coder->options->alignment > LZMA_SUBBLOCK_ALIGNMENT_MAX)
        {
            subblock_encoder_end(next->coder, allocator);
            return LZMA_OPTIONS_ERROR;
        }
        next->coder->alignment.multiple = next->coder->options->alignment;
        next->coder->subfilter.allow = next->coder->options->allow_subfilters;
        subblock_size_limit = next->coder->options->subblock_data_size;
    }
    else
    {
        next->coder->alignment.multiple = LZMA_SUBBLOCK_ALIGNMENT_DEFAULT;
        next->coder->subfilter.allow = false;
        subblock_size_limit = LZMA_SUBBLOCK_DATA_SIZE_DEFAULT;
    }

    return_if_error(subblock_data_size(next->coder, allocator, subblock_size_limit));

    return lzma_next_filter_init(&next->coder->next, allocator, filters + 1);
}
