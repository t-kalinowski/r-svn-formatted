...
/* Map the Windows error number in ERROR to a locale-dependent error message
   ...
   GetLastError. */
char ZEXPORT *gz_strwinerror(error) DWORD error;
{
...
local gzFile gz_open(path, fd, mode) const char *path;
int fd;
const char *mode;
{
    gz_statep state;
            ...
            : open(path,
                   ...
                   O_LARGEFILE |
                       ...
                       (state->mode == GZ_READ ? O_RDONLY
                                               : (O_WRONLY | O_CREAT | (state->mode == GZ_WRITE ? O_TRUNC : O_APPEND))),
                   0666);
    if (state->fd == -1)
    {
        free(state);
...
/* Create an error message in allocated memory and set state->err and
   ...
   out of memory. */
void ZEXPORT gz_error(state, err, msg) gz_statep state;
int err;
const char *msg;
{
