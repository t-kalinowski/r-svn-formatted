...
void makefixed()
{
    ...
    for (;;)
    {
        if ((low % 7) == 0)
            printf("\n        ");
        printf("{%u,%u,%d}", state.lencode[low].op, state.lencode[low].bits, state.lencode[low].val);
        if (++low == size)
            break;
        putchar(',');
    }
...
int flush;
{
    ...
    for (;;)
        ...
        case LEN:
            ...
            {
                Tracevv((stderr,
                         this.val >= 0x20 && this.val < 0x7f ? "inflate:         literal '%c'\n"
...
uInt dictLength;
{
    ...
    {
        zmemcpy(state->window, dictionary + dictLength - state->wsize, state->wsize);
        state->whave = state->wsize;
    }
    else
    {
