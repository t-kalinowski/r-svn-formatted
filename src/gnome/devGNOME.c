...
static void GNOME_Text(double, double, int, char *, double, double, double, DevDesc *);
...
static int GNOME_Open(DevDesc *dd, gnomeDesc *gtkd, char *dsp, double w, double h)
{
    ...
    gtk_signal_connect(GTK_OBJECT(gtkd->canvas), "configure_event", (GtkSignalFunc)configure_event, (gpointer)dd);
...
static void GNOME_Rect(double x0, double y0, double x1, double y1, int coords, int bg, int fg, DevDesc *dd)
{
    ...
    item = gnome_canvas_item_new(gnome_canvas_root(GNOME_CANVAS(gtkd->canvas)), gnome_canvas_rect_get_type(), "x1", x0,
                                 "y1", y0, "x2", x1, "y2", y1, "width_pixels", (guint)dd->gp.lwd, NULL);
    ...
    if (fg != NA_INTEGER)
    {
        ...
        gnome_canvas_item_set(item, "outline_color_gdk", &gcol_outline, NULL);
    }
}
...
static void GNOME_Circle(double x, double y, int coords, double r, int col, int border, DevDesc *dd)
{
    ...
    item = gnome_canvas_item_new(gnome_canvas_root(GNOME_CANVAS(gtkd->canvas)), gnome_canvas_ellipse_get_type(), "x1",
                                 x1, "y1", y1, "x2", x2, "y2", y2, "width_pixels", (guint)dd->gp.lwd, NULL);
...
static void GNOME_Line(double x1, double y1, double x2, double y2, int coords, DevDesc *dd)
{
    ...
    item = gnome_canvas_item_new(gnome_canvas_root(GNOME_CANVAS(gtkd->canvas)), gnome_canvas_line_get_type(), "points",
                                 points, "width_pixels", (guint)dd->gp.lwd, NULL);
...
static void GNOME_Polyline(int n, double *x, double *y, int coords, DevDesc *dd)
{
    ...
    if (dd->gp.col != NA_INTEGER)
    {
        ...
        if (dd->gp.lty != 0)
        {
            ...
            gnome_canvas_item_set(item, "fill_stipple", &stipple, NULL);
        }
        gnome_canvas_item_set(item, "fill_color_gdk", &gcol_fill, NULL);
    }
...
static void GNOME_Polygon(int n, double *x, double *y, int coords, int bg, int fg, DevDesc *dd)
{
    ...
    item = gnome_canvas_item_new(gnome_canvas_root(GNOME_CANVAS(gtkd->canvas)), gnome_canvas_polygon_get_type(),
                                 "points", points, "width_units", (double)2, NULL);
...
static void GNOME_Text(double x, double y, int coords, char *str, double xc, double yc, double rot, DevDesc *dd)
{
    GnomeCanvasItem *item;
    gnomeDesc *gtkd = (gnomeDesc *)dd->deviceSpecific;
    GdkColor gcol_fill;
    ...
    item = gnome_canvas_item_new(gnome_canvas_root(GNOME_CANVAS(gtkd->canvas)), gnome_canvas_text_get_type(), "text",
                                 ...
                                 "font", "-*-times-medium-r-*-*-120-*-*-*-*-*-*-*", "fill_color_gdk", &gcol_fill, NULL);
}
