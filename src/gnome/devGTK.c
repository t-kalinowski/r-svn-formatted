...
static gint configure_event(GtkWidget *widget, GdkEventConfigure *event, gpointer data)
{
    ...
    // create pixmap
    gtkd->pixmap = gdk_pixmap_new(gtkd->drawingarea->window, gtkd->drawingarea->allocation.width,
                                  gtkd->drawingarea->allocation.height, -1);

    // clear the pixmap to background color
    if (gtkd->wgc != NULL)
    {
        gdk_gc_set_foreground(gtkd->wgc, &gtkd->bgcolor);
        gdk_draw_rectangle(gtkd->pixmap, gtkd->wgc, TRUE, 0, 0, gtkd->drawingarea->allocation.width,
...
static gint expose_event(GtkWidget *widget, GdkEventExpose *event, gpointer data)
{
    ...
    else
    {
        ...
        gdk_draw_pixmap(gtkd->drawingarea->window, gtkd->wgc, gtkd->pixmap, event->area.x, event->area.y, event->area.x,
...
static int GTK_Open(DevDesc *dd, gtkDesc *gtkd, char *dsp, double w, double h)
{
    ...
    gtk_signal_connect(GTK_OBJECT(gtkd->drawingarea), "configure_event", (GtkSignalFunc)configure_event, (gpointer)dd);
    gtk_signal_connect(GTK_OBJECT(gtkd->drawingarea), "expose_event", (GtkSignalFunc)expose_event, (gpointer)dd);
...
static void GTK_Rect(double x0, double y0, double x1, double y1, int coords, int bg, int fg, DevDesc *dd)
{
    ...
    {
        SetColor(dd, bg);
        gdk_draw_rectangle(gtkd->drawingarea->window, gtkd->wgc, TRUE, (gint)x0, (gint)y0, (gint)x1 - (gint)x0,
                           (gint)y1 - (gint)y0);
    ...
    if (fg != NA_INTEGER)
    {
        ...
        gdk_draw_rectangle(gtkd->drawingarea->window, gtkd->wgc, FALSE, (gint)x0, (gint)y0, (gint)x1 - (gint)x0,
                           (gint)y1 - (gint)y0);
...
static void GTK_Circle(double x, double y, int coords, double r, int col, int border, DevDesc *dd)
{
    ...
    {
        SetColor(dd, col);
        gdk_draw_arc(gtkd->drawingarea->window, gtkd->wgc, TRUE, ix - ir, iy - ir, 2 * ir, 2 * ir, 0, 23040);
        gdk_draw_arc(gtkd->pixmap, gtkd->wgc, TRUE, ix - ir, iy - ir, 2 * ir, 2 * ir, 0, 23040);
    }
    if (border != NA_INTEGER)
    {
        SetLineType(dd, dd->gp.lty, dd->gp.lwd);
        SetColor(dd, border);
        gdk_draw_arc(gtkd->drawingarea->window, gtkd->wgc, FALSE, ix - ir, iy - ir, 2 * ir, 2 * ir, 0, 23040);
        gdk_draw_arc(gtkd->pixmap, gtkd->wgc, FALSE, ix - ir, iy - ir, 2 * ir, 2 * ir, 0, 23040);
    }
}
...
static void GTK_Line(double x1, double y1, double x2, double y2, int coords, DevDesc *dd)
{
    ...
    SetLineType(dd, dd->gp.lty, dd->gp.lwd);
    gdk_draw_line(gtkd->drawingarea->window, gtkd->wgc, xx1, yy1, xx2, yy2);
    gdk_draw_line(gtkd->pixmap, gtkd->wgc, xx1, yy1, xx2, yy2);
}
...
static void GTK_Polyline(int n, double *x, double *y, int coords, DevDesc *dd)
{
    ...
    SetColor(dd, dd->gp.col);
    SetLineType(dd, dd->gp.lty, dd->gp.lwd);
    gdk_draw_lines(gtkd->drawingarea->window, gtkd->wgc, points, n);
...
static void GTK_Polygon(int n, double *x, double *y, int coords, int bg, int fg, DevDesc *dd)
{
    ...
    {
        SetColor(dd, bg);
        gdk_draw_polygon(gtkd->drawingarea->window, gtkd->wgc, TRUE, points, n);
    ...
    if (fg != NA_INTEGER)
    {
        SetColor(dd, fg);
        SetLineType(dd, dd->gp.lty, dd->gp.lwd);
        gdk_draw_polygon(gtkd->drawingarea->window, gtkd->wgc, FALSE, points, n);
