...
static void gnome_find_dialog_init(GnomeFindDialog *find_dialog)
{
    ...
    gtk_signal_connect(GTK_OBJECT(forwards_radio), "clicked", GTK_SIGNAL_FUNC(forwards_clicked_cb),
                       (gpointer)find_dialog);
    gtk_signal_connect(GTK_OBJECT(backwards_radio), "clicked", GTK_SIGNAL_FUNC(backwards_clicked_cb),
                       (gpointer)find_dialog);
    ...
    gtk_signal_connect(GTK_OBJECT(top_radio), "clicked", GTK_SIGNAL_FUNC(top_clicked_cb), (gpointer)find_dialog);
    gtk_signal_connect(GTK_OBJECT(cursor_radio), "clicked", GTK_SIGNAL_FUNC(cursor_clicked_cb), (gpointer)find_dialog);
    gtk_signal_connect(GTK_OBJECT(bottom_radio), "clicked", GTK_SIGNAL_FUNC(bottom_clicked_cb), (gpointer)find_dialog);
...
GtkWidget *gnome_find_dialog_new(const gchar *title, const gchar *find_text, gboolean show_case_sensitive,
                                 gboolean show_wrap_search, gboolean show_reg_exp)
{
    GnomeFindDialog *find_dialog;
    ...
    if (show_case_sensitive || show_wrap_search || show_reg_exp)
    {
        ...
        if (show_reg_exp)
        {
            ...
            gtk_signal_connect(GTK_OBJECT(reg_exp_check), "toggled", GTK_SIGNAL_FUNC(reg_exp_toggled_cb),
                               (gpointer)find_dialog);
        }
