...
int R_ReadConsole(char *prompt, unsigned char *buf, int len, int addtohistory)
{
    ...
    else
    {
        ...
        gtk_signal_connect(GTK_OBJECT(R_gtk_terminal_text), "console_line_ready",
...
int main(int ac, char **av)
{
    ...
    gnome_init("R.gnome", g_strdup_printf("%s.%s %s (%s %s, %s)", R_MAJOR, R_MINOR, R_STATUS, R_MONTH, R_DAY, R_YEAR),
               ac, av);
...
static void pagertb_start(GtkWidget *widget, gpointer data)
{
    ...
    if (data != NULL)
    {
        ...
        gtk_signal_emit_by_name(GTK_OBJECT(pager_data->text), "key_press_event", &event, &retval);
    }
}
...
int R_ShowFiles(int nfile, char **file, char **title, char *wtitle, int del, char *pager)
{
    ...
    gtk_table_attach(GTK_TABLE(table), pager_data->text, 0, 1, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL,
    ...
    for (i = 0; i < nfile; i++)
    {
        if ((title[i] != NULL) && (*title[i] != NULL))
        {
            ...
            gtk_text_insert(GTK_TEXT(pager_data->text), titlefont, &R_gnome_userprefs.pager_title_textcolor,
                            &R_gnome_userprefs.pager_title_bgcolor, buf, strlen(buf));
        }
            ...
            do
            {
                ...
                for (j = buf, k = buf; j < buf + readlen; j++)
                {
                    if (*j == '\b')
                    {
                        k--;
                        if (k != modestart)
                            gtk_text_insert(GTK_TEXT(pager_data->text), NULL, NULL, NULL, modestart, k - modestart);
                        modestart = k;
                        emmode = TRUE;
                    }
                    else
                    {
                        *k = *j;
                        ...
                        if (emmode)
                        {
                            gtk_text_insert(GTK_TEXT(pager_data->text), emfont, &R_gnome_userprefs.pager_text_textcolor,
                                            &R_gnome_userprefs.pager_text_bgcolor, k - 1, 1);
                            modestart = k;
                            emmode = FALSE;
                        }
                    }
        ...
        else
        {
            ...
            gtk_text_insert(GTK_TEXT(pager_data->text), NULL, NULL, NULL, buf, strlen(buf));
        }
    }
