...
static const struct poptOption popt_options[] = {{"version", '\0', POPT_ARG_NONE, &arg_version, 0, "", NULL},
...
void handle_args()
{
    ...
    if (arg_nsize)
    {
        ...
        if (ierr)
        {
            ...
            fprintf(stderr, "--nsize %ld'%c': too large", value, (ierr == 1) ? 'M' : ((ierr == 2) ? 'K' : 'k'));
        }
