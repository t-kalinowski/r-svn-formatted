...
GtkWidget *prefs_text_page(void)
{
    ...
    gtk_signal_connect(GTK_OBJECT(font_picker), "font_set", GTK_SIGNAL_FUNC(prefs_font_picker_font_set), NULL);
...
GtkWidget *prefs_history_frame(void)
{
    ...
    gtk_table_attach(GTK_TABLE(table), savetofile, 0, 2, 1, 2, GTK_FILL, 0, GNOME_PAD, 0);
    gtk_table_attach(GTK_TABLE(table), dummy, 0, 1, 2, 3, GTK_FILL, 0, GNOME_PAD_BIG, 0);
    gtk_table_attach(GTK_TABLE(table), nameentry, 1, 2, 2, 3, GTK_FILL, 0, 0, 0);
    gtk_table_attach(GTK_TABLE(table), dontsave, 0, 2, 3, 4, GTK_FILL, 0, GNOME_PAD, GNOME_PAD_SMALL);
