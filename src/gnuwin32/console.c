...
void xbufaddxc(xbuf p, wchar_t c)
{
    ...
    case L'\t':
        ...
        *p->free = '\0';
        ...
        for (i = wcswidth(p->s[p->ns - 1]); (i % TABSIZE); i++)
            XPUTC(L' ');
        break;
    case L'\n':
        XPUTC(L'\0');
...
static size_t enctowcs(wchar_t *wc, char *s, int n)
{
    ...
    if ((pb = strchr(s, UTF8in[0])) && *(pb + 1) == UTF8in[1] && *(pb + 2) == UTF8in[2])
    {
        ...
        *pb = UTF8in[0]; /* preserve input string, needed in consolewrites */
        pb += 3;
        pe = pb;
        while (*pe && !((pe = strchr(pe, UTF8out[0])) && *(pe + 1) == UTF8out[1] && *(pe + 2) == UTF8out[2]))
            pe++;
        if (!*pe)
            return nc; /* FIXME */
        ;
...
ConsoleData newconsoledata(font f, int rows, int cols, int bufbytes, int buflines, rgb *guiColors, int kind,
                           int buffered, int cursor_blink)
{
    ConsoleData p;
...
static int intersect_input(ConsoleData p, int apply)
{
    ...
    if (my1 < CURROW || my0 > CURROW)
        return (0);
    if (my0 < CURROW)
        mx0 = 0;
    if (my1 > CURROW)
        mx1 = COLS;
    if (mx1 < CURCOL || col_to_pos(p, mx0) >= max_pos)
        return (0);
    mx0 = max(mx0, prompt_wid);
    while (col_to_pos(p, mx1) >= max_pos)
        mx1--;
    if (apply)
    {
        p->mx0 = mx0;
...
static void writelineHelper(ConsoleData p, int fch, int lch, rgb fgr, rgb bgr, int j, int len, wchar_t *s)
{
    rect r;
...
static int writeline(control c, ConsoleData p, int i, int j)
{
    ...
    if ((CURROW >= 0) && (CURCOL >= FC) && (CURCOL < FC + COLS) && (i == NUMLINES - 1) &&
        (p->sel == 0 || !intersect_input(p, 0)))
    {
        if (!p->overwrite)
        {
            if (p->cursor_blink)
            {
                setcaret(c, BORDERX + (CURCOL - FC) * FW, BORDERY + j * FH, p->cursor_blink == 1 ? 1 : FW / 4, FH);
                showcaret(c, 1);
            }
            else
                showcaret(c, 0);
        ...
        { /* determine the width of the current char */
            ...
            if (p->cursor_blink < 2)
            {
                r = rect(BORDERX + (CURCOL - FC) * FW, BORDERY + j * FH, w0 * FW, FH);
                gfillrect(BM, highlight, r);
        ...
        else
        {
            if (p->cursor_blink)
            {
                setcaret(c, BORDERX + (CURCOL - FC) * FW, BORDERY + j * FH, p->cursor_blink == 1 ? 1 : FW, FH);
                showcaret(c, 1);
            }
            else
                showcaret(c, 0);
...
void drawconsole(control c, rect r) /* r is unused here */
{
    ...
    if (!BM)
        return;
    ; /* This is a workaround for PR#1711.
BM should never be null here */
    ...
    if (maxwd < COLS - 1)
        maxwd = COLS - 1;
    maxwd += FC;
    gchangescrollbar(c, HWINSB, FC, maxwd - FC, COLS, p->kind == CONSOLE || NUMLINES > ROWS);
...
void console_mouserep(control c, int button, point pt)
{
    ...
    if ((button & LeftButton) && (p->sel))
        console_mousedrag(c, button, pt);
}
...
static void performCompletion(control c)
{
    ...
    wchar_t orig[alen + 1], pline[2 * alen + 1], *pchar = pline, achar;
...
static int CleanTranscript(wchar_t *tscpt, wchar_t *cmds)
{
    ...
    while (*tscpt)
    {
        if (startofline)
        {
            ...
            while (*tscpt == L' ' || *tscpt == L'\t')
                tscpt++;
            /* skip over the prompt prefix */
            while (*tscpt && !wcschr(nonprefix, *tscpt))
                tscpt++;
...
void consolepaste(control c)
{
    ...
    if (OpenClipboard(NULL) && (hglb = GetClipboardData(CF_UNICODETEXT)) && (pc = (wchar_t *)GlobalLock(hglb)))
    {
        if (p->clp)
        {
            new = realloc((void *)p->clp, (wcslen(p->clp) + wcslen(pc) + 1) * sizeof(wchar_t));
        }
        else
        {
        ...
        if (new)
        {
            ...
            for (i = 0; i < wcslen(pc); i++)
                if (IsSurrogatePairsHi(pc[i]) && i + 1 < wcslen(pc) && IsSurrogatePairsLo(pc[i + 1]))
                {
                    pc[i] = L'?';
                    pc[i + 1] = L'?';
                    i++;
...
void consolepastecmds(control c)
{
    ...
    if (OpenClipboard(NULL) && (hglb = GetClipboardData(CF_UNICODETEXT)) && (pc = (wchar_t *)GlobalLock(hglb)))
    {
        if (p->clp)
        {
            new = realloc((void *)p->clp, (wcslen(p->clp) + CleanTranscript(pc, 0)) * sizeof(wchar_t));
        }
        else
        {
...
static void checkvisible(control c)
{
    ...
    setfirstvisible(c, NUMLINES - ROWS);
    newfc = 0;
    while ((CURCOL <= newfc) || (CURCOL > newfc + COLS - 2))
        newfc += 5;
    if (newfc != FC)
        setfirstcol(c, newfc);
}
...
int consolereads(control c, const char *prompt, char *buf, int len, int addtohistory)
{
    ...
    for (; P < aLine + pre_prompt_len; P++)
        if (*P == L'\r')
            ...
            w0 += mbcslocale ? Ri18n_wcwidth(*P) : 1;
    USER(NUMLINES - 1) = w0;
...
void consoleresize(console c, rect r)
{
    ...
    if (!p->lbuf)
        return;
    ; /* don't implement resize if no content
yet in pager */
    if (CURROW >= 0)
    {
        if (NUMLINES > ROWS)
...
void setconsoleoptions(const char *fnname, int fnsty, int fnpoints, int rows, int cols, int consx, int consy,
                       ...
                       int buflines, int buffered, int cursor_blink)
{
...
console newconsole(char *name, int flags)
{
    ...
    p = newconsoledata((consolefn) ? consolefn : FixedFont, consoler, consolec, consolebufb, consolebufl, guiColors,
