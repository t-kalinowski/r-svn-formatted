...
static int intersect_input(ConsoleData p, int apply)
{
    ...
    if (my1 > CURROW)
        mx1 = COLS;
    if (mx1 < CURCOL || col_to_byte(p, mx0) >= max_byte)
        return (0);
    mx0 = max(mx0, prompt_wid);
    while (col_to_byte(p, mx1) >= max_byte)
        mx1--;
    if (apply)
    {
        p->mx0 = mx0;
...
static void performCompletion(control c)
{
    ...
    sprintf(cmd, "rcompgen:::.win32consoleCompletion(\"%s\", %d)", pline, cursor_position);
