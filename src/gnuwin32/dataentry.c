...
#endif
#define BOXW(x)                                                                                                        \
    (min(((x < 100 && DE->nboxchars == 0) ? DE->boxw[x] : DE->box_w), DE->p->w - DE->boxw[0] - 2 * DE->bwidth - 2))
...
SEXP do_dataentry(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (DE->ymaxused > 10000)
        DE->yScrollbarScale = DE->ymaxused / 1000;
...
static void setcellwidths(DEstruct DE)
{
    ...
    { /* 100 on-screen columns cannot occur */
        ...
        if ((w += dw) > DE->p->w || (!DE->isEditor && i > DE->xmaxused - DE->colmin + 1))
        {
            DE->nwide = i;
            DE->windowWidth = w - dw;
            break;
...
static void drawwindow(DEstruct DE)
{
    ...
    if (!DE->isEditor && DE->nhigh > DE->ymaxused + 1)
        DE->nhigh = DE->ymaxused + 1;
    ...
    deredraw(DE);
    /* row/col 1 = pos 0 */
    gchangescrollbar(DE->de, VWINSB, (DE->rowmin - 1) / DE->yScrollbarScale, DE->ymaxused / DE->yScrollbarScale,
                     max(DE->nhigh / DE->yScrollbarScale, 1), 0);
    gchangescrollbar(DE->de, HWINSB, (DE->colmin - 1) / DE->xScrollbarScale, DE->xmaxused / DE->xScrollbarScale,
...
static void drawrow(DEstruct DE, int whichrow)
{
    ...
    cleararea(DE, src_x, src_y, DE->windowWidth, DE->box_h, (whichrow > 0) ? DE->p->bg : bbg);
...
static Rboolean getccol(DEstruct DE)
{
    ...
    {
        newcol = TRUE;
        SET_VECTOR_ELT(DE->work, wcol - 1, ssNewVector(DE, REALSXP, max(100, wrow)));
...
{
    SEXP cvec;
    int wcol = DE->ccol + DE->colmin - 1, wrow = DE->rowmin + DE->crow - 1, wrow0;
    Rboolean newcol;
...
static void printstring(DEstruct DE, char *ibuf, int buflen, int row, int col, int left)
{
    ...
    cleararea(DE, x_pos + 1, y_pos + 1, bw - 1, DE->box_h - 1, (row == 0 || col == 0) ? bbg : DE->p->bg);
...
static void clearrect(DEstruct DE)
{
    ...
    cleararea(DE, x_pos, y_pos, BOXW(DE->ccol + DE->colmin - 1), DE->box_h, DE->p->bg);
}
...
static void cleararea(DEstruct DE, int xpos, int ypos, int width, int height, rgb col)
{
...
static void drawrectangle(DEstruct DE, int xpos, int ypos, int width, int height, int lwd, int fore)
{
    ...
    gdrawrect(DE->de, lwd, 0, (fore == 1) ? DE->p->ufg : DE->p->bg, rect(xpos, ypos, width, height), 1,
...
static void de_normalkeyin(control c, int k)
{
    ...
    st = ggetkeystate();
    if ((DE->p->chbrk) && (k == DE->p->chbrk) && ((!DE->p->modbrk) || ((DE->p->modbrk) && (st == DE->p->modbrk))))
    {
        DE->p->fbrk(c);
        return;
    }
...
static void de_ctrlkeyin(control c, int key)
{
    ...
    st = ggetkeystate();
    if ((DE->p->chbrk) && (key == DE->p->chbrk) && ((!DE->p->modbrk) || ((DE->p->modbrk) && (st == DE->p->modbrk))))
    {
        DE->p->fbrk(c);
        return;
    }
...
static void de_mousedown(control c, int buttons, point xy)
{
    ...
    if (buttons & LeftButton)
    {
        ...
        else if (buttons & DblClick)
        {
            ...
            rr = rect(x + DE->text_xoffset, y - DE->text_yoffset - 1, bw - DE->text_xoffset - 2,
...
{
    DEstruct DE = getdata(c);
    if (DE->p->w != DE->oldWIDTH || DE->p->h != DE->oldHEIGHT)
        drawwindow(DE);
    else
        deredraw(DE);
}
...
static void deredraw(DEstruct DE)
{
    ...
    for (i = 1; i < DE->nhigh; i++)
        drawrectangle(DE, 0, DE->hwidth + i * DE->box_h, DE->boxw[0], DE->box_h, 1, 1);
    if (DE->isEditor)
    {
        DE->colmax = DE->colmin + (DE->nwide - 2);
    ...
    for (i = DE->colmin; i <= DE->colmax; i++)
        drawcol(DE, i);
    gfillrect(DE->de, DE->p->bg,
              rect(DE->windowWidth + 1, DE->hwidth, DE->p->w - DE->windowWidth - 1, DE->p->h - DE->hwidth));
    highlightrect(DE);
}
...
static void copyarea(DEstruct DE, int src_x, int src_y, int dest_x, int dest_y)
{
    ...
    copyrect(DE->de, pt(dest_x, dest_y), rect(src_x, src_y, DE->windowWidth - mx, DE->windowHeight - my));
}

static void copyH(DEstruct DE, int src_x, int dest_x, int width)
{
    copyrect(DE->de, pt(dest_x, DE->hwidth), rect(src_x, DE->hwidth, width, DE->windowHeight - DE->hwidth));
}
...
static Rboolean initwin(DEstruct DE, char *title)
{
    ...
    gchangescrollbar(DE->de, VWINSB, 0, DE->ymaxused / DE->yScrollbarScale, max(DE->nhigh / DE->yScrollbarScale, 1), 0);
    setcellwidths(DE);
    gchangescrollbar(DE->de, HWINSB, 0, DE->xmaxused / DE->xScrollbarScale, max(DE->nwide / DE->xScrollbarScale, 1), 0);
    show(DE->de);
...
static void popupclose(control c)
{
    ...
    {
        if (isNull(tvec))
            SET_VECTOR_ELT(DE->work, popupcol - 1, ssNewVector(DE, REALSXP, 100));
        else
    ...
    {
        if (isNull(tvec))
            SET_VECTOR_ELT(DE->work, popupcol - 1, ssNewVector(DE, STRSXP, 100));
        else
...
static void de_popupmenu(DEstruct DE, int x_pos, int y_pos, int col)
{
    ...
    wconf = newwindow(G_("Variable editor"), rect(x_pos + r.x - 150, y_pos + r.y - 50, 300, 100),
...
static void de_paste(control c)
{
    ...
    closerect(DE);
    if (clipboardhastext() && !getstringfromclipboard(DE->buf, BUFSIZE - 1))
    {
...
static void de_popup_vw(DEstruct DE)
{
    ...
    devw = newwindow(G_("Cell width(s)"), rect(0, 0, 250, 60), Titlebar | Centered | Closebox | Modal);
    setdata(devw, DE);
...
{
    DEstruct DE = getdata(c);
    if (((DE->p->w == r.width) && (DE->p->h == r.height)) || (r.width == 0) || (r.height == 0)) /* minimize */
        return;
    ;
    DE->p->w = r.width;
    DE->p->h = r.height;
...
static void menudehelp(control m)
{
    char s[] = GN_(
        ...
        "Ctrl+F: move down one screenful\n\tPgUp or Ctrl+B: move up one screenful\n\tHome: move to (1,1) cell\n\tEnd: "
        "show last rows of last column.\n   Mouse: left-click in a cell, use the scrollbar(s).\n\nEditing.\n  Type in "
        "the currently hightlighted cell\n  Double-click in a cell for an editable field\n\nMisc.\n  Ctrl-L redraws "
        "the screen, auto-resizing the columns\n  Ctrl-C copies selected cell\n  Ctrl-V pastes to selected cell\n  "
        "Right-click menu for copy, paste, autosize currently selected column\n\n");
    askok(G_(s));
}

static MenuItem DePopup[28] = {{GN_("Help"), menudehelp, 0},
                               {"-", 0, 0},
...
static dataeditor newdataeditor(DEstruct DE, char *title)
{
    ...
    DE->p = newconsoledata((consolefn) ? consolefn : FixedFont, pagerrow, pagercol, 0, 0, consolefg, consoleuser,
                           consolebg, DATAEDITOR, 0);
    if (!DE->p)
        return NULL;
    ...
    if (DE->isEditor)
    {
        ...
        setdata(m, DE);
        MCHECK(m = newmenuitem("-", 0, NULL));
        MCHECK(m = DE->de_mvw = newmenuitem(G_("Cell widths ..."), 0, menudecellwidth));
        setdata(m, DE);
