...
static void clearrect(DEstruct DE)
{
    ...
    cleararea(DE, x_pos, y_pos, BOXW(DE->ccol + DE->colmin - 1), DE->box_h, DE->p->guiColors[dataeditbg]);
}
...
static void drawrectangle(DEstruct DE, int xpos, int ypos, int width, int height, int lwd, int fore)
{
    ...
    gdrawrect(DE->de, lwd, 0, (fore == 1) ? DE->p->guiColors[dataedituser] : DE->p->guiColors[dataeditbg],
              rect(xpos, ypos, width, height), 1, PS_ENDCAP_SQUARE, PS_JOIN_BEVEL, 10);
}
...
static void deredraw(DEstruct DE)
{
    ...
    for (i = DE->colmin; i <= DE->colmax; i++)
        drawcol(DE, i);
    gfillrect(DE->de, DE->p->guiColors[dataeditbg],
