...
SEXP do_dataentry(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (xmaxused > 10000)
        xScrollbarScale = xmaxused / 1000;
    if (ymaxused > 10000)
        yScrollbarScale = ymaxused / 1000;
...
static void drawwindow(void)
{
    ...
    deredraw();
    /* row/col 1 = pos 0 */
    gchangescrollbar(de, VWINSB, (rowmin - 1) / yScrollbarScale, ymaxused / yScrollbarScale,
                     max(nhigh / yScrollbarScale, 1), 0);
    gchangescrollbar(de, HWINSB, (colmin - 1) / xScrollbarScale, xmaxused / xScrollbarScale,
...
static void printstring(char *ibuf, int buflen, int row, int col, int left)
{
    ...
    cleararea(x_pos + 1, y_pos + 1, bw - 1, box_h - 1, (row == 0 || col == 0) ? bbg : p->bg);
...
static void drawrectangle(int xpos, int ypos, int width, int height, int lwd, int fore)
{
    ...
    gdrawrect(de, lwd, 0, (fore == 1) ? p->ufg : p->bg, rect(xpos, ypos, width, height), 1, PS_ENDCAP_SQUARE,
              PS_JOIN_BEVEL, 10);
}
...
static void deredraw(void)
{
    ...
    if (hwidth > 0)
        gfillrect(de, bbg, rect(0, 0, WIDTH, hwidth));
    ...
    for (i = colmin; i <= colmax; i++)
        drawcol(i);
    gfillrect(de, p->bg, rect(windowWidth + 1, hwidth, WIDTH - windowWidth - 1, HEIGHT - hwidth));
    highlightrect();
}
...
static void copyarea(int src_x, int src_y, int dest_x, int dest_y)
{
    ...
    copyrect(de, pt(dest_x, dest_y), rect(src_x, src_y, windowWidth - mx, windowHeight - my));
}

static void copyH(int src_x, int dest_x, int width)
{
    copyrect(de, pt(dest_x, hwidth), rect(src_x, hwidth, width, windowHeight - hwidth));
}
...
static Rboolean initwin(void)
{
    ...
    gchangescrollbar(de, VWINSB, 0, ymaxused / yScrollbarScale, max(nhigh / yScrollbarScale, 1), 0);
    setcellwidths();
...
static void de_paste(control c)
{
    ...
    closerect();
    if (clipboardhastext() && !getstringfromclipboard(buf, BUFSIZE - 1))
    {
...
static void de_sbf(control c, int pos)
{
    ...
    else
    {
        ...
        if (rowmin > ymaxused - nhigh + 2)
            rowmin = max(1, ymaxused - nhigh + 2);
    }
    drawwindow();
}
