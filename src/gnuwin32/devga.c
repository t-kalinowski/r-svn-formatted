...
static void SaveAsPostscript(DevDesc *dd, char *fn)
{
    ...
    if (PSDeviceDriver(ndd, fn, paper, family, afmpaths, bg, fg, GConvertXUnits(1.0, NDC, INCHES, dd),
...
static int setupScreenDevice(DevDesc *dd, gadesc *xd, int w, int h, int recording)
{
    menu m;
    int iw, ih;
    double dw, dh, d;
...
static int GA_Open(DevDesc *dd, gadesc *xd, char *dsp, double w, double h, int recording)
{
    rect rr;
...
int GADeviceDriver(DevDesc *dd, char *display, double width, double height, double pointsize, int recording)
{
...
static void SaveAsPng(DevDesc *dd, char *fn)
{
    ...
    gsetcliprect(xd->bm, getrect(xd->bm));
    R_SaveAsPng(xd->bm, xd->windowWidth, xd->windowHeight, privategetpixel, 0, fp);
