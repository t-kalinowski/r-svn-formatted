...
static void menups(control m)
{
    ...
    if (!fn)
        return;
    fixslash(fn);
    sprintf(cmd, "dev.print(device=postscript, file=\"%s\", horizontal=FALSE)", fn);
...
static void menuwm(control m)
{
    ...
    if (!fn)
        return;
    fixslash(fn);
    sprintf(cmd, "dev.print(device=x11,\"win.metafile:%s\",%f,%f,%d)", fn, (float)GConvertXUnits(1.0, NDC, INCHES, dd),
            (float)GConvertYUnits(1.0, NDC, INCHES, dd), dd->dp.ps);
    private_devcopy(dd, cmd);
}
...
static void menuclpwm(control m)
{
    ...
    sprintf(cmd, "dev.print(device=x11,\"win.metafile\",%f,%f,%d)", (float)GConvertXUnits(1.0, NDC, INCHES, dd),
            (float)GConvertYUnits(1.0, NDC, INCHES, dd), dd->dp.ps);
    private_devcopy(dd, cmd);
}
...
static void menuprint(control m)
{
    ...
    GInit(&ndd->dp);
    if (!X11DeviceDriver(ndd, "win.print", GConvertXUnits(1.0, NDC, INCHES, dd), GConvertYUnits(1.0, NDC, INCHES, dd),
                         dd->gp.ps))
    {
        free(ndd);
        return;
    ...
    gsetcursor(xd->gawin, WatchCursor);
    gsetVar(install(".Device"), mkString("win.print"), R_NilValue);
    addDevice(ndd);
    initDisplayList(ndd);
...
static int X11_Open(DevDesc *dd, x11Desc *xd, char *dsp, double w, double h)
{
    ...
    else
    {
        ...
        if (strcmp(dsp, s) || (c && (c != ':')))
            return 0;
        dsp[ls] = c;
        xd->gawin = newmetafile((ld > ls) ? &dsp[ls + 1] : "", rect(0, 0, lmm * w, lmm * h));
        xd->kind = 2;
        if (!xd->gawin)
            return 0;
    }
