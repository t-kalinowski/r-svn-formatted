...
static int Load_Unzip_DLL()
{
    ...
    if (dwVerInfoSize)
    {
        ...
        if (GetFileVersionInfo(szFullPath, 0L, dwVerInfoSize, lpstrVffInfo))
        {
            ...
            fRetName = VerQueryValue(lpstrVffInfo, TEXT("\\StringFileInfo\\040904E4\\CompanyName"), (LPVOID)&lszVerName,
                                     &cchVer);
            if (!fRet || !fRetName || (lstrcmpi(lszVer, UNZ_DLL_VERSION) != 0) ||
