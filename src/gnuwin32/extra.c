...
SEXP do_winver(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    { /* unsupported, probably cannot be reached */
...
SEXP do_loadRconsole(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    {
        getActive(&gui);
        if (loadRconsole(&gui, (CHAR(STRING_ELT(sfile, 0)))))
            applyGUI(&gui);
    }
    else
...
SEXP do_chooseFiles(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    *list = '\0'; /* no initialization */
    askfilenames(CHAR(STRING_ELT(caption, 0)), path, multi, cfilters, filterindex, list, 65500,
...
int winAccess(const char *path, int mode)
{
    ...
    if (GA_isNT)
    {
        ...
        /* get size */
        GetFileSecurity(path, OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION, 0, 0,
                        &size);
        error = GetLastError();
        if (error != ERROR_INSUFFICIENT_BUFFER)
            return -1;
        sdPtr = (SECURITY_DESCRIPTOR *)alloca(size);
        if (!GetFileSecurity(path, OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION,
                             sdPtr, size, &size))
            return -1;
        /*
