...
SEXP do_winver(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    */
    ...
    { /* should not get here */
        sprintf(ver, "Windows %d.%d (build %d) %s", (int)osvi.dwMajorVersion, (int)osvi.dwMinorVersion,
...
void internal_shellexec(const char *file)
{
    ...
    if (ret <= 32)
    { /* an error condition */
        if (ret == ERROR_FILE_NOT_FOUND || ret == ERROR_PATH_NOT_FOUND || ret == SE_ERR_FNF || ret == SE_ERR_PNF)
            error(_("'%s' not found"), file);
        if (ret == SE_ERR_ASSOCINCOMPLETE || ret == SE_ERR_NOASSOC)
            error(_("file association for '%s' not available or invalid"), file);
...
static void internal_shellexecW(const wchar_t *file)
{
    ...
    { /* an error condition */
        ...
        if (ret == SE_ERR_ASSOCINCOMPLETE || ret == SE_ERR_NOASSOC)
            error(_("file association for '%ls' not available or invalid"), file);
        if (ret == SE_ERR_ACCESSDENIED || ret == SE_ERR_SHARE)
...
SEXP do_windialog(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    message = CADR(args);
    if (!isString(message) || length(message) != 1 || strlen(translateChar(STRING_ELT(message, 0))) > 255)
...
SEXP do_windialogstring(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    string = askstring(translateChar(STRING_ELT(message, 0)), translateChar(STRING_ELT(def, 0)));
    if (string)
...
SEXP do_wingetmenuitems(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (items->numItems == 0)
    {
        sprintf(msgbuf, _("unable to retrieve items for %s (%s)"), translateChar(STRING_ELT(mname, 0)), errmsg);
        freemenuitems(items);
...
SEXP do_winmenuadd(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    { /* add an item */
        ...
        res = winaddmenuitem(translateChar(STRING_ELT(sitem, 0)), translateChar(STRING_ELT(smenu, 0)),
                             translateChar(STRING_ELT(CADDR(args), 0)), errmsg);
        if (res > 0)
        {
...
SEXP do_winmenudel(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    { /* delete an item */
        if (!isString(sitem) || length(sitem) != 1)
            error(_("invalid '%s' argument"), "itemname");
        res = windelmenuitem(translateChar(STRING_ELT(sitem, 0)), translateChar(STRING_ELT(smenu, 0)), errmsg);
        if (res > 0)
        {
...
SEXP do_winprogressbar(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    pbar->width = width;
    pbar->wprog = newwindow(title, rect(0, 0, width + 40, haveLabel ? 100 : 80), Titlebar | Centered);
    setbackground(pbar->wprog, dialog_bg());
    if (haveLabel)
        pbar->lab = newlabel(label, rect(10, 15, width + 20, 25), AlignCenter);
    pbar->pb = newprogressbar(rect(20, haveLabel ? 50 : 30, width, 20), 0, width, 1, 1);
...
SEXP do_savehistory(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (CharacterMode == RGui)
    {
        ...
        wgl_savehistoryW(filenameToWchar(STRING_ELT(sfile, 0), 0), R_HistorySize);
    }
...
SEXP do_sysinfo(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    {
        if (osvi.wServicePackMajor > 0)
            sprintf(ver, "build %d, Service Pack %d", LOWORD(osvi.dwBuildNumber), (int)osvi.wServicePackMajor);
        else
...
SEXP do_selectlist(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    wselect = newwindow(haveTitle ? translateChar(STRING_ELT(CADDDR(args), 0))
                        ...
                        rect(0, 0, xmax, ymax), Titlebar | Centered | Modal | Floating);
...
SEXP do_writeClipboard(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (n > 0)
    {
        ...
        if ((hglb = GlobalAlloc(GHND, len)) && (s = (char *)GlobalLock(hglb)))
        {
            if (raw)
                for (i = 0; i < n; i++)
                    *s++ = RAW(text)[i];
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (getCharCE(el) == CE_UTF8)
        {
            if (!GetFullPathNameW(filenameToWchar(el, FALSE), MAX_PATH, wtmp, &wtmp2) ||
                ...
                errorcall(call, "path[%d]=\"%ls\": %s", i + 1, filenameToWchar(el, FALSE), formatError(GetLastError()));
            wcstoutf8(longpath, wlongpath, wcslen(wlongpath) + 1);
            SET_STRING_ELT(ans, i, mkCharCE(longpath, CE_UTF8));
        }
        else
        {
            if (!GetFullPathName(translateChar(el), MAX_PATH, tmp, &tmp2) || !GetLongPathName(tmp, longpath, MAX_PATH))
...
static int countFilenamesW(const wchar_t *list)
{
    ...
    int count;
    count = 0;
    for (temp = list; *temp; temp += wcslen(temp) + 1)
        count++;
    return count;
}
...
SEXP do_chooseFiles(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < length(filters) / 2; i++)
    {
        ...
        temp += wcslen(temp) + 1;
        wcscpy(temp, filenameToWchar(STRING_ELT(filters, i + length(filters) / 2), 0));
        temp += wcslen(temp) + 1;
    }
    *temp = 0;

    res = askfilenamesW(filenameToWchar(STRING_ELT(caption, 0), 0), path, multi, cfilters, filterindex, NULL);
    ...
    if (count < 2)
        ...
        PROTECT(ans = allocVector(STRSXP, count - 1));
...
SEXP getWindowTitle(void)
{
    ...
    case RGui:
        if (RguiMDI & RW_MDI)
            res = GA_gettext(RFrame);
        else
            res = GA_gettext(RConsole);
        break;
    case RTerm:
...
SEXP do_setTitle(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    checkArity(op, args);
    if (!isString(title) || LENGTH(title) != 1 || STRING_ELT(title, 0) == NA_STRING)
...
SEXP do_setStatusBar(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    checkArity(op, args);
    if (!isString(text) || LENGTH(text) != 1 || STRING_ELT(text, 0) == NA_STRING)
...
static BOOL CALLBACK EnumWindowsProc(HWND handle, LPARAM param)
{
    ...
    {
        if (EnumProcessId)
        { /* restrict to R windows only */
            DWORD processId;
            GetWindowThreadProcessId(handle, &processId);
            if (processId != EnumProcessId)
                return TRUE;
        }
        if (!EnumMinimized && IsIconic(handle))
            return TRUE;
        if (EnumCount >= length(EnumResult))
        {
            int newlen = 2 * length(EnumResult);
            REPROTECT(EnumResult = lengthgets(EnumResult, newlen), EnumIndex);
            setAttrib(EnumResult, R_NamesSymbol, lengthgets(getAttrib(EnumResult, R_NamesSymbol), newlen));
        }
...
static void ArrangeWindows(int n, void **windows, int action, int preserve, int outer)
{
    ...
    else
    {
        ...
        {
        case CASCADE:
            CascadeWindows(parent, 0, prect, n, (HWND FAR *)windows);
            break;
        case TILEHORIZ:
            TileWindows(parent, MDITILE_HORIZONTAL, prect, n, (HWND FAR *)windows);
            break;
        case TILEVERT:
            TileWindows(parent, MDITILE_VERTICAL, prect, n, (HWND FAR *)windows);
            break;
        }
    }
}
...
menu getGraphMenu(const char *menuname)
{
    ...
    if (strcmp(menuname, "Main") == 0)
        return (xd->mbar);
    else if (strcmp(menuname, "Popup") == 0)
        return (xd->grpopup);
    else
        return (NULL);
}
...
int winAccessW(const wchar_t *path, int mode)
{
    ...
    if (mode & X_OK)
        ...
        { /* Directory, so OK */
            ...
            if (p == NULL || !((wcsicmp(p, L".exe") == 0) || (wcsicmp(p, L".com") == 0) || (wcsicmp(p, L".bat") == 0) ||
                               (wcsicmp(p, L".cmd") == 0)))
                return -1;
        }
    {
        ...
        /* get size */
        GetFileSecurityW(path, OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION, 0,
                         0, &size);
        error = GetLastError();
        if (error != ERROR_INSUFFICIENT_BUFFER)
            return -1;
        sdPtr = (SECURITY_DESCRIPTOR *)alloca(size);
        if (!GetFileSecurityW(path, OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION,
                              sdPtr, size, &size))
            return -1;
        /*
        ...
        if (!AccessCheck(sdPtr, hToken, desiredAccess, &genMap, &privSet, &privSetSize, &grantedAccess, &accessYesNo))
        {
            CloseHandle(hToken);
            return -1;
        }
        ...
        if ((mode & W_OK) && !(attr & FILE_ATTRIBUTE_DIRECTORY) && (attr & FILE_ATTRIBUTE_READONLY))
            return -1;
    }
    return 0;
}
...
const static struct
{
    const char *reg;
    HKEY key;
} KeyTable[] = {{"HCC", HKEY_CURRENT_CONFIG},
...
const char *formatError(DWORD res)
{
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, res,
...
static SEXP readRegistryKey(HKEY hkey, int depth)
{
    ...
    if (depth <= 0)
        return mkString("<subkey>");
    res =
        RegQueryInfoKey(hkey, NULL, NULL, NULL, &nsubkeys, &maxsubkeylen, NULL, &nval, &maxvalnamlen, NULL, NULL, NULL);
    if (res != ERROR_SUCCESS)
    ...
    if (nval > 0)
    {
        ...
        {
            size = size0;
            res = RegEnumValueW(hkey, i, (LPWSTR)name, &size, NULL, NULL, NULL, NULL);
            if (res != ERROR_SUCCESS)
                break;
        ...
        /* now sort by name */
        PROTECT(sind = allocVector(INTSXP, nval));
        indx = INTEGER(sind);
        for (i = 0; i < nval; i++)
            indx[i] = i;
    ...
    if (nsubkeys > 0)
    {
        ...
        {
            size = size0;
            res = RegEnumKeyExW(hkey, i, (LPWSTR)name, &size, NULL, NULL, NULL, NULL);
            if (res != ERROR_SUCCESS)
                break;
        ...
        /* now sort by name */
        PROTECT(sind = allocVector(INTSXP, nsubkeys));
        indx = INTEGER(sind);
        for (i = 0; i < nsubkeys; i++)
            indx[i] = i;
...
size_t Rmbrtowc(wchar_t *wc, const char *s)
{
    ...
    else if (byte < 0xF0)
    {
        ...
        {
            *w = (wchar_t)(((byte & 0x0F) << 12) | ((s[1] & 0x3F) << 6) | (s[2] & 0x3F));
            byte = *w;
