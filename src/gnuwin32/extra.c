...
SEXP do_unlink(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < nfiles; i++)
    {
        ...
        {
            strcpy(tmp, dir);
            strcat(tmp, find_data.cFileName);
...
SEXP do_winmenuadd(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    { /* add an item */
        res = winaddmenuitem(CHAR(STRING(sitem)[0]), CHAR(STRING(smenu)[0]), CHAR(STRING(CADDR(args))[0]), errmsg);
        if (res > 0)
        {
...
SEXP do_winmenudel(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    { /* delete an item */
        res = windelmenuitem(CHAR(STRING(sitem)[0]), CHAR(STRING(smenu)[0]), errmsg);
        if (res > 0)
        {
