...
SEXP do_sysinfo(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if ((int)osvi.dwMajorVersion >= 5)
    {
        if (osvi.wServicePackMajor > 0)
            snprintf(ver, 256, "build %d, Service Pack %d", LOWORD(osvi.dwBuildNumber), (int)osvi.wServicePackMajor);
        else
            snprintf(ver, 256, "build %d", LOWORD(osvi.dwBuildNumber));
    }
    else
        snprintf(ver, 256, "build %d, %s", LOWORD(osvi.dwBuildNumber), osvi.szCSDVersion);
...
typedef BOOL(WINAPI *LPFN_GFIBH_EX)(HANDLE, FILE_INFO_BY_HANDLE_CLASS, LPVOID, DWORD);
...
static int isSameFile(HANDLE a, HANDLE b)
{
    ...
    {
        initialized = TRUE;
        gfibh = (LPFN_GFIBH_EX)GetProcAddress(GetModuleHandle(TEXT("kernel32")), "GetFileInformationByHandleEx");
    }
    if (gfibh == NULL)
        return -1;
    ...
    if (!gfibh(a, FileIdInfo, &aid, sizeof(FILE_ID_INFO)) || !gfibh(b, FileIdInfo, &bid, sizeof(FILE_ID_INFO)))
        /* on Vista and Win7 it is expected to fail because FileIdInfo
           is not supported */
        return -1;

    if (aid.VolumeSerialNumber == bid.VolumeSerialNumber && !memcmp(&aid.FileId, &bid.FileId, sizeof(FILE_ID_128)))
...
static Rboolean getFinalPathName(const char *orig, char *res)
{
    ...
    {
        initialized = TRUE;
        gfpnbh = (LPFN_GFPNBH)GetProcAddress(GetModuleHandle(TEXT("kernel32")), "GetFinalPathNameByHandleA");
    }
    if (gfpnbh == NULL)
        return FALSE;
#endif
    ...
    horig = CreateFile(orig, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING,
    ...
    /* FILE_FLAG_BACKUP_SEMANTICS needed to open a directory */
    hres = CreateFile(res, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING,
                      FILE_ATTRIBUTE_HIDDEN | FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
static Rboolean getFinalPathNameW(const wchar_t *orig, wchar_t *res)
{
    ...
    {
        initialized = TRUE;
        gfpnbhw = (LPFN_GFPNBHW)GetProcAddress(GetModuleHandle(TEXT("kernel32")), "GetFinalPathNameByHandleW");
    }
    if (gfpnbhw == NULL)
        return FALSE;
#endif
    ...
    /* FILE_FLAG_BACKUP_SEMANTICS needed to open a directory */
    horig = CreateFileW(orig, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING,
                        FILE_ATTRIBUTE_HIDDEN | FILE_FLAG_BACKUP_SEMANTICS, NULL);
    if (horig == INVALID_HANDLE_VALUE)
        return FALSE;
    ...
    else if (len >= 6 && Ri18n_iswctype(res[4], Ri18n_wctype("alpha")) && res[5] == L':' && res[6] == L'\\')
        /* \\?\D: */
        strip = 4;
    else
    {
    ...
    /* sanity check if the file exists using the normalized path, a normalized
       path to an existing file should still be working */
    /* FILE_FLAG_BACKUP_SEMANTICS needed to open a directory */
    hres = CreateFileW(res, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING,
                       FILE_ATTRIBUTE_HIDDEN | FILE_FLAG_BACKUP_SEMANTICS, NULL);
    if (hres == INVALID_HANDLE_VALUE)
    {
        CloseHandle(horig);
        ...
        else if (getCharCE(el) == CE_UTF8)
        {
            ...
            if (getFinalPathNameW(wel, wtmp))
            {
                ...
                if (norm[0] == L'\\' && norm[1] == L'\\')
                {
                    ...
                    if (res && res <= 32768 && Ri18n_iswctype(wlongpath[0], Ri18n_wctype("alpha")) &&
                        wlongpath[1] == L':')
                    {
            ...
            if (!ok)
            {
                if (mustWork == 1)
                {
                    errorcall(call, "path[%d]=\"%ls\": %s", i + 1, wel, formatError(GetLastError()));
                }
                else if (mustWork == NA_LOGICAL)
                {
                    warningcall(call, "path[%d]=\"%ls\": %s", i + 1, wel, formatError(GetLastError()));
                }
            }
        ...
        else
        {
            ...
            if (getFinalPathName(tel, tmp))
            {
                ...
                if (norm[0] == '\\' && norm[1] == '\\')
                {
                    ...
                    if (res && res <= MAX_PATH && isalpha(longpath[0]) && longpath[1] == ':')
                    {
            ...
            if (!ok)
            {
                if (mustWork == 1)
                {
                    errorcall(call, "path[%d]=\"%s\": %s", i + 1, tel, formatError(GetLastError()));
                }
                else if (mustWork == NA_LOGICAL)
                {
                    warningcall(call, "path[%d]=\"%s\": %s", i + 1, tel, formatError(GetLastError()));
                }
                if (!norm)
                {
                    if (fslash)
