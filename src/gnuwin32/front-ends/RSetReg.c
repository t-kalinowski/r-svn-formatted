...
int main(int argc, char **argv)
{
    ...
    if (delete)
    {
        ...
        if (RegOpenKeyEx(hk, keyname, 0, KEY_SET_VALUE, &hkey) == ERROR_SUCCESS)
        {
            ...
            RegCloseKey(hkey);
            /* delete key if empty */
            if (!subkeys && RegOpenKeyEx(hk, "Software\\R-core", 0, KEY_SET_VALUE, &hkey) == ERROR_SUCCESS)
            {
    ...
    else
    {
        ...
        snprintf(keyname, 60, "Software\\%s\\R", PRODUCER);
        if ((rc = RegOpenKeyEx(hk, keyname, 0, KEY_ALL_ACCESS, &hkey)) != ERROR_SUCCESS)
        {
