...
int main(int argc, char **argv)
{
    ...
    { /* remove the keys */
        ...
        if ((rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, keyname, 0, KEY_SET_VALUE, &hkey)) == ERROR_SUCCESS)
        {
    ...
    {
        RHome = getRHOMElong();
        if ((rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, keyname, 0, KEY_ALL_ACCESS, &hkey)) != ERROR_SUCCESS)
        {
