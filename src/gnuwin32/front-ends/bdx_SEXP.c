 ...
 */
static int EXTPTRSXP2LPSTREAM(RCOM_OBJHANDLE pHandle, LPSTREAM *pStream);
...
int BDX2SEXP(BDX_Data const *pBDXData, SEXP *pSEXPData)
{
    ...
    case BDX_STRING:
        ...
        for (i = 0; i < lTotalSize; i++)
        {
            lStringSEXP = allocString(strlen(pBDXData->data.raw_data[i].string_value));
...
#if 1
            HRESULT lRc = CoUnmarshalInterface(lStream, &IID_IDispatch, (void *)&lDispatch);
#else
            HRESULT lRc = CoGetInterfaceAndReleaseStream(lStream, &IID_IDispatch, (void *)&lDispatch);
#endif
            if (FAILED(lRc))
            {
    ...
    case BDX_SPECIAL:
        ...
        for (i = 0; i < lTotalSize; i++)
        {
            REAL(lData)[i] = getDoubleFromSpecialValue(pBDXData->data.raw_data[i].special_value);
        }
        break;
    case BDX_GENERIC:
        /* generic array */
        ...
        for (i = 0; i < lTotalSize; i++)
        {
            ...
            case BDX_BOOL:
                ...
                LOGICAL(lSEXP)[0] = pBDXData->data.raw_data_with_type[i].raw_data.bool_value;
                break;
            case BDX_INT:
                lSEXP = allocVector(INTSXP, 1);
                INTEGER(lSEXP)[0] = pBDXData->data.raw_data_with_type[i].raw_data.int_value;
                break;
            case BDX_DOUBLE:
                lSEXP = allocVector(REALSXP, 1);
                REAL(lSEXP)[0] = pBDXData->data.raw_data_with_type[i].raw_data.double_value;
                break;
            case BDX_STRING:
                lSEXP = allocVector(STRSXP, 1);
                lStringSEXP = allocString(strlen(pBDXData->data.raw_data_with_type[i].raw_data.string_value));
                strcpy(CHAR(lStringSEXP), pBDXData->data.raw_data_with_type[i].raw_data.string_value);
                SET_STRING_ELT(lSEXP, 0, lStringSEXP);
                break;
            case BDX_SPECIAL:
                lSEXP = allocVector(REALSXP, 1);
                REAL(lSEXP)[0] = getDoubleFromSpecialValue(pBDXData->data.raw_data_with_type[i].raw_data.special_value);
                break;
            default:
                lSEXP = allocVector(REALSXP, 1);
                REAL(lSEXP)[0] = getDoubleFromSpecialValue(BDX_SV_NULL);
                BDX_ERR(printf("unknown BDX type %d in generic vector element %d, using NA\n",
                               pBDXData->data.raw_data_with_type[i].type & BDX_SMASK, i));
            }
...
int SEXP2BDX(struct SEXPREC const *pSexp, BDX_Data **ppBDXData)
{
    ...
    *ppBDXData = 0;
    ...
    if (TYPEOF(lDimension) == INTSXP)
    {
        ...
        lData->dimensions = (BDX_Dimension *)calloc(lData->dim_count, sizeof(BDX_Dimension));
    ...
    if (!lGeneric)
    {
        ...
        lData->data.raw_data = (BDX_RawData *)calloc(lTotalSize, sizeof(BDX_RawData));
    ...
    if (lGeneric)
    {
        ...
        if ((lData->type & BDX_CMASK) == BDX_SCALAR)
        {
            ...
            lData->data.raw_data[0].special_value = getSpecialValueFromDouble(REAL(sexp)[0]);
        }
        else
        {
            /* BDX_GENERIC */
            lData->type = (lData->type & ~BDX_SMASK) | BDX_GENERIC;
            lData->data.raw_data_with_type = (BDX_RawDataWithType *)calloc(lTotalSize, sizeof(BDX_RawDataWithType));
            switch (TYPEOF(sexp))
            {
            case LGLSXP:
                for (i = 0; i < lTotalSize; i++)
                {
                    ...
                    if (lData->data.raw_data_with_type[i].raw_data.special_value == BDX_SV_UNK)
                    {
                        ...
                        lData->data.raw_data_with_type[i].raw_data.double_value = LOGICAL(sexp)[i];
                    }
                }
                break;
            case INTSXP:
            ...
            case REALSXP:
                ...
                {
                    lData->data.raw_data_with_type[i].type = BDX_SPECIAL;
                    lData->data.raw_data_with_type[i].raw_data.special_value = getSpecialValueFromDouble(REAL(sexp)[i]);
                    if (lData->data.raw_data_with_type[i].raw_data.special_value == BDX_SV_UNK)
                    {
                        lData->data.raw_data_with_type[i].type = BDX_DOUBLE;
                        lData->data.raw_data_with_type[i].raw_data.double_value = REAL(sexp)[i];
                    }
                }
                break;
            default:
                for (i = 0; i < lTotalSize; i++)
                {
                    ...
                    case LGLSXP:
                        ...
                        {
                            lData->data.raw_data_with_type[i].type = BDX_BOOL;
                            lData->data.raw_data_with_type[i].raw_data.bool_value = LOGICAL(lElementSexp)[0];
                        }
                        break;
                    case INTSXP:
                        lData->data.raw_data_with_type[i].type = BDX_SPECIAL;
                        ...
                        {
                            lData->data.raw_data_with_type[i].type = BDX_INT;
                            lData->data.raw_data_with_type[i].raw_data.int_value = INTEGER(lElementSexp)[0];
                        }
                        break;
                    case REALSXP:
                        lData->data.raw_data_with_type[i].type = BDX_SPECIAL;
                        ...
                        {
                            lData->data.raw_data_with_type[i].type = BDX_DOUBLE;
                            lData->data.raw_data_with_type[i].raw_data.double_value = REAL(lElementSexp)[0];
                        }
                        break;
                    case EXTPTRSXP:
                        ...
                        /* no EXTPTRSXP for arrays */
                        {
                            ...
                            {
                                lData->type |= BDX_POINTER;
                                lData->data.raw_data_with_type[i].raw_data.ptr = NULL; /* NULL pointer */
                                BDX_TRACE(printf("SEXP2BDX: error %d marshalling COM object at index %d\n", lRc, i));
                            }
                        }
                        break;
...
static int EXTPTRSXP2LPSTREAM(RCOM_OBJHANDLE pHandle, LPSTREAM *pStream)
{
     ...
     * the current one.
     */
    hr = CoMarshalInterThreadInterfaceInStream(&IID_IUnknown, lUnk, &lStream);
    if (FAILED(hr))
    {
