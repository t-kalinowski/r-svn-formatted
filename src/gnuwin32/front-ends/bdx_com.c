...
static int VariantBoolArray2BDX(VARIANT VariantData, BDX_Data *ppBDXData, unsigned int pTotalElements);
static int VariantI2Array2BDX(VARIANT VariantData, BDX_Data *ppBDXData, unsigned int pTotalElements);
static int VariantI4Array2BDX(VARIANT VariantData, BDX_Data *ppBDXData, unsigned int pTotalElements);
static int VariantUI1Array2BDX(VARIANT VariantData, BDX_Data *ppBDXData, unsigned int pTotalElements);
static int VariantR4Array2BDX(VARIANT VariantData, BDX_Data *ppBDXData, unsigned int pTotalElements);
static int VariantR8Array2BDX(VARIANT VariantData, BDX_Data *ppBDXData, unsigned int pTotalElements);
static int VariantErrorArray2BDX(VARIANT VariantData, BDX_Data *ppBDXData, unsigned int pTotalElements);
static int VariantStringArray2BDX(VARIANT VariantData, BDX_Data *ppBDXData, unsigned int pTotalElements);
static int VariantVariantArray2BDX(VARIANT VariantData, BDX_Data *ppBDXData, unsigned int pTotalElements);
...
static int BDXScalar2Variant(BDX_Data *pBDXData, VARIANT *pVariantData)
{
    ...
    case BDX_STRING: {
        ...
        pVariantData->bstrVal = ANSI2BSTR(pBDXData->data.raw_data[0].string_value);
    }
    break;
    case BDX_HANDLE: {
    ...
    case BDX_SPECIAL:
        ...
        V_ERROR(pVariantData) = getSCODEFromSpecialValue(pBDXData->data.raw_data[0].special_value);
        break;
    default:
...
static int BDXGenericArray2Variant(BDX_Data *pBDXData, VARIANT *pVariantData)
{
    ...
    pVariantData->parray = SafeArrayCreate(VT_VARIANT, pBDXData->dim_count, lArrayBounds);
    free(lArrayBounds);
    ...
    for (i = 0; i < lMaxIndex; i++)
    {
        ...
        case BDX_BOOL:
            ...
            V_BOOL(&lVariantArray[i]) =
                pBDXData->data.raw_data_with_type[i].raw_data.bool_value ? VARIANT_TRUE : VARIANT_FALSE;
            break;
        case BDX_INT:
            V_VT(&lVariantArray[i]) = VT_I4;
            V_I4(&lVariantArray[i]) = pBDXData->data.raw_data_with_type[i].raw_data.int_value;
            break;
        case BDX_DOUBLE:
            V_VT(&lVariantArray[i]) = VT_R8;
            V_R8(&lVariantArray[i]) = pBDXData->data.raw_data_with_type[i].raw_data.double_value;
            break;
        case BDX_STRING: {
            V_VT(&lVariantArray[i]) = VT_BSTR;
            V_BSTR(&lVariantArray[i]) = ANSI2BSTR(pBDXData->data.raw_data_with_type[i].raw_data.string_value);
        }
        break;
        case BDX_HANDLE: {
            LPSTREAM lStream = pBDXData->data.raw_data_with_type[i].raw_data.ptr;
            HRESULT lRc = CoUnmarshalInterface(lStream, &IID_IDispatch, (void *)V_DISPATCH(&lVariantArray[i]));
            if (FAILED(lRc))
            {
        ...
        default:
            BDX_ERR(
                printf("unsupported BDX type %08x found at index %d\n", pBDXData->data.raw_data_with_type[i].type, i));
            return -2;
        }
    }
...
int VariantScalar2BDX(VARIANT VariantData, BDX_Data **ppBDXData)
{
    ...
    *ppBDXData = 0;
    ...
    case VT_ERROR: /* special values (e.g. NA, NaN) */
    {
        ...
        lData->data.raw_data[0].special_value = getSpecialValueFromSCODE(lCode);
    }
    break;
    ...
    case VT_DISPATCH: /* COM objects (IDispatch) */
    {
        ...
        lRc = CoMarshalInterThreadInterfaceInStream(&IID_IUnknown, (LPUNKNOWN)V_DISPATCH(&VariantData), &lStream);
...
static int VariantBoolArray2BDX(VARIANT VariantData, BDX_Data *pBDXData, unsigned int pTotalElements)
{
    /* access the data */
    VARIANT_BOOL HUGEP *lBoolArray;
    SAFEARRAY *lArray;
...
/* 04-05-12 | baier | handle VT_BYREF */
static int VariantI2Array2BDX(VARIANT VariantData, BDX_Data *pBDXData, unsigned int pTotalElements)
{
    /* access the data */
...
/* 04-05-12 | baier | handle VT_BYREF */
static int VariantI4Array2BDX(VARIANT VariantData, BDX_Data *pBDXData, unsigned int pTotalElements)
{
    /* access the data */
    long HUGEP *lIntArray;
    SAFEARRAY *lArray;
...
/* 04-05-12 | baier | handle VT_BYREF */
static int VariantUI1Array2BDX(VARIANT VariantData, BDX_Data *pBDXData, unsigned int pTotalElements)
{
    /* access the data */
...
/* 04-05-12 | baier | handle VT_BYREF */
static int VariantR4Array2BDX(VARIANT VariantData, BDX_Data *pBDXData, unsigned int pTotalElements)
{
    /* access the data */
...
/* 04-05-12 | baier | handle VT_BYREF */
static int VariantR8Array2BDX(VARIANT VariantData, BDX_Data *pBDXData, unsigned int pTotalElements)
{
    /* access the data */
...
/* 05-06-08 | baier | new */
static int VariantErrorArray2BDX(VARIANT VariantData, BDX_Data *pBDXData, unsigned int pTotalElements)
{
    /* access the data */
    SCODE HUGEP *lErrorArray;
    SAFEARRAY *lArray;
    ...
    for (i = 0; i < pTotalElements; i++)
    {
        pBDXData->data.raw_data[i].special_value = getSpecialValueFromSCODE(lErrorArray[i]);
    }
...
/* 04-05-12 | baier | handle VT_BYREF */
static int VariantStringArray2BDX(VARIANT VariantData, BDX_Data *pBDXData, unsigned int pTotalElements)
{
    /* access the data */
    BSTR HUGEP *lStringArray;
    SAFEARRAY *lArray;
...
static int VariantVariantArray2BDX(VARIANT VariantData, BDX_Data *pBDXData, unsigned int pTotalElements)
{
    ...
    if (lCoerceTo != VT_ERROR)
    {
        for (i = 0; i < pTotalElements; i++)
        {
            ...
            VariantChangeTypeEx(&lTmpVariant, &lVariantArray[i], MAKELCID(MAKELANGID(LANG_ENGLISH, SUBLANG_NEUTRAL), 0),
                                VARIANT_ALPHABOOL, lCoerceTo);
            switch (lCoerceTo)
            {
            case VT_BOOL:
        ...
        for (i = 0; i < pTotalElements; i++)
        {
            ...
            case VT_ERROR:
                pBDXData->data.raw_data_with_type[i].type = BDX_SPECIAL;
                {
                    ...
                    pBDXData->data.raw_data_with_type[i].raw_data.special_value = getSpecialValueFromSCODE(sc);
                }
                break;
            default:
                BDX_ERR(printf("VariantVariantArray2BDX: internal error (#1, type %d at %i)\n", V_VT(&lVariantArray[i]),
                               i));
                break;
            }
        }
