...
int main(int argc, char **argv)
{
    ...
    if (access("unins000.dat", 4) == 0)
        read_unist_file("unins000.dat");
    ...
    while (fgets(line, 500, fp))
    {
        ...
        if (strcmp(line + 34, "etc/Rconsole") == 0)
            continue;
        if (strcmp(line + 34, "etc/Rprofile.site") == 0)
            continue;
        if (line[33] == '*')
    ...
    if (wrong)
        fprintf(stderr, "WARNING: %d files changed\n", wrong);
    if (wrong || miss)
    {
