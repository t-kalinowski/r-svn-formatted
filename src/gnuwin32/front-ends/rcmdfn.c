...
int rcmdfn(int cmdarg, int argc, char **argv)
{
    ...
    if (strncmp(argv[0] + len - 4, "Rcmd", 4) == 0 || strncmp(argv[0] + len - 4, "rcmd", 4) == 0 ||
        strncmp(argv[0] + len - 8, "Rcmd.exe", 8) == 0 || strncmp(argv[0] + len - 8, "rcmd.exe", 8) == 0)
        strcpy(RCMD, "Rcmd");
    ...
    else
    {
        ...
        if (cmdarg > 0 && argc > cmdarg)
        {
            ...
            else if (strcmp(".bat", p + strlen(p) - 4) == 0)
                strcpy(cmd, "");
            else if (strcmp(".exe", p + strlen(p) - 4) == 0)
                strcpy(cmd, "");
            else
            {
                strcpy(cmd, "perl ");
