...
int rcmdfn(int cmdarg, int argc, char **argv)
{
    ...
    char *RHome, PERL5LIB[MAX_PATH], TEXINPUTS[MAX_PATH], BUFFER[10000], RHOME[MAX_PATH], *p, cmd[CMD_LEN],
        Rversion[25], HOME[MAX_PATH + 10], RSHARE[MAX_PATH];
    char RCMD[] = "R CMD";
    ...
    else
    {
        ...
        strcat(PERL5LIB, RHome);
        strcat(PERL5LIB, "\\share\\perl;");
        if ((p = getenv("PERL5LIB")))
            strcat(PERL5LIB, p);
        putenv(PERL5LIB);
        ...
        if (cmdarg > 0 && argc > cmdarg)
        {
            ...
            else
            {
                if (!strcmp(".sh", p + strlen(p) - 3))
                {
                    ...
                    strcpy(cmd, "");
                else if (!strcmp(".exe", p + strlen(p) - 4))
                    strcpy(cmd, "");
                else
                {
                    strcpy(cmd, "perl ");
