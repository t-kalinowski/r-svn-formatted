    ...
    *pData = lData;
    ...
    case NILSXP:
      ...
      printf (">> %s is a NULL value\n",
        ...
        lData->raw_data = (BDX_RawData *)malloc(sizeof(BDX_RawData));
    ...
    else
    {
	  ...
	  printf (">> %s is a vector of length %d\n",
		  pSymbol,LENGTH (pExpression));
            ...
            lData->raw_data = (BDX_RawData *)malloc(sizeof(BDX_RawData) * lData->dimensions[0]);
        ...
        else
        {
            ...
            if (TYPEOF(lDimension) == INTSXP)
            {
                ...
                lData->raw_data = (BDX_RawData *)malloc(sizeof(BDX_RawData) * lTotalSize);
...
int R_Proxy_set_symbol(char const *pSymbol, BDX_Data const *pData)
{
    ...
    case BDX_SCALAR: {
        ...
        case BDX_DOUBLE:
            ...
            break;
        case BDX_STRING: {
            SEXP lStringSExp = allocString(strlen(pData->raw_data[0].string_value));
            PROTECT(lStringSExp);
            lProtectCount++;
    ...
    case BDX_ARRAY: {
        ...
        case BDX_STRING: {
            ...
            for (i = 0; i < lTotalSize; i++)
            {
                SEXP lStringSExp = allocString(strlen(pData->raw_data[i].string_value));
                PROTECT(lStringSExp);
                lProtectCount++;
