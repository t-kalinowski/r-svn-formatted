...
bitmap imagetobitmap(image img)
{
    ...
    screendc = GetDC(0); /* get screen dc */
    hb = CreateDIBitmap(screendc, &bmi->bmiHeader, CBM_INIT, data, bmi, DIB_RGB_COLORS);
...
void getbitmapdata2(bitmap obj, unsigned char **data, int *row_bytes)
{
    ...
    *data = (unsigned char *)malloc(size);
    if (*data)
    {
        ret = GetDIBits(get_context(obj), (HBITMAP)obj->handle, 1, r.height, (LPSTR)*data, &bmi, DIB_RGB_COLORS);
        if (!ret)
        {
            free(*data);
            *data = NULL;
