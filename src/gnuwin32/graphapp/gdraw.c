...
typedef BOOL(WINAPI *alpha_blend_t)(HDC hdcDest, int nXOriginDest, int nYOriginDest, int nWidthDest, int hHeightDest,
...
static int haveAlpha()
{
    ...
    if (haveAlphaBlend < 0)
    {
        ...
        GetVersionEx(&os);
        if (VER_PLATFORM_WIN32_WINDOWS != os.dwPlatformId || os.dwMajorVersion != 4 || os.dwMinorVersion != 10)
        {
...
rgb ggetpixel(drawing d, point p)
{
    ...
    c = ((c & 0x000000FFL) << 16) | (c & 0x0000FF00L) | ((c & 0x00FF0000L) >> 16);
    return c;
}
...
void gdrawline(drawing d, int width, int style, rgb c, point p1, point p2, int fast, int lend, int ljoin, float lmitre)
{
    point p[2];
    p[0] = p1;
    p[1] = p2;
...
void gdrawpolyline(drawing d, int width, int style, rgb c, point p[], int n, int closepath, int fast, int lend,
                   int ljoin, float lmitre)
{
    int tmpx, tmpy, tmp;
    HDC dc = GETHDC(d);
...
void gdrawrect(drawing d, int width, int style, rgb c, rect r, int fast, int lend, int ljoin, float lmitre)
{
    int x0 = r.x;
    int y0 = r.y;
    int x1 = r.x + r.width;
...
void gdrawellipse(drawing d, int width, rgb border, rect r, int fast, int lend, int ljoin, float lmitre)
{
    HDC dc = GETHDC(d);
    LOGBRUSH lb;
    HPEN gpen;
...
void gcharmetric(drawing d, font f, int c, int *ascent, int *descent, int *width)
{
    ...
    else if ((first <= c) && (c <= last))
    {
        ...
        */
        if ((c != ' ') && (tm.tmPitchAndFamily & TMPF_TRUETYPE))
        {
            ...
            m2.eM11.fract = m2.eM12.fract = m2.eM21.fract = m2.eM22.fract = (short)0;
        ...
        else if ((first <= c) && (c <= last))
        {
            ...
            */
            if ((c != ' ') && (tm.tmPitchAndFamily & TMPF_TRUETYPE))
            {
                ...
                m2.eM21.value = m2.eM12.value = (WORD)0;
                m2.eM11.fract = m2.eM12.fract = m2.eM21.fract = m2.eM22.fract = (short)0;
                if (GetGlyphOutlineW(dc, c, GGO_METRICS, &gm, 0, NULL, &m2) != GDI_ERROR)
                {
                    *descent = gm.gmBlackBoxY - gm.gmptGlyphOrigin.y;
                    *ascent = gm.gmptGlyphOrigin.y + 1;
                }
    ...
    else
    {
        if (c > 127)
        {
            ...
            *width = tm.tmAveCharWidth;
            if (wctomb(s, (wchar_t)c) >= 1 && GetTextExtentPoint32(dc, (LPSTR)s, 1, &size))
            {
                *ascent = size.cy - *descent - extra;
                *width = size.cx;
            }
...
font gnewfont(drawing d, const char *face, int style, int size, double rot)
{
    ...
    if (d && ((d->kind == PrinterObject) || (d->kind == MetafileObject)))
    {
        TEXTMETRIC tm;
...
#define MEASUREDEV(a)                                                                                                  \
    {                                                                                                                  \
        return measuredev(dev, a);                                                                                     \
    }

int devicewidth(drawing dev) MEASUREDEV(HORZRES) int deviceheight(drawing dev)
    MEASUREDEV(VERTRES) int devicewidthmm(drawing dev) MEASUREDEV(HORZSIZE) int deviceheightmm(drawing dev)
        MEASUREDEV(VERTSIZE) int devicepixelsx(drawing dev) MEASUREDEV(LOGPIXELSX) int devicepixelsy(drawing dev)
            MEASUREDEV(LOGPIXELSY)
...
void BringToTop(window c, int stay) /* stay=0 for regular, 1 for topmost, 2 for toggle */
{
    ...
    if (stay)
        SetWindowPos(c->handle, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
    else
        SetWindowPos(c->handle, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
    TopmostDialogs &= !MB_TOPMOST;
...
static cp2charset_table cp2charset[] = {
    {874, THAI_CHARSET},        {932, SHIFTJIS_CHARSET},    {936, GB2312_CHARSET},   {949, HANGUL_CHARSET},
    {950, CHINESEBIG5_CHARSET}, {1250, EASTEUROPE_CHARSET}, {1251, RUSSIAN_CHARSET}, {1252, ANSI_CHARSET},
    {1253, GREEK_CHARSET},      {1254, TURKISH_CHARSET},    {1255, HEBREW_CHARSET},  {1256, ARABIC_CHARSET},
    {1257, BALTIC_CHARSET},     {1258, VIETNAMESE_CHARSET}, {1361, JOHAB_CHARSET},
};
