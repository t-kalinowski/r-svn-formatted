...
{
    FILE *file;
    int depth, clear_code, eof_code, running_code, running_bits, max_code_plus_one, prev_code, current_code, stack_ptr,
        shift_state;
...
static int read_gif_code(FILE *file, GifDecoder *decoder)
{
    ...
    while (decoder->shift_state < decoder->running_bits)
    {
        ...
        decoder->shift_data |= ((unsigned long)next_byte) << decoder->shift_state;
    ...
    if (++decoder->running_code > decoder->max_code_plus_one && decoder->running_bits < LZ_BITS)
    {
            ...
            else
            {
                ...
                if (prefix[current_code] == NO_SUCH_CODE)
                {
                    ...
                    if (current_code == decoder->running_code - 2)
                    {
                        ...
                        suffix[decoder->running_code - 2] = stack[stack_ptr++] =
 ...
 */
static void write_gif_code(FILE *file, GifEncoder *encoder, int code)
{
 ...
 */
static void init_gif_encoder(FILE *file, GifEncoder *encoder, int depth)
{
 ...
 */
static void write_gif_line(FILE *file, GifEncoder *encoder, byte *line, int length)
{
...
image load_gif(char *filename)
{
    ...
    {
        if ((ext->data) && (len(ext->data[0]) == 4) && (ext->data[0][0] & 0x01)) /* transparent flag */
        {
