...
extern int do_unzip(char *zipname, char *dest, int nfiles, char **files, int nxfiles, char **xfiles, int over);
...
void next2(button b)
{
    ...
    if (!rwb)
    {
        ...
        {
            delobj(lwarn2);
            lwarn2 = newlabel("R is not yet installed: please install it first", rect(10, 210, 390, 20), AlignLeft);
            return;
        }
    }
        ...
        if (!p)
        {
            ...
            for (p = getenv("PATH"); p;)
            {
                ...
                if ((q = strchr(buf, ';')))
                    *q = '\0';
                dosslash(buf);
                if (strlen(buf) > 10 && stricmp(buf + strlen(buf) - 10, buf2) == 0)
                {
                    buf[strlen(buf) - 4] = '\0';
                    break;
                }
...
void page1()
{
    ...
    ypos = 50;
    lwhat1 = newlabel("What do you want to install?", rect(10, ypos, 200, 20), AlignLeft);
    sys = newradiobutton("R version", rect(100, ypos + 30, 75, 20), cSys);
    fRver = newfield(Rver, rect(180, ypos + 30, 75, 20));
    pkg = newradiobutton("An add-on package", rect(100, ypos + 60, 200, 20), cPkg);
    if (FullInstall)
        check(sys);
    else
    {
    ...
    ypos = 200;
    ldest = newlabel("Install directory:", rect(10, ypos + 2, 80, 20), AlignRight);
...
void page2()
{
    ...
    lwhat2 = newlabel("Select the components you want to install", rect(10, 50, 390, 20), AlignLeft);
    ...
    ypos += 17;
    chmhelp = newcheckbox("Compiled HTML help", rect(xpos, ypos, 150, 20), NULL);
    strcpy(str, Rver);
    strcat(str, "ch.zip");
    if (!fexists(str))
    {
    ...
    ypos += 17;
    ltxhelp = newcheckbox("latex help (for off-line printing)", rect(xpos, ypos, 300, 20), NULL);
    strcpy(str, Rver);
    strcat(str, "l.zip");
    if (!fexists(str))
    {
    ...
    ypos += 17;
    srcsp = newcheckbox("files for building packages from source", rect(xpos, ypos, 300, 20), NULL);
    strcpy(str, Rver);
    strcat(str, "sp.zip");
    if (!fexists(str))
    {
        uncheck(srcsp);
        disable(srcsp); /* hide(srcsp);*/
    }
    else
    {
        if (rwsp)
    ...
    ypos += 17;
    pdf = newcheckbox("PDF manuals", rect(xpos, ypos, 150, 20), NULL);
    strcpy(str, Rver);
    strcat(str, "d1.zip");
    if (!fexists(str))
    {
        uncheck(pdf);
        disable(pdf); /* hide(pdf);*/
    }
    else
    {
        if (rwd)
    ...
    refpdf = newcheckbox("reference manual", rect(xpos + 150, ypos, 150, 20), NULL);
    strcpy(str, Rver);
    strcat(str, "d2.zip");
    if (!fexists(str))
    {
        uncheck(refpdf);
        disable(refpdf); /* hide(pdf);*/
    }
    else
    {
        if (rwd2)
    ...
    ypos += 25;
    lwarn2 = newlabel("Components that are not found will be greyed out", rect(10, ypos, 390, 20), AlignLeft);

    enable(bBack);
    setaction(bBack, back2);
    show(bBack);
    hide(bFinish);
    enable(bCancel);
    show(bCancel);
    enable(bNext);
    setaction(bNext, next2);
    show(bNext);
    if (zips == 0)
    {
        disable(bNext);
        delobj(lwarn2);
        lwarn2 = newlabel("No components found in the source directory", rect(10, ypos, 390, 20), AlignLeft);
        return;
    }
    if (!isenabled(basepkg))
    {
        ...
        {
            delobj(lwarn2);
            lwarn2 = newlabel("R is not yet installed: please install it first", rect(10, ypos, 390, 20), AlignLeft);
            disable(bNext);
        }
    }
...
void pagepkg1()
{
    ...
    else
    {
        while ((de = readdir(dir)))
        {
            ...
            if (!strcmp(p, "rw"))
                continue;
            if (strcmp(p1, ".zip") && strcmp(p1, ".ZIP"))
                continue;
...
void pagepkg2()
{
    ...
    lwhat3 = newlabel("Select the components you want to install", rect(10, 50, 390, 20), AlignLeft);
...
void pagepkg3()
{
    ...
    strcpy(unztext, "installing ");
    strcat(unztext, selpkg);
    strcat(unztext, "\n\n");
...
{
    w = newwindow("R for Windows installer", rect(0, 0, 400, 300), StandardWindow & ~Maximize);
