...
static xbuf file2xbuf(const char *name, int enc, int del)
{
    ...
    if (enc == CE_UTF8)
    {
        ...
        f = CreateFileW(wfn, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
    }
    else
...
static int pageraddfile(const char *wtitle, const char *filename, int enc, int deleteonexit)
{
    ConsoleData p = getdata(pagerInstance);
    int i;
...
static pager pagercreate(void)
{
    ...
    p = newconsoledata((consolefn) ? consolefn : FixedFont, pagerrow, pagercol, 0, 0, guiColors, PAGER, 0, 0);
    if (!p)
        return NULL;
...
static pager newpager1win(const char *wtitle, const char *filename, int enc, int deleteonexit)
{
    ...
    if (!pageraddfile(wtitle, filename, enc, deleteonexit))
        return NULL;
    pagerupdateview();
    return pagerInstance;
}

static pager newpagerNwin(const char *wtitle, const char *filename, int enc, int deleteonexit)
{
    pager c = pagercreate();
    ConsoleData p;
...
pager newpager(const char *title, const char *filename, int enc, const char *header, int deleteonexit)
{
