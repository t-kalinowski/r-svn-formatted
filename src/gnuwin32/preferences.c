...
static void do_apply()
{
    ...
    {
        if (!haveusedapager || askokcancel(_("Do not change pager type if any pager is open\nProceed?")) == YES)
...
static void save(button b)
{
    ...
    file = askfilesavewithdir(_("Select directory for file 'Rconsole'"), "Rconsole", buf);
    if (!file)
        return;
    strcpy(buf, file);
    p = buf + strlen(buf) - 2;
...
void Rgui_configure()
{
    ...
    wconfig = newwindow(_("Rgui Configuration Editor"), rect(0, 0, 550, 450), Titlebar | Centered | Modal);
    setbackground(wconfig, dialog_bg());
    l_mdi = newlabel(_("Single or multiple windows"), rect(10, 10, 150, 20), AlignLeft);
