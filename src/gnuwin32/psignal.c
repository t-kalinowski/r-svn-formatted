...
static int downhill_Signal_Init(void)
{
    ...
    {
        if (!(downhill_Signal_Info = calloc(sizeof(struct downhill_Signal_Struct), NSIG)) ||
            !(IGotASignal = CreateEvent(NULL, FALSE, FALSE, NULL)) || !SetConsoleCtrlHandler(hwIntrHandler, TRUE))

        {
            if (downhill_Signal_Info)
                free(downhill_Signal_Info);
            if (IGotASignal)
                CloseHandle(IGotASignal);
            errno = ENOMEM;
            return FALSE;
        }
    }
