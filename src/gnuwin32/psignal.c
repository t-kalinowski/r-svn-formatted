...
static void PrivateSignalHandler(int signal_Number)
{
    ...
    else
    {
        ...
        if (downhill_Signal_Info[signal_Number].signal_Handler == SIG_DFL)
        {
        ...
        else if ((downhill_Signal_Info[signal_Number].signal_Handler == SIG_IGN))
        { /* IGNORE */
        }
        else
        {
            ...
            signal_HandlerOld = downhill_Signal_Info[signal_Number].signal_Handler;
            if (downhill_Signal_Info[signal_Number].signal_Flags & SA_RESETHAND)
            {
                downhill_Signal_Info[signal_Number].signal_Handler = SIG_DFL;
            }
            /* Do the action */
            if ((signal_Number == SIGCHLD) && (downhill_Signal_Info[signal_Number].signal_Flags & SA_NOCLDSTOP))
            {
                /* Ignore SIGCHLD */
            }
            else
            {
                ...
                sigset_t sigset_MaskNew = downhill_Signal_Info[signal_Number].signal_Mask;
...
static int downhill_Signal_Init(void)
{
    ...
    if (downhill_Signal_Info == NULL)
    {
        ...
        /* Get some memory */
        downhill_Signal_Info = calloc(sizeof(struct downhill_Signal_Struct), NSIG);
        if (downhill_Signal_Info == NULL)
        {
            errno = ENOMEM;
            return FALSE;
        ...
        {
            if ((downhill_Signal_Info[signal_Index].signal_Handler = signal(signal_Index, PrivateSignalHandler)) ==
                SIG_ERR)
                downhill_Signal_Info[signal_Index].signal_Handler = SIG_IGN;
        }
    }
    return TRUE;
}
...
/* Set a signal action ===================================================== */
int sigaction(int signal_Number, struct sigaction *sigaction_Info, struct sigaction *sigaction_InfoOld)
{
    ...
    if (IS_SIGNAL(signal_Number))
    {
        ...
        {
            sigaction_InfoOld->sa_handler = downhill_Signal_Info[signal_Number].signal_Handler;
            sigaction_InfoOld->sa_mask = downhill_Signal_Info[signal_Number].signal_Mask;
            sigaction_InfoOld->sa_flags = downhill_Signal_Info[signal_Number].signal_Flags;
        }
        if (sigaction_Info != NULL)
        {
            downhill_Signal_Info[signal_Number].signal_Handler = sigaction_Info->sa_handler;
            downhill_Signal_Info[signal_Number].signal_Count = 0;
            downhill_Signal_Info[signal_Number].signal_Extra = 0;
            downhill_Signal_Info[signal_Number].signal_Mask = sigaction_Info->sa_mask;
            downhill_Signal_Info[signal_Number].signal_Flags = sigaction_Info->sa_flags;
        }
    }
    else
    {
...
sighandler_t _psignal(int signal_Number, sighandler_t signal_Handler)
{
    ...
    signal_HandlerOld = downhill_Signal_Info[signal_Number].signal_Handler;
    downhill_Signal_Info[signal_Number].signal_Handler = signal_Handler;
    downhill_Signal_Info[signal_Number].signal_Count = 0;
    downhill_Signal_Info[signal_Number].signal_Extra = 0;
...
int sigprocmask(int mask_Function, sigset_t *sigset_Info, sigset_t *sigset_InfoOld)
{
    int signal_Index;
    ...
    for (signal_Index = 1; signal_Index < NSIG; signal_Index++)
    {
        if ((!sigismember(&downhill_Sigset_Mask, signal_Index)) && (sigismember(&sigset_MaskOld, signal_Index)) &&
