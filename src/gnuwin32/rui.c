...
static menuitem msource, mdisplay, mload, msave, mloadhistory, msavehistory, mpaste, mpastecmds, mcopy, mcopypaste,
    mlazy, mcomplete, mfncomplete, mconfig, mls, mrm, msearch, mde, mtools, mstatus;
static int lmanintro, lmanref, lmandata, lmanlang, lmanext, lmanint, lmanadmin, lmanSweave;
static menu m;
...
void readconsolecfg()
{
    ...
    if (!strcmp(gui.style, "normal"))
        sty = Plain;
    if (!strcmp(gui.style, "bold"))
        sty = Bold;
    if (!strcmp(gui.style, "italic"))
        sty = Italic;
...
int RguiPackageMenu(PkgMenuItems pmenu)
{
    ...
    MCHECK(pmenu->mpkgm = newmenuitem(G_("Set CRAN mirror..."), 0, menupkgcranmirror));
    MCHECK(pmenu->mrepos = newmenuitem(G_("Select repositories..."), 0, menupkgrepos));
    MCHECK(pmenu->mpkgi = newmenuitem(G_("Install package(s)..."), 0, menupkginstallpkgs));
    MCHECK(pmenu->mpkgu = newmenuitem(G_("Update packages..."), 0, menupkgupdate));
    MCHECK(newmenuitem("-", 0, NULL));
    MCHECK(pmenu->mpkgil = newmenuitem(G_("Install package(s) from local files..."), 0, menupkginstalllocal));
    return 0;
}
...
int RguiCommonHelp(menu m, HelpMenuItems hmenu)
{
    ...
    if (!check_doc_file("doc\\manual\\R-FAQ.html"))
        disable(hmenu->mFAQ);
    ...
    if (!lmanintro && !lmanref && !lmandata && !lmanlang && !lmanext && !lmanint && !lmanadmin && !lmanSweave)
    {
    ...
    else
    {
        ...
        MCHECK(hmenu->mmanintro = newmenuitem("An &Introduction to R", 0, menumainman));
        if (!lmanintro)
            disable(hmenu->mmanintro);
        MCHECK(hmenu->mmanref = newmenuitem("R &Reference", 0, menumainref));
        if (!lmanref)
            disable(hmenu->mmanref);
        MCHECK(hmenu->mmandata = newmenuitem("R Data Import/Export", 0, menumaindata));
        if (!lmandata)
            disable(hmenu->mmandata);
        MCHECK(hmenu->mmanlang = newmenuitem("R Language Definition", 0, menumainlang));
        if (!lmanlang)
            disable(hmenu->mmanlang);
        ...
        MCHECK(hmenu->mmanSweave = newmenuitem("Sweave User", 0, menumainSweave));
        if (!lmanSweave)
            disable(hmenu->mmanSweave);
    }
    ...
    if (!check_doc_file("doc\\html\\index.html"))
        disable(hmenu->mhelpstart);
    MCHECK(hmenu->mhelpsearch = newmenuitem(G_("Search help..."), 0, menuhelpsearch));
    MCHECK(hmenu->msearchRsite = newmenuitem("search.r-project.org ...", 0, menusearchRsite));
    MCHECK(newmenuitem("-", 0, NULL));
...
int setupui(void)
{
    ...
    if (RguiMDI & RW_MDI)
    {
        ...
        RFrame = newwindow(
            ...
            "RGui (32-bit)",
#endif
            MDIsize, StandardWindow | Menubar | Workspace);
    ...
    else
    {
        ...
        if (!(RConsole = newconsole("R Console (64-bit)", flags)))
            return 0;
#else
        if (!(RConsole = newconsole("R Console (32-bit)", flags)))
            return 0;
#endif
        TRACERUI("Console done");
    }
    ...
    if (ismdi())
    {
        ...
        if (RguiMDI & RW_TOOLBAR)
            check(mtools);
        if (RguiMDI & RW_STATUSBAR)
            check(mstatus);
    }
...
menuItems *wingetmenuitems(const char *mname, char *errmsg)
{
    ...
    items->numItems = j;
    if (j == 0)
        sprintf(errmsg, G_("menu %s does not exist"), mname);
...
int winaddmenu(const char *name, char *errmsg)
{
    ...
    if (nmenus >= alloc_menus)
    {
        ...
        else
        {
            ...
            usermenunames = (char **)realloc(usermenunames, sizeof(char *) * alloc_menus);
        }
    }
    if (strlen(name) > 500)
    {
    ...
    else
    {
        ...
        if (m)
        {
            if (alloc_items <= nitems)
            {
                ...
                {
                    alloc_items += 100;
                    umitems = (Uitem *)realloc(umitems, sizeof(Uitem) * alloc_items);
                }
            }
