                    ...
                    pinfo *pi)
{
    ...
    */
    breakaway = FALSE;
    if (!IsWindows8OrGreater() && IsProcessInJob(GetCurrentProcess(), NULL, &inJob) && inJob)
    {
        ...
        ret = QueryInformationJobObject(NULL, JobObjectExtendedLimitInformation, &jeli,
                                        sizeof(JOBOBJECT_EXTENDED_LIMIT_INFORMATION), NULL);
        breakaway = ret && (jeli.BasicLimitInformation.LimitFlags & JOB_OBJECT_LIMIT_BREAKAWAY_OK);
    }
    ...
    if (job)
    {
        ...
        ret = SetInformationJobObject(job, JobObjectExtendedLimitInformation, &jeli,
    ...
    if (job)
    {
        ...
        ret = SetInformationJobObject(job, JobObjectAssociateCompletionPortInformation, &cport,
    ...
    if (enc == CE_UTF8)
    {
        ...
        ret = CreateProcessW(NULL, wcmd, &sa, &sa, TRUE, flags, NULL, NULL, &wsi, &(pi->pi));
    }
    else
        ret = CreateProcess(NULL, ecmd, &sa, &sa, TRUE, flags, NULL, NULL, &si, &(pi->pi));
...
static void waitForJob(pinfo *pi, DWORD timeoutMillis, int *timedout)
{
    ...
    for (;;)
    {
        ret = GetQueuedCompletionStatus(pi->port, &code, &key, &overlapped, queryMillis);
        if (ret && code == JOB_OBJECT_MSG_ACTIVE_PROCESS_ZERO && (HANDLE)key == pi->job)
            break;
        ...
        ret = QueryInformationJobObject(pi->job, JobObjectBasicAccountingInformation, &jbai,
                   ...
                   const char *ferr, int timeout, int *timedout)
{
    ...
    if (fout && fout[0])
        close2 = 1;
    if (fout && fout[0] && ferr && streql(fout, ferr))
        hERR = hOUT;
    else
    {
                 ...
                 const char *ferr, int timeout)
{
    ...
    hTHIS = GetCurrentProcess();
    r->write = hWritePipe;
    DuplicateHandle(hTHIS, hReadPipe, hTHIS, &r->read, 0, FALSE, DUPLICATE_SAME_ACCESS);
...
static size_t Wpipe_read(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    rpipe *rp = ((RWpipeconn)con->private)->rp;
    DWORD ntoread, read;
...
static size_t Wpipe_write(const void *ptr, size_t size, size_t nitems, Rconnection con)
{
    rpipe *rp = ((RWpipeconn)con->private)->rp;
