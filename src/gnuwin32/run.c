...
static HANDLE pcreate(const char *cmd, const char *finput, int newconsole, int visible, int inpipe)
{
    DWORD ret;
    ...
    ret = CreateProcess(NULL, ecmd, &sa, &sa, TRUE, (newconsole && (visible == 1)) ? CREATE_NEW_CONSOLE : 0, NULL, NULL,
                        &si, &pi);
    CloseHandle(hTHIS);
    if (finput && finput[0])
    {
