...
void R_getProcTime(double *data)
{
    ...
    user = 1e-5 * ((double)User.dwLowDateTime + (double)User.dwHighDateTime * 4294967296.0);
    user = floor(user) / 100.0;
    kernel = 1e-5 * ((double)Kernel.dwLowDateTime + (double)Kernel.dwHighDateTime * 4294967296.0);
...
SEXP do_system(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    {
        if (flag == 2)
            flag = 1; /* ignore std.output.on.console */
        if (TYPEOF(Stdout) == STRSXP)
            fout = CHAR(STRING_ELT(Stdout, 0));
 ...
 show.output.on.console for Rgui */
    ...
    else
    {
        ...
        if (m && TYPEOF(Stdout) == LGLSXP && asLogical(Stdout))
            m = 3;
        fp = rpipeOpen(CHAR(STRING_ELT(cmd, 0)), getCharCE(STRING_ELT(cmd, 0)), vis, CHAR(STRING_ELT(fin, 0)), m, fout,
                       ferr, timeout);
        if (!fp)
        {
    ...
    {
        ll = 124;
        warning(_("command '%s' timed out after %ds"), CHAR(STRING_ELT(cmd, 0)), timeout);
    }
    else if (flag == 3 && ll)
    {
        warning(_("running command '%s' had status %d"), CHAR(STRING_ELT(cmd, 0)), ll);
    }
    if (flag == 3)
