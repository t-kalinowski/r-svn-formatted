...
void R_WriteConsole(const char *buf, int len)
{
    R_ProcessEvents();
    if (ptr_WriteConsole)
        ptr_WriteConsole(buf, len);
    else
        ptr_WriteConsoleEx(buf, len, 0);
}
...
void R_WriteConsoleEx(const char *buf, int len, int otype)
{
    ...
    if (ptr_WriteConsole)
        ...
        ptr_WriteConsoleEx(buf, len, otype);
}
...
{
    completionrequest.result = InThreadTabHook(completionrequest.buf, completionrequest.offset, completionrequest.loc);
}
...
static int ReaderThreadTabHook(char *buf, int offset, int *loc)
{
    ...
    SendMessage(ReadMsgWindow, WM_RREADMSG_EVENT, 0, (LPARAM)2 /* completion needed */);
...
static void __cdecl ReaderThread(void *unused)
{
    while (1)
    {
        ...
        SendMessage(ReadMsgWindow, WM_RREADMSG_EVENT, 0, (LPARAM)1 /* line available */);
    }
}
...
static void Rstd_CleanUp(SA_TYPE saveact, int status, int runLast)
{
    ...
    if (R_CollectWarnings && saveact != SA_SUICIDE && CharacterMode == RTerm) /* no point in doing this for Rgui
...
void R_SetWin32(Rstart Rp)
{
    ...
    default:
        R_GUIType = "unknown";
        EmitEmbeddedUTF8 = (GetACP() != 65001) && (Rp->EmitEmbeddedUTF8 == TRUE);
    }
    ...
    */
    ...
    for (i = 1; i < ac; i++)
        ...
        else if (!strcmp(av[i], "--cd-to-userdocs"))
        {
            ...
            if (SUCCEEDED(SHGetFolderPathW(NULL, CSIDL_PERSONAL | CSIDL_FLAG_CREATE, NULL, 0, mydocs)))
        ...
        if (processing && **++av == '-')
        {
            ...
            else if (CharacterMode == RTerm && !strcmp(*av, "-f"))
            {
                ...
                if (strcmp(*av, "-"))
                {
                    ...
                    if (!ifp)
                    {
                        snprintf(s, 1024, _("cannot open file '%s': %s"), *av, strerror(errno));
                        R_Suicide(s);
                    }
                }
            }
            else if (CharacterMode == RTerm && !strncmp(*av, "--file=", 7))
            {
                ...
                if (strcmp((*av) + 7, "-"))
                {
                    ...
                    if (!ifp)
                    {
                        snprintf(s, 1024, _("cannot open file '%s': %s"), (*av) + 7, strerror(errno));
                        R_Suicide(s);
                    }
                }
            }
    ...
    if (strlen(cmdlines))
    {
        ...
        Rp->ReadConsole = FileReadConsole;
        {
            ...
            snprintf(ifile, 1024, "%s/Rscript%x%x", tm, getpid(), (unsigned int)GetTickCount());
            ifp = fopen(ifile, "w+b");
            if (!ifp)
    ...
    if (InThreadReadConsole)
    {
        ...
        if (ok)
        {
            ...
            WNDCLASS wndclass = {0, ReadMsgWindowProc, 0, 0, instance, NULL, 0, 0, NULL, "RReadMsg"};
            ReadMsgWindow = NULL;
            if (RegisterClass(&wndclass))
            {
                ReadMsgWindow = CreateWindow("RReadMsg", "RReadMsg", 0, 1, 1, 1, 1, HWND_MESSAGE, NULL, instance, NULL);
            }
            if (!ReadMsgWindow)
                ok = FALSE;
        }
        if (!ok || !(EhiWakeUp = CreateEvent(NULL, FALSE, FALSE, NULL)) || (_beginthread(ReaderThread, 0, NULL) == -1))
