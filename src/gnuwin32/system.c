...
void R_WriteConsoleEx(const char *buf, int len, int otype)
{
    ...
    if (TrueWriteConsole)
        ...
        TrueWriteConsoleEx(buf, len, otype);
}
...
int cmdlineoptions(int ac, char **av)
{
    ...
    */
    ...
    if (q && q[0])
    {
        ...
        {
            snprintf(s, 1024, _("WARNING: R_MAX_MEM_SIZE value is invalid: ignored\n"));
            R_ShowMessage(s);
        }
        else
            R_max_memory = value;
    ...
    while (--ac)
    {
        if (processing && **++av == '-')
        {
            ...
            else if (!strncmp(*av, "--max-mem-size", 14))
            {
                ...
                {
                    if (ierr < 0)
                        snprintf(s, 1024, _("WARNING: --max-mem-size value is invalid: ignored\n"));
                    else
                        snprintf(s, 1024, _("WARNING: --max-mem-size=%lu%c: too large and ignored\n"),
                                 (unsigned long)value, (ierr == 1) ? 'M' : ((ierr == 2) ? 'K' : 'G'));
                    R_ShowMessage(s);
                }
                else if (value < 32 * Mega)
                {
                    snprintf(s, 1024, _("WARNING: --max-mem-size=%4.1fM: too small and ignored\n"),
                             value / (1024.0 * 1024.0));
                    R_ShowMessage(s);
                }
                else if (value > Virtual)
                {
                    snprintf(s, 1024, _("WARNING: --max-mem-size=%4.0fM: too large and taken as %uM\n"),
                             value / (1024.0 * 1024.0), (unsigned int)(Virtual / (1024.0 * 1024.0)));
