...
void GClip()
{
    if (GP->xpd != clipstate)
    {
        if (GP->xpd)
            ...
            DevClip(XFMAP(GP->plt[0]), XFMAP(GP->plt[1]), YFMAP(GP->plt[2]), YFMAP(GP->plt[3]));
        clipstate = GP->xpd;
    }
}
...
void GArrow(double xfrom, double yfrom, double xto, double yto, double length, double angle, int code)
{
...
void GTitles(char *main, char *sub, char *xlab, char *ylab)
{
    ...
    if (main)
    {
        GP->font = GP->fontmain;
        GP->cex = GP->cexmain;
        GText(0.5 * (GP->plt[0] + GP->plt[1]), 0.5 * (GP->plt[3] + 1.0), main, 0.5, 0.5, 0.0);
    }
    if (sub)
    {
        GP->font = GP->fontsub;
        GP->cex = GP->cexsub;
        GText(0.5 * (GP->plt[0] + GP->plt[1]), GP->plt[2] - yl * (GP->mgp[0] + 2.0), xlab, 0.5, 0.0, 0.0);
    }
    if (xlab)
    {
        GP->font = GP->fontlab;
        GP->cex = GP->cexlab;
        GText(0.5 * (GP->plt[0] + GP->plt[1]), GP->plt[2] - yl * (GP->mgp[0] + 1.0), xlab, 0.5, 0.0, 0.0);
    }
    if (ylab)
    {
        GP->font = GP->fontlab;
        GP->cex = GP->cexlab;
        GText(GP->plt[0] - xl * GP->mgp[0], 0.5 * (GP->plt[2] + GP->plt[3]), ylab, 0.5, 0.0, 90.0);
    }
    GP->font = fontsave;
    GP->cex = GP->cex;
    GP->xpd = xpdsave;
