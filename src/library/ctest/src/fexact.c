...
static int f2xact(Sint *nrow, Sint *ncol, double *table, Sint *ldtabl, double *expect, double *percnt, double *emin,
                  double *prt, double *pre, double *fact, Sint *ico, Sint *iro, Sint *kyy, Sint *idif, Sint *irn,
                  Sint *key, Sint *ldkey, Sint *ipoin, double *stp, Sint *ldstp, Sint *ifrq, double *dlp, double *dsp,
                  double *tm, Sint *key2, Sint *iwk, double *rwk);
static int f3xact(Sint *nrow, Sint *irow, Sint *ncol, Sint *icol, double *dlp, Sint *mm, double *fact, Sint *ico,
                  Sint *iro, Sint *it, Sint *lb, Sint *nr, Sint *nt, Sint *nu, Sint *itc, Sint *ist, double *stv,
                  double *alen, double *tol);
static int f4xact(Sint *nrow, Sint *irow, Sint *ncol, Sint *icol, double *dsp, double *fact, Sint *icstk, Sint *ncstk,
                  Sint *lstk, Sint *mstk, Sint *nstk, Sint *nrstk, Sint *irstk, double *ystk, double *tol);
static int f5xact(double *pastp, double *tol, Sint *kval, Sint *key, Sint *ldkey, Sint *ipoin, double *stp, Sint *ldstp,
                  Sint *ifrq, Sint *npoin, Sint *nr, Sint *nl, Sint *ifreq, Sint *itop, Sint *ipsh);
static int f6xact(Sint *nrow, Sint *irow, Sint *iflag, Sint *kyy, Sint *key, Sint *ldkey, Sint *last, Sint *ipn);
static int f7xact(Sint *nrow, Sint *imax, Sint *idif, Sint *k, Sint *ks, Sint *iflag);
static int f8xact(Sint *irow, Sint *is, Sint *i1, Sint *izero, Sint *new);
static double f9xact(Sint *n, Sint *mm, Sint *ir, double *fact);
static int f10act(Sint *nrow, Sint *irow, Sint *ncol, Sint *icol, double *val, Sint *xmin, double *fact, Sint *nd,
                  Sint *ne, Sint *m);
...
void fexact(Sint *nrow, Sint *ncol, double *table, Sint *ldtabl, double *expect, double *percnt, double *emin,
            double *prt, double *pre, Sint *workspace)
{
...
#define rwrk ((float *)equiv)
    ...
    f2xact(nrow, ncol, &table[*ldtabl + 1], ldtabl, expect, percnt, emin, prt, pre, &dwrk[i1 - 1], &iwrk[i2 - 1],
           &iwrk[i3 - 1], &iwrk[i3a - 1], &iwrk[i3b - 1], &iwrk[i3c - 1], &iwrk[i4 - 1], &ldkey, &iwrk[i5 - 1],
           &dwrk[i6 - 1], &ldstp, &iwrk[i7 - 1], &dwrk[i8 - 1], &dwrk[i9 - 1], &dwrk[i9a - 1], &iwrk[i10 - 1],
           &iwrk[iiwk - 1], &dwrk[irwk - 1]);
...
int f2xact(Sint *nrow, Sint *ncol, double *table, Sint *ldtabl, double *expect, double *percnt, double *emin,
           double *prt, double *pre, double *fact, Sint *ico, Sint *iro, Sint *kyy, Sint *idif, Sint *irn, Sint *key,
           Sint *ldkey, Sint *ipoin, double *stp, Sint *ldstp, Sint *ifrq, double *dlp, double *dsp, double *tm,
           Sint *key2, Sint *iwk, double *rwk)
{
    ...
    static Sint itmp;
    static double dspt;
    static Sint itop, jstp, ntot, jstp2, jstp3, jstp4, i, j, k, n, iflag, ncell, ifreq;
    static Sint chisq;
    static Sint ikkey;
    static double pastp;
    static Sint ikstp, k1;
    static double dd, df;
    static Sint ikstp2, i31, i32, i33, i34, i35, i36, i37, i38, i39, i41, i42, i43, i44, i45, i46, i47, i48, ii, kb, kd,
        ks;
    static double pv;
...
L240:
    ...
    if (k1 > 1)
    {
        ...
        if (dlp[itp] > 0.)
        {
            ...
            dlp[itp] = 0.;
            f3xact(&nro2, &irn[nrb], &k1, &ico[kb + 1], &dlp[itp], &ntot, fact, &iwk[i31], &iwk[i32], &iwk[i33],
                   &iwk[i34], &iwk[i35], &iwk[i36], &iwk[i37], &iwk[i38], &iwk[i39], &rwk[i310], &rwk[i311], &tol);
            dlp[itp] = min(0., dlp[itp]);
            /* Compute shortest path */
            dsp[itp] = dspt;
            f4xact(&nro2, &irn[nrb], &k1, &ico[kb + 1], &dsp[itp], fact, &iwk[i47], &iwk[i41], &iwk[i42], &iwk[i43],
...
L300:
    ...
    else if (pastp < obs2)
    {
        ...
        else
        {
            ...
            d1 = pastp + ddf;
            f5xact(&d1, &tol, &kval, &key[jkey], ldkey, &ipoin[jkey], &stp[jstp], ldstp, &ifrq[jstp], &ifrq[jstp2],
...
L310:
    iflag = 1;
    f6xact(&nro, &iro[1], &iflag, &kyy[1], &key[ikkey + 1], ldkey, &last, &ipo);
    /* Update pointers */
    if (iflag == 3)
    {
        --k;
...
int f3xact(Sint *nrow, Sint *irow, Sint *ncol, Sint *icol, double *dlp, Sint *mm, double *fact, Sint *ico, Sint *iro,
           ...
           double *tol)
{
    ...
    {
        f10act(nrow, &irow[1], ncol, &icol[1], &val, &xmin, fact, &lb[1], &nu[1], &nr[1]);
    }
    if (!xmin)
    {
        if (icol[*ncol] <= icol[1] + *nrow)
        {
            f10act(ncol, &icol[1], nrow, &irow[1], &val, &xmin, fact, &lb[1], &nu[1], &nr[1]);
        }
    }
...
L90:
    ...
    {
        f10act(&nro, &iro[irl], &nco, &ico[1], &val, &xmin, fact, &lb[1], &nu[1], &nr[1]);
    }
    if (!xmin)
    {
        if (ico[nco] <= ico[1] + nro)
        {
            f10act(&nco, &ico[1], &nro, &iro[irl], &val, &xmin, fact, &lb[1], &nu[1], &nr[1]);
        }
    }
...
L100:
    ...
    nrt = iro[irl];
    nct = ico[1];
    lb[1] = (Sint)((double)((nrt + 1) * (nct + 1)) / (double)(nn + nr1 * nc1s + 1) - *tol) - 1;
    nu[1] = (Sint)((double)((nrt + nc1s) * (nct + nr1)) / (double)(nn + nr1 + nc1s)) - lb[1] + 1;
    nr[1] = nrt - lb[1];
    /* Generate a node */
L110:
    --nu[lev];
...
L120:
    ...
    if (lev < nc1s)
    {
        ...
        nc1 = nco - lev;
        nct = ico[lev];
        lb[lev] = (Sint)((double)((nrt + 1) * (nct + 1)) / (double)(nn1 + nr1 * nc1 + 1) - *tol);
        nu[lev] = (Sint)((double)((nrt + nc1) * (nct + nr1)) / (double)(nn1 + nr1 + nc1) - lb[lev] + 1);
        nr[lev] = nrt - lb[lev];
        goto L120;
    }
...
int f4xact(Sint *nrow, Sint *irow, Sint *ncol, Sint *icol, double *dsp, double *fact, Sint *icstk, Sint *ncstk,
           Sint *lstk, Sint *mstk, Sint *nstk, Sint *nrstk, Sint *irstk, double *ystk, double *tol)
{
    /* System generated locals */
    Sint ikh;
...
L60:
    ...
    else if (irt > ict)
    {
        ...
        f11act(&icstk[istk * *ncol + 1], &j, &nco, &icstk[(istk + 1) * *ncol + 1]);
        ikh = irt - ict;
        f8xact(&irstk[istk * *nrow + 1], &ikh, &i, &nro, &irstk[(istk + 1) * *nrow + 1]);
    }
    else
    {
        --nro;
        f11act(&irstk[istk * *nrow + 1], &i, &nro, &irstk[(istk + 1) * *nrow + 1]);
        ikh = ict - irt;
        f8xact(&icstk[istk * *ncol + 1], &ikh, &j, &nco, &icstk[(istk + 1) * *ncol + 1]);
    }
...
int f5xact(double *pastp, double *tol, Sint *kval, Sint *key, Sint *ldkey, Sint *ipoin, double *stp, Sint *ldstp,
           Sint *ifrq, Sint *npoin, Sint *nr, Sint *nl, Sint *ifreq, Sint *itop, Sint *ipsh)
{
    /* Local variables */
    static Sint itmp;
...
int f6xact(Sint *nrow, Sint *irow, Sint *iflag, Sint *kyy, Sint *key, Sint *ldkey, Sint *last, Sint *ipn)
{
    static Sint kval, j;
...
int f7xact(Sint *nrow, Sint *imax, Sint *idif, Sint *k, Sint *ks, Sint *iflag)
{
    /* Local variables */
...
int f10act(Sint *nrow, Sint *irow, Sint *ncol, Sint *icol, double *val, Sint *xmin, double *fact, Sint *nd, Sint *ne,
           Sint *m)
{
    /* Local variables */
