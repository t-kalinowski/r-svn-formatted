...
void nscor1(double *s, int *n, int *n2, double *work, int *ifault)
{
    ...
    *ifault = 0;
    ...
    for (i = 1; i <= *n2; ++i)
    {
        ...
        {
            scor +=
                exp(work[(j << 2) + 2] + ai1 * work[(j << 2) + 3] + ani * work[(j << 2) + 4] + c) * work[(j << 2) + 1];
        }
        s[i] = scor * hh;
        c += log(ani / (double)i);
    }
...
void nscor2(float *s, int *n, int *n2, int *ier)
{
    ...
    */
    ...
    const float eps[4] = {.419885f, .450536f, .456936f, .468488f}, dl1[4] = {.112063f, .12177f, .239299f, .215159f},
                dl2[4] = {.080122f, .111348f, -.211867f, -.115049f}, gam[4] = {.474798f, .469051f, .208597f, .259784f},
...
static double correc(int i, int n)
{
    ...
    const float c1[7] = {9.5f, 28.7f, 1.9f, 0.f, -7.f, -6.2f, -1.6f},
                ...
                c3[7] = {93380.f, 175160.f, 410400.f, 2157600.f, 2.376e6f, 2.065e6f, 2.065e6f};
