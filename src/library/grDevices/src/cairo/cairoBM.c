...
static Rboolean BM_Open(pDevDesc dd, pX11Desc xd, int width, int height)
{
    ...
    {
        xd->cs = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, xd->windowWidth, xd->windowHeight);
...
static Rboolean BMDeviceDriver(pDevDesc dd, int kind, SEXP filename, int quality, int width, int height, int ps, int bg,
                               int res, int antialias, const char *family, double dpi, const char *symbolfamily,
                               Rboolean usePUA)
{
    pX11Desc xd;
    ...
    if (!(xd = (pX11Desc)calloc(1, sizeof(X11Desc))))
        return FALSE;
    strncpy(xd->filename, R_ExpandFileName(translateCharFP(filename)), PATH_MAX - 1);
...
SEXP in_Cairo(SEXP args)
{
    ...
    BEGIN_SUSPEND_INTERRUPTS
    {
        ...
        if (!BMDeviceDriver(dev, devtable[type].gtype, filename, quality, width, height, pointsize, bgcolor, res,
                            antialias, family, dpi, symbolfamily, usePUA))
        {
            free(dev);
