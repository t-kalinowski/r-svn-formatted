...
static void in_chull(int *n, double *x, int *m, int *in, int *ia, int *ib, int *ih, int *nh, int *il)
{
...
#define y(k) x[k + x_dim1]
    ...
    if (kx == kn)
    { /* if the max and min are equal,
       * all m points lie on a vertical line */
        goto L_vertical;
    }

    if (maxe || mine)
    { /* if maxe (or mine) is TRUE, there are several
maxima (or minima) with equal first coordinates */

        if (maxe)
        { /* have several points with the (same) largest x[] */
            for (i = 1; i <= *m; ++i)
            {
                j = in[i];
                if (x[j] != x[kx])
        ...
        if (mine)
        { /* have several points with the (same) smallest x[] */
            for (i = 1; i <= *m; ++i)
            {
                j = in[i];
                if (x[j] != x[kn])
...
L13:
    ...
    do
    {
        if (mxb != 0)
        {
            ...
            {
                ilinh = il[inh];
                split(*n, &x[x_offset], mb, &ib[nib], ih[inh], ih[ilinh], -1, &ia[nia], &ia[ma], &mxa, &ib[nib], &mbb,
                      &mxb);
                mb = mbb;
                goto L13;
            }
