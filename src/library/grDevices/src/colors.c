...
static char *RGB2rgb(unsigned int r, unsigned int g, unsigned int b)
{
    ColBuf[0] = '#';
...
static char *RGBA2rgb(unsigned int r, unsigned int g, unsigned int b, unsigned int a)
{
    ColBuf[0] = '#';
    ColBuf[1] = HexDigits[(r >> 4) & 15];
    ColBuf[2] = HexDigits[r & 15];
...
SEXP hsv(SEXP h, SEXP s, SEXP v, SEXP a)
{
    ...
    else
    {
        for (i = 0; i < max; i++)
        {
            ...
            vv = REAL(v)[i % nv];
            aa = REAL(a)[i % na];
            if (hh < 0 || hh > 1 || ss < 0 || ss > 1 || vv < 0 || vv > 1 || aa < 0 || aa > 1)
                error(_("invalid hsv color"));
            hsv2rgb(hh, ss, vv, &r, &g, &b);
            SET_STRING_ELT(c, i, mkChar(RGBA2rgb(ScaleColor(r), ScaleColor(g), ScaleColor(b), ScaleAlpha(aa))));
        }
    }
    UNPROTECT(5);
    return c;
...
SEXP rgb(SEXP r, SEXP g, SEXP b, SEXP a, SEXP MCV, SEXP nam)
{
    ...
    if (mV == 255.0)
    {
        if (isNull(a))
        {
            _R_set_c_RGB(CheckColor(INTEGER(r)[i % nr]), CheckColor(INTEGER(g)[i % ng]),
                         CheckColor(INTEGER(b)[i % nb]));
        }
        else
        {
            _R_set_c_RGBA(CheckColor(INTEGER(r)[i % nr]), CheckColor(INTEGER(g)[i % ng]),
                          CheckColor(INTEGER(b)[i % nb]), CheckAlpha(INTEGER(a)[i % na]));
        }
    }
    else if (max_1)
    {
        if (isNull(a))
        {
            _R_set_c_RGB(ScaleColor(REAL(r)[i % nr]), ScaleColor(REAL(g)[i % ng]), ScaleColor(REAL(b)[i % nb]));
        }
        else
        {
            _R_set_c_RGBA(ScaleColor(REAL(r)[i % nr]), ScaleColor(REAL(g)[i % ng]), ScaleColor(REAL(b)[i % nb]),
                          ScaleAlpha(REAL(a)[i % na]));
        }
    }
    else
    { /* maxColorVal not in {1, 255} */
        if (isNull(a))
        {
            _R_set_c_RGB(ScaleColor(REAL(r)[i % nr] / mV), ScaleColor(REAL(g)[i % ng] / mV),
                         ScaleColor(REAL(b)[i % nb] / mV));
        }
        else
        {
            _R_set_c_RGBA(ScaleColor(REAL(r)[i % nr] / mV), ScaleColor(REAL(g)[i % ng] / mV),
                          ScaleColor(REAL(b)[i % nb] / mV), ScaleAlpha(REAL(a)[i % na] / mV));
        }
    }
    if (length(nam) != 0)
...
SEXP gray(SEXP lev, SEXP a)
{
    ...
    else
    {
        ...
        for (i = 0; i < (nlev > na ? nlev : na); i++)
        {
            ...
            SET_STRING_ELT(ans, i, mkChar(RGBA2rgb(ilevel, ilevel, ilevel, ScaleAlpha(aa))));
        }
    }
    UNPROTECT(3);
    return ans;
...
SEXP RGB2hsv(SEXP rgb)
{
    ...
    if (!isMatrix(rgb))
        error("rgb is not a matrix (internally)");
    dd = getAttrib(rgb, R_DimSymbol);
    if (INTEGER(dd)[0] != 3)
        error("rgb must have 3 rows (internally)");
    n = INTEGER(dd)[1];
    ...
    if ((dd = getAttrib(rgb, R_DimNamesSymbol)) != R_NilValue && (names = VECTOR_ELT(dd, 1)) != R_NilValue)
    ...
    {
        rgb2hsv(REAL(rgb)[i3 + 0], REAL(rgb)[i3 + 1], REAL(rgb)[i3 + 2], &REAL(ans)[i3 + 0], &REAL(ans)[i3 + 1],
                &REAL(ans)[i3 + 2]);
    }
    UNPROTECT(2);
    return ans;
...
static rcolor Palette[MAX_PALETTE_SIZE] = {0xff000000, 0xff0000ff, 0xff00cd00, 0xffff0000,
                                           0xffffff00, 0xffff00ff, 0xff00ffff, 0xffbebebe};
