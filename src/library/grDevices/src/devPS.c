...
static int GetCharInfo(char *buf, FontMetricInfo *metrics, CNAME *charnames, CNAME *encnames, int reencode)
{
...
static int GetKPX(char *buf, int nkp, FontMetricInfo *metrics, CNAME *charnames)
{
    ...
    while (R_gzgets(fp, buf, BUFSIZE))
    {
        ...
        case StartKernPairs:
            ...
            if (nKPX > 0)
            {
                ...
                if (!metrics->KernPairs)
                    goto pserror;
            }
            break;

        case KPX:
            if (mode != StartKernData || i >= nKPX)
                goto pserror;
            if (GetKPX(buf, i, metrics, charnames))
                i++;
            break;
    ...
    for (p = str1; *p; p++)
    {
        ...
        if (useKerning)
        {
            ...
            for (i = metrics->KPstart[p1]; i < metrics->KPend[p1]; i++)
                ...
                if (metrics->KernPairs[i].c2 == p2 && metrics->KernPairs[i].c1 == p1)
                {
                                 ...
                                 Rboolean isSymbol, const char *encoding)
{
    ...
    if (Unicode && !isSymbol && c >= 128 && c < 65536)
    { /* Unicode */
        void *cd = NULL;
        const char *i_buf;
        char *o_buf, out[2];
...
static encodinginfo addEncoding(const char *encpath, Rboolean isPDF)
{
    ...
    if (encoding)
    {
        if (LoadEncoding(encpath, encoding->name, encoding->convname, encoding->encnames, encoding->enccode, isPDF))
        {
 ...
 */
static encodinglist addDeviceEncoding(encodinginfo encoding, encodinglist devEncs)
{
    encodinglist newenc = makeEncList();
    if (!newenc)
    {
...
SEXP Type1FontInUse(SEXP name, SEXP isPDF)
{
    ...
    return ScalarLogical(findLoadedFont(CHAR(STRING_ELT(name, 0)), NULL, asLogical(isPDF)) != NULL);
}
...
SEXP CIDFontInUse(SEXP name, SEXP isPDF)
{
    if (!isString(name) || LENGTH(name) > 1)
        error(_("invalid font name or more than one font name"));
    return ScalarLogical(findLoadedCIDFont(CHAR(STRING_ELT(name, 0)), asLogical(isPDF)) != NULL);
}
...
static const char *fontMetricsFileName(const char *family, int faceIndex, const char *fontdbname)
{
    ...
    for (i = 0; i < nfonts && !found; i++)
    {
        ...
        if (strcmp(family, fontFamily) == 0)
        {
            ...
            result = CHAR(STRING_ELT(VECTOR_ELT(VECTOR_ELT(fontdb, i), 1), faceIndex));
        }
    }
    if (!found)
        warning(_("font family '%s' not found in PostScript font database"), family);
...
static Rboolean isCIDFont(const char *family, const char *fontdbname, cidfontfamily defaultCIDFont)
{
    /*
     * If family is "" then we're referring to the default device
     * font, so the test is just whether the default font is
...
static type1fontfamily addLoadedFont(type1fontfamily font, Rboolean isPDF)
{
    ...
    if (fontfamily)
    {
        ...
        else
        {
            ...
            for (i = 0; i < 4; i++)
            {
                 ...
                 */
                safestrcpy(fontfamily->cidfonts[i]->name, getFontName(name, fontdbname), 50);
            }
            /*
            ...
            for (i = 0; i < 1; i++)
            {
                ...
                if (!PostScriptLoadFontMetrics(afmpath, &(fontfamily->symfont->metrics), fontfamily->symfont->name,
...
static type1fontfamily addFont(const char *name, Rboolean isPDF, encodinglist deviceEncodings)
{
        ...
        else
        {
            ...
            else
            {
                ...
                for (i = 0; i < 5; i++)
                {
                    ...
                    if (!PostScriptLoadFontMetrics(afmpath, &(fontfamily->fonts[i]->metrics),
                                                   fontfamily->fonts[i]->name, fontfamily->fonts[i]->charnames,
                                                   /*
                                                    * Reencode all but
                                                    * symbol face
                                                    */
    ...
    if (fontfamily)
    {
        ...
        else
        {
            ...
            for (i = 0; i < 5; i++)
            {
                ...
                fontfamily->fonts[i] = font;
                if (!PostScriptLoadFontMetrics(afmpaths[i], &(fontfamily->fonts[i]->metrics),
                                               fontfamily->fonts[i]->name, fontfamily->fonts[i]->charnames,
                                               /*
                                                * Reencode all but
                                                * symbol face
 ...
 */
static cidfontlist addDeviceCIDFont(cidfontfamily font, cidfontlist devFonts, int *index)
{
    cidfontlist newfont = makeCIDFontList();
    *index = 0;
    if (!newfont)
...
static type1fontlist addDeviceFont(type1fontfamily font, type1fontlist devFonts, int *index)
{
    type1fontlist newfont = makeFontList();
    *index = 0;
    if (!newfont)
...
static void specialCaseCM(FILE *fp, type1fontfamily family, int familynum)
{
    ...
    fprintf(fp, " /parenleftbt /bracketlefttp /bracketleftex /bracketleftbt /bracelefttp /braceleftmid /braceleftbt "
                "/braceex\n");
    fprintf(fp, " /.notdef /angleright /integral /integraltp /integralex /integralbt /parenrighttp /parenrightex\n");
    fprintf(fp, " /parenrightbt /bracketrighttp /bracketrightex /bracketrightbt /bracerighttp /bracerightmid "
                "/bracerightbt /.notdef\n");
    fprintf(fp, "] def\n");
...
static void PSEncodeFonts(FILE *fp, PostScriptDesc *pd)
{
    ...
    while (fonts)
    {
         ...
         */
        encodinginfo encoding = findDeviceEncoding(fonts->family->encoding->encpath, pd->encodings, &dontcare);
        /*
         ...
         */
        if (!encoding || (encoding == pd->encodings->encoding && !haveWrittenDefaultEnc))
        {
            /*
            ...
            if (encoding != pd->encodings->encoding)
            {
                ...
                else
                {
                     ...
                     */
                    encodinglist enclist = addDeviceEncoding(encoding, pd->encodings);
                    if (enclist)
        ...
        if (strcmp(fonts->family->fonts[4]->name, "CMSY10 CMBSY10 CMMI10") == 0)
        {
    ...
    while (cidfonts)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            case 0:
                ...
                break;
            case 1:
                fmt = "/%s-Bold-%s findfont\n";
                break;
            case 2:
                fmt = "/%s-%s findfont [1 0 .3 1 0 0] makefont\n";
                break;
            case 3:
                fmt = "/%s-Bold-%s findfont [1 0 .3 1 0 0] makefont\n";
                break;
            default:
                break;
            }
...
static void PSFileHeader(FILE *fp, const char *papername, double paperwidth, double paperheight, Rboolean landscape,
                         int EPSFheader, Rboolean paperspecial, double left, double bottom, double right, double top,
                         const char *title, PostScriptDesc *pd)
{
    int i;
    SEXP prolog;
    type1fontlist fonts = pd->fonts;
...
static void PostScriptSetLineTexture(FILE *fp, const char *dashlist, int nlty, double lwd, int lend)
{
    ...
    */
    ...
    for (i = 0; i < nlty; i++)
    {
        dash[i] = lwd * ((i % 2) ? (dashlist[i] + a) : ((nlty == 1 && dashlist[i] == 1.) ? 1. : dashlist[i] - a));
        if (dash[i] < 0)
            dash[i] = 0;
        if (dash[i] > .01)
            allzero = FALSE;
...
{
    double x = fround(x1, 2) - fround(x0, 2), y = fround(y1, 2) - fround(y0, 2);
...
static void PostScriptTextKern(FILE *fp, double x, double y, const char *str, double xc, double rot,
                               const pGEcontext gc, pDevDesc dd)
{
    ...
    if (haveKerning)
    {
        ...
        for (i = 0; i < n - 1; i++)
        {
            ...
            for (j = metrics->KPstart[p1]; j < metrics->KPend[p1]; j++)
                if (metrics->KernPairs[j].c2 == p2 && metrics->KernPairs[j].c1 == p1)
                {
                    PostScriptText2(fp, x, y, str + nout, i + 1 - nout, relative, rot, gc, dd);
                    nout = i + 1;
...
static void PS_Path(double *x, double *y, int npoly, int *nper, Rboolean winding, const pGEcontext gc, pDevDesc dd);
...
static void PS_fillStroke(SEXP path, int rule, const pGEcontext gc, pDevDesc dd);
...
Rboolean PSDeviceDriver(pDevDesc dd, const char *file, const char *paper, const char *family, const char **afmpaths,
                        ...
                        const char *cmd, const char *title, SEXP fonts, const char *colormodel, int useKern,
                        Rboolean fillOddEven)
{
    ...
    if (!isNull(fonts))
    {
        ...
        for (i = 0; i < nfonts; i++)
        {
             ...
             * using this font.
             */
            if (findDeviceFont(name, pd->fonts, &index) || findDeviceCIDFont(name, pd->cidfonts, &cidindex))
                gotFonts++;
            else
            {
                /*
                ...
                if (font || cidfont)
                {
                    if (isType1Font(name, PostScriptFonts, NULL))
                    {
                        if ((fontlist = addDeviceFont(font, pd->fonts, &dontcare)))
                        {
                            pd->fonts = fontlist;
                            gotFonts++;
                        }
                    ...
                    {
                        if ((cidfontlist = addDeviceCIDFont(cidfont, pd->cidfonts, &dontcare)))
                        {
    ...
    else if (!strcmp(pd->papername, "Letter") || !strcmp(pd->papername, "letter") || !strcmp(pd->papername, "US") ||
...
static Rboolean PS_Open(pDevDesc dd, PostScriptDesc *pd)
{
    ...
    if (pd->landscape)
        PSFileHeader(pd->psfp, pd->papername, pd->paperwidth, pd->paperheight, pd->landscape, !(pd->onefile),
                     pd->paperspecial, dd->bottom, dd->left, dd->top, dd->right, pd->title, pd);
    else
        PSFileHeader(pd->psfp, pd->papername, pd->paperwidth, pd->paperheight, pd->landscape, !(pd->onefile),
                     pd->paperspecial, dd->left, dd->bottom, dd->right, dd->top, pd->title, pd);
...
static FontMetricInfo *CIDsymbolmetricInfo(const char *family, PostScriptDesc *pd)
{
    ...
    if (fontfamily)
    {
        ...
        error(_("CID family '%s' not included in postscript() device"), family);
    return result;
}

static FontMetricInfo *metricInfo(const char *family, int face, PostScriptDesc *pd)
{
    FontMetricInfo *result = NULL;
    int fontIndex;
...
static double PS_StrWidth(const char *str, const pGEcontext gc, pDevDesc dd)
{
    ...
    if (isType1Font(gc->fontfamily, PostScriptFonts, pd->defaultFont))
    {
        return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((const unsigned char *)str, CE_NATIVE,
                                                                     metricInfo(gc->fontfamily, face, pd), pd->useKern,
                                                                     face, convname(gc->fontfamily, pd));
    }
    else
    { /* cidfont(gc->fontfamily, PostScriptFonts) */
        if (face < 5)
        {
            return floor(gc->cex * gc->ps + 0.5) *
                   PostScriptStringWidth((const unsigned char *)str, CE_NATIVE, NULL, FALSE, face, NULL);
        }
        else
        {
            return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((const unsigned char *)str, CE_NATIVE,
                                                                         /* Send symbol face metric info */
                                                                         CIDsymbolmetricInfo(gc->fontfamily, pd), FALSE,
                                                                         face, NULL);
        }
    }
}
...
static double PS_StrWidthUTF8(const char *str, const pGEcontext gc, pDevDesc dd)
{
    ...
    if (isType1Font(gc->fontfamily, PostScriptFonts, pd->defaultFont))
    {
        return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((const unsigned char *)str, CE_UTF8,
                                                                     metricInfo(gc->fontfamily, face, pd), pd->useKern,
                                                                     face, convname(gc->fontfamily, pd));
    }
    else
    { /* cidfont(gc->fontfamily, PostScriptFonts) */
        ...
        {
            return floor(gc->cex * gc->ps + 0.5) *
                   PostScriptStringWidth((const unsigned char *)str, CE_UTF8, NULL, FALSE, face, NULL);
        }
        else
        {
            return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((const unsigned char *)str, CE_UTF8,
...
static void PS_MetricInfo(int c, const pGEcontext gc, double *ascent, double *descent, double *width, pDevDesc dd)
{
    ...
    { /* cidfont(gc->fontfamily, PostScriptFonts) */
        ...
        {
            PostScriptMetricInfo(c, ascent, descent, width, CIDsymbolmetricInfo(gc->fontfamily, pd), TRUE, "");
        }
    }
...
static void PS_imagedata(rcolorPtr raster, int w, int h, PostScriptDesc *pd)
{
    ...
    for (int i = 0; i < w * h; i++)
        fprintf(pd->psfp, "%02x%02x%02x", R_RED(raster[i]), R_GREEN(raster[i]), R_BLUE(raster[i]));
}

static void PS_grayimagedata(rcolorPtr raster, int w, int h, PostScriptDesc *pd)
{
    /* Weights as in PDF gray conversion */
    for (int i = 0; i < w * h; i++)
    {
        double r = 0.213 * R_RED(raster[i]) + 0.715 * R_GREEN(raster[i]) + 0.072 * R_BLUE(raster[i]);
...
static void PS_writeRaster(unsigned int *raster, int w, int h, double x, double y, double width, double height,
                           double rot, Rboolean interpolate, pDevDesc dd)
{
    PostScriptDesc *pd = (PostScriptDesc *)dd->deviceSpecific;
                      ...
                      Rboolean interpolate, const pGEcontext gc, pDevDesc dd)
{
    ...
    if (interpolate)
    {
        ...
        PS_writeRaster(newRaster, newW, newH, x, y, width, height, rot, FALSE, dd);
        vmaxset(vmax);
    }
    else
    {
        PS_writeRaster(raster, w, h, x, y, width, height, rot, FALSE, dd);
    }
#else
    PS_writeRaster(raster, w, h, x, y, width, height, rot, interpolate, dd);
#endif
}
        ...
        for (i = 0; i < npoly; i++)
        {
            ...
            for (j = 1; j < nper[i]; j++)
            {
                if (j % 100 == 0)
                    ...
                    PostScriptRLineTo(pd->psfp, x[index - 1], y[index - 1], x[index], y[index]);
                index++;
            }
            fprintf(pd->psfp, "cp\n");
        }
...
static void mbcsToSbcs(const char *in, char *out, const char *encoding, int enc)
{
    ...
    if ((void *)-1 == (cd = Riconv_open(encoding, (enc == CE_UTF8) ? "UTF-8" : "")))
...
next_char:
    ...
    if (status == (size_t)-1) /* internal error? */
        error("conversion failure from %s to %s on '%s' in 'mbcsToSbcs'", (enc == CE_UTF8) ? "UTF-8" : "native",
              encoding, in);
}
                     ...
                     pDevDesc dd)
{
    ...
    if (gc->fontface == 5)
    {
        ...
        {
            drawSimpleText(x, y, str, rot, hadj, translateCIDFont(gc->fontfamily, gc->fontface, pd), gc, dd);
            return;
        }
        else
        {
    ...
    if (isCIDFont(gc->fontfamily, PostScriptFonts, pd->defaultCIDFont))
    {
        ...
        if (!cidfont)
            error(_("family '%s' not included in postscript() device"), gc->fontfamily);
        ...
        if (ucslen != (size_t)-1)
        {
            ...
            else
            {
                ...
                {
                    SetColor(gc->col, dd);
                    PostScriptHexText(pd->psfp, x, y, (char *)buf, nb - o_len, hadj, rot);
                }
            }
            return;
        }
...
static void XF_FileHeader(FILE *fp, const char *papername, Rboolean landscape, Rboolean onefile)
{
    fprintf(fp, "#FIG 3.2\n");
...
static int XF_SetLty(int lty)
{
    ...
    case LTY_DOTDASH:
        return 3;
    default:
        warning(_("unimplemented line texture %08x: using Dash-double-dotted"), lty);
        return 4;
    }
}
...
static int XFigBaseNum(const char *name)
{
    ...
    {
        warning(_("unknown postscript font family '%s', using Helvetica"), name);
        i = 16;
    }
    return i;
}
...
static void XFig_Text(double x, double y, const char *str, double rot, double hadj, const pGEcontext gc, pDevDesc dd)
{
    ...
    if (style == 5)
        ...
        fontnum = pd->fontnum + styles[style - 1];
...
static double XFig_StrWidth(const char *str, const pGEcontext gc, pDevDesc dd)
{
    ...
    return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((const unsigned char *)str, CE_NATIVE,
                                                                 &(pd->fonts->family->fonts[face - 1]->metrics), FALSE,
                                                                 face, "latin1");
}
...
const char *PDFblendModes[] = {"Normal",     "Multiply",   "Screen",    "Overlay",   "Darken",     "Lighten",
...
void PDF_MetricInfo(int c, const pGEcontext gc, double *ascent, double *descent, double *width, pDevDesc dd);
...
static void PDF_Path(double *x, double *y, int npoly, int *nper, Rboolean winding, const pGEcontext gc, pDevDesc dd);
...
double PDF_StrWidth(const char *str, const pGEcontext gc, pDevDesc dd);
...
static SEXP PDF_defineGroup(SEXP source, int op, SEXP destination, pDevDesc dd);
...
static void PDF_fillStroke(SEXP path, int rule, const pGEcontext gc, pDevDesc dd);
...
static void catDefn(char *buf, int i, PDFDesc *pd)
{
    ...
    if (len + buflen + 1 >= pd->definitions[i].nchar)
    {
        ...
        tmp = realloc(pd->definitions[i].str, (pd->definitions[i].nchar) * sizeof(char));
        if (!tmp)
...
{
    size_t len = strlen(pd->definitions[i].str);
    pd->definitions[i].str = realloc(pd->definitions[i].str, (len + 1) * sizeof(char));
...
static int growDefinitions(PDFDesc *pd)
{
    if (pd->numDefns == pd->maxDefns)
    {
        ...
        if (!tmp)
            error(_("failed to increase 'maxDefns'"));
        pd->definitions = tmp;
...
static void addRGBExpGradientFunction(SEXP gradient, int i, double start, double end, int toDefn, PDFDesc *pd)
{
    char buf[300];
    ...
    snprintf(buf, 300,
             "<<\n/FunctionType 2\n/Domain [%.4f %.4f]\n/C0 [%0.4f %0.4f %0.4f]\n/C1 [%0.4f %0.4f %0.4f]\n/N 1\n>>\n",
             start, end, R_RED(col1) / 255.0, R_GREEN(col1) / 255.0, R_BLUE(col1) / 255.0, R_RED(col2) / 255.0,
             R_GREEN(col2) / 255.0, R_BLUE(col2) / 255.0);
    catDefn(buf, toDefn, pd);
}

static void addAlphaExpGradientFunction(SEXP gradient, int i, double start, double end, int toDefn, PDFDesc *pd)
{
    char buf[300];
    rcolor col1 = 0, col2 = 0; // -Wall
    switch (R_GE_patternType(gradient))
    ...
    snprintf(buf, 300, "<<\n/FunctionType 2\n/Domain [%.4f %.4f]\n/C0 [%0.4f]\n/C1 [%0.4f]\n/N 1\n>>\n", start, end,
             R_ALPHA(col1) / 255.0, R_ALPHA(col2) / 255.0);
    catDefn(buf, toDefn, pd);
}

static void addStitchedGradientFunction(SEXP gradient, int nStops, int toDefn, Rboolean alpha, PDFDesc *pd)
{
...
static void addGradientFunction(SEXP gradient, int toDefn, Rboolean alpha, PDFDesc *pd)
{
    int nStops = 0; // -Wall
    switch (R_GE_patternType(gradient))
    {
...
static void addLinearGradient(SEXP gradient, char *colormodel, int toDefn, PDFDesc *pd)
{
    int defNum = growDefinitions(pd);
    char buf[200];
    char colorspace[12];
    ...
    snprintf(buf, 200, "<<\n/ShadingType 2\n/ColorSpace %s\n/Coords [%.4f %.4f %.4f %.4f]\n/Function\n", colorspace,
             R_GE_linearGradientX1(gradient), R_GE_linearGradientY1(gradient), R_GE_linearGradientX2(gradient),
...
static void addRadialGradient(SEXP gradient, char *colormodel, int toDefn, PDFDesc *pd)
{
    int defNum = growDefinitions(pd);
    char buf[200];
    char colorspace[12];
    ...
    snprintf(buf, 200, "<<\n/ShadingType 3\n/ColorSpace %s\n/Coords [%.4f %.4f %.4f %.4f %.4f %.4f]\n/Function\n",
             colorspace, R_GE_radialGradientCX1(gradient), R_GE_radialGradientCY1(gradient),
             R_GE_radialGradientR1(gradient), R_GE_radialGradientCX2(gradient), R_GE_radialGradientCY2(gradient),
...
static int newTiling(SEXP pattern, PDFDesc *pd)
{
    ...
    snprintf(buf, 100, "/BBox [%d %d %d %d]\n", (int)(0.5 + R_GE_tilingPatternX(pattern)),
             (int)(0.5 + R_GE_tilingPatternY(pattern)),
             (int)(0.5 + R_GE_tilingPatternX(pattern) + R_GE_tilingPatternWidth(pattern)),
             (int)(0.5 + R_GE_tilingPatternY(pattern) + R_GE_tilingPatternHeight(pattern)));
    catDefn(buf, defNum, pd);
    snprintf(buf, 100, "/XStep %d /YStep %d\n", (int)(0.5 + R_GE_tilingPatternWidth(pattern)),
...
static int PDFwriteResourceDictionary(int objOffset, Rboolean endpage, int excludeDef, PDFDesc *pd);
...
static int countPatterns(PDFDesc *pd)
{
    ...
    for (i = 0; i < pd->numDefns; i++)
    {
        if (pd->definitions[i].type == PDFshadingPattern || pd->definitions[i].type == PDFtilingPattern)
        {
            count++;
        }
    }
...
static Rboolean appendingPathWithText(PDFDesc *pd)
{
    ...
    if (pd->appendingPath >= 0 && pd->pathContainsText)
    {
...
static void blendModeFromCompositingOperator(int op, char *mode, int size, PDFDesc *pd)
{
    int blendMode;
    switch (op)
    {
...
static int PDFwrite(char *buf, size_t size, const char *fmt, PDFDesc *pd, ...)
{
    ...
    else if (pd->appendingPattern >= 0 && (pd->appendingPattern > pd->appendingMask) &&
             (pd->appendingPattern > pd->appendingGroup))
    {
        addToPattern(buf, pd);
    }
    else if (pd->appendingMask >= 0 && (pd->appendingMask > pd->appendingPattern) &&
...
static void PDFwriteSoftMaskDefs(int objoffset, PDFDesc *pd)
{
    ...
    for (i = 0; i < pd->numDefns; i++)
    {
        if (pd->definitions[i].type == PDFsoftMask || pd->definitions[i].type == PDFshadingSoftMask)
        {
            PDFwrite(buf, 100, "/Def%d %d 0 R\n", pd, i, i + objoffset);
        }
    }
...
static void PDFwriteDefinitions(int resourceDictOffset, PDFDesc *pd)
{
    ...
    for (int i = 0; i < pd->numDefns; i++)
    {
        ...
        if (pd->definitions[i].type == PDFclipPath || pd->definitions[i].type == PDFstrokePath ||
            pd->definitions[i].type == PDFfillPath || pd->definitions[i].type == PDFfillStrokePath ||
 ...
 */
static int addRaster(rcolorPtr raster, int w, int h, Rboolean interpolate, PDFDesc *pd)
{
    int i, alpha = 0;
    rcolorPtr newRaster;
...
static void writeRasterXObject(rasterImage raster, int n, int mask, int maskObj, PDFDesc *pd)
{
    ...
    if (pd->useCompression)
    {
        ...
        if (res != Z_OK)
            error("internal error %d in writeRasterXObject", res);
        Free(buf);
        buf = buf2;
    }
 ...
 */
static Rboolean addPDFDeviceCIDfont(cidfontfamily family, PDFDesc *pd, int *fontIndex)
{
...
static Rboolean addPDFDevicefont(type1fontfamily family, PDFDesc *pd, int *fontIndex)
{
    Rboolean result = FALSE;
    ...
    {
        int dontcare;
        encodinginfo encoding = findDeviceEncoding(family->encoding->encpath, pd->encodings, &dontcare);
        if (encoding)
        {
            pd->fonts = fontlist;
            result = TRUE;
                         ...
                         Rboolean useCompression)
{
    ...
    else if (!strcmp(pd->papername, "USr") || !strcmp(pd->papername, "usr"))
    {
        pd->pageheight = 8.5;
        pd->pagewidth = 11.0;
    }
...
static void PDF_SetLineColor(int color, pDevDesc dd)
{
    ...
    if (color != pd->current.col)
    {
        ...
        {
            PDFwrite(buf, 100, "%.3f %.3f %.3f RG\n", pd, R_RED(color) / 255.0, R_GREEN(color) / 255.0,
                     R_BLUE(color) / 255.0);
        }
        else
        {
...
static void PDF_SetPatternFill(SEXP ref, pDevDesc dd)
{
    ...
    if (pd->current.patternfill != patternIndex)
    {
        ...
        if (length(ref) > 1)
        {
            ...
            PDFwrite(buf, 100, "/Def%d gs /Pattern cs /Def%d scn\n", pd, maskIndex, patternIndex);
        }
        else
        {
            PDFwrite(buf, 100, "/Pattern cs /Def%d scn\n", pd, patternIndex);
        }
...
/* Note that the line texture is scaled by the line width.*/
static void PDFSetLineTexture(PDFDesc *pd, const char *dashlist, int nlty, double lwd, int lend)
{
    double dash[8], a = (lend == GE_BUTT_CAP) ? 0. : 1.;
    int i;
    Rboolean allzero = TRUE;
...
static void PDF_SetLineStyle(const pGEcontext gc, pDevDesc dd)
{
    ...
    if (pd->current.lty != newlty || pd->current.lwd != newlwd || pd->current.lend != newlend)
    {
        pd->current.lwd = newlwd;
        pd->current.lty = newlty;
...
static void PDF_Encodings(PDFDesc *pd)
{
    ...
    while (enclist)
    {
        ...
        if (strcmp(encoding->name, "WinAnsiEncoding") == 0 || strcmp(encoding->name, "MacRomanEncoding") == 0 ||
        ...
        else
        {
            ...
            int len;
            char buf[128];
            for (enc_first = 0; encoding->enccode[enc_first] != '[' && encoding->enccode[enc_first] != '\0';
                 enc_first++)
                ;
            ...
            while (encoding->enccode[enc_first])
            {
                ...
                for (len = 0;
                     (encoding->enccode[enc_first + len] != ' ') && (encoding->enccode[enc_first + len] != ']') &&
                     (encoding->enccode[enc_first + len] != '\t') && (encoding->enccode[enc_first + len] != '\0') &&
...
static void PDFwritesRGBcolorspace(PDFDesc *pd)
{
    ...
    snprintf(buf, BUFSIZE2, "%s%slibrary%sgrDevices%sicc%s%s", R_Home, FILESEP, FILESEP, FILESEP, FILESEP,
...
static const char *KnownSanSerif[] = {"AvantGarde", "Helvetica-Narrow", "URWGothic", "NimbusSan"};
...
static int PDFwriteResourceDictionary(int objOffset, Rboolean endpage, int excludeDef, PDFDesc *pd)
{
    ...
    if (pd->cidfonts)
    {
        ...
        while (fontlist)
        {
            for (i = 0; i < 5; i++)
            {
                PDFwrite(buf, 100, "/F%d %d 0 R ", pd, 1000 + cidnfonts + 1, ++objCount);
                cidnfonts++;
            }
            fontlist = fontlist->next;
        }
    ...
    if (nraster > 0 || pd->numDefns > 0)
    {
        ...
        {
            PDFwrite(buf, 100, "  /Im%d %d 0 R\n", pd, i, pd->rasters[i].nobj);
            if (pd->masks[i] >= 0)
                PDFwrite(buf, 100, "  /Mask%d %d 0 R\n", pd, pd->masks[i], pd->rasters[i].nmaskobj);
        }
...
static void PDF_endfile(PDFDesc *pd)
{
    ...
    fprintf(pd->pdffp, "] /Count %d /MediaBox [0 0 %d %d] >>\nendobj\n", pd->pageno, (int)(0.5 + pd->paperwidth),
            (int)(0.5 + pd->paperheight));
        ...
        while (fontlist)
        {
            for (i = 0; i < 4; i++)
            {
                ...
                fprintf(pd->pdffp,
                         ...
                         */
                        getCIDFontPDFResource(fontlist->cidfamily->fxname), fontlist->cidfamily->cmap /* /Encoding   */
                );
                cidnfonts++;
            }
            ...
            pd->pos[++pd->nobjs] = (int)ftell(pd->pdffp);
            fprintf(pd->pdffp, "%d 0 obj\n<<\n/Type /Font\n/Subtype /Type1\n/Name /F%d\n/BaseFont /%s\n>>\nendobj\n",
                    pd->nobjs, 1000 + cidnfonts + 1, fontlist->cidfamily->symfont->name);
            cidnfonts++;
            fontlist = fontlist->next;
        }
    }
...
static void PDF_endpage(PDFDesc *pd)
{
    ...
    if (pd->nobjs + 2 * (pd->numRasters - pd->writtenRasters) + 500 >= pd->max_nobjs)
    {
    ...
    for (int i = pd->writtenRasters; i < pd->numRasters; i++)
    {
        ...
        writeRasterXObject(pd->rasters[i], pd->nobjs, pd->masks[i], pd->nobjs + 1, pd);
        if (pd->masks[i] >= 0)
        {
            pd->pos[++pd->nobjs] = (int)ftell(pd->pdffp);
...
static void PDF_NewPage(const pGEcontext gc, pDevDesc dd)
{
    ...
    if (pd->useCompression)
    {
        ...
        if (!pd->pdffp)
        {
            pd->pdffp = pd->mainfp;
            pd->useCompression = 0;
            warning(_("Cannot open temporary file '%s' for compression (reason: %s); compression has been turned off "
                      "for this device"),
                    tmp, strerror(errno));
        }
        free(tmp);
...
static void PDF_Circle(double x, double y, double r, const pGEcontext gc, pDevDesc dd)
{
    ...
    if (r <= 0.0)
        return; /* since PR#14797 use 0-sized pch=1, but now
    ...
    if (code)
    {
        if (semiTransparent(gc->col) || semiTransparent(gc->fill) || r > 10 || !pd->dingbats)
        {
            /*
             ...
             */
            {
                ...
                PDFwrite(buf, 100, "  %.2f %.2f %.2f %.2f %.2f %.2f c\n", pd, x - r, y + s, x - s, y + r, x, y + r);
                PDFwrite(buf, 100, "  %.2f %.2f %.2f %.2f %.2f %.2f c\n", pd, x + s, y + r, x + r, y + s, x + r, y);
                PDFwrite(buf, 100, "  %.2f %.2f %.2f %.2f %.2f %.2f c\n", pd, x + r, y - s, x + s, y - r, x, y - r);
                PDFwrite(buf, 100, "  %.2f %.2f %.2f %.2f %.2f %.2f c\n", pd, x - s, y - r, x - r, y - s, x - r, y);
                if (pd->appendingPath < 0)
                {
        ...
        else
        {
            ...
            */
            ...
            {
                tr = (R_OPAQUE(gc->fill)) + 2 * (R_OPAQUE(gc->col)) - 1;
            }
            if (!pd->inText)
                texton(pd);
            PDFwrite(buf, 100, "/F1 1 Tf %d Tr %.2f 0 0 %.2f %.2f %.2f Tm", pd, tr, a, a, xx, yy);
...
static int PDFfontNumber(const char *family, int face, PDFDesc *pd)
{
    ...
    if (strlen(family) > 0)
    {
         ...
         */
        type1fontfamily fontfamily = findDeviceFont(family, pd->fonts, &fontIndex);
        cidfontfamily cidfontfamily = findDeviceCIDFont(family, pd->cidfonts, &cidfontIndex);
        if (fontfamily)
...
static void PDFWriteT1KerningString(const char *str, FontMetricInfo *metrics, const pGEcontext gc, PDFDesc *pd)
{
    unsigned char p1, p2;
    size_t i, n;
                          ...
                          pDevDesc dd)
{
    ...
    if (pd->appendingPath >= 0 && pd->pathContainsDrawing)
    {
    ...
    else
    {
        ...
        PDFwrite(buf, 200, "/F%d 1 Tf %.2f %.2f %.2f %.2f %.2f %.2f Tm ", pd, font, a, b, bm, a, x, y);
        if (pd->useKern && isType1Font(gc->fontfamily, PDFFonts, pd->defaultFont))
        {
            PDFWriteT1KerningString(str, PDFmetricInfo(gc->fontfamily, face, pd), gc, pd);
        }
        else
        {
            PDFWriteString(str, strlen(str), pd);
                      ...
                      pDevDesc dd)
{
    ...
    else
    {
        ...
        if (pd->useKern && isType1Font(gc->fontfamily, PDFFonts, pd->defaultFont))
        {
            PDFWriteT1KerningString(str1, PDFmetricInfo(gc->fontfamily, face, pd), gc, pd);
        }
        else
        {
...
static FontMetricInfo *PDFCIDsymbolmetricInfo(const char *family, PDFDesc *pd)
{
    FontMetricInfo *result = NULL;
    if (strlen(family) > 0)
    {
         ...
         */
        cidfontfamily fontfamily = findDeviceCIDFont(family, pd->cidfonts, &dontcare);
        if (fontfamily)
            result = &(fontfamily->symfont->metrics);
        else
        {
...
static FontMetricInfo *PDFmetricInfo(const char *family, int face, PDFDesc *pd)
{
    FontMetricInfo *result = NULL;
    if (strlen(family) > 0)
    {
         ...
         */
        type1fontfamily fontfamily = findDeviceFont(family, pd->fonts, &dontcare);
        if (fontfamily)
            result = &(fontfamily->fonts[face - 1]->metrics);
        else
        {
...
double PDF_StrWidth(const char *str, const pGEcontext gc, pDevDesc dd)
{
    ...
    { /* cidfont(gc->fontfamily) */
        if (gc->fontface < 5)
        {
            return floor(gc->cex * gc->ps + 0.5) *
                   PostScriptStringWidth((const unsigned char *)str, CE_NATIVE, NULL, FALSE, gc->fontface, NULL);
        }
        else
        {
            return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((const unsigned char *)str, CE_NATIVE,
...
static double PDF_StrWidthUTF8(const char *str, const pGEcontext gc, pDevDesc dd)
{
    ...
    { /* cidfont(gc->fontfamily) */
        ...
        {
            return floor(gc->cex * gc->ps + 0.5) *
                   PostScriptStringWidth((const unsigned char *)str, CE_UTF8, NULL, FALSE, gc->fontface, NULL);
        }
        else
        {
            return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((const unsigned char *)str, CE_UTF8,
...
void PDF_MetricInfo(int c, const pGEcontext gc, double *ascent, double *descent, double *width, pDevDesc dd)
{
    ...
    { /* cidfont(gc->fontfamily) */
        ...
        {
            PostScriptMetricInfo(c, ascent, descent, width, PDFCIDsymbolmetricInfo(gc->fontfamily, pd), TRUE, "");
        }
    }
    *ascent = floor(gc->cex * gc->ps + 0.5) * *ascent;
    *descent = floor(gc->cex * gc->ps + 0.5) * *descent;
    ...
    if (pd->appendingPath < 0)
    {
        ...
        if (index >= 0)
        {
            if (trans != R_NilValue)
            {
                ...
                PDFwrite(buf, 4, "q\n", pd);
                /* Apply the transform */
                PDFwrite(buf, 100, "%f %f %f %f %f %f cm\n", pd, REAL(trans)[0], REAL(trans)[3], REAL(trans)[1],
                         REAL(trans)[4], REAL(trans)[2], REAL(trans)[5]);
            }
    ...
    if (index >= 0)
    {
        ...
        if (code)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                PDFFillStrokePath(index, rule, pd);
                break;
            }
        }
    }
