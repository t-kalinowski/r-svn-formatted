...
static void writeMaskXObject(rasterImage raster, int n, PDFDesc *pd)
{
    ...
    int res = compress(buf, &outlen, buf, inlen);
    if (res != Z_OK)
        error("internal error %d in writeMaskXObject", res);
    fprintf(pd->pdffp, "%d 0 obj <<\n", n);
    fprintf(pd->pdffp, "  /Type /XObject\n");
    fprintf(pd->pdffp, "  /Subtype /Image\n");
    fprintf(pd->pdffp, "  /Width %d\n", raster.w);
...
static void PDFwritesRGBcolorspace(PDFDesc *pd)
{
    ...
    snprintf(buf, BUFSIZE2, "%s%slibrary%sgrDevices%sicc%ssrgb.flate", R_Home, FILESEP, FILESEP, FILESEP, FILESEP);
...
static void PDF_endfile(PDFDesc *pd)
{
    ...
    fprintf(pd->mainfp, "] /Count %d /MediaBox [0 0 %d %d] >>\nendobj\n", pd->pageno, (int)(0.5 + pd->paperwidth),
    ...
    /* Dingbats always F1 */
    if (pd->fontUsed[1])
        fprintf(pd->mainfp, " /F1 %d 0 R ", ++tempnobj);
    ...
    if (pd->cidfonts)
    {
        ...
        while (fontlist)
        {
            for (i = 0; i < 5; i++)
            {
                fprintf(pd->mainfp, "/F%d %d 0 R ", 1000 + cidnfonts + 1, ++tempnobj);
                cidnfonts++;
            }
            fontlist = fontlist->next;
        }
            ...
            for (i = 0; i < 5; i++)
            {
                if (nfonts >= 100 || pd->fontUsed[nfonts])
                {
                    ...
                    if (!base)
                    {
                        ...
                        fprintf(pd->mainfp, "/FirstChar %d /LastChar %d /Widths [\n", first, last);
