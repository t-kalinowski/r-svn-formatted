...
static double PostScriptStringWidth(unsigned char *p, FontMetricInfo *metrics, int face)
{
    int sum = 0, i;
    short wx;
    ...
    if (utf8locale && !utf8strIsASCII((char *)p) && face != 5)
    {
        ...
        while (*p)
        {
            ...
            for (i = metrics->KPstart[wc]; i < metrics->KPend[wc]; i++)
                ...
                if (metrics->KernPairs[i].c2 == wc2 && metrics->KernPairs[i].c1 == wc)
                {
        ...
        else
        {
            ...
            for (i = 0; i < 5; i++)
            {
                ...
                if (!PostScriptLoadFontMetrics(Family[family].afmfile[i], &(fontfamily->fonts[i]->metrics),
                                               ...
                                               encoding->encnames, (i < 4) ? 1 : 0))
                {
                    warning("cannot read afm file %s", Family[family].afmfile[i]);
                    freeFontFamily(fontfamily);
                    fontfamily = NULL;
                    break;
                }
...
static void utf8toLatin1(char *in, char *out)
{
    ...
    if (!wbuff)
        error("allocation failure in utf8toLatin1");
...
static void PS_Text(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    if (R_OPAQUE(gc->col))
    {
        ...
        if (utf8locale && !utf8strIsASCII(str) && pd->current.font != 5)
        {
            ...
            if (!buff)
                error("allocation failure in PS_Text");
...
static int XF_SetLty(int lty)
{
    ...
    case LTY_DOTDASH:
        return 3;
    default:
        warning("unimplemented line texture %u: using Dash-double-dotted", lty);
        return 4;
    }
}
...
static void PDF_endfile(PDFDesc *pd)
{
    ...
    fprintf(pd->pdffp, "]\n/Count %d\n/MediaBox [0 0 %d %d]\n>>\nendobj\n", pd->pageno, (int)(0.5 + 72 * pd->width),
            (int)(0.5 + 72 * pd->height));
...
static void PDF_Text(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    if ((pd->versionMajor >= 1 && pd->versionMinor >= 4) || (R_OPAQUE(gc->col)))
    {
        ...
        if (utf8locale && !utf8strIsASCII(str1) && face < 5)
        {
            buff = alloca(strlen(str) + 1); /* Output string cannot be longer */
            if (!buff)
                error("allocation failure in PDF_Text");
            utf8toLatin1(str, buff);
            str1 = buff;
        }
#endif
