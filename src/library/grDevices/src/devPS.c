...
{
    if (sscanf(buf, "FontBBox %hd %hd %hd %hd", &(cidmetrics->FontBBox[0]), &(cidmetrics->FontBBox[1]),
               &(cidmetrics->FontBBox[2]), &(cidmetrics->FontBBox[3])) != 4)
        return 0;
#ifdef DEBUG_PS2
    Rprintf("FontBBox %d %d %d %d\n", (cidmetrics->FontBBox[0]), (cidmetrics->FontBBox[1]), (cidmetrics->FontBBox[2]),
...
static int GetCIDCharInfo(char *buf, CIDFontMetricInfo *cidmetrics)
{
    ...
    if (!MatchKey(p, "B "))
        return 0;
    p = SkipToNextItem(p);
    sscanf(p, "%hd %hd %hd %hd", &(cidmetrics->CharInfo[nchar].BBox[0]), &(cidmetrics->CharInfo[nchar].BBox[1]),
           &(cidmetrics->CharInfo[nchar].BBox[2]), &(cidmetrics->CharInfo[nchar].BBox[3]));

#ifdef DEBUG_PS2
    Rprintf("nchar = %04x %d %d %d %d %d\n", nchar, cidmetrics->CharInfo[nchar].WX, cidmetrics->CharInfo[nchar].BBox[0],
            cidmetrics->CharInfo[nchar].BBox[1], cidmetrics->CharInfo[nchar].BBox[2],
 ...
 */
static int LoadEncoding(char *encpath, char *encname, char *encconvname, CNAME *encnames, char *enccode, Rboolean isPDF)
{
    char buf[BUFSIZE];
    int i;
    FILE *fp;
...
static int PostScriptLoadCIDFontMetrics(const char *const fontpath, CIDFontMetricInfo *cidmetrics, char *fontname)
{
    ...
    snprintf(buf, BUFSIZE, "%s%slibrary%sgrDevices%sCID%s%s", R_Home, FILESEP, FILESEP, FILESEP, FILESEP, fontpath);
#ifdef DEBUG_PS
 ...
 */
static encodinginfo findDeviceEncoding(char *encpath, encodinglist enclist, int *index)
{
    encodinginfo encoding = NULL;
    int found = 0;
    *index = 0;
 ...
 */
static encodinginfo addEncoding(char *encpath, Rboolean isPDF)
{
...
static type1fontfamily findLoadedFont(char *name, char *encoding, Rboolean isPDF)
{
 ...
 */
static cidfontfamily findDeviceCIDFont(char *name, cidfontlist fontlist, int *index)
{
    cidfontfamily font = NULL;
    int found = 0;
    *index = 0;
 ...
 * font database
 */
static char *fontMetricsFileName(char *family, int faceIndex, char *fontdbname)
{
    int i, nfonts;
    char *result = NULL;
    int found = 0;
...
{
    char *result = CHAR(STRING_ELT(getAttrib(getFont(family, fontdbname), R_ClassSymbol), 0));
    return result;
}

static Rboolean isType1Font(char *family, char *fontdbname, type1fontfamily defaultFont)
{
    /*
    ...
    if (strlen(family) == 0)
    {
        ...
        return !strcmp(getFontType(family, fontdbname), "Type1Font");
}

static Rboolean isCIDFont(char *family, char *fontdbname, cidfontfamily defaultCIDFont)
{
    /*
     * If family is "" then we're referring to the default device
     * font, so the test is just whether the default font is
    ...
    if (strlen(family) == 0)
    {
        ...
        return !strcmp(getFontType(family, fontdbname), "CIDFont");
}
    ...
    if (fontfamily)
    {
        ...
        else
        {
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                /* ### */
                if (!PostScriptLoadCIDFontMetrics(afmpath, &(fontfamily->cidfonts[i]->cidmetrics),
...
static type1fontfamily addFont(char *name, Rboolean isPDF, encodinglist deviceEncodings)
{
...
static type1fontfamily addDefaultFontFromAFMs(char *encpath, char **afmpaths, Rboolean isPDF,
...
static void PSEncodeFonts(FILE *fp, PostScriptDesc *pd)
{
    ...
    while (cidfonts)
    {
        ...
        fprintf(fp, CIDBoldFontStr, cidfonts->cidfamily->cidfonts[0]->name, cidfonts->cidfamily->cidfonts[0]->name);
        for (i = 0; i < 4; i++)
        {
            char *fmt = NULL /* -Wall */;
            ...
            fprintf(fp, fmt, cidfonts->cidfamily->cidfonts[0]->name, cidfonts->cidfamily->cmap);
            fprintf(fp, "dup length dict begin\n");
            fprintf(fp, "  {1 index /FID ne {def} {pop pop} ifelse} forall\n");
            fprintf(fp, "  currentdict\n");
            fprintf(fp, "  end\n");
...
static void PSFileHeader(FILE *fp, char *papername, double paperwidth, double paperheight, Rboolean landscape,
                         int EPSFheader, Rboolean paperspecial, double left, double bottom, double right, double top,
                         char *title, PostScriptDesc *pd)
{
    int i;
    SEXP prolog;
    type1fontlist fonts = pd->fonts;
...
#ifdef SUPPORT_MBCS
static void PostScriptHexText(FILE *fp, double x, double y, char *str, int strlen, double xc, double yc, double rot)
{
...
static void PS_Circle(double x, double y, double r, R_GE_gcontext *gc, NewDevDesc *dd);
...
static void PS_NewPage(R_GE_gcontext *gc, NewDevDesc *dd);
static Rboolean PS_Open(NewDevDesc *, PostScriptDesc *);
static void PS_Polygon(int n, double *x, double *y, R_GE_gcontext *gc, NewDevDesc *dd);
static void PS_Polyline(int n, double *x, double *y, R_GE_gcontext *gc, NewDevDesc *dd);
static void PS_Rect(double x0, double y0, double x1, double y1, R_GE_gcontext *gc, NewDevDesc *dd);
static void PS_Size(double *left, double *right, double *bottom, double *top, NewDevDesc *dd);
static double PS_StrWidth(char *str, R_GE_gcontext *gc, NewDevDesc *dd);
static void PS_Text(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc, NewDevDesc *dd);
#ifdef SUPPORT_MBCS
static void PS_TextCIDWrapper(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc,
                              NewDevDesc *dd);
#endif
 ...
 */
static CIDFontMetricInfo *CIDmetricInfo(char *family, int face, PostScriptDesc *pd)
{
...
static FontMetricInfo *metricInfo(char *family, int face, PostScriptDesc *pd)
{
    FontMetricInfo *result = NULL;
    int fontIndex;
...
static double PS_StrWidth(char *str, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    if (isType1Font(gc->fontfamily, PostScriptFonts, pd->defaultFont))
    {
        return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((unsigned char *)str,
                                                                     metricInfo(gc->fontfamily, face, pd), NULL, face,
    ...
    { /* cidfont(gc->fontfamily, PostScriptFonts) */
        if (face < 5)
        {
            return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((unsigned char *)str, NULL,
                                                                         CIDmetricInfo(gc->fontfamily, face, pd), face,
                                                                         NULL);
        }
        else
        {
            return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((unsigned char *)str,
                                                                         ...
                                                                         CIDsymbolmetricInfo(gc->fontfamily, pd), NULL,
                                                                         face, NULL);
        }
    }
}
...
static void PS_MetricInfo(int c, R_GE_gcontext *gc, double *ascent, double *descent, double *width, NewDevDesc *dd)
{
    ...
    if (isType1Font(gc->fontfamily, PostScriptFonts, pd->defaultFont))
    {
        PostScriptMetricInfo(c, ascent, descent, width, metricInfo(gc->fontfamily, face, pd));
    }
    else
    { /* cidfont(gc->fontfamily, PostScriptFonts) */
        if (face < 5)
        {
            PostScriptCIDMetricInfo(c, ascent, descent, width, CIDmetricInfo(gc->fontfamily, face, pd));
        }
        else
        {
            PostScriptMetricInfo(c, ascent, descent, width, CIDsymbolmetricInfo(gc->fontfamily, pd));
        }
    }
...
static void drawSimpleText(double x, double y, char *str, double rot, double hadj, int font, R_GE_gcontext *gc,
                           NewDevDesc *dd)
{
    PostScriptDesc *pd = (PostScriptDesc *)dd->deviceSpecific;
...
#ifdef SUPPORT_MBCS
static void PS_TextCIDWrapper(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc, NewDevDesc *dd)
{
    char *str1 = str;
    char *buff;
    ...
    if (gc->fontface == 5)
    {
        ...
        {
            drawSimpleText(x, y, str1, rot, hadj, translateCIDFont(gc->fontfamily, gc->fontface, pd), gc, dd);
            return;
        }
        else
        {
    ...
    if (mbcslocale && isCIDFont(gc->fontfamily, PostScriptFonts, pd->defaultCIDFont))
    {
        size_t ucslen;
        int fontIndex;
        ...
        if (!strcmp(locale2charset(NULL), cidfont->encoding))
        {
            ...
            {
                SetColor(gc->col, dd);
                PostScriptHexText(pd->psfp, x, y, str, strlen(str), hadj, 0.0, rot);
            }
            return;
        }
        ...
        if ((size_t)-1 != ucslen)
        {
            ...
            else
            {
                ...
                {
                    SetColor(gc->col, dd);
                    PostScriptHexText(pd->psfp, x, y, (char *)buf, nb - o_len, hadj, 0.0, rot);
                }
            }
            return;
        }
...
static void XF_FileHeader(FILE *fp, char *papername, Rboolean landscape, Rboolean onefile)
{
    fprintf(fp, "#FIG 3.2\n");
...
static void XFig_Circle(double x, double y, double r, R_GE_gcontext *gc, NewDevDesc *dd);
...
static void XFig_Polygon(int n, double *x, double *y, R_GE_gcontext *gc, NewDevDesc *dd);
static void XFig_Polyline(int n, double *x, double *y, R_GE_gcontext *gc, NewDevDesc *dd);
static void XFig_Rect(double x0, double y0, double x1, double y1, R_GE_gcontext *gc, NewDevDesc *dd);
static void XFig_Size(double *left, double *right, double *bottom, double *top, NewDevDesc *dd);
static double XFig_StrWidth(char *str, R_GE_gcontext *gc, NewDevDesc *dd);
static void XFig_Text(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc, NewDevDesc *dd);
...
static void XFig_Text(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    if (R_OPAQUE(gc->col))
    {
         ...
         */
        fprintf(fp, "%d %d ", (int)(16.667 * GStrHeight(str, DEVICE, GetDevice(devNumber((DevDesc *)dd))) + 0.5),
                (int)(16.667 * GStrWidth(str, DEVICE, GetDevice(devNumber((DevDesc *)dd))) + 0.5));
...
static void XFig_MetricInfo(int c, R_GE_gcontext *gc, double *ascent, double *descent, double *width, NewDevDesc *dd)
{
    ...
    PostScriptMetricInfo(c, ascent, descent, width, &(pd->fonts->family->fonts[face - 1]->metrics));
    *ascent = floor(gc->cex * gc->ps + 0.5) * *ascent;
    *descent = floor(gc->cex * gc->ps + 0.5) * *descent;
    *width = floor(gc->cex * gc->ps + 0.5) * *width;
}
...
static void PDF_Circle(double x, double y, double r, R_GE_gcontext *gc, NewDevDesc *dd);
...
static void PDF_Polygon(int n, double *x, double *y, R_GE_gcontext *gc, NewDevDesc *dd);
static void PDF_Polyline(int n, double *x, double *y, R_GE_gcontext *gc, NewDevDesc *dd);
...
static double PDF_StrWidth(char *str, R_GE_gcontext *gc, NewDevDesc *dd);
static void PDF_Text(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc, NewDevDesc *dd);
#ifdef SUPPORT_MBCS
static void PDF_TextCIDWrapper(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc,
                               NewDevDesc *dd);
#endif
...
Rboolean PDFDeviceDriver(NewDevDesc *dd, char *file, char *paper, char *family, char **afmpaths, char *encoding,
                         char *bg, char *fg, double width, double height, double ps, int onefile, int pagecentre,
                         char *title, SEXP fonts, int versionMajor, int versionMinor)
{
    /* If we need to bail out with some sort of "error" */
    /* then we must free(dd) */
...
{
    return (pd->versionMajor > 1 || (pd->versionMajor == 1 && pd->versionMinor >= 4));
}
...
static void PDF_endfile(PDFDesc *pd)
{
    ...
    if (pd->cidfonts)
    {
        ...
        while (fontlist)
        {
            for (i = 0; i < 5; i++)
            {
                fprintf(pd->pdffp, "/F%d %d 0 R ", 1000 + cidnfonts + 1, ++tempnobj);
                cidnfonts++;
            }
            fontlist = fontlist->next;
        }
...
static void PDF_Rect(double x0, double y0, double x1, double y1, R_GE_gcontext *gc, NewDevDesc *dd)
{
     ...
     */
    if ((semiTransparent(gc->col) || semiTransparent(gc->fill)) && alphaVersion(pd))
    {
        if (pd->inText)
            textoff(pd);
        PDF_SetFill(gc->fill, dd);
        ...
        if (code)
        {
            ...
            else
            {
                ...
                */
                ...
                if (!pd->inText)
                    texton(pd);
                fprintf(pd->pdffp, "/F1 1 Tf %d Tr %.2f 0 0 %.2f %.2f %.2f Tm", tr, a, a, xx, yy);
...
static void PDFSimpleText(double x, double y, char *str, double rot, double hadj, int font, R_GE_gcontext *gc,
                          NewDevDesc *dd)
{
    PDFDesc *pd = (PDFDesc *)dd->deviceSpecific;
                               ...
                               NewDevDesc *dd)
{
    ...
    if (mbcslocale && isCIDFont(gc->fontfamily, PDFFonts, pd->defaultCIDFont) && face != 5)
    {
        ...
        if ((size_t)-1 != ucslen)
        {
            ...
            if ((size_t)-1 == status)
                warning(_("failed in text conversion to encoding '%s'"), cidfont->encoding);
            else if (alphaVersion(pd) || (R_OPAQUE(gc->col)))
            {
                PDF_SetFill(gc->col, dd);
                fprintf(pd->pdffp, "/F%d 1 Tf %.2f %.2f %.2f %.2f %.2f %.2f Tm <",
                        PDFfontNumber(gc->fontfamily, face, pd), a, b, -b, a, x, y);
...
static CIDFontMetricInfo *PDFCIDmetricInfo(char *family, int face, PDFDesc *pd)
{
...
static FontMetricInfo *PDFCIDsymbolmetricInfo(char *family, PDFDesc *pd)
{
...
static FontMetricInfo *PDFmetricInfo(char *family, int face, PDFDesc *pd)
{
    FontMetricInfo *result;
    if (strlen(family) > 0)
    {
...
static double PDF_StrWidth(char *str, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    if (isType1Font(gc->fontfamily, PDFFonts, pd->defaultFont))
    {
        return floor(gc->cex * gc->ps + 0.5) *
               PostScriptStringWidth((unsigned char *)str, PDFmetricInfo(gc->fontfamily, gc->fontface, pd), NULL,
                                     gc->fontface, PDFconvname(gc->fontfamily, pd));
    }
    else
    { /* cidfont(gc->fontfamily) */
        ...
        {
            return floor(gc->cex * gc->ps + 0.5) *
                   PostScriptStringWidth((unsigned char *)str, NULL, PDFCIDmetricInfo(gc->fontfamily, gc->fontface, pd),
                                         gc->fontface, NULL);
        }
        else
        {
...
static void PDF_MetricInfo(int c, R_GE_gcontext *gc, double *ascent, double *descent, double *width, NewDevDesc *dd)
{
    ...
    if (isType1Font(gc->fontfamily, PDFFonts, pd->defaultFont))
    {
        PostScriptMetricInfo(c, ascent, descent, width, PDFmetricInfo(gc->fontfamily, gc->fontface, pd));
    }
    else
    { /* cidfont(gc->fontfamily) */
        if (face < 5)
        {
            PostScriptCIDMetricInfo(c, ascent, descent, width, PDFCIDmetricInfo(gc->fontfamily, gc->fontface, pd));
        }
        else
        {
            PostScriptMetricInfo(c, ascent, descent, width, PDFCIDsymbolmetricInfo(gc->fontfamily, pd));
        }
    }
    *ascent = floor(gc->cex * gc->ps + 0.5) * *ascent;
    *descent = floor(gc->cex * gc->ps + 0.5) * *descent;
...
SEXP PDF(SEXP args)
{
    ...
    GEDevDesc *dd;
    char *vmax;
    char *file, *paper, *encoding, *family = NULL /* -Wall */, *bg, *fg, *title, call[] = "PDF", *afms[5];
    double height, width, ps;
