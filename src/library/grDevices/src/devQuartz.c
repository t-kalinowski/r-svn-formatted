...
{
    return GConvertX(x, GMapUnits(0), GMapUnits(1), GetDevice(devNumber((DevDesc *)((QuartzDesc *)desc)->dev)));
}
double QuartzDevice_UserY(QuartzDesc_t desc, double y)
{
    return GConvertX(y, GMapUnits(0), GMapUnits(1), GetDevice(devNumber((DevDesc *)((QuartzDesc *)desc)->dev)));
}
...
void *QuartzDevice_Create(
    ...
    int (*locatePoint)(QuartzDesc_t dev, void *userInfo, double *x, double *y),
    void (*close)(QuartzDesc_t dev, void *userInfo), void (*newPage)(QuartzDesc_t dev, void *userInfo),
    void (*state)(QuartzDesc_t dev, void *userInfo, int state),
...
SEXP Quartz(SEXP args)
{
    ...
    { /* try internal modules next */
        ...
        {
        case QBE_COCOA:
            succ = QuartzCocoa_DeviceCreate(dev, type, file, width, height, ps, family, antialias, smooth, autorefresh,
                                            quartzpos, bg, title, dpi);
            break;
        case QBE_NATIVE:
            ...
            if (succ)
                break;
        case QBE_CARBON:
            succ = QuartzCarbon_DeviceCreate(dev, type, file, width, height, ps, family, antialias, smooth, autorefresh,
                                             quartzpos, bg, title, dpi);
            break;
        case QBE_PDF:
            succ = QuartzPDF_DeviceCreate(dev, type, file, width, height, ps, family, antialias, smooth, autorefresh,
                                          quartzpos, bg, title, dpi);
            break;
        case QBE_BITMAP:
            succ = QuartzBitmap_DeviceCreate(dev, type, file, width, height, ps, family, antialias, smooth, autorefresh,
                                             quartzpos, bg, dpi);
            break;
        }
    }
