...
static void RQuartz_Raster(unsigned int *raster, int w, int h, double x, double y, double width, double height,
...
static QuartzFunctions_t qfn = {
    QuartzDevice_Create,          QuartzDevice_DevNumber,         QuartzDevice_Kill,
    QuartzDevice_ResetContext,    QuartzDevice_GetWidth,          QuartzDevice_GetHeight,
    QuartzDevice_SetSize,         QuartzDevice_GetScaledWidth,    QuartzDevice_GetScaledHeight,
    QuartzDevice_SetScaledSize,   QuartzDevice_GetXScale,         QuartzDevice_GetYScale,
    QuartzDevice_SetScale,        QuartzDevice_SetTextScale,      QuartzDevice_GetTextScale,
    QuartzDevice_SetPointSize,    QuartzDevice_GetPointSize,      QuartzDevice_GetDirty,
    QuartzDevice_SetDirty,        QuartzDevice_ReplayDisplayList, QuartzDevice_GetSnapshot,
    QuartzDevice_RestoreSnapshot, QuartzDevice_GetAntialias,      QuartzDevice_SetAntialias,
    QuartzDevice_GetBackground,   QuartzDevice_Activate,          QuartzDevice_SetParameter,
...
__attribute__((constructor)) static void RQ_init()
{
    ...
    if ((r = dlsym(RTLD_NEXT, "CGFontGetGlyphsForUnichars")) || (r = dlsym(RTLD_NEXT, "CGFontGetGlyphsForUnicodes")) ||
        (r = dlsym(RTLD_DEFAULT, "CGFontGetGlyphsForUnichars")) ||
        (r = dlsym(RTLD_DEFAULT, "CGFontGetGlyphsForUnicodes")))
        RQFontGetGlyphsForUnichars = (RQFontGetGlyphsForUnichars_t)r;
    else
        error("Cannot load CoreGraphics"); /* this should never be reached but I suppose it's better than a hidden
                                              segfault */
}
...
CGFontRef RQuartz_Font(CTXDESC)
{
    ...
    if (fontFace < 1 || fontFace > 5)
        fontFace = 1; /* just being paranoid */
    if (fontFace == 5)
        fontName = "Symbol";
    else
 ...
 proper font name from the family name and face */
           ...
           text-matching heuristics ... very nasty ... */
            ...
            while (!atsFont)
            { /* try different faces until exhausted or successful */
                strcpy(compositeFontName, fontFamily);
                if (fontFace == 2 || fontFace == 4)
                    strcat(compositeFontName, " Bold");
                if (fontFace == 3 || fontFace == 4)
                    strcat(compositeFontName, " Italic");
                ...
                {
                    if (fontFace == 1)
                    { /* more guessing - fontFace == 1 may need Regular or Roman */
                    ...
                    else if (fontFace == 3 || fontFace == 4)
                    { /* Oblique is sometimes used instead of Italic (e.g. in Helvetica) */
                        strcpy(compositeFontName, fontFamily);
                        if (fontFace == 4)
                            strcat(compositeFontName, " Bold");
                ...
                if (!atsFont)
                { /* try to fall back to a more plain face */
                    if (fontFace == 4)
                        fontFace = 2;
                    else if (fontFace != 1)
                        fontFace = 1;
...
void RQuartz_Set(CGContextRef ctx, const pGEcontext gc, int flags)
{
    ...
    {
        int fill = gc->fill;
        CGFloat fillColor[] = {R_RED(fill) / 255.0, R_GREEN(fill) / 255.0, R_BLUE(fill) / 255.0, R_ALPHA(fill) / 255.0};
    ...
    {
        int stroke = gc->col;
        CGFloat strokeColor[] = {R_RED(stroke) / 255.0, R_GREEN(stroke) / 255.0, R_BLUE(stroke) / 255.0,
                           ...
                           double rot, Rboolean interpolate, const pGEcontext gc, pDevDesc dd)
{
    ...
    img = CGImageCreate(w, h, 8, /* bits per channel */
                        32,      /* bits per pixel */
                        4 * w,   /* bytes per row */
                        cs,      /* color space */
    /* R uses AGBR which is so unusual (inverted RGBA) that it corresponds to endinness inverse(!) to the host with
     * alpha last (=RGBA).  */
...
static void RQuartz_Path(double *x, double *y, int npoly, int *nper, Rboolean winding, CTXDESC)
{
    int i, j, index;
    DRAWSPEC;
    if (!ctx)
...
static void RQuartz_MetricInfo(int c, const pGEcontext gc, double *ascent, double *descent, double *width, pDevDesc dd)
{
    DRAWSPEC;
    if (!ctx)
...
static int has_wss()
{
    ...
    if (darwin_version() < 11.0)
    { /* before Lion we get reliable information from the bootstrap info */
        kern_return_t kr;
        ...
        if (kr == KERN_SUCCESS)
        {
            ...
            kr = bootstrap_info(bport, &serviceNames, &serviceNameCount, &serverNames, &serverNameCount, &active,
                                &activeCount);
            if (kr == KERN_SUCCESS)
            {
                unsigned int i = 0;
    ...
    else
    {
        ...
        if (dict)
        { /* allright, let's see if the session is current */
