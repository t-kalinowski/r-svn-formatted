    ...
    if (strlen(fontfamily) > 0)
    {
        ...
        for (i = 0; i < length(names); i++)
            if (0 == strcmp(fontfamily, CHAR(STRING_ELT(names, i))))
                break;
        if (i < length(names))
            fontName = CFStringCreateWithCString(kCFAllocatorDefault, CHAR(STRING_ELT(VECTOR_ELT(db, i), fontface - 1)),
...
CGFontRef RQuartz_Font(CTXDESC)
{
    ...
    if (font == NULL)
    {
        ...
        cFontName[0] = '\0';
        CFStringGetCString(
            fontName, cFontName, 128,
            kCFStringEncodingUTF8); /* FIXME: potentially UTF-8 is not the current locale - does it matter? */
