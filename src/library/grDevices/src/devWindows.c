...
#define SMALLEST 1
    ...
    static void SetFont(pGEcontext gc, double rot, gadesc *xd)
    {
        ...
        if (size < SMALLEST)
            size = SMALLEST;
        if (size != xd->fontsize || face != xd->fontface || rot != xd->fontangle ||
            strcmp(gc->fontfamily, xd->fontfamily))
        {
            if (xd->font)
                del(xd->font);
            doevent();
...
#define MCHECK(m)                                                                                                      \
    ...
    static Rboolean setupScreenDevice(pDevDesc dd, gadesc * xd, double w, double h, Rboolean recording, int resize,
                                      int xpos, int ypos)
    {
        menu m;
        int iw, ih;
    ...
    static Rboolean GA_Open(pDevDesc dd, gadesc * xd, const char *dsp, double w, double h, Rboolean recording,
                            int resize, int canvascolor, double gamma, int xpos, int ypos, int bg)
    {
        rect rr;
    ...
    static void GA_Resize(pDevDesc dd)
    {
        ...
        if (xd->resize)
        {
            ...
            else if (xd->resizing == 3)
            {
                ...
                dd->right = iw0 + shift;
                xd->xshift = shift;
                gchangescrollbar(xd->gawin, HWINSB, max(-shift, 0) / SF, xd->origWidth / SF - 1, xd->windowWidth / SF,
                                 0);
                if (ih0 < ih)
                    shift = (ih - ih0) / 2.0;
                else
                ...
                dd->bottom = ih0 + shift;
                xd->yshift = shift;
                gchangescrollbar(xd->gawin, VWINSB, max(-shift, 0) / SF, xd->origHeight / SF - 1, xd->windowHeight / SF,
                                 0);
...
#define DRAW2(col)                                                                                                     \
    ...
    static void GA_Raster(unsigned int *raster, int w, int h, double x, double y, double width, double height,
    ...
    static void GA_Text_UTF8(double x, double y, const char *str, double rot, double hadj, const pGEcontext gc,
                             pDevDesc dd)
    {
    ...
    static int Load_Rbitmap_Dll()
    {
        if (!RbitmapAlreadyLoaded)
        {
            ...
            if (((hRbitmapDll = LoadLibrary(szFullPath)) != NULL) &&
                ((R_SaveAsPng = (R_SaveAsBitmap)GetProcAddress(hRbitmapDll, "R_SaveAsPng")) != NULL) &&
                ((R_SaveAsBmp = (R_SaveAsBitmap)GetProcAddress(hRbitmapDll, "R_SaveAsBmp")) != NULL) &&
                ((R_SaveAsJpeg = (R_SaveAsBitmap)GetProcAddress(hRbitmapDll, "R_SaveAsJpeg")) != NULL) &&
                ((R_SaveAsTIFF = (R_SaveAsBitmap)GetProcAddress(hRbitmapDll, "R_SaveAsTIFF")) != NULL))
            {
    ...
    SEXP devga(SEXP args)
    {
        ...
        BEGIN_SUSPEND_INTERRUPTS
        {
            ...
            if (!GADeviceDriver(dev, display, width, height, ps, (Rboolean)recording, resize, bg, canvas, gamma, xpos,
                                ypos, (Rboolean)buffered, psenv, restoreConsole, title, clickToConfirm, fillOddEven))
            {
                char type[100], *p;
                free(dev);
                if (display[0])
