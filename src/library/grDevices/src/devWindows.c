...
static Rboolean GADeviceDriver(pDevDesc dd, const char *display, double width, double height, double pointsize,
                               ...
                               Rboolean clickToConfirm, Rboolean fillOddEven, const char *family, int quality,
...
#define SH                                                                                                             \
    if (xd->kind == SCREEN && xd->buffered && GA_xd)                                                                   \
    GA_Timer(xd)
...
static char *translateFontFamily(const char *family)
{
    ...
    PROTECT_WITH_INDEX(windowsenv = findVar(install(".WindowsEnv"), graphicsNS), &xpi);
...
static void SetFont(pGEcontext gc, double rot, gadesc *xd)
{
    ...
    if (size < SMALLEST)
        size = SMALLEST;
    if (size != xd->fontsize || face != xd->fontface || rot != xd->fontangle || strcmp(gc->fontfamily, xd->fontfamily))
    {
        if (xd->font)
            del(xd->font);
        doevent();
        ...
        {
            xd->font = gnewfont2(xd->gawin, fontfamily, fontstyle[face - 1], size, rot, usePoints, quality);
        }
        else
        {
            xd->font = gnewfont2(xd->gawin, fontname[face - 1], fontstyle[face - 1], size, rot, usePoints, quality);
        }
        if (xd->font)
        {
        ...
        else
        {
            ...
            if (face > 4)
                face = 1;
            xd->font = gnewfont2(xd->gawin, "Arial", fontstyle[face - 1], size, rot, usePoints, quality);
            if (!xd->font)
...
static void HelpResize(window w, rect r)
{
        ...
        return;
    {
        ...
        if (r.width)
        {
            if ((xd->windowWidth != r.width) || ((xd->windowHeight != r.height)))
            {
...
static void menufilebitmap(control m)
{
    ...
    if (m == xd->mpng)
        ...
        SaveAsTiff(dd, fn);
    else if (m == xd->mjpeg50)
        SaveAsJpeg(dd, 50, fn);
    else if (m == xd->mjpeg75)
        SaveAsJpeg(dd, 75, fn);
    else
        SaveAsJpeg(dd, 100, fn);
    gsetcursor(xd->gawin, ArrowCursor);
    show(xd->gawin);
}
...
static void menups(control m)
{
    ...
    setuserfilter(
        "Encapsulated postscript files (*.eps)\0*.eps\0Postscript files (*.ps)\0*.ps\0All files (*.*)\0*.*\0\0");
...
#define pCHECK                                                                                                         \
    if ((TYPEOF(vDL) != VECSXP) || (TYPEOF(VECTOR_ELT(vDL, 0)) != INTSXP) || (LENGTH(VECTOR_ELT(vDL, 0)) != 1) ||      \
...
#define pMOVE(a)                                                                                                       \
    {                                                                                                                  \
        ...
        if (pCURRENTPOS < 0)                                                                                           \
            pCURRENTPOS = 0;                                                                                           \
        if (pCURRENTPOS > pNUMPLOTS - 1)                                                                               \
            pCURRENTPOS = pNUMPLOTS - 1;                                                                               \
        Replay(dd, vDL);                                                                                               \
        SETDL;                                                                                                         \
    }
...
#define pMUSTEXIST                                                                                                     \
    if (!pEXIST)                                                                                                       \
    {                                                                                                                  \
        R_ShowMessage(_("no plot history!"));                                                                          \
        return;                                                                                                        \
    }
...
static Rboolean setupScreenDevice(pDevDesc dd, gadesc *xd, double w, double h, Rboolean recording, int resize, int xpos,
                                  int ypos)
{
    menu m;
    int iw, ih;
        ...
        xd->rescale_factor = dw / dw0;
    {
        ...
        if (!(xd->gawin = newwindow("R Graphics", rect(grx, gry, iw, ih),
                                    Document | StandardWindow | Menubar | VScrollbar | HScrollbar | CanvasSize)))
        {
    ...
    MCHECK(xd->bm = newbitmap(getwidth(xd->gawin), getheight(xd->gawin), getdepth(xd->gawin)));
    MCHECK(xd->bm2 = newbitmap(getwidth(xd->gawin), getheight(xd->gawin), getdepth(xd->gawin)));
...
static Rboolean GA_Open(pDevDesc dd, gadesc *xd, const char *dsp, double w, double h, Rboolean recording, int resize,
                        int canvascolor, double gamma, int xpos, int ypos, int bg)
{
    ...
    else
    {
        ...
        xd->w = MM_PER_INCH * w;
        xd->h = MM_PER_INCH * h;
        xd->gawin = newmetafile(buf, MM_PER_INCH * w, MM_PER_INCH * h, xd->xpinch, xd->ypinch);
        xd->kind = METAFILE;
        xd->fast = 0; /* use scalable line widths */
        if (!xd->gawin)
        {
...
static void GA_Resize(pDevDesc dd)
{
    ...
    if (xd->resize)
    {
        int iw, ih, iw0 = dd->right - dd->left, ih0 = dd->bottom - dd->top;
...
static void GA_NewPage(const pGEcontext gc, pDevDesc dd)
{
    ...
    xd->warn_trans = FALSE;
    {
        ...
        else
        {
            ...
            if (alpha < 255)
                xd->bgcolor = (alpha * xd->bgcolor + (255 - alpha) * xd->canvascolor) / 255;
        }
    }
    if (xd->kind != SCREEN)
    {
        ...
        if (R_OPAQUE(xd->bg) || xd->kind == BMP || xd->kind == JPEG || xd->kind == TIFF)
        {
...
static void GA_Close(pDevDesc dd)
{
    ...
    else if ((xd->kind == PNG) || (xd->kind == JPEG) || (xd->kind == BMP) || (xd->kind == TIFF))
    {
        if (xd->kind == TIFF)
            xd->npage++;
        SaveAsBitmap(dd, xd->res_dpi);
...
static void GA_Circle(double x, double y, double radius, const pGEcontext gc, pDevDesc dd)
{
    ...
    id = 2 * radius + 0.5;
    if (id < 2)
        id = 2; /* diameter 1 is near-invisible */
...
static Rboolean GA_Locator(double *x, double *y, pDevDesc dd)
{
    ...
    while (!xd->clicked)
    {
        ...
        { /* closing the window on other systems calls error().
                               ...
                               double xpinch, double ypinch)
{
    ...
    xd->xpinch = 0.0;
    xd->ypinch = 0.0;
    if (R_FINITE(xpinch) && xpinch > 0.0 && R_FINITE(ypinch) && ypinch > 0.0)
    {
        xd->xpinch = xpinch;
        xd->ypinch = ypinch;
    }
    ...
    if (xd->fp || xd->kind == TIFF)
    {
        ...
        if (data)
        {
            ...
            else if (xd->kind == TIFF)
            {
                ...
                R_SaveAsTIFF(data, xd->windowWidth, xd->windowHeight, privategetpixel2, 0, buf, res, xd->quality);
            }
            else
...
static void SaveAsTiff(pDevDesc dd, const char *fn)
{
    ...
    {
        png_rows = r2.width;
        R_SaveAsTIFF(data, xd->windowWidth, xd->windowHeight, privategetpixel2, 0, fn, 0, 1 /* no compression */);
        free(data);
    }
    else
        warning(_("processing of the plot ran out of memory"));
...
static int Load_Rcairo_Dll()
{
    if (!RcairoAlreadyLoaded)
    {
            ...
            ((R_devCairo = (R_SaveAsBitmap)GetProcAddress(hRcairoDll, "in_Cairo")) != NULL))
        {
            ...
            R_pangoVersion = (R_pangoVersion_t)GetProcAddress(hRcairoDll, "in_PangoVersion");
            R_cairoFT = (R_cairoFT_t)GetProcAddress(hRcairoDll, "in_CairoFT");
            RcairoAlreadyLoaded = 1;
        }
        else
        {
            if (hRcairoDll != NULL)
                FreeLibrary(hRcairoDll);
            RcairoAlreadyLoaded = -1;
            char buf[1000];
...
{
    if (!Load_Rcairo_Dll() || R_cairoVersion == NULL)
        return mkString("");
    else
