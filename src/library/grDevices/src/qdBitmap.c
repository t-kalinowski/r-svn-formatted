...
void QuartzBitmap_Close(QuartzDesc_t dev, void *userInfo)
{
    ...
    if (qbd->path && qbd->uti)
    {
        ...
        CFStringRef pathString = CFStringCreateWithBytes(kCFAllocatorDefault, (UInt8 *)qbd->path, strlen(qbd->path),
                                                         kCFStringEncodingUTF8, FALSE);
        CFURLRef path;
        if (CFStringFind(pathString, CFSTR("://"), 0).location != kCFNotFound)
        {
            CFStringRef pathEscaped = CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, pathString, NULL,
        ...
        {
            path = CFURLCreateFromFileSystemRepresentation(kCFAllocatorDefault, (const UInt8 *)qbd->path,
                                                           strlen(qbd->path), FALSE);
        }
        CFRelease(pathString);

        CFStringRef scheme = CFURLCopyScheme(path);
        CFStringRef type = CFStringCreateWithBytes(kCFAllocatorDefault, (UInt8 *)qbd->uti, strlen(qbd->uti),
                                                   kCFStringEncodingUTF8, FALSE);
                                   ...
                                   Rboolean autorefresh, int quartzpos, int bg, double *dpi)
{
    ...
    CFStringRef mine =
        CFStringCreateWithBytes(kCFAllocatorDefault, (UInt8 *)type, strlen(type), kCFStringEncodingUTF8, FALSE);
    if (CFArrayContainsValue(types, CFRangeMake(0, CFArrayGetCount(types)), mine))
    {
        ...
        if (!(qd = QuartzDevice_Create(dd, dpi[0] / 72.0, dpi[1] / 72.0, pointsize, width, height, bg, antialias,
                                       smooth, QuartzBitmap_GetCGContext, NULL, /* locate */
