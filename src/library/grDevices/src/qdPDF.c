...
QuartzDesc_t QuartzPDF_DeviceCreate(void *dd, QuartzFunctions_t *fn, QuartzParameters_t *par)
{
    ...
    if (par->parv)
        dev->data = (CFMutableDataRef)CFRetain((CFTypeRef)par->parv); /* parv if set is CFMutableDataRef to write to */
    ...
    { /* optional PDF auxiliary info: we add creator and title (if present) - we could support more ... */
        ...
        ai = CFDictionaryCreate(0, (void *)keys, (void *)values, numK, &kCFTypeDictionaryKeyCallBacks,
                                &kCFTypeDictionaryValueCallBacks);
        while (numK)
            CFRelease(values[--numK]);
    }
