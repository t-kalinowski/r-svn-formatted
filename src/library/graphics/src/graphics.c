...
#define EPS_FAC_1 16
    ...
    {
        warning(_("nonfinite axis=%d limits [GScale(%g,%g,..); log=%d] -- corrected now"), axis, min, max, log);
        if (!R_FINITE(min))
...
#define EPS_CC 1e-15
        ...
        warning(_("nonfinite axis=%d *range* [GScale(%g,%g,..); log=%d] => / %g"), axis, min, max, log, d1);
        min /= d1;
        max /= d1;
