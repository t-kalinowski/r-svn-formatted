...
static SEXP FixupPch(SEXP pch, int dflt)
{
    ...
    {
        for (i = 0; i < n; i++)
            INTEGER(ans)[i] = R_FINITE(REAL(pch)[i]) ? (int)REAL(pch)[i] : NA_INTEGER;
    }
...
SEXP C_plot_window(SEXP args)
{
...
#ifdef DEBUG_axis
    REprintf("plot.window(): (xmin,xmax) = (%g,%g); (ymin,ymax) = (%g,%g)", xmin, xmax, ymin, ymax);
    ...
    if (R_FINITE(asp) && asp > 0)
    { // finite 'asp' > 0 specified
        double pin1 = GConvertXUnits(1.0, NPC, INCHES, dd), pin2 = GConvertYUnits(1.0, NPC, INCHES, dd),
               // FIXME:  |zmax - zmin| may be Inf, even when both are finite
            xdelta = fabs(xmax - xmin) / asp, ydelta = fabs(ymax - ymin), scale, xscale, yscale, xadd, yadd;
    ...
    { /* asp is NA, or not positive and finite -- includes logscale ! */
                          ...
                          double *low, double *high)
{
    ...
    { // redo w/o overflow (trying to prevent eager optimization by compilers):
        eps = right * FLT_EPSILON;
        eps -= left * FLT_EPSILON;
    }
    else
    {
        eps *= FLT_EPSILON;
...
SEXP C_axis(SEXP args)
{
    ...
    */
...
#ifdef DEBUG_axis
    REprintf("C_axis(side=%d, x_ax=%s, las=%d, perpendicular=%s, gap = %g):\n", side, x_axis ? "TRUE" : "FALSE",
             gpptr(dd)->las, perpendicular ? "TRUE" : "FALSE", gap);
#endif
    ...
    { //--- x-axis -- horizontal ======================================
        ...
        double axis_base, tck_offset, axis_low = GConvertX(fmin2(high, fmax2(low, REAL(at)[0])), USER, NFC, dd),
        ...
        if (side == 1)
        {
            ...
            if (doticks)
            {
                if (R_FINITE(gpptr(dd)->tck))
                {
                    ...
                    {
                        double xu = GConvertXUnits(gpptr(dd)->tck, NPC, INCHES, dd),
        ...
        { // Tickmark labels. ------------------------------------
            ...
            if (side == 1)
            {
                axis_lab = -axis_base + GConvertYUnits(gpptr(dd)->mgp[1] - lineoff, LINES, NFC, dd) +
            ...
            for (i = istart; i != iend; i += incr)
            {
                ...
                if (low < x && x < high)
                {
                    if (isExpression(lab))
                    {
                        GMMathText(VECTOR_ELT(lab, ind[i]), side, axis_lab, 0, x, gpptr(dd)->las, padjval, dd);
                    }
                    else
                    {
    ...
    { //--- y-axis -- vertical =============================================
        ...
        // (low, high) := Lim(limits[], log)
        GetAxisLimits(limits[0], limits[1], logflag, &low, &high);
        double axis_base, tck_offset, axis_low = GConvertY(fmin2(high, fmax2(low, REAL(at)[0])), USER, NFC, dd),
        ...
        { // Tickmark labels. ------------------------------------
            ...
            if (side == 2)
            {
                axis_lab = -axis_base + GConvertXUnits(gpptr(dd)->mgp[1] - lineoff, LINES, NFC, dd) +
            ...
            for (i = istart; i != iend; i += incr)
            {
                ...
                if (low < y && y < high)
                {
                    if (isExpression(lab))
                    {
                        GMMathText(VECTOR_ELT(lab, ind[i]), side, axis_lab, 0, y, gpptr(dd)->las, padjval, dd);
                    }
                    else
                    {
                        SEXP label = STRING_ELT(lab, ind[i]);
...
SEXP C_path(SEXP args)
{
    ...
    PROTECT(nper = CAR(args));
    args = CDR(args);
    npoly = LENGTH(nper);

    PROTECT(rule = CAR(args));
    args = CDR(args);
    ...
    GPath(xx, yy, npoly, INTEGER(nper), INTEGER(rule)[0] == 1, INTEGER(col)[0], INTEGER(border)[0], dd);
...
SEXP C_raster(SEXP args)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        GConvert(&x1, &y1, USER, DEVICE, dd);
        if (R_FINITE(x0) && R_FINITE(y0) && R_FINITE(x1) && R_FINITE(y1))
            GRaster(image, INTEGER(dim)[1], INTEGER(dim)[0], x0, y0, x1 - x0, y1 - y0, REAL(angle)[i % LENGTH(angle)],
...
static void drawPolygon(int n, double *x, double *y, int lty, int fill, int border, pGEDevDesc dd)
{
    if (lty == NA_INTEGER)
        gpptr(dd)->lty = dpptr(dd)->lty;
    else
...
static double ComputeAtValueFromAdj(double adj, int side, Rboolean outer, pGEDevDesc dd)
{
    double at = 0; /* -Wall */
    switch (side % 2)
    {
...
static double ComputeAtValue(double at, double adj, int side, int las, Rboolean outer, pGEDevDesc dd)
{
    if (!R_FINITE(at))
    {
...
SEXP C_mtext(SEXP args)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (isExpression(text))
            GMMathText(VECTOR_ELT(text, i % ntext), sideval, lineval, outerval, atval, gpptr(dd)->las, padjval, dd);
        else
        {
...
static void drawdend(int node, double *x, double *y, SEXP dnd_llabels, pGEDevDesc dd)
{
