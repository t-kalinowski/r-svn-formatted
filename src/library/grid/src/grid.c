 ...
 */
static void drawArrow(double *x, double *y, SEXP type, int i, R_GE_gcontext *gc, GEDevDesc *dd)
{
    int nt = LENGTH(type);
...
static void calcArrow(double x1, double y1, double x2, double y2, SEXP angle, SEXP length, int i, LViewportContext vpc,
                      double vpWidthCM, double vpHeightCM, double *vertx, double *verty, R_GE_gcontext *gc,
                      GEDevDesc *dd)
{
    int na = LENGTH(angle);
            ...
            if (draw && doDrawing)
            {
                ...
                if (R_FINITE(xx[i]) && R_FINITE(yy[i]))
                {
                    ...
                    if (isExpression(txt))
                        ...
                        GEText(xx[i], yy[i], translateChar(STRING_ELT(txt, i % LENGTH(txt))), CE_NATIVE /*FIX*/,
