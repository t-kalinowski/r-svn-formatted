    ...
    for (i = 0; i < nx; i++)
    {
        ...
        else if ((R_FINITE(xold) && R_FINITE(yold)) && !(R_FINITE(xx[i]) && R_FINITE(yy[i])))
        {
            if (i - start > 1)
            {
                ...
                if (!isNull(arrow))
                {
                     ...
                     */
                    arrows(xx + start, yy + start, i - start, arrow, 0, start == 0, FALSE, vpc, vpWidthCM, vpHeightCM,
                           &gc, dd);
                }
            }
        }
        else if ((R_FINITE(xold) && R_FINITE(yold)) && (i == nx - 1))
        {
            ...
            if (!isNull(arrow))
            {
                 ...
                 */
                arrows(xx + start, yy + start, nx - start, arrow, 0, start == 0, TRUE, vpc, vpWidthCM, vpHeightCM, &gc,
                       dd);
            }
        }
        xold = xx[i];
 ...
 * are unit objects
 */
SEXP gridXspline(SEXP x, SEXP y, SEXP s, SEXP o, SEXP a, SEXP index, double theta, Rboolean draw)
{
    int i, j, nx, np, nloc;
    double *xx, *yy, *ss;
    double vpWidthCM, vpHeightCM;
    ...
    for (i = 0; i < np; i++)
    {
        ...
        PROTECT(points = GEXspline(nx, xx, yy, ss, LOGICAL(o)[0], draw, &gc, dd));
...
SEXP L_xsplineBounds(SEXP x, SEXP y, SEXP s, SEXP o, SEXP a, SEXP index, SEXP theta)
{
