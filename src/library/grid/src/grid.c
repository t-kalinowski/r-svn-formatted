...
SEXP L_xspline(SEXP x, SEXP y, SEXP s, SEXP o, SEXP index)
{
    ...
    for (i = 0; i < np; i++)
    {
        ...
        for (j = 0; j < nx; j++)
        {
            ...
            {
                if (j - start > 1)
                    GEXspline(j - start, xx + start, yy + start, ss + start, LOGICAL(o)[0], &gc, dd);
            }
            else if ((R_FINITE(xold) && R_FINITE(yold)) && (j == nx - 1))
                GEXspline(nx - start, xx + start, yy + start, ss + start, LOGICAL(o)[0], &gc, dd);
            xold = xx[j];
            yold = yy[j];
        }
    }
...
static void drawArrow(double *x, double *y, int type, R_GE_gcontext *gc, int i, GEDevDesc *dd)
{
    switch (type)
    {
    case 1:
