...
Rboolean relativeUnit(SEXP unit, int index, pGEDevDesc dd)
{
    return pureNullUnit(unit, index, dd);
}

void findRelWidths(SEXP layout, int *relativeWidths, pGEDevDesc dd)
{
    int i;
...
void findRelHeights(SEXP layout, int *relativeHeights, pGEDevDesc dd)
{
    int i;
...
void allocateKnownWidths(SEXP layout, int *relativeWidths, double parentWidthCM, double parentHeightCM,
                         LViewportContext parentContext, const pGEcontext parentgc, pGEDevDesc dd, double *npcWidths,
                         double *widthLeftCM)
{
    int i;
    SEXP widths = layoutWidths(layout);
    for (i = 0; i < layoutNCol(layout); i++)
        ...
        {
            npcWidths[i] =
                transformWidth(widths, i, parentContext, parentgc, parentWidthCM, parentHeightCM, 0, 0, dd) * 2.54;
...
void allocateKnownHeights(SEXP layout, int *relativeHeights, double parentWidthCM, double parentHeightCM,
                          LViewportContext parentContext, const pGEcontext parentgc, pGEDevDesc dd, double *npcHeights,
                          double *heightLeftCM)
{
    int i;
    SEXP heights = layoutHeights(layout);
    for (i = 0; i < layoutNRow(layout); i++)
        ...
        {
            npcHeights[i] =
                transformHeight(heights, i, parentContext, parentgc, parentWidthCM, parentHeightCM, 0, 0, dd) * 2.54;
 ...
 */
double totalWidth(SEXP layout, int *relativeWidths, LViewportContext parentContext, const pGEcontext parentgc,
                  pGEDevDesc dd)
{
    int i;
    SEXP widths = layoutWidths(layout);
...
double totalHeight(SEXP layout, int *relativeHeights, LViewportContext parentContext, const pGEcontext parentgc,
                   pGEDevDesc dd)
{
    int i;
    SEXP heights = layoutHeights(layout);
...
void allocateRespected(SEXP layout, int *relativeWidths, int *relativeHeights, double *reducedWidthCM,
                       double *reducedHeightCM, LViewportContext parentContext, const pGEcontext parentgc,
                       pGEDevDesc dd, double *npcWidths, double *npcHeights)
{
    int i;
    SEXP widths = layoutWidths(layout);
    SEXP heights = layoutHeights(layout);
    int respect = layoutRespect(layout);
    double sumWidth = totalWidth(layout, relativeWidths, parentContext, parentgc, dd);
    double sumHeight = totalHeight(layout, relativeHeights, parentContext, parentgc, dd);
    double denom, mult;
        ...
        for (i = 0; i < layoutNCol(layout); i++)
            if (relativeWidths[i])
                if (colRespected(i, layout))
                {
                     ...
                     */
                    npcWidths[i] = pureNullUnitValue(widths, i) / denom * mult;
        ...
        for (i = 0; i < layoutNRow(layout); i++)
            if (relativeHeights[i])
                if (rowRespected(i, layout))
                {
                    ...
                    npcHeights[i] = pureNullUnitValue(heights, i) / denom * mult;
                    *reducedHeightCM -= npcHeights[i];
                }
    }
}

void setRespectedZero(SEXP layout, int *relativeWidths, int *relativeHeights, double *npcWidths, double *npcHeights)
{
    int i;
    for (i = 0; i < layoutNCol(layout); i++)
        if (relativeWidths[i])
 ...
 */
double totalUnrespectedWidth(SEXP layout, int *relativeWidths, LViewportContext parentContext,
...
void setRemainingWidthZero(SEXP layout, int *relativeWidths, double *npcWidths)
{
    int i;
    for (i = 0; i < layoutNCol(layout); i++)
        if (relativeWidths[i])
...
void allocateRemainingWidth(SEXP layout, int *relativeWidths, double remainingWidthCM, LViewportContext parentContext,
                            const pGEcontext parentgc, pGEDevDesc dd, double *npcWidths)
{
    int i;
    SEXP widths = layoutWidths(layout);
    double sumWidth;
...
void setRemainingHeightZero(SEXP layout, int *relativeHeights, double *npcHeights)
{
    int i;
    for (i = 0; i < layoutNRow(layout); i++)
        if (relativeHeights[i])
...
void allocateRemainingHeight(SEXP layout, int *relativeHeights, double remainingHeightCM,
                             LViewportContext parentContext, const pGEcontext parentgc, pGEDevDesc dd,
                             double *npcHeights)
{
    int i;
    SEXP heights = layoutHeights(layout);
...
static void subRegion(SEXP layout, int minrow, int maxrow, int mincol, int maxcol, double widths[], double heights[],
                      double parentWidthCM, double parentHeightCM, double *left, double *bottom, double *width,
                      double *height)
{
    ...
    *left = parentWidthCM * hjust - totalWidth * hjust + sumDims(widths, 0, mincol - 1);
    *bottom = parentHeightCM * vjust + (1 - vjust) * totalHeight - sumDims(heights, 0, maxrow);
    /*
                        ...
                        const pGEcontext parentgc, pGEDevDesc dd)
{
     ...
     */
    allocateKnownWidths(layout, relativeWidths, parentWidthCM, parentHeightCM, parentContext, parentgc, dd, npcWidths,
                        &reducedWidthCM);
    allocateKnownHeights(layout, relativeHeights, parentWidthCM, parentHeightCM, parentContext, parentgc, dd,
                         npcHeights, &reducedHeightCM);
    ...
    else
    {
         ...
         */
        setRespectedZero(layout, relativeWidths, relativeHeights, npcWidths, npcHeights);
    }
...
Rboolean checkPosRowPosCol(SEXP vp, SEXP parent)
{
    ...
    if (!isNull(viewportLayoutPosRow(vp)) &&
        (INTEGER(viewportLayoutPosRow(vp))[0] < 1 || INTEGER(viewportLayoutPosRow(vp))[1] > nrow))
        error(_("invalid 'layout.pos.row'"));
    if (!isNull(viewportLayoutPosCol(vp)) &&
        (INTEGER(viewportLayoutPosCol(vp))[0] < 1 || INTEGER(viewportLayoutPosCol(vp))[1] > ncol))
...
void calcViewportLocationFromLayout(SEXP layoutPosRow, SEXP layoutPosCol, SEXP parent, LViewportLocation *vpl)
{
    ...
    subRegion(viewportLayout(parent), minrow, maxrow, mincol, maxcol, REAL(viewportLayoutWidths(parent)),
              REAL(viewportLayoutHeights(parent)), REAL(viewportWidthCM(parent))[0], REAL(viewportHeightCM(parent))[0],
              &x, &y, &width, &height);
