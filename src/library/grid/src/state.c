...
SEXP gridCallback(GEevent task, pGEDevDesc dd, SEXP data)
{
            ...
            if (LOGICAL(gridStateElement(dd, GSS_ENGINEDLON))[0])
            {
                ...
                if (data != R_NilValue)
                {
                    ...
                    if (isVector(CAR(args)))
                    {
                        ...
                        if (isString(name) && (!strcmp(CHAR(STRING_ELT(name, 0)), "C_par") ||
                                               !strcmp(CHAR(STRING_ELT(name, 0)), "C_plot_new")))
                        {
                            newpage = 0;
                        }
                    }
    ...
    case GE_RestoreSnapshotState: {
         ...
         */
        PROTECT(snapshotEngineVersion = getAttrib(data, install("engineVersion")));
        ...
        else
        {
            for (i = 0; i < nState; i++)
            {
                ...
                if (!strcmp(CHAR(STRING_ELT(getAttrib(state, install("pkgName")), 0)), "grid"))
                {
                    gridState = state;
                }
            }
        ...
        if (!isNull(gridState) && !isNull(VECTOR_ELT(gridState, 0)))
        {
            ...
            if (dlIndex > 0)
            {
                 ...
                 *  'grid' DL)
                 */
                setGridStateElement(dd, GSS_DL, VECTOR_ELT(gridState, 0));
                setGridStateElement(dd, GSS_DLINDEX, VECTOR_ELT(gridState, 1));
            }
        }
        UNPROTECT(2);
    }
    ...
    case GE_ScalePS:
        ...
        REAL(scale)[0] = REAL(gridStateElement(dd, GSS_SCALE))[0] * REAL(data)[0];
