...
{
    if (isSimpleUnit(unit))
        return REAL(unit)[index % LENGTH(unit)];
...
{
    if (isSimpleUnit(unit))
        return INTEGER(getAttrib(unit, install("unit")))[0];
...
double evaluateGrobUnit(double value, SEXP grob, double vpwidthCM, double vpheightCM, int nullLMode, int nullAMode,
                        /*
                         * Evaluation type
                         * 0 = x, 1 = y, 2 = width, 3 = height
                         */
                        int evalType, pGEDevDesc dd)
{
    ...
    getViewportTransform(currentvp, dd, &vpWidthCM, &vpHeightCM, savedTransform, &rotationAngle);
    /*
    ...
    case 1:
        ...
        else
        {
            ...
            transformLocn(unitx, unity, 0, vpc, &gc, vpWidthCM, vpHeightCM, dd, transform, &xx, &yy);
            location(xx, yy, lin);
            trans(lin, invt, lout);
            xx = locationX(lout);
            yy = locationY(lout);
    ...
    case 2:
        ...
        {
            result = transformWidthtoINCHES(unitx, 0, vpc, &gc, vpWidthCM, vpHeightCM, dd);
        }
        break;
    case 3:
    case 4:
    case 5:
        ...
        {
            result = transformHeighttoINCHES(unity, 0, vpc, &gc, vpWidthCM, vpHeightCM, dd);
        }
        break;
    }
    /* Call postDraw(grob)
...
double evaluateGrobXUnit(double value, SEXP grob, double vpheightCM, double vpwidthCM, int nullLMode, int nullAMode,
                         pGEDevDesc dd)
{
    return evaluateGrobUnit(value, grob, vpheightCM, vpwidthCM, nullLMode, nullAMode, 0, dd);
}

double evaluateGrobYUnit(double value, SEXP grob, double vpheightCM, double vpwidthCM, int nullLMode, int nullAMode,
                         pGEDevDesc dd)
{
    return evaluateGrobUnit(value, grob, vpheightCM, vpwidthCM, nullLMode, nullAMode, 1, dd);
}

double evaluateGrobWidthUnit(SEXP grob, double vpheightCM, double vpwidthCM, int nullLMode, int nullAMode,
                             pGEDevDesc dd)
{
    return evaluateGrobUnit(1, grob, vpheightCM, vpwidthCM, nullLMode, nullAMode, 2, dd);
}
                 ...
                 double thisCM, double otherCM, int nullLMode, int nullAMode, pGEDevDesc dd)
{
    ...
    case L_MYSTRINGWIDTH: /* FIXME: Remove this when I can */
        if (isExpression(data))
            ...
            result =
                result * fromDeviceWidth(GEStrWidth(CHAR(STRING_ELT(data, 0)), getCharCE(STRING_ELT(data, 0)), gc, dd),
                                         GE_INCHES, dd);
        break;
    case L_STRINGHEIGHT:
    ...
    case L_GROBX:
        ...
        break;
    case L_GROBY:
        result = evaluateGrobYUnit(value, data, otherCM, thisCM, nullLMode, nullAMode, dd);
        break;
    case L_GROBWIDTH:
        result = value * evaluateGrobWidthUnit(data, thisCM, otherCM, nullLMode, nullAMode, dd);
        break;
    case L_GROBHEIGHT:
...
double transformLocation(double location, int unit, SEXP data, double scalemin, double scalemax, const pGEcontext gc,
                         double thisCM, double otherCM, int nullLMode, int nullAMode, pGEDevDesc dd)
{
...
double transformX(SEXP x, int index, LViewportContext vpc, const pGEcontext gc, double widthCM, double heightCM,
                  int nullLMode, int nullAMode, pGEDevDesc dd)
{
    ...
    case L_SUM:
        ...
        for (i = 0; i < n; i++)
        {
            result += transformX(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_summing, dd);
        }
        result *= value;
        break;
    case L_MIN:
        ...
        for (i = 0; i < n; i++)
        {
            temp = transformX(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_minimising, dd);
            if (temp < result)
                result = temp;
        }
        result *= value;
    ...
    case L_MAX:
        ...
        for (i = 0; i < n; i++)
        {
            temp = transformX(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_maximising, dd);
            if (temp > result)
                result = temp;
        }
        result *= value;
    ...
    default:
        ...
        result = transformLocation(value, unit, data, vpc.xscalemin, vpc.xscalemax, gc, widthCM, heightCM, nullLMode,
                                   nullamode, dd);
    }
...
double transformY(SEXP y, int index, LViewportContext vpc, const pGEcontext gc, double widthCM, double heightCM,
                  int nullLMode, int nullAMode, pGEDevDesc dd)
{
    double result;
    ...
    case L_SUM:
        ...
        for (i = 0; i < n; i++)
        {
            result += transformY(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_summing, dd);
        }
        result *= value;
        break;
    case L_MIN:
        ...
        for (i = 0; i < n; i++)
        {
            temp = transformY(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_minimising, dd);
            if (temp < result)
                result = temp;
        }
        result *= value;
    ...
    case L_MAX:
        ...
        for (i = 0; i < n; i++)
        {
            temp = transformY(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_maximising, dd);
            if (temp > result)
                result = temp;
        }
        result *= value;
    ...
    default:
        nullamode = nullAMode ? nullAMode : L_plain;
        result = transformLocation(value, unit, data, vpc.yscalemin, vpc.yscalemax, gc, heightCM, widthCM, nullLMode,
                                   nullamode, dd);
    }
...
double transformDimension(double dim, int unit, SEXP data, double scalemin, double scalemax, const pGEcontext gc,
                          double thisCM, double otherCM, int nullLMode, int nullAMode, pGEDevDesc dd)
{
    double result = dim;
    switch (unit)
...
double transformWidth(SEXP width, int index, LViewportContext vpc, const pGEcontext gc, double widthCM, double heightCM,
                      int nullLMode, int nullAMode, pGEDevDesc dd)
{
    double result;
    ...
    case L_SUM:
        ...
        for (i = 0; i < n; i++)
        {
            result += transformWidth(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_summing, dd);
        }
        result *= value;
        break;
    case L_MIN:
        ...
        for (i = 0; i < n; i++)
        {
            temp = transformWidth(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_minimising, dd);
            if (temp < result)
                result = temp;
        }
        result *= value;
    ...
    case L_MAX:
        ...
        for (i = 0; i < n; i++)
        {
            temp = transformWidth(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_maximising, dd);
            if (temp > result)
                result = temp;
        }
        result *= value;
    ...
    default:
        nullamode = nullAMode ? nullAMode : L_plain;
        result = transformDimension(value, unit, data, vpc.xscalemin, vpc.xscalemax, gc, widthCM, heightCM, nullLMode,
                                    nullamode, dd);
    }
...
double transformHeight(SEXP height, int index, LViewportContext vpc, const pGEcontext gc, double widthCM,
                       double heightCM, int nullLMode, int nullAMode, pGEDevDesc dd)
{
    double result;
    ...
    case L_SUM:
        ...
        for (i = 0; i < n; i++)
        {
            result += transformHeight(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_summing, dd);
        }
        result *= value;
        break;
    case L_MIN:
        ...
        for (i = 0; i < n; i++)
        {
            temp = transformHeight(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_minimising, dd);
            if (temp < result)
                result = temp;
        }
        result *= value;
    ...
    case L_MAX:
        ...
        for (i = 0; i < n; i++)
        {
            temp = transformHeight(data, i, vpc, gc, widthCM, heightCM, nullLMode, L_maximising, dd);
            if (temp > result)
                result = temp;
        }
        result *= value;
    ...
    default:
        nullamode = nullAMode ? nullAMode : L_plain;
        result = transformDimension(value, unit, data, vpc.yscalemin, vpc.yscalemax, gc, heightCM, widthCM, nullLMode,
                                    nullamode, dd);
    }
 ...
 */
double transformXtoINCHES(SEXP x, int index, LViewportContext vpc, const pGEcontext gc, double widthCM, double heightCM,
                          pGEDevDesc dd)
{
    return transformX(x, index, vpc, gc, widthCM, heightCM, 0, 0, dd);
}

double transformYtoINCHES(SEXP y, int index, LViewportContext vpc, const pGEcontext gc, double widthCM, double heightCM,
                          pGEDevDesc dd)
{
    return transformY(y, index, vpc, gc, widthCM, heightCM, 0, 0, dd);
}

void transformLocn(SEXP x, SEXP y, int index, LViewportContext vpc, const pGEcontext gc, double widthCM,
                   double heightCM, pGEDevDesc dd, LTransform t, double *xx, double *yy)
{
    LLocation lin, lout;
...
double transformWidthtoINCHES(SEXP w, int index, LViewportContext vpc, const pGEcontext gc, double widthCM,
                              double heightCM, pGEDevDesc dd)
{
    return transformWidth(w, index, vpc, gc, widthCM, heightCM, 0, 0, dd);
}

double transformHeighttoINCHES(SEXP h, int index, LViewportContext vpc, const pGEcontext gc, double widthCM,
                               double heightCM, pGEDevDesc dd)
{
    return transformHeight(h, index, vpc, gc, widthCM, heightCM, 0, 0, dd);
}

void transformDimn(SEXP w, SEXP h, int index, LViewportContext vpc, const pGEcontext gc, double widthCM,
                   double heightCM, pGEDevDesc dd, double rotationAngle, double *ww, double *hh)
{
    LLocation din, dout;
    LTransform r;
...
double transformFromINCHES(double value, int unit, const pGEcontext gc, double thisCM, double otherCM, pGEDevDesc dd)
{
    /*
     * Convert to NPC
     */
 ...
 */
double transformXYFromINCHES(double location, int unit, double scalemin, double scalemax, const pGEcontext gc,
...
double transformWidthHeightFromINCHES(double dimension, int unit, double scalemin, double scalemax, const pGEcontext gc,
                                      double thisCM, double otherCM, pGEDevDesc dd)
{
...
SEXP validData(SEXP data, SEXP validUnits, int n)
{
    ...
    for (int i = 0; i < nUnit; i++)
    {
        ...
        if (unitIsString && !Rf_isString(singleData) && !Rf_isExpression(singleData))
        {
...
SEXP constructUnits(SEXP amount, SEXP data, SEXP unit)
{
    ...
    if (nUnit == 1)
    {
        ...
        if (!(isStringUnit(u) || isGrobUnit(u)))
        {
            ...
            if (referenced)
                amount = PROTECT(duplicate(amount));
...
SEXP addUnit(SEXP u1, SEXP u2)
{
    ...
    if (isSum1)
    {
        ...
        { // Multiply the data with the value of the summation unit
            for (int j = 0; j < lengthData1; j++)
            {
...
SEXP summaryUnits(SEXP units, SEXP op_type)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < m; j++)
        {
            ...
            all_type =
                all_type &&
                (j == 0 || (current_type == first_type && R_compute_identical(uData(unit_temp), first_data, 15)));
