...
static SEXP R_target, R_defined, R_nextMethod, R_dot_nextMethod, R_loadMethod_name;
...
static SEXP do_dispatch(SEXP fname, SEXP ev, SEXP mlist, int firstTry, int evalArgs)
{
    ...
    {
        if (!firstTry)
            error(_("no matching method for function '%s' (argument '%s', with class \"%s\")"), CHAR(asChar(fname)),
...
SEXP R_nextMethodCall(SEXP matched_call, SEXP ev)
{
    ...
    {
        SEXP ee = e, dots;
        PROTECT(dots = allocVector(LANGSXP, 1));
        nprotect++;
    ...
    if (prim_case)
    {
        ...
        if (error_flag)
            Rf_error(_("error in evaluating a 'primitive' next method: %s"), R_curErrorBuf());
    }
    else
        val = eval(e, ev);
    UNPROTECT(nprotect);
