...
static SEXP s_dot_Methods, s_skeleton, s_expression, s_function, s_getAllMethods, s_objectsEnv, s_MethodsListSelect,
    s_sys_dot_frame, s_sys_dot_call, s_sys_dot_function, s_generic, s_missing, s_generic_dot_skeleton, s_subset_gets,
    s_element_gets, s_argument, s_allMethods, s_base;
...
static SEXP R_target, R_defined, R_nextMethod, R_dot_nextMethod, R_loadMethod_name, R_methods_name, R_tripleColon_name;
...
static SEXP R_evalHandleError(SEXP e, SEXP env, SEXP (*handler)(SEXP, void *), void *hdata)
{
...
static SEXP R_evalHandleErrorProtect(SEXP e, SEXP env, SEXP (*handler)(SEXP, void *), void *hdata,
                                     void (*finally)(void *), void *fdata)
{
    R_evalWrapperCleanup_t data = {.e = e, .env = env, .finally = finally, .fdata = fdata};

    return R_withCallingErrorHandler(&evalWrapperCleanup, &data, handler, hdata);
}
...
static SEXP R_conditionMessage(SEXP cond)
{
    ...
    if (TYPEOF(out) != STRSXP)
        error(_("unexpected type '%s' for condition message"), type2char(TYPEOF(out)));
    if (length(out) != 1)
...
SEXP R_initMethodDispatch(SEXP envir)
{
    ...
    R_set_standardGeneric_ptr((table_dispatch_on ? R_dispatchGeneric : R_standardGeneric), Methods_Namespace);
    R_set_quick_method_check((table_dispatch_on ? R_quick_dispatch : R_quick_method_check));

    /* Some special lists of primitive skeleton calls.
       These will be promises under lazy-loading.
    */
    PROTECT(R_short_skeletons = findVar(install(".ShortPrimitiveSkeletons"), Methods_Namespace));
    ...
    UNPROTECT(1);
    PROTECT(R_empty_skeletons = findVar(install(".EmptyPrimitiveSkeletons"), Methods_Namespace));
    ...
    if (R_short_skeletons == R_UnboundValue || R_empty_skeletons == R_UnboundValue)
        error(
            _("could not find the skeleton calls for 'methods' (package detached?): expect very bad things to happen"));
...
static SEXP R_S_MethodsListSelectCleanup(SEXP err, void *data)
{
    ...
    error(_("S language method selection did not return normally when called from internal dispatch for function '%s'"),
          check_symbol_or_string(fname, TRUE, _("Function name for method selection called internally")));
    return R_NilValue;
}
...
static SEXP R_S_MethodsListSelect(SEXP fname, SEXP ev, SEXP mlist, SEXP f_env)
{
    ...
    val = R_evalHandleError(e, Methods_Namespace, &R_S_MethodsListSelectCleanup, fname);
    UNPROTECT(1);
    return val;
}
    ...
    if (generic == R_UnboundValue)
    {
        ...
        if (IS_GENERIC(vl))
        {
            ...
            if (strlen(pkg))
            {
                gpackage = PACKAGE_SLOT(vl);
                check_single_string(gpackage, FALSE, "The \"package\" slot in generic function object");
                if (strcmp(pkg, CHAR(STRING_ELT(gpackage, 0))))
...
SEXP R_getGeneric(SEXP name, SEXP mustFind, SEXP env, SEXP package)
{
    ...
    if (value == R_UnboundValue)
    {
        ...
        {
            if (env == R_GlobalEnv)
                error(_("no generic function definition found for '%s'"), CHAR(asChar(name)));
            else
...
SEXP R_standardGeneric(SEXP fname, SEXP ev, SEXP fdef)
{
    ...
    case CLOSXP:
        f_env = CLOENV(fdef);
        PROTECT(mlist = findVar(s_dot_Methods, f_env));
        nprotect++;
    ...
    if (isNull(f))
    {
        ...
        nprotect++;
        if (isNull(value))
            error(_("no direct or inherited method for function '%s' for this call"), CHAR(asChar(fname)));
        mlist = value;
...
{
    if (!isSymbol(symbol))
        error(_("invalid symbol in checking for missing argument in method dispatch: expected a name, got an object of "
                "class \"%s\""),
              class_string(symbol));
    if (isNull(ev))
    {
        ...
        error(_("invalid environment in checking for missing argument, '%s', in methods dispatch: got an object of "
                "class \"%s\""),
...
static SEXP argEvalCleanup(SEXP err, void *data_)
{
    ...
    error(_("error in evaluating the argument '%s' in selecting a method for function '%s': %s"),
          CHAR(PRINTNAME(data->arg_sym)), CHAR(asChar(data->fname)), CHAR(STRING_ELT(R_conditionMessage(err), 0)));
    return R_NilValue;
}
...
static SEXP do_dispatch(SEXP fname, SEXP ev, SEXP mlist, int firstTry, int evalArgs)
{
    ...
    if (TYPEOF(ev) != ENVSXP)
    {
        error(_("(in selecting a method for function '%s') the 'environment' argument for dispatch must be an R "
                "environment; got an object of class \"%s\""),
    ...
    if (evalArgs)
    {
        ...
        else
        {
            /*  get its class */
            SEXP arg, class_obj;
            PROTECT(arg = R_evalHandleError(arg_sym, ev, &argEvalCleanup, &cleandata));
            nprotect++;
            PROTECT(class_obj = R_data_class(arg, TRUE));
            nprotect++;
...
function  MethodListSelect in R */
        error(_("recursive use of function '%s' in method selection, with no default method"), CHAR(asChar(fname)));
        return (R_NilValue);
    }
...
{
    Rf_error(_("error in evaluating a 'primitive' next method: %s"), CHAR(STRING_ELT(R_conditionMessage(err), 0)));
    return R_NilValue;
}
...
SEXP R_nextMethodCall(SEXP matched_call, SEXP ev)
{
    ...
    if (prim_case)
        val = R_evalHandleErrorProtect(e, ev, &R_nextMethodCallCleanup, NULL, &R_nextMethodCallFinally, op);
    else
        val = eval(e, ev);
    UNPROTECT(2);
    return val;
...
static SEXP R_loadMethod(SEXP def, SEXP fname, SEXP ev)
{
    ...
    if (found < length(attrib))
    {
        ...
        SETCAR(e, lang3(R_tripleColon_name, R_methods_name, R_loadMethod_name));
        val = CDR(e);
        SETCAR(val, def);
        val = CDR(val);
        SETCAR(val, fname);
...
static const char *check_single_string(SEXP obj, Rboolean nonEmpty, const char *what)
{
    ...
    {
        error(_("'%s' must be a single string (got an object of class \"%s\")"), what, class_string(obj));
    }
    return string;
}
...
SEXP R_methodsPackageMetaName(SEXP prefix, SEXP name, SEXP pkg)
{
    ...
    prefixString = check_single_string(prefix, TRUE, "The internal prefix (e.g., \"C\") for a meta-data object");
    nameString = check_single_string(
        name, FALSE, "The name of the object (e.g,. a class or generic function) to find in the meta-data");
    pkgString = check_single_string(pkg, FALSE, "The name of the package for a meta-data object");
    if (*pkgString)
...
{
    if (TYPEOF(e1) == STRSXP && TYPEOF(e2) == STRSXP && LENGTH(e1) == 1 && LENGTH(e2) == 1 &&
...
SEXP R_getClassFromCache(SEXP class, SEXP table)
{
    ...
    if (TYPEOF(class) == STRSXP)
    {
        ...
        else if (TYPEOF(package) == STRSXP)
        {
            ...
            if (TYPEOF(defPkg) == STRSXP && length(defPkg) == 1 && STRING_ELT(defPkg, 0) != STRING_ELT(package, 0))
                return R_NilValue;
            else
                return value;
        }
...
SEXP R_dispatchGeneric(SEXP fname, SEXP ev, SEXP fdef)
{
    ...
    case SPECIALSXP:
    case BUILTINSXP:
        PROTECT(fdef = R_primitive_generic(fdef));
        nprotect++;
    ...
    PROTECT(mtable = findVarInFrame(f_env, R_allmtable));
    nprotect++;
    ...
    PROTECT(sigargs = findVarInFrame(f_env, R_sigargs));
    nprotect++;
    PROTECT(siglength = findVarInFrame(f_env, R_siglength));
    nprotect++;
    if (sigargs == R_UnboundValue || siglength == R_UnboundValue || mtable == R_UnboundValue)
        error("generic \"%s\" seems not to have been initialized for table dispatch---need to have '.SigArgs' and "
              "'.AllMtable' assigned in its environment");
    nargs = asInteger(siglength);
    PROTECT(classes = allocVector(VECSXP, nargs));
    nprotect++;
    ...
    for (i = 0; i < nargs; i++)
    {
        ...
        else
        {
            ...
            {
                SEXP arg = PROTECT(R_evalHandleError(arg_sym, ev, &argEvalCleanup, &cleandata));
