...
static SEXP s_dot_Arguments, s_expression, s_function, s_mergeGenericFunctions, s_objectsEnv, s_MethodsListSelect,
    ...
    if (value == R_NilValue)
    {
        ...
        if (value == R_NilValue)
        {
            ...
            *inherited = 1;
            ...
            error("Recursive use of function \"%s\" in method selection, with no default method", CHAR(asChar(fname)));
    }
    return value;
}
...
static SEXP do_dispatch(SEXP fname, SEXP ev, SEXP optional, SEXP mlist, SEXP *final_p)
{
    char *arg_name, *class;
    ...
    *final_p = mlist; /* to signal possible re-computation of mlist to add
    ...
    {
        error("No matching method or default for argument \"%s\" of class %s", CHAR(PRINTNAME(arg_sym)), class);
        return (R_NilValue); /* -Wall */
    }
    ...
    if (!isFunction(method))
    {
        ...
        child = this_method;
    if ((inherited && method != R_NilValue && fname == R_NilValue) || (this_method != child))
