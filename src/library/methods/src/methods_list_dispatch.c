...
static SEXP R_find_method(SEXP mlist, char *class, SEXP fname)
{
    ...
    {
        error("No \"allMethods\" slot found in \"mlist\" object for %s!", CHAR_STAR(fname));
...
SEXP R_standardGeneric(SEXP fname, SEXP ev, SEXP fdef)
{
    ...
    case BUILTINSXP:
        ...
        break;
    default:
        error("Invalid type for generic function \"%s\": \"%s\"", CHAR_STAR(fsym), type2str(TYPEOF(fdef)));
    }
    switch (TYPEOF(mlist))
    {
    case LANGSXP:
...
SEXP R_missingArg(SEXP symbol, SEXP ev)
{
    if (!isSymbol(symbol))
        error("invalid `symbol' argument: expected a name, got a \"%s\"",
              CHAR_STAR((isObject(symbol) ? R_data_class(symbol, 1) : type2str(TYPEOF(symbol)))));
    if (!isEnvironment(ev))
        error("invalid `envir' argument: expected an environment, got a \"%s\"",
              CHAR_STAR((isObject(ev) ? R_data_class(ev, 1) : type2str(TYPEOF(ev)))));
...
SEXP R_nextMethodCall(SEXP matched_call, SEXP ev)
{
    ...
    */
    ...
    for (i = 0; i < nargs; i++)
    {
        ...
        if (prim_case && i == 0)
        {
            PROTECT(arg1 = R_tryEval(this_sym, ev, &error_flag));
            nprotect++;
            if (error_flag)
