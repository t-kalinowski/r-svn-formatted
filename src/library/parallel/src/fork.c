...
static void wait_for_child_ci(child_info_t *ci)
{
    ...
    if (waitpid(ci->pid, &wstat, WNOHANG) == ci->pid && (WIFEXITED(wstat) || WIFSIGNALED(wstat)))
    {
...
SEXP mc_fork(SEXP sEstranged)
{
    ...
    if (fd_used_by_children(pipefd[0]) || fd_used_by_children(pipefd[1]) || fd_used_by_children(sipfd[0]) ||
        fd_used_by_children(sipfd[1]))
...
{
    int *fd, fds, i = 0;
    if (TYPEOF(sFDS) != INTSXP)
        error("descriptors must be integers");
    fds = LENGTH(sFDS);
    fd = INTEGER(sFDS);
    while (i < fds)
        close_non_child_fd(fd[i++]);
...
SEXP mc_send_child_stdin(SEXP sPid, SEXP what)
{
    ...
    if (TYPEOF(what) != RAWSXP)
        error("what must be a raw vector");
    child_info_t *ci = children;
    pid_t ppid = getpid();
    while (ci)
    {
...
SEXP mc_select_children(SEXP sTimeout, SEXP sWhich)
{
    ...
    if (sr < 1)
        return ScalarLogical(TRUE); /* TRUE on timeout */
    ci = children;
#ifdef MC_DEBUG
...
#ifdef MC_DEBUG
    if (!ci)
        Dprintf("read_child(%d) - pid is not in the list of children\n", pid);
#endif
    if (!ci)
        return R_NilValue; /* if the child doesn't exist anymore, returns NULL */
...
SEXP mc_read_children(SEXP sTimeout)
{
    ...
    if (isReal(sTimeout) && LENGTH(sTimeout) == 1)
    {
        ...
        if (tov < 0.0)
            tvp = 0; /* Note: I'm not sure we really should allow this .. */
        else
        {
            tv.tv_sec = (int)tov;
    ...
    if (sr < 1)
        return ScalarLogical(1); /* TRUE on timeout */
    ci = children;
    while (ci)
    {
        if (!ci->detached && ci->ppid == ppid)
...
#if defined(CPU_ZERO) && defined(CPU_COUNT) && defined(CPU_SETSIZE) && defined(CPU_SET) && defined(CPU_SET_S) &&       \
    defined(CPU_ISSET)
