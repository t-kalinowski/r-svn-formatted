                ...
                if (TYPEOF(pkg) == VECSXP && LENGTH(pkg) > 1)
                {
                    ...
                    if (TYPEOF(dpath) == STRSXP && LENGTH(dpath) > 0)
                    {
                        ...
                        if (dll)
                        {
                            ...
                            /* it would be faster to use R_dlsym since we already have DllInfo but that is hidden so
                             * let's waste more cycles.. */
                            QuartzCocoa_InhibitEventLoop = (QuartzCocoa_InhibitEventLoop_t)R_FindSymbol(
                                "QuartzCocoa_InhibitEventLoop", "grDevices", (R_RegisteredNativeSymbol *)&symbol);
...
SEXP mc_select_children(SEXP sTimeout, SEXP sWhich)
{
...
#endif
    if (zombies)
    { /* oops, this should never really happen - it did
       * while we had a bug in rm_child_ but hopefully
       * not anymore */
        while (zombies)
        { /* this is rather more complicated than it
    ...
    maxfd = 0;
    while (ci && ci->pid)
    { /* pass 1 - count the FDs (in theory not
