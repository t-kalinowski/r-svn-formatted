...
static double evaluate(splPTR sp, double x, int nder)
{
    ...
    while (outer--)
        for (apt = sp->a, lpt = sp->ldel + outer, rpt = sp->rdel, inner = outer + 1; inner--; lpt--, rpt++, apt++)
            // FIXME: divides by zero
...
SEXP spline_basis(SEXP knots, SEXP order, SEXP xvals, SEXP derivs)
{
    ...
    SEXP val = PROTECT(allocMatrix(REALSXP, ord, nx)), offsets = PROTECT(allocVector(INTSXP, nx));
    ...
    for (int i = 0; i < nx; i++)
    {
        ...
        int io = ioff[i] = sp->curs - ord, der_i = ders[i % nd];
        if (io < 0 || io > nk)
        {
            for (int j = 0; j < ord; j++)
            {
        ...
        { /* slow method for derivatives */
            ...
            for (int ii = 0; ii < ord; ii++)
            {
                for (int j = 0; j < ord; j++)
                    sp->a[j] = 0;
                sp->a[ii] = 1;
                valM[i * ord + ii] = evaluate(sp, xx[i], der_i);
            }
        }
        else
