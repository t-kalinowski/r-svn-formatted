...
static void siftup(int l, int r, double *window, int *outlist, int *nrlist, int print_level)
{
    ...
    int i = l, j, nrold = nrlist[i];
    double x = window[i];
    if (print_level >= 2)
        Rprintf("siftup(%d,%d): x=%g: ", l, r, x);
    while ((j = 2 * i) <= r)
    {
        if (j < r)
...
static void inittree(R_xlen_t n, int k, int k2, const double *data,
...
#define Rm_PR(_F_, _A_, _k_)                                                                                           \
    for (int j = 0; j <= _k_; j++)                                                                                     \
    Rprintf(_F_, _A_)
#define RgPRINT_j(A_J, _k_)                                                                                            \
    Rm_PR("% 6.4g", A_J, _k_);                                                                                         \
    Rprintf("\n")
#define RdPRINT_j(A_J, _k_)                                                                                            \
    Rm_PR("% 6d", A_J, _k_);                                                                                           \
    Rprintf("\n")
...
#define R_PRINT_4vec()                                                                                                 \
    ...
    RdPRINT_j(j, 2 * k);                                                                                               \
    Rprintf(" %9s: ", "window []");                                                                                    \
    RwwPRINTj(window[j], 2 * k);                                                                                       \
    Rprintf(" %9s: ", " nrlist[]");                                                                                    \
    RdPRINT_j(nrlist[j], 2 * k);                                                                                       \
    Rprintf(" %9s: ", "outlist[]");                                                                                    \
    RdPRINT_j(outlist[j], k)
...
static void downtoleave(int outvirt, int k, double *window, int *outlist, int *nrlist, int print_level)
{
    if (print_level >= 2)
        Rprintf(" downtoleave(%d, %d)  ", outvirt, k);
    for (;;)
    {
        int childl = outvirt * 2, childr = childl - 1;
...
static void uptoleave(int outvirt, int k, double *window, int *outlist, int *nrlist, int print_level)
{
    if (print_level >= 2)
        Rprintf(" uptoleave(%d, %d)  ", outvirt, k);
    for (;;)
    {
        int childl = outvirt * 2, childr = childl + 1;
        if (window[childl + k] > window[childr + k])
...
static void upperoutupperin(int outvirt, int k, double *window, int *outlist, int *nrlist, int print_level)
{
    if (print_level >= 2)
        Rprintf("UpperoutUPPERin(%d, %d)\n  ", outvirt, k);
    uptoleave(outvirt, k, window, outlist, nrlist, print_level);
    int father = outvirt / 2;
    while (window[outvirt + k] < window[father + k])
    {
        if (print_level >= 2)
            Rprintf(" UpperoutUP(win[%d]):  ", outvirt + k);
    ...
    { /* compute (0-index) median[i] == X*_{i+1} */
        ...
        if (out > k)
            if (data[nrnew] >= window[k])
                ...
                upperoutdownin(outvirt, k, nrnew, outnext, data, window, outlist, nrlist, print_level);
        else if (out < k)
            if (data[nrnew] < window[k])
                downoutdownin(outvirt, k, window, outlist, nrlist, print_level);
            else
                downoutupperin(outvirt, k, nrnew, outnext, data, window, outlist, nrlist, print_level);
                    ...
                    int end_rule, int print_level)
{
    ...
    int *nrlist = (int *)R_alloc(2 * k + 1, sizeof(int)), *outlist = (int *)R_alloc(k + 1, sizeof(int));

    inittree(n, k, k2, x,
             /* initialize the 3 vectors: */ window, outlist, nrlist, print_level);

    runmedint(n, k, k2, x, median, window, outlist, nrlist, end_rule, print_level);
}
