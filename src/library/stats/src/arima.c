...
SEXP KalmanLike(SEXP sy, SEXP mod, SEXP sUP, SEXP op, SEXP update)
{
    ...
    SEXP sZ = getListElement(mod, "Z"), sa = getListElement(mod, "a"), sP = getListElement(mod, "P"),
         sT = getListElement(mod, "T"), sV = getListElement(mod, "V"), sh = getListElement(mod, "h"),
    ...
    double *y = REAL(sy), *Z = REAL(sZ), *T = REAL(sT), *V = REAL(sV), *P = REAL(sP), *a = REAL(sa), *Pnew = REAL(sPn),
           h = asReal(sh);
...
SEXP KalmanSmooth(SEXP sy, SEXP mod, SEXP sUP)
{
    ...
    int n = LENGTH(sy), p = LENGTH(sa);
    double *y = REAL(sy), *Z = REAL(sZ), *a, *P, *T = REAL(sT), *V = REAL(sV), h = asReal(sh), *Pnew;
    double *at, *rt, *Pt, *gains, *resids, *Mt, *L, gn, *Nt;
    Rboolean var = TRUE;

    PROTECT(ssa = duplicate(sa));
    a = REAL(ssa);
    PROTECT(ssP = duplicate(sP));
    P = REAL(ssP);
    PROTECT(ssPn = duplicate(sPn));
    Pnew = REAL(ssPn);
...
SEXP KalmanFore(SEXP nahead, SEXP mod, SEXP update)
{
    ...
    if (TYPEOF(sZ) != REALSXP || TYPEOF(sa) != REALSXP || TYPEOF(sP) != REALSXP || TYPEOF(sT) != REALSXP ||
        TYPEOF(sV) != REALSXP)
        error(_("invalid argument type"));

    int n = asInteger(nahead), p = LENGTH(sa);
    double *Z = REAL(sZ), *a = REAL(sa), *P = REAL(sP), *T = REAL(sT), *V = REAL(sV), h = asReal(sh);
    double *mm, *anew, *Pnew;
...
{
    int *arma = INTEGER(sarma), mp = arma[0], mq = arma[1], msp = arma[2], v, n = LENGTH(sin);
    ...
    for (int i = 0; i < n; i++)
        params[i] = in[i];
    if (mp > 0)
        partrans(mp, in, params);
    v = mp + mq;
    if (msp > 0)
        partrans(msp, in + v, params + v);
    return res;
}
...
SEXP ARIMA_transPars(SEXP sin, SEXP sarma, SEXP strans)
{
    ...
    int mp = arma[0], mq = arma[1], msp = arma[2], msq = arma[3], ns = arma[4], i, j, p = mp + ns * msp,
        q = mq + ns * msq, v;
...
{
    int *arma = INTEGER(sarma), mp = arma[0], mq = arma[1], msp = arma[2], i, v, n = LENGTH(in);
...
{
    int *arma = INTEGER(sarma), mp = arma[0], mq = arma[1], msp = arma[2], n = LENGTH(in);
...
{
    SEXP sPhi = getListElement(mod, "phi"), sTheta = getListElement(mod, "theta"),
         sDelta = getListElement(mod, "Delta"), sa = getListElement(mod, "a"), sP = getListElement(mod, "P"),
         sPn = getListElement(mod, "Pn");

    if (TYPEOF(sPhi) != REALSXP || TYPEOF(sTheta) != REALSXP || TYPEOF(sDelta) != REALSXP || TYPEOF(sa) != REALSXP ||
        TYPEOF(sP) != REALSXP || TYPEOF(sPn) != REALSXP)
        error(_("invalid argument type"));

    SEXP res, nres, sResid = R_NilValue;
    int n = LENGTH(sy), rd = LENGTH(sa), p = LENGTH(sPhi), q = LENGTH(sTheta), d = LENGTH(sDelta), r = rd - d;
...
static void inclu2(size_t np, double *xnext, double *xrow, double ynext, double *d, double *rbar, double *thetab)
{
...
#define _rrz(j) rrz[j]
    ...
    {
        int r2 = max(p + q, p + 1);
        SEXP sgam = PROTECT(allocMatrix(REALSXP, r2, r2)), sg = PROTECT(allocVector(REALSXP, r2));
         ...
         */
        SEXP callS = PROTECT(lang4(install("solve.default"), sgam, sg, sTol)), su = PROTECT(eval(callS, R_BaseEnv));
        double *u = REAL(su);
        /* SX = A SU A^T */
        /* A[i,j]  = ttheta[j-i] */
        /* SU[i,j] = u[abs(i-j)] */
                    ...
                    for (int L = k; L - k < q + 1; ++L)
                        for (int m = 0; j + m < p; ++m)
                            for (int n = m; n - m < q + 1; ++n)
                                P[r * i + j] +=
                                    phi[i + k] * phi[j + m] * _ttheta(L - k) * _ttheta(n - m) * u[abs(L - n)];
        UNPROTECT(4);
