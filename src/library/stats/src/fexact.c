...
static void f2xact(int nrow, int ncol, const int table[], int ldtabl, double expect, double percnt, double emin,
...
static double f3xact(int nrow, const int irow[], int ncol, const int icol[], int ntot, const double fact[], int *ico,
                     int *iro, int *it, int *lb, int *nr, int *nt, int *nu, int *itc, int *ist, double *stv,
                     double *alen, double tol, int ldst);
static double f4xact(int nrow, int *irow, int ncol, int *icol, double dspt, const double fact[], int *icstk, int *ncstk,
                     int *lstk, int *mstk, int *nstk, int *nrstk, int *irstk, double *ystk, double tol);
static void f5xact(double pastp, double tol, int *kval, int *key, int ldkey, int *ipoin, double *stp, int ldstp,
                   int *ifrq, int *npoin, int *nr, int *nl, int ifreq, int *itop, Rboolean psh);
static Rboolean f6xact(int nrow, int *irow, const int kyy[], int *key, int ldkey, int *last, int *ipn);
...
void fexact(int nrow, int ncol, const int table[], int ldtabl, double expect, double percnt, double emin, double *prt,
...
#define i_int 2
    ...
    double *equiv;
    int iwkmax = 2 * (int)(workspace / 2), iwkpt = 0,
...
#define rwrk ((float *)equiv)
    ...
    if (mult * (double)ldkey > INT_MAX)
        error(_("integer overflow would happen in 'mult * ldkey' = %g"), mult * (double)ldkey);
    ldstp = mult * ldkey;
     ...
     */
    f2xact(nrow, ncol, table, ldtabl, expect, percnt, emin, prt, pre,
           dwrk + i1,  // fact
           iwrk + i2,  // ico
           iwrk + i3,  // iro
           iwrk + i3a, // kyy
...
void f2xact(int nrow, int ncol, const int table[], int ldtabl, double expect, double percnt, double emin, double *prt,
            double *pre, double *fact, int *ico, int *iro, int *kyy, int *idif, int *irn, int *key, int ldkey,
            int *ipoin, double *stp, int ldstp, int *ifrq, double *LP, double *SP, double *tm, int *key2, int *iwk,
            double *rwk, int n2_stack)
{
    /*
      -----------------------------------------------------------------------
      Name:		F2XACT
    ...
    const char *ch_err_5 = "The hash table key cannot be computed because the largest key\n"
    ...
    Rboolean ok_f7, nr_gt_nc, maybe_chisq = (expect > 0.), chisq = FALSE /* -Wall */, psh;
...
L240:
    ...
    if (k1 > 1)
    {
        ...
        if (LP[itp] > 0.)
        {
            ...
            LP[itp] =
                f3xact(nro2, &irn[nrb], k1, &ico[kb + 1], ntot, fact, &iwk[i31], &iwk[i32], &iwk[i33], &iwk[i34],
                       &iwk[i35], &iwk[i36], &iwk[i37], &iwk[i38], &iwk[i39], &rwk[i310], &rwk[i311], tol, n2_stack);
            if (LP[itp] > 0.)
            { /* can this happen? */
            ...
            SP[itp] = f4xact(nro2, &irn[nrb], k1, &ico[kb + 1], dspt, fact, &iwk[i47], &iwk[i41], &iwk[i42], &iwk[i43],
...
L300:
    ...
    else if (pastp < obs2)
    {
        if (chisq)
        {
            df = (double)((nro2 - 1) * (k1 - 1));
#ifdef USING_R
            pv = pgamma(fmax2(0., tmp + (pastp + drn) * 2.) / 2., df / 2., /*scale = */ 1.,
                        /*lower_tail = */ FALSE, /*log_p = */ TRUE);
            *pre += (double)ifreq * exp(pastp + drn + pv);
#else
            double d1 = fmax2(0., tmp + (pastp + drn) * 2.) / 2., d2 = df / 2.;
            int ifault;
        ...
        else
        {
            ...
            f5xact(pastp + ddf, tol, &kval, &key[jkey], ldkey, &ipoin[jkey], &stp[jstp], ldstp, &ifrq[jstp],
                   &ifrq[jstp2], &ifrq[jstp3], &ifrq[jstp4], ifreq, &itop, psh);
            psh = FALSE;
        }
    }
...
double f3xact(int nrow, const int irow[], int ncol, const int icol[], int ntot, const double fact[], int *ico, int *iro,
              int *it, int *lb, int *nr, int *nt, int *nu, int *itc, int *ist, double *stv, double *alen, double tol,
              int ldst)
{
    /*
     -----------------------------------------------------------------------
    ...
    if (nrow * ncol == 4)
    {
        int n11 = (irow[1] + 1) * (icol[1] + 1) / (ntot + 2), n12 = irow[1] - n11;
    ...
    {
        xmin = f10act(nrow, &irow[1], ncol, &icol[1], &val, fact, &lb[1], &nu[1], &nr[1]);
    }
    else
        xmin = FALSE;
    if (!xmin && icol[ncol] <= icol[1] + nrow)
    {
        xmin = f10act(ncol, &icol[1], nrow, &irow[1], &val, fact, &lb[1], &nu[1], &nr[1]);
    }
    if (xmin)
        return -val;
...
LoopNode: /* Generate a node */
    ...
    while (1)
    {
        ...
        int nn1 = nt[lev], nrt = nr[lev];
        ++lev;
        int nc1 = nco - lev, nct = ico[lev];
    ...
    { /* 3 rows and 2 columns */
        int nn1 = nn - iro[irl] + 2, ic1 = ico[1] - lb[1], ic2 = ico[2] - lb[2],
            n11 = (iro[irl + 1] + 1) * (ic1 + 1) / nn1, n12 = iro[irl + 1] - n11;
    ...
    { /* Column marginals are new node */
        ...
        double dky = (double)kyy, dkey = it[1] * dky + it[2];
        for (i = 3; i <= nco; ++i)
        {
            dkey = it[i] + dkey * dky;
        }
...
L200: /* Pop item from stack */
    if (nitc > 0)
    {
        ...
        {
            xmin = f10act(nro, &iro[irl], nco, &ico[1], &val, fact, &lb[1], &nu[1], &nr[1]);
        }
        else
            xmin = FALSE;

        if (!xmin && ico[nco] <= ico[1] + nro)
            xmin = f10act(nco, &ico[1], nro, &iro[irl], &val, fact, &lb[1], &nu[1], &nr[1]);
        if (xmin)
        {
            if (vmn > val)
                vmn = val;
...
double f4xact(int nrow, int *irow, int ncol, int *icol, double dspt, const double fact[], int *icstk, int *ncstk,
              int *lstk, int *mstk, int *nstk, int *nrstk, int *irstk, double *ystk, double tol)
{
    /*
      -----------------------------------------------------------------------
    ...
    L60:
        ...
        if (irt == ict)
        {
            --nro;
            --nco;
            f11act(&irstk[istk * nrow + 1], i, nro, &irstk[(istk + 1) * nrow + 1]);
            f11act(&icstk[istk * ncol + 1], j, nco, &icstk[(istk + 1) * ncol + 1]);
        }
        else if (irt > ict)
        {
            --nco;
            f11act(&icstk[istk * ncol + 1], j, nco, &icstk[(istk + 1) * ncol + 1]);
            f8xact(&irstk[istk * nrow + 1], irt - ict, i, nro, &irstk[(istk + 1) * nrow + 1]);
        }
        else
        {
            --nro;
            f11act(&irstk[istk * nrow + 1], i, nro, &irstk[(istk + 1) * nrow + 1]);
            f8xact(&icstk[istk * ncol + 1], ict - irt, j, nco, &icstk[(istk + 1) * ncol + 1]);
        }
    ...
    do
    {
        ...
        for (;; ++l)
        {
            ...
            if (n == 1)
            {
                if (irstk[l + istk * nrow] < irstk[l - 1 + istk * nrow])
                    goto L60;
            }
            else if (n == 2)
            {
                if (icstk[l + istk * ncol] < icstk[l - 1 + istk * ncol])
                    goto L60;
            }
        }
    } while (1);
...
void f5xact(double pastp, double tol, int *kval, int *key, int ldkey, int *ipoin, double *stp, int ldstp, int *ifrq,
            int *npoin, int *nr, int *nl, int ifreq, int *itop, Rboolean psh)
{
    /*
      -----------------------------------------------------------------------
      Name:	      F5XACT aka "PUT"
...
L40: /* Find location, if any, of pastp */
    ...
    ipn = ipoin[itp];
    double test1 = pastp - tol, test2 = pastp + tol;
...
Rboolean f6xact(int nrow, int *irow, const int kyy[], int *key, int ldkey, int *last, int *ipn)
{
    --key;
...
Rboolean f10act(int nrow, const int irow[], int ncol, const int icol[], double *val, const double fact[], int *nd,
                int *ne, int *m)
{
    /*
      -----------------------------------------------------------------------
...
L30:
    z = 1. / (y * y);
    return (f + (y - .5) * log(y) - y + a1 + (((-a2 * z + a3) * z - a4) * z + a5) / y);
}
...
SEXP Fexact(SEXP x, SEXP pars, SEXP work, SEXP smult)
{
    ...
    fexact(nr, nc, INTEGER(x), nr, rp[0], rp[1], rp[2], // = (expect, percnt, emin)
