...
SEXP cfilter(SEXP sx, SEXP sfilter, SEXP ssides, SEXP scircular)
{
    ...
    if (sides == NA_INTEGER || circular == NA_LOGICAL)
        error("invalid input");
...
{
    if (TYPEOF(x) != REALSXP || TYPEOF(filter) != REALSXP || TYPEOF(out) != REALSXP)
        error("invalid input");
...
static void acf0(double *x, int n, int ns, int nl, Rboolean correlation, double *acf)
{
...
{
    int nx = nrows(x), ns = ncols(x), lagmax = asInteger(lmax), cor = asLogical(sCor);
