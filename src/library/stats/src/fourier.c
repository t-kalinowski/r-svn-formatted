...
static uint64_t nextn0_64(uint64_t n, const int f[], int nf)
{
    ...
    { // or give an error?  previously was much more problematic
        warning(_("nextn<64>() found no solution < %ld = UINT64_MAX (the maximal integer)"), UINT64_MAX);
...
SEXP nextn(SEXP n, SEXP f)
{
    ...
    if (TYPEOF(f) != INTSXP)
    {
        PROTECT(f = coerceVector(f, INTSXP));
        nprot++;
    }
    ...
    if (nf == 0)
        error(_("no factors"));
    if (nf < 0)
        error(_("too many factors")); // < 0 : from integer overflow
    for (int i = 0; i < nf; i++)
    ...
    if (!use_int && nn)
    {
        ...
        {
            if (!ISNAN(d_n[i]) && d_n[i] > n_max)
                n_max = d_n[i];
        }
        if (n_max <= INT_MAX / f_[0])
        { // maximal n[] should not be too large to find "next n"
            use_int = TRUE;
            n = PROTECT(n = coerceVector(n, INTSXP));
            nprot++;
        }
    }
    SEXP ans = PROTECT(allocVector(use_int ? INTSXP : REALSXP, nn));
    nprot++;
    if (nn == 0)
        return (ans);
    if (use_int)
    {
        int *n_ = INTEGER(n), *r = INTEGER(ans);
        for (R_xlen_t i = 0; i < nn; i++)
        {
            if (n_[i] == NA_INTEGER)
                r[i] = NA_INTEGER;
    ...
    { // use "double" (as R has no int64 ..)
        double *n_ = REAL(n), *r = REAL(ans);
        for (R_xlen_t i = 0; i < nn; i++)
        {
            if (ISNAN(n_[i]))
                r[i] = NA_REAL;
            ...
            else
            {
                ...
                if (n_n > max_dbl_int)
                    warning(_("nextn() = %" PRIu64 " > 2^53 may not be exactly representable in R (as \"double\")"),
                            n_n);
                r[i] = (double)n_n;
            }
        }
