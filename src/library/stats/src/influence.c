...
#include <R_ext/RS.h>
void F77_NAME(lminfl)(double *x, int *ldx, int *n, int *k, int *docoef, double *qraux, double *resid, double *hat,
...
SEXP influence(SEXP mqr, SEXP do_coef, SEXP e, SEXP stol)
{
    ...
    SEXP coefficients;
    if (docoef)
        coefficients = PROTECT(allocMatrix(REALSXP, n, k));
    else
        coefficients = PROTECT(allocVector(REALSXP, 0));
    SEXP sigma = PROTECT(allocVector(REALSXP, n));
    F77_CALL(lminfl)(REAL(qr), &n, &n, &k, &docoef, REAL(qraux), REAL(e), rh, REAL(coefficients), REAL(sigma), &tol);
