...
static void collap(int nvar, double *x, double *y, int locy, int *dim, int *config);
static void adjust(int nvar, double *x, double *y, double *z, int *locz, int *dim, int *config, double *d);

/* Table of constant values */

static void loglin(int nvar, int *dim, int ncon, int *config, int ntab, double *table, double *fit, int *locmar,
                   int nmar, double *marg, int nu, double *u, double maxdev, int maxit, double *dev, int *nlast,
                   int *ifault)
{
    // nvar could be zero (no-segfault test)
    if (!nvar)
        error("no variables"); // not translated
...
void adjust(int nvar, double *x, double *y, double *z, int *locz, int *dim, int *config, double *d)
{
...
SEXP LogLin(SEXP dtab, SEXP conf, SEXP table, SEXP start, SEXP snmar, SEXP eps, SEXP iter)
{
    ...
    SEXP fit = PROTECT(TYPEOF(start) == REALSXP ? duplicate(start) : coerceVector(start, REALSXP)),
         locmar = PROTECT(allocVector(INTSXP, ncon)), marg = PROTECT(allocVector(REALSXP, nmar)),
         u = PROTECT(allocVector(REALSXP, ntab)), dev = PROTECT(allocVector(REALSXP, maxit));
    dtab = PROTECT(coerceVector(dtab, INTSXP));
    conf = PROTECT(coerceVector(conf, INTSXP));
    table = PROTECT(coerceVector(table, REALSXP));
    loglin(nvar, INTEGER(dtab), ncon, INTEGER(conf), ntab, REAL(table), REAL(fit), INTEGER(locmar), nmar, REAL(marg),
