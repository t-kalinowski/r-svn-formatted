...
*/
{
    ...
    F77_CALL(dqrdc2)(VECTOR(xt), &n, &n, &p, &tol, &rank, qraux, pivot, work);
    ...
    F77_CALL(dqrcf)(VECTOR(xt), &NROW(x), &rank, qraux, yt.vec, &NCOL(y), coeft.vec, &info);
...
/* Log determinant of square matrix */
{
    ...
    F77_CALL(dqrdc2)(VECTOR(xtmp), &n, &n, &p, &tol, &rank, qraux, pivot, work);
        ...
        for (i = 0; i < nser; i++)
        {
            for (j = 0; j < nser; j++)
            {
                ...
                for (k = 0; k < nser; k++)
                    ...
                    {
                        MATRIX(D2)
                        [nser * i + j][nser * k + l] = (i == k) * MATRIX(sg)[j][l] + MATRIX(sh)[i][k] * (j == l);
                    }
            }
        }
...
void multi_yw(double *acf, int *pn, int *pomax, int *pnser, double *coef, double *pacf, double *var, double *aic,
              int *porder, int *puseaic);
static void whittle(Array acf, int nlag, Array *A, Array *B, Array p_forward, Array v_forward, Array p_back,
                    Array v_back);
static void whittle2(Array acf, Array Aold, Array Bold, int lag, char *direction, Array A, Array K, Array E);

void multi_yw(double *acf, int *pn, int *pomax, int *pnser, double *coef, double *pacf, double *var, double *aic,
              int *porder, int *useaic)
{
    int i, m;
