 ...
 */
static SEXP ConvInfoMsg(char *msg, int iter, int whystop, double fac, double minFac, int maxIter, double convNew)
{
...
SEXP nls_iter(SEXP m, SEXP control, SEXP doTraceArg)
{
    ...
    int i;
#define CONV_INFO_MSG(_STR_, _I_) ConvInfoMsg(_STR_, i, _I_, fac, minFac, maxIter, convNew)
...
#define NON_CONV_FINIS_2(_ID_, _MSG_, _A1_, _A2_)                                                                      \
    ...
    { // ---------------------------------------------
        ...
        double *par = REAL(pars), *npar = REAL(newPars), *nIncr = REAL(newIncr);
        int evalCnt = -1;
        if (printEval)
            evalCnt = 1;
        ...
        {
            UNPROTECT(9);
            NON_CONV_FINIS_2(2, _("step factor %g reduced below 'minFactor' of %g"), fac, minFac);
        }
    }
    ...
    if (!hasConverged)
    {
        NON_CONV_FINIS_1(3, _("number of iterations exceeded maximum of %d"), maxIter);
    }
    else
...
SEXP numeric_deriv(SEXP expr, SEXP theta, SEXP rho, SEXP dir, SEXP eps_, SEXP centr)
{
    ...
    SEXP pars = PROTECT(allocVector(VECSXP, LENGTH(theta))), ans = PROTECT(duplicate(eval(expr, rho)));
...
#define CHECK_FN_VAL(_r_, _ANS_)                                                                                       \
    ...
    for (int start = 0, i = 0; i < LENGTH(theta); i++)
    {
        ...
        {
            double origPar = pars_i[j], xx = fabs(origPar), delta = (xx == 0) ? eps : xx * eps;
            ...
            { // forward difference  (previously hardwired):
                for (int k = 0; k < LENGTH(ans); k++)
                {
