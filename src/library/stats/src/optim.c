...
SEXP optim(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    else if (strcmp(tn, "L-BFGS-B") == 0)
    {
        ...
        args = CDR(args);
        slower = CAR(args); /* coerce in calling code */
        args = CDR(args);
        supper = CAR(args);
        for (i = 0; i < npar; i++)
        {
    ...
    if (!isNull(OS->names))
        setAttrib(par, R_NamesSymbol, OS->names);
...
SEXP optimhess(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < npar; i++)
        for (j = 0; j < npar; j++)
        {
            if (i == j)
                continue;
            REAL(ans)[i * npar + j] = 0.5 * (REAL(ans)[i * npar + j] + REAL(ans)[j * npar + i]);
        }
