...
void F77_NAME(ditsum)(const double d[], const double g[], int iv[], const int *liv, const int *lv, const int *n,
...
void Rf_divset(int alg, int iv[], int liv, int lv, double v[])
{
    ...
    if (PRUNIT <= liv)
        iv[PRUNIT] = 0; /* suppress all Fortran output */
    if (ALGSAV <= liv)
        iv[ALGSAV] = alg;
    if (alg < 1 || alg > 4)
    ...
    iv[1] = 12;
    if (alg > 2)
        error(_("port algorithms 3 or higher are not supported"));
    iv[IVNEED] = 0;
    iv[LASTIV] = miv;
    iv[LASTV] = mv;
    iv[LMAT] = mv + 1;
...
void F77_NAME(dn2cvp)(const int iv[], int *liv, int *lv, int *p, const double v[])
{
    /* Done elsewhere */
}
...
void F77_NAME(dv2axy)(int *n, double w[], const double *a, const double x[], const double y[])
{
    int i, nn = *n;
    double aa = *a;
    for (i = 0; i < nn; i++)
...
void F77_NAME(dv7ipr)(int *n, const int ip[], double x[])
{
    ...
    for (i = 0; i < nn; i++)
        xcp[i] = x[ip[i] - 1]; /* ip contains 1-based indices */
    Memcpy(x, xcp, nn);
    R_Free(xcp);
}
...
void F77_NAME(dv7prm)(int *n, const int ip[], double x[])
{
    ...
    for (i = 0; i < nn; i++)
        xcp[ip[i] - 1] = x[i]; /* ip contains 1-based indices */
    Memcpy(x, xcp, nn);
    R_Free(xcp);
}

/* dv7scl... scale src by *scal to dest */
void F77_NAME(dv7scl)(int *n, double dest[], const double *scal, const double src[])
{
    int nn = *n;
    double sc = *scal;
    while (nn-- > 0)
...
static double *check_gv(SEXP gr, SEXP hs, SEXP rho, int n, double *gv, double *hv)
{
    ...
    for (int i = 0; i < n; i++)
        if (ISNAN(gv[i]))
            error("NA/NaN gradient evaluation");
    if (hv)
    {
        ...
        if (!isReal(hval) || LENGTH(dim) != 2 || INTEGER(dim)[0] != n || INTEGER(dim)[1] != n)
        ...
        for (i = 0, pos = 0; i < n; i++) /* copy lower triangle row-wise */
            for (j = 0; j <= i; j++)
            {
                hv[pos] = rhval[i + j * n];
                if (ISNAN(hv[pos]))
                    error("NA/NaN Hessian evaluation");
                pos++;
            }
        UNPROTECT(1);
    }
...
void nlminb_iterate(double b[], double d[], double fx, double g[], double h[], int iv[], int liv, int lv, int n,
                    double v[], double x[])
{
    int lh = (n * (n + 1)) / 2;
    if (b)
    ...
    else
    {
        if (g)
        {
            ...
            F77_CALL(drmnf)(d, &fx, iv, &liv, &lv, &n, v, x);
    }
}
...
SEXP port_nlminb(SEXP fn, SEXP gr, SEXP hs, SEXP rho, SEXP lowerb, SEXP upperb, SEXP d, SEXP iv, SEXP v)
{
    int i, n = LENGTH(d);
    SEXP xpt;
    ...
    do
    {
        nlminb_iterate(b, REAL(d), fx, g, h, INTEGER(iv), LENGTH(iv), LENGTH(v), n, REAL(v), REAL(xpt));
        if (INTEGER(iv)[0] == 2 && g)
...
void nlsb_iterate(double b[], double d[], double dr[], int iv[], int liv, int lv, int n, int nd, int p, double r[],
                  double rd[], double v[], double x[])
{
    int ione = 1;
    if (b)
        F77_CALL(drn2gb)(b, d, dr, iv, &liv, &lv, &n, &nd, &ione, &nd, &p, r, rd, v, x);
    else
        F77_CALL(drn2g)(d, dr, iv, &liv, &lv, &n, &nd, &ione, &nd, &p, r, rd, v, x);
}
...
static void neggrad(SEXP gf, SEXP rho, SEXP gg)
{
    ...
    int *dims = INTEGER(getAttrib(val, R_DimSymbol)), *gdims = INTEGER(getAttrib(gg, R_DimSymbol));
    int i, ntot = gdims[0] * gdims[1];

    if (TYPEOF(val) != TYPEOF(gg) || !isMatrix(val) || dims[0] != gdims[0] || dims[1] != gdims[1])
        error(_("'gradient' must be a numeric matrix of dimension (%d,%d)"), gdims[0], gdims[1]);
    for (i = 0; i < ntot; i++)
        REAL(gg)[i] = -REAL(val)[i];
    UNPROTECT(1);
}
 ...
 */
static SEXP eval_check_store(SEXP fcn, SEXP rho, SEXP vv)
{
    SEXP v = PROTECT(eval(fcn, rho));
    if (TYPEOF(v) != TYPEOF(vv) || LENGTH(v) != LENGTH(vv))
        error(_("fcn produced mode %d, length %d - wanted mode %d, length %d"), TYPEOF(v), LENGTH(v), TYPEOF(vv),
              LENGTH(vv));
    switch (TYPEOF(v))
    {
    case LGLSXP:
...
SEXP port_nlsb(SEXP m, SEXP d, SEXP gg, SEXP iv, SEXP v, SEXP lowerb, SEXP upperb)
{
    ...
    do
    {
        nlsb_iterate(b, REAL(d), REAL(gg), INTEGER(iv), LENGTH(iv), LENGTH(v), n, nd, p, REAL(rr), rd, REAL(v),
                     REAL(x));
        switch (INTEGER(iv)[0])
        {
        case -3:
