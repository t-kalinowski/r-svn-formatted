...
SEXP Random2(SEXP args)
{
    ...
    else if (streql(dn, "rcauchy"))
        fn = &rcauchy;
    else if (streql(dn, "rf"))
        fn = &rf;
    else if (streql(dn, "rgamma"))
        fn = &rgamma;
    else if (streql(dn, "rlnorm"))
        fn = &rlnorm;
    else if (streql(dn, "rlogis"))
        fn = &rlogis;
    ...
    else if (streql(dn, "rnorm"))
        fn = &rnorm;
    else if (streql(dn, "runif"))
        fn = &runif;
    else if (streql(dn, "rweibull"))
        fn = &rweibull;
...
SEXP Rmultinom(SEXP args)
{
    ...
    args = CDR(args);
    n = asInteger(CAR(args));
    args = CDR(args); /* n= #{samples} */
    size = asInteger(CAR(args));
    args = CDR(args); /* X ~ Multi(size, prob) */
    if (n == NA_INTEGER || n < 0)
...
SEXP r2dtable(SEXP n, SEXP r, SEXP c)
{
    ...
    */
    ...
    for (i = 0; i < n_of_samples; i++)
    {
        ...
        rcont2(&nr, &nc, row_sums, col_sums, &n_of_cases, fact, jwork, INTEGER(tmp));
