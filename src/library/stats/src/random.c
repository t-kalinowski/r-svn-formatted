...
static R_INLINE SEXP random2(SEXP sn, SEXP sa, SEXP sb, ran2 fn, SEXPTYPE type)
{
    ...
    PROTECT_INDEX pxi;
    PROTECT_WITH_INDEX(x, &pxi);
    R_xlen_t na = XLENGTH(sa), nb = XLENGTH(sb);
    ...
    {
        Rboolean naflag = FALSE;
        SEXP a = PROTECT(coerceVector(sa, REALSXP)), b = PROTECT(coerceVector(sb, REALSXP));
        R_xlen_t i0 = 0;
        SEXPTYPE use_type = type;
        GetRNGstate();
...
static R_INLINE SEXP random3(SEXP sn, SEXP sa, SEXP sb, SEXP sc, ran3 fn, SEXPTYPE type)
{
    ...
    PROTECT_INDEX pxi;
    PROTECT_WITH_INDEX(x, &pxi);
    R_xlen_t na = XLENGTH(sa), nb = XLENGTH(sb), nc = XLENGTH(sc);
