...
#define SSPLINE_COMP(_SPAR_)                                                                                           \
    *lspar = ratio * R_pow(16., (_SPAR_)*6. - 2.);                                                                     \
    ...
    if (*icrit == 4)
    { // find zero s* F(s*) == 0,  F(spar) := df(spar) - dofoff
        SSPLINE_COMP(ax);
        double fa = *crit;
        if (*ier)
        {
            warning(_("finding df(spar) == df: too small left boundary 'low=%g'"), ax);
            goto L_do_minimize;
        }
        if (tracing)
        {
            Rprintf("sbart (ratio = %15.8g) uniroot iterations; f(low=%g) = %g\n", ratio, ax,
                    fa); // TODO: add more, incl  HEADER for subseqent
        }
        SSPLINE_COMP(bx);
        double fb = *crit;
        if (*ier)
        {
            warning(_("finding df(spar) == df: too large right boundary 'high=%g'"), bx);
            goto L_do_minimize;
        }
        if (tracing)
            Rprintf(" ... f(high=%g) = %g\n", bx, fb);
...
#define ZEROIN_MY_TRACE                                                                                                \
    if (tracing)                                                                                                       \
    Rprintf("%3d(%3d): [%11.6g %11.6g] -> (%9.5g %9.5g) \n", *Maxit - maxit + 1, *ier, b, c, fb, fc)

#define ZEROIN_RETURN(R)                                                                                               \
    x = R;                                                                                                             \
    goto L_Zero_End;
#define ZEROIN_FN_GET(_FX_, _X_)                                                                                       \
    SSPLINE_COMP(_X_);                                                                                                 \
    _FX_ = *crit;                                                                                                      \
    ZEROIN_MY_TRACE
#define ZEROIN_TRACE
