...
void starma(Starma G, int *ifault)
{
    ...
    double *phi = G->phi, *theta = G->theta, *a = G->a, *P = G->P, *V = G->V, *thetab = G->thetab, *xnext = G->xnext,
...
/*  start of AS 182 */
void forkal(Starma G, int d, int il, double *delta, double *y, double *amse, int *ifault)
{
    ...
    Free(G->a);
    G->a = a = Calloc(rd, double);
    Free(G->P);
    G->P = P = Calloc(rz, double);
