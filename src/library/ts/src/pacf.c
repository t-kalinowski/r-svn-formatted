...
typedef struct
{
    ...
    double *a, *p, *v, *thetab, *xnext, *xrow, *rbar, *w, *wkeep, delta, *resid, *phi, *theta, s2, *reg, *params;
#ifdef ONE_global
...
void arma0fa(double *inparams, double *res)
{
    ...
    it = 0;
    iupd = 1;
    F77_CALL(karma)(&ip, &iq, &ir, &np, phi, theta, a, p, v, &n, w, resid, &sumlog, &ssq, &iupd, &delta, &it);
...
void arma0_kfore(int *pd, int *psd, int *n_ahead, double *x, double *var)
{
    ...
    F77_CALL(forkal)
    (&ip, &iq, &ir, &np, &ird, &irz, &d, &il, &n, &nrbar, phi, theta, del + 1, w, x, var, a1, p1, v, resid, xnext, xrow,
