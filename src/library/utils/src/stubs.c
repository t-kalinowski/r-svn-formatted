...
{
    if (ptr_R_addhistory)
        ptr_R_addhistory(call, op, CDR(args), rho);
    return R_NilValue;
}
...
static void R_de_Init(void)
{
    ...
    if (!res)
        error(_("X11 dataentry cannot be loaded"));
    de_ptr->de = (R_X11DataEntryRoutine)R_FindSymbol("in_RX11_dataentry", "R_de", NULL);
    de_ptr->dv = (R_X11DataViewer)R_FindSymbol("in_R_X11_dataviewer", "R_de", NULL);
    de_init = 1;
    return;
}
...
SEXP dataviewer(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (ptr_do_dataviewer)
        ...
        return X11_do_dataviewer(call, op, args, env);
}

SEXP selectlist(SEXP call, SEXP op, SEXP args, SEXP env)
{
    if (ptr_do_selectlist)
        return ptr_do_selectlist(call, op, CDR(args), env);
    return R_NilValue;
}
#endif
...
SEXP octsize(SEXP size)
{
    ...
    Rbyte *ra = RAW(ans);
    if (!R_FINITE(s) && s >= 0)
        error("size must be finite and >= 0");
