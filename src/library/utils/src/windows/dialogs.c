...
SEXP winDialog(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    message = CADR(args);
    if (!isString(message) || length(message) != 1 || strlen(translateChar(STRING_ELT(message, 0))) > 999)
...
SEXP winMenuItems(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (items->numItems == 0)
    {
        snprintf(msgbuf, 256, _("unable to retrieve items for %s (%s)"), translateChar(STRING_ELT(mname, 0)), errmsg);
        freemenuitems(items);
