...
static pascal OSStatus MyGetSetItemData(ControlRef browser, DataBrowserItemID itemID, DataBrowserPropertyID property,
                                        DataBrowserItemDataRef itemData, Boolean changeValue);

static pascal void MyItemNotification(ControlRef browser, DataBrowserItemID itemID,
                                      DataBrowserItemNotification message);
pascal OSStatus BrowserEventHandler(EventHandlerCallRef, EventRef, void *);

static pascal Boolean MyItemComparison(ControlRef browser, DataBrowserItemID itemOneID, DataBrowserItemID itemTwoID,
...
void OpenDataBrowser(void)
{
    ...
    CreateNewWindow(kDocumentWindowClass, kWindowStandardHandlerAttribute | kWindowStandardDocumentAttributes, &bounds,
                    &window);
    ...
    InstallWindowEventHandler(window, NewEventHandlerUPP(BrowserEventHandler),
    ...
    SetWindowProperty(window, kMyCreator, kMyDataBrowser, sizeof(browser), &browser);

    InitContainers();

    AddDataBrowserItems(browser, kDataBrowserNoItem, NumOfRoots, RootItems, kDataBrowserItemNoProperty);
...
static void ConfigureDataBrowser(ControlRef browser)
{
    ...
    case kDataBrowserListView: {
        ...
        columnDesc.propertyDesc.propertyFlags = kDataBrowserPropertyIsMutable | kDataBrowserListViewSelectionColumn |
        ...
        columnDesc.propertyDesc.propertyFlags =
            kDataBrowserListViewSelectionColumn | kDataBrowserListViewDefaultColumnFlags;
        ...
        columnDesc.headerBtnDesc.titleString =
            CFStringCreateWithPascalString(CFAllocatorGetDefault(), "\pSize", kCFStringEncodingMacRoman);
...
static ControlRef GetDataBrowserFromWindow(WindowRef window)
{
    ...
    if (window != NULL)
        GetWindowProperty(window, kMyCreator, kMyDataBrowser, sizeof(browser), NULL, &browser);
...
void InstallDataBrowserCallbacks(ControlRef browser)
{
    ...
    myCallbacks.u.v1.itemDataCallback = NewDataBrowserItemDataUPP(MyGetSetItemData);

    myCallbacks.u.v1.itemCompareCallback = NewDataBrowserItemCompareUPP(MyItemComparison);

    myCallbacks.u.v1.itemNotificationCallback = NewDataBrowserItemNotificationUPP(MyItemNotification);
...
static Boolean AppendString(Str255 ioDestString, ConstStr255Param inSourceString)
{
    SInt16 oldSize = ioDestString[0];

    if (oldSize + inSourceString[0] <= 255)
    {
        BlockMoveData(&(inSourceString[1]), &(ioDestString[oldSize + 1]), inSourceString[0]);
...
// --------------------------------------------------------------------
static void GenerateString(DataBrowserItemID itemID, DataBrowserPropertyID property, StringPtr string)
{
    Str255 numString;
    Boolean itemXProperty = property == kTypeColumn || property == kDataBrowserItemSelfIdentityProperty;

    string[0] = 0;
    NumToString(itemID, numString);

    if (itemXProperty)
        AppendString(string, "\pItem ");
...
static pascal void MyItemNotification(ControlRef browser, DataBrowserItemID itemID, DataBrowserItemNotification message)
{
    UInt32 i;
    ...
    case kDataBrowserContainerOpened: { // Generate some valid itemIDs
        ...
        { // Set up variable height rows
            ...
            if (variableHeightRows)
                for (j = 0; j < ItemsPerContainer[itemID]; j++)
                    SetDataBrowserTableViewItemRowHeight(browser, SubItemsID[itemID][j],
                                                         20 + (SubItemsID[itemID][j] - 1) % 10 * 3);
        }
        // if(myItems)
        // free(myItems);
    }
...
pascal OSStatus BrowserEventHandler(EventHandlerCallRef a, EventRef inEvent, void *b)
{
