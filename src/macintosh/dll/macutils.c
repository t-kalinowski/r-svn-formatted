...
OSErr FSMakeFSSpecFromPath(ConstStr255Param fileName, FSSpecPtr spec)
{
    ...
    while (true)
    {
        if ((err = FSMakeFSSpec(vRefNum, dirID, Name[0] == 0 ? NULL : Name, spec)) != noErr ||
...
/* The Apple MoreFiles 1.2.1 collection is
   Copyright © 1992-1994 Apple Computer, Inc.
   All rights reserved.
*/

OSErr GetDirectoryID(short vRefNum, long dirID, StringPtr name, long *theDirID, Boolean *isDirectory)
{
    CInfoPBRec pb;
    Str31 tempName;
    OSErr error;
...
{
    return GetDirectoryID(spec->vRefNum, spec->parID, (StringPtr)spec->name, theDirID, isDirectory);
}
