...
#define kStyleResType                                                                                                  \
    'styl' // We don't use this since
           // we don't do styled text

#define kToySurprise 'ToyS' // Script file creator
#define kCompiledScript                                                                                                \
    'osas' // Script file type for
           // compiled scripts
#define kTextScript                                                                                                    \
    'TEXT' // Script file type for
           // text scripts
...
pascal OSErr FSpLocationFromFullPath(short fullPathLength, const void *fullPath, FSSpec *spec);
...
SEXP do_applescript(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if ((rc = FSpLocationFromFullPath(strlen(fullpath), fullpath, &scriptfss)) != noErr)
    {
...
OSErr ExecuteScript(FSSpec theFileToExecute, AEDesc *result)
{
    ...
    */
    ...
    if (tempHandle == nil)
    {
        ...
        if (theErr == noErr)
        {
            theOSAErr = OSACompileExecute(gASComponent, &source, kOSANullScript, modeFlags, &resultingID);
            ...
            if (theOSAErr == noErr)
            {
                ...
                */
                theOSAErr = OSACoerceToDesc(gASComponent, resultingID, typeWildCard, modeFlags, result);
            }
    ...
    else
    {
        ...
        theErr = AECreateDesc(kOSAScriptResourceType, (Ptr)*tempHandle, (Size)(GetHandleSize(tempHandle)), &source);
        if (theErr != noErr)
        {
            theErr = theOSAErr;
            goto BAILLABEL;
        ...
        theOSAErr = OSALoadExecute(gASComponent, &source, kOSANullScript, modeFlags, &resultingID);
        if (theOSAErr != noErr)
        {
            theErr = theOSAErr;
            goto BAILLABEL;
...
BAILLABEL:;
    // Clean up time
    if (resultingID != 0)
        OSADispose(gASComponent, resultingID);
    if (tempHandle != nil)
        DisposeHandle(tempHandle);
    if (source.dataHandle != nil)
        AEDisposeDesc(&source);
    if (gASComponent != 0)
        CloseComponent(gASComponent);
