...
short replyLevels[] = {kAENoReply, kAEWaitReply, kAEQueueReply};
...
static int do_unzip(char *zipname, char *dest, int nfiles, char **files, int nxfiles, char **xfiles, int over)
{
    ...
    if ((err = LaunchUnZipTool(&gTheAddress)) != noErr)
    { /* we cannot find MacZip */
        StopAlert(kNoFind, nil);
        return;
    }

    /* prepares the AppleEvent */
    AECreateAppleEvent(kCMDEventClass, kCMDEvent, &gTheAddress, kAutoGenerateReturnID, kAnyTransactionID, &ourEvent);

    /* Add the command string to the AE */
    AEPutParamPtr(&ourEvent, keyDirectObject, typeChar, commandstr, sizeof(char) * strlen(commandstr));

    /* Send the Event */

    if (replyLevels[replyValue] == kAEWaitReply && !gSwitchLayer)
    {
        AESend(&ourEvent, &ourReply, (gSendInteractArray[gInteract] + gSwitchLayer) + replyLevels[replyValue],
               kAENormalPriority, kAEDefaultTimeout, gAEIdleUPP, nil);
    }
    ...
    if (replyLevels[replyValue] == kAEWaitReply)
...
OSErr LaunchUnZipTool(AEDesc *theAddress)
{
    ...
    if (myError == noErr && csBlockPtr->ioActMatchCount != 0)
    {
        ...
        if (myError == noErr)
        {
            ...
            AECreateDesc(typeProcessSerialNumber, (Ptr)&launchUnZipTool.launchProcessSN, sizeof(ProcessSerialNumber),
                         theAddress);
        }
    }
