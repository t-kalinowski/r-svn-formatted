...
static Rotate(BitMap *sourceMap, BitMap *destMap, int rot)
{
    ...
    if (rot <= 360)
    {
        for (i = 0; i < checkHeight; i++)
        {
            ...
            {
                if (inStrRange(BackTranX(tranBitX(j, rot), tranBitY(i, checkHeight, rot), rot),
                               BackTranY(tranBitX(j, rot), tranBitY(i, checkHeight, rot), rot)))
                    if (BitTst(sourceMap->baseAddr,
                               tranStrY(BackTranY(tranBitX(j, rot), tranBitY(i, checkHeight, rot), rot)) * sourceBits +
                                   BackTranX(tranBitX(j, rot), tranBitY(i, checkHeight, rot), rot)))
                    {
...
void RasterTextRotation(char *str, int nstr, int just, int rot)
{
    ...
    if (rot <= 90)
    {
        newWidth = mapLength * cos(toRadian(rot)) + mapHeight * sin(toRadian(rot));
        newHeight = mapLength * sin(toRadian(rot)) + mapHeight * cos(toRadian(rot));
        SetRect(&destRect, -mapHeight * sin(toRadian(rot)), -newHeight, mapLength * cos(toRadian(rot)) + gcon,
                0); // gcon at the end?
    }
    else if (rot <= 180)
    {
        newWidth = mapLength * cos(toRadian(180 - rot)) + mapHeight * sin(toRadian(180 - rot));
        newHeight = mapLength * sin(toRadian(180 - rot)) + mapHeight * cos(toRadian(180 - rot));
        SetRect(&destRect, -newWidth, -mapLength * sin(toRadian(180 - rot)) - gcon, gcon,
                mapHeight * cos(toRadian(180 - rot)));
    }
    else if (rot <= 270)
    {
        newWidth = mapLength * cos(toRadian(rot - 180)) + mapHeight * sin(toRadian(rot - 180));
        newHeight = mapLength * sin(toRadian(rot - 180)) + mapHeight * cos(toRadian(rot - 180));
        SetRect(&destRect, -mapLength * cos(toRadian(rot - 180)) - gcon, -gcon, mapHeight * sin(toRadian(rot - 180)),
                newHeight);
    }
    else if (rot < 360)
    {
        newWidth = mapLength * cos(toRadian(360 - rot)) + mapHeight * sin(toRadian(360 - rot));
        newHeight = mapLength * sin(toRadian(360 - rot)) + mapHeight * cos(toRadian(360 - rot));
        SetRect(&destRect, -gcon, -mapHeight * cos(toRadian(360 - rot)), newWidth,
                mapLength * sin(toRadian(360 - rot)) + gcon);
    }
...
int tranBitY(int y, int checkHeight, int rot)
{
    if (rot <= 90)
        ...
        return (checkHeight - y) - (mapLength * sin(toRadian(rot - 180)) + mapHeight * cos(toRadian(rot - 180)));
    else if (rot < 360)
...
int tranBitX(int y, int rot)
{
    if (rot <= 90)
        ...
        return y - (mapLength * cos(toRadian(180 - rot)) + mapHeight * sin(toRadian(180 - rot)));
    else if (rot <= 270)
...
Boolean inStrRange(int x, int y)
{
    if ((x <= 0) || (y <= -gcon) || (x >= mapLength) || (y >= mapHeight - gcon))
        return false;
    return true;
}
