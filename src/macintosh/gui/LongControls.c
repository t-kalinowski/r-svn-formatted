...
void LCSetValue(ControlHandle inControl, SInt32 inValue)
{
    ...
    newControlValue = controlMin + FixRound(FixMul(FixDiv(inValue - (*aux)->min, (*aux)->max - (*aux)->min),
                                                   BSL(controlMax - controlMin, 16)));
...
void LCSynch(ControlHandle inControl)
{
    ...
    (*aux)->value =
        (*aux)->min + FixMul(FixRatio(controlValue - controlMin, controlMax - controlMin), (*aux)->max - (*aux)->min);
}
