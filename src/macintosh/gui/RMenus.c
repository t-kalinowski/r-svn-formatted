...
OSErr MyOpenDocument(FSSpec *documentFSSpec, SFTypeList *typeList)
{
    ...
    if (anErr == noErr)
    {
        ...
        if (anErr == noErr)
        {
            ...
            NavTypeListHandle deftypeList = (NavTypeListHandle)GetResource('open', 128);
            deftypeList = nil; /* we apply no filter for the moment */

            anErr = AECreateDesc(typeFSS, &defaultLocationfss, sizeof(defaultLocationfss), &defaultLocation);

            /* Call NavGetFile() with specified options and
                        declare our app-defined functions and type list
                      */
            anErr = NavGetFile(&defaultLocation, &reply, &dialogOptions, nil, nil, nil, deftypeList, nil);
            ...
            if (anErr == noErr && reply.validRecord)
            {
                ...
                if (anErr == noErr)
                {
                    ...
                    for (index = 1; index <= count; index++)
                    {
                        ...
                        anErr = AEGetNthPtr(&(reply.selection), index, typeFSS, &theKeyword, &actualType,
...
OSErr DoSaveAs(const FSSpec *suggestedTarget, WindowPtr window)
{
    ...
    if (anErr == noErr && reply.validRecord)
    {
        ...
        /* Get a pointer to selected file */
        anErr = AEGetNthPtr(&(reply.selection), 1, typeFSS, &theKeyword, &actualType, &mytarget, sizeof(mytarget),
                            &actualSize);
...
SavingOption DoWeSaveIt(WindowPtr window)
{
    ...
    if (WEGetModCount(GetWindowWE(window)) > 0)
    {
        ...
        if (alertResult == kButtonCancel)
            saving = savingCancel;
        else if (alertResult == kButtonSave)
            saving = savingYes;
        else
            saving = savingNo;
    }
...
int GetScreenRes(void)
{
    ...
    if (gScreenRes != 72 && gScreenRes != 144 && gScreenRes != 300 && gScreenRes != 600)
        gScreenRes = 72;
    return (gScreenRes);
}
...
OSErr OpenSelection(FSSpecPtr theDoc)
{
    ...
    // Create an empty
    myErr = AECreateAppleEvent(kAEFinderEvents, kAEOpenSelection, &myAddressDesc, kAutoGenerateReturnID,
...
static pascal void NavEventFilter(NavEventCallbackMessage inSelector, NavCBRec *inPB, void *inUserData)
{
    ...
    case kNavCBStart: {
        ...
        /*		if ( ( err = GetDialogItemAsControl (dialog, cd -> numItems + kItemFormatPopup, & cd -> formatPopup ) ) !=
           noErr )
                {
                    return ;
                }
                if ( ( err = GetDialogItemAsControl ( dialog, cd -> numItems + kItemStationeryCheckbox, & cd ->
           stationeryCheckbox ) ) != noErr )
                {
                    return ;
                }
                //	set up the format popup
                SetControlValue ( cd -> formatPopup, ( cd -> fileType == kTypeText ) ? kItemTextFormat :
           kItemUnicodeTextFormat ) ; break ;
    */
    }
    }
}

static OSStatus CreateNavTypeList(OSType inApplicationSignature, UInt16 inNumTypes, const OSType *inSFTypeList,
                                  NavTypeListHandle *outNavTypeList)
{
    ...
    *outNavTypeList =
        (NavTypeListHandle)NewHandleClear((sizeof(NavTypeList) - sizeof(OSType)) + (inNumTypes * sizeof(OSType)));
    if ((err = MemError()) != noErr)
    {
        return err;
    }
