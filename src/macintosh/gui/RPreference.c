...
/* save the prefs structure in the prefs resource file */
{
    ...
    if (prefsResRefNum != -1)
    {
        ...
        if (prefHandle == nil)
        {
            ...
            if (prefHandle != nil)
            {
                ...
                *(appPrefs *)*prefHandle = *thePrefsTypePtr;
                AddResource(prefHandle, kPrefsResourceType, kPrefsResourceID, "\pRPrefs");
                retCode = ResError();
                if (retCode != noErr)
        ...
        if (retCode == noErr)
        {
            ...
            if (finderMessageHandle != nil && HomeResFile((Handle)finderMessageHandle) != prefsResRefNum)
            {
                ...
                retCode = ResError();
                if (retCode != noErr)
                    DisposeHandle(finderMessageHandle);
            }
        }
   ...
   create a preferences file if necessary */
{
    ...
    retCode = FindFolder(kOnSystemDisk, kPreferencesFolderType, kCreateFolder, &prefsVRefNum, &prefsDirID);
