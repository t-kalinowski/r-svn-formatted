...
static Boolean PropertyOf(const AERecord *spec, SInt16 *propLevel, PropArray properties)
{
    ...
    if (AEGetParamPtr(&objSpec, keyAEDesiredClass, typeType, &actualType, &theType, sizeof(theType), &actualSize) !=
        noErr)
        goto cleanup;

    // sanity check: make sure the key form is formPropertyID
    // this is probably redundant, but checking doesn't hurt
    if (AEGetParamPtr(&objSpec, keyAEKeyForm, typeEnumerated, &actualType, &theType, sizeof(theType), &actualSize) !=
        noErr)
        goto cleanup;
    ...
    if (AEGetParamPtr(&objSpec, keyAEKeyData, typeType, &actualType, &theType, sizeof(theType), &actualSize) != noErr)
        goto cleanup;
...
#pragma unused(refCon)
    ...
    if (!PropertyOf(ae, &propLevel, properties) || (propLevel != 2) || (properties[0] != pContents) ||
    ...
    if (AEGetParamPtr(ae, keyAERequestedType, typeType, &actualType, &requestedType, sizeof(requestedType),
                      &actualSize) != noErr)
    {
...
OSStatus InstallCoreHandlers(void)
{
    ...
    if ((err = AEInstallEventHandler(kAECoreSuite, kAEGetData, NewAEEventHandlerUPP(HandleGetData), 0L, false)) !=
        noErr)
    {
        return err;
    }

    if ((err = AEInstallEventHandler(kAECoreSuite, kAESetData, NewAEEventHandlerUPP(HandleSetData), 0L, false)) !=
        noErr)
    {
        return err;
    }
...
OSStatus CreatePSNBasedAppleEvent(const ProcessSerialNumber *inTargetPSN, AEEventClass inEventClass,
                                  AEEventID inEventID, AppleEvent *outAE)
{
    ...
    if ((err = AECreateDesc(typeProcessSerialNumber, inTargetPSN, sizeof(*inTargetPSN), &targetAddress)) != noErr)
    {
        goto cleanup;
    }

    //	create the Apple event
    if ((err = AECreateAppleEvent(inEventClass, inEventID, &targetAddress, kAutoGenerateReturnID, kAnyTransactionID,
                                  outAE)) != noErr)
    {
        goto cleanup;
    }
...
OSStatus CreateObjectSpecifier(const AEDesc *inContainerDesc, DescType inDesiredClass, AEKeyword inKeyForm,
                               const AEDesc *inKeyData, AEDesc *outObjectSpecifier)
{
    AEDesc recordDesc;
    OSStatus err;
    ...
    if ((err = AEPutParamPtr(&recordDesc, keyAEDesiredClass, typeType, &inDesiredClass, sizeof(inDesiredClass))) !=
        noErr)
    {
        goto cleanup;
    }
    ...
    //	add key form field
    if ((err = AEPutParamPtr(&recordDesc, keyAEKeyForm, typeEnumerated, &inKeyForm, sizeof(inKeyForm))) != noErr)
    {
        goto cleanup;
    }
...
OSStatus CreatePropertySpecifier(const AEDesc *inContainerSpec, AEKeyword inPropertyTag, AEDesc *outPropertySpecifier)
{
    AEDesc propertyDesc;
    OSStatus err;
    ...
    if ((err = CreateObjectSpecifier(inContainerSpec, cProperty, formPropertyID, &propertyDesc,
                                     outPropertySpecifier)) != noErr)
    {
        goto cleanup;
    }
...
OSStatus CreateFinderObjectSpecifier(const FSSpec *inThing, AEDesc *outThingSpec)
{
    Handle alias = nil;
    AEDesc nullDesc;
    AEDesc aliasDesc;
...
OSStatus SendGetDataEvent(const ProcessSerialNumber *inTargetPSN, const AEDesc *inObjectSpec, DescType inRequestedType,
                          AEDesc *outResult)
{
    ...
    {
        if ((err = AEPutParamPtr(&ae, keyAERequestedType, typeType, &inRequestedType, sizeof(inRequestedType))) !=
            noErr)
        {
            goto cleanup;
        }
    ...
    if ((err = AESend(&ae, &reply, kAEWaitReply + kAENeverInteract, kAENormalPriority, kAEDefaultTimeout, nil, nil)) !=
        noErr)
    {
        goto cleanup;
    }
...
OSStatus GetFileRect(const FSSpec *inThing, Rect *outFileRect)
{
