...
OSStatus InstallCoreHandlers(void)
{
    ...
    if ((err = AEInstallEventHandler(kAECoreSuite, kAEGetData, NewAEEventHandlerProc(HandleGetData), 0L, false)) !=
        noErr)
    {
        return err;
    }

    if ((err = AEInstallEventHandler(kAECoreSuite, kAESetData, NewAEEventHandlerProc(HandleSetData), 0L, false)) !=
        noErr)
    {
        return err;
    }
