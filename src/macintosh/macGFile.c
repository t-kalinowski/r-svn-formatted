...
/* ××××××××××××××××××××× doCopyGraResource
 */
OSErr doCopyGraResource(ResType resourceType, SInt16 resourceID, SInt16 sourceFileRefNum, SInt16 destFileRefNum)
{
...
OSErr doWriteFile(WindowPtr windowPtr)
{
    ...
    osError = FindFolder(fileSpecActual.vRefNum, kTemporaryFolderType, kCreateFolder, &tempFileVolNum, &tempFileDirID);
    if (osError == noErr)
