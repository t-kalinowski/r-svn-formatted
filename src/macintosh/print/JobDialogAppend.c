...
pascal TPPrDlg initialisationFunction(THPrint hPrint)
{
    ...
    GetDialogItemAsControl((DialogPtr)gTPrDlgStructurePtr, gFirstAppendedItemNo, &controlHdl);
#ifdef nomore
    GetControlData(controlHdl, kControlNoPart, kControlPopupButtonMenuHandleTag, sizeof(menuHdl), (Ptr)&menuHdl, NULL);
#endif
    ...
    GetDialogItemAsControl((DialogPtr)gTPrDlgStructurePtr, gFirstAppendedItemNo + 2, &controlHdl);
    SetControlValue(controlHdl, 1);

    GetDialogItemAsControl((DialogPtr)gTPrDlgStructurePtr, gFirstAppendedItemNo + 4, &controlHdl);
...
pascal void itemEvaluationFunction(TPPrDlg theDialog, short itemHit)
{
    ...
    if (localizedItemNo > 0)
    {
        if (localizedItemNo == iPopupButton)
        {
            ...
            GetControlData(controlHdl, kControlNoPart, kControlPopupButtonMenuHandleTag, sizeof(menuHdl), (Ptr)&menuHdl,
                           NULL);
