...
static void CalculatePageRect(const Rect *inPaperRect, const PageMarginRec *inPageMargins, Rect *outPageRect);
static void ValidateMargins(const Rect *inPaperRect, const Rect *inMaxPageRect, PageMarginRec *ioMargins);
...
Boolean IncludePostScriptInSpoolFile(PMPrintSession printSession)
{
    ...
    if (status == noErr)
    {
        ...
        {
            if (CFStringCompare(CFArrayGetValueAtIndex(supportedFormats, i), kPMDocumentFormatPICTPS,
                                kCFCompareCaseInsensitive) == kCFCompareEqualTo)
            {
                ...
                arrayOfGraphicsContexts =
                    CFArrayCreate(CFAllocatorGetDefault(), (const void **)strings, 1, &kCFTypeArrayCallBacks);
...
OSStatus DoTextPrint(WindowRef inWindow)
{
    ...
    if ((WEGetUserInfo(kPageFormatTag, (SInt32 *)&flattenedPageFormat, we) == noErr) && (flattenedPageFormat != nil))
    {
...
static void ValidateMargins(const Rect *inPaperRect,   //	paper rectangle
...
static void CalculatePageRect(const Rect *inPaperRect, const PageMarginRec *inPageMargins, Rect *outPageRect)
{
    ...
    outPageRect->right =
        outPageRect->left + ((inPaperRect->right - inPaperRect->left) - (marginRect.left + marginRect.right));
    outPageRect->bottom =
        outPageRect->top + ((inPaperRect->bottom - inPaperRect->top) - (marginRect.top + marginRect.bottom));
}
