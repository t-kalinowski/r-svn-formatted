...
int stat(char *fileName, struct stat *statbuf)
{
    ...
    pb.hFileInfo.ioVRefNum = pb.hFileInfo.ioFDirIndex = pb.hFileInfo.ioDirID = 0;
...
offset to 1970.01.01 */
    ...
    {
        statbuf->st_mode = pb.hFileInfo.ioFlFndrInfo.fdType == 'APPL' || pb.hFileInfo.ioFlFndrInfo.fdType == 'MPST'
                               ? S_IEXEC
                               : S_IFREG;
        statbuf->st_nlink = 0;
