...
int R_ReadConsoleXX(char *prompt, unsigned char *buf, int len, int addtohistory)
{
...
SEXP do_helpstart(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (err != noErr)
    {
        sprintf(errbuf, "error code %d creating file spec for help file %s", err, buf);
        error(errbuf);
        return R_NilValue;
    }
...
OSStatus GoToMyHelpPage(CFStringRef pagePath,   /* If NULL, goes to main TOC */
                        CFStringRef anchorName) /* If NULL, goes to top of page */
{
    ...
    myBookName = CFBundleGetValueForInfoDictionaryKey(myAppsBundle, CFSTR("CFBundleHelpBookName"));
    if (myAppsBundle == NULL)
    {
        err = fnfErr;
        goto bail;
...
SEXP do_helpitem(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (type == 1)
    {
        ...
        if (err != noErr)
        {
            sprintf(errbuf, "error code %d creating file spec for help file %s", err, item);
            error(errbuf);
            return R_NilValue;
        }
...
SEXP do_edit(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    err = ReadTextFile(&tempeditFSS, GetWindowWE(Edit_Windows[Edit_Window - 1]));
...
#define Max_Nsize                                                                                                      \
    50000000                    /* must be < LONG_MAX (= 2^32 - 1 =)                                                   \
                       2147483647 = 2.1e9 */
...
static char *rmspace(char *s)
{
    ...
    for (i = strlen(s) - 1; isspace((int)s[i]); i--)
        s[i] = '\0';
