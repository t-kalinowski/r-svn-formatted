...
pascal void _WETabDrawText(Ptr pText, SInt32 textLength, Fixed slop, JustStyleCode styleRunPosition, WEReference we)
{
...
pascal SInt32 _WETabPixelToChar(Ptr pText, SInt32 textLength, Fixed slop, Fixed *width, WEEdge *edge,
                                JustStyleCode styleRunPosition, Fixed hPos, WEReference we)
{
    SInt32 beginChar = 0;
    SInt32 offset = 0;
    SInt32 ii;
    ...
    for (ii = 0; ii < textLength; ii++)
    {
        ...
        if (pText[ii] == '\t')
        {
            ...
            lastWidth = *width;
            offset += PixelToChar(pText + beginChar, ii - beginChar, slop, lastWidth, (Boolean *)edge, width,
    ...
    {
        lastWidth = *width;
        offset += PixelToChar(pText + beginChar, textLength - beginChar, slop, lastWidth, (Boolean *)edge, width,
                              styleRunPosition, kOneToOneScaling, kOneToOneScaling);
    }
...
pascal SInt16 _WETabCharToPixel(Ptr pText, SInt32 textLength, Fixed slop, SInt32 offset, SInt16 direction,
                                JustStyleCode styleRunPosition, SInt16 hPos, WEReference we)
{
    ...
    width = CharToPixel(pText + beginChar, textLength - beginChar, slop, offset - beginChar, direction,
                        styleRunPosition, kOneToOneScaling, kOneToOneScaling);
    totalWidth += width;
...
pascal StyledLineBreakCode _WETabLineBreak(Ptr pText, SInt32 textLength, SInt32 textStart, SInt32 textEnd,
                                           Fixed *textWidth, SInt32 *textOffset, WEReference we)
{
    LongRect destRect;
