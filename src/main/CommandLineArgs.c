...
/*
  ...
  in global variables.
 */
attribute_hidden SEXP do_commandArgs(SEXP call, SEXP op, SEXP args, SEXP env)
{
    int i;
    SEXP vals;
...
void R_common_command_line(int *pac, char **argv, Rstart Rp)
{
    ...
    while (--ac)
    {
        if (processing && **++av == '-')
        {
            ...
            else if (!strcmp(*av, "--no-echo") || !strcmp(*av, "--slave") || // "deprecated" from R 4.0.0 (spring 2020)
                     !strcmp(*av, "-s"))
            {
                Rp->R_Quiet = TRUE;
                Rp->R_NoEcho = TRUE;
