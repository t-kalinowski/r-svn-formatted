...
static DL_FUNC User_unif_fun, User_unif_nseed, User_unif_seedloc;
...
static Rboolean GetRNGkind(SEXP seeds)
{
    ...
    RNG_kind = newRNG;
    N01_kind = newN01;
    Sample_kind = newSample;
    return FALSE;
invalid:
    RNG_kind = RNG_DEFAULT;
    N01_kind = N01_DEFAULT;
    Sample_kind = Sample_DEFAULT;
...
SEXP attribute_hidden do_setseed(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!isNull(sampkind))
        Samp_kind((Sampletype)asInteger(sampkind));
