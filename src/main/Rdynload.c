...
{
    sym->styles = (R_NativeArgStyle *)malloc(sizeof(R_NativeArgStyle) * (size_t)croutine->numArgs);
    if (!sym->styles)
        error("allocation failure in R_setArgStyles");
    if (sym->styles)
        memcpy(sym->styles, croutine->styles, sizeof(R_NativeArgStyle) * (size_t)croutine->numArgs);
}
...
attribute_hidden int R_moduleCdynload(const char *module, int local, int now)
{
...
SEXP attribute_hidden do_getSymbolInfo(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    checkArity(op, args);
    SEXP sname = CAR(args), spackage = CADR(args), withRegistrationInfo = CADDR(args);
