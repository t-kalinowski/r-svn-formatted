...
#endif
{
    ...
    while (fgets(sm, BUF_SIZE, fp))
    {
        ...
        if (!(p = Renviron_strchr(s, '=')) || !complete_line)
        {
            ...
            else if (strlen(msg) + strlen(line_prefix) + 45 + strlen(truncated_msg) + strlen(ignored_msg) < MSG_SIZE)
            {
                strcat(msg, line_prefix);
                strncat(msg, s, 45);
            ...
            if (!complete_line)
            {
                if (strlen(msg) + strlen(too_long) + strlen(ignored_msg) < MSG_SIZE)
                {
        ...
        *p = '\0';
        char *lhs = rmspace(s), *rhs = findterm(rmspace(p + 1));
        /* set lhs = rhs */
