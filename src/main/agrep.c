...
SEXP attribute_hidden do_agrep(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (useBytes)
        PROTECT(call = lang3(s_nchar, pat, ScalarString(mkChar("bytes"))));
    else
        PROTECT(call = lang3(s_nchar, pat, ScalarString(mkChar("chars"))));
                ...
                else
                {
                    ...
                    for (k = 1; k < nr; k++)
                    {
                        for (l = 1; l < nc; l++)
                        {
                            if (xi[k - 1] == yj[l - 1])
                            {
                                MAT(dists, k, l) = MAT(dists, k - 1, l - 1);
                                if (opt_counts)
                                    MAT(paths, k, l) = 'M';
                            }
                            else
    ...
    for (i = 0; i < nx; i++)
    {
        ...
        else
        {
            ...
            if (opt_counts)
            {
                ...
                if (pmatch == NULL)
                    error("allocation failure in adist");
            }
            ...
            for (j = 0; j < ny; j++)
            {
                ...
                else
                {
                    ...
                    {
                        rc = tre_regawexec(&reg, wtransChar(elt), &match, params, 0);
                        vmaxset(vmax);
                    }
                    else
                    {
                        ...
                        rc = tre_regaexec(&reg, t, &match, params, 0);
                        vmaxset(vmax);
                    }
                    if (rc == REG_OK)
                    {
...
SEXP attribute_hidden do_aregexec(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    pmatch = (regmatch_t *)malloc(nmatch * sizeof(regmatch_t));
    if (pmatch == NULL)
        error("allocation failure in aregexec");
