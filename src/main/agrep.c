...
SEXP attribute_hidden do_adist(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    int cost_ins_opt, cost_del_opt, cost_sub_opt, partial_opt, all_opt, icase_opt, useBytes;
    int i, j, k, nx, ny, nxy;
    ...
    y = CAR(args);
    args = CDR(args);
    cost_ins_opt = asInteger(CAR(args));
    args = CDR(args);
    cost_del_opt = asInteger(CAR(args));
    args = CDR(args);
    cost_sub_opt = asInteger(CAR(args));
    args = CDR(args);
    partial_opt = asInteger(CAR(args));
    args = CDR(args);
    all_opt = asLogical(CAR(args));
    args = CDR(args);
    icase_opt = asLogical(CAR(args));
    args = CDR(args);
    useBytes = asLogical(CAR(args));

    if (partial_opt == NA_INTEGER)
        partial_opt = 0;
    if (all_opt == NA_INTEGER)
        all_opt = 0;
    if (icase_opt == NA_INTEGER)
        icase_opt = 0;
    if (useBytes == NA_INTEGER)
        useBytes = 0;
