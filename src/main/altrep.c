...
#define SET_ALTREP_CLASS_SERIALIZED_CLASS(x, csym, psym, stype) SET_ATTRIB(x, list3(csym, psym, stype))
...
#define ALTREP_CLASS_BASE_TYPE(x) ALTREP_SERIALIZED_CLASS_TYPE(ALTREP_CLASS_SERIALIZED_CLASS(x))
...
static void RegisterClass(SEXP class, int type, const char *cname, const char *pname, DllInfo *dll)
{
    PROTECT(class);
    if (Registry == NULL)
    {
...
#define GENERIC_METHODS_TABLE(x, class) ((class##_methods_t *)CLASS_METHODS_TABLE(ALTREP_CLASS(x)))
...
#define DO_DISPATCH(type, fun, ...) type##_METHODS_TABLE(DISPATCH_TARGET(__VA_ARGS__))->fun(__VA_ARGS__)
...
static SEXP find_namespace(void *data)
{
    return R_FindNamespace((SEXP)data);
}
static SEXP handle_namespace_error(SEXP cond, void *data)
{
    return R_NilValue;
}
...
SEXP attribute_hidden ALTREP_UNSERIALIZE_EX(SEXP info, SEXP state, SEXP attr, int objf, int levs)
{
    ...
    if (type != rtype)
        warning("serialized class '%s' from package '%s' has type %s; "
                ...
                CHAR(PRINTNAME(csym)), CHAR(PRINTNAME(psym)), type2char(type), type2char(rtype));
...
R_xlen_t /*attribute_hidden*/ ALTREP_TRUELENGTH(SEXP x)
{
    return 0;
}
...
static SEXP altrep_UnserializeEX_default(SEXP class, SEXP state, SEXP attr, int objf, int levs)
{
    altrep_methods_t *m = CLASS_METHODS_TABLE(class);
...
static SEXP altrep_DuplicateEX_default(SEXP x, Rboolean deep)
{
    ...
    if (ans != NULL && ans != x)
    { /* leave attributes alone if returning original */
...
static int altinteger_Elt_default(SEXP x, R_xlen_t i)
{
    return INTEGER(x)[i];
}
...
static altinteger_methods_t altinteger_default_methods = {.UnserializeEX = altrep_UnserializeEX_default,
...
#define INIT_CLASS(cls, type)                                                                                          \
    do                                                                                                                 \
    {                                                                                                                  \
        *((type##_methods_t *)(CLASS_METHODS_TABLE(cls))) = type##_default_methods;                                    \
    } while (FALSE)
...
static R_altrep_class_t make_altrep_class(int type, const char *cname, const char *pname, DllInfo *dll)
{
    ...
    {
    case INTSXP:
        MAKE_CLASS(class, altinteger);
        break;
    case REALSXP:
        MAKE_CLASS(class, altreal);
        break;
    case STRSXP:
        MAKE_CLASS(class, altstring);
        break;
    default:
        error("unsupported ALTREP class");
    }
...
#define DEFINE_CLASS_CONSTRUCTOR(cls, type)                                                                            \
    R_altrep_class_t R_make_##cls##_class(const char *cname, const char *pname, DllInfo *dll)                          \
    {                                                                                                                  \
...
static void reinit_altrep_class(SEXP class)
{
    ...
    {
    case INTSXP:
        INIT_CLASS(class, altinteger);
        break;
    case REALSXP:
        INIT_CLASS(class, altreal);
        break;
    case STRSXP:
        INIT_CLASS(class, altstring);
        break;
    default:
        error("unsupported ALTREP class");
    }
...
#define DEFINE_METHOD_SETTER(CNAME, MNAME)                                                                             \
    void R_set_##CNAME##_##MNAME##_method(R_altrep_class_t cls, R_##CNAME##_##MNAME##_method_t fun)                    \
    {                                                                                                                  \
...
#define COMPACT_INTSEQ_SERIALIZED_STATE_FIRST(info) (TYPEOF(info) == INTSXP ? INTEGER0(info)[1] : (int)REAL0(info)[1])
#define COMPACT_INTSEQ_SERIALIZED_STATE_INCR(info) (TYPEOF(info) == INTSXP ? INTEGER0(info)[2] : (int)REAL0(info)[2])
                                       ...
                                       void (*inspect_subtree)(SEXP, int, int, int))
{
    ...
    Rprintf(" %d : %d (%s)", n1, n2, COMPACT_SEQ_EXPANDED(x) == R_NilValue ? "compact" : "expanded");
    Rprintf("\n");
    return TRUE;
}
                                        ...
                                        void (*inspect_subtree)(SEXP, int, int, int))
{
    ...
    Rprintf(" %ld : %ld (%s)", n1, n2, COMPACT_SEQ_EXPANDED(x) == R_NilValue ? "compact" : "expanded");
    Rprintf("\n");
    return TRUE;
}
...
{
    R_altrep_class_t cls = R_make_altreal_class("compact_realseq", "base", NULL);
...
#define DEFERRED_STRING_ARG(x) DEFERRED_STRING_STATE_ARG(DEFERRED_STRING_STATE(x))
#define DEFERRED_STRING_SCIPEN(x) DEFERRED_STRING_STATE_SCIPEN(DEFERRED_STRING_STATE(x))
...
{
    SEXP state = DEFERRED_STRING_STATE(x);
    return state == R_NilValue ? XLENGTH(DEFERRED_STRING_EXPANDED(x)) : XLENGTH(DEFERRED_STRING_STATE_ARG(state));
}
...
static int deferred_string_Is_sorted(SEXP x)
{
    ...
    else
    {
        ...
        {
        case INTSXP:
            return INTEGER_IS_SORTED(arg);
        case REALSXP:
            return REAL_IS_SORTED(arg);
        default:
            return UNKNOWN_SORTEDNESS;
        }
    }
...
static SEXP deferred_string_Extract_subset(SEXP x, SEXP indx, SEXP call)
{
    ...
    if (!OBJECT(x) && ATTRIB(x) == R_NilValue && DEFERRED_STRING_STATE(x) != R_NilValue)
    {
...
{
    R_altrep_class_t cls = R_make_altstring_class("deferred_string", "base", NULL);
...
static SEXP make_mmap_state(SEXP file, size_t size, int type, Rboolean ptrOK, Rboolean wrtOK, Rboolean serOK)
{
    ...
    {
    case INTSXP:
        dsizes[1] = size / sizeof(int);
        break;
    case REALSXP:
        dsizes[1] = size / sizeof(double);
        break;
    default:
        error("mmap for %s not supported yet", type2char(type));
    }
...
static SEXP make_mmap(void *p, SEXP file, size_t size, int type, Rboolean ptrOK, Rboolean wrtOK, Rboolean serOK)
{
    SEXP state = PROTECT(make_mmap_state(file, size, type, ptrOK, wrtOK, serOK));
...
static void register_mmap_eptr(SEXP eptr)
{
    ...
    {
        cleancount = MAXCOUNT;
        for (SEXP last = mmap_list, next = CDR(mmap_list); next != R_NilValue; next = CDR(next))
    ...
    SETCDR(mmap_list, CONS(R_MakeWeakRefC(eptr, R_NilValue, mmap_finalize, TRUE), CDR(mmap_list)));
...
Rboolean mmap_Inspect(SEXP x, int pre, int deep, int pvec, void (*inspect_subtree)(SEXP, int, int, int))
{
...
{
    R_altrep_class_t cls = R_make_altinteger_class("mmap_integer", MMAPPKG, dll);
...
{
    R_altrep_class_t cls = R_make_altreal_class("mmap_real", MMAPPKG, dll);
    mmap_real_class = cls;
...
static SEXP mmap_file(SEXP file, int type, Rboolean ptrOK, Rboolean wrtOK, Rboolean serOK, Rboolean warn)
{
    error("mmop objects not supported on Windows yet");
}
#else
...
SEXP attribute_hidden do_mmap_file(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (stype != R_NilValue)
    {
        ...
        if (strcmp(typestr, "double") == 0)
            type = REALSXP;
        else if (strcmp(typestr, "integer") == 0 || strcmp(typestr, "int") == 0)
            type = INTSXP;
        else
...
SEXP attribute_hidden do_munmap_file(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!(R_altrep_inherits(x, mmap_integer_class) || R_altrep_inherits(x, mmap_real_class)))
...
Rboolean wrapper_Inspect(SEXP x, int pre, int deep, int pvec, void (*inspect_subtree)(SEXP, int, int, int))
{
...
{
    R_altrep_class_t cls = R_make_altinteger_class("wrap_integer", WRAPPKG, dll);
...
{
    R_altrep_class_t cls = R_make_altreal_class("wrap_real", WRAPPKG, dll);
    wrap_real_class = cls;
