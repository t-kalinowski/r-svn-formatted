    ...
    else
    {
        ...
        for (i = 0; i < LENGTH(class); i++)
            for (j = 0; j < length(classes); j++)
                if (strcmp(CHAR(STRING_ELT(class, i)), CHAR(STRING_ELT(classes, j))) == 0)
                    matched = TRUE;
        UNPROTECT(1);
    }
    if (matched)
...
SEXP attribute_hidden do_rapply(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    args = CDR(args);
    if (!isFunction(FUN))
        errorcall(call, _("invalid 'f' argument"));
    classes = CAR(args);
    args = CDR(args);
    if (!isString(classes))
        errorcall(call, _("invalid 'classes' argument"));
    deflt = CAR(args);
    args = CDR(args);
    how = CAR(args);
    if (!isString(how))
        errorcall(call, _("invalid 'how' argument"));
