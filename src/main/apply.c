...
SEXP attribute_hidden do_lapply(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    SEXP tmp = PROTECT(LCONS(R_Bracket2Symbol, LCONS(X, LCONS(isym, R_NilValue))));
    SEXP R_fcall = PROTECT(LCONS(FUN, LCONS(tmp, LCONS(R_DotsSymbol, R_NilValue))));
    ...
    */
    {
        ...
        /* Notice that it is OK to have one arg to LCONS do memory
           ...
           protection of its args internally), but not both of them,
           since the computation of one may destroy the other */
        PROTECT(tmp = LCONS(R_Bracket2Symbol, LCONS(X, LCONS(isym, R_NilValue))));
...
static SEXP do_one(SEXP X, SEXP FUN, SEXP classes, SEXP deflt, Rboolean replace, SEXP rho)
{
...
SEXP attribute_hidden do_islistfactor(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    { // recursive:  isVectorList(X) <==> X is VECSXP or EXPRSXP
