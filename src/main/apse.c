...
#define APSE_DEBUG_SINGLE(ap, i)                                                                                       \
    APSE_DEBUG(printf("%c %2ld %2ld %s\n", isprint(ap->text[ap->text_position]) ? ap->text[ap->text_position] : '.',   \
                      ap->text_position, i, _apse_fbin(ap->state[i], ap->pattern_size, 1)))

#define APSE_DEBUG_MULTIPLE_FIRST(ap, i)                                                                               \
    APSE_DEBUG(printf("%c %2ld %2ld", isprint(ap->text[ap->text_position]) ? ap->text[ap->text_position] : '.',        \
                      ap->text_position, i))

#define APSE_DEBUG_MULTIPLE_REST(ap, i, j)                                                                             \
    APSE_DEBUG(printf(" %s", _apse_fbin(ap->state[j], ap->pattern_size, i == ap->bitvectors_in_state - 1)))
...
static char *_apse_fbin(apse_vec_t v, apse_size_t n, apse_bool_t last)
{
    ...
    if (v)
    {
        static const char *b = "0000100001001100001010100110111000011001010111010011101101111111";
        apse_size_t i;
...
apse_bool_t apse_set_pattern(apse_t *ap, unsigned char *pattern, apse_size_t pattern_size)
{
    ...
    for (i = 0; i < pattern_size; i++)
        APSE_BIT_SET(ap->case_mask, (unsigned)pattern[i], ap->bitvectors_in_state, i);
                                      ...
                                      apse_bool_t caseignore)
{
    ...
    if (!ap->fold_mask)
    {
        ...
        ap->fold_mask = calloc((apse_size_t)APSE_CHAR_MAX, ap->bytes_in_state);
        if (!ap->fold_mask)
            goto out;

        memcpy(ap->fold_mask, ap->case_mask, APSE_CHAR_MAX * ap->bytes_in_state);
...
apse_t *apse_create(unsigned char *pattern, apse_size_t pattern_size, apse_size_t edit_distance)
{
    ...
    APSE_DEBUG(printf("(pattern = \"%s\", pattern_size = %ld)\n", pattern, pattern_size));
    ...
    printf("(bytes_in_state = %ld, states = %ld, bytes_in_all_states = %ld)\n", ap->bytes_in_state,
           ap->edit_distance + 1, ap->bytes_in_all_states);
...
apse_bool_t apse_set_insertions(apse_t *ap, apse_size_t insertions)
{
    ...
    APSE_DEBUG((printf("(edit distances: insertions = %ld, deletions = %ld, substitutions = %ld)\n",
                       ap->edit_insertions, ap->edit_deletions, ap->edit_substitutions)));
...
static const char *apse_match_state_name(apse_t *ap)
{
    ...
    {
    case APSE_MATCH_STATE_BOT:
        return "BOT";
    case APSE_MATCH_STATE_SEARCH:
        return "SEARCH";
    case APSE_MATCH_STATE_BEGIN:
        return "BEGIN";
    case APSE_MATCH_STATE_FAIL:
        return "FAIL";
    case APSE_MATCH_STATE_GREEDY:
        return "GREEDY";
    case APSE_MATCH_STATE_END:
        return "END";
    case APSE_MATCH_STATE_EOT:
        return "EOT";
    default:
        return "***UNKNOWN***";
    }
...
#ifdef APSE_DEBUGGING
    printf("(match end %ld)\n", ap->match_end);
    printf("(match string \"%.*s\")\n", (int)(ap->match_end - ap->match_begin + 1), ap->text + ap->match_begin);
    ...
    case APSE_MATCH_STATE_BEGIN: {
        ...
        if (equal == ap->edit_distance + 1 && ap->is_greedy == 0 ||
            equal < ap->prev_equal && ap->prev_active && active > ap->prev_active &&
                !APSE_BIT_TST(ap->state, ap->edit_distance, ap->bitvectors_in_state,
    ...
    if (match & ap->match_end_bitmask)
    {
        if (ap->match_state == APSE_MATCH_STATE_BEGIN)
        {
            if (ap->is_greedy)
            {
                ...
                APSE_DEBUG(printf("(match state %s)\n", apse_match_state_name(ap)));
                APSE_DEBUG(printf("(greedy match continue %ld)\n", ap->text_position));
            }
            else
            {
                ap->match_state = APSE_MATCH_STATE_END;
...
static apse_bool_t _apse_match_single_simple(apse_t *ap)
{
    ...
    {
        apse_vec_t t = ap->pattern_mask[(unsigned)ap->text[ap->text_position] * ap->bitvectors_in_state];
        apse_size_t h, g;
...
static apse_bool_t _apse_match_single_complex(apse_t *ap)
{
    ...
    for (; ap->text_position < ap->text_size; ap->text_position++)
    {
        ...
        apse_vec_t t = ap->pattern_mask[(unsigned int)o * ap->bitvectors_in_state];
        apse_size_t h, g;
...
static apse_bool_t _apse_match_multiple_complex(apse_t *ap)
{
    ...
    {
        unsigned char o = ap->text[ap->text_position];
        apse_vec_t *t = ap->pattern_mask + (unsigned)o * ap->bitvectors_in_state;
        apse_vec_t c, d;
