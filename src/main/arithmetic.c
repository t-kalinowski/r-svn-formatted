...
SEXP attribute_hidden do_arith(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    else if (argc == 2)
    {
        ...
        if (IS_SCALAR(arg1, REALSXP))
        {
            ...
            {
                double x1 = REAL(arg1)[0];
                double x2 = INTEGER(arg2)[0] != NA_INTEGER ? (double)INTEGER(arg2)[0] : NA_REAL;
        ...
        else if (IS_SCALAR(arg1, INTSXP))
        {
            if (IS_SCALAR(arg2, REALSXP))
            {
                double x1 = INTEGER(arg1)[0] != NA_INTEGER ? (double)INTEGER(arg1)[0] : NA_REAL;
                double x2 = REAL(arg2)[0];
    ...
    else if (argc == 1)
    {
        ...
        else if (IS_SCALAR(arg1, INTSXP))
        {
            ...
            case MINUSOP:
                ans = ScalarValue1(arg1);
                INTEGER(ans)[0] = INTEGER(arg1)[0] == NA_INTEGER ? NA_INTEGER : -INTEGER(arg1)[0];
                return ans;
            }
        }
    }
...
#define FIXUP_NULL_AND_CHECK_TYPES(v, vpi)                                                                             \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
        case NILSXP:                                                                                                   \
            REPROTECT(v = allocVector(REALSXP, 0), vpi);                                                               \
            break;                                                                                                     \
        case CPLXSXP:                                                                                                  \
        case REALSXP:                                                                                                  \
        case INTSXP:                                                                                                   \
...
SEXP attribute_hidden R_binary(SEXP call, SEXP op, SEXP x, SEXP y)
{
    ...
    */
    ...
    if (dims != R_NilValue)
    {
        if (!((xarray && (nx == 0) && (ny > 1)) || (yarray && (ny == 0) && (nx > 1))))
        {
...
static SEXP real_binary(ARITHOP_TYPE code, SEXP s1, SEXP s2)
{
    ...
    case PLUSOP:
        if (TYPEOF(s1) == REALSXP && TYPEOF(s2) == REALSXP)
        {
            ...
            else if (n1 == 1)
            {
                ...
                MOD_ITERATE_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, da[i] = dx[i1] + dy[i2];);
        }
        else if (TYPEOF(s1) == INTSXP)
            MOD_ITERATE_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, REAL(ans)[i] = R_INTEGER(s1, i1) + REAL(s2)[i2];);
        else if (TYPEOF(s2) == INTSXP)
            MOD_ITERATE_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, REAL(ans)[i] = REAL(s1)[i1] + R_INTEGER(s2, i2););
        break;
    case MINUSOP:
        if (TYPEOF(s1) == REALSXP && TYPEOF(s2) == REALSXP)
        {
    ...
    case POWOP:
        if (TYPEOF(s1) == REALSXP && TYPEOF(s2) == REALSXP)
        {
            ...
            else if (n1 == 1)
            {
                ...
                MOD_ITERATE_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, da[i] = R_POW(dx[i1], dy[i2]););
        }
        else if (TYPEOF(s1) == INTSXP)
            MOD_ITERATE_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, REAL(ans)[i] = R_POW(R_INTEGER(s1, i1), REAL(s2)[i2]););
        else if (TYPEOF(s2) == INTSXP)
            MOD_ITERATE_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, REAL(ans)[i] = R_POW(REAL(s1)[i1], R_INTEGER(s2, i2)););
        break;
    case MODOP:
        if (TYPEOF(s1) == REALSXP && TYPEOF(s2) == REALSXP)
            MOD_ITERATE_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, REAL(ans)[i] = myfmod(REAL(s1)[i1], REAL(s2)[i2]););
        else if (TYPEOF(s1) == INTSXP)
            MOD_ITERATE_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2,
    ...
    case IDIVOP:
        if (TYPEOF(s1) == REALSXP && TYPEOF(s2) == REALSXP)
            MOD_ITERATE_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, REAL(ans)[i] = myfloor(REAL(s1)[i1], REAL(s2)[i2]););
        else if (TYPEOF(s1) == INTSXP)
            MOD_ITERATE_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2,
...
SEXP attribute_hidden do_math2(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    default:
        errorcall(call, _("unimplemented real function of %d numeric arguments"), 2);
    }
...
SEXP attribute_hidden do_math3(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    default:
        errorcall(call, _("unimplemented real function of %d numeric arguments"), 3);
    }
    return op; /* never used; to keep -Wall happy */
} /* do_math3() */
...
SEXP attribute_hidden do_math4(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    default:
        errorcall(call, _("unimplemented real function of %d numeric arguments"), 4);
    }
    return op; /* never used; to keep -Wall happy */
}
...
#endif
    default:
        errorcall(call, _("unimplemented real function of %d numeric arguments"), 5);
    }
    return op; /* never used; to keep -Wall happy */
} /* do_math5() */
