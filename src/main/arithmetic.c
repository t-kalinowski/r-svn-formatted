...
static SEXP real_binary(ARITHOP_TYPE code, SEXP s1, SEXP s2)
{
    ...
    case PLUSOP:
        if (TYPEOF(s1) == REALSXP && TYPEOF(s2) == REALSXP)
        {
            ...
            MOD_ITERATE2_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, REAL(ans)[i] = R_INTEGER(s1, i1) + REAL(s2)[i2];);
        else if (TYPEOF(s2) == INTSXP)
            MOD_ITERATE2_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, REAL(ans)[i] = REAL(s1)[i1] + R_INTEGER(s2, i2););
        break;
    case MINUSOP:
        if (TYPEOF(s1) == REALSXP && TYPEOF(s2) == REALSXP)
        {
    ...
    case MODOP:
        if (TYPEOF(s1) == REALSXP && TYPEOF(s2) == REALSXP)
            MOD_ITERATE2_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, REAL(ans)[i] = myfmod(REAL(s1)[i1], REAL(s2)[i2]););
        else if (TYPEOF(s1) == INTSXP)
            MOD_ITERATE2_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2,
    ...
    case IDIVOP:
        if (TYPEOF(s1) == REALSXP && TYPEOF(s2) == REALSXP)
            MOD_ITERATE2_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2, REAL(ans)[i] = myfloor(REAL(s1)[i1], REAL(s2)[i2]););
        else if (TYPEOF(s1) == INTSXP)
            MOD_ITERATE2_CHECK(NINTERRUPT, n, n1, n2, i, i1, i2,
