...
SEXP attribute_hidden do_matrix(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    snc = CAR(args);
    args = CDR(args);
    byrow = asLogical(CAR(args));
    args = CDR(args);
    if (byrow == NA_INTEGER)
        error(_("invalid '%s' argument"), "byrow");
    dimnames = CAR(args);
    args = CDR(args);
    miss_nr = asLogical(CAR(args));
    args = CDR(args);
    ...
    else if (miss_nr)
    {
        if (lendat > (double)nc * INT_MAX)
            error("data is too long"); // incl lendat > nc == 0
    ...
    else if (miss_nc)
    {
        if (lendat > (double)nr * INT_MAX)
            error("data is too long"); // incl lendat > nr == 0
    ...
    if (lendat > 0)
    {
        ...
        { // ==> nrc > 0
            if (((lendat > nr) && (lendat / nr) * nr != lendat) || ((lendat < nr) && (nr / lendat) * lendat != nr))
                ...
                warning(_("data length [%d] is not a sub-multiple or multiple of the number of columns [%d]"), lendat,
                        nc);
            else if (nrc != lendat)
...
SEXP DropDims(SEXP x)
{
    ...
    SEXP dimnames = PROTECT(getAttrib(x, R_DimNamesSymbol)), newnames = R_NilValue;
    if (n <= 1)
    {
...
SEXP attribute_hidden do_lengths(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    { // atomic: every element has length 1
        for (i = 0, ans_elt = INTEGER(ans); i < x_len; i++, ans_elt++)
            *ans_elt = 1;
    }
...
SEXP attribute_hidden do_rowscols(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (LENGTH(dim) != 2)
        error(_("a matrix-like object is required as argument to '%s'"), (PRIMVAL(op) == 2) ? "col" : "row");
...
static void internal_matprod(double *x, int nrx, int ncx, double *y, int nry, int ncy, double *z)
{
    LDOUBLE sum;
#define MATPROD_BODY                                                                                                   \
...
static void simple_matprod(double *x, int nrx, int ncx, double *y, int nry, int ncy, double *z)
{
    double sum;
    MATPROD_BODY;
}

static void internal_crossprod(double *x, int nrx, int ncx, double *y, int nry, int ncy, double *z)
{
    LDOUBLE sum;
...
static void simple_crossprod(double *x, int nrx, int ncx, double *y, int nry, int ncy, double *z)
{
    double sum;
    CROSSPROD_BODY;
}

static void internal_tcrossprod(double *x, int nrx, int ncx, double *y, int nry, int ncy, double *z)
{
    LDOUBLE sum;
...
static void simple_tcrossprod(double *x, int nrx, int ncx, double *y, int nry, int ncy, double *z)
{
    double sum;
    TCROSSPROD_BODY;
}
...
static void matprod(double *x, int nrx, int ncx, double *y, int nry, int ncy, double *z)
{
    ...
    case MATPROD_DEFAULT_SIMD:
        if (mayHaveNaNOrInf_simd(x, NRX * ncx) || mayHaveNaNOrInf_simd(y, NRY * ncy))
        {
            simple_matprod(x, nrx, ncx, y, nry, ncy, z);
            return;
        }
    ...
    if (ncy == 1) /* matrix-vector or dot product */
        F77_CALL(dgemv)(transN, &nrx, &ncx, &one, x, &nrx, y, &ione, &zero, z, &ione FCONE);
    else if (nrx == 1) /* vector-matrix */
        /* Instead of xY, compute (xY)^T == (Y^T)(x^T)
           The result is a vector, so transposing its content is no-op */
        F77_CALL(dgemv)(transT, &nry, &ncy, &one, y, &nry, x, &ione, &zero, z, &ione FCONE);
    else /* matrix-matrix or outer product */
        F77_CALL(dgemm)(transN, transN, &nrx, &ncy, &ncx, &one, x, &nrx, y, &nry, &zero, z, &nrx FCONE FCONE);
}

static void internal_cmatprod(Rcomplex *x, int nrx, int ncx, Rcomplex *y, int nry, int ncy, Rcomplex *z)
{
    LDOUBLE sum_i, sum_r;
...
static void simple_cmatprod(Rcomplex *x, int nrx, int ncx, Rcomplex *y, int nry, int ncy, Rcomplex *z)
{
    double sum_i, sum_r;
    CMATPROD_BODY;
}

static void internal_ccrossprod(Rcomplex *x, int nrx, int ncx, Rcomplex *y, int nry, int ncy, Rcomplex *z)
{
    LDOUBLE sum_i, sum_r;
...
static void simple_ccrossprod(Rcomplex *x, int nrx, int ncx, Rcomplex *y, int nry, int ncy, Rcomplex *z)
{
    double sum_i, sum_r;
    CCROSSPROD_BODY;
}

static void internal_tccrossprod(Rcomplex *x, int nrx, int ncx, Rcomplex *y, int nry, int ncy, Rcomplex *z)
{
    LDOUBLE sum_i, sum_r;
...
static void simple_tccrossprod(Rcomplex *x, int nrx, int ncx, Rcomplex *y, int nry, int ncy, Rcomplex *z)
{
    double sum_i, sum_r;
    TCCROSSPROD_BODY;
}
...
static void cmatprod(Rcomplex *x, int nrx, int ncx, Rcomplex *y, int nry, int ncy, Rcomplex *z)
{
    ...
    case MATPROD_BLAS:
        break;
    case MATPROD_DEFAULT_SIMD:
        if (cmayHaveNaNOrInf_simd(x, NRX * ncx) || cmayHaveNaNOrInf_simd(y, NRY * ncy))
        {
            simple_cmatprod(x, nrx, ncx, y, nry, ncy, z);
            return;
        }
    ...
    F77_CALL(zgemm)(transa, transb, &nrx, &ncy, &ncx, &one, x, &nrx, y, &nry, &zero, z, &nrx FCONE FCONE);
#endif
}
...
static void symcrossprod(double *x, int nr, int nc, double *z)
{
    ...
    F77_CALL(dsyrk)(uplo, trans, &nc, &nr, &one, x, &nr, &zero, z, &nc FCONE FCONE);
    for (int i = 1; i < nc; i++)
        for (int j = 0; j < i; j++)
            z[i + NC * j] = z[j + NC * i];
}
...
static void crossprod(double *x, int nrx, int ncx, double *y, int nry, int ncy, double *z)
{
    ...
    if (ncy == 1) /* matrix-vector or dot product */
        F77_CALL(dgemv)(transT, &nrx, &ncx, &one, x, &nrx, y, &ione, &zero, z, &ione FCONE);
    else if (ncx == 1) /* vector-matrix */
        /* Instead of (x^T)Y, compute ((x^T)Y)^T == (Y^T)x
           The result is a vector, so transposing its content is no-op */
        F77_CALL(dgemv)(transT, &nry, &ncy, &one, y, &nry, x, &ione, &zero, z, &ione FCONE);
    else /* matrix-matrix  or outer product */
        F77_CALL(dgemm)(transT, transN, &ncx, &ncy, &nrx, &one, x, &nrx, y, &nry, &zero, z, &ncx FCONE FCONE);
}
...
static void ccrossprod(Rcomplex *x, int nrx, int ncx, Rcomplex *y, int nry, int ncy, Rcomplex *z)
{
    ...
    F77_CALL(zgemm)(transa, transb, &ncx, &ncy, &nrx, &one, x, &nrx, y, &nry, &zero, z, &ncx FCONE FCONE);
#endif
}
...
static void symtcrossprod(double *x, int nr, int nc, double *z)
{
    ...
    F77_CALL(dsyrk)(uplo, trans, &nr, &nc, &one, x, &nr, &zero, z, &nr FCONE FCONE);
    for (int i = 1; i < nr; i++)
        for (int j = 0; j < i; j++)
            z[i + nr * j] = z[j + nr * i];
}

static void tcrossprod(double *x, int nrx, int ncx, double *y, int nry, int ncy, double *z)
{
    R_xlen_t NRX = nrx, NRY = nry;
    if (nrx == 0 || ncx == 0 || nry == 0 || ncy == 0)
    {
        /* zero-extent operations should return zeroes */
        for (R_xlen_t i = 0; i < NRX * nry; i++)
            z[i] = 0;
        return;
    }
    ...
    if (nry == 1) /* matrix-vector or dot product */
        ...
        /* Instead of x(Y^T), compute (x(Y^T))^T == Y(x^T)
           The result is a vector, so transposing its content is no-op */
        F77_CALL(dgemv)(transN, &nry, &ncy, &one, y, &nry, x, &ione, &zero, z, &ione FCONE);
    else /* matrix-matrix or outer product */
        F77_CALL(dgemm)(transN, transT, &nrx, &nry, &ncx, &one, x, &nrx, y, &nry, &zero, z, &nrx FCONE FCONE);
}

static void tccrossprod(Rcomplex *x, int nrx, int ncx, Rcomplex *y, int nry, int ncy, Rcomplex *z)
{
    R_xlen_t NRX = nrx, NRY = nry;
    if (nrx == 0 || ncx == 0 || nry == 0 || ncy == 0)
    {
    ...
    F77_CALL(zgemm)(transa, transb, &nrx, &nry, &ncx, &one, x, &nrx, y, &nry, &zero, z, &nrx FCONE FCONE);
#endif
}
...
#define YDIMS_ET_CETERA                                                                                                \
    ...
    { /* op == 1: crossprod() */
        ...
        if (xdims != R_NilValue || ydims != R_NilValue)
        {
            ...
            {
                if (ldx == 2)
                { /* not nrx==1 : .. fixed, ihaka 2003-09-30 */
    ...
    { /* op == 2: tcrossprod() */
        ...
        if (mode == CPLXSXP)
            if (sym)
                tccrossprod(COMPLEX(CAR(args)), nrx, ncx, COMPLEX(CAR(args)), nry, ncy, COMPLEX(ans));
            else
                tccrossprod(COMPLEX(CAR(args)), nrx, ncx, COMPLEX(CADR(args)), nry, ncy, COMPLEX(ans));
        else
        {
            if (sym)
                symtcrossprod(REAL(CAR(args)), nrx, ncx, REAL(ans));
            else
                tcrossprod(REAL(CAR(args)), nrx, ncx, REAL(CADR(args)), nry, ncy, REAL(ans));
        }
...
{
    SEXP a, r, dims, dimnames, dimnamesnames = R_NilValue, ndimnamesnames, rnames, cnames;
    ...
    if (!isNull(dimnames))
    {
        ...
        if (!isNull(dimnamesnames))
        {
            ...
            SET_VECTOR_ELT(ndimnamesnames, 0, (ldim == 2) ? STRING_ELT(dimnamesnames, 1) : R_BlankString);
...
SEXP attribute_hidden do_aperm(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    {
        if (LENGTH(perm) != n)
            error(_("'perm' is of wrong length %d (!= %d)"), LENGTH(perm), n);
        if (isString(perm))
        {
            ...
            for (i = 0; i < n; i++)
            {
                ...
                for (j = 0; j < n; j++)
                    if (streql(translateChar(STRING_ELT(dnna, j)), this))
                    {
                        pp[i] = j;
                        break;
                    }
    ...
    if (resize)
    {
        ...
        if (nmdm != R_NilValue)
        { // dimsr needs correctly permuted names()
            PROTECT(nmdm);
...
SEXP attribute_hidden do_array(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    case LGLSXP:
        if (nans && lendat)
            xcopyLogicalWithRecycle(LOGICAL(ans), LOGICAL(vals), 0, nans, lendat);
        else
            for (i = 0; i < nans; i++)
                LOGICAL(ans)[i] = NA_LOGICAL;
        break;
    case INTSXP:
        if (nans && lendat)
            xcopyIntegerWithRecycle(INTEGER(ans), INTEGER(vals), 0, nans, lendat);
        else
            for (i = 0; i < nans; i++)
                INTEGER(ans)[i] = NA_INTEGER;
        break;
    ...
    case CPLXSXP:
        if (nans && lendat)
            xcopyComplexWithRecycle(COMPLEX(ans), COMPLEX(vals), 0, nans, lendat);
        else
        {
            Rcomplex na_cmplx;
            na_cmplx.r = NA_REAL;
...
#define mk_DIAG(_zero_)                                                                                                \
    for (R_xlen_t i = 0; i < NR * nc; i++)                                                                             \
        ra[i] = _zero_;                                                                                                \
    R_xlen_t i, i1;                                                                                                    \
    MOD_ITERATE1(mn, nx, i, i1, { ra[i * (NR + 1)] = rx[i1]; });
...
SEXP attribute_hidden do_backsolve(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    */
    ...
    if (k > 0 && ncb > 0)
    {
        ...
        double one = 1.0;
        F77_CALL(dtrsm)
        ("L", upper ? "U" : "L", trans ? "T" : "N", "N", &k, &ncb, &one, rr, &nrr, REAL(ans),
         &k FCONE FCONE FCONE FCONE);
    }
    UNPROTECT(nprot);
    return ans;
}
