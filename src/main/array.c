    ...
    if (lendat > 0)
    {
        if (lendat > 1 && (nr * nc) % lendat != 0)
        {
            if (((lendat > nr) && (lendat / nr) * nr != lendat) || ((lendat < nr) && (nr / lendat) * lendat != nr))
                warning("data length [%d] is not a sub-multiple or multiple of the number of rows [%d] in matrix",
                        lendat, nr);
            else if (((lendat > nc) && (lendat / nc) * nc != lendat) || ((lendat < nc) && (nc / lendat) * lendat != nc))
                warning("data length [%d] is not a sub-multiple or multiple of the number of columns [%d] in matrix",
                        lendat, nc);
        }
...
SEXP do_colsum(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (NaRm == NA_LOGICAL)
        errorcall(call, "invalid value of na.rm");
    keepNA = !NaRm;
