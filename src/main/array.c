...
SEXP attribute_hidden do_length(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (PRIMVAL(op))
    { /* xlength */
        if (isObject(CAR(args)) && DispatchOrEval(call, op, "xlength", args, rho, &ans, 0, 1))
            return (ans);
...
SEXP attribute_hidden do_colsum(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    { /* columns */
        ...
        if (R_num_math_threads > 0)
            ...
            nthreads = 1; /* for now */
#pragma omp parallel for num_threads(nthreads) default(none) private(j, i, ix, rx)                                     \
    firstprivate(x, ans, n, p, type, cnt, sum, NaRm, keepNA, R_NaReal, R_NaInt, OP)
#endif
        for (j = 0; j < p; j++)
        {
            switch (type)
