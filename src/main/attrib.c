...
SEXP dimnamesgets(SEXP vec, SEXP val)
{
    ...
    for (i = 0; i < k; i++)
    {
        ...
        {
            if (!isVector(this))
                error(_("invalid type (%s) for 'dimnames' (must be a vector)"), type2char(TYPEOF(this)));
...
SEXP attribute_hidden do_AT(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (can_test_S4Object && !R_seemsS4Object(object))
    {
        ...
        {
            if (isString(class) && install(CHAR(STRING_ELT(class, 0))) == install("classRepresentation"))
            {
