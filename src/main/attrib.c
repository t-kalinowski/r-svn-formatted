...
SEXP dimnamesgets(SEXP vec, SEXP val)
{
    ...
    if ((k = LENGTH(dims)) != length(val))
        error("length of dimnames [%d] must match that of dims [%d]", length(val), k);
    ...
    if (value == R_NilValue)
    {
        ...
        if (isSymbol(name))
        {
            ...
            if (isNull(classString))
                error("Can't get a slot (\"%s\") from an object of type \"%s\"", CHAR(asChar(input)),
...
SEXP do_AT(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (length(class) == 1)
    {
        ...
        *  should eventually be able to grab class definition pointer
        ...
        if (!quick && (findVar(class_name, env) == R_UnboundValue))
        ...
        error("Trying to get slot \"%s\" from an object with S3 class c(\"%s\", \"%s\", ...) (not a formally defined "
              "class)",
              CHAR(PRINTNAME(nlist)), CHAR(STRING_ELT(class, 0)), CHAR(STRING_ELT(class, 1)));
    ans = R_do_slot(object, nlist);
    UNPROTECT(1);
    return ans;
}
