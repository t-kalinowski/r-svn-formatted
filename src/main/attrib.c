...
SEXP getAttrib(SEXP vec, SEXP name)
{
    ...
    if (ATTRIB(vec) == R_NilValue && !(TYPEOF(vec) == LISTSXP || TYPEOF(vec) == LANGSXP || TYPEOF(vec) == DOTSXP))
        return R_NilValue;
...
attribute_hidden SEXP do_shortRowNames(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (type >= 1)
    {
        int n = (isInteger(s) && LENGTH(s) == 2 && INTEGER(s)[0] == NA_INTEGER) ? INTEGER(s)[1]
                                                                                : (isNull(s) ? 0 : LENGTH(s));
...
void copyMostAttribNoTs(SEXP inp, SEXP ans)
{
    ...
    for (s = ATTRIB(inp); s != R_NilValue; s = CDR(s))
    {
        if ((TAG(s) != R_NamesSymbol) && (TAG(s) != R_ClassSymbol) && (TAG(s) != R_TspSymbol) &&
            (TAG(s) != R_DimSymbol) && (TAG(s) != R_DimNamesSymbol))
        {
            installAttrib(ans, TAG(s), CAR(s));
        }
...
attribute_hidden SEXP tspgets(SEXP vec, SEXP val)
{
    ...
    if (IS_S4_OBJECT(vec))
    {                        /* leave validity checking to validObject */
...
SEXP attribute_hidden do_classgets(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (MAYBE_SHARED(CAR(args)) || ((!IS_ASSIGNMENT_CALL(call)) && MAYBE_REFERENCED(CAR(args))))
...
{
    SEXP symb = CAR(obj);
    static SEXP if_sym = 0, while_sym, for_sym, eq_sym, gets_sym, lpar_sym, lbrace_sym, call_sym;
    if (!if_sym)
    {
    ...
    if (isSymbol(symb))
    {
        if (symb == if_sym || symb == for_sym || symb == while_sym || symb == lpar_sym || symb == lbrace_sym ||
...
attribute_hidden void InitS3DefaultTypes(void)
{
    for (int type = 0; type < MAX_NUM_SEXPTYPE; type++)
    {
        ...
        Type2DefaultClass[type].vector = createDefaultClass(R_NilValue, R_NilValue, part3, part4);
...
SEXP attribute_hidden R_data_class2(SEXP obj)
{
    ...
    { // length(klass) == 0 , i.e., no class *attribute*: attr(obj, "class") is NULL
        ...
        {
        case 0:
            defaultClass = Type2DefaultClass[t].vector;
            break;
        case 2:
            defaultClass = Type2DefaultClass[t].matrix;
            break;
        default:
            defaultClass = Type2DefaultClass[t].array;
            break;
        }
...
SEXP attribute_hidden R_do_data_class(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
    if (PRIMVAL(op) == 1)
    { // .cache_class() - typically re-defining existing cache
...
SEXP attribute_hidden do_namesgets(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (CADR(args) == R_NilValue && getAttrib(CAR(args), R_NamesSymbol) == R_NilValue)
        return CAR(args);
    PROTECT(args = ans);
    if (MAYBE_SHARED(CAR(args)) || ((!IS_ASSIGNMENT_CALL(call)) && MAYBE_REFERENCED(CAR(args))))
    ...
    if (TYPEOF(CAR(args)) == S4SXP)
    {
        ...
        error(_("invalid to use names()<- on an S4 object of class '%s'"), klass);
    }
    SEXP names = CADR(args);
    if (names != R_NilValue && !(TYPEOF(names) == STRSXP && ATTRIB(names) == R_NilValue))
    {
...
SEXP attribute_hidden do_dimgets(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (CADR(args) == R_NilValue)
    {
        ...
        for (s = ATTRIB(x); s != R_NilValue; s = CDR(s))
            if (TAG(s) == R_DimSymbol || TAG(s) == R_NamesSymbol)
                break;
        if (s == R_NilValue)
            return x;
    }
    PROTECT(args = ans);
    if (MAYBE_SHARED(x) || ((!IS_ASSIGNMENT_CALL(call)) && MAYBE_REFERENCED(x)))
...
SEXP attribute_hidden do_levelsgets(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!isNull(CADR(args)) && any_duplicated(CADR(args), FALSE))
        errorcall(call, _("factor level [%d] is duplicated"), any_duplicated(CADR(args), FALSE));
    args = ans;
    if (MAYBE_SHARED(CAR(args)) || ((!IS_ASSIGNMENT_CALL(call)) && MAYBE_REFERENCED(CAR(args))))
        SETCAR(args, duplicate(CAR(args)));
...
SEXP attribute_hidden do_attributesgets(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (nattrs > 0)
    {
        ...
        for (i = 0; i < nattrs; i++)
        {
            if (i == i0)
                continue;
            setAttrib(object, installTrChar(STRING_ELT(names, i)), VECTOR_ELT(attrs, i));
        }
    }
...
SEXP attribute_hidden do_attr(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (do_attr_formals == NULL)
        do_attr_formals = allocFormalsList3(install("x"), install("which"), R_ExactSymbol);
...
{
    SEXP valueClass = PROTECT(R_data_class(value, FALSE)), objClass = PROTECT(R_data_class(obj, FALSE));
...
SEXP attribute_hidden do_attrgets(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    { /* @<- */
        ...
        obj = CAR(ans);
        if (MAYBE_SHARED(obj) || ((!IS_ASSIGNMENT_CALL(call)) && MAYBE_REFERENCED(obj)))
    ...
    { // attr(obj, "name") <- value :
        ...
        if (do_attrgets_formals == NULL)
            do_attrgets_formals = allocFormalsList3(install("x"), install("which"), install("value"));
...
*/
SEXP attribute_hidden R_getS4DataSlot(SEXP obj, SEXPTYPE type)
{
