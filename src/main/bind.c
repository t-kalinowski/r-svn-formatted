...
static void RawAnswer(SEXP x, struct BindData *data, SEXP call)
{
    ...
    case RAWSXP:
        ...
        break;
    default:
        errorcall(call, _("type '%s' is unimplemented in '%s'"), type2char(TYPEOF(x)), "RawAnswer");
    }
}
...
static SEXP NewName(SEXP base, SEXP tag, R_xlen_t seqno, int count)
{
    ...
    if (*CHAR(base))
    {
        ...
        else
        {
            ...
            char *cbuf;
            cbuf = R_AllocStringBuffer(strlen(sb) + (size_t)IndexWidth(seqno), &cbuff);
...
static SEXP c_Extract_opt(SEXP ans, Rboolean *recurse, Rboolean *usenames, SEXP call)
{
...
SEXP attribute_hidden do_c_dflt(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    Rboolean usenames = TRUE, recurse = FALSE;
    ...
    int mode = NILSXP;
    if (data.ans_flags & 512)
        mode = EXPRSXP;
    else if (data.ans_flags & 256)
        mode = VECSXP;
...
SEXP attribute_hidden do_bind(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (force_identical_methods == -1)
    {
        ...
        force_identical_methods = ((force != NULL) && StringTrue(force)) ? 1 : 0;
    }
    ...
    for (a = CDR(args); a != R_NilValue; a = CDR(a))
    {
        ...
        if (compatible && isObject(obj))
        {
            ...
            for (int i = 0; i < length(classlist); i++)
            {
                ...
                SEXP classmethod = R_LookupMethod(install(buf), env, env, R_BaseNamespace);
...
static SEXP cbind(SEXP call, SEXP args, SEXPTYPE mode, SEXP rho, int deparse_level)
{
    ...
    for (t = args; t != R_NilValue; t = CDR(t), na++)
    {
        ...
        if (length(dims) == 2)
        {
            if (mrows == -1)
                ...
                error(_("number of rows of matrices must match (see arg %d)"), na + 1);
            cols += INTEGER(dims)[1];
        }
    ...
    { /* everything else, currently REALSXP, INTSXP, LGLSXP */
        for (t = args; t != R_NilValue; t = CDR(t))
        {
            ...
            if (isMatrix(u) || length(u) >= lenmin)
            {
                ...
                { /* RAWSXP */
                    ...
                    {
                        R_xlen_t i, i1;
                        MOD_ITERATE1(idx, k, i, i1, { REAL(result)[n++] = (unsigned char)RAW(u)[i1]; });
                    }
                    else
                        /* not sure this can be reached, but to be safe: */
                        error(_("cannot create a matrix of type '%s'"), type2char(mode));
                }
            }
        }
    }
    ...
    for (t = args; t != R_NilValue; t = CDR(t), na++)
    {
        ...
        if (length(dims) == 2)
        {
            if (mcols == -1)
                ...
                error(_("number of columns of matrices must match (see arg %d)"), na + 1);
            rows += INTEGER(dims)[0];
        }
        else if (length(u) >= lenmin)
        {
