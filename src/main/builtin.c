...
SEXP do_newenv(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    PROTECT(s = NewEnvironment(R_NilValue, R_NilValue, CADR(args)));
...
static void cat_newline(SEXP labels, int *width, int lablen, int ntot)
{
    ...
    *width = 0;
    ...
    {
        Rprintf("%s ", EncodeString(CHAR(STRING_ELT(labels, ntot % lablen)), 1, 0, Rprt_adj_left));
...
SEXP do_makevector(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    case LISTSXP:
        ...
        break;
    default:
        error("vector: cannot make a vector of mode \"%s\".", CHAR(STRING_ELT(s, 0)));
    }
