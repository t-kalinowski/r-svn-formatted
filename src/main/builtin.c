...
SEXP attribute_hidden do_onexit(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (do_onexit_formals == NULL)
        do_onexit_formals = allocFormalsList3(install("expr"), install("add"), install("after"));
    ...
    while (ctxt != R_ToplevelContext && !((ctxt->callflag & CTXT_FUNCTION) && ctxt->cloenv == rho))
...
SEXP attribute_hidden do_args(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (TYPEOF(CAR(args)) == BUILTINSXP || TYPEOF(CAR(args)) == SPECIALSXP)
    {
        ...
        PROTECT_WITH_INDEX(env = findVarInFrame3(R_BaseEnv, install(".ArgsEnv"), TRUE), &xp);
        ...
        UNPROTECT(1); /* s2 */
        REPROTECT(env = findVarInFrame3(R_BaseEnv, install(".GenericArgsEnv"), TRUE), xp);
        if (TYPEOF(env) == PROMSXP)
            REPROTECT(env = eval(env, R_BaseEnv), xp);
        PROTECT(s2 = findVarInFrame3(env, install(nm), TRUE));
        if (s2 != R_UnboundValue)
...
SEXP attribute_hidden do_newenv(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (!isEnvironment(enclos) && !isEnvironment((enclos = simple_as_environment(enclos))))
...
SEXP attribute_hidden do_parentenv(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (!isEnvironment(arg) && !isEnvironment((arg = simple_as_environment(arg))))
...
SEXP attribute_hidden do_parentenvgets(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    else if (!isEnvironment(parent) && !isEnvironment((parent = simple_as_environment(parent))))
...
static void cat_newline(SEXP labels, int *width, int lablen, int ntot)
{
    ...
    *width = 0;
    ...
    {
        Rprintf("%s ", EncodeString(STRING_ELT(labels, ntot % lablen), 1, 0, Rprt_adj_left));
        ...
        if (n > 0)
        {
            ...
            for (i = 0; i < n; i++, ntot++)
            {
                ...
                if (i < (n - 1))
                {
                    ...
                    ntot--; /* we don't print sep after last, so don't advance */
            }
        }
    }
...
SEXP attribute_hidden do_makelist(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    SEXP next;
    for (next = args, n = 0, havenames = FALSE; next != R_NilValue; next = CDR(next))
    {
...
SEXP attribute_hidden do_makevector(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    case LISTSXP:
        if (len > INT_MAX)
            error("too long for a pairlist");
        s = allocList((int)len);
        break;
    default:
        error(_("vector: cannot make a vector of mode '%s'."), translateChar(STRING_ELT(s, 0))); /* should be ASCII */
    }
...
SEXP xlengthgets(SEXP x, R_xlen_t len)
{
    ...
    if (len < 0)
        error(_("invalid value")); // e.g. -999 from asVecSize()
    if (isNull(x) && len > 0)
    ...
    if (xnames != R_NilValue)
        ...
        names = R_NilValue; /*- just for -Wall --- should we do this ? */
    switch (TYPEOF(x))
    {
    case NILSXP:
        break;
...
static SEXP setDflt(SEXP arg, SEXP dflt)
{
    if (dflt)
    {
        ...
        error(_("duplicate 'switch' defaults: '%s' and '%s'"), CHAR(STRING_ELT(dflt1, 0)), CHAR(STRING_ELT(dflt2, 0)));
