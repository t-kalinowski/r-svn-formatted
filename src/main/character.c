        ...
        for (i = 0; i < len; i++)
        {
            ...
            else
            {
                ...
                if (stop > start + vlen - 1)
                    stop = start + vlen - 1;
                substrset(cbuff.data, translateChar(STRING_ELT(value, i % v)), start, stop);
...
SEXP attribute_hidden do_strsplit(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (fixed_opt && perl_opt)
        warningcall(call, _("argument '%s' will be ignored"), "perl = TRUE");
    if (fixed_opt && !extended_opt)
        warningcall(call, _("argument '%s' will be ignored"), "extended = FALSE");
    ...
    if (tlen == 1 && strlen(translateChar(STRING_ELT(tok, 0))) == 0)
        tlen = 0;
...
SEXP attribute_hidden do_makenames(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
    {
...
#endif
        {
            if (_this[0] == '.')
            {
                if (l >= 1 && isdigit(0xff & (int)_this[1]))
                    need_prefix = TRUE;
            }
            else if (!isalpha(0xff & (int)_this[0]))
                need_prefix = TRUE;
        }
        if (need_prefix)
        {
...
SEXP attribute_hidden do_grep(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (fixed_opt && igcase_opt)
        warningcall(call, _("argument '%s' will be ignored"), "ignore.case = TRUE");
    if (fixed_opt && !extended_opt)
        warningcall(call, _("argument '%s' will be ignored"), "extended = FALSE");
...
static char *string_adj(char *target, char *orig, char *repl, regmatch_t *regmatch)
{
    int i, k;
...
static SEXP gregexpr_Regexc(const regex_t *reg, const char *string, int useBytes)
{
    ...
    while (!foundAll)
    {
        if (offset < len && Rregexec(reg, string, 1, regmatch, 0, offset) == 0)
        {
...
SEXP attribute_hidden do_agrep(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    aps = apse_create((unsigned char *)str, (apse_size_t)strlen(str), max_distance_opt);
    if (!aps)
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (!apse_set_caseignore_slice(aps, 0, (apse_ssize_t)strlen(str), (apse_bool_t)igcase_opt))
        {
...
SEXP attribute_hidden do_charToRaw(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (LENGTH(x) > 1)
        warningcall(call,
                    _("argument should be a character vector of length 1\nall but the first element will be ignored"));
...
static int mbrtoint(int *w, const char *s)
{
    ...
    else if (byte < 0xF0)
    {
        ...
        {
            *w = (int)(((byte & 0x0F) << 12) | ((s[1] & 0x3F) << 6) | (s[2] & 0x3F));
            byte = *w;
    ...
    else if (byte < 0xFC)
    {
        ...
        {
            *w = (int)(((byte & 0x03) << 24) | ((s[1] & 0x3F) << 18) | ((s[2] & 0x3F) << 12) | ((s[3] & 0x3F) << 6) |
                       (s[4] & 0x3F));
            byte = *w;
            return 5;
        }
    ...
    else
    {
        ...
        if (((s[1] & 0xC0) == 0x80) && ((s[2] & 0xC0) == 0x80) && ((s[3] & 0xC0) == 0x80) && ((s[4] & 0xC0) == 0x80) &&
            ((s[5] & 0xC0) == 0x80))
        {
            *w = (int)(((byte & 0x01) << 30) | ((s[1] & 0x3F) << 24) | ((s[2] & 0x3F) << 18) | ((s[3] & 0x3F) << 12) |
                       ((s[5] & 0x3F) << 6) | (s[5] & 0x3F));
            byte = *w;
            return 5;
        }
...
#ifdef GLOB_ABORTED
        if (res == GLOB_ABORTED)
            warningcall(call, _("read error on '%s'"), translateChar(STRING_ELT(x, i)));
#endif
#ifdef GLOB_NOSPACE
        if (res == GLOB_NOSPACE)
