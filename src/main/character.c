...
static int substrset(char *buf, int blen, const char *const str, int slen, cetype_t ienc, int sa, int so)
{
    ...
    if (ienc == CE_UTF8)
    {
        ...
        if (in != out)
            memmove(buf + in, buf + out, blen - out + 1);
        adj = in - out;
        memcpy(buf, str, in);
    }
    else if (ienc == CE_LATIN1)
...
SEXP attribute_hidden do_strsplit(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    {
        for (i = 0; i < tlen; i++)
            if (getCharCE(STRING_ELT(tok, 0)) == CE_UTF8)
                use_UTF8 = TRUE;
        for (i = 0; i < len; i++)
            if (getCharCE(STRING_ELT(x, 0)) == CE_UTF8)
                use_UTF8 = TRUE;
    }
...
SEXP attribute_hidden do_grep(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if ((fixed_opt || perl_opt) && !useBytes)
    {
        if (getCharCE(STRING_ELT(pat, 0)) == CE_UTF8)
            use_UTF8 = TRUE;
        for (i = 0; i < n; i++)
            if (getCharCE(STRING_ELT(vec, 0)) == CE_UTF8)
                use_UTF8 = TRUE;
    }
    if (use_UTF8)
    {
...
SEXP attribute_hidden do_gsub(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (fixed_opt)
    { /* we don't have UTF-8 version of regex.c */
        if (getCharCE(STRING_ELT(pat, 0)) == CE_UTF8)
            use_UTF8 = TRUE;
...
SEXP attribute_hidden do_regexpr(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if ((fixed_opt || perl_opt) && !useBytes)
    {
        if (getCharCE(STRING_ELT(pat, 0)) == CE_UTF8)
            use_UTF8 = TRUE;
        for (i = 0; i < n; i++)
            if (getCharCE(STRING_ELT(text, 0)) == CE_UTF8)
                use_UTF8 = TRUE;
    }
    if (use_UTF8)
    {
    ...
    else if ((rc = regcomp(&reg, spat, cflags)))
    {
        char errbuf[1001];
        regerror(rc, &reg, errbuf, 1001);
        warning(_("regcomp error:  '%s'"), errbuf);
