...
int R_nchar(SEXP string, nchar_type type_, Rboolean allowNA, Rboolean keepNA, const char *msg_name)
{
    ...
    case Chars:
        ...
        else if (IS_BYTES(string))
        {
            ...
            {
                if (msg_name)
                    error(_("number of characters is not computable in \"bytes\" encoding, %s"), msg_name);
                else
                    return -2;
            }
            return NA_INTEGER;
    ...
    case Width:
        ...
        else if (IS_BYTES(string))
        {
            ...
            { /* could do width 0 */
                if (msg_name)
                    error(_("width is not computable for %s in \"bytes\" encoding"), msg_name);
                else
                    return -2;
            }
            return NA_INTEGER;
        ...
        else if (mbcslocale)
        {
            ...
            if (nc >= 0)
            {
                ...
                /* working in wchar_t restricts this to the BMP on
                   ...
                   charsets cover. */
                wchar_t *wc = (wchar_t *)R_AllocStringBuffer((nc + 1) * sizeof(wchar_t), &cbuff);
                mbstowcs(wc, xi, nc + 1);
...
attribute_hidden SEXP do_nchar(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (R_xlen_t i = 0; i < len; i++)
    {
        ...
        case -2:
            if (type_ == Chars)
                error(_("number of characters is not computable in \"bytes\" encoding, element %ld"), (long)i + 1);
            else /* type_ == Width */
                error(_("width is not computable in \"bytes\" encoding, element %ld"), (long)i + 1);
        default:
            s_[i] = res;
            break;
        }
...
*/
static void substr(const char *str, int len, int ienc, int sa, int so, R_xlen_t idx, int isascii, const char **rfrom,
...
attribute_hidden SEXP do_substr(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (len > 0)
    {
        SEXP sa = CADR(args), so = CADDR(args);
        int k = LENGTH(sa), l = LENGTH(so);
        if (!isInteger(sa) || !isInteger(so) || k == 0 || l == 0)
            error(_("invalid substring arguments"));

        for (R_xlen_t i = 0; i < len; i++)
        {
            int start = INTEGER(sa)[i % k], stop = INTEGER(so)[i % l];
            ...
            else
            {
                ...
                substr(ss, slen, ienc, start, stop, i, IS_ASCII(el), &rfrom, &rlen, el == lastel);
...
attribute_hidden SEXP do_startsWith(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    R_xlen_t n1 = XLENGTH(x), n2 = XLENGTH(Xfix), n = (n1 > 0 && n2 > 0) ? ((n1 >= n2) ? n1 : n2) : 0;
    if (n == 0)
...
static void substrset(char *buf, const char *const str, cetype_t ienc, int sa, int so, R_xlen_t xidx, R_xlen_t vidx)
{
...
attribute_hidden SEXP do_substrgets(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (len > 0)
    {
        ...
        for (i = 0; i < len; i++)
        {
            ...
            stop = INTEGER(so)[i % l];
            if (el == NA_STRING || v_el == NA_STRING || start == NA_INTEGER || stop == NA_INTEGER)
            {
                SET_STRING_ELT(s, i, NA_STRING);
                continue;
            }
...
#define LC_VOWEL(i) (s[i] == 'a' || s[i] == 'e' || s[i] == 'i' || s[i] == 'o' || s[i] == 'u')
...
donesc : { // remove internal spaces as required
...
static int vowels[] = {0x61,  0x65,  0x69,  0x6f,  0x75,  0xe0,  0xe1,  0x2e,  0xe3,  0xe4,  0xe5,  0xe8,  0xe9,
                       0xea,  0xeb,  0xec,  0xed,  0xee,  0xef,  0xf2,  0xf3,  0xf4,  0xf5,  0xf6,  0xf8,  0xf9,
                       0xfa,  0xfb,  0xfc,  0x101, 0x103, 0x105, 0x113, 0x115, 0x117, 0x118, 0x11b, 0x129, 0x12b,
                       0x12d, 0x12f, 0x131, 0x14d, 0x14f, 0x151, 0x169, 0x16b, 0x16d, 0x16f, 0x171, 0x173};
...
attribute_hidden SEXP do_tolower(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (IS_UTF8(xi) || (!latin1locale && IS_LATIN1(xi)))
            use_UTF8 = TRUE;
    }
            ...
            else
            {
                ...
                if (nc >= 0)
                {
                    if (ienc == CE_UTF8)
                    {
                        ...
                        R_wchar_t *wcr = (R_wchar_t *)R_AllocStringBuffer((nc + 1) * sizeof(R_wchar_t), &cbuff);
...
#define ISORT(_base, _num, _TYPE, _comp)                                                                               \
    {                                                                                                                  \
        ...
        int _i, _j;                                                                                                    \
        for (_i = 1; _i < _num; _i++)                                                                                  \
            for (_j = _i; _j > 0 && (*_comp)(_base + _j - 1, _base + _j) > 0; _j--)                                    \
...
attribute_hidden SEXP do_chartr(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (IS_UTF8(STRING_ELT(old, 0)) || (!latin1locale && IS_LATIN1(STRING_ELT(old, 0))))
        use_WC = TRUE;
    if (IS_UTF8(STRING_ELT(_new, 0)) || (!latin1locale && IS_LATIN1(STRING_ELT(_new, 0))))
        use_WC = TRUE;
    ...
    if (mbcslocale || use_WC == TRUE)
    {
        ...
        else if (use_WC && IS_LATIN1(STRING_ELT(old, 0)))
        {
            ...
            nc = (int)utf8towcs(NULL, s, 0);
            if (nc < 0)
                error(_("invalid UTF-8 string 'old'")); // but must be valid
            wc = (wchar_t *)R_AllocStringBuffer((nc + 1) * sizeof(wchar_t), &cbuff);
            utf8towcs(wc, s, nc + 1);
        }
        else
