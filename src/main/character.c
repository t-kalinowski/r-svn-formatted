...
SEXP do_agrep(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if ((apse_set_deletions(aps, max_deletions_opt) < 0) || (apse_set_insertions(aps, max_insertions_opt) < 0) ||
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (apse_set_caseignore_slice(aps, 1, (apse_ssize_t)strlen(str), (apse_bool_t)igcase_opt) < 0)
            errorcall(call, "invalid agrep specification");
