...
SEXP do_abbrev(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (warn)
        warningcall(call, "abbreviate used with non-ASCII chars");
    UNPROTECT(1);
    return (ans);
}
...
static char *string_adj(char *target, char *orig, char *repl, regmatch_t *regmatch, int nsubexpr)
{
    int i, k;
...
static void wtr_build_spec(const wchar_t *s, struct wtr_spec *trs)
{
    ...
    for (i = 0; i < len - 2;)
    {
        ...
        if (s[i + 1] == L'-')
        {
            new->type = WTR_RANGE;
            if (s[i] > s[i + 2])
                error("decreasing range specification (`%lc-%lc')", s[i], s[i + 2]);
            new->u.r.first = s[i];
            new->u.r.last = s[i + 2];
            i = i + 3;
        }
...
static void tr_build_spec(const char *s, struct tr_spec *trs)
{
    ...
    for (i = 0; i < len - 2;)
    {
        ...
        if (s[i + 1] == '-')
        {
            new->type = TR_RANGE;
            if (s[i] > s[i + 2])
                error("decreasing range specification (`%c-%c')", s[i], s[i + 2]);
            new->u.r.first = s[i];
            new->u.r.last = s[i + 2];
            i = i + 3;
        }
...
SEXP do_chartr(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (utf8locale)
    {
        ...
        if (nc < 0)
            errorcall(call, "invalid multibyte string 'old'");
        AllocBuffer((nc + 1) * sizeof(wchar_t));
        wc = (wchar_t *)cbuff.data;
        ...
        if (nc < 0)
            errorcall(call, "invalid multibyte string 'new'");
        AllocBuffer((nc + 1) * sizeof(wchar_t));
        wc = (wchar_t *)cbuff.data;
