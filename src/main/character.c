...
SEXP do_nchar(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        INTEGER(s)[i] = (ch == NA_STRING) ? R_print.na_width : strlen(CHAR(ch));
    }
...
SEXP do_grep(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (STRING_ELT(vec, i) != NA_STRING && regexec(&reg, CHAR(STRING_ELT(vec, i)), 0, NULL, 0) == 0)
        {
            INTEGER(ind)[i] = 1;
            nmatches++;
        }
...
SEXP do_toupper(SEXP call, SEXP op, SEXP args, SEXP env)
{
    SEXP x, y;
    int i, n;
    char *p;
