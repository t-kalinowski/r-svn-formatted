...
SEXP do_codesgets(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    y = CADR(args);
    if (!isFactor(x))
        errorcall(call, "lhs not a factor\n");
    if (!isNumeric(y))
        errorcall(call, "rhs not numeric\n");
    nx = LENGTH(x);
    ny = LENGTH(x);
    lx = LEVELS(x);
    if (nx != ny)
        errorcall(call, "unequal lengths\n");
    for (i = 0; i < nx; i++)
    {
        switch (TYPEOF(y))
        {
        ...
        for (i = 0; i < lenx; i++)
        {
            ...
            else
            {
                ...
                case STRSXP:
                    ...
                    break;
                case CPLXSXP:
                    LOGICAL(ans)[i] = !(FINITE(COMPLEX(CAR(x))[0].r) && FINITE(COMPLEX(CAR(x))[0].i));
                    break;
                }
            }
            x = CDR(x);
...
static SEXP coerceToFactor(SEXP v)
{
    ...
    case LGLSXP:
        ...
        for (i = 0; i < n; i++)
        {
            FACTOR(ans)[i] = (LOGICAL(v)[i] == NA_LOGICAL) ? NA_FACTOR : ((LOGICAL(v)[i] == 0) ? 1 : 2);
        }
...
static SEXP coerceToInteger(SEXP v)
{
    ...
    case ORDSXP:
        for (i = 0; i < n; i++)
        {
            INTEGER(ans)[i] = (FACTOR(v)[i] == NA_FACTOR) ? NA_INTEGER : FACTOR(v)[i];
        }
        break;
    case REALSXP:
        for (i = 0; i < n; i++)
    ...
    UNPROTECT(1);
    if (warn)
        warning("integer conversion: some values were too large and were converted to NA\n");
    return ans;
}
...
static SEXP coerceToReal(SEXP v)
{
    ...
    case ORDSXP:
        for (i = 0; i < n; i++)
        {
            REAL(ans)[i] = (FACTOR(v)[i] == NA_FACTOR) ? NA_REAL : FACTOR(v)[i];
        }
        break;
    case INTSXP:
        for (i = 0; i < n; i++)
...
static SEXP coerceToString(SEXP v)
{
    ...
    PROTECT(ans);
    ATTRIB(ans) = duplicate(ATTRIB(v));
    if (isFactor(v) && (tmpchar = getAttrib(v, install("levels"))) != R_NilValue && TYPEOF(tmpchar) == STRSXP &&
