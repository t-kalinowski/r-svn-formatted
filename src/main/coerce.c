...
static SEXP coerceToSymbol(SEXP v)
{
    ...
    int warn = 0;
    if (length(v) <= 0)
        error("Invalid data of mode \"%s\" (too short)", CHAR(type2str(TYPEOF(v))));
    PROTECT(v);
    switch (TYPEOF(v))
    {
    case LGLSXP:
...
static SEXP coercePairList(SEXP v, SEXPTYPE type)
{
    ...
    else if (isVectorizable(v))
    {
        ...
        error("pairlist object cannot be coerced to %s", CHAR(type2str(type)));
...
static SEXP coerceVectorList(SEXP v, SEXPTYPE type)
{
    ...
    else if (isVectorizable(v))
    {
        ...
        error("(list) object cannot be coerced to %s", CHAR(type2str(type)));
...
SEXP do_isvector(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    else if (streql(CHAR(STRING_ELT(CADR(args), 0)), CHAR(type2str(TYPEOF(CAR(args))))))
    {
        LOGICAL(ans)[0] = 1;
    }
    else
...
static classType classTable[] = {{"logical", LGLSXP, TRUE},       {"integer", INTSXP, TRUE},
                                 {"double", REALSXP, TRUE},       {"complex", CPLXSXP, TRUE},
                                 {"character", STRSXP, TRUE},     {"expression", EXPRSXP, TRUE},
                                 {"list", VECSXP, TRUE},          {"environment", ENVSXP, FALSE},
                                 {"char", CHARSXP, TRUE},         {"externalptr", EXTPTRSXP, FALSE},
                                 {"weakref", WEAKREFSXP, FALSE},  {"name", SYMSXP, FALSE},
    ...
    else
    {
        ...
        if (valueType != -1)
        {
            ...
            if (classTable[whichType].canChange)
            {
                ...
                error("\"%s\" can only be set as the class if the object has this type; found \"%s\"", valueString,
