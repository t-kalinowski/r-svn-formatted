...
attribute_hidden SEXP do_asCharacterFactor(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    SEXP x;
    checkArity(op, args);
...
#define MAYBE_CACHE_DO_IS_AS_VECTORS_EXPERI                                                                            \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            do_is_as_vector_experiments = ((vector_experi != NULL) && StringTrue(vector_experi)) ? 1 : 0;              \
        }                                                                                                              \
    } while (0)
...
attribute_hidden SEXP do_asvector(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    SEXP x = CAR(args);
    int type = (!strcmp("function", CHAR(STRING_ELT(CADR(args), 0)))) /* ASCII */
                   ? CLOSXP
...
attribute_hidden SEXP do_str2lang(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    */
    ...
    for (int i = 0; i < LENGTH(args); i++)
        if (!ENC_KNOWN(STRING_ELT(args, i)) && !IS_ASCII(STRING_ELT(args, i)))
        {
            allKnown = FALSE;
            break;
        }
...
attribute_hidden SEXP do_is(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    case INTSXP: /* is.integer */
        LOGICAL0(ans)[0] = (TYPEOF(CAR(args)) == INTSXP) && !inherits(CAR(args), "factor");
        break;
    ...
    case ENVSXP: /* is.environment */
        ...
        break;
    case VECSXP: /* is.list */
        LOGICAL0(ans)[0] = (TYPEOF(CAR(args)) == VECSXP || TYPEOF(CAR(args)) == LISTSXP);
        break;
    case LISTSXP:                                                                         /* is.pairlist */
        LOGICAL0(ans)[0] = (TYPEOF(CAR(args)) == LISTSXP || TYPEOF(CAR(args)) == NILSXP); /* pairlist() -> NULL */
        break;
    ...
    case 100:                                                             /* is.numeric */
        LOGICAL0(ans)[0] = isNumeric(CAR(args)) && !isLogical(CAR(args)); /* isNumeric excludes factors */
        break;
    case 101: /* is.matrix */
...
*/
{
    ...
    case LISTSXP: {
        ...
        for (i = 0; i < n; i++, x = CDR(x))
        {
            ...
            if ((DispatchOrEval(call2, op, "anyNA", args2, env, &ans, 0, 1) && asLogical(ans)) ||
                anyNA(call2, op, args2, env))
            {
                UNPROTECT(2);
                return TRUE;
            }
...
attribute_hidden SEXP do_anyNA(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    else
    {
        ...
        if (do_anyNA_formals == NULL)
            do_anyNA_formals = allocFormalsList2(install("x"), R_RecursiveSymbol);
        PROTECT(args = matchArgs_NR(do_anyNA_formals, args, call));
        if (CADR(args) == R_MissingArg)
            SETCADR(args, ScalarLogical(FALSE));
        ans = ScalarLogical(anyNA(call, op, args, rho));
        UNPROTECT(1);
    }
    return ans;
...
attribute_hidden SEXP do_substitute(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (do_substitute_formals == NULL)
        do_substitute_formals = allocFormalsList2(install("expr"), install("env"));
    ...
    else
    {
        ...
        if (valueType != (SEXPTYPE)-1)
        {
            ...
            if (classTable[whichType].canChange)
            {
                ...
                error(_("\"%s\" can only be set as the class if the object has this type; found \"%s\""), valueString,
        ...
        else if (!strcmp("numeric", valueString))
        {
            ...
            case REALSXP:
                break;
            default:
                PROTECT(obj = coerceVector(obj, REALSXP));
                nProtect++;
            }
        }
        ...
        { /* set the class but don't do the coercion; that's
