...
static SEXP coercePairList(SEXP v, SEXPTYPE type)
{
    ...
    else if (isVectorizable(v))
    {
        ...
        error(_("'pairlist' object cannot be coerced to '%s'"), type2char(type));
...
SEXP coerceVector(SEXP v, SEXPTYPE type)
{
    ...
    case LANGSXP:
        ...
        if (n == 0)
            break; /* Can this actually happen? */
        i = 0;
        op = CAR(v);
...
#define COERCE_ERROR error(_("cannot coerce type %s to %s vector"), type2char(TYPEOF(v)), type2char(type))
...
static SEXP ascommon(SEXP call, SEXP u, SEXPTYPE type)
{
    ...
    else if (isSymbol(u) && type == VECSXP)
    {
        ...
        errorcall(call, _("cannot coerce to vector"));
    return u; /* -Wall */
}
...
SEXP attribute_hidden do_ascharacter(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    checkArity(op, args);
    x = CAR(args);
    if (TYPEOF(x) == STRSXP && ATTRIB(x) == R_NilValue)
        return x;
...
SEXP attribute_hidden do_is(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    case ENVSXP: /* is.environment */
        ...
        break;
    case VECSXP: /* is.list */
        LOGICAL(ans)[0] = (TYPEOF(CAR(args)) == VECSXP || TYPEOF(CAR(args)) == LISTSXP);
        break;
    case LISTSXP:                                                                        /* is.pairlist */
        LOGICAL(ans)[0] = (TYPEOF(CAR(args)) == LISTSXP || TYPEOF(CAR(args)) == NILSXP); /* pairlist() -> NULL */
        break;
    ...
    case 301: /* is.language */
        LOGICAL(ans)[0] = (TYPEOF(CAR(args)) == SYMSXP || TYPEOF(CAR(args)) == LANGSXP || TYPEOF(CAR(args)) == EXPRSXP);
        break;
...
#define LIST_VEC_NA(s)                                                                                                 \
    ...
    {                                                                                                                  \
        ...
        case STRSXP:                                                                                                   \
            ...
            break;                                                                                                     \
        case CPLXSXP:                                                                                                  \
            LOGICAL(ans)[i] = (ISNAN(COMPLEX(s)[0].r) || ISNAN(COMPLEX(s)[0].i));                                      \
            break;                                                                                                     \
        default:                                                                                                       \
            LOGICAL(ans)[i] = 0;                                                                                       \
        }                                                                                                              \
...
SEXP attribute_hidden do_storage_mode(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!isValidString(value) || STRING_ELT(value, 0) == NA_STRING)
        errorcall(call, _("'value' must be non-null character string"));
    if (getAttrib(obj, install("Csingle")) != R_NilValue)
        isSingle = TRUE;
    type = str2type(CHAR(STRING_ELT(value, 0)));
    if (type == (SEXPTYPE)-1)
    {
        ...
        {
            warningcall(call, "use of 'real' is deprecated: use 'double' instead");
            type = REALSXP;
        }
        else if (streql(CHAR(STRING_ELT(value, 0)), "single"))
        {
            warningcall(call, "use of 'single' is deprecated: use mode<- instead");
            type = REALSXP;
            setSingle = TRUE;
        }
        else
    ...
    if (TYPEOF(obj) == type && isSingle == setSingle)
        return obj;
    if (isFactor(obj))
        errorcall(call, _("invalid to change the storage mode of a factor"));
    ...
    ans = coerceVector(obj, type);
    DUPLICATE_ATTRIB(ans, obj);
    setAttrib(ans, install("Csingle"), setSingle ? ScalarLogical(TRUE) : R_NilValue);
    UNPROTECT(1);
    return ans;
}
#endif
