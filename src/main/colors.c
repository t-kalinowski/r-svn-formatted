...
SEXP attribute_hidden do_hsv(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    args = CDR(args);
    PROTECT(s = coerceVector(CAR(args), REALSXP));
    args = CDR(args);
    PROTECT(v = coerceVector(CAR(args), REALSXP));
    args = CDR(args);
    PROTECT(gm = coerceVector(CAR(args), REALSXP));
    args = CDR(args);
...
SEXP attribute_hidden do_hcl(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    PROTECT(h = coerceVector(CAR(args), REALSXP));
    args = CDR(args);
    PROTECT(c = coerceVector(CAR(args), REALSXP));
    args = CDR(args);
    PROTECT(l = coerceVector(CAR(args), REALSXP));
    args = CDR(args);
    PROTECT(a = coerceVector(CAR(args), REALSXP));
    args = CDR(args);
...
SEXP attribute_hidden do_rgb(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (OP)
    { /* op == 1:  rgb256() :*/
        PROTECT(r = coerceVector(CAR(args), INTSXP));
        args = CDR(args);
        PROTECT(g = coerceVector(CAR(args), INTSXP));
        args = CDR(args);
        PROTECT(b = coerceVector(CAR(args), INTSXP));
        args = CDR(args);
        PROTECT(a = coerceVector(CAR(args), INTSXP));
        args = CDR(args);
    }
    else
    {
        ...
        args = CDR(args);
        PROTECT(g = coerceVector(CAR(args), REALSXP));
        args = CDR(args);
        PROTECT(b = coerceVector(CAR(args), REALSXP));
        args = CDR(args);
        PROTECT(a = coerceVector(CAR(args), REALSXP));
        args = CDR(args);
        mV = asReal(CAR(args));
...
#define _R_set_c_RGBA(_R, _G, _B, _A)                                                                                  \
    ...
    if (OP)
    { /* OP == 1:  rgb256() :*/
        _R_set_c_RGBA(CheckColor(INTEGER(r)[i % nr]), CheckColor(INTEGER(g)[i % ng]), CheckColor(INTEGER(b)[i % nb]),
...
SEXP attribute_hidden do_col2RGB(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    else
    {
        ...
        {
            col = INTEGER(icol)[i];
            col = (col > 0) ? R_ColorTable[(col - 1) % R_ColorTableSize] : dpptr(CurrentDevice())->bg;
            INTEGER(ans)[i4 + 0] = R_RED(col);
            INTEGER(ans)[i4 + 1] = R_GREEN(col);
            INTEGER(ans)[i4 + 2] = R_BLUE(col);
            INTEGER(ans)[i4 + 3] = R_ALPHA(col);
...
SEXP attribute_hidden do_RGB2hsv(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    PROTECT(rgb = coerceVector(CAR(args), REALSXP));
    args = CDR(args);
    if (!isMatrix(rgb))
