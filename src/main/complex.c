...
static void complex_div(double complex *c, double complex *a, double complex *b)
{
    *c = *a / *b;
}
#endif
...
static void complex_pow(Rcomplex *r, Rcomplex *a, Rcomplex *b)
{
    ...
    if (b->i == 0.)
    { /* ^ "real" : be fast (and more accurate)*/
        if (b->r == 1.)
        { /* a^1 */
            r->r = a->r;
            r->i = a->i;
...
SEXP complex_binary(ARITHOP_TYPE code, SEXP s1, SEXP s2)
{
    ...
    case PLUSOP:
        ...
        {
#ifdef HAVE_C99_COMPLEX
            C99_COMPLEX(ans)[i] = C99_COMPLEX(s1)[i % n1] + C99_COMPLEX(s2)[i % n2];
#else
            x1 = COMPLEX(s1)[i % n1];
            x2 = COMPLEX(s2)[i % n2];
    ...
    case POWOP:
        ...
        {
#ifdef HAVE_C99_COMPLEX
            C99_COMPLEX(ans)[i] = mycpow(C99_COMPLEX(s1)[i % n1], C99_COMPLEX(s2)[i % n2]);
#else
            x1 = COMPLEX(s1)[i % n1];
            x2 = COMPLEX(s2)[i % n2];
...
static void z_logbase(double complex *r, double complex *z, double complex *base)
{
    *r = clog(*z) / clog(*base);
}
...
static void z_atan2(double complex *r, double complex *csn, double complex *ccs)
{
    if (*ccs == 0)
    {
        if (*csn == 0)
...
SEXP complex_math1(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    case 3:
        naflag = cmath1(z_sqrt, COMPLEX(x), COMPLEX(y), n);
        break;

    case 10:
        naflag = cmath1(z_exp, COMPLEX(x), COMPLEX(y), n);
        break;

    case 20:
        naflag = cmath1(z_cos, COMPLEX(x), COMPLEX(y), n);
        break;
    case 21:
        naflag = cmath1(z_sin, COMPLEX(x), COMPLEX(y), n);
        break;
    case 22:
        naflag = cmath1(z_tan, COMPLEX(x), COMPLEX(y), n);
        break;
    case 23:
        naflag = cmath1(z_acos, COMPLEX(x), COMPLEX(y), n);
        break;
    case 24:
    ...
    case 30:
        naflag = cmath1(z_cosh, COMPLEX(x), COMPLEX(y), n);
        break;
    case 31:
        naflag = cmath1(z_sinh, COMPLEX(x), COMPLEX(y), n);
        break;
    case 32:
        naflag = cmath1(z_tanh, COMPLEX(x), COMPLEX(y), n);
        break;
    case 33:
        naflag = cmath1(z_acosh, COMPLEX(x), COMPLEX(y), n);
        break;
    case 34:
        naflag = cmath1(z_asinh, COMPLEX(x), COMPLEX(y), n);
        break;
    case 35:
        naflag = cmath1(z_atanh, COMPLEX(x), COMPLEX(y), n);
        break;
...
SEXP do_polyroot(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (COMPLEX(z)[i].r != 0.0 || COMPLEX(z)[i].i != 0.0)
            degree = i;
    }
    ...
    {
        if (n > 49)
            errorcall(call, _("polynomial degree too high (49 max)"));
        ...
        if (fail)
            errorcall(call, _("root finding code failed"));
        UNPROTECT(2);
