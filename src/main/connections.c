...
#else
        warning(_("cannot open file '%s', reason '%s'"), name, "fdopen is not supported on this platform");
#endif
    }
    if (!fp)
    {
...
SEXP attribute_hidden do_readchar(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0, m = 0; i < n; i++)
    {
        ...
        onechar = isRaw ? rawFixedString(bytes, len, nbytes, &np) : readFixedString(con, len);
        if (onechar != R_NilValue)
        {
            SET_STRING_ELT(ans, i, onechar);
            m++;
...
SEXP attribute_hidden do_sink(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (tee == NA_LOGICAL)
        error(_("invalid value for '%s'"), "split");
