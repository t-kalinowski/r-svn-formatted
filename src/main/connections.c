...
static size_t bzfile_read(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    ...
    while (nleft > 0)
    {
        ...
        size_t n = BZ2_bzRead(&bzerror, bz->bfp, (char *)ptr + nread, nleft);
...
static void raw_resize(Rrawconn this, size_t needed)
{
    ...
    if (needed > 8192)
        nalloc = 1.2 * needed; /* 20% over-allocation */
    else
        while (nalloc < needed)
...
static void text_init(Rconnection con, SEXP text, int type)
{
    ...
    for (i = 0; i < nlines; i++)
        nchars +=
            strlen(type == 1 ? translateChar(STRING_ELT(text, i))
...
SEXP attribute_hidden do_writelines(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (con_num == R_OutputCon)
    {
        ...
        do
        {
            ...
            for (i = 0; i < length(text); i++)
                Rconn_printf(con0, "%s%s", useBytes ? CHAR(STRING_ELT(text, i)) : translateChar(STRING_ELT(text, i)),
                             ssep);
            con0->fflush(con0);
    ...
    {
        for (i = 0; i < length(text); i++)
            Rconn_printf(con, "%s%s", useBytes ? CHAR(STRING_ELT(text, i)) : translateChar(STRING_ELT(text, i)), ssep);
    }
...
static int rawRead(char *p, int size, int n, Rbyte *bytes, int nbytes, int *np)
{
    int avail, m;
...
SEXP attribute_hidden do_readbin(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    int i, size, signd, swap, n, m = 0, sizedef = 4, mode = 1, nbytes = 0, np = 0;
    const char *what;
    void *p = NULL;
    ...
    else if (!strcmp(what, "complex"))
    {
        ...
        p = (void *)COMPLEX(ans);
        m = isRaw ? rawRead(p, size, n, bytes, nbytes, &np) : con->read(p, size, n, con);
        if (swap)
            for (i = 0; i < m; i++)
            {
    ...
    else
    {
        ...
        else
        {
            ...
            if (mode == 1)
            {
                for (i = 0, m = 0; i < n; i++)
                {
                    s = isRaw ? rawRead(buf, size, 1, bytes, nbytes, &np) : con->read(buf, size, 1, con);
                    if (s)
                        m++;
                    else
                        break;
...
SEXP attribute_hidden do_getconnection(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (what == NA_INTEGER || what < 0 || what >= NCONNECTIONS || !Connections[what])
...
attribute_hidden SEXP R_decompress1(SEXP in)
{
    ...
    if (res != Z_OK)
        error("internal error %d in R_decompress1", res);
...
attribute_hidden SEXP R_decompress2(SEXP in)
{
    unsigned int inlen, outlen;
    int res;
