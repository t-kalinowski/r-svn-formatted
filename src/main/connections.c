    ...
    if (con->inconv)
    {
        while (con->navail <= 0)
        {
            ...
            */
            ...
            if (inew == 0)
                return R_EOF;
            if (checkBOM && con->inavail >= 2 && ((int)con->iconvbuff[0] & 0xff) == 255 &&
            ...
            if (inew == 0)
                return R_EOF;
            if (checkBOM8 && con->inavail >= 3 && !memcmp(con->iconvbuff, "\xef\xbb\xbf", 3))
            {
                con->inavail -= (short)3;
...
static size_t NORET null_read(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    error(_("%s not enabled for this connection"), "'read'");
}

static size_t NORET null_write(const void *ptr, size_t size, size_t nitems, Rconnection con)
{
    error(_("%s not enabled for this connection"), "'write'");
}

void init_con(Rconnection new, const char *description, int enc, const char *const mode)
{
...
static Rboolean file_open(Rconnection con)
{
    ...
    if (strcmp(name, "stdin"))
    {
        ...
        if (strchr(mode, 't') && (!strcmp(con->encname, "UTF-16LE") || !strcmp(con->encname, "UCS-2LE")))
        {
...
static Rconnection newfile(const char *description, int enc, const char *mode, int raw)
{
    Rconnection new;
...
static char *win_getlasterror_str(void)
{
    ...
    err_msg_len =
        FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL,
                      GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&lpv_tempmsg, 0, NULL);
...
static Rboolean fifo_open(Rconnection con)
{
    ...
    */
    ...
    if (con->canwrite)
    {
        ...
        this->hdl_namedpipe = CreateNamedPipeA(
            hch_pipename, (con->canread ? PIPE_ACCESS_DUPLEX : PIPE_ACCESS_OUTBOUND) | FILE_FLAG_OVERLAPPED,
            PIPE_TYPE_BYTE, PIPE_UNLIMITED_INSTANCES, 0, 0, FILE_FLAG_NO_BUFFERING, &win_namedpipe_secattr);
    ...
    if ((boo_retvalue || GetLastError() == 231) && this->hdl_namedpipe <= (HANDLE)(LONG_PTR)0)
    {
        DWORD dwo_openmode = 0;
        if (con->canread)
            dwo_openmode |= GENERIC_READ;
        if (con->canwrite)
            dwo_openmode |= GENERIC_WRITE;
        this->hdl_namedpipe = CreateFileA(hch_pipename, dwo_openmode, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
...
static void fifo_close(Rconnection con)
{
    ...
    if (this->overlapped_write)
        CloseHandle(this->overlapped_write);
}
...
static size_t fifo_read(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    ...
    ReadFile(this->hdl_namedpipe, buffer, (size * sizeof(wchar_t)) * nitems, (LPDWORD)&read_byte,
...
static size_t fifo_write(const void *ptr, size_t size, size_t nitems, Rconnection con)
{
    ...
    /* Write data */
    if (WriteFile(this->hdl_namedpipe, buffer, size * sizeof(wchar_t) * nitems, (LPDWORD)&written_bytes, NULL) ==
            FALSE &&
...
#ifdef Win32
extern Rconnection newWpipe(const char *description, int enc, const char *mode);
#endif
...
static Rboolean gzfile_open(Rconnection con)
{
    ...
    if (strchr(con->mode, 'w'))
        snprintf(mode, 6, "wb%1d", gzcon->compress);
    else if (con->mode[0] == 'a')
        snprintf(mode, 6, "ab%1d", gzcon->compress);
    else
        strcpy(mode, "rb");
    errno = 0; /* precaution */
    name = R_ExpandFileName(con->description);
    ...
    if (!fp)
    {
        warning(_("cannot open compressed file '%s', probable reason '%s'"), name, strerror(errno));
        return FALSE;
    }
...
static Rboolean bzfile_open(Rconnection con)
{
    ...
    if (!fp)
    {
        warning(_("cannot open bzip2-ed file '%s', probable reason '%s'"), name, strerror(errno));
        return FALSE;
    }
    if (isDir(fp))
    {
    ...
    if (con->canread)
    {
        ...
        if (bzerror != BZ_OK)
        {
            ...
            fclose(fp);
            warning(_("file '%s' appears not to be compressed by bzip2"), R_ExpandFileName(con->description));
            return FALSE;
        }
    }
    else
    {
        ...
        if (bzerror != BZ_OK)
        {
            ...
            fclose(fp);
            warning(_("initializing bzip2 compression for file '%s' failed"), R_ExpandFileName(con->description));
            return FALSE;
        }
    }
    bz->fp = fp;
...
static size_t bzfile_read(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    Rbzfileconn bz = con->private;
    int nread = 0, nleft;
    int bzerror;
    ...
    while (nleft > 0)
    {
        ...
        { /* this could mean multiple streams so we need to check */
            ...
            {
                if (nUnused > 0)
                { /* unused bytes present - need to retain them */
...
static size_t bzfile_write(const void *ptr, size_t size, size_t nitems, Rconnection con)
{
    Rbzfileconn bz = con->private;
    int bzerror;
...
static Rconnection newbzfile(const char *description, const char *mode, int compress)
{
    Rconnection new;
    new = (Rconnection)malloc(sizeof(struct Rconn));
    if (!new)
...
static size_t xzfile_read(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    Rxzfileconn xz = con->private;
    lzma_stream *strm = &(xz->stream);
    lzma_ret ret;
...
static Rboolean clp_open(Rconnection con)
{
    ...
    con->canread = !con->canwrite;
    this->pos = 0;
    if (strlen(con->encname) > 0 && strcmp(con->encname, "native.enc") && strcmp(con->encname, "UTF-16LE"))
    ...
    if (con->canread)
    {
        ...
        HGLOBAL hglb;
        wchar_t *wpc;
        if (GA_clipboardhastext() && OpenClipboard(NULL) && (hglb = GetClipboardData(CF_UNICODETEXT)) &&
...
static void clp_writeout(Rconnection con)
{
    ...
    if ((hglb = GlobalAlloc(GHND, (wlen + 1) * sizeof(wchar_t))) && (s = (wchar_t *)GlobalLock(hglb)))
    {
...
static size_t clp_read(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    Rclpconn this = con->private;
...
static size_t clp_write(const void *ptr, size_t size, size_t nitems, Rconnection con)
{
    Rclpconn this = con->private;
...
static size_t raw_read(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    Rrawconn this = con->private;
...
static int raw_fgetc(Rconnection con)
{
    ...
    if (this->pos >= this->nbytes)
        ...
        return (int)RAW(this->data)[this->pos++];
}
...
static double raw_seek(Rconnection con, double where, int origin, int rw)
{
    ...
    {
    case 2:
        newpos = (double)this->pos + where;
        break;
    case 3:
        newpos = (double)this->nbytes + where;
        break;
    default:
        newpos = where;
    }
...
static Rconnection getConnectionCheck(SEXP rcon, const char *cls, const char *var)
{
...
static void outtext_init(Rconnection con, SEXP stext, const char *mode, int idx)
{
    ...
    else
    {
        ...
        else
        {
            ...
            val = findVar1(this->namesymbol, VECTOR_ELT(OutTextData, idx), STRSXP, FALSE);
...
SEXP attribute_hidden do_textconnection(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    checkArity(op, args);
    sdesc = CAR(args);
    if (!isString(sdesc) || LENGTH(sdesc) != 1 || STRING_ELT(sdesc, 0) == NA_STRING)
        error(_("invalid '%s' argument"), "description");
...
static int con_close1(Rconnection con)
{
    ...
    if (con->inconv)
        Riconv_close(con->inconv);
    if (con->outconv)
        Riconv_close(con->outconv);
    con->destroy(con);
    free(con->class);
    con->class = NULL;
    free(con->description);
    ...
    /* clear the pushBack */
    if (con->nPushBack > 0)
    {   // already cleared on closed connection,
...
attribute_hidden size_t Rconn_getline(Rconnection con, char *buf, size_t bufsize)
{
    int c;
    ssize_t nbuf = -1;
...
SEXP attribute_hidden do_readLines(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    con = getConnection(asInteger(CAR(args)));
    args = CDR(args);
    n = asVecSize(CAR(args));
    args = CDR(args);
    if (n == -999)
        error(_("invalid '%s' argument"), "n");
    ok = asLogical(CAR(args));
    args = CDR(args);
    if (ok == NA_LOGICAL)
        error(_("invalid '%s' argument"), "ok");
    warn = asLogical(CAR(args));
    args = CDR(args);
    if (warn == NA_LOGICAL)
    ...
    for (nread = 0; nread < nnn; nread++)
    {
        ...
        if (nread == 0 && utf8locale && strlen(buf) >= 3 && !memcmp(buf, "\xef\xbb\xbf", 3))
            qbuf = buf + 3;
...
no_more_lines:
    ...
    { /* incomplete last line */
        if (con->text && !con->blocking && (strcmp(con->class, "gzfile") != 0))
        {
            /* push back the rest */
...
SEXP attribute_hidden do_readbin(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!strcmp(what, "character"))
    {
        ...
        for (i = 0, m = 0; i < n; i++)
        {
            onechar = isRaw ? rawOneString(bytes, nbytes, &np) : readOneString(con);
...
SEXP attribute_hidden do_readchar(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!isRaw && con->text && (con->buff || con->nPushBack >= 0 || con->inconv))
        ...
        warning(_("text connection used with %s(), results may be incorrect"), "readChar");
    for (i = 0, m = 0; i < n; i++)
    {
...
SEXP attribute_hidden do_writechar(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!isRaw && con->text && con->outconv)
        /* could be turned into runtime error */
        warning(_("text connection used with %s(), results may be incorrect"), "writeChar");
...
SEXP attribute_hidden do_clearpushback(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (con->nPushBack > 0)
    { // so con_close1 has not been called
        for (j = 0; j < con->nPushBack; j++)
            free(con->PushBack[j]);
        free(con->PushBack);
        con->nPushBack = 0;
        ...
        { /* call to file() */
            ...
            else
            {
                ...
                if (!raw)
                {
                    ...
                    {
                        raw = TRUE;
                        warning(_("using 'raw = TRUE' because '%s' is a fifo or pipe"), url);
                    }
                    else if (!res && !(sb.st_mode & S_IFREG) && strcmp(efn, "/dev/null"))
                        ...
                        warning(_("'raw = FALSE' but '%s' is not a regular file"), url);
                }
#endif
                if (!raw && (!strlen(open) || streql(open, "r") || streql(open, "rt")))
                {
...
static Rboolean gzcon_open(Rconnection con)
{
    ...
    else
    {
        /* write a header */
        char head[11];
        snprintf(head, 11, "%c%c%c%c%c%c%c%c%c%c", gz_magic[0], gz_magic[1], Z_DEFLATED, 0 /*flags*/, 0, 0, 0,
...
static size_t gzcon_read(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    Rgzconn priv = con->private;
    Rconnection icon = priv->con;
...
SEXP attribute_hidden do_gzcon(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    m = incon->mode;
    if (strcmp(m, "r") == 0 || strncmp(m, "rb", 2) == 0)
        mode = "rb";
    ...
    new->encname[100 - 1] = '\0';
    new->ex_ptr = PROTECT(R_MakeExternalPtr((void *)new->id, install("connection"), R_NilValue));
    if (incon->isopen)
        new->open(new);
...
attribute_hidden SEXP R_compress1(SEXP in)
{
    ...
    *((unsigned int *)buf) = (unsigned int)uiSwap(inlen);
    ...
    if (res != Z_OK)
        error("internal error %d in R_compress1", res);
...
attribute_hidden SEXP R_decompress1(SEXP in, Rboolean *err)
{
    const void *vmax = vmaxget();
    uLong inlen, outlen;
    int res;
...
attribute_hidden SEXP R_compress2(SEXP in)
{
    const void *vmax = vmaxget();
...
attribute_hidden SEXP R_decompress2(SEXP in, Rboolean *err)
{
    const void *vmax = vmaxget();
    unsigned int inlen, outlen;
    int res;
...
SEXP attribute_hidden do_sockselect(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (!immediate)
        Rsockselect(nsock, INTEGER(insockfd), LOGICAL(val), LOGICAL(write), timeout);
...
attribute_hidden SEXP R_decompress3(SEXP in, Rboolean *err)
{
    const void *vmax = vmaxget();
    unsigned int inlen, outlen;
    ...
    if (type == 'Z')
    {
        ...
        {
            warning("internal error %d in R_decompress3 %d", ret, strm.avail_in);
            *err = TRUE;
            return R_NilValue;
        }
        lzma_end(&strm);
...
SEXP attribute_hidden do_memCompress(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    case 2:    /*gzip */
    {
        ...
        uLong inlen = XLENGTH(from), outlen = (uLong)(1.001 * (double)inlen + 20);
        buf = (Bytef *)R_alloc(outlen, sizeof(Bytef));
        res = compress(buf, &outlen, (Bytef *)RAW(from), inlen);
        if (res != Z_OK)
            error("internal error %d in memCompress", res);
        ans = allocVector(RAWSXP, outlen);
        memcpy(RAW(ans), buf, outlen);
        break;
    }
    case 3: /* bzip */
    {
        ...
        buf = R_alloc(outlen, sizeof(char));
        res = BZ2_bzBuffToBuffCompress(buf, &outlen, (char *)RAW(from), inlen, 9, 0, 0);
        if (res != BZ_OK)
            error("internal error %d in memCompress", res);
        ans = allocVector(RAWSXP, outlen);
        memcpy(RAW(ans), buf, outlen);
...
SEXP attribute_hidden do_memDecompress(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    case 2:    /* gzip */
    {
        ...
        while (1)
        {
            ...
            if (res >= 0)
                break;
            error("internal error %d in memDecompress(%s)", res, "type = \"gzip\"");
        }
    ...
    case 3: /* bzip2 */
    {
        ...
        while (1)
        {
            ...
            if (res == BZ_OK)
                break;
            error("internal error %d in memDecompress(%s)", res, "type = \"bzip2\"");
        }
        ans = allocVector(RAWSXP, outlen);
        memcpy(RAW(ans), buf, outlen);
        break;
    ...
    case 4: /* xz */
    {
        ...
        while (1)
        {
            ...
            if (strm.avail_in > 0)
            {
                ...
                {
                    error("internal error %d in memDecompress(%s) at %d", ret, "type = \"xz\"", strm.avail_in);
                }
            }
            else
            {
