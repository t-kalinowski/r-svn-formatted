...
SEXP R_sysframe(int n, RCNTXT *cptr)
{
    ...
    if (n < 0)
        errorcall(R_GlobalContext->call, _("not that many enclosing environments"));
...
int R_sysparent(int n, RCNTXT *cptr)
{
    ...
    SEXP s;
    if (n <= 0)
        errorcall(R_ToplevelContext->call, _("only positive arguments are allowed"));
...
Rboolean R_ToplevelExec(void (*fun)(void *), void *data)
{
    ...
    begincontext(&thiscontext, CTXT_TOPLEVEL, R_NilValue, R_GlobalEnv, R_NilValue, R_NilValue, R_GlobalEnv);
