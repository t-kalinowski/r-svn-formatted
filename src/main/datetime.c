    ...
    for (R_xlen_t i = 0; i < n; i++)
    {
        ...
        else
        {
            ...
            [i] = ((tmp == -1.)
                   ...
                   && (tm.tm_sec != 59) && ((tm.tm_sec = 58), (mktime0(&tm, !isgmt) != -2.)))
                      ? NA_REAL
                      : tmp + (secs - fsecs);
#endif
        }
...
SEXP attribute_hidden do_POSIXlt2D(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (int i = 3; i < 6; i++)
        if ((nlen[i] = XLENGTH(VECTOR_ELT(x, i))) > n)
            n = nlen[i];
    if ((nlen[0] = XLENGTH(VECTOR_ELT(x, 0))) > n)
        n = nlen[0]; // sec; need for Inf,NaN,..
    if ((nlen[8] = XLENGTH(VECTOR_ELT(x, 8))) > n)
        n = nlen[8]; // isdst
    if (n > 0)
    {
    ...
    for (R_xlen_t i = 0; i < n; i++)
    {
        ...
        /* mktime ignores tm.tm_wday and tm.tm_yday */
        tm.tm_isdst = 0;
        if (tm.tm_mday == NA_INTEGER || tm.tm_mon == NA_INTEGER || tm.tm_year == NA_INTEGER ||
            validate_tm(&tm) < 0) // non-finite: take the 'sec'
