...
static int validate_tm(struct tm *tm)
{
    ...
    if (tm->tm_hour == 24 && tm->tm_min == 0 && tm->tm_sec == 0)
    {
        ...
        {
            if (tm->tm_mday > days_in_month[tm->tm_mon] + ((tm->tm_mon == 1 && isleap(1900 + tm->tm_year) ? 1 : 0)))
            {
                tm->tm_mon++;
                tm->tm_mday = 1;
                if (tm->tm_mon == 12)
    ...
    if (abs(tm->tm_mday) > 366)
    {
        ...
        {
            --tm->tm_mon;
            tm->tm_mday += days_in_month[tm->tm_mon] + ((tm->tm_mon == 1 && isleap(1900 + tm->tm_year)) ? 1 : 0);
        }
    ...
    while (tm->tm_mday > (tmp = days_in_month[tm->tm_mon] + ((tm->tm_mon == 1 && isleap(1900 + tm->tm_year)) ? 1 : 0)))
    {
        res++;
        if (++tm->tm_mon > 11)
        {
...
static double mktime00(struct tm *tm)
{
    ...
    if ((tm->tm_wday = (day + 4) % 7) < 0)
        tm->tm_wday += 7;

    return tm->tm_sec + (tm->tm_min * 60) + (tm->tm_hour * 3600) + (day + excess * 730485.0) * 86400.0;
}
...
SEXP attribute_hidden do_asPOSIXct(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < 6; i++)
        if ((nlen[i] = LENGTH(VECTOR_ELT(x, i))) > n)
            n = nlen[i];
    if ((nlen[8] = LENGTH(VECTOR_ELT(x, 8))) > n)
        n = nlen[8];
    if (n > 0)
    {
        for (i = 0; i < 6; i++)
...
SEXP attribute_hidden do_strptime(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < N; i++)
    {
        ...
        invalid = STRING_ELT(x, i % n) == NA_STRING ||
                  !R_strptime(CHAR(STRING_ELT(x, i % n)), CHAR(STRING_ELT(sformat, i % m)), &tm, &psecs, &offset);
        if (!invalid)
        {
...
SEXP attribute_hidden do_D2POSIXlt(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (R_FINITE(REAL(x)[i]))
        {
            ...
            /* month within year */
            for (mon = 0; day >= (tmp = (days_in_month[mon]) + ((mon == 1 && isleap(y + 1900)) ? 1 : 0));
                 day -= tmp, mon++)
                ;
            tm.tm_mon = mon;
            tm.tm_mday = day + 1;
...
SEXP attribute_hidden do_POSIXlt2D(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        /* mktime ignores tm.tm_wday and tm.tm_yday */
        tm.tm_isdst = 0;
        if (tm.tm_mday == NA_INTEGER || tm.tm_mon == NA_INTEGER || tm.tm_year == NA_INTEGER || validate_tm(&tm) < 0)
            REAL(ans)[i] = NA_REAL;
        else
        {
