...
attribute_hidden SEXP do_traceOnOff(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
    SEXP onOff = CAR(args);
    Rboolean trace = (PRIMVAL(op) == 0), prev = trace ? GET_TRACE_STATE : GET_DEBUG_STATE;
    ...
    if (length(onOff) > 0)
    {
        ...
        if (_new == TRUE || _new == FALSE)
            if (trace)
                ...
                SET_DEBUG_STATE(_new);
        else
            error(_("Value for '%s' must be TRUE or FALSE"), trace ? "tracingState" : "debuggingState");
    }
...
attribute_hidden SEXP do_tracemem(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (TYPEOF(object) == ENVSXP || TYPEOF(object) == PROMSXP)
        errorcall(call, _("'tracemem' is not useful for promise and environment objects"));
...
attribute_hidden SEXP NORET do_tracemem(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
    check1arg(args, call, "x");
...
attribute_hidden SEXP NORET do_untracemem(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
    check1arg(args, call, "x");
    errorcall(call, _("R was not compiled with support for memory profiling"));
...
attribute_hidden SEXP do_retracemem(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (do_retracemem_formals == NULL)
        do_retracemem_formals = allocFormalsList2(install("x"), R_PreviousSymbol);
    ...
    if (previous != R_NilValue)
    {
        ...
        if (R_current_trace_state())
        {
            /* FIXME: previous will have <0x....> whereas other values are
               without the < > */
            Rprintf("tracemem[%s -> %p]: ", translateChar(STRING_ELT(previous, 0)), (void *)object);
            memtrace_stack_dump();
        }
    }
    UNPROTECT(1);
