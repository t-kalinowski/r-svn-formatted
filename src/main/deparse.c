...
SEXP do_dump(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    {
        if (!(fp = R_fopen(R_ExpandFileName(CHAR(STRING_ELT(file, 0))), (append) ? "a" : "w")))
            errorcall(call, "unable to open file");
        for (i = 0; i < nobjs; i++)
        {
            fprintf(fp, "\"%s\" <-\n", CHAR(STRING_ELT(names, i)));
...
static void vector2buff(SEXP vector)
{
    ...
    if (tlen == 0)
    {
        ...
        {
        case LGLSXP:
            print2buff("logical(0)");
            break;
        case INTSXP:
            print2buff("numeric(0)");
            break;
        case REALSXP:
            print2buff("numeric(0)");
            break;
        case CPLXSXP:
            print2buff("complex(0)");
            break;
        case STRSXP:
            print2buff("character(0)");
            break;
        }
    }
    else if (tlen == 1)
