...
{
    SEXP a = ATTRIB(s);
    if (length(a) > (except_names ? 3 : 2))
        return (TRUE);
    while (!isNull(a))
    {
        if (!((TAG(a) == R_SrcrefSymbol) || (TAG(a) == R_NamesSymbol && except_names)))
            return (TRUE);
        a = CDR(a);
    }
    return (FALSE);
...
static void attr2(SEXP s, LocalParseData *d)
{
    ...
    while (!isNull(a))
    {
        if (TAG(a) != R_SrcrefSymbol && (TAG(a) != R_NamesSymbol || !nice_names))
        {
            print2buff(", ", d);
...
{
    PPinfo fop;
    Rboolean lookahead = FALSE, lbreak = FALSE, parens, fnarg = d->fnarg, outerparens, doquote, doAttr = TRUE;
    SEXP op, t;
...
static void vector2buff(SEXP vector, LocalParseData *d)
{
    ...
    Rboolean surround = FALSE, allNA, need_c = (tlen > 1 || nv != R_NilValue),
