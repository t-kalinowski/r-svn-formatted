...
SEXP deparse1line_(SEXP call, Rboolean abbrev, int opts)
{
    ...
    PROTECT(temp = deparse1WithCutoff(call, abbrev, MAX_Cutoff, backtick, opts, -1));
...
static attr_type attr1(SEXP s, LocalParseData *d)
{
    ...
    { // attr <= OK_NAMES
        ...
        REprintf("before returning attr1() = ", (attr == OK_NAMES ? "OK_NAMES" : "SIMPLE"));
#endif
    }
    return attr;
}
...
static void deparse2buff(SEXP s, LocalParseData *d)
{
    ...
    if (IS_S4_OBJECT(s) || TYPEOF(s) == S4SXP)
    {
        ...
        { // regular S4 objects
            ...
            if (TYPEOF(slotNms) == STRSXP && (n = LENGTH(slotNms)))
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    SET_VECTOR_ELT(slotlist, i, R_do_slot(s, installTrChar(STRING_ELT(slotNms, i))));
                }
    ...
    case LANGSXP:
        ...
        else if (TYPEOF(CAR(s)) == CLOSXP || TYPEOF(CAR(s)) == SPECIALSXP || TYPEOF(CAR(s)) == BUILTINSXP)
        {
...
static void vector2buff(SEXP vector, LocalParseData *d)
{
    ...
    Rboolean need_c = (tlen > 1 || !isNull(nv)), // (?) only TRUE iff SHOW_ATTR_OR_NMS
    ...
    if (tlen == 0)
    {
        ...
        REprintf("vector2buff(<tlen = 0>): (do_names, STR_names) = (%s,%s), attr = %s\n", STR_names ? "TRUE" : "FALSE",
                 ...
                 attr == STRUC_NMS_A
                     ? "STRUC_NMS_A"
                     : (attr == STRUC_ATTR ? "STRUC_ATTR" : (attr == OK_NAMES ? "OK_NAMES" : "SIMPLE")));
#endif
