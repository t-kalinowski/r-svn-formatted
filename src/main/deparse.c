...
SEXP deparse1m(SEXP call, Rboolean abbrev, int opts)
{
    ...
    SEXP result = deparse1WithCutoff(call, abbrev, DEFAULT_Cutoff, backtick, opts, 0);
...
attribute_hidden SEXP deparse1w(SEXP call, Rboolean abbrev, int opts)
{
    Rboolean backtick = TRUE;
...
static SEXP deparse1WithCutoff(SEXP call, Rboolean abbrev, int cutoff, Rboolean backtick, int opts, int nlines)
{
    ...
    */
    ...
    LocalParseData localData = {.linenumber = 0,
                                .len = 0,
                                .incurly = 0,
                                .inlist = 0,
                                .startline = TRUE,
    ...
    {                                               // default: nlines = -1 (from R), or = 0 (from other C fn's)
...
SEXP deparse1line_(SEXP call, Rboolean abbrev, int opts)
{
    Rboolean backtick = TRUE;
    int lines;
    SEXP temp = PROTECT(deparse1WithCutoff(call, abbrev, MAX_Cutoff, backtick, opts, -1));
...
{
    Rboolean backtick = TRUE;
    return deparse1WithCutoff(call, FALSE, DEFAULT_Cutoff, backtick, DEFAULTDEPARSE, /* nlines = */ 1);
}
...
{
    checkArity(op, args);
    SEXP names = CAR(args), file = CADR(args);
...
static Rboolean needsparens(PPinfo mainop, SEXP arg, unsigned int left, unsigned int deepLeft)
{
    PPinfo arginfo;
    if (TYPEOF(arg) == LANGSXP)
    {
        ...
        {
            if ((TYPEOF(SYMVALUE(CAR(arg))) == BUILTINSXP) || (TYPEOF(SYMVALUE(CAR(arg))) == SPECIALSXP))
            {
    ...
    {
        if (mainop.precedence > PREC_SUM || (mainop.precedence == PREC_SUM && left == mainop.rightassoc))
        {
            return TRUE;
        }
    }
...
static Rboolean usable_nice_names(SEXP x, Rboolean isAtomic)
{
    if (TYPEOF(x) == STRSXP)
    {
        ...
        if (isAtomic) // c(*, recursive=, use.names=): cannot use these as nice_names
            for (i = 0; i < n; i++)
            {
                if (STRING_ELT(x, i) == NA_STRING || strcmp(CHAR(STRING_ELT(x, i)), "recursive") == 0 ||
...
static const char *attrT2char(attr_type typ)
{
    ...
    case STRUC_NMS_A:
        return "STRUC_NMS_A";
    default:
        return "_unknown_ attr_type -- should *NOT* happen!";
    }
}
...
static attr_type attr1(SEXP s, LocalParseData *d)
{
    ...
    Rboolean nice_names = d->opts & NICE_NAMES, show_attr = d->opts & SHOWATTRIBUTES, has_names = !isNull(nm), ok_names;
#ifdef DEBUG_DEPARSE
...
static void attr2(SEXP s, LocalParseData *d, Rboolean not_names)
{
    ...
    while (!isNull(a))
    {
        if (TAG(a) != R_SrcrefSymbol && !(TAG(a) == R_NamesSymbol && not_names))
        {
            print2buff(", ", d);
...
static void deparse2buff(SEXP s, LocalParseData *d)
{
    ...
    if (IS_S4_OBJECT(s) || hasS4_t)
    {
        ...
        SEXP class = getAttrib(s, R_ClassSymbol), cl_def = TYPEOF(class) == STRSXP ? STRING_ELT(class, 0) : R_NilValue;
        if (TYPEOF(cl_def) == CHARSXP)
        { // regular S4 objects
            print2buff("new(\"", d);
    ...
    case LANGSXP:
        ...
        if (maybe_quote)
        {
            ...
            doquote = // := op is not `~` (tilde) :
                !((TYPEOF(op) == SYMSXP) && !strcmp(CHAR(PRINTNAME(op)), "~"));
            if (doquote)
            {
                print2buff("quote(", d);
                d->opts &= SIMPLE_OPTS;
        ...
        if (TYPEOF(op) == SYMSXP)
        {
                ...
                (userbinop = isUserBinop(op)))
            {
                ...
                case PP_DOLLAR: {
                    ...
                    SEXP rhs = CADR(s);
                    if (TYPEOF(rhs) != SYMSXP && !(isValidString(rhs) && STRING_ELT(rhs, 0) != NA_STRING))
                        fop.kind = PP_FUNCALL;
                    break;
                }
                default:
            ...
            else
            {
                ...
                else if (isSymbol(CAR(s)) && TYPEOF(val) == CLOSXP && streql(CHAR(PRINTNAME(CAR(s))), ":::"))
                { // ::: is special case
                    deparse2buff(CADR(s), d);
                    print2buff(":::", d);
                    deparse2buff(CADDR(s), d);
        ...
        else if (TYPEOF(op) == CLOSXP || TYPEOF(op) == SPECIALSXP || TYPEOF(op) == BUILTINSXP)
        {
...
static void vector2buff(SEXP vector, LocalParseData *d)
{
    ...
    if (TYPEOF(vector) == INTSXP && tlen > 1)
    {
        ...
        intSeq = (vec[0] != NA_INTEGER && vec[1] != NA_INTEGER && fabs(d_i) == 1);
        if (intSeq)
            for (i = 2; i < tlen; i++)
            {
    ...
    if (!need_c)
        need_c = do_names; // c(a = *) but not c(1)
#ifdef DEBUG_DEPARSE
    ...
    { // tlen > 0;  _not_ INTSXP
        ...
        for (i = 0; i < tlen; i++)
        {
            ...
            else if (TYPEOF(vector) == CPLXSXP && (ISNA(COMPLEX(vector)[i].r) && ISNA(COMPLEX(vector)[i].i)))
            {
                strp = allNA ? "NA_complex_" : EncodeElement(vector, i, quote, '.');
            }
            else if (TYPEOF(vector) == CPLXSXP && (ISNAN(COMPLEX(vector)[i].r) || !R_FINITE(COMPLEX(vector)[i].i)))
            {
                if (!buff)
                    buff = alloca(NB2);
                strp = EncodeNonFiniteComplexElement(COMPLEX(vector)[i], buff);
            }
            else if (allNA && TYPEOF(vector) == STRSXP && STRING_ELT(vector, i) == NA_STRING)
            {
                strp = "NA_character_";
            }
