...
void attribute_hidden R_AllocStringBuffer(int blen, DeparseBuffer *buf)
{
    if (blen >= 0)
    {
...
void attribute_hidden R_FreeStringBuffer(DeparseBuffer *buf)
{
    if (buf->data != NULL)
    {
        free(buf->data);
...
{
    SEXP a = ATTRIB(s);
    if (length(a) > 1 || (length(a) == 1 && (TYPEOF(s) != CLOSXP || TAG(a) != R_SourceSymbol)))
        return (TRUE);
    else
        return (FALSE);
}
...
static void vector2buff(SEXP vector, LocalParseData *d)
{
    ...
    else
    {
        ...
        for (i = 0; i < tlen; i++)
        {
            ...
            else if (allNA && TYPEOF(vector) == CPLXSXP && (ISNA(COMPLEX(vector)[i].r) || ISNA(COMPLEX(vector)[i].i)))
            {
                strp = "NA_complex_";
            }
