...
static double PS_StrWidth(char *str, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    return floor(gc->cex * gc->ps + 0.5) *
           PostScriptStringWidth((unsigned char *)str, metricInfo(gc->fontfamily, face, pd));
}
...
static double XFig_StrWidth(char *str, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    return floor(gc->cex * gc->ps + 0.5) *
           PostScriptStringWidth((unsigned char *)str, &(pd->fonts->family->fonts[face - 1]->metrics));
}
...
static void PDF_endfile(PDFDesc *pd)
{
    ...
    for (i = 0; i < 256 && pd->colAlpha[i] >= 0; i++)
    {
        pd->pos[++pd->nobjs] = (int)ftell(pd->pdffp);
        fprintf(pd->pdffp, "%d 0 obj\n<<\n/Type /ExtGState\n/CA %1.3f >>\nendobj\n", pd->nobjs,
                1 - pd->colAlpha[i] / 255.0);
    }
    for (i = 0; i < 256 && pd->fillAlpha[i] >= 0; i++)
    {
        pd->pos[++pd->nobjs] = (int)ftell(pd->pdffp);
        fprintf(pd->pdffp, "%d 0 obj\n<<\n/Type /ExtGState\n/ca %1.3f\n>>\nendobj\n", pd->nobjs,
                1 - pd->fillAlpha[i] / 255.0);
    }
...
static void PDF_Line(double x1, double y1, double x2, double y2, R_GE_gcontext *gc, NewDevDesc *dd)
{
     ...
     * Only try to do real transparency if version at least 1.4
     */
    if ((semiTransparent(gc->col) && alphaVersion(pd)) || (R_ALPHA(gc->col) == 0))
    {
        PDF_SetLineColor(gc->col, dd);
        PDF_SetLineStyle(gc->lty, gc->lwd, dd);
        if (pd->inText)
...
static void PDF_Text(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc, NewDevDesc *dd)
{
     ...
     * Only try to do real transparency if version at least 1.4
     */
    if ((pd->versionMajor >= 1 && pd->versionMinor >= 4) || (R_ALPHA(gc->col) == 0))
    {
