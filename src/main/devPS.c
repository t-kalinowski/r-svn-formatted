...
static int LoadEncoding(char *encpath, char *encname, char *enccode, Rboolean isPDF)
{
    char buf[BUFSIZE];
    int i;
    FILE *fp;
...
static void PSEncodeFont(FILE *fp, char *encname, PostScriptDesc *pd)
{
    ...
    for (j = 0; j < pd->nfonts; j++)
    {
        ...
        if (fontencpath)
        {
            ...
            else if (strcmp(fontencname, "ISOLatin1Encoding"))
            {
                fprintf(fp, "%% begin encoding\n%s def\n%% end encoding\n", encoding);
            }
        }
...
static void PSFileHeader(FILE *fp, char *encname, char *papername, double paperwidth, double paperheight,
                         ...
                         double right, double top, char *title, PostScriptDesc *pd)
{
    int i, j;
    SEXP prolog;
 ...
 * font database
 */
static char *fontsMetricsFileName(int fontIndex, int faceIndex, PostScriptDesc *pd)
{
    SEXP graphicsNS, PSenv, fontdb, fontnames;
    int i, nfonts;
    char *result = NULL;
...
static Rboolean PS_Open(NewDevDesc *dd, PostScriptDesc *pd)
{
    ...
    if (pd->nfonts > 0)
    {
        char **fontfamilynames;
        FontMetricInfo *fontsMetrics;
        if (!(fontsMetrics = (FontMetricInfo *)malloc(pd->nfonts * 5 * sizeof(FontMetricInfo))))
        {
            fontsMetrics = NULL;
        ...
        for (j = 0; j < pd->nfonts; j++)
        {
            for (i = 0; i < 5; i++)
            {
                ...
                if (!afmpath)
                    return FALSE;
                if (!PostScriptLoadFontMetrics(afmpath, &(pd->fontsMetrics[j * 5 + i]), pd->fontfamilynames[j * 5 + i],
                                               (i < 4) ? 1 : 0))
                {
    ...
    if (pd->landscape)
        PSFileHeader(pd->psfp, pd->encname, pd->papername, pd->paperwidth, pd->paperheight, pd->landscape,
                     !(pd->onefile), pd->paperspecial, dd->bottom, dd->left, dd->top, dd->right, pd->title, pd);
    else
        PSFileHeader(pd->psfp, pd->encname, pd->papername, pd->paperwidth, pd->paperheight, pd->landscape,
                     !(pd->onefile), pd->paperspecial, dd->left, dd->bottom, dd->right, dd->top, pd->title, pd);
...
static double PS_StrWidth(char *str, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    if (face < 1 || face > 5)
        face = 1;
    return floor(gc->cex * gc->ps + 0.5) * PostScriptStringWidth((unsigned char *)str, &(pd->metrics[face - 1]));
}
...
static double PDF_StrWidth(char *str, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    return floor(gc->cex * gc->ps + 0.5) *
           PostScriptStringWidth((unsigned char *)str, &(pd->metrics[gc->fontface - 1]));
}
