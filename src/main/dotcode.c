        ...
        */
        ...
        if (!*fun && !(*fun = R_FindSymbol(buf, dll.DLLname, symbol)))
        {
            if (strlen(dll.DLLname))
                errorcall(call, _("%s symbol name \"%s\" not in DLL for package \"%s\""),
                          symbol->type == R_FORTRAN_SYM ? "Fortran" : "C", buf, dll.DLLname);
            else
                        ...
                        int targetType, char *encname)
{
        ...
        else
        {
            ...
            if (strlen(encname))
            {
                ...
                restart_in:
                    ...
                    if (res == -1)
                        error(_("conversion problem in re-encoding to '%s'"), encname);
                    *outbuf = '\0';
                }
                Riconv_close(obj);
            }
    ...
    case LISTSXP:
        if (Fort)
            error(_("invalid mode to pass to Fortran (arg %d)"), narg);
        ...
        else
        {
            ...
            if (strlen(encname))
            {
                ...
                restart_out:
                    ...
                    if (res == -1)
                        error(_("conversion problem in re-encoding from '%s'"), encname);
                    *outbuf = '\0';
...
static SEXP enctrim(SEXP args, char *name, int len)
{
    ...
    for (s = args; s != R_NilValue;)
    {
        ...
        if (ss == R_NilValue && TAG(s) == EncSymbol)
        {
            sx = CAR(s);
            if (pkgused++ == 1)
                warning(_("ENCODING used more than once"));
...
SEXP attribute_hidden do_symbol(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    warningcall(call, _("'%s' is deprecated"), PRIMVAL(op) ? "symbol.For" : "symbol.C");
...
SEXP attribute_hidden do_isloaded(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    {
        if (!(R_FindSymbol(sym, pkg, NULL)) && !(R_FindSymbol(sym, pkg, &symbol)))
            val = 0;
    }
...
SEXP attribute_hidden do_External(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    args = resolveNativeRoutine(args, (DL_FUNC *)&fun, &symbol, buf, NULL, NULL, NULL, call);
...
SEXP attribute_hidden do_dotCode(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    args = resolveNativeRoutine(args, &ofun, &symbol, symName, &nargs, &naok, &dup, call);
    fun = (VarFun)ofun;
            ...
            char **results)
{
    ...
    case STRSXP:
        if (nres > 0)
            results[0] = (char *)RObjToCPtr(s, 1, 1, 0, 0, (const char *)NULL, NULL, 0, "");
        break;
    case VECSXP:
        n = length(s);
        if (nres < n)
        ...
        for (i = 0; i < n; i++)
        {
            results[i] = (char *)RObjToCPtr(VECTOR_ELT(s, i), 1, 1, 0, 0, (const char *)NULL, NULL, 0, "");
        }
        break;
    case LISTSXP:
        n = length(s);
        ...
        for (i = 0; i < n; i++)
        {
            results[i] = (char *)RObjToCPtr(s, 1, 1, 0, 0, (const char *)NULL, NULL, 0, "");
            s = CDR(s);
        }
        break;
    }
