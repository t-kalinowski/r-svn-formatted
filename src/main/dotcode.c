...
static void *RObjToCPtr(SEXP ans, int naok, int dup, int narg, int Fort, const char *name, R_toCConverter **converter,
                        int targetType)
{
    ...
    case VECSXP:
        if (Fort)
            error(_("invalid mode to pass to Fortran (arg %d)"), narg + 1);
        if (!dup)
...
SEXP attribute_hidden do_dotCode(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    */
    ...
    for (na = 0, pa = args; pa != R_NilValue; pa = CDR(pa), na++)
    {
        ...
        cargs[na] = RObjToCPtr(ans, naok, dup, na, Fort, symName, argConverters + na, checkTypes ? checkTypes[na] : 0);
    ...
    if (dup)
    {
        ...
        for (na = 0, pa = args; pa != R_NilValue; pa = CDR(pa), na++)
        {
            ...
            else
            {
                void *p = cargs[na];
                SEXP arg = CAR(pa);
                R_NativePrimitiveArgType type = checkTypes ? checkTypes[na] : TYPEOF(CAR(pa));
                int n = length(arg);
