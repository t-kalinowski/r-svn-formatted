...
static int do_unzip(char *zipname, char *dest, int nfiles, char **files)
{
        ...
        for (i = 0; i < gi.number_entry; i++)
        {
            if (i > 0)
                if (unzGoToNextFile(uf) != UNZ_OK)
                    break;
            if (extract_one(uf, dest, NULL) != UNZ_OK)
                break;
        }
    }
    else
    {
        for (i = 0; i < nfiles; i++)
        {
            if (unzLocateFile(uf, files[i], 0) != UNZ_OK)
                break;
            if (extract_one(uf, dest, files[i]) != UNZ_OK)
                break;
        }
        unzCloseCurrentFile(uf);
        if (err != UNZ_OK)
...
*/
static unzFile unzOpen(const char *path)
{
    unz_s us;
    unz_s *s;
...
*/
static int unzlocal_GetCurrentFileInfoInternal OF(
...
/*
  ...
  return UNZ_OK if there is no problem.
*/
static int unzGetCurrentFileInfo(unzFile file, unz_file_info *pfile_info, char *szFileName, uLong fileNameBufferSize,
...
*/
static int unzLocateFile(unzFile file, const char *szFileName, int iCaseSensitivity)
{
    unz_s *s;
    int err;
...
*/
static int unzReadCurrentFile(unzFile file, voidp buf, unsigned int len)
{
    int err = UNZ_OK;
    uInt iRead = 0;
    unz_s *s;
