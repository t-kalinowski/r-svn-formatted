...
static void setFileTime(const char *fn, uLong dosdate)
{
    ...
    hFile = CreateFileA(fn, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
...
static int extract_one(unzFile uf, const char *const dest, const char *const filename, SEXP names, int *nnames,
                       int overwrite, int junk, int setTime)
{
    int err = UNZ_OK;
    FILE *fout;
...
#ifdef Win32
    if (setTime)
        setFileTime(outname, file_info.dosDate);
#else
    if (setTime)
        setFileTime(outname, file_info.tmu_date);
#endif
    return err;
}
                    ...
                    int overwrite, int junk, int setTime)
{
    ...
    { /* all files */
        ...
        for (i = 0; i < gi.number_entry; i++)
        {
            ...
            if ((err = extract_one(uf, dest, NULL, names, nnames, overwrite, junk, setTime)) != UNZ_OK)
                break;
#ifdef Win32
            R_ProcessEvents();
#else
    ...
    else
    {
        ...
        for (i = 0; i < nfiles; i++)
        {
            if ((err = unzLocateFile(uf, files[i], 1)) != UNZ_OK)
                break;
            if ((err = extract_one(uf, dest, files[i], names, nnames, overwrite, junk, setTime)) != UNZ_OK)
                break;
#ifdef Win32
            R_ProcessEvents();
#else
...
static SEXP ziplist(const char *zipname)
{
    ...
    for (i = 0; i < nfiles; i++)
    {
        ...
        snprintf(date, 50, "%d-%02d-%02d %02d:%02d", file_info.tmu_date.tm_year, file_info.tmu_date.tm_mon + 1,
                 file_info.tmu_date.tm_mday, file_info.tmu_date.tm_hour, file_info.tmu_date.tm_min);
...
local unzFile unzOpenInternal(const void *path, int is64bitOpenFunction)
{
    ...
    us.filestream = fopen_func(path, ZLIB_FILEFUNC_MODE_READ | ZLIB_FILEFUNC_MODE_EXISTING);
                                                ...
                                                char *szComment, uLong commentBufferSize)
{
    ...
    if (file == NULL)
        return UNZ_PARAMERROR;
    s = (unz64_s *)file;
    if (fseek_func(s->filestream, s->pos_in_central_dir + s->byte_before_the_zipfile, ZLIB_FILEFUNC_SEEK_SET) != 0)
        err = UNZ_ERRNO;
    ...
    if ((err == UNZ_OK) && (extraField != NULL))
    {
        ...
        if ((file_info.size_file_extra > 0) && (extraFieldBufferSize > 0))
            if (fread_func(s->filestream, extraField, (size_t)uSizeRead) != uSizeRead)
                err = UNZ_ERRNO;
    ...
    if ((err == UNZ_OK) && (file_info.size_file_extra != 0))
    {
        ...
        while (acc < file_info.size_file_extra)
        {
            ...
            if (headerId == 0x0001)
            {
                ...
                {
                    if (unz64local_getLong64(s->filestream, &file_info.uncompressed_size) != UNZ_OK)
                        err = UNZ_ERRNO;
                }

                if (file_info.compressed_size == (ZPOS64_T)(unsigned long)-1)
                {
                    if (unz64local_getLong64(s->filestream, &file_info.compressed_size) != UNZ_OK)
                        err = UNZ_ERRNO;
                }
                ...
                if (file_info_internal.offset_curfile == (ZPOS64_T)(unsigned long)-1)
                {
                    ...
                    if (unz64local_getLong64(s->filestream, &file_info_internal.offset_curfile) != UNZ_OK)
                        err = UNZ_ERRNO;
                }
                                                     ...
                                                     uInt *psize_local_extrafield)
{
    ...
    *psize_local_extrafield = 0;
    ...
    if (fseek_func(s->filestream, s->cur_file_info_internal.offset_curfile + s->byte_before_the_zipfile,
...
*/
static int unzOpenCurrentFile3(unzFile file, int *method, int *level, int raw, const char *password)
{
    int err = UNZ_OK;
    uInt iSizeVar;
    unz64_s *s;
    ...
    while (pfile_in_zip_read_info->stream.avail_out > 0)
    {
        if ((pfile_in_zip_read_info->stream.avail_in == 0) && (pfile_in_zip_read_info->rest_read_compressed > 0))
        {
            ...
            if (fseek_func(pfile_in_zip_read_info->filestream,
                           ...
                           ZLIB_FILEFUNC_SEEK_SET) != 0)
                return UNZ_ERRNO;
            if (fread_func(pfile_in_zip_read_info->filestream, pfile_in_zip_read_info->read_buffer, uReadThis) !=
                uReadThis)
                return UNZ_ERRNO;
        ...
        else if (pfile_in_zip_read_info->compression_method == Z_BZIP2ED)
        {
            ...
            pfile_in_zip_read_info->bstream.total_in_lo32 = (unsigned int)pfile_in_zip_read_info->stream.total_in;
            pfile_in_zip_read_info->bstream.total_in_hi32 = 0;
            pfile_in_zip_read_info->bstream.next_out = (char *)pfile_in_zip_read_info->stream.next_out;
            pfile_in_zip_read_info->bstream.avail_out = pfile_in_zip_read_info->stream.avail_out;
            pfile_in_zip_read_info->bstream.total_out_lo32 = (unsigned int)pfile_in_zip_read_info->stream.total_out;
