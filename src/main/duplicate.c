...
unsigned long attribute_hidden get_duplicate_counter(void)
{
...
SEXP duplicate(SEXP s)
{
    ...
    if (RTRACE(s) && !(TYPEOF(s) == CLOSXP || TYPEOF(s) == BUILTINSXP || TYPEOF(s) == SPECIALSXP ||
                       TYPEOF(s) == PROMSXP || TYPEOF(s) == ENVSXP))
    {
        memtrace_report(s, t);
        SET_RTRACE(t, 1);
    }
...
static SEXP duplicate1(SEXP s, Rboolean deep)
{
    ...
    { /* surely it only makes sense in this case*/
...
SEXP R_shallow_duplicate_attr(SEXP x)
{
    return duplicate_attr(x, FALSE);
}
SEXP R_duplicate_attr(SEXP x)
{
    return duplicate_attr(x, TRUE);
}
