...
void GEregisterSystem(GEcallback cb, int *systemRegisterIndex)
{
    ...
    *systemRegisterIndex = 0;
     ...
     */
    registeredSystems[*systemRegisterIndex] = (GESystemDesc *)calloc(1, sizeof(GESystemDesc));
...
R_GE_lineend GE_LENDpar(SEXP value, int ind)
{
    ...
    if (isString(value))
    {
        ...
        error(_("invalid line end")); /*NOTREACHED, for -Wall : */
        return 0;
    }
...
R_GE_linejoin GE_LJOINpar(SEXP value, int ind)
{
    ...
    if (isString(value))
    {
        ...
        error(_("invalid line join")); /*NOTREACHED, for -Wall : */
        return 0;
    }
    else if (isInteger(value))
    {
...
static void getClipRect(double *x1, double *y1, double *x2, double *y2, pGEDevDesc dd)
{
...
static void getClipRectToDevice(double *x1, double *y1, double *x2, double *y2, pGEDevDesc dd)
{
    double factor = 4;
    double dx, dy, d;
...
/* Clip the line
   ...
   dd->dev->gp.xpd) */
static Rboolean clipLine(double *x1, double *y1, double *x2, double *y2, int toDevice, pGEDevDesc dd)
{
    int dummy1, dummy2;
    cliprect cr;
...
static void CScliplines(int n, double *x, double *y, const pGEcontext gc, int toDevice, pGEDevDesc dd)
{
    int ind1, ind2;
    /*int firstPoint = 1;*/
    int count = 0;
...
static void clipPolyline(int n, double *x, double *y, const pGEcontext gc, int clipToDevice, pGEDevDesc dd)
{
...
static int clipPoly(double *x, double *y, int n, int store, int toDevice, double *xout, double *yout, pGEDevDesc dd)
{
    int i, cnt = 0;
    GClipState cs[4];
    GClipRect clip;
...
static Rboolean mustClip(double xmin, double xmax, double ymin, double ymax, int toDevice, pGEDevDesc dd)
{
    GClipRect clip;
    if (toDevice)
        getClipRectToDevice(&clip.xmin, &clip.ymin, &clip.xmax, &clip.ymax, dd);
        ...
        getClipRect(&clip.xmin, &clip.ymin, &clip.xmax, &clip.ymax, dd);
    return (clip.xmin > xmin || clip.xmax < xmax || clip.ymin > ymin || clip.ymax < ymax);
}
...
static void reorderVertices(int n, double *x, double *y, pGEDevDesc dd)
{
    ...
    if (n < 2 || x[0] < xmin || x[0] > xmax || y[0] < ymin || y[0] > ymax)
    {
        return;
    }
    else
    {
        ...
        while (start < n && x[start] >= xmin && x[start] <= xmax && y[start] >= ymin && y[start] <= ymax)
        {
            start++;
        }
        if (start == n)
...
static int clipCircleCode(double x, double y, double r, int toDevice, pGEDevDesc dd)
{
    int result;
...
static int clipRectCode(double x0, double y0, double x1, double y1, int toDevice, pGEDevDesc dd)
{
    int result;
    /* determine clipping region */
    double xmin, xmax, ymin, ymax;
              ...
              Rboolean interpolate, const pGEcontext gc, pGEDevDesc dd)
{
    ...
    if (width != 0 && height != 0)
    {
        dd->dev->raster(raster, w, h, x, y, width, height, angle, interpolate, gc, dd->dev);
    }
}
...
static int clipTextCode(double x, double y, const char *str, cetype_t enc, double width, double height, double rot,
                        double hadj, const pGEcontext gc, int toDevice, pGEDevDesc dd)
{
    ...
    if (!R_FINITE(height))
        height = GEStrHeight(str, enc, gc, dd);
    ...
    return clipRectCode(toDeviceX(left, GE_INCHES, dd), toDeviceY(bottom, GE_INCHES, dd),
                        toDeviceX(right, GE_INCHES, dd), toDeviceY(top, GE_INCHES, dd), toDevice, dd);
}

static void clipText(double x, double y, const char *str, cetype_t enc, double width, double height, double rot,
                     double hadj, const pGEcontext gc, int toDevice, pGEDevDesc dd)
{
    int result;
    void (*textfn)(double x, double y, const char *str, double rot, double hadj, const pGEcontext gc, pDevDesc dd);
    ...
    {
        result = clipTextCode(x, y, str, enc, width, height, rot, hadj, gc, toDevice, dd);
        switch (result)
        {
...
static int VFontFamilyCode(char *fontfamily)
{
    if (strlen(fontfamily) > 7)
    {
        ...
        if (!strncmp(fontfamily, "Hershey", 7) && j < 9)
            return 100 + j;
...
static int VFontFaceCode(int familycode, int fontface)
{
    ...
    if (!(face >= VFontTable[familycode].minface && face <= VFontTable[familycode].maxface))
    {
        ...
        default:
             ...
             */
            error(_("font face %d not supported for font family '%s'"), fontface, VFontTable[familycode].name);
        }
    }
    return face;
}
            ...
            for (s = str;; s++)
            {
                if (*s == '\n' || *s == '\0')
                {
                    ...
                    *sb = '\0';
                    ...
                    clipText(xleft, ybottom, str, enc2, w, h, rot, hadj, gc, dd->dev->canClip, dd);
                    sb = sbuf;
                    i++;
                }
                else
...
void GEMetricInfo(int c, const pGEcontext gc, double *ascent, double *descent, double *width, pGEDevDesc dd)
{
    ...
    else
    {
        ...
        */
        ...
        static double last_cex = 0.0, last_ps = 0.0, a = 0.0, d = 0.0, w = 0.0;
        static char last_family[201];
        if (dd == last_dd && dd->dev->close == last_close && abs(c) == 77 && gc->cex == last_cex && gc->ps == last_ps &&
 ...
 */
void GEStrMetric(const char *str, cetype_t enc, const pGEcontext gc, double *ascent, double *descent, double *width,
                 pGEDevDesc dd)
{
    /*
     * If the fontfamily is a Hershey font family, call R_GE_VStrHeight
    ...
    *width = 0.0;
    ...
    else
    {
         ...
         * the current font pointsize in device coordinates.
         */
        double lineheight = gc->lineheight * gc->cex * dd->dev->cra[1] * gc->ps / dd->dev->startps;
        int n;
        char *sb, *sbuf;
        cetype_t enc2;
        int noMetricInfo;
        ...
        *sb = '\0';
        ...
        else
        {
            ...
            else
            {
                while (*s != '\0')
                {
                    GEMetricInfo((unsigned char)*s++, gc, &asc, &dsc, &wid, dd);
                    if (asc > *ascent)
                        *ascent = asc;
                }
            }
...
void GEplaySnapshot(SEXP snapshot, pGEDevDesc dd)
{
    ...
    PROTECT(snapshotEngineVersion = getAttrib(snapshot, install("engineVersion")));
...
int GEstring_to_pch(SEXP pch)
{
    ...
    else if (mbcslocale)
    {
         ...
         */
        unsigned int ucs = 0;
        if ((int)mbtoucs(&ucs, CHAR(pch), R_MB_CUR_MAX) > 0)
            ipch = ucs;
        else
            error(_("invalid multibyte char in pch=\"c\""));
        if (ipch > 127)
...
unsigned int GE_LTYpar(SEXP value, int ind)
{
    ...
    {
        error(_("invalid line type")); /*NOTREACHED, for -Wall : */
        return 0;
    }
}
 ...
 */
void R_GE_rasterResizeForRotation(unsigned int *sraster, int w, int h, unsigned int *newRaster, int wnew, int hnew,
                                  const pGEcontext gc)
{
    int i, j, inew, jnew;
    int xoff = (wnew - w) / 2;
 ...
 */
void R_GE_rasterRotate(unsigned int *sraster, int w, int h, double angle, unsigned int *draster, const pGEcontext gc,
                       Rboolean smoothAlpha)
{
    ...
    for (i = 0; i < h; i++)
    {
        ...
        for (j = 0; j < w; j++)
        {
            ...
            word01 = *(sline + w + xp);
            word11 = *(sline + w + xp + 1);
            rval = ((16 - xf) * (16 - yf) * R_RED(word00) + xf * (16 - yf) * R_RED(word10) +
                    (16 - xf) * yf * R_RED(word01) + xf * yf * R_RED(word11) + 128) /
                   256;
            gval = ((16 - xf) * (16 - yf) * R_GREEN(word00) + xf * (16 - yf) * R_GREEN(word10) +
                    (16 - xf) * yf * R_GREEN(word01) + xf * yf * R_GREEN(word11) + 128) /
                   256;
            bval = ((16 - xf) * (16 - yf) * R_BLUE(word00) + xf * (16 - yf) * R_BLUE(word10) +
                    (16 - xf) * yf * R_BLUE(word01) + xf * yf * R_BLUE(word11) + 128) /
                   256;
            if (smoothAlpha)
            {
                aval = ((16 - xf) * (16 - yf) * R_ALPHA(word00) + xf * (16 - yf) * R_ALPHA(word10) +
                        (16 - xf) * yf * R_ALPHA(word01) + xf * yf * R_ALPHA(word11) + 128) /
                       256;
            }
            else
            {
                aval = (int)fmax2(fmax2(R_ALPHA(word00), R_ALPHA(word10)), fmax2(R_ALPHA(word01), R_ALPHA(word11)));
            }
