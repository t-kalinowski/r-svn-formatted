 ...
 */
SEXP attribute_hidden Rf_findVar1w(SEXP symbol, SEXP rho, SEXPTYPE mode, int inherits)
{
    SEXP vl;
    while (rho != R_BaseEnv && rho != R_EmptyEnv)
    {
...
SEXP attribute_hidden do_remove(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < LENGTH(name); i++)
    {
        ...
        if (!done)
            warning(_("remove: variable \"%s\" was not found"), CHAR(PRINTNAME(tsym)));
    }
    return R_NilValue;
}
...
SEXP attribute_hidden do_importIntoEnv(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            if (SYMVALUE(expsym) == R_UnboundValue)
                errorcall(call, _("exported symbol '%s' has no value"), CHAR(PRINTNAME(expsym)));
            val = SYMVALUE(expsym);
        }
        else
            val = CAR(binding);
