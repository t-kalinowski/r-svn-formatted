...
{
    VECTOR(table)[hashcode % HASHSIZE(table)] = DeleteItem(symbol, VECTOR(table)[hashcode % HASHSIZE(table)]);
    return;
}
...
SEXP R_HashFrame(SEXP rho)
{
    ...
    while (!isNull(frame))
    {
        ...
        {
            HASHVALUE(PRINTNAME(TAG(frame))) = R_Newhashpjw(CHAR(PRINTNAME(TAG(frame))));
