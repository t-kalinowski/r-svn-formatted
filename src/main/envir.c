    ...
    { /* have get(.) */
        if (rval == R_UnboundValue)
        {
            if (gmode == ANYSXP)
                ...
                errorcall(call, "variable \"%s\" of mode \"%s\" was not found", CHAR(PRINTNAME(t1)),
...
static SEXP checkNSname(SEXP call, SEXP name)
{
    ...
    /* else fall through */
    default:
        errorcall(call, "bad name space name");
    }
    return name;
}
...
SEXP do_importIntoEnv(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        for (env = expenv, binding = R_NilValue; env != R_NilValue && binding == R_NilValue; env = ENCLOS(env))
            if (env == R_BaseNamespace)
            {
        ...
        {
            if (SYMVALUE(expsym) == R_UnboundValue)
                errorcall(call, "exported symbol '%s' has no value", CHAR(PRINTNAME(expsym)));
            val = SYMVALUE(expsym);
        }
        else
            val = CAR(binding);
