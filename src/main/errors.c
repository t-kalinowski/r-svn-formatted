...
attribute_hidden void NORET R_signalErrorConditionEx(SEXP cond, SEXP call, int exitOnly)
{
...
attribute_hidden void NORET R_signalErrorCondition(SEXP cond, SEXP call)
{
...
attribute_hidden SEXP R_makeNotSubsettableError(SEXP x, SEXP call)
{
    ...
    PROTECT(cond);
    Rsnprintf_mbcs(emsg_buf, BUFSIZE - 1, R_MSG_ob_nonsub, type2char(TYPEOF(x)));
...
static SEXP R_makeStackOverflowError(const char *type, SEXP call, intptr_t usage)
{
    int csize = 2;
    const char *msg;
    ...
    if (strcmp(type, "CStackOverflowError") == 0)
    {
        ...
        error("unknown stack overflow error: %s", type);
...
attribute_hidden void R_InitConditions()
{
    R_protectStackOverflowError = R_makeStackOverflowError("protectStackOverflowError", R_NilValue, 0);
    MARK_NOT_MUTABLE(R_protectStackOverflowError);
    R_PreserveObject(R_protectStackOverflowError);

    R_expressionStackOverflowError = R_makeStackOverflowError("expressionStackOverflowError", R_NilValue, 0);
    MARK_NOT_MUTABLE(R_expressionStackOverflowError);
    R_PreserveObject(R_expressionStackOverflowError);

    R_nodeStackOverflowError = R_makeStackOverflowError("nodeStackOverflowError", R_NilValue, 0);
