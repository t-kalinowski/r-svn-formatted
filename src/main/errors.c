...
void error(const char *format, ...)
{
    ...
    errorcall(R_GlobalContext ? R_GlobalContext->call : R_NilValue, "%s", buf);
}
...
#ifdef NEW_CONDITION_HANDLING
static SEXP mkHandlerEntry(SEXP class, SEXP parentenv, SEXP handler, SEXP rho, SEXP result, int calling)
{
...
SEXP do_addCondHands(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = n - 1; i >= 0; i--)
    {
        ...
        SEXP entry = mkHandlerEntry(class, parentenv, handler, target, result, calling);
...
static SEXP findSimpleExceptionHandler()
{
    ...
    {
        SEXP entry = CAR(list);
        if (!strcmp(CHAR(ENTRY_CLASS(entry)), "simpleException") || !strcmp(CHAR(ENTRY_CLASS(entry)), "exception") ||
...
static void vsignalWarning(SEXP call, const char *format, va_list ap)
{
    ...
    if (SYMVALUE(hooksym) != R_UnboundValue && SYMVALUE(quotesym) != R_UnboundValue)
    {
        ...
        if (IS_CALLING_ENTRY(entry))
        {
            ...
            if (h == R_RestartToken)
            {
                ...
                if (TYPEOF(msg) == STRSXP && LENGTH(msg) > 0)
                    ...
                    error("error message not a strring");
