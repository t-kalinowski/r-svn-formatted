...
static void doprof(int sig) /* sig is ignored in Windows */
{
    ...
    if (R_Mem_Profiling)
    {
        ...
        if ((len = strlen(buf)) < PROFLINEMAX)
            snprintf(buf + len, PROFBUFSIZ - len, ":%lu:%lu:%lu:%lu:", (unsigned long)smallv, (unsigned long)bigv,
    ...
    for (RCNTXT *cptr = R_GlobalContext; cptr != NULL; cptr = findProfContext(cptr))
    {
        if ((cptr->callflag & (CTXT_FUNCTION | CTXT_BUILTIN)) && TYPEOF(cptr->call) == LANGSXP)
        {
            ...
            if (strlen(buf) < PROFLINEMAX)
            {
                ...
                else if ((CAR(fun) == R_DoubleColonSymbol || CAR(fun) == R_TripleColonSymbol ||
                          CAR(fun) == R_DollarSymbol) &&
                         TYPEOF(CADR(fun)) == SYMSXP && TYPEOF(CADDR(fun)) == SYMSXP)
                {
                    ...
                    snprintf(itembuf, PROFITEMMAX - 1, "%s%s%s", CHAR(PRINTNAME(CADR(fun))), CHAR(PRINTNAME(CAR(fun))),
                             CHAR(PRINTNAME(CADDR(fun))));
                }
                else if (CAR(fun) == R_Bracket2Symbol && TYPEOF(CADR(fun)) == SYMSXP &&
                         ((TYPEOF(CADDR(fun)) == SYMSXP || TYPEOF(CADDR(fun)) == STRSXP ||
                           TYPEOF(CADDR(fun)) == INTSXP || TYPEOF(CADDR(fun)) == REALSXP) &&
                          length(CADDR(fun)) > 0))
                {
                    ...
                    snprintf(itembuf, PROFITEMMAX - 1, "%s[[%s]]", CHAR(PRINTNAME(arg1)), arg2buf);
                }
                else
                {
...
static void R_InitProfiling(SEXP filename, int append, double dinterval, int mem_profiling, int gc_profiling,
                            int line_profiling, int filter_callframes, int numfiles, int bufsize)
{
#ifndef Win32
    struct itimerval itv;
#else
    ...
    DuplicateHandle(Proc, GetCurrentThread(), Proc, &MainThread, 0, FALSE, DUPLICATE_SAME_ACCESS);
    wait = interval / 1000;
...
{
    SEXP filename;
    int append_mode, mem_profiling, gc_profiling, line_profiling, filter_callframes;
    double dinterval;
    ...
    args = CDR(args);
    append_mode = asLogical(CAR(args));
    args = CDR(args);
    dinterval = asReal(CAR(args));
    args = CDR(args);
    mem_profiling = asLogical(CAR(args));
    args = CDR(args);
    gc_profiling = asLogical(CAR(args));
    args = CDR(args);
    line_profiling = asLogical(CAR(args));
    args = CDR(args);
    filter_callframes = asLogical(CAR(args));
    args = CDR(args);
    numfiles = asInteger(CAR(args));
    args = CDR(args);
    if (numfiles < 0)
    ...
    if (LENGTH(filename))
        R_InitProfiling(filename, append_mode, dinterval, mem_profiling, gc_profiling, line_profiling,
                        filter_callframes, numfiles, bufsize);
    else
        R_EndProfiling();
    return R_NilValue;
}
    ...
    if (PRVALUE(e) == R_UnboundValue)
    {
        ...
        if (PRSEEN(e))
        {
            ...
            else
            {
                ...
                SET_PRSEEN(e, 1);
                warningcall(R_GlobalContext->call, _("restarting interrupted promise evaluation"));
            }
        }
...
SEXP eval(SEXP e, SEXP rho)
{
    ...
    case SYMSXP:
        ...
        else if (tmp == R_MissingArg && !DDVAL(e))
        {
            ...
            if (*n)
                error(_("argument \"%s\" is missing, with no default"), CHAR(PRINTNAME(e)));
            else
        ...
        else if (TYPEOF(tmp) == PROMSXP)
        {
            ...
            ENSURE_NAMED(tmp); /* needed for .Last.value - LT */
        break;
    case PROMSXP:
        if (PRVALUE(e) == R_UnboundValue)
...
static struct
{
    unsigned long count, envcount, bdcount;
} jit_info = {0, 0, 0};
...
static int JIT_score(SEXP e)
{
    if (TYPEOF(e) == LANGSXP)
    {
        ...
        else if (fun == R_ForSymbol || fun == R_WhileSymbol || fun == R_RepeatSymbol)
...
static R_INLINE Rboolean R_CheckJIT(SEXP fun)
{
    ...
    if (jit_strategy < 0)
    {
        int dflt = R_jit_enabled == 1 ? STRATEGY_NO_SMALL : STRATEGY_TOP_SMALL_MAYBE;
        int val = dflt;
    ...
    if (R_jit_enabled > 0 && TYPEOF(body) != BCODESXP && !R_disable_bytecode && !NOJIT(fun))
    {
...
{
    if (cmpenv != top)
        for (SEXP frame = FRAME(cmpenv); frame != R_NilValue; frame = CDR(frame))
            if (TAG(frame) == sym)
                return TRUE;
    return FALSE;
}
            ...
            if (jit_env_match(jit_cache_env(entry), fun))
            {
                ...
                if (jit_expr_match(jit_cache_expr(entry), BODY(fun)))
                {
                    ...
                    if (getAttrib(BODY(fun), R_SrcrefSymbol) != R_NilValue ||
                        jit_srcref_match(jit_cache_srcref(entry), getAttrib(fun, R_SrcrefSymbol)))
                    {
                        PRINT_JIT_INFO;
...
{
    SEXP result;
    if (!isNull(srcrefs) && length(srcrefs) > ind && !isNull(result = VECTOR_ELT(srcrefs, ind)) &&
        TYPEOF(result) == INTSXP && length(result) >= 6)
        return result;
    else
        return R_NilValue;
}
...
static int countCycleRefs(SEXP rho, SEXP val)
{
    ...
    int crefs = 0;
    for (SEXP b = FRAME(rho); b != R_NilValue && REFCNT(b) == 1; b = CDR(b))
    {
        if (BNDCELL_TAG(b))
            continue;
        SEXP v = CAR(b);
...
static R_INLINE SEXP R_execClosure(SEXP call, SEXP newrho, SEXP sysparent, SEXP rho, SEXP arglist, SEXP op);
...
#ifdef ADJUST_ENVIR_REFCNTS
    Rboolean is_getter_call = (CADR(call) == R_TmpvalSymbol && !R_isReplaceSymbol(CAR(call)));
#endif
...
static R_INLINE Rboolean asLogicalNoNA(SEXP s, SEXP call, SEXP rho)
{
    ...
    if (len > 1)
    {
        ...
        R_BadValueInRCode(s, call, rho, "the condition has length > 1", _("the condition has length > 1"),
                          _("the condition has length > 1 and only the first element will be used"),
                          "_R_CHECK_LENGTH_1_CONDITION_", TRUE /* by default issue warning */);
    }
    if (len > 0)
    {
    ...
    if (cond == NA_LOGICAL)
    {
        char *msg = len ? (isLogical(s) ? _("missing value where TRUE/FALSE needed")
...
#define ALLOC_LOOP_VAR(v, val_type, vpi)                                                                               \
    do                                                                                                                 \
    {                                                                                                                  \
        if (v == R_NilValue || MAYBE_SHARED(v) || ATTRIB(v) != R_NilValue || (v) != CAR(cell))                         \
        {                                                                                                              \
...
static R_INLINE SEXP GET_BINDING_CELL(SEXP symbol, SEXP rho)
{
    ...
    else
    {
        ...
        return (!R_VARLOC_IS_NULL(loc) && !IS_ACTIVE_BINDING(loc.cell)) ? loc.cell : R_NilValue;
    }
}
...
SEXP attribute_hidden do_for(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    dbg = RDEBUG(rho);
    if (R_jit_enabled > 2 && !dbg && !R_disable_bytecode && rho == R_GlobalEnv && isUnmodifiedSpecSym(CAR(call), rho) &&
...
static SEXP evalseq(SEXP expr, SEXP rho, int forcelocal, R_varloc_t tmploc, R_varloc_t *ploc)
{
    ...
    if (isSymbol(expr))
    { /* now we are down to the target symbol */
        PROTECT(expr);
        if (forcelocal)
        {
        ...
        int maybe_in_assign = ploc->cell ? ASSIGNMENT_PENDING(ploc->cell) : FALSE;
        if (ploc->cell)
    ...
    else if (isLanguage(expr))
    {
        ...
        if (MAYBE_REFERENCED(nval) && (MAYBE_SHARED(nval) || MAYBE_SHARED(CAR(val))))
            nval = shallow_duplicate(nval);
        UNPROTECT(4);
...
static R_INLINE SEXP try_assign_unwrap(SEXP value, SEXP sym, SEXP rho, SEXP cell)
{
    ...
    else
    {
        ...
        if (!MAYBE_SHARED(value))
        {
            ...
            if (!IS_ACTIVE_BINDING(cell) && value == BINDING_VALUE(cell))
                return R_tryUnwrap(value);
        }
    }
#endif
...
static SEXP applydefine(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    R_varloc_t lhsloc;
    lhs = evalseq(CADR(expr), rho, PRIMVAL(op) == 1 || PRIMVAL(op) == 3, tmploc, &lhsloc);
    if (lhsloc.cell == NULL)
    ...
    while (isLanguage(CADR(expr)))
    {
        ...
        else
        {
            ...
            if (TYPEOF(CAR(expr)) == LANGSXP &&
                (CAR(CAR(expr)) == R_DoubleColonSymbol || CAR(CAR(expr)) == R_TripleColonSymbol) &&
 ...
 */
attribute_hidden int DispatchAnyOrEval(SEXP call, SEXP op, const char *generic, SEXP args, SEXP rho, SEXP *ans,
                                       int dropmissing, int argsevald)
{
    if (R_has_methods(op))
    {
 ...
 */
attribute_hidden int DispatchOrEval(SEXP call, SEXP op, const char *generic, SEXP args, SEXP rho, SEXP *ans,
                                    int dropmissing, int argsevald)
{
        ...
        if (IS_S4_OBJECT(x) && R_has_methods(op))
        {
            ...
            else
            {
                ...
                */
                ...
                {
                    PROTECT(argValue = CONS_NR(x, evalArgs(CDR(argValue), rho, dropmissing, call, 1)));
...
static void findmethod(SEXP Class, const char *group, const char *generic, SEXP *sxp, SEXP *gr, SEXP *meth, int *which,
        ...
        *sxp = R_LookupMethod(*meth, rho, rho, R_BaseEnv);
        if (isFunction(*sxp))
        {
            *gr = R_BlankScalarString;
            if (whichclass > 0)
                updateObjFromS4Slot(objSlot, ss);
            break;
        }
...
static SEXP classForGroupDispatch(SEXP obj)
{
    ...
    return IS_S4_OBJECT(obj) ? R_data_class2(obj) : getAttrib(obj, R_ClassSymbol);
}

attribute_hidden int DispatchGroup(const char *group, SEXP call, SEXP op, SEXP args, SEXP rho, SEXP *ans)
{
    ...
    { // try to use S4 :
        ...
        SEXP value;
        if (R_has_methods(op) && (value = R_possible_dispatch(call, op, args, rho, FALSE)))
        {
            *ans = value;
            return 1;
        }
    ...
    SEXP lmeth = R_NilValue, lsxp = R_NilValue, lgr = R_NilValue, rmeth = R_NilValue, rsxp = R_NilValue,
         rgr = R_NilValue;
    int lwhich, rwhich;
    findmethod(lclass, group, generic, &lsxp, &lgr, &lmeth, &lwhich, args, rho);
    PROTECT(lgr);

    if (nargs == 2)
        findmethod(rclass, group, generic, &rsxp, &rgr, &rmeth, &rwhich, CDR(args), rho);
    else
        rwhich = 0;
    PROTECT(rgr);
    ...
    SEXP newvars = PROTECT(
        createS3Vars(PROTECT(mkString(generic)), lgr, PROTECT(stringSuffix(lclass, lwhich)), m, rho, R_BaseEnv));
...
#else
#define SETSTACK_INTSEQ(idx, rn1, rn2) SETSTACK(idx, seq_int((int)rn1, (int)rn2))
#endif
...
#define INTEGER_TO_LOGICAL(x) ((x) == NA_INTEGER ? NA_LOGICAL : (x) ? TRUE : FALSE)
...
static R_INLINE SEXP getPrimitive(SEXP symbol, SEXPTYPE type)
{
    ...
    if (TYPEOF(value) != type)
    {
        ...
        if (TYPEOF(value) != type)
            ...
            error(_("\"%s\" is not a %s function"), CHAR(PRINTNAME(symbol)),
                  type == BUILTINSXP ? "BUILTIN" : "SPECIAL");
    }
    return value;
}

static SEXP cmp_relop(SEXP call, int opval, SEXP opsym, SEXP x, SEXP y, SEXP rho)
{
...
/* The current (as of r67808) Windows toolchain compiles explicit sqrt
   ...
   around this. */
#if (defined(_WIN32) || defined(_WIN64)) && defined(__GNUC__) && __GNUC__ <= 4
...
static struct
{
    ...
    SEXP sym;
    double (*fun)(double);
} math1funs[] = {{"floor", NULL, floor},       {"ceiling", NULL, ceil},  {"sign", NULL, sign},

                 {"expm1", NULL, expm1},       {"log1p", NULL, log1p},

                 {"cos", NULL, cos},           {"sin", NULL, sin},       {"tan", NULL, tan},
                 {"acos", NULL, acos},         {"asin", NULL, asin},     {"atan", NULL, atan},

                 {"cosh", NULL, cosh},         {"sinh", NULL, sinh},     {"tanh", NULL, tanh},
                 {"acosh", NULL, acosh},       {"asinh", NULL, asinh},   {"atanh", NULL, atanh},
...
#define DO_COLON()                                                                                                     \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            if (R_FINITE(rn1) && R_FINITE(rn2) && INT_MIN <= rn1 && INT_MAX >= rn1 && INT_MIN <= rn2 &&                \
                INT_MAX >= rn2 && rn1 == (int)rn1 && rn2 == (int)rn2)                                                  \
            {                                                                                                          \
...
#define DO_SEQ_LEN()                                                                                                   \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            if (1 <= rlen && INT_MAX >= rlen && rlen == (int)rlen)                                                     \
            {                                                                                                          \
                SKIP_OP(); /* skip 'call' index */                                                                     \
...
#define RCNTXT_ELEMS ((sizeof(RCNTXT) + sizeof(R_bcstack_t) - 1) / sizeof(R_bcstack_t))
...
#define BNDCELL_WRITABLE(v) (v != R_NilValue && !BINDING_IS_LOCKED(v) && !IS_ACTIVE_BINDING(v))
...
#define GET_CACHED_BINDING_CELL(vcache, sidx) (vcache ? VCACHE(CACHEIDX(sidx)) : R_NilValue)
...
static int tryAssignDispatch(char *generic, SEXP call, SEXP lhs, SEXP rhs, SEXP rho, SEXP *pv)
{
    int result;
...
#define FAST_VECELT_OK(vec)                                                                                            \
    (ATTRIB(vec) == R_NilValue || (TAG(ATTRIB(vec)) == R_DimSymbol && CDR(ATTRIB(vec)) == R_NilValue))

static R_INLINE void VECSUBSET_PTR(SEXP vec, R_bcstack_t *si, R_bcstack_t *sv, SEXP rho, SEXP consts, int callidx,
                                   Rboolean subset2)
{
...
static R_INLINE Rboolean setElementFromScalar(SEXP vec, R_xlen_t i, R_bcstack_t *srhs)
{
    if (i < 0)
        return FALSE;
...
static R_INLINE void VECSUBASSIGN_PTR(SEXP vec, R_bcstack_t *srhs, R_bcstack_t *si, R_bcstack_t *sv, SEXP rho,
...
#define DO_VECSUBASSIGN(rho, sub2)                                                                                     \
    ...
    {                                                                                                                  \
        ...
        if (srhs->tag && si->tag == INTSXP && srhs->tag == TYPEOF(vec))                                                \
        {                                                                                                              \
            R_xlen_t i = si->u.ival;                                                                                   \
...
#define FIXUP_SCALAR_LOGICAL(rho, callidx, arg, op, warn_level)                                                        \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            SETSTACK(-1, ScalarLogical(asLogical2(val, /*checking*/ 1, VECTOR_ELT(constants, callidx), rho)));         \
        }                                                                                                              \
    } while (0)
...
#define GET_VEC_LOOP_VALUE(var)                                                                                        \
    ...
    {                                                                                                                  \
        ...
        if (BNDCELL_TAG(cell) || (var) != CAR(cell) || MAYBE_SHARED(var) || ATTRIB(var) != R_NilValue)                 \
        {                                                                                                              \
...
static R_INLINE Rboolean GETSTACK_LOGICAL_NO_NA_PTR(R_bcstack_t *s, int callidx, SEXP constants, SEXP rho)
{
...
static Rboolean maybeClosureWrapper(SEXP expr)
{
    ...
    if (!(sym == R_DotInternalSym || sym == R_DotExternalSym || sym == R_DotExternal2Sym ||
          sym == R_DotExternalgraphicsSym || sym == R_DotCallSym || sym == R_DotFortranSym || sym == R_DotCSym ||
          sym == R_DotCallgraphicsSym))
...
SEXP attribute_hidden R_getBCInterpreterExpression()
{
    ...
    else if (TYPEOF(exp) == SYMSXP || maybeClosureWrapper(exp) || maybePrimitiveCall(exp))
    {
...
static SEXP bcEval(SEXP body, SEXP rho, Rboolean useCache)
{
    ...
    BEGIN_MACHINE
    {
        ...
        OP(RETURN, 0) : retvalue = GETSTACK(-1);
        goto done;
        OP(GOTO, 1) :
        {
            int label = GETOP();
        ...
        OP(BRIFNOT, 2) :
        {
            int callidx = GETOP();
            int label = GETOP();
            Rboolean cond = GETSTACK_LOGICAL_NO_NA_PTR(R_BCNodeStackTop - 1, callidx, constants, rho);
            BCNPOP_IGNORE_VALUE();
            if (!cond)
            {
        ...
        OP(POP, 0) : BCNPOP_IGNORE_VALUE();
        NEXT();
        OP(DUP, 0) : BCNDUP();
        NEXT();
        OP(PRINTVALUE, 0) : PrintValue(BCNPOP());
        NEXT();
        OP(STARTLOOPCNTXT, 2) :
        {
        ...
        BCNPUSH_LOGICAL(TRUE);
        NEXT();
        OP(LDFALSE, 0) : R_Visible = TRUE;
        BCNPUSH_LOGICAL(FALSE);
        NEXT();
...
//#define REPORT_OVERRIDEN_BUILTINS
        ...
        OP(PUSHARG, 0) : PUSHCALLARG(BCNPOP());
        NEXT();
        OP(PUSHCONSTARG, 1) :
        {
            SEXP value = VECTOR_ELT(constants, GETOP());
        ...
        OP(MAKECLOSURE, 1) :
        {
            ...
            if (LENGTH(fb) > 2)
            {
                ...
                if (!isNull(srcref))
                    setAttrib(value, R_SrcrefSymbol, srcref);
            }
            R_Visible = TRUE;
            BCNPUSH(value);
            NEXT();
        ...
        OP(MATSUBSET, 1) : DO_MATSUBSET(rho, FALSE);
        NEXT();
        OP(VECSUBASSIGN, 1) : DO_VECSUBASSIGN(rho, FALSE);
        OP(MATSUBASSIGN, 1) : DO_MATSUBASSIGN(rho, FALSE);
        NEXT();
        OP(AND1ST, 2) :
        {
            int callidx = GETOP();
...
#define STACKVAL_MAYBE_SHARED(idx)                                                                                     \
    (IS_STACKVAL_BOXED(idx) && MAYBE_SHARED(GETSTACK_SXPVAL_PTR(R_BCNodeStackTop + (idx))))
        ...
        OP(SWITCH, 4) :
        {
            ...
            if (TYPEOF(value) == STRSXP)
            {
                ...
                else
                {
                    ...
                    which = n - 1;
                    for (i = 0; i < n - 1; i++)
                        if (pmatch(STRING_ELT(value, 0), STRING_ELT(names, i), 1 /* exact */))
                        {
                            which = i;
                            break;
                        }
        ...
        OP(MATSUBSET2, 1) : DO_MATSUBSET(rho, TRUE);
        NEXT();
        OP(VECSUBASSIGN2, 1) : DO_VECSUBASSIGN(rho, TRUE);
        OP(MATSUBASSIGN2, 1) : DO_MATSUBASSIGN(rho, TRUE);
        NEXT();
        OP(STARTSUBSET2_N, 2) : DO_STARTDISPATCH_N("[[");
        OP(STARTSUBASSIGN2_N, 2) : DO_START_ASSIGN_DISPATCH_N("[[<-");
        OP(SUBSET_N, 2) : DO_SUBSET_N(rho, FALSE);
        NEXT();
        OP(SUBSET2_N, 2) : DO_SUBSET_N(rho, TRUE);
        NEXT();
        OP(SUBASSIGN_N, 2) : DO_SUBASSIGN_N(rho, FALSE);
        NEXT();
        OP(SUBASSIGN2_N, 2) : DO_SUBASSIGN_N(rho, TRUE);
        NEXT();
        OP(LOG, 1) : DO_LOG();
        NEXT();
        OP(LOGBASE, 1) : DO_LOGBASE();
        NEXT();
        OP(MATH1, 2) : DO_MATH1();
        NEXT();
        OP(DOTCALL, 2) : DO_DOTCALL();
        NEXT();
        OP(COLON, 1) : DO_COLON();
        NEXT();
        OP(SEQALONG, 1) : DO_SEQ_ALONG();
        NEXT();
...
#else
SEXP R_bcEncode(SEXP x)
{
    return x;
}
SEXP R_bcDecode(SEXP x)
{
    return duplicate(x);
}
#endif
...
#define CHECK_ALL_CONSTANTS
    ...
    for (i = 0; i < n; i += opinfo[ipc[i]].argc + 1)
        if (ipc[i] == LDCONST_OP || ipc[i] == PUSHCONSTARG_OP || ipc[i] == CALLSPECIAL_OP)
            loadableConsts++;
...
FILE *R_OpenCompiledFile(char *fname, char *buf, size_t bsize)
{
    ...
    if (cname != NULL && R_FileExists(cname) &&
        (strcmp(fname, cname) == 0 || !R_FileExists(fname) || R_FileMtime(cname) > R_FileMtime(fname)))
...
SEXP R_ParseEvalString(const char *str, SEXP env)
{
    ...
    if (status != PARSE_OK || TYPEOF(ps) != EXPRSXP || LENGTH(ps) != 1)
        error("parse error");
