...
#ifndef Win32
#if (defined(__APPLE__) || defined(_REENTRANT) || defined(HAVE_OPENMP)) && !defined(HAVE_PTHREAD)
#define HAVE_PTHREAD
#endif
#ifdef HAVE_PTHREAD
#include <pthread.h>
...
SEXP applyClosure(SEXP call, SEXP op, SEXP arglist, SEXP rho, SEXP suppliedvars)
{
    ...
    return R_execClosure(call, newrho, (R_GlobalContext->callflag == CTXT_GENERIC) ? R_GlobalContext->sysparent : rho,
                         rho, arglist, op);
}
...
static R_INLINE Rboolean asLogicalNoNA(SEXP s, SEXP call)
{
    ...
    if (cond == NA_LOGICAL)
    {
        char *msg = length(s) ? (isLogical(s) ? _("missing value where TRUE/FALSE needed")
...
#define FIXUP_SCALAR_LOGICAL(callidx, arg, op)                                                                         \
    ...
    {                                                                                                                  \
        SEXP val = GETSTACK(-1);                                                                                       \
        if (TYPEOF(val) != LGLSXP || XLENGTH(val) != 1 || ATTRIB(val) != NULL)                                         \
        {                                                                                                              \
            if (!isNumber(val))                                                                                        \
...
static R_INLINE Rboolean GETSTACK_LOGICAL_NO_NA_PTR(R_bcstack_t *s, int callidx, SEXP constants)
{
#ifdef TYPED_STACK
...
static SEXP bcEval(SEXP body, SEXP rho, Rboolean useCache)
{
    ...
    BEGIN_MACHINE
    {
        ...
        OP(BRIFNOT, 2) :
        {
            int callidx = GETOP();
            int label = GETOP();
            Rboolean cond = GETSTACK_LOGICAL_NO_NA_PTR(R_BCNodeStackTop - 1, callidx, constants);
            BCNPOP_IGNORE_VALUE();
            if (!cond)
            {
            ...
            if (s->tag && !BINDING_IS_LOCKED(loc))
            {
                ...
                if (NOT_SHARED(x) && IS_SIMPLE_SCALAR(x, s->tag))
                {
                    ...
                    case REALSXP:
                        ...
                        NEXT();
                    case INTSXP:
                        INTEGER(x)[0] = s->u.ival;
                        NEXT();
                    case LGLSXP:
                        LOGICAL(x)[0] = s->u.ival;
                        NEXT();
                    }
                }
            }
