...
attribute_hidden int DispatchGroup(const char *group, SEXP call, SEXP op, SEXP args, SEXP rho, SEXP *ans)
{
    ...
    lclass = IS_S4_OBJECT(CAR(args)) ? R_data_class2(CAR(args)) : getAttrib(CAR(args), R_ClassSymbol);

    if (nargs == 2)
        rclass = IS_S4_OBJECT(CADR(args)) ? R_data_class2(CADR(args)) : getAttrib(CADR(args), R_ClassSymbol);
    else
        rclass = R_NilValue;
    ...
    for (i = 0; i < nargs; i++)
    {
        t = IS_S4_OBJECT(CAR(s)) ? R_data_class2(CAR(s)) : getAttrib(CAR(s), R_ClassSymbol);
        set = 0;
        if (isString(t))
        {
            for (j = 0; j < length(t); j++)
            {
                if (!strcmp(translateChar(STRING_ELT(t, j)), translateChar(STRING_ELT(lclass, lwhich))))
                {
...
#define DO_FAST_BINOP(op, a, b)                                                                                        \
    ...
    {                                                                                                                  \
        ...
        SEXP ans;                                                                                                      \
        if (NO_REFERENCES(sa) && TYPEOF(sa) == REALSXP)                                                                \
            ans = sa;                                                                                                  \
        else if (NO_REFERENCES(sb) && TYPEOF(sb) == REALSXP)                                                           \
            ans = sb;                                                                                                  \
        else                                                                                                           \
...
#define DO_FAST_BINOP_INT(op, a, b)                                                                                    \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            SEXP sb = R_BCNodeStackTop[-1];                                                                            \
            SEXP ans;                                                                                                  \
            if (NO_REFERENCES(sa) && TYPEOF(sa) == INTSXP)                                                             \
                ans = sa;                                                                                              \
            else if (NO_REFERENCES(sb) && TYPEOF(sb) == INTSXP)                                                        \
                ans = sb;                                                                                              \
            else                                                                                                       \
...
#ifdef TYPED_STACK
#define GET_CACHED_BINDING_CELL(vcache, sidx) (vcache ? vcache[CACHEIDX(sidx)].u.sxpval : R_NilValue)
#define GET_SMALLCACHE_BINDING_CELL(vcache, sidx) (vcache ? vcache[sidx].u.sxpval : R_NilValue)

#define SET_CACHED_BINDING(vcache, sidx, cell)                                                                         \
    do                                                                                                                 \
    {                                                                                                                  \
        if (vcache)                                                                                                    \
            vcache[CACHEIDX(sidx)].u.sxpval = (cell);                                                                  \
    } while (0)
#else
...
static R_INLINE void VECSUBSET_PTR(R_bcstack_t *sx, R_bcstack_t *si, R_bcstack_t *sv, SEXP rho)
{
    SEXP idx, args, value;
...
#define DO_VECSUBSET(rho)                                                                                              \
    do                                                                                                                 \
    {                                                                                                                  \
        VECSUBSET_PTR(R_BCNodeStackTop - 2, R_BCNodeStackTop - 1, R_BCNodeStackTop - 2, rho);                          \
        R_BCNodeStackTop--;                                                                                            \
    } while (0)
...
static R_INLINE void SETVECSUBSET_PTR(R_bcstack_t *sx, R_bcstack_t *srhs, R_bcstack_t *si, R_bcstack_t *sv, SEXP rho)
{
    SEXP idx, args, value;
    SEXP vec = GETSTACK_PTR(sx);
...
{
    SETVECSUBSET_PTR(R_BCNodeStackTop - 3, R_BCNodeStackTop - 2, R_BCNodeStackTop - 1, R_BCNodeStackTop - 3, rho);
    R_BCNodeStackTop -= 2;
}
       ...
       unserializing a compiled expression. */
        ...
        OP(SETVAR, 1) :
        {
            ...
            if (s->tag && !BINDING_IS_LOCKED(loc))
            {
                ...
                int type = TYPEOF(x);
                if (type == s->tag && !MAYBE_SHARED(x) && XLENGTH(x) == 1 && ATTRIB(x) == R_NilValue)
                {
                    switch (type)
                    {
                    case REALSXP:
        ...
        OP(VECSUBSET, 0) : DO_VECSUBSET(rho);
        NEXT();
        OP(MATSUBSET, 0) : DO_MATSUBSET(rho);
        NEXT();
        OP(SETVECSUBSET, 0) : DO_SETVECSUBSET(rho);
        NEXT();
        OP(SETMATSUBSET, 0) : DO_SETMATSUBSET(rho);
        NEXT();
        OP(AND1ST, 2) :
        {
            int callidx = GETOP();
        ...
        OP(SWAP, 0) :
        {
            ...
            if (!tmp.tag && MAYBE_REFERENCED(tmp.u.sxpval))
            {
                ...
                if (MAYBE_SHARED(tmp.u.sxpval) || (!cont.tag && MAYBE_SHARED(cont.u.sxpval)))
