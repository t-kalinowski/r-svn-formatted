...
#define Builtin1(do_fun, which, rho)                                                                                   \
    ...
    {                                                                                                                  \
        ...
        R_BCNodeStackTop[-1] = do_fun(FakeCall1, SYMVALUE(which), R_BCNodeStackTop[-1], rho);                          \
        NEXT();                                                                                                        \
    } while (0)

#define NewBuiltin1(do_fun, which, rho)                                                                                \
    do                                                                                                                 \
    {                                                                                                                  \
...
#define Builtin2(do_fun, which, rho)                                                                                   \
    ...
    {                                                                                                                  \
        ...
        R_BCNodeStackTop--;                                                                                            \
        R_BCNodeStackTop[-1] = do_fun(FakeCall2, SYMVALUE(which), R_BCNodeStackTop[-1], rho);                          \
        NEXT();                                                                                                        \
    } while (0)
...
#define FIXUP_SCALAR_LOGICAL(arg, op)                                                                                  \
    do                                                                                                                 \
    {                                                                                                                  \
...
static R_INLINE void checkForMissings(SEXP args, SEXP call)
{
    SEXP a, c;
    int n;
    for (a = args, c = CDR(call), n = 1; a != R_NilValue; a = CDR(a), c = CDR(c), n++)
        if (CAR(a) == R_MissingArg)
        {
            ...
            /* check for a missing argument as in evalList */
                ...
                errorcall(call, _("'%s' is missing"), CHAR(PRINTNAME(CAR(c))));
...
static SEXP bcEval(SEXP body, SEXP rho)
{
    ...
    BEGIN_MACHINE
    {
        ...
        NEXT();
        OP(PUSHCONSTARG, 1) : DO_LDCONST(value);
        PUSHCALLARG(value);
        NEXT();
