...
SEXP attribute_hidden do_for(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (NAMED(val) > 0)
    {
        ...
        case VECSXP:
            val0 = val;
            PROTECT(val = allocVector(TYPEOF(val0), n));
            nprotect++;
            for (i = 0; i < n; i++)
            {
        ...
        case LISTSXP:
            val0 = val;
            PROTECT(val = allocList(n));
            nprotect++;
            for (el = val; CDR(val0) != R_NilValue; val0 = CDR(val0), el = CDR(el))
            {
 ...
 */
attribute_hidden int DispatchOrEval(SEXP call, SEXP op, char *generic, SEXP args, SEXP rho, SEXP *ans, int dropmissing,
                                    int argsevald)
{
...
attribute_hidden int DispatchGroup(char *group, SEXP call, SEXP op, SEXP args, SEXP rho, SEXP *ans)
{
