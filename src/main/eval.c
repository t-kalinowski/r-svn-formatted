...
static SEXP applydefine(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    SET_PRVALUE(tmp, rhs);
    PROTECT(expr = assignCall(install(asym[PRIMVAL(op)]), CDR(lhs), install(buf), R_GetVarLocSymbol(tmploc), CDDR(expr),
                              tmp));
    expr = eval(expr, rho);
    UNPROTECT(5);
...
#define Builtin1(do_fun, which)                                                                                        \
    do                                                                                                                 \
    {                                                                                                                  \
        R_BCNodeStackTop[-1] = CONS(R_BCNodeStackTop[-1], R_NilValue);                                                 \
        R_BCNodeStackTop[-1] = do_fun(FakeCall1, SYMVALUE(which), R_BCNodeStackTop[-1], R_NilValue);                   \
        NEXT();                                                                                                        \
    } while (0)

#define NewBuiltin1(do_fun, which)                                                                                     \
    do                                                                                                                 \
    {                                                                                                                  \
...
#define Builtin2(do_fun, which)                                                                                        \
    do                                                                                                                 \
    {                                                                                                                  \
        SEXP tmp = CONS(R_BCNodeStackTop[-1], R_NilValue);                                                             \
        R_BCNodeStackTop[-2] = CONS(R_BCNodeStackTop[-2], tmp);                                                        \
        R_BCNodeStackTop--;                                                                                            \
        R_BCNodeStackTop[-1] = do_fun(FakeCall2, SYMVALUE(which), R_BCNodeStackTop[-1], R_NilValue);                   \
        NEXT();                                                                                                        \
    } while (0)

#define NewBuiltin2(do_fun, opval, opsym)                                                                              \
    do                                                                                                                 \
    {                                                                                                                  \
        SEXP x = R_BCNodeStackTop[-2];                                                                                 \
        SEXP y = R_BCNodeStackTop[-1];                                                                                 \
...
static SEXP bcEval(SEXP body, SEXP rho)
{
    ...
    BEGIN_MACHINE
    {
        ...
        OP(BRIFNOT, 1) :
        {
            ...
            if (cond == NA_LOGICAL)
                error(isLogical(value) ? _("missing value where logical needed")
        ...
        OP(DFLTSUBASSIGN2, 0) : DO_DFLTDISPATCH(do_subassign2_dflt, R_Subassign2Sym);
        OP(DOLLAR, 2) :
        {
            int dispatched = FALSE;
            SEXP call = VECTOR_ELT(constants, GETOP());
