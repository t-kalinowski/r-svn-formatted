...
#define DO_LOOP_RDEBUG(call, op, args, rho, bgn)                                                                       \
    do                                                                                                                 \
    {                                                                                                                  \
        if (bgn && RDEBUG(rho))                                                                                        \
        {                                                                                                              \
...
static R_INLINE SEXP getSrcrefs(SEXP call, SEXP args)
{
    ...
    if (TYPEOF(srcrefs) == VECSXP && length(srcrefs) == length(args) + 1)
        return srcrefs;
    return R_NilValue;
}
    ...
    if (args != R_NilValue)
    {
        ...
        while (args != R_NilValue)
        {
            if (srcrefs != R_NilValue)
            {
                ...
                if (TYPEOF(R_Srcref) != INTSXP || length(R_Srcref) < 6)
                { /* old code will have length 6; new code length 8 */
...
#define DO_FAST_RELOP2(op, a, b)                                                                                       \
    ...
    {                                                                                                                  \
        ...
        SEXP val;                                                                                                      \
        SKIP_OP();                                                                                                     \
        if (ISNAN(__a__) || ISNAN(__b__))                                                                              \
            val = ScalarLogical(NA_LOGICAL);                                                                           \
        else                                                                                                           \
            val = (__a__ op __b__) ? R_TrueValue : R_FalseValue;                                                       \
...
#define FastRelop2(op, opval, opsym)                                                                                   \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            if (TYPEOF(x) == REALSXP && LENGTH(x) == 1 && TYPEOF(y) == REALSXP && LENGTH(y) == 1)                      \
                DO_FAST_RELOP2(op, REAL(x)[0], REAL(y)[0]);                                                            \
            else if (TYPEOF(x) == INTSXP && LENGTH(x) == 1 && TYPEOF(y) == REALSXP && LENGTH(y) == 1)                  \
            {                                                                                                          \
                double xd = INTEGER(x)[0] == NA_INTEGER ? NA_REAL : INTEGER(x)[0];                                     \
                DO_FAST_RELOP2(op, xd, REAL(y)[0]);                                                                    \
            }                                                                                                          \
            else if (TYPEOF(x) == REALSXP && LENGTH(x) == 1 && TYPEOF(y) == INTSXP && LENGTH(y) == 1)                  \
            {                                                                                                          \
                double yd = INTEGER(y)[0] == NA_INTEGER ? NA_REAL : INTEGER(y)[0];                                     \
                DO_FAST_RELOP2(op, REAL(x)[0], yd);                                                                    \
            }                                                                                                          \
            else if (TYPEOF(x) == INTSXP && LENGTH(x) == 1 && TYPEOF(y) == INTSXP && LENGTH(y) == 1)                   \
            {                                                                                                          \
                double xd = INTEGER(x)[0] == NA_INTEGER ? NA_REAL : INTEGER(x)[0];                                     \
                double yd = INTEGER(y)[0] == NA_INTEGER ? NA_REAL : INTEGER(y)[0];                                     \
...
#define Builtin1(do_fun, which, rho)                                                                                   \
    ...
    {                                                                                                                  \
        ...
        R_BCNodeStackTop[-1] = do_fun(call, getPrimitive(which, BUILTINSXP), R_BCNodeStackTop[-1], rho);               \
        NEXT();                                                                                                        \
    } while (0)
...
#define FastBinary(op, opval, opsym)                                                                                   \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            else if (TYPEOF(x) == INTSXP && LENGTH(x) == 1 && INTEGER(x)[0] != NA_INTEGER && TYPEOF(y) == REALSXP &&   \
                     LENGTH(y) == 1)                                                                                   \
...
static R_INLINE SEXP getvar(SEXP symbol, SEXP rho, Rboolean dd, Rboolean keepmiss)
{
...
#define PUSHCALLARG_CELL(c)                                                                                            \
    ...
    {                                                                                                                  \
        SEXP __cell__ = (c);                                                                                           \
        if (R_BCNodeStackTop[-2] == R_NilValue)                                                                        \
            R_BCNodeStackTop[-2] = __cell__;                                                                           \
        else                                                                                                           \
            SETCDR(R_BCNodeStackTop[-1], __cell__);                                                                    \
...
#define DO_ISTEST(fun)                                                                                                 \
    do                                                                                                                 \
    {                                                                                                                  \
        R_BCNodeStackTop[-1] = fun(R_BCNodeStackTop[-1]) ? R_TrueValue : R_FalseValue;                                 \
        NEXT();                                                                                                        \
    } while (0)
#define DO_ISTYPE(type)                                                                                                \
    do                                                                                                                 \
    {                                                                                                                  \
        R_BCNodeStackTop[-1] = TYPEOF(R_BCNodeStackTop[-1]) == type ? mkTrue() : mkFalse();                            \
        NEXT();                                                                                                        \
    } while (0)
...
static SEXP numVecElt(SEXP vec, SEXP idx)
{
    ...
    {
    case REALSXP:
        return ScalarReal(REAL(vec)[i]);
    case INTSXP:
        return ScalarInteger(INTEGER(vec)[i]);
    case LGLSXP:
        return ScalarLogical(LOGICAL(vec)[i]);
    case CPLXSXP:
        return ScalarComplex(COMPLEX(vec)[i]);
    case RAWSXP:
...
static SEXP numMatElt(SEXP mat, SEXP idx, SEXP jdx)
{
    ...
    {
    case REALSXP:
        return ScalarReal(REAL(mat)[k]);
    case INTSXP:
        return ScalarInteger(INTEGER(mat)[k]);
    case LGLSXP:
        return ScalarLogical(LOGICAL(mat)[k]);
    case CPLXSXP:
        return ScalarComplex(COMPLEX(mat)[k]);
    default:
...
static SEXP setNumVecElt(SEXP vec, SEXP idx, SEXP value)
{
    ...
    {
    case REALSXP:
        REAL(vec)[i] = asReal(value);
        break;
    case INTSXP:
        INTEGER(vec)[i] = asInteger(value);
        break;
    case LGLSXP:
        LOGICAL(vec)[i] = asLogical(value);
        break;
    case CPLXSXP:
        COMPLEX(vec)[i] = asComplex(value);
        break;
    default:
        error(_("not a simple vector"));
    }
...
static SEXP setNumMatElt(SEXP mat, SEXP idx, SEXP jdx, SEXP value)
{
    ...
    {
    case REALSXP:
        REAL(mat)[k] = asReal(value);
        break;
    case INTSXP:
        INTEGER(mat)[k] = asInteger(value);
        break;
    case LGLSXP:
        LOGICAL(mat)[k] = asLogical(value);
        break;
    case CPLXSXP:
        COMPLEX(mat)[k] = asComplex(value);
        break;
    default:
        error(_("not a simple matrix"));
    }
...
static R_INLINE Rboolean SET_BINDING_VALUE(SEXP loc, SEXP value)
{
    ...
    if (loc != R_NilValue && !(loc->sxpinfo.gp & SPECIAL_BINDING_MASK) && CAR(loc) != R_UnboundValue)
    {
        if (CAR(loc) != value)
            SETCAR(loc, value);
        return TRUE;
...
static SEXP bcEval(SEXP body, SEXP rho)
{
    ...
    BEGIN_MACHINE
    {
        ...
        OP(RETURN, 0) : value = R_BCNodeStackTop[-1];
        goto done;
        OP(GOTO, 1) :
        {
            int label = GETOP();
        ...
        NEXT();
        OP(DUP, 0) : value = R_BCNodeStackTop[-1];
        BCNPUSH(value);
        NEXT();
        ...
        NEXT();
        OP(PUSHTRUEARG, 0) : PUSHCALLARG(mkTrue());
        NEXT();
        OP(PUSHFALSEARG, 0) : PUSHCALLARG(mkFalse());
        NEXT();
        OP(CALL, 1) :
        {
