...
SEXP eval(SEXP e, SEXP rho)
{
    ...
    if (R_EvalDepth > R_Expressions)
        errorcall(R_NilValue, "evaluation nested too deeply: infinite recursion / options(expressions=)?");
    if (++evalcount > 100)
    {
    ...
    case SYMSXP:
        ...
        else if (tmp == R_MissingArg && !DDVAL(e))
        {
            ...
            if (*n)
                error("Argument \"%s\" is missing, with no default", CHAR(PRINTNAME(e)));
            else
    ...
    for (next = FORMALS(op); next != R_NilValue; next = CDR(next))
    {
        ...
        if (missing)
        {
            ...
            if (TYPEOF(val) == PROMSXP && PRENV(val) == rho)
            {
                ...
                if (deflt == R_NilValue)
                    error("Symbol \"%s\" not in environment of method", CHAR(PRINTNAME(symbol)));
...
static Rboolean asLogicalNoNA(SEXP s, SEXP call)
{
    ...
    if (cond == NA_LOGICAL)
    {
        char *msg = length(s) ? (isLogical(s) ? "missing value where TRUE/FALSE needed"
 ...
 */
int DispatchOrEval(SEXP call, SEXP op, char *generic, SEXP args, SEXP rho, SEXP *ans, int dropmissing, int argsevald)
{
...
int DispatchGroup(char *group, SEXP call, SEXP op, SEXP args, SEXP rho, SEXP *ans)
{
    ...
    if (lsxp != rsxp)
    {
        if (isFunction(lsxp) && isFunction(rsxp))
        {
            warning("Incompatible methods (\"%s\", \"%s\") for \"%s\"", CHAR(PRINTNAME(lmeth)), CHAR(PRINTNAME(rmeth)),
                    generic);
            UNPROTECT(2);
            return 0;
        }
...
static SEXP bcEval(SEXP body, SEXP rho)
{
    ...
    /* check version */
    {
        ...
        if (version < R_bcMinVersion || version > R_bcVersion)
        {
            if (version >= 2)
            {
                ...
                error("bytecode version is too new");
        }
    }
    ...
    BEGIN_MACHINE
    {
        ...
        OP(BRIFNOT, 1) :
        {
            ...
            if (cond == NA_LOGICAL)
                error(isLogical(value) ? "missing value where logical needed"
        ...
        OP(STARTFOR, 2) :
        {
            ...
            if (isVector(seq))
                ...
                error("invalid sequence argument in for loop");
            BCNPUSH(value);
