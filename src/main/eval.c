...
static R_INLINE int bcStackScalarEx(R_bcstack_t *s, scalar_value_t *v, SEXP *pv)
{
#ifdef TYPED_STACK
    int tag = s->tag;
...
static R_INLINE int bcStackScalarRealEx(R_bcstack_t *s, scalar_value_t *px, SEXP *pv)
{
...
#ifdef NO_SAVE_ALLOC
#define DO_FAST_BINOP(fun, a, b, v)                                                                                    \
    do                                                                                                                 \
    {                                                                                                                  \
        SKIP_OP();                                                                                                     \
...
#define DO_FAST_BINOP_INT(fun, a, b, v)                                                                                \
    do                                                                                                                 \
    {                                                                                                                  \
...
#define DO_FAST_BINOP(fun, a, b, ans)                                                                                  \
    do                                                                                                                 \
    {                                                                                                                  \
        SKIP_OP();                                                                                                     \
        double dval = fun(a, b);                                                                                       \
...
#define DO_FAST_BINOP_INT(fun, a, b, ans)                                                                              \
    do                                                                                                                 \
    {                                                                                                                  \
        double dval = fun((double)(a), (double)(b));                                                                   \
        if (dval <= INT_MAX && dval >= INT_MIN + 1)                                                                    \
...
#define GET_SMALLCACHE_BINDING_CELL(vcache, sidx) (vcache ? VCACHE(sidx) : R_NilValue)
...
static R_INLINE SEXP FIND_VAR_NO_CACHE(SEXP symbol, SEXP rho, SEXP cell)
{
    ...
    if (cell != R_NilValue || rho == R_BaseEnv || rho == R_BaseNamespace)
...
#define DO_VECSUBSET(rho, sub2)                                                                                        \
    ...
    {                                                                                                                  \
        int callidx = GETOP();                                                                                         \
        VECSUBSET_PTR(R_BCNodeStackTop - 2, R_BCNodeStackTop - 1, R_BCNodeStackTop - 2, rho, constants, callidx,       \
                      sub2);                                                                                           \
        R_BCNodeStackTop--;                                                                                            \
        R_Visible = TRUE;                                                                                              \
    } while (0)
...
static R_INLINE Rboolean setElementFromScalar(SEXP vec, R_xlen_t i, int typev, scalar_value_t *v)
{
    ...
    if (TYPEOF(vec) == REALSXP)
    {
        ...
        case REALSXP:
            ...
            return TRUE;
        case INTSXP:
            REAL(vec)[i] = INTEGER_TO_REAL(v->ival);
            return TRUE;
        case LGLSXP:
            REAL(vec)[i] = LOGICAL_TO_REAL(v->ival);
            return TRUE;
        }
    }
...
#define DO_VECSUBASSIGN(rho, sub2)                                                                                     \
    ...
    {                                                                                                                  \
        int callidx = GETOP();                                                                                         \
        VECSUBASSIGN_PTR(R_BCNodeStackTop - 3, R_BCNodeStackTop - 2, R_BCNodeStackTop - 1, R_BCNodeStackTop - 3, rho,  \
                         constants, callidx, sub2);                                                                    \
        R_BCNodeStackTop -= 2;                                                                                         \
    } while (0)
