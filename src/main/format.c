...
attribute_hidden void formatRawS(SEXP x, R_xlen_t n, int *fieldwidth)
{
    *fieldwidth = 2;
}
...
attribute_hidden void formatStringS(SEXP x, R_xlen_t n, int *fieldwidth, int quote)
{
    int xmax = 0;
    int l;
    ...
    for (R_xlen_t i = 0; i < n; i++)
    {
        if (STRING_ELT(x, i) == NA_STRING)
        {
            ...
            l = Rstrlen(STRING_ELT(x, i), quote) + (quote ? 2 : 0);
        if (l > xmax)
            xmax = l;
    }
    *fieldwidth = xmax;
...
void formatIntegerS(SEXP x, R_xlen_t n, int *fieldwidth)
{
    ...
    int xmin = INT_MAX, xmax = INT_MIN, naflag = 0, sorted;
    ...
    */
    if (tmpmin != NULL && tmpmax != NULL && TYPEOF(tmpmin) == INTSXP && TYPEOF(tmpmax) == INTSXP)
    {
...
#define KP_MAX 27
#else
static const double tbl[] = {1e00, 1e01, 1e02, 1e03, 1e04, 1e05, 1e06, 1e07, 1e08, 1e09, 1e10, 1e11,
...
{
    Rboolean naflag = FALSE, nanflag = FALSE, posinf = FALSE, neginf = FALSE;
    int neg = 0;
    int mnl = INT_MAX, mxl, rgt, mxsl, mxns;
...
void formatComplex(const Rcomplex *x, R_xlen_t n, int *wr, int *dr, int *er, // (w,d,e) for Re(.)
                   int *wi, int *di, int *ei,                                // (w,d,e) for Im(.)
                   int nsmall)
{
    /* format.info() for  x[1..n] for both Re & Im */
    Rboolean all_re_zero = TRUE, all_im_zero = TRUE, naflag = FALSE, rnan = FALSE, rposinf = FALSE, rneginf = FALSE,
    ...
    for (R_xlen_t i = 0; i < n; i++)
    {
        ...
        {
            Rboolean roundingwidens;
            int left, right, sleft, neg_i, kpower, nsig;
...
void formatComplexS(SEXP x, R_xlen_t n, int *wr, int *dr, int *er, int *wi, int *di, int *ei, int nsmall)
{
    /* format.info() for  x[1..n] for both Re & Im */
    ...
    *ei = 0;
    ITERATE_BY_REGION_PARTIAL(x, px, idx, nb, Rcomplex, COMPLEX, 0, n, {
        formatComplex(px, nb, &tmpwr, &tmpdr, &tmper, &tmpwi, &tmpdi, &tmpei, nsmall);
        if (tmpwr > *wr)
            *wr = tmpwr;
        if (tmpdr && !*dr)
            *dr = tmpdr;
