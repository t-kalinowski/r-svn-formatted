...
attribute_hidden unsigned short *_controlify(pGEDevDesc dd, const unsigned char *src, int typeface, int fontindex)
{
    unsigned short *dest;
    unsigned char c, d;
    unsigned char esc[3];
    ...
    */
    ...
    while (*src != (unsigned char)'\0')
    {
       ...
       here, which appears elsewhere below). */
        ...
        if ((raw_fontnum == HERSHEY_EUC) && (*src & 0x80) && (*(src + 1) & 0x80))
        {
       ...
       pattern (no ligatures supported in non-Hershey fonts) */
        if (1) /* _plotter->drawstate->font_type == F_HERSHEY) */
        {
            ...
            for (i = 0; i < NUM_LIGATURES; i++)
                if ((_ligature_tbl[i].font == raw_fontnum) &&
                    (strncmp((char *)src, _ligature_tbl[i].from, strlen(_ligature_tbl[i].from)) == 0))
                {
                    matched = true;
                    break;
                }
        ...
        if (c != (unsigned char)'\\') /* ordinary char, may pass through */
        {
                ...
                && _hershey_font_info[raw_fontnum].iso8859_1)
            {
                ...
                if (matched) /* it's a raised character */
                {
                    ...
                    dest[j++] = (unsigned short)(CONTROL_CODE | C_BEGIN_SUPERSCRIPT);
                    if (_raised_char_tbl[i].underscored) /* also underline */
                    {
                        dest[j++] = (unsigned short)(CONTROL_CODE | C_PUSH_LOCATION);
                        dest[j++] = fontword | (unsigned short)_raised_char_tbl[i].to;
                        dest[j++] = (unsigned short)(CONTROL_CODE | C_POP_LOCATION);
                        /* select appropriate HersheySymbol font */
                        dest[j++] = symbol_fontword | (unsigned short)VECTOR_SYMBOL_FONT_UNDERSCORE;
                    }
                    else /* just print raised char, no underline */
                        dest[j++] = fontword | (unsigned short)_raised_char_tbl[i].to;

                    dest[j++] = (unsigned short)(CONTROL_CODE | C_END_SUPERSCRIPT);
               ...
               check if this char should be deligatured */
                ...
                if (matched)
                {
                    ...
                    {
                        dest[j++] = fontword | (unsigned short)_deligature_char_tbl[i].to[0];
                        dest[j++] = fontword | (unsigned short)_deligature_char_tbl[i].to[1];
                        continue; /* back to top of while loop */
                    }
                }
            }
        ...
        else /* character is a backslash */
        {
            ...
            if (1 /* _plotter->drawstate->font_type == F_HERSHEY */
                && esc[0] == '#' && esc[1] == 'H' && src[0] >= '0' && src[0] <= '9' && src[1] >= '0' && src[1] <= '9' &&
                 ...
                 (src[3] >= 'A' && src[3] <= 'F')))
            {
                ...
                jis_glyphindex = (hexnum[3] + 16 * hexnum[2] + 256 * hexnum[1] + 4096 * hexnum[0]);
                ...
                && _hershey_font_info[raw_fontnum].iso8859_1)
            {
                ...
                if (matched)
                {
                    ...
                    {
                        dest[j++] = fontword | (unsigned short)_deligature_escape_tbl[i].to[0];
                        dest[j++] = fontword | (unsigned short)_deligature_escape_tbl[i].to[1];
                ...
                if (matched) /* it's an 8-bit ISO8859-1 character */
                {
                    ...
                    if (1) /* _plotter->drawstate->font_type == F_HERSHEY) */
                    {
                        ...
                        if (matched2) /* it's a raised character */
                        {
                            ...
                            {
                                dest[j++] = (unsigned short)(CONTROL_CODE | C_PUSH_LOCATION);
                                dest[j++] = fontword | (unsigned short)_raised_char_tbl[k].to;
                                dest[j++] = (unsigned short)(CONTROL_CODE | C_POP_LOCATION);
                                /* select appropriate HersheySymbol font */
                                dest[j++] = symbol_fontword | (unsigned short)VECTOR_SYMBOL_FONT_UNDERSCORE;
                            }
            ...
            if (1) /* _plotter->drawstate->font_type == F_HERSHEY) */
            {
                ...
                if (matched) /* it's a special character */
                {
                    ...
                    if (_special_escape_tbl[i].byte == FINAL_LOWERCASE_S)
                        dest[j++] = fontword | (unsigned short)(_special_escape_tbl[i].byte);
                    else
...
#else
    _codestring_len(codestring) const unsigned short *codestring;
#endif
{
    int i = 0;
