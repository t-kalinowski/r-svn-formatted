...
SEXP do_setGraphicsEventEnv(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!dd->canGenMouseDown && !dd->canGenMouseUp && !dd->canGenMouseMove && !dd->canGenKeybd && !dd->canGenIdle)
...
Rboolean haveListeningDev()
{
    ...
    if (!NoDevices())
    {
        ...
        {
            if ((gd = GEgetDevice(i)) && (dd = gd->dev) && dd->gettingEvent)
            {
                ret = TRUE;
                break;
            }
...
static const char *keynames[] = {"Left", "Up", "Right", "Down", "F1",  "F2",   "F3",   "F4",  "F5",   "F6",  "F7",
...
Rboolean doesIdle(pDevDesc dd)
{
    ...
    return (handler != R_UnboundValue) && (handler != R_NilValue);
}
