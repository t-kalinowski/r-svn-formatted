...
void GLine(double x1, double y1, double x2, double y2, int coords, DevDesc *dd)
{
     ...
     */
    GClip(dd);
    GELine(x1, y1, x2, y2, Rf_gpptr(dd)->col, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty, Rf_gpptr(dd)->lwd,
           (GEDevDesc *)dd);
}
...
{
    ((GEDevDesc *)dd)
        ->dev->metricInfo(c & 0xFF, Rf_gpptr(dd)->font, Rf_gpptr(dd)->cex, (double)Rf_gpptr(dd)->ps, ascent, descent,
                          width, ((GEDevDesc *)dd)->dev);
    if (units != DEVICE)
    {
...
void GPolygon(int n, double *x, double *y, int coords, int bg, int fg, DevDesc *dd)
{
     ...
     */
    GClip(dd);
    GEPolygon(n, xx, yy, fg, bg, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty, Rf_gpptr(dd)->lwd, (GEDevDesc *)dd);
    vmaxset(vmaxsave);
}
...
void GPolyline(int n, double *x, double *y, int coords, DevDesc *dd)
{
     ...
     */
    GClip(dd);
    GEPolyline(n, xx, yy, Rf_gpptr(dd)->col, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty, Rf_gpptr(dd)->lwd,
               (GEDevDesc *)dd);
    vmaxset(vmaxsave);
}
...
void GCircle(double x, double y, int coords, double radius, int bg, int fg, DevDesc *dd)
{
     ...
     */
    GClip(dd);
    GECircle(x, y, ir, fg, bg, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty, Rf_gpptr(dd)->lwd, (GEDevDesc *)dd);
}
...
void GRect(double x0, double y0, double x1, double y1, int coords, int bg, int fg, DevDesc *dd)
{
     ...
     */
    GClip(dd);
    GERect(x0, y0, x1, y1, fg, bg, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty, Rf_gpptr(dd)->lwd, (GEDevDesc *)dd);
}
...
double GStrHeight(char *str, GUnit units, DevDesc *dd)
{
     ...
     * use these instead of "" and 1 below
     */
    h = GEStrHeight(str, "", Rf_gpptr(dd)->font, 1, Rf_gpptr(dd)->cex, (double)Rf_gpptr(dd)->ps, (GEDevDesc *)dd);
    if (units != DEVICE)
...
void GText(double x, double y, int coords, char *str, double xc, double yc, double rot, DevDesc *dd)
{
     ...
     */
    GClip(dd);
    GEText(x, y, str, xc, yc, rot, Rf_gpptr(dd)->col, Rf_gpptr(dd)->gamma,
           /*
            * FIXME:  If/When GStrWidth gets passed fontfamily and lineheight
            * use these instead of "" and 1 below
            */
           "", Rf_gpptr(dd)->font, 1, Rf_gpptr(dd)->cex, (double)Rf_gpptr(dd)->ps, (GEDevDesc *)dd);
}
...
void GSymbol(double x, double y, int coords, int pch, DevDesc *dd)
{
     ...
     */
    GClip(dd);
    GESymbol(x, y, pch, size, Rf_gpptr(dd)->col, Rf_gpptr(dd)->bg, Rf_gpptr(dd)->gamma, LTY_SOLID, Rf_gpptr(dd)->lwd,
...
int devNumber(DevDesc *dd)
{
    int i;
    for (i = 1; i < R_MaxDevices; i++)
        if (R_Devices[i] != NULL && ((GEDevDesc *)R_Devices[i])->dev == (NewDevDesc *)dd)
            return i;
    return 0;
}
