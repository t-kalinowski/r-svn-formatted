...
static void mapNDC2Dev(DevDesc *dd)
{
    ...
    if (dd->newDevStruct)
    {
        ...
        Rf_gpptr(dd)->ndc2dev.ax = Rf_dpptr(dd)->ndc2dev.ax = ((GEDevDesc *)dd)->dev->left;
        Rf_gpptr(dd)->ndc2dev.by = Rf_dpptr(dd)->ndc2dev.by =
            (((GEDevDesc *)dd)->dev->top - ((GEDevDesc *)dd)->dev->bottom);
        Rf_gpptr(dd)->ndc2dev.ay = Rf_dpptr(dd)->ndc2dev.ay = ((GEDevDesc *)dd)->dev->bottom;
    }
    else
    {
...
static void updateOuterMargins(DevDesc *dd)
{
    ...
    case LINES:
        ...
        Rf_gpptr(dd)->omd[0] = Rf_dpptr(dd)->omd[0] = GConvertYUnits(Rf_gpptr(dd)->oma[0], LINES, NDC, dd);
        Rf_gpptr(dd)->omd[1] = Rf_dpptr(dd)->omd[1] = GConvertXUnits(Rf_gpptr(dd)->oma[1], LINES, NDC, dd);
        Rf_gpptr(dd)->omd[2] = Rf_dpptr(dd)->omd[2] = GConvertYUnits(Rf_gpptr(dd)->oma[2], LINES, NDC, dd);
        Rf_gpptr(dd)->omd[3] = Rf_dpptr(dd)->omd[3] = GConvertXUnits(Rf_gpptr(dd)->oma[3], LINES, NDC, dd);
        break;
    case INCHES:
        ...
        Rf_gpptr(dd)->omd[0] = Rf_dpptr(dd)->omd[0] = GConvertYUnits(Rf_gpptr(dd)->omi[0], INCHES, NDC, dd);
        Rf_gpptr(dd)->omd[1] = Rf_dpptr(dd)->omd[1] = GConvertXUnits(Rf_gpptr(dd)->omi[1], INCHES, NDC, dd);
        Rf_gpptr(dd)->omd[2] = Rf_dpptr(dd)->omd[2] = GConvertYUnits(Rf_gpptr(dd)->omi[2], INCHES, NDC, dd);
        Rf_gpptr(dd)->omd[3] = Rf_dpptr(dd)->omd[3] = GConvertXUnits(Rf_gpptr(dd)->omi[3], INCHES, NDC, dd);
        break;
    case NDC:
        Rf_gpptr(dd)->oma[0] = Rf_dpptr(dd)->oma[0] = GConvertYUnits(Rf_gpptr(dd)->omd[0], NDC, LINES, dd);
        Rf_gpptr(dd)->oma[1] = Rf_dpptr(dd)->oma[1] = GConvertXUnits(Rf_gpptr(dd)->omd[1], NDC, LINES, dd);
        Rf_gpptr(dd)->oma[2] = Rf_dpptr(dd)->oma[2] = GConvertYUnits(Rf_gpptr(dd)->omd[2], NDC, LINES, dd);
        Rf_gpptr(dd)->oma[3] = Rf_dpptr(dd)->oma[3] = GConvertXUnits(Rf_gpptr(dd)->omd[3], NDC, LINES, dd);
        Rf_gpptr(dd)->omi[0] = Rf_dpptr(dd)->omi[0] = GConvertYUnits(Rf_gpptr(dd)->omd[0], NDC, INCHES, dd);
        Rf_gpptr(dd)->omi[1] = Rf_dpptr(dd)->omi[1] = GConvertXUnits(Rf_gpptr(dd)->omd[1], NDC, INCHES, dd);
        Rf_gpptr(dd)->omi[2] = Rf_dpptr(dd)->omi[2] = GConvertYUnits(Rf_gpptr(dd)->omd[2], NDC, INCHES, dd);
        Rf_gpptr(dd)->omi[3] = Rf_dpptr(dd)->omi[3] = GConvertXUnits(Rf_gpptr(dd)->omd[3], NDC, INCHES, dd);
        break;
    default:
        break; /*nothing (-Wall) */
    }
    ...
    for (i = 1; i < n; i++)
    {
        ...
        if (CSclipline(&x1, &y1, &x2, &y2, &cr, &ind1, &ind2, DEVICE, dd))
        {
            if (ind1 && ind2)
            {
                ...
                if (dd->newDevStruct)
                    ((GEDevDesc *)dd)
                        ->dev->polyline(2, xx, yy, Rf_gpptr(dd)->col, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty,
                                        Rf_gpptr(dd)->lwd, ((GEDevDesc *)dd)->dev);
                else
            ...
            else if (ind1)
            {
                ...
                if (i == n - 1)
                {
                    if (dd->newDevStruct)
                        ((GEDevDesc *)dd)
                            ->dev->polyline(count, xx, yy, Rf_gpptr(dd)->col, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty,
                                            Rf_gpptr(dd)->lwd, ((GEDevDesc *)dd)->dev);
                    else
...
void GLine(double x1, double y1, double x2, double y2, int coords, DevDesc *dd)
{
    ...
    if (clip_ok)
    {
        if (dd->newDevStruct)
        {
            ...
            GConvert(&x2, &y2, coords, DEVICE, dd);
            ((GEDevDesc *)dd)
                ->dev->line(x1, y1, x2, y2, Rf_gpptr(dd)->col, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty,
                            Rf_gpptr(dd)->lwd, ((GEDevDesc *)dd)->dev);
        }
        else
...
void GMetricInfo(int c, double *ascent, double *descent, double *width, GUnit units, DevDesc *dd)
{
    if (dd->newDevStruct)
        ((GEDevDesc *)dd)
            ->dev->metricInfo(c & 0xFF, Rf_gpptr(dd)->font, Rf_gpptr(dd)->cex, (double)Rf_gpptr(dd)->ps, ascent,
                              descent, width, ((GEDevDesc *)dd)->dev);
    else
   ...
   If mode = 1, clip to the device extent */
    ...
    else
    {
        ...
        if (npts > 1)
        {
            ...
            if (dd->newDevStruct)
                ((GEDevDesc *)dd)
                    ->dev->polygon(npts, xc, yc, fg, bg, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty, Rf_gpptr(dd)->lwd,
                                   ((GEDevDesc *)dd)->dev);
            else
...
void GCircle(double x, double y, int coords, double radius, int bg, int fg, DevDesc *dd)
{
    ...
    case -2: /* No clipping;  draw all of circle */
        ...
        if (dd->newDevStruct)
            ((GEDevDesc *)dd)
                ->dev->circle(x, y, ir, fg, bg, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty, Rf_gpptr(dd)->lwd,
                              ((GEDevDesc *)dd)->dev);
        else
        ...
        else
        {
            ...
            else
            {
                ...
                if (npts > 1)
                {
                    ...
                    if (dd->newDevStruct)
                        ((GEDevDesc *)dd)
                            ->dev->polygon(npts, xcc, ycc, fg, bg, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty,
                                           Rf_gpptr(dd)->lwd, ((GEDevDesc *)dd)->dev);
                    else
...
void GRect(double x0, double y0, double x1, double y1, int coords, int bg, int fg, DevDesc *dd)
{
    ...
    case 1: /* rectangle totally inside;  draw all */
        ...
        if (dd->newDevStruct)
            ((GEDevDesc *)dd)
                ->dev->rect(x0, y0, x1, y1, fg, bg, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->lty, Rf_gpptr(dd)->lwd,
                            ((GEDevDesc *)dd)->dev);
        else
...
static void clipText(double x, double y, char *str, double rot, int clipToDevice, double hadj, DevDesc *dd)
{
    ...
    case 1: /* text totally inside;  draw all */
        if (dd->newDevStruct)
        {
            ...
            ((GEDevDesc *)dd)
                ->dev->text(x, y, str, rot, hadj, Rf_gpptr(dd)->col, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->font,
                            Rf_gpptr(dd)->cex, (double)Rf_gpptr(dd)->ps, ((GEDevDesc *)dd)->dev);
        }
        else
...
void GBox(int which, DevDesc *dd)
{
    ...
    case 1: /* Plot */
        ...
        case 'N': /* nothing */
            break;
        default:
            warning("invalid par(\"bty\") = '%c'; no box() drawn.", Rf_gpptr(dd)->bty);
        }
        break;
    case 2: /* Figure */
        GPolygon(4, x, y, NFC, NA_INTEGER, Rf_gpptr(dd)->col, dd);
...
void GSymbol(double x, double y, int coords, int pch, DevDesc *dd)
{
    ...
    else
    {
        ...
        case 0: /* S square */
            xc = RADIUS * GSTR_0;
            GConvert(&x, &y, coords, INCHES, dd);
            GRect(x - xc, y - xc, x + xc, y + xc, INCHES, NA_INTEGER, Rf_gpptr(dd)->col, dd);
            break;
        ...
        case 15: /* S filled square */
            ...
            xx[3] = x - xc;
            yy[3] = y + xc;
            GPolygon(4, xx, yy, INCHES, Rf_gpptr(dd)->col, NA_INTEGER, dd);
            break;
        ...
        case 17: /* S filled point-up triangle */
            ...
            xx[2] = x - xc;
            yy[2] = y - yc;
            GPolygon(3, xx, yy, INCHES, Rf_gpptr(dd)->col, NA_INTEGER, dd);
            break;
        ...
        case 22: /* squares */
            ...
            GConvert(&x, &y, coords, INCHES, dd);
            GRect(x - xc, y - xc, x + xc, y + xc, INCHES, Rf_gpptr(dd)->bg, Rf_gpptr(dd)->col, dd);
            break;
        ...
        case 23: /* diamonds */
            ...
            xx[3] = x - xc;
            yy[3] = y;
            GPolygon(4, xx, yy, INCHES, Rf_gpptr(dd)->bg, Rf_gpptr(dd)->col, dd);
            break;
        ...
        case 24: /* triangle (point up) */
            ...
            xx[2] = x - xc;
            yy[2] = y - yc;
            GPolygon(3, xx, yy, INCHES, Rf_gpptr(dd)->bg, Rf_gpptr(dd)->col, dd);
            break;
...
unsigned int number2col(char *nm)
{
    ...
    if (indx == 0)
        return Rf_dpptr(CurrentDevice())->bg;
    else
...
void copyDisplayList(int fromDevice)
{
    ...
    if (dd->newDevStruct)
    {
        ((GEDevDesc *)dd)->dev->displayList = Rf_displayList(R_Devices[fromDevice]);
        copyGPar(Rf_dpSavedptr(R_Devices[fromDevice]), Rf_dpSavedptr(dd));
        playDisplayList(dd);
        if (!((GEDevDesc *)dd)->dev->displayListOn)
            initDisplayList(dd);
    }
