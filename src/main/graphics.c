...
/* Clip the line
   ...
   dd->gp.xpd) */
static int clipLine(double *x1, double *y1, double *x2, double *y2, int coords, int toDevice, DevDesc *dd)
{
    double temp;
    int dummy1, dummy2;
    int xpdsaved = 0; /* -Wall */
...
static void convertCircle(double x, double y, int coords, double r, int numVertices, double *xc, double *yc,
                          DevDesc *dd)
{
    int i;
...
static void clipCircle(double x, double y, int coords, double r, int bg, int fg, int clipToDevice, DevDesc *dd)
{
    char *vmax;
    double *xc, *yc;
    ...
    default: /* Partial clipping; draw poly[line|gon] */
        ...
        else
        {
            ...
            if (npts > 1)
            {
                ...
                npts = GClipPolygon(xc, yc, result, coords, 1, xcc, ycc, dd);
...
static void clipRect(double x0, double y0, double x1, double y1, int coords, int bg, int fg, int clipToDevice,
                     DevDesc *dd)
{
    char *vmax;
    double *xc, *yc;
...
void GPretty(double *lo, double *up, int *ndiv)
{
    ...
    nu = ceil(*up / unit);
#ifdef DEBUG_PLOT
    REprintf("GPretty(%g,%g,%d): cell= %g, base= %g, unit= %g; (ns,nu)=(%d,%d)", *lo, *up, *ndiv, cell, base, unit, ns,
             nu);
#endif
