...
static int jit_stack_size()
{
    ...
    if (p)
    {
        ...
        if (xdouble >= 0 && xdouble <= 1000)
            ...
            warning("R_PCRE_JIT_STACK_MAXSIZE invalid and ignored");
    }
    return stmax;
}
...
static long R_pcre_max_recursions()
{
    ...
    */
    ...
    if (R_CStackDir == 1)
    {
        stack_used = (R_CStackStart >= current_frame) ? R_CStackStart - current_frame : fallback_used;
    }
    else
    {
        stack_used = (current_frame >= R_CStackStart) ? current_frame - R_CStackStart : fallback_used;
    }
...
static void R_pcre_prepare(const char *pattern, SEXP subject, Rboolean use_UTF8, Rboolean caseless,
                           Rboolean always_study, const unsigned char **tables, pcre **re, pcre_extra **re_extra)
{
    int erroffset;
    const char *errorptr;
    int options = 0;
    ...
    *re = pcre_compile(pattern, options, &errorptr, &erroffset, *tables);
    ...
    if (pcre_st)
    {
        *re_extra = pcre_study(*re, R_PCRE_use_JIT ? PCRE_STUDY_JIT_COMPILE : 0, &errorptr);
        if (errorptr)
    ...
    for (itok = 0; itok < tlen; itok++)
    {
        ...
        { /* NA token doesn't split */
            for (i = itok; i < len; i += tlen)
                SET_VECTOR_ELT(ans, i, ScalarString(markBytes(STRING_ELT(x, i), useBytes)));
            continue;
        }
        else if (!CHAR(this)[0])
        { /* empty */
        ...
        else if (perl_opt)
        {
            ...
            R_pcre_prepare(split, x, use_UTF8, FALSE, TRUE, &tables, &re_pcre, &re_pe);
#endif
            vmax2 = vmaxget();
            for (i = itok; i < len; i += tlen)
            {
                ...
                if (*bufp)
                {
                    int rc;
#ifdef HAVE_PCRE2
                    while ((rc = pcre2_match(re, (PCRE2_SPTR)bufp, PCRE2_ZERO_TERMINATED, 0, eflag, mdata, mcontext)) >=
                           0)
                    {
                        ovector = pcre2_get_ovector_pointer(mdata);
#else
                    while ((rc = pcre_exec(re_pcre, re_pe, bufp, (int)strlen(bufp), 0, 0, ovector, ovecsize)) >= 0)
                    {
#endif
                ...
                {
#ifdef HAVE_PCRE2
                    int rc = pcre2_match(re, (PCRE2_SPTR)bufp, PCRE2_ZERO_TERMINATED, 0, eflag, mdata, mcontext);
#else
                    int rc = pcre_exec(re_pcre, re_pe, bufp, (int)strlen(bufp), 0, 0, ovector, ovecsize);
#endif
                    R_pcre_exec_error(rc, i);
                    if (ovector[1] > 0)
                    {
            ...
            */
            ...
            for (i = itok; i < len; i += tlen)
            {
                ...
                for (j = 0; j < ntok; j++)
                {
                    ...
                    SET_STRING_ELT(t, j, mkCharWLenASCII(wpt, regmatch[0].rm_so, ascii_split && ascii_xi));
                }
                if (*wbufp)
                    SET_STRING_ELT(t, ntok, mkCharWLenASCII(wbufp, (int)wcslen(wbufp), ascii_split && ascii_xi));
                vmaxset(vmax2);
            }
            tre_regfree(&reg);
        }
        else
        { /* ERE in normal chars -- single byte or MBCS */
            regex_t reg;
            regmatch_t regmatch[1];
            int rc;
            int cflags = REG_EXTENDED;
            ...
            */
            ...
            for (i = itok; i < len; i += tlen)
            {
                ...
                for (j = 0; j < ntok; j++)
                {
                    int rc;
                    if (useBytes)
                        rc = tre_regexecb(&reg, bufp, 1, regmatch, 0);
                    else
                        rc = tre_regexec(&reg, bufp, 1, regmatch, 0);
...
static int fgrep_one(const char *pat, const char *target, Rboolean useBytes, Rboolean use_UTF8, int *next)
{
...
static int fgrep_one_bytes(const char *pat, int patlen, const char *target, int len, Rboolean useBytes,
                           Rboolean use_UTF8)
{
    int i = -1;
    const char *p;
    ...
    if (!use_UTF8 && !useBytes && mbcslocale)
    { /* skip along by chars */
        mbstate_t mb_st;
        int ib, used;
        mbs_init(&mb_st);
        ...
        {
            if (strncmp(pat, target + ib, patlen) == 0)
                return ib;
            used = (int)Mbrtowc(NULL, target + ib, R_MB_CUR_MAX, &mb_st);
            if (used <= 0)
                break;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (STRING_ELT(text, i) != NA_STRING)
        {
            ...
            else if (perl_opt)
            {
                ...
                int rc = pcre2_match(re, (PCRE2_SPTR)s, PCRE2_ZERO_TERMINATED, 0, eflag, mdata, mcontext);
#else
...
static R_size_t fgrepraw1(SEXP pat, SEXP text, R_size_t offset)
{
 ...
 single UTF8 chars (up to 3 bytes) */
    ...
    case 3: {
        ...
        while (offset < n)
        {
            if (haystack[offset] == needle[0] && haystack[offset + 1] == needle[1] && haystack[offset + 2] == needle[2])
                return offset;
            offset++;
        }
        return (R_size_t)-1;
...
SEXP attribute_hidden do_grepraw(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    text = CAR(args);
    args = CDR(args);
    offset = asInteger(CAR(args));
    args = CDR(args);
    igcase_opt = asLogical(CAR(args));
    args = CDR(args);
    fixed_opt = asLogical(CAR(args));
    args = CDR(args);
    value = asLogical(CAR(args));
    args = CDR(args);
    ...
    if (fixed_opt)
    {
        ...
        if (!all)
        {
            ...
            if (res == -1)
                return allocVector(value ? RAWSXP : INTSXP, 0);
            if (!value)
                return ScalarInteger((int)(res + 1));
...
#define MAX_MATCHES_MINIBUF 32
            ...
            ans = allocVector(INTSXP, nmatches);
            if (nmatches <= MAX_MATCHES_MINIBUF)
            { /* our min-buffer was enough, great */
                if (nmatches)
                    memcpy(INTEGER(ans), matches, nmatches * sizeof(int));
                return ans;
            }
            ...
            nmatches = MAX_MATCHES_MINIBUF;
            offset =
                matches[MAX_MATCHES_MINIBUF - 1] + LENGTH(pat) - 1; /* matches are 1-based, we are 0-based hence - 1 */
            while (offset < n)
            {
                offset = fgrepraw1(pat, text, offset);
                if (offset == -1)
    ...
    if (rc)
        reg_report(rc, &reg, NULL /* pat is not necessarily a C string */);
    ...
    { /* match only once */
        ...
        if (value)
        {
            if (rc != REG_OK ||
                ptag.rm_eo ==
                    ptag.rm_so) /* TODO: is this good enough? it is the same as matching an empty string ... */
    ...
    while (1)
    {
        ...
        if (value)
            res_val[res_ptr++] = ptag.rm_eo - ptag.rm_so;
        offset += ptag.rm_eo;
        nmatches++;
        if (ptag.rm_eo == 0)
        { /* empty string matched => trouble; FIXME: we may want to consider just advancing anyway */
            int infinite_match = 1;
            /* the only place where this is acceptable is "^" as that will go away in the next step */
            if (nmatches == 1)
            { /* to see if that is true, re-run the match with REG_NOTBOL (added above) */
                rc = tre_regnexecb(&reg, (const char *)RAW(text) + offset, LENGTH(text) - offset, 1, &ptag, eflags);
    ...
    if (value)
    { /* for values we store in fact the absolute start offsets and length in the integer vector */
        ...
        {
            if (invert)
            { /* for invert=TRUE store the current piece up to the match */
            ...
            { /* for invert=FALSE store the matched piece */
        ...
        if (invert)
        { /* add the last piece after the last match */
    ...
    { /* if values are not needed, we just collect all the start offsets */
        ans = allocVector(INTSXP, nmatches);
        res_val = INTEGER(ans);
        while (res_head != R_NilValue)
        {
            ...
            if (len)
                memcpy(res_val, INTEGER(vec), len * sizeof(int));
            res_val += len;
            res_head = CDR(res_head);
        }
    }
...
*/
#ifdef HAVE_PCRE2
static char *R_pcre_string_adj(char *target, const char *orig, const char *repl, PCRE2_SIZE *ovec, Rboolean use_UTF8,
                               int ncap)
#else
static char *R_pcre_string_adj(char *target, const char *orig, const char *repl, int *ovec, Rboolean use_UTF8, int ncap)
#endif
{
    uint64_t i, nb;
    int k;
...
static wchar_t *wstring_adj(wchar_t *target, const wchar_t *orig, const wchar_t *repl, regmatch_t *regmatch)
{
    int i, k;
...
SEXP attribute_hidden do_gsub(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!useBytes)
    {
        ...
        if (!fixed_opt && mbcslocale)
            use_UTF8 = TRUE;
        if (!use_UTF8)
            use_UTF8 = have_utf8(pat, 1) || have_utf8(rep, 1) || have_utf8(text, n);
        if (!use_UTF8 && !latin1locale)
            use_UTF8 = have_latin1(pat, 1) || have_latin1(rep, 1) || have_latin1(text, n);
    }
    ...
    else if (use_WC)
    {
        wpat = wtransChar2(STRING_ELT(pat, 0));
        if (!wpat)
            error(_("'pattern' is invalid"));
        wrep = wtransChar2(STRING_ELT(rep, 0));
        if (!wrep)
            error(_("'replacement' is invalid"));
        ascii_patrep = (IS_ASCII(STRING_ELT(pat, 0)) && IS_ASCII(STRING_ELT(rep, 0)));
    }
    else if (use_UTF8)
    {
        spat = trCharUTF82(STRING_ELT(pat, 0));
    ...
    else
    {
        ...
        else
        {
            rc = tre_regwcomp(&reg, wpat, cflags);
            if (rc)
                reg_report(rc, &reg, translateChar(STRING_ELT(pat, 0)));
            replen = wcslen(wrep);
        }
    }
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (fixed_opt)
        {
            ...
            if (st < 0)
                SET_STRING_ELT(ans, i, markBytes(STRING_ELT(text, i), useBytes));
            else if (STRING_ELT(rep, 0) == NA_STRING)
                SET_STRING_ELT(ans, i, NA_STRING);
            else
            {
                if (global)
                { /* need to find max number of matches */
                    const char *ss = s;
                    int sst = st;
                    nr = 0;
                    do
                    {
                        ...
                        ss += sst + patlen;
                        slen -= (int)(sst + patlen);
                    } while ((sst = fgrep_one_bytes(spat, patlen, ss, slen, useBytes, use_UTF8)) >= 0);
                }
                else
                    nr = 1;
        ...
        else if (perl_opt)
        {
            ...
            while ((ncap = pcre2_match(re, (PCRE2_SPTR)s, (PCRE2_SIZE)ns, (PCRE2_SIZE)offset, eflag, mdata,
                                       mcontext)) >= 0)
            {
        ...
        else
        {
            ...
            else
            {
                ...
                for (j = offset; ws[j]; j++)
                    *u++ = ws[j];
                *u = L'\0';
                SET_STRING_ELT(ans, i, mkCharWLenASCII(cbuf, (u - cbuf), ascii_patrep && ascii_texti));
            }
            R_Free(cbuf);
        }
        vmaxset(vmax);
...
static SEXP gregexpr_Regexc(const regex_t *reg, SEXP sstr, int useBytes, int use_WC, R_xlen_t i, SEXP itype, int *nwarn)
{
...
*/
#ifdef HAVE_PCRE2
static Rboolean ovector_extract_start_length(Rboolean use_UTF8, PCRE2_SIZE *ovector, int *mptr, int *lenptr,
                                             const char *string)
#else
...
/* this function generalizes the parsing of the "ovector" from pcre
   ...
   Toby Dylan Hocking 2011-03-10 */
#ifdef HAVE_PCRE2
static Rboolean extract_match_and_groups(Rboolean use_UTF8, PCRE2_SIZE *ovector, int capture_count, int *mptr,
                                         int *lenptr, int *cptr, int *clenptr, const char *string, int capture_stride)
#else
...
#ifdef HAVE_PCRE2
static SEXP R_pcre2_gregexpr(const char *pattern, const char *string, pcre2_code *re, Rboolean useBytes,
                             Rboolean use_UTF8, pcre2_match_data *mdata, pcre2_match_context *mcontext,
                             int capture_count, SEXP capture_names, R_xlen_t n, SEXP itype)
#else
static SEXP R_pcre_gregexpr(const char *pattern, const char *string, pcre *re_pcre, pcre_extra *re_pe,
                            Rboolean useBytes, Rboolean use_UTF8, int *ovector, int ovector_size, int capture_count,
                            SEXP capture_names, R_xlen_t n, SEXP itype)
#endif
{
    ...
    while (!foundAll)
    {
        ...
        if (use_UTF8)
            eflag |= PCRE2_NO_UTF_CHECK;
        int rc = pcre2_match(re, (PCRE2_SPTR)string, slen, start, eflag, mdata, mcontext);
...
SEXP attribute_hidden do_regexpr(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    else if (perl_opt)
    {
        ...
        info_code = pcre2_pattern_info(re, PCRE2_INFO_CAPTURECOUNT, &capture_count);
        if (info_code < 0)
        ...
        info_code = pcre_fullinfo(re_pcre, re_pe, PCRE_INFO_CAPTURECOUNT, &capture_count);
        if (info_code < 0)
    ...
    { /* regexpr */
        ...
        if (perl_opt && capture_count)
        {
            ...
            is = il = INTEGER(ans); // not actually used, but is + i has to be legal
        vmax = vmaxget();
        for (i = 0; i < n; i++)
        {
            //	    if ((i+1) % NINTERRUPT == 0) R_CheckUserInterrupt();
            ...
            else
            {
                ...
                if (fixed_opt)
                {
                    ...
                    if (!useBytes && use_UTF8)
                    {
                        INTEGER(matchlen)[i] = INTEGER(ans)[i] >= 0 ? (int)utf8towcs(NULL, spat, 0) : -1;
                    }
                    else if (!useBytes && mbcslocale)
                    {
                        INTEGER(matchlen)[i] = INTEGER(ans)[i] >= 0 ? (int)mbstowcs(NULL, spat, 0) : -1;
                    }
                    else
                        INTEGER(matchlen)[i] = INTEGER(ans)[i] >= 0 ? (int)strlen(spat) : -1;
                }
                else if (perl_opt)
                {
                    int rc;
                    ...
                    if (use_UTF8)
                        eflag |= PCRE2_NO_UTF_CHECK;
                    rc = pcre2_match(re, (PCRE2_SPTR)s, PCRE2_ZERO_TERMINATED, 0, eflag, mdata, mcontext);
                    ovector = pcre2_get_ovector_pointer(mdata);
#else
                    ...
                    if (rc >= 0)
                    {
                        extract_match_and_groups(use_UTF8, ovector, (int)capture_count,
...
#ifdef HAVE_PCRE2
                            elt = R_pcre2_gregexpr(spat, s, re, useBytes, use_UTF8, mdata, mcontext, (int)capture_count,
                                                   capture_names, i, itype);
#else
                            elt = R_pcre_gregexpr(spat, s, re_pcre, re_pe, useBytes, use_UTF8, ovector, ovector_size,
                                                  (int)capture_count, capture_names, i, itype);
#endif
                    }
                }
                else
                    elt = gregexpr_Regexc(&reg, STRING_ELT(text, i), useBytes, use_WC, i, itype, &nwarn);
            }
            SET_VECTOR_ELT(ans, i, elt);
            vmaxset(vmax);
        }
...
SEXP attribute_hidden do_regexec(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        else
        {
            vmax = vmaxget();
            if (useBytes)
                rc = tre_regexecb(&reg, CHAR(STRING_ELT(text, i)), nmatch, pmatch, 0);
            else if (use_WC)
            {
...
SEXP attribute_hidden do_pcre_config(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    setAttrib(ans, R_NamesSymbol, nm);
    SET_STRING_ELT(nm, 0, mkChar("UTF-8"));
    pcre_config(PCRE_CONFIG_UTF8, &res);
    lans[0] = res;
    SET_STRING_ELT(nm, 1, mkChar("Unicode properties"));
    pcre_config(PCRE_CONFIG_UNICODE_PROPERTIES, &res);
    lans[1] = res;
    SET_STRING_ELT(nm, 2, mkChar("JIT"));
...
#endif
    lans[2] = res;
    pcre_config(PCRE_CONFIG_STACKRECURSE, &res);
    lans[3] = res;
    SET_STRING_ELT(nm, 3, mkChar("stack"));
    UNPROTECT(1);
    return ans;
