    ...
    for (itok = 0; itok < tlen; itok++)
    {
        ...
        { /* empty */
            ...
            for (i = itok; i < len; i += tlen)
            {
                ...
                if (!useBytes && (use_UTF8 || mbcslocale) && !IS_ASCII(STRING_ELT(x, i)))
                {
...
static int fgrep_one_bytes(const char *pat, size_t patlen, const char *target, int len, Rboolean useBytes,
                           Rboolean use_UTF8)
{
    int i = -1;
    const char *p;
        ...
        else
        {
            ...
            else
            {
                ...
                for (j = offset; ws[j]; j++)
                    *u++ = ws[j];
                *u = L'\0';
                SET_STRING_ELT(ans, i, mkCharWLenASCII(cbuf, (int)(u - cbuf), ascii_patrep && ascii_texti));
            }
            R_Free(cbuf);
        }
        vmaxset(vmax);
