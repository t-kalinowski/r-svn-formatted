...
#define MAX_MATCHES_MINIBUF 32
            ...
            if (value)
            { /* value=TRUE is pathetic for fixed=TRUE as it is just rep(pat, nmatches) */
    ...
    { /* match only once */
        ...
        {
            if (rc != REG_OK)
                return allocVector(RAWSXP,
                                   0); /* TODO: is this good enough? it is the same as matching an empty string ... */
