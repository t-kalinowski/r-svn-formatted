...
{
    int num_eq = 1, single_NA = 1, attr_as_set = 1, ignore_bytecode = 1, ignore_env = 0, ignore_srcref = 1,
        NA_distinct_from_NaN = 1, nargs = length(args), flags;
    ...
    SEXP y = CAR(args);
    args = CDR(args);
    num_eq = asLogical(CAR(args));
    args = CDR(args);
    single_NA = asLogical(CAR(args));
    args = CDR(args);
    attr_as_set = asLogical(CAR(args));
    args = CDR(args);
    if (nargs >= 6)
    ...
    if (ignore_bytecode == NA_LOGICAL)
        error(_("invalid '%s' value"), "ignore.bytecode");
    if (ignore_env == NA_LOGICAL)
        error(_("invalid '%s' value"), "ignore.environment");
    if (ignore_srcref == NA_LOGICAL)
        error(_("invalid '%s' value"), "ignore.srcref");
    if (NA_distinct_from_NaN == NA_LOGICAL)
        error(_("invalid '%s' value"), "NA.distinct.from.NaN");

    flags = (num_eq ? 0 : 1) + (single_NA ? 0 : 2) + (attr_as_set ? 0 : 4) + (ignore_bytecode ? 0 : 8) +
            (ignore_env ? 0 : 16) + (ignore_srcref ? 0 : 32) + (NA_distinct_from_NaN ? 0 : 64);
