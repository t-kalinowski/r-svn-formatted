...
static void PrintEnvironment(SEXP x)
{
    ...
    if (x == R_GlobalEnv)
        ...
        Rprintf("<%s>", translateChar(STRING_ELT(R_PackageEnvName(x), 0)));
...
static void inspect_tree(int pre, SEXP v, int deep, int pvec)
{
    ...
    */
    ...
    switch (TYPEOF(v))
    { /* for native vectors print the first elements in-line */
    case LGLSXP:
        if (XLENGTH(v) > 0)
        {
            unsigned int i = 0;
        ...
        case LANGSXP: {
            ...
            while (lc != R_NilValue)
            {
                ...
                if (TAG(lc) && TAG(lc) != R_NilValue)
                {
                    pp(pre + 2);
                    Rprintf(
                        "TAG: "); /* TAG should be a one-liner since it's a symbol so we don't put it on an extra line*/
                ...
                if (BNDCELL_TAG(lc))
                {
                    ...
                    case LGLSXP:
                        ...
                        break;
                    default:
                        error("unknown immediate binding type");
                    }
                }
                else
