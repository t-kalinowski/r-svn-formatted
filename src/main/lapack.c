...
static sDL_FUNC ptr_svd, ptr_rs, ptr_rg, ptr_zgesv, ptr_zgeqp3, ptr_qr_coef_cmplx, ptr_qr_qy_cmplx, ptr_svd_cmplx,
...
static void La_Init(void)
{
    ...
    if (!(ptr_svd = (sDL_FUNC)R_FindSymbol("modLa_svd", "lapack", NULL)))
        return;
    if (!(ptr_rs = (sDL_FUNC)R_FindSymbol("modLa_rs", "lapack", NULL)))
        return;
    if (!(ptr_rg = (sDL_FUNC)R_FindSymbol("modLa_rg", "lapack", NULL)))
        return;
    if (!(ptr_zgesv = (sDL_FUNC)R_FindSymbol("modLa_zgesv", "lapack", NULL)))
        return;
    if (!(ptr_zgeqp3 = (sDL_FUNC)R_FindSymbol("modLa_zgeqp3", "lapack", NULL)))
        return;
    if (!(ptr_qr_coef_cmplx = (sDL_FUNC)R_FindSymbol("modqr_coef_cmplx", "lapack", NULL)))
        return;
    if (!(ptr_qr_qy_cmplx = (sDL_FUNC)R_FindSymbol("modqr_qy_cmplx", "lapack", NULL)))
        return;
    if (!(ptr_svd_cmplx = (sDL_FUNC)R_FindSymbol("modLa_svd_cmplx", "lapack", NULL)))
        return;
    if (!(ptr_rs_cmplx = (sDL_FUNC)R_FindSymbol("modLa_rs_cmplx", "lapack", NULL)))
        return;
    if (!(ptr_rg_cmplx = (sDL_FUNC)R_FindSymbol("modLa_rg_cmplx", "lapack", NULL)))
        return;
...
SEXP La_svd(SEXP jobu, SEXP jobv, SEXP x, SEXP s, SEXP u, SEXP v)
{
    if (!initialized)
        La_Init();
    if (initialized > 0)
        return (*ptr->svd)(jobu, jobv, x, s, u, v);
    else
    {
