    ...
    case CPLXSXP:
        for (i = 0; i < len; i++)
            ...
            [i] = (ISNAN(COMPLEX(arg)[i].r) || ISNAN(COMPLEX(arg)[i].i))
                      ? NA_LOGICAL
                      : (COMPLEX(arg)[i].r == 0. && COMPLEX(arg)[i].i == 0.);
        break;
    case RAWSXP:
        for (i = 0; i < len; i++)
...
SEXP attribute_hidden do_logic2(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    s1 = eval(s1, env);
    if (!isNumber(s1))
        errorcall(call, _("invalid 'x' type in 'x %s y'"), PRIMVAL(op) == 1 ? "&&" : "||");
    x1 = asLogical(s1);
...
static int checkValues(int op, int na_rm, int *x, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (!na_rm && x[i] == NA_LOGICAL)
            return NA_LOGICAL;
        if (x[i] == TRUE && op == _OP_ANY)
            return TRUE;
        if (x[i] == FALSE && op == _OP_ALL)
            return FALSE;
    }
    switch (op)
    {
...
SEXP attribute_hidden do_logic3(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (s = args; s != R_NilValue; s = CDR(s))
    {
        ...
        if (!narm && val == NA_LOGICAL)
            break;
        if (PRIMVAL(op) == _OP_ANY && val)
            break;
        if (PRIMVAL(op) == _OP_ALL && !val)
            break;
    }
    UNPROTECT(2);
