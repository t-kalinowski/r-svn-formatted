...
Rboolean pmatch(SEXP formal, SEXP tag, Rboolean exact)
{
    ...
    else
    {
        ...
        Rboolean res = psmatch(translateCharUTF8(f), translateCharUTF8(t), exact);
        vmaxset(vmax);
        return res;
    }
}
        ...
        if (ftag != R_DotsSymbol && ftag != R_NilValue)
        {
            for (b = supplied, i = 1; b != R_NilValue; b = CDR(b), i++)
            {
                ...
                if (btag != R_NilValue)
                {
                    ...
                    {
                        if (fargused[arg_i] == 2)
                            errorcall(call, _("formal argument \"%s\" matched by multiple actual arguments"),
                                      CHAR(PRINTNAME(TAG(f))));
                        if (ARGUSED(b) == 2)
                            errorcall(call, _("argument %d matches multiple formal arguments"), i);
                        SETCAR(a, CAR(b));
            ...
            else
            {
                for (b = supplied, i = 1; b != R_NilValue; b = CDR(b), i++)
                {
                    if (ARGUSED(b) != 2 && TAG(b) != R_NilValue && pmatch(TAG(f), TAG(b), seendots))
                    {
                        ...
                        {
                            warningcall(call, _("partial argument match of '%s' to '%s'"), CHAR(PRINTNAME(TAG(b))),
                                        CHAR(PRINTNAME(TAG(f))));
                        }
                        SETCAR(a, CAR(b));
                        if (CAR(b) != R_MissingArg)
    ...
    else
    {
        ...
        if (last != R_NilValue)
        {
            ...
            errorcall(call /* R_GlobalContext->call */,
                      ngettext("unused argument %s", "unused arguments %s", (unsigned long)length(unusedForError)),
...
static R_INLINE void patchArgument(SEXP suppliedSlot, SEXP name, fstype_t *farg, SEXP cloenv)
{
    ...
    if (value == R_MissingArg)
    {
        ...
        {
            if (farg)
                *farg = FS_MATCHED_MISSING;
            return;
        }
        if (farg)
            *farg = FS_MATCHED_LOCAL;
    }
    else if (farg)
        *farg = FS_MATCHED_PRESENT;
...
SEXP attribute_hidden patchArgsByActuals(SEXP formals, SEXP supplied, SEXP cloenv)
{
    ...
    if (!nfarg)
        nfarg = 1; // avoid undefined behaviour
    fstype_t farg[nfarg];
    for (i = 0; i < nfarg; i++)
        farg[i] = FS_UNMATCHED;
