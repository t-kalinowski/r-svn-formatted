...
again:
    ...
    if (warn_bad_sexp_type != 0)
    {
        ...
        if (warn_bad_sexp_type == FREESXP)
            error("GC encountered a node (%p) with type FREESXP (was %d)", warn_bad_sexp_type_sexp,
                  warn_bad_sexp_type_old_type);
        else
            error("GC encountered a node (%p) with an unknown SEXP type: %d", warn_bad_sexp_type_sexp,
                  warn_bad_sexp_type);
#else
        error("GC encountered a node (%p) with an unknown SEXP type: %d", warn_bad_sexp_type_sexp, warn_bad_sexp_type);
#endif
    }
}
