...
#ifdef BC_INT_STACK
    R_BCIntStackBase = (IStackval *)malloc(R_BCINTSTACKSIZE * sizeof(IStackval));
...
static void checkMSet(SEXP mset)
{
    ...
    SEXP isize = TAG(mset);
    if (MAYBE_REFERENCED(mset) || ((store != R_NilValue) && (TYPEOF(store) != VECSXP || MAYBE_REFERENCED(store))) ||
        (TYPEOF(npreserved) != INTSXP || XLENGTH(npreserved) != 1 || MAYBE_REFERENCED(npreserved)) ||
