...
static void AdjustHeapSize(int size_needed)
{
    ...
    double vect_occup = ((double)(R_SmallVallocSize + R_LargeVallocSize + size_needed)) / R_VSize;
    ...
    if (vect_occup > 1.0)
    {
        int k = (R_SmallVallocSize + R_LargeVallocSize + size_needed - R_VSize - 1) / R_VGrowIncr + 1;
...
SEXP do_gc(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    INTEGER(value)[2] = 10.0 * (R_NSize - R_Collected) / 1048576.0 * sizeof(SEXPREC) + 0.999;
