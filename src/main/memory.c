...
again:
    ...
    if (first_bad_sexp_type != 0)
    {
        ...
        if (first_bad_sexp_type == FREESXP)
            error("GC encountered a node (%p) with type FREESXP (was %s)", first_bad_sexp_type_sexp,
                  sexptype2char(first_bad_sexp_type_old_type));
        else
            error("GC encountered a node (%p) with an unknown SEXP type: %s", first_bad_sexp_type_sexp,
