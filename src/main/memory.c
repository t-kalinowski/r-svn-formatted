...
SEXP attribute_hidden do_gc(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    INTEGER(value)[8] = (R_MaxNSize < R_SIZE_T_MAX) ? (10. * R_MaxNSize / Mega * sizeof(SEXPREC) + 0.999) : NA_INTEGER;
    INTEGER(value)[9] = (R_MaxVSize < R_SIZE_T_MAX) ? (10. * R_MaxVSize / Mega * vsfac + 0.999) : NA_INTEGER;
    if (reset_max)
    {
...
SEXP allocVector(SEXPTYPE type, R_len_t length)
{
    ...
    default:
        error(_("invalid type/length (%d/%d) in vector allocation"), type, length);
    }
...
#ifdef USE_TYPE_CHECKING
    if (TYPEOF(x) != STRSXP)
        error("%s() can only be applied to a '%s', not a '%s'", "STRING_ELT", "charcter vector", type2char(TYPEOF(x)));
#endif
...
static void R_InitMemReporting(char *filename, int append, R_size_t threshold)
{
    if (R_MemReportingOutfile != NULL)
        R_EndMemReporting();
