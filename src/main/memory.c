...
attribute_hidden const char *sexptype2char(SEXPTYPE type)
{
    switch (type)
    {
    case NILSXP:
...
#define NODE_GEN_IS_YOUNGER(s, g) (!NODE_IS_MARKED(s) || NODE_GENERATION(s) < (g))
#define NODE_IS_OLDER(x, y)                                                                                            \
    (NODE_IS_MARKED(x) && (y) && (!NODE_IS_MARKED(y) || NODE_GENERATION(x) > NODE_GENERATION(y)))
...
#endif
#define R_PAGE_SIZE (((BASE_PAGE_SIZE - sizeof(PAGE_HEADER)) / sizeof(SEXPREC)) * sizeof(SEXPREC) + sizeof(PAGE_HEADER))
#define NODE_SIZE(c) ((c) == 0 ? sizeof(SEXPREC) : sizeof(SEXPREC_ALIGN) + NodeClassSize[c] * sizeof(VECREC))
...
#define DO_CHILDREN4(__n__, dc__action__, dc__str__action__, dc__extra__)                                              \
    do                                                                                                                 \
    {                                                                                                                  \
...
#define FORWARD_AND_PROCESS_ONE_NODE(s, tp)                                                                            \
    do                                                                                                                 \
    {                                                                                                                  \
        SEXP fpn__n__ = (s);                                                                                           \
        int __tp__ = (tp);                                                                                             \
        ...
        {                                                                                                              \
            if (TYPEOF(fpn__n__) == __tp__ && !HAS_GENUINE_ATTRIB(fpn__n__))                                           \
            {                                                                                                          \
                MARK_AND_UNSNAP_NODE(fpn__n__);                                                                        \
                PROCESS_ONE_NODE(fpn__n__);                                                                            \
            }                                                                                                          \
            else                                                                                                       \
                FORWARD_NODE(fpn__n__);                                                                                \
        }                                                                                                              \
    } while (0)
...
static void DEBUG_CHECK_NODE_COUNTS(char *where)
{
    ...
    {
        for (s = NEXT_NODE(R_GenHeap[i].New), NewCount = 0; s != R_GenHeap[i].New; s = NEXT_NODE(s))
        {
            NewCount++;
            if (i != NODE_CLASS(s))
                gc_error("Inconsistent class assignment for node!\n");
        }
        for (gen = 0, OldCount = 0, OldToNewCount = 0; gen < NUM_OLD_GENERATIONS; gen++)
        {
            for (s = NEXT_NODE(R_GenHeap[i].Old[gen]); s != R_GenHeap[i].Old[gen]; s = NEXT_NODE(s))
            {
                OldCount++;
                if (i != NODE_CLASS(s))
                    gc_error("Inconsistent class assignment for node!\n");
            ...
            for (s = NEXT_NODE(R_GenHeap[i].OldToNew[gen]); s != R_GenHeap[i].OldToNew[gen]; s = NEXT_NODE(s))
            {
                OldToNewCount++;
                if (i != NODE_CLASS(s))
                    gc_error("Inconsistent class assignment for node!\n");
...
{
    int i, gen, OldCount;
    REprintf("\n%s, VSize = %lu", full_gc ? "Full" : "Minor", R_SmallVallocSize + R_LargeVallocSize);
...
static void DEBUG_ADJUST_HEAP_PRINT(double node_occup, double vect_occup)
{
    ...
    alloc = R_LargeVallocSize + sizeof(SEXPREC_ALIGN) * R_GenHeap[LARGE_NODE_CLASS].AllocCount;
...
static void TryToReleasePages(void)
{
    ...
    if (release_count == 0)
    {
        ...
        for (i = 0; i < NUM_SMALL_NODE_CLASSES; i++)
        {
            ...
            for (page = R_GenHeap[i].pages, rel_pages = 0, last = NULL; rel_pages < maxrel_pages && page != NULL;)
            {
                int j, in_use;
...
static int RunGenCollect(R_size_t size_needed)
{
    ...
    while (num_old_gens_to_collect < NUM_OLD_GENERATIONS)
    {
        ...
        {
            collect_counts[num_old_gens_to_collect] = collect_counts_max[num_old_gens_to_collect];
...
again:
    ...
    {
        if (R_Collected < R_MinFreeFrac * R_NSize || VHEAP_FREE() < size_needed + R_MinFreeFrac * R_VSize)
        {
            num_old_gens_to_collect++;
...
attribute_hidden SEXP do_gc(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    REAL(value)[8] = (R_MaxNSize < R_SIZE_T_MAX) ? 0.1 * ceil(10. * R_MaxNSize / Mega * sizeof(SEXPREC)) : NA_REAL;
...
char *R_alloc(size_t nelem, int eltsize)
{
    ...
    if (dsize > 0)
    {
        ...
        if (dsize > R_XLEN_T_MAX) /* currently 4096 TB */
            error(_("cannot allocate memory block of size %0.f Tb"), dsize / R_pow_di(1024.0, 4));
        s = allocVector(RAWSXP, size + 1);
#else
        if (dsize > R_LEN_T_MAX) /* must be in the Gb range */
            error(_("cannot allocate memory block of size %0.1f Gb"), dsize / R_pow_di(1024.0, 3));
        s = allocVector(RAWSXP, size + 1);
#endif
        ATTRIB(s) = R_VStack;
        R_VStack = s;
...
SEXP allocVector3(SEXPTYPE type, R_xlen_t length, R_allocator_t *allocator)
{
    ...
    if (size > 0)
    {
        ...
        else
        {
            ...
            if (size < (R_SIZE_T_MAX / sizeof(VECREC)) - hdrsize)
            { /*** not sure this test is quite right -- why subtract the header? LT */
                ...
                mem = allocator ? custom_node_alloc(allocator, hdrsize + size * sizeof(VECREC))
...
static void R_gc_internal(R_size_t size_needed)
{
    ...
    if (!R_GCEnabled || R_in_gc)
    {
        ...
        if (num_old_gens_to_collect < NUM_OLD_GENERATIONS && VHEAP_FREE() < size_needed + R_MinFreeFrac * R_VSize)
            num_old_gens_to_collect++;
...
again:
    ...
    if (first_bad_sexp_type != 0)
    {
        ...
        if (first_bad_sexp_type == FREESXP)
            snprintf(msg, 256,
                     ...
                     " at memory.c:%d",
                     (void *)first_bad_sexp_type_sexp, sexptype2char(first_bad_sexp_type_old_type),
    ...
    if (R_LogicalNAValue != NULL && LOGICAL(R_LogicalNAValue)[0] != NA_LOGICAL)
    {
        LOGICAL(R_LogicalNAValue)[0] = NA_LOGICAL;
...
{
    error(ngettext("unprotect(): only %d protected item", "unprotect(): only %d protected items", R_PPStackTop),
          R_PPStackTop);
}
...
void NORET R_signal_reprotect_error(PROTECT_INDEX i)
{
    error(ngettext("R_Reprotect: only %d protected item, can't reprotect index %d",
                   "R_Reprotect: only %d protected items, can't reprotect index %d", R_PPStackTop),
          R_PPStackTop, i);
}
...
attribute_hidden void initStack(void)
{
    R_PPStackTop = 0;
}
...
void *R_chk_calloc(size_t nelem, size_t elsize)
{
    ...
    if (!p) /* problem here is that we don't have a format for size_t. */
        error(_("'R_Calloc' could not allocate memory (%.0f of %u bytes)"), (double)nelem, elsize);
    return (p);
}
...
void *R_chk_realloc(void *ptr, size_t size)
{
    ...
    if (ptr)
        ...
        p = malloc(size);
    if (!p)
        error(_("'R_Realloc' could not re-allocate memory (%.0f bytes)"), (double)size);
    return (p);
}
...
void R_ReleaseObject(SEXP object)
{
    ...
    {
        int bin = PTRHASH(object) % PHASH_SIZE;
        SET_VECTOR_ELT(R_PreciousList, bin, DeleteFromList(object, VECTOR_ELT(R_PreciousList, bin)));
    }
    else
...
{
    DECREMENT_REFCNT(CHK(x));
}
void(INCREMENT_REFCNT)(SEXP x)
{
    INCREMENT_REFCNT(CHK(x));
}
void(DISABLE_REFCNT)(SEXP x)
{
    DISABLE_REFCNT(CHK(x));
}
...
{
    return IS_ASSIGNMENT_CALL(CHK(x));
}
void(MARK_ASSIGNMENT_CALL)(SEXP x)
{
    MARK_ASSIGNMENT_CALL(CHK(x));
}

void(SET_ATTRIB)(SEXP x, SEXP v)
{
    if (TYPEOF(v) != LISTSXP && TYPEOF(v) != NILSXP)
        error("value of 'SET_ATTRIB' must be a pairlist or NULL, not a '%s'", type2char(TYPEOF(v)));
...
int(LENGTH)(SEXP x)
{
    return x == R_NilValue ? 0 : LENGTH(CHK2(x));
}
...
{
    return STDVEC_LENGTH(CHK2(x));
}
R_xlen_t(STDVEC_TRUELENGTH)(SEXP x)
{
    return STDVEC_TRUELENGTH(CHK2(x));
}
...
void *(STDVEC_DATAPTR)(SEXP x)
{
    ...
    if (!isVector(x) && TYPEOF(x) != WEAKREFSXP)
        error("STDVEC_DATAPTR can only be applied to a vector, not a '%s'", type2char(TYPEOF(x)));
    CHKZLN(x);
...
{
    if (TYPEOF(x) != LGLSXP)
        error("%s() can only be applied to a '%s', not a '%s'", "LOGICAL", "logical", type2char(TYPEOF(x)));
    CHKZLN(x);
    return LOGICAL(x);
}
...
int *(INTEGER)(SEXP x)
{
    if (TYPEOF(x) != INTSXP && TYPEOF(x) != LGLSXP)
        error("%s() can only be applied to a '%s', not a '%s'", "INTEGER", "integer", type2char(TYPEOF(x)));
    CHKZLN(x);
    return INTEGER(x);
}
...
{
    if (TYPEOF(x) != RAWSXP)
        error("%s() can only be applied to a '%s', not a '%s'", "RAW", "raw", type2char(TYPEOF(x)));
    CHKZLN(x);
    return RAW(x);
}
...
{
    if (TYPEOF(x) != CPLXSXP)
        error("%s() can only be applied to a '%s', not a '%s'", "COMPLEX", "complex", type2char(TYPEOF(x)));
    CHKZLN(x);
    return COMPLEX(x);
}
...
{
    if (TYPEOF(x) != STRSXP)
        error("%s() can only be applied to a '%s', not a '%s'", "STRING_PTR_RO", "character", type2char(TYPEOF(x)));
    CHKZLN(x);
...
void(SET_STRING_ELT)(SEXP x, R_xlen_t i, SEXP v)
{
    if (TYPEOF(CHK(x)) != STRSXP)
        error("%s() can only be applied to a '%s', not a '%s'", "SET_STRING_ELT", "character vector",
              type2char(TYPEOF(x)));
...
static R_INLINE SEXP CHKCONS(SEXP e)
{
    ...
    case EXTPTRSXP: /**** use separate accessors? */
        return CHK(e);
    default:
        error("CAR/CDR/TAG or similar applied to %s object", type2char(TYPEOF(e)));
    }
}
#else
...
attribute_hidden int(BNDCELL_TAG)(SEXP cell)
{
    return BNDCELL_TAG(cell);
}
attribute_hidden void(SET_BNDCELL_TAG)(SEXP cell, int val)
{
    SET_BNDCELL_TAG(cell, val);
}
attribute_hidden double(BNDCELL_DVAL)(SEXP cell)
{
    return BNDCELL_DVAL(cell);
}
attribute_hidden int(BNDCELL_IVAL)(SEXP cell)
{
    return BNDCELL_IVAL(cell);
}
attribute_hidden int(BNDCELL_LVAL)(SEXP cell)
{
    return BNDCELL_LVAL(cell);
}
attribute_hidden void(SET_BNDCELL_DVAL)(SEXP cell, double v)
{
    SET_BNDCELL_DVAL(cell, v);
}
attribute_hidden void(SET_BNDCELL_IVAL)(SEXP cell, int v)
{
    SET_BNDCELL_IVAL(cell, v);
}
attribute_hidden void(SET_BNDCELL_LVAL)(SEXP cell, int v)
{
    SET_BNDCELL_LVAL(cell, v);
}
attribute_hidden void(INIT_BNDCELL)(SEXP cell, int type)
{
...
SEXP(TAG)(SEXP e)
{
    return CHK(TAG(CHKCONS(e)));
}
SEXP(CAR0)(SEXP e)
{
    return CHK(CAR0(CHKCONS(e)));
}
SEXP(CDR)(SEXP e)
{
    return CHK(CDR(CHKCONS(e)));
}
SEXP(CAAR)(SEXP e)
{
    return CHK(CAAR(CHKCONS(e)));
}
SEXP(CDAR)(SEXP e)
{
    return CHK(CDAR(CHKCONS(e)));
}
SEXP(CADR)(SEXP e)
{
    return CHK(CADR(CHKCONS(e)));
}
SEXP(CDDR)(SEXP e)
{
    return CHK(CDDR(CHKCONS(e)));
}
SEXP(CDDDR)(SEXP e)
{
    return CHK(CDDDR(CHKCONS(e)));
}
SEXP(CADDR)(SEXP e)
{
    return CHK(CADDR(CHKCONS(e)));
}
SEXP(CADDDR)(SEXP e)
{
    return CHK(CADDDR(CHKCONS(e)));
}
SEXP(CAD4R)(SEXP e)
{
...
{
    SEXP cell;
    if (CHKCONS(x) == NULL || x == R_NilValue || CHKCONS(CDR(x)) == NULL || CDR(x) == R_NilValue)
        error(_("bad value"));
    cell = CDR(x);
    CLEAR_BNDCELL_TAG(cell);
...
SEXP(SETCADDDR)(SEXP x, SEXP y)
{
    SEXP cell;
    if (CHKCONS(x) == NULL || x == R_NilValue || CHKCONS(CDR(x)) == NULL || CDR(x) == R_NilValue ||
        CHKCONS(CDDR(x)) == NULL || CDDR(x) == R_NilValue || CHKCONS(CDDDR(x)) == NULL || CDDDR(x) == R_NilValue)
        error(_("bad value"));
    cell = CDDDR(x);
    CLEAR_BNDCELL_TAG(cell);
    FIX_REFCNT(cell, CAR(cell), y);
...
{
    return EXTPTR_PROT(CHK(x));
}
SEXP(EXTPTR_TAG)(SEXP x)
{
    return EXTPTR_TAG(CHK(x));
}
void *(EXTPTR_PTR)(SEXP x)
{
    return EXTPTR_PTR(CHK(x));
}

void(SET_MISSING)(SEXP x, int v)
{
    SET_MISSING(CHKCONS(x), v);
}

/* Closure Accessors */
SEXP(FORMALS)(SEXP x)
{
    return CHK(FORMALS(CHK(x)));
}
SEXP(BODY)(SEXP x)
{
    return CHK(BODY(CHK(x)));
}
SEXP(CLOENV)(SEXP x)
{
    return CHK(CLOENV(CHK(x)));
}
int(RDEBUG)(SEXP x)
{
    return RDEBUG(CHK(x));
...
{
    FIX_REFCNT(x, FORMALS(x), v);
    CHECK_OLD_TO_NEW(x, v);
    FORMALS(x) = v;
}
void(SET_BODY)(SEXP x, SEXP v)
{
    FIX_REFCNT(x, BODY(x), v);
    CHECK_OLD_TO_NEW(x, v);
    BODY(x) = v;
}
void(SET_CLOENV)(SEXP x, SEXP v)
{
    FIX_REFCNT(x, CLOENV(x), v);
    CHECK_OLD_TO_NEW(x, v);
    CLOENV(x) = v;
}
...
{
    return CHK(PRINTNAME(CHK(x)));
}
SEXP(SYMVALUE)(SEXP x)
{
    return CHK(SYMVALUE(CHK(x)));
}
SEXP(INTERNAL)(SEXP x)
{
    return CHK(INTERNAL(CHK(x)));
}
int(DDVAL)(SEXP x)
{
    return DDVAL(CHK(x));
...
SEXP(FRAME)(SEXP x)
{
    return CHK(FRAME(CHK(x)));
}
SEXP(ENCLOS)(SEXP x)
{
    return CHK(ENCLOS(CHK(x)));
}
SEXP(HASHTAB)(SEXP x)
{
    return CHK(HASHTAB(CHK(x)));
}
int(ENVFLAGS)(SEXP x)
{
    return ENVFLAGS(CHK(x));
}

void(SET_FRAME)(SEXP x, SEXP v)
{
    FIX_REFCNT(x, FRAME(x), v);
    CHECK_OLD_TO_NEW(x, v);
    FRAME(x) = v;
}
...
attribute_hidden SEXP(SET_CXTAIL)(SEXP x, SEXP v)
{
    ...
    if (TYPEOF(v) != CHARSXP && TYPEOF(v) != NILSXP)
        error("value of 'SET_CXTAIL' must be a char or NULL, not a '%s'", type2char(TYPEOF(v)));
#endif
...
static void R_OutputStackTrace(FILE *file)
{
    ...
    for (cptr = R_GlobalContext; cptr; cptr = cptr->nextcontext)
    {
        if ((cptr->callflag & (CTXT_FUNCTION | CTXT_BUILTIN)) && TYPEOF(cptr->call) == LANGSXP)
        {
            ...
            fprintf(file, "\"%s\" ", TYPEOF(fun) == SYMSXP ? CHAR(PRINTNAME(fun)) : "<Anonymous>");
        }
    }
}
