...
#define NODE_IS_OLDER(x, y) (NODE_IS_MARKED(x) && (!NODE_IS_MARKED(y) || NODE_GENERATION(x) > NODE_GENERATION(y)))
...
Rboolean ALTREP_INSPECT(SEXP x, int pre, int deep, int pvec, void (*inspect_subtree)(SEXP, int, int, int))
{
    return FALSE;
}
void *ALTVEC_DATAPTR(SEXP x, Rboolean writeable)
{
    return NULL;
}
void *ALTVEC_DATAPTR_OR_NULL(SEXP x, Rboolean writeable)
{
    return NULL;
}
