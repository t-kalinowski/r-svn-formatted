...
static int MatchVar(SEXP var1, SEXP var2)
{
    ...
    if (isString(var1) && isString(var2))
        return (strcmp(translateChar(STRING_ELT(var1, 0)), translateChar(STRING_ELT(var2, 0))) == 0);
...
SEXP attribute_hidden do_modelframe(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (nc > 0)
    {
        ...
        for (i = 0; i < nc; i++)
        {
            ...
            if (nrows(ans) != nr)
                errorcall(call, _("variable lengths differ (found for '%s')"), translateChar(STRING_ELT(names, i)));
        }
    }
    else
        nr = length(row_names);
