...
attribute_hidden SEXP installNoTrChar(SEXP charSXP)
{
    SEXP sym;
    int i, hashcode;
    ...
    /* Check to see if the symbol is already present;  if it is, return it. */
    for (sym = R_SymbolTable[i]; sym != R_NilValue; sym = CDR(sym))
        if (strcmp(CHAR(charSXP), CHAR(PRINTNAME(CAR(sym)))) == 0)
            return (CAR(sym));
    /* Create a new symbol node and link it into the table. */
    ...
    if (len > MAXIDSIZE)
        error(_("variable names are limited to %d bytes"), MAXIDSIZE);
    if (IS_ASCII(charSXP) || (IS_UTF8(charSXP) && utf8locale) || (IS_LATIN1(charSXP) && latin1locale))
...
attribute_hidden SEXP installS3Signature(const char *className, const char *methodName)
{
...
SEXP attribute_hidden do_internal(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (INTERNAL(fun) == R_NilValue)
        errorcall(call, _("there is no .Internal function '%s'"), CHAR(PRINTNAME(fun)));
    ...
    if (flag < 2 && flag == R_Visible)
    {
        ...
        if (strcmp(nm, "eval") && strcmp(nm, "options") && strcmp(nm, "Recall") && strcmp(nm, "do.call") &&
            strcmp(nm, "switch") && strcmp(nm, "recordGraphics") && strcmp(nm, "writeBin") && strcmp(nm, "NextMethod"))
