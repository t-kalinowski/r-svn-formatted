    ...
    for (i = 0; i < nclass + S4toS3; i++)
    {
        ...
        if (isFunction(sxp))
        {
            ...
            if (S4toS3 && (i < nclass))
            {
                ...
                { /* Design error: S3 method defined for S4 class
...
SEXP attribute_hidden do_usemethod(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    */
    val = findVar1(install(translateChar(STRING_ELT(generic, 0))), ENCLOS(env), FUNSXP,
                   TRUE); /* That has evaluated promises */
