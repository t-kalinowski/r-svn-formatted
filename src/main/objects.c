...
static SEXP GetObject(RCNTXT *cptr)
{
    ...
    if (TYPEOF(b) != CLOSXP)
        error("non-closure generic function");
    formals = FORMALS(b);
...
SEXP do_nextmethod(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (TYPEOF(s) != CLOSXP)
    {
        errorcall(R_NilValue, "function is not a closure, but of type %d", TYPEOF(s));
    }
...
SEXP do_standardGeneric(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (isNull(fdef))
        error("Call to standardGeneric(\"%s\") apparently not from the body of that generic function",
...
SEXP do_set_prim_method(SEXP op, char *code_string, SEXP fundef, SEXP mlist)
{
    ...
    if (errorcase)
    {
        error("Invalid primitive methods code (\"%s\"): should be \"clear\", \"reset\", \"set\", or \"suppress\"",
              code_string);
        return R_NilValue;
    }
    switch (TYPEOF(op))
