...
attribute_hidden SEXP R_LookupMethod(SEXP method, SEXP rho, SEXP callrho, SEXP defrho)
{
    ...
    if (lookup_baseenv_after_globalenv == -1)
    {
        ...
        lookup_baseenv_after_globalenv = ((lookup != NULL) && StringFalse(lookup)) ? 0 : 1;
    }

    if (lookup_report_search_path_uses == -1)
    {
        lookup = getenv("_R_S3_METHOD_LOOKUP_REPORT_SEARCH_PATH_USES_");
        lookup_report_search_path_uses = ((lookup != NULL) && StringTrue(lookup)) ? 1 : 0;
    }
    ...
    if (lookup_baseenv_after_globalenv)
    {
        if (top == R_GlobalEnv)
            ...
            top = ENCLOS(top);
        REPROTECT(val = findFunWithBaseEnvAfterGlobalEnv(method, top), validx);
    }
    else if (lookup_report_search_path_uses)
    {
        if (top != R_GlobalEnv)
            REPROTECT(val = findFunInEnvRange(method, ENCLOS(top), R_GlobalEnv), validx);
        if (val == R_UnboundValue)
        {
            REPROTECT(val = findFunInEnvRange(method, ENCLOS(R_GlobalEnv), R_EmptyEnv), validx);
            if ((val != R_UnboundValue) && (CLOENV(val) != R_BaseNamespace) && (CLOENV(val) != R_BaseEnv))
            {
        ...
        REPROTECT(val = findFunInEnvRange(method, ENCLOS(top), R_EmptyEnv), validx);
...
{
    return (arg == obj) || (TYPEOF(arg) == PROMSXP && PRVALUE(arg) == obj);
}
#endif
...
static SEXP dispatchMethod(SEXP op, SEXP sxp, SEXP dotClass, RCNTXT *cptr, SEXP method, const char *generic, SEXP rho,
...
attribute_hidden SEXP NORET do_usemethod(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (do_usemethod_formals == NULL)
        do_usemethod_formals = allocFormalsList2(install("generic"), install("object"));
    ...
    if (lookup_use_topenv_as_defenv == -1)
    {
        ...
        lookup_use_topenv_as_defenv = ((lookup != NULL) && StringFalse(lookup)) ? 0 : 1;
    }
    ...
    */
    ...
    {
        val = findVar1(installTrChar(STRING_ELT(generic, 0)), ENCLOS(env), FUNSXP, TRUE); /* That has evaluated
                                                                                           * promises */
...
attribute_hidden SEXP do_nextmethod(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    */
    ...
    SEXP newvars = PROTECT(createS3Vars(generic, group, s, method, callenv, defenv));
...
int R_check_class_and_super(SEXP x, const char **valid, SEXP rho)
{
    ...
    if (IS_S4_OBJECT(x))
    {
         ...
         */
        PROTECT(_call = lang6(s_selectSuperCl, classExts, ScalarLogical(1), ScalarLogical(1), ScalarLogical(0),
                              ScalarLogical(1)));
...
attribute_hidden SEXP do_standardGeneric(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (isNull(fdef))
        error(_("call to standardGeneric(\"%s\") apparently not from the body of that generic function"),
              translateChar(STRING_ELT(arg, 0)));
...
SEXP do_set_prim_method(SEXP op, const char *code_string, SEXP fundef, SEXP mlist)
{
    ...
    if (errorcase)
    {
        error(_("invalid primitive methods code (\"%s\"): should be \"clear\", \"reset\", \"set\", or \"suppress\""),
              code_string);
        return R_NilValue;
    }
    switch (TYPEOF(op))
 ...
 */
attribute_hidden SEXP R_possible_dispatch(SEXP call, SEXP op, SEXP args, SEXP rho, Rboolean promisedArgs)
{
...
SEXP R_getClassDef_R(SEXP what)
{
    ...
    if (!isMethodsDispatchOn())
        error(_("'methods' package not yet loaded"));
...
Rboolean R_isVirtualClass(SEXP class_def, SEXP env)
{
    ...
    if (!isVCl_sym)
        isVCl_sym = install("isVirtualClass");
...
SEXP R_do_new_object(SEXP class_def)
{
    ...
    if (asLogical(e) != 0)
    { /* includes NA, TRUE, or anything other than FALSE */
    ...
    Rboolean xDataType = TYPEOF(value) == ENVSXP || TYPEOF(value) == SYMSXP || TYPEOF(value) == EXTPTRSXP;
    if ((TYPEOF(value) == S4SXP || getAttrib(e, R_PackageSymbol) != R_NilValue) && !xDataType)
    {
...
SEXP asS4(SEXP s, Rboolean flag, int complete)
{
    ...
    else
    {
        if (complete)
        {
            ...
            if ((value = R_getS4DataSlot(s, ANYSXP)) != R_NilValue && !IS_S4_OBJECT(value))
            {
                UNPROTECT(1);
                return value;
            }
