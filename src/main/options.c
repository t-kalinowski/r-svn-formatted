...
void attribute_hidden InitOptions(void)
{
    ...
    {
    case DEFAULT_TRANSLIT_NO:
        p = "no";
        break;
    case DEFAULT_TRANSLIT_NONASCII:
        p = "non-ASCII";
        break;
    case DEFAULT_TRANSLIT_YES:
        p = "yes";
        break;
    case DEFAULT_TRANSLIT_R211:
        p = "R 2.11";
        break;
    }
    SETCAR(v, mkString(p));
        ...
        { /* name = value  ---> assignment */
            ...
            if (argi == R_NilValue)
            {
                ...
                const char *mandatory[] = {"prompt", "continue", "expressions", "width", "deparse.cutoff", "digits",
                                           ...
                                           "PCRE_use_JIT", "default.translit", "PCRE_limit_recursion", "rl_word_breaks",
                                           /* ^^^ from InitOptions ^^^ */
            ...
            else if (streql(CHAR(namei), "digits"))
            {
                ...
                if (k < R_MIN_DIGITS_OPT || k > R_MAX_DIGITS_OPT)
                    error(_("invalid 'digits' parameter, allowed %d...%d"), R_MIN_DIGITS_OPT, R_MAX_DIGITS_OPT);
                SET_VECTOR_ELT(value, i, SetOption(tag, ScalarInteger(k)));
            }
            else if (streql(CHAR(namei), "expressions"))
            {
                ...
                if (k < R_MIN_EXPRESSIONS_OPT || k > R_MAX_EXPRESSIONS_OPT)
                    error(_("'expressions' parameter invalid, allowed %d...%d"), R_MIN_EXPRESSIONS_OPT,
