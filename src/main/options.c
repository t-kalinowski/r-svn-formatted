        ...
        { /* name = value  ---> assignment */
            ...
            if (streql(CHAR(namei), "width"))
            {
                ...
                if (k < R_MIN_WIDTH_OPT || k > R_MAX_WIDTH_OPT)
                    errorcall(call, _("invalid width parameter, allowed %d...%d"), R_MIN_WIDTH_OPT, R_MAX_WIDTH_OPT);
            ...
            else if (streql(CHAR(namei), "digits"))
            {
                ...
                if (k < R_MIN_DIGITS_OPT || k > R_MAX_DIGITS_OPT)
                    errorcall(call, _("invalid digits parameter, allowed %d...%d"), R_MIN_DIGITS_OPT, R_MAX_DIGITS_OPT);
                SET_VECTOR_ELT(value, i, SetOption(tag, ScalarInteger(k)));
            }
            else if (streql(CHAR(namei), "expressions"))
            {
                ...
                if (k < R_MIN_EXPRESSIONS_OPT || k > R_MAX_EXPRESSIONS_OPT)
                    errorcall(call, _("expressions parameter invalid, allowed %d...%d"), R_MIN_EXPRESSIONS_OPT,
            ...
            {
                if (TYPEOF(argi) != STRSXP || LENGTH(argi) != 1 || strlen(CHAR(STRING_ELT(argi, 0))) != 1)
            ...
            else if (streql(CHAR(namei), "max.contour.segments"))
            {
                k = asInteger(argi);
                if (k < 0 || k == NA_INTEGER)
                    errorcall(call, _("max.contour.segment parameter invalid"));
