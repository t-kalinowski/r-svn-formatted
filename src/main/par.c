...
static SEXP Query(const char *what, pGEDevDesc dd)
{
    ...
    else if (streql(what, "cxy"))
    {
        ...
        /* == par("cin") / par("pin") : */
        REAL(value)
        [0] = Rf_dpptr(dd)->cra[0] * dd->dev->ipr[0] / Rf_dpptr(dd)->pin[0] *
              (Rf_dpptr(dd)->usr[1] - Rf_dpptr(dd)->usr[0]);
        REAL(value)
        [1] = Rf_dpptr(dd)->cra[1] * dd->dev->ipr[1] / Rf_dpptr(dd)->pin[1] *
...
SEXP attribute_hidden do_layout(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < nrow * ncol; i++)
        Rf_dpptr(dd)->order[i] = Rf_gpptr(dd)->order[i] = INTEGER(CAR(args))[i];
    args = CDR(args);
    ...
    /* respect.mat */
    for (i = 0; i < nrow * ncol; i++)
        Rf_dpptr(dd)->respect[i] = Rf_gpptr(dd)->respect[i] = INTEGER(CAR(args))[i];
