...
static const ParTab ParTable[] = {{"adj", 0},
                                  {"ann", 0},
                                  {"ask", 1},
                                  {"bg", 0},
                                  {"bty", 0},
...
static int ParCode(const char *what)
{
    ...
    for (i = 0; ParTable[i].name; i++)
        if (!strcmp(what, ParTable[i].name))
            return ParTable[i].code;
    return -1;
}
...
static SEXP Query(const char *what, pGEDevDesc dd)
{
    ...
    else if (streql(what, "pch"))
    {
        ...
        /* we need to be careful that par("pch") is converted back
           to the same value */
        if (known_to_be_latin1 && val <= -32 && val >= -255)
            val = -val;
...
SEXP attribute_hidden do_layout(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < nrow * ncol; i++)
        dpptr(dd)->order[i] = gpptr(dd)->order[i] = (unsigned short)INTEGER(CAR(args))[i];
    args = CDR(args);
    ...
    /* respect.mat */
    for (i = 0; i < nrow * ncol; i++)
        dpptr(dd)->respect[i] = gpptr(dd)->respect[i] = (unsigned char)INTEGER(CAR(args))[i];
