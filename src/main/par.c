...
static void Specify(char *what, SEXP value, DevDesc *dd)
{
    ...
    else if (streql(what, "mfg"))
    {
        ...
        if (gpptr(dd)->mfind)
            ...
            dpptr(dd)->currentFigure = (row - 1) * ncol + col;
        /*
    ...
    else if (streql(what, "usr"))
    {
        ...
        else
        {
            ...
            gpptr(dd)->logusr[0] = dpptr(dd)->logusr[0] = R_Log10(REAL(value)[0]);
            gpptr(dd)->logusr[1] = dpptr(dd)->logusr[1] = R_Log10(REAL(value)[1]);
        }
        if (gpptr(dd)->ylog)
        {
        ...
        else
        {
            ...
            gpptr(dd)->logusr[2] = dpptr(dd)->logusr[2] = R_Log10(REAL(value)[2]);
            gpptr(dd)->logusr[3] = dpptr(dd)->logusr[3] = R_Log10(REAL(value)[3]);
        }
...
void Specify2(char *what, SEXP value, DevDesc *dd)
{
    ...
    else if (streql(what, "yaxt"))
    {
        ...
        warning("parameter \"%s\" couldn't be set in high-level plot() function", what);
}
...
static SEXP Query(char *what, DevDesc *dd)
{
    ...
    else if (streql(what, "cxy"))
    {
        ...
        /* == par("cin") / par("pin") : */
        REAL(value)
        [0] = dpptr(dd)->cra[0] * dpptr(dd)->ipr[0] / dpptr(dd)->pin[0] * (dpptr(dd)->usr[1] - dpptr(dd)->usr[0]);
        REAL(value)
        [1] = dpptr(dd)->cra[1] * dpptr(dd)->ipr[1] / dpptr(dd)->pin[1] * (dpptr(dd)->usr[3] - dpptr(dd)->usr[2]);
    }
...
SEXP do_layout(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < nrow; i++)
        for (j = 0; j < ncol; j++)
            dpptr(dd)->order[i][j] = gpptr(dd)->order[i][j] = INTEGER(CAR(args))[i + j * nrow];
    args = CDR(args);

    /* num.figures: */
    dpptr(dd)->currentFigure = gpptr(dd)->currentFigure = dpptr(dd)->lastFigure = gpptr(dd)->lastFigure =
        INTEGER(CAR(args))[0];
    args = CDR(args);
    /* col.widths: */
    for (j = 0; j < ncol; j++)
    ...
    for (j = 0; j < ncmcol; j++)
    {
        dpptr(dd)->cmWidths[INTEGER(CAR(args))[j] - 1] = gpptr(dd)->cmWidths[INTEGER(CAR(args))[j] - 1] = 1;
    }
    args = CDR(args);
    /* cm.heights: */
    ...
    for (i = 0; i < ncmrow; i++)
    {
        dpptr(dd)->cmHeights[INTEGER(CAR(args))[i] - 1] = gpptr(dd)->cmHeights[INTEGER(CAR(args))[i] - 1] = 1;
    }
    args = CDR(args);
    ...
    for (i = 0; i < nrow; i++)
        for (j = 0; j < ncol; j++)
            dpptr(dd)->respect[i][j] = gpptr(dd)->respect[i][j] = INTEGER(CAR(args))[i + j * nrow];
