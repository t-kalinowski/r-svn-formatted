...
static int Specify(char *what, SEXP value, DevDesc *dd)
{
    ...
    else if (streql(what, "mfg"))
    {
        ...
        if (dd->gp.mfind)
            ...
            dd->dp.currentFigure = (row - 1) * ncol + col;
        /*
    ...
    else if (streql(what, "usr"))
    {
        ...
        else
        {
            ...
            dd->gp.logusr[0] = dd->dp.logusr[0] = R_Log10(REAL(value)[0]);
            dd->gp.logusr[1] = dd->dp.logusr[1] = R_Log10(REAL(value)[1]);
        }
        if (dd->gp.ylog)
        {
        ...
        else
        {
            ...
            dd->gp.logusr[2] = dd->dp.logusr[2] = R_Log10(REAL(value)[2]);
            dd->gp.logusr[3] = dd->dp.logusr[3] = R_Log10(REAL(value)[3]);
        }
...
static SEXP Query(char *what, DevDesc *dd)
{
    ...
    else if (streql(what, "cxy"))
    {
        value = allocVector(REALSXP, 2);
        /* == par("cin") / par("pin") : */
        REAL(value)[0] = dd->dp.cra[0] * dd->dp.ipr[0] / dd->dp.pin[0] * (dd->dp.usr[1] - dd->dp.usr[0]);
        REAL(value)[1] = dd->dp.cra[1] * dd->dp.ipr[1] / dd->dp.pin[1] * (dd->dp.usr[3] - dd->dp.usr[2]);
    }
