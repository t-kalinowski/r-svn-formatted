...
SEXP attribute_hidden do_paste(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    { // we allow one less for capture from earlier versions
	    ...
	    REprintf("%d arguments passed to .Internal(%s) which requires %d;\n an S4 method"
            ...
            error(ngettext("%d argument passed to .Internal(%s) which requires %d",
                           "%d arguments passed to .Internal(%s) which requires %d", (unsigned long)nargs),
                  nargs, PRIMNAME(op), PRIMARITY(op));
    }
#endif
    ...
    int sepw, u_sepw;
    Rboolean sepASCII = TRUE, sepUTF8 = FALSE, sepBytes = FALSE, sepKnown = FALSE, use_sep = (PRIMVAL(op) == 0);
    if (use_sep)
    { /* paste(..., sep, .) */
        sep = CADR(args);
...
SEXP attribute_hidden do_filepath(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (int i = 0; i < maxlen; i++)
    {
        ...
        if (streql(csep, "/") || streql(csep, "\\"))
        {
            ...
            {
                buf--;
                if (*buf == csep[0] && buf > cbuf && (buf != cbuf + 2 || cbuf[1] != ':'))
                    *buf = '\0';
            }
        }
#endif
