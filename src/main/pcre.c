...
static int length_adj(char *orig, char *repl, int *ovec, int nsubexpr, Rboolean useBytes)
{
    int k, n, nb;
    char *p = repl;
...
static char *string_adj(char *target, char *orig, char *repl, int *ovec, Rboolean useBytes)
{
    int i, k, nb;
...
SEXP attribute_hidden do_pgsub(char *spat, char *srep, SEXP vec, int global, int igcase_opt, int useBytes)
{
    SEXP ans;
...
SEXP attribute_hidden do_gpregexpr(char *spat, SEXP text, int igcase_opt, int useBytes)
{
