...
static char *pcre_string_adj(char *target, const char *orig, const char *repl, int *ovec, int ienc)
{
    int i, k, nb;
    const char *p = repl;
    char *t = target, c;
...
SEXP attribute_hidden do_pgsub(SEXP pat, SEXP rep, SEXP text, int global, int igcase_opt, int useBytes)
{
    SEXP ans;
