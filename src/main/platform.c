...
static void count_files(const char *dnp, int *count, Rboolean allfiles, Rboolean recursive, const regex_t *reg)
{
    DIR *dir;
    struct dirent *de;
    char p[PATH_MAX];
                ...
                if (recursive)
                {
                    ...
                    if ((sb.st_mode & S_IFDIR) > 0)
                    {
                        if (strcmp(de->d_name, ".") && strcmp(de->d_name, ".."))
                        {
                            ...
                            list_files(p, stem2, count, ans, allfiles, recursive, reg);
                        }
                        continue;
                    }
                }
...
SEXP attribute_hidden do_listfiles(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < ndir; i++)
    {
        if (STRING_ELT(d, i) == NA_STRING)
            continue;
        dnp = R_ExpandFileName(translateChar(STRING_ELT(d, i)));
        list_files(dnp, fullnames ? dnp : NULL, &count, ans, allfiles, recursive, pattern ? &reg : NULL);
    }
    if (pattern)
        tre_regfree(&reg);
