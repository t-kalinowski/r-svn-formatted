...
SEXP attribute_hidden do_fileinfo(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
    {
                ...
                == 0)
        {
            ...
            REAL(mtime)[i] = (double)STAT_TIMESPEC(sb, st_mtim).tv_sec + 1e-9 * STAT_TIMESPEC(sb, st_mtim).tv_nsec;
            REAL(ctime)[i] = (double)STAT_TIMESPEC(sb, st_ctim).tv_sec + 1e-9 * STAT_TIMESPEC(sb, st_ctim).tv_nsec;
            REAL(atime)[i] = (double)STAT_TIMESPEC(sb, st_atim).tv_sec + 1e-9 * STAT_TIMESPEC(sb, st_atim).tv_nsec;
#else
...
SEXP attribute_hidden do_Cstack_info(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    INTEGER(ans)[1] = (R_CStackLimit == -1) ? NA_INTEGER : R_CStackDir * (R_CStackStart - (uintptr_t)&ans);
...
#define TMN_REPARSE_DATA_BUFFER_HEADER_SIZE FIELD_OFFSET(TMN_REPARSE_DATA_BUFFER, SubstituteNameOffset)
...
SEXP attribute_hidden do_mkjunction(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    DWORD dwBytes;
    const BOOL bOK = DeviceIoControl(hd, FSCTL_SET_REPARSE_POINT, &rdb,
                                     rdb.ReparseDataLength + TMN_REPARSE_DATA_BUFFER_HEADER_SIZE, NULL, 0, &dwBytes, 0);
