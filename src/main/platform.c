...
SEXP attribute_hidden do_fileappend(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    { /* common case */
        ...
        {
            status = 0;
            if (!(fp2 = R_fopen(R_ExpandFileName(CHAR(STRING_ELT(f2, i))), "rb")))
                continue;
            while ((nchar = fread(buf, 1, APPENDBUFSIZE, fp2)) == APPENDBUFSIZE)
                if (fwrite(buf, 1, APPENDBUFSIZE, fp1) != APPENDBUFSIZE)
                    goto append_error;
    ...
    else
    {
        for (i = 0; i < n; i++)
        {
            if (STRING_ELT(f1, i % n1) == R_NilValue || STRING_ELT(f2, i % n2) == R_NilValue)
                ...
                LOGICAL(ans)[i] = R_AppendFile(CHAR(STRING_ELT(f1, i % n1)), CHAR(STRING_ELT(f2, i % n2)));
        }
    }
done:
    UNPROTECT(1);
...
SEXP attribute_hidden do_filecreate(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    {
        LOGICAL(ans)[i] = 0;
        if (STRING_ELT(fn, i) != R_NilValue && (fp = R_fopen(R_ExpandFileName(CHAR(STRING_ELT(fn, i))), "w")) != NULL)
        {
            LOGICAL(ans)[i] = 1;
            fclose(fp);
        }
...
SEXP attribute_hidden do_fileremove(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (STRING_ELT(f, i) != R_NilValue)
            LOGICAL(ans)[i] = (remove(R_ExpandFileName(CHAR(STRING_ELT(f, i)))) == 0);
    }
    UNPROTECT(1);
    return ans;
}
...
SEXP attribute_hidden do_indexsearch(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < npath; i++)
    {
        snprintf(linebuf, 256, "%s%s%s%s%s", CHAR(STRING_ELT(path, i)), CHAR(STRING_ELT(sep, 0)), "help",
            ...
            while (filbuf(linebuf, fp))
            {
                if (strncmp(linebuf, topicbuf, ltopicbuf) == 0)
                {
                    ...
                    if (!strcmp(ctype, "html"))
                        snprintf(topicbuf, 256, "%s%s%s%s%s%s", CHAR(STRING_ELT(path, i)), CHAR(STRING_ELT(sep, 0)),
                                 "html", CHAR(STRING_ELT(sep, 0)), p, ".html");
                    else if (!strcmp(ctype, "R-ex"))
                        snprintf(topicbuf, 256, "%s%s%s%s%s%s", CHAR(STRING_ELT(path, i)), CHAR(STRING_ELT(sep, 0)),
                                 "R-ex", CHAR(STRING_ELT(sep, 0)), p, ".R");
                    else if (!strcmp(ctype, "latex"))
                        snprintf(topicbuf, 256, "%s%s%s%s%s%s", CHAR(STRING_ELT(path, i)), CHAR(STRING_ELT(sep, 0)),
                                 "latex", CHAR(STRING_ELT(sep, 0)), p, ".tex");
                    else /* type = "help" */
                        snprintf(topicbuf, 256, "%s%s%s%s%s", CHAR(STRING_ELT(path, i)), CHAR(STRING_ELT(sep, 0)),
...
SEXP attribute_hidden do_fileaccess(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
        INTEGER(ans)[i] = access(R_ExpandFileName(CHAR(STRING_ELT(fn, i))), modemask);
    UNPROTECT(1);
    return ans;
}
#else
...
SEXP attribute_hidden do_pathexpand(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
        SET_STRING_ELT(ans, i, mkChar(R_ExpandFileName(CHAR(STRING_ELT(fn, i)))));
    UNPROTECT(1);
    return ans;
}
