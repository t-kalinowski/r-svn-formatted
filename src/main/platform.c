...
SEXP attribute_hidden do_fileremove(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (STRING_ELT(f, i) != R_NilValue)
            LOGICAL(ans)[i] = (remove(R_ExpandFileName(translateChar(STRING_ELT(f, i)))) == 0);
    }
    UNPROTECT(1);
    return ans;
}
...
static void list_files(const char *dnp, const char *stem, int *count, SEXP ans, int allfiles, int recursive,
...
SEXP attribute_hidden do_indexsearch(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < npath; i++)
    {
        snprintf(linebuf, 256, "%s%s%s%s%s", translateChar(STRING_ELT(path, i)), CHAR(STRING_ELT(sep, 0)), "help",
                 CHAR(STRING_ELT(sep, 0)), CHAR(STRING_ELT(indexname, 0)));
            ...
            while (filbuf(linebuf, fp))
            {
                if (strncmp(linebuf, topicbuf, ltopicbuf) == 0)
                {
                    ...
                    if (!strcmp(ctype, "html"))
                        snprintf(topicbuf, 256, "%s%s%s%s%s%s", translateChar(STRING_ELT(path, i)),
                                 CHAR(STRING_ELT(sep, 0)), "html", CHAR(STRING_ELT(sep, 0)), p, ".html");
                    else if (!strcmp(ctype, "R-ex"))
                        snprintf(topicbuf, 256, "%s%s%s%s%s%s", translateChar(STRING_ELT(path, i)),
                                 CHAR(STRING_ELT(sep, 0)), "R-ex", CHAR(STRING_ELT(sep, 0)), p, ".R");
                    else if (!strcmp(ctype, "latex"))
                        snprintf(topicbuf, 256, "%s%s%s%s%s%s", translateChar(STRING_ELT(path, i)),
                                 CHAR(STRING_ELT(sep, 0)), "latex", CHAR(STRING_ELT(sep, 0)), p, ".tex");
                    else /* type = "help" */
                        snprintf(topicbuf, 256, "%s%s%s%s%s", translateChar(STRING_ELT(path, i)),
                                 CHAR(STRING_ELT(sep, 0)), ctype, CHAR(STRING_ELT(sep, 0)), p);
...
SEXP attribute_hidden do_fileaccess(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
        INTEGER(ans)[i] = Raccess(R_ExpandFileName(translateChar(STRING_ELT(fn, i))), modemask);
    UNPROTECT(1);
    return ans;
}
#else
...
SEXP attribute_hidden do_pathexpand(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
    {
        SEXP tmp = markKnown(R_ExpandFileName(translateChar(STRING_ELT(fn, i))), STRING_ELT(fn, i));
