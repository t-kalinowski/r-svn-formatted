...
#define SEC_TO_UNIX_EPOCH 11644473600LL
            {
                ...
                HANDLE h;
                int success = 0;
                h = CreateFileW(wfn, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
SEXP attribute_hidden do_readlink(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        res = readlink(R_ExpandFileName(translateChar(STRING_ELT(paths, i))), buf, PATH_MAX);
        if (res >= 0)
...
SEXP attribute_hidden do_eSoftVersion(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (ok && !strcmp(dl_info1.dli_fname, dl_info2.dli_fname))
    {
        ...
        if (dlsym(RTLD_DEFAULT, "do_eSoftVersion") == NULL && dlsym(RTLD_DEFAULT, "dladdr") != NULL)
        {
