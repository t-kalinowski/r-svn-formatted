...
SEXP attribute_hidden do_indexsearch(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < npath; i++)
    {
        snprintf(linebuf, 256, "%s%s%s%s%s", translateChar(STRING_ELT(path, i)), csep, "help", csep, "AnIndex");
            ...
            while (filbuf(linebuf, fp))
            {
                if (strncmp(linebuf, topicbuf, ltopicbuf) == 0)
                {
                    ...
                    while (isspace((int)*p))
                        p++;
                    fclose(fp);
                    snprintf(topicbuf, 256, "%s%s%s%s%s", translateChar(STRING_ELT(path, i)), csep, "help", csep, p);
