...
{
    machar(&R_AccuracyInfo.ibeta, &R_AccuracyInfo.it, &R_AccuracyInfo.irnd, &R_AccuracyInfo.ngrd,
           &R_AccuracyInfo.machep, &R_AccuracyInfo.negep, &R_AccuracyInfo.iexp, &R_AccuracyInfo.minexp,
           &R_AccuracyInfo.maxexp, &R_AccuracyInfo.eps, &R_AccuracyInfo.epsneg, &R_AccuracyInfo.xmin,
           &R_AccuracyInfo.xmax);
...
# define MACH_SIZE 18
    ...
    */
    ...
    SEXP ans = PROTECT(allocVector(VECSXP, MACH_SIZE)), nms = PROTECT(allocVector(STRSXP, MACH_SIZE));
...
*/
    ...
    if (sizeof(LDOUBLE) > sizeof(double))
    {
        ...
        machar_LD(&R_LD_AccuracyInfo.ibeta, &R_LD_AccuracyInfo.it, &R_LD_AccuracyInfo.irnd, &R_LD_AccuracyInfo.ngrd,
                  &R_LD_AccuracyInfo.machep, &R_LD_AccuracyInfo.negep, &R_LD_AccuracyInfo.iexp,
                  &R_LD_AccuracyInfo.minexp, &R_LD_AccuracyInfo.maxexp, &R_LD_AccuracyInfo.eps,
                  &R_LD_AccuracyInfo.epsneg, &R_LD_AccuracyInfo.xmin, &R_LD_AccuracyInfo.xmax);
#define PT1 19
...
static int defaultLocaleACP(const char *ctype)
{
    ...
    /* It is not clear from the Microsoft documentation that GetLocaleInfoEx
       ...
       get clarified. */
    if (!GetLocaleInfoEx(wctype, LOCALE_IDEFAULTANSICODEPAGE, wdefaultCP, sizeof(wdefaultCP)))
        return 0;
...
SEXP attribute_hidden do_fileshow(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    tl = CAR(args);
    args = CDR(args);
    dl = (Rboolean)asLogical(CAR(args));
    args = CDR(args);
    pg = CAR(args);
    n = 0; /* -Wall */
...
SEXP attribute_hidden do_fileappend(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    { /* common case */
        ...
        {
            status = 0;
            if (STRING_ELT(f2, i) == NA_STRING || !(fp2 = RC_fopen(STRING_ELT(f2, i), "rb", TRUE)))
                continue;
            char *buf = (char *)malloc(APPENDBUFSIZE);
            if (!buf)
            {
    ...
    else
    {
        for (int i = 0; i < n; i++)
        {
            if (STRING_ELT(f1, i % n1) == R_NilValue || STRING_ELT(f2, i % n2) == R_NilValue)
                ...
                LOGICAL(ans)[i] = R_AppendFile(STRING_ELT(f1, i % n1), STRING_ELT(f2, i % n2));
        }
    }
done:
    UNPROTECT(1);
...
SEXP attribute_hidden do_filesymlink(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    pCSL = (PCSL)GetProcAddress(GetModuleHandle(TEXT("kernel32.dll")), "CreateSymbolicLinkW");
    if (!pCSL)
...
SEXP attribute_hidden do_filerename(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n1; i++)
    {
        if (STRING_ELT(f1, i) == NA_STRING || STRING_ELT(f2, i) == NA_STRING)
        {
            LOGICAL(ans)[i] = 0;
            continue;
        }
        ...
        if (res)
        {
            warning(_("cannot rename file '%ls' to '%ls', reason '%s'"), from, to, formatError(GetLastError()));
        }
        LOGICAL(ans)[i] = (res == 0);
#else
...
SEXP attribute_hidden do_fileinfo(SEXP call, SEXP op, SEXP args, SEXP rho)
{
...
#ifdef UNIX_EXTRAS
    SEXP uid = R_NilValue, gid = R_NilValue, uname = R_NilValue, grname = R_NilValue; // silence -Wall
#endif
#ifdef Win32
    SEXP exe = R_NilValue;
    struct _stati64 sb;
    ...
    for (int i = 0; i < n; i++)
    {
                ...
                == 0)
        {
            ...
            REAL(mtime)
            [i] = (double)STAT_TIMESPEC(sb, st_mtim).tv_sec + 1e-9 * (double)STAT_TIMESPEC(sb, st_mtim).tv_nsec;
            REAL(ctime)
            [i] = (double)STAT_TIMESPEC(sb, st_ctim).tv_sec + 1e-9 * (double)STAT_TIMESPEC(sb, st_ctim).tv_nsec;
            REAL(atime)
            [i] = (double)STAT_TIMESPEC(sb, st_atim).tv_sec + 1e-9 * (double)STAT_TIMESPEC(sb, st_atim).tv_nsec;
#else
#ifdef Win32
...
#define SEC_TO_UNIX_EPOCH 11644473600LL
            {
                ...
                HANDLE h;
                int success = 0;
                h = CreateFileW(wfn, 0, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
...
static void list_dirs(const char *dnp, const char *nm, Rboolean full, int *count, SEXP *pans, int *countmax,
                      PROTECT_INDEX idx, Rboolean recursive)
{
    DIR *dir;
    struct dirent *de;
    char p[PATH_MAX];
            ...
            if (!res && (sb.st_mode & S_IFDIR) > 0)
            {
                if (strcmp(de->d_name, ".") && strcmp(de->d_name, ".."))
                {
                    ...
                    else
                    {
                        ...
                        SET_STRING_ELT(*pans, (*count)++, mkChar(full ? p : de->d_name));
                    }
                }
            }
        }
...
SEXP attribute_hidden do_fileaccess(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
        if (STRING_ELT(fn, i) != NA_STRING)
        {
            ...
            INTEGER(ans)[i] = -1; /* treat NA as non-existent file */
    UNPROTECT(1);
    return ans;
}
...
{
    HANDLE hd = CreateFileW(name, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING,
                            FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT, 0);
    if (hd == INVALID_HANDLE_VALUE)
    {
        warning("cannot open reparse point '%ls', reason '%s'", name, formatError(GetLastError()));
        return 1;
    }
    REPARSE_GUID_DATA_BUFFER rgdb = {0};
    rgdb.ReparseTag = IO_REPARSE_TAG_MOUNT_POINT;
    DWORD dwBytes;
    BOOL res = DeviceIoControl(hd, FSCTL_DELETE_REPARSE_POINT, &rgdb, REPARSE_GUID_DATA_BUFFER_HEADER_SIZE, NULL, 0,
                               &dwBytes, 0);
    CloseHandle(hd);
    if (res == 0)
        warning("cannot delete reparse point '%ls', reason '%s'", name, formatError(GetLastError()));
...
SEXP attribute_hidden do_setlocale(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    case 9: /* LC_MEASUREMENT */
        warning(_("%s does not exist on Windows"),
                (cat == 7) ? "LC_MESSAGES" : (cat == 8) ? "LC_PAPER" : "LC_MEASUREMENT");
        p = NULL;
        warned = TRUE;
        break;
#else /* not Win32 */
    ...
    if (localeCP && systemCP != localeCP && oldCP != localeCP)
    {
        ...
        warning(_("using locale code page other than %d%s may cause problems"), systemCP,
...
SEXP attribute_hidden do_dircreate(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (show && res && serrno != EEXIST)
        warning(_("cannot create dir '%s', reason '%s'"), dir, strerror(serrno));
        ...
        while ((p = wcschr(p + 1, L'\\')))
        {
            *p = L'\0';
            if (*(p - 1) != L':')
            {
                res = _wmkdir(dir);
                serrno = errno;
                if (res && serrno != EEXIST && !maybeshare)
                    goto end;
            }
            maybeshare = 0;
            *p = L'\\';
...
static void copyFileTime(const wchar_t *from, const wchar_t *to)
{
    ...
    hFrom = CreateFileW(from, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
    ...
    hTo = CreateFileW(to, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
                   ...
                   int dates, int depth)
{
    ...
    { /* a directory */
        ...
        if (res)
        {
            ...
            {
                struct _stati64 dsb;
                if (over && _wstati64(dest, &dsb) == 0 && (dsb.st_mode & S_IFDIR) == 0)
                {

                    warning(_("cannot overwrite non-directory %ls with directory %ls"), dest, this);
                    return 1;
                }
            }
            else
        ...
        if ((dir = _wopendir(this)) != NULL)
        {
            ...
            while ((de = _wreaddir(dir)))
            {
                ...
                nfail += do_copy(from, p, to, over, recursive, perms, dates, depth);
            }
            _wclosedir(dir);
        }
        else
...
#ifdef STAT_TIMESPEC
    ftime = (double)STAT_TIMESPEC(sb, st_mtim).tv_sec + 1e-9 * (double)STAT_TIMESPEC(sb, st_mtim).tv_nsec;
...
static int do_copy(const char *from, const char *name, const char *to, int over, int recursive, int perms, int dates,
                   int depth)
{
    R_CheckUserInterrupt(); // includes stack check
    if (depth > 100)
    ...
    { /* a directory */
        ...
        if (res)
        {
            ...
            {
                struct stat dsb;
                if (over && stat(dest, &dsb) == 0 && (dsb.st_mode & S_IFDIR) == 0)
                {

                    warning(_("cannot overwrite non-directory %s with directory %s"), dest, this);
                    return 1;
                }
            }
            else
...
SEXP attribute_hidden do_Cstack_info(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    INTEGER(ans)[1] = (R_CStackLimit == -1) ? NA_INTEGER : (int)(R_CStackDir * (R_CStackStart - (uintptr_t)&ans));
...
static int winSetFileTime(const char *fn, double ftime)
{
    ...
    if (!SystemTimeToFileTime(&st, &modft))
        return 0;

    hFile = CreateFile(fn, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
SEXP attribute_hidden do_setFileTime(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
    const char *fn;
    double ftime;
    ...
    m = XLENGTH(times);
    if (!m && n)
        error(_("'%s' must be of length at least one"), "time");
...
SEXP attribute_hidden do_mkjunction(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    HANDLE hd = CreateFileW(to, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING,
                            FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT, 0);
    if (hd == INVALID_HANDLE_VALUE)
    {
        warning("cannot open reparse point '%ls', reason '%s'", to, formatError(GetLastError()));
    ...
    DWORD dwBytes;
    const BOOL bOK = DeviceIoControl(hd, FSCTL_SET_REPARSE_POINT, &rdb, 8 /* header */ + rdb.ReparseDataLength, NULL, 0,
                                     &dwBytes, 0);
    CloseHandle(hd);
    if (!bOK)
        warning("cannot set reparse point '%ls', reason '%s'", to, formatError(GetLastError()));
...
SEXP attribute_hidden do_eSoftVersion(SEXP call, SEXP op, SEXP args, SEXP rho)
{
...
#if defined(HAVE_DLADDR) && defined(HAVE_REALPATH) && defined(HAVE_DLSYM) && defined(HAVE_DECL_RTLD_DEFAULT) &&        \
    ...
    */
    ...
    if (!dladdr((void *)do_eSoftVersion, &dl_info1))
        ok = FALSE;
    if (!dladdr((void *)dladdr, &dl_info2))
        ok = FALSE;
