...
SEXP do_fileinfo(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (STRING(fn)[i] != R_NilValue && stat(R_ExpandFileName(CHAR(STRING(fn)[i])), &sb) == 0)
        {
            ...
            if (stpwd)
                STRING(uname)[i] = mkChar(stpwd->pw_name);
            else
                STRING(uname)[i] = NA_STRING;
            stgrp = getgrgid(sb.st_gid);
            if (stgrp)
                STRING(grname)[i] = mkChar(stgrp->gr_name);
            else
...
SEXP do_fileaccess(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
        INTEGER(ans)[i] = access(R_ExpandFileName(CHAR(STRING(fn)[i])), modemask);
    UNPROTECT(1);
    return ans;
}
#else
