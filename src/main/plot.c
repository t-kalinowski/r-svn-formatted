        ...
        for (i = 0; i < n; i++)
        {
            ...
            if (dolabels)
            {
                ...
                if (x > low && x < high)
                {
                    if (isExpression(lab))
                    {
                        GMMathText(VECTOR(lab)[ind[i]], side, axis_lab, 0, x, dd->gp.las, dd);
                    }
                    else
                    {
                        ...
                        {
                            GMtext(CHAR(STRING(lab)[ind[i]]), side, axis_lab, 0, x, dd->gp.las, dd);
                            tlast = temp + 0.5 * labw;
                        }
                    }
                }
        ...
        for (i = 0; i < n; i++)
        {
            ...
            if (dolabels)
            {
                ...
                if (y > low && y < high)
                {
                    if (isExpression(lab))
                    {
                        GMMathText(VECTOR(lab)[ind[i]], side, axis_lab, 0, y, dd->gp.las, dd);
                    }
                    else
                    {
                        ...
                        {
                            GMtext(CHAR(STRING(lab)[ind[i]]), side, axis_lab, 0, y, dd->gp.las, dd);
                            tlast = temp + 0.5 * labw;
                        }
                    }
                }
...
SEXP do_plot_xy(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    {
        if (isString(CAR(args)) && LENGTH(CAR(args)) == 1 && LENGTH(pch = STRING(CAR(args))[0]) >= 1)
        {
            if (LENGTH(pch) > 1)
...
SEXP do_text(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (R_FINITE(xx) && R_FINITE(yy))
        {
            ...
            if (vectorFonts)
                GVText(xx, yy, INCHES, CHAR(STRING(txt)[i % ntxt]), INTEGER(vfont)[0], INTEGER(vfont)[1], adjx, adjy,
                       dd->gp.srt, dd);
            else if (isExpression(txt))
...
SEXP do_title(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (Main != R_NilValue)
    {
        ...
        {
            GMathText(hpos, vpos, where, VECTOR(Main)[0], adj, adjy, 0.0, dd);
        }
        else
        {
            n = length(Main);
            offset = 0.5 * (n - 1) + vpos;
            for (i = 0; i < n; i++)
                GText(hpos, offset - i, where, CHAR(STRING(Main)[i]), adj, adjy, 0.0, dd);
        }
    }
    if (sub != R_NilValue)
    {
        ...
        if (isExpression(sub))
            GMMathText(VECTOR(sub)[0], 1, vpos, where, hpos, 0, dd);
        else
        {
            n = length(sub);
            for (i = 0; i < n; i++)
                GMtext(CHAR(STRING(sub)[i]), 1, vpos, where, hpos, 0, dd);
        }
    }
    if (xlab != R_NilValue)
    {
        ...
        if (isExpression(xlab))
            GMMathText(VECTOR(xlab)[0], 1, vpos, where, hpos, 0, dd);
        else
        {
            n = length(xlab);
            for (i = 0; i < n; i++)
                GMtext(CHAR(STRING(xlab)[i]), 1, vpos + i, where, hpos, 0, dd);
        }
    }
    if (ylab != R_NilValue)
    {
        ...
        if (isExpression(ylab))
            GMMathText(VECTOR(ylab)[0], 2, vpos, where, hpos, 0, dd);
        else
        {
            n = length(ylab);
            for (i = 0; i < n; i++)
                GMtext(CHAR(STRING(ylab)[i]), 2, vpos - i, where, hpos, 0, dd);
        }
    }
    GMode(0, dd);
    GRestorePars(dd);
    ...
    else
    {
        ...
        while (k < npts)
        {
            ...
            else
            {
                ...
                if (plot)
                    drawLabel(xi, yi, INTEGER(pos)[imin], offset, CHAR(STRING(l)[imin]), dd);
            }
        }
        GMode(0, dd);
