...
SEXP do_polygon(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < nx; i++)
    {
        ...
        else if ((FINITE(xold) && FINITE(yold)) && !(FINITE(xx) && FINITE(yy)))
        {
            if (i - start > 1)
            {
                ...
                else
                {
                    char *vmax;
                    double *xc, *yc;
                    int npts = GClipPolygon(x + start, y + start, i - start, USER, 0, xc, yc, dd);
                    if (npts > 1)
                    {
                        vmax = vmaxget();
                        xc = (double *)R_alloc(npts, sizeof(double));
                        yc = (double *)R_alloc(npts, sizeof(double));
                        npts = GClipPolygon(x + start, y + start, i - start, USER, 1, xc, yc, dd);
                        GPolygon(npts, xc, yc, USER, INTEGER(col)[num % ncol], INTEGER(border)[0], dd);
                        vmaxset(vmax);
                    }
                }
                num++;
        ...
        else if ((FINITE(xold) && FINITE(yold)) && (i == nx - 1))
        { /* last */
            if (dd->gp.xpd)
            {
            ...
            else
            {
                char *vmax;
                double *xc, *yc;
                int npts = GClipPolygon(x + start, y + start, nx - start, USER, 0, xc, yc, dd);
                if (npts > 1)
                {
                    vmax = vmaxget();
                    xc = (double *)R_alloc(npts, sizeof(double));
                    yc = (double *)R_alloc(npts, sizeof(double));
                    npts = GClipPolygon(x + start, y + start, nx - start, USER, 1, xc, yc, dd);
                    GPolygon(npts, xc, yc, USER, INTEGER(col)[num % ncol], INTEGER(border)[0], dd);
                    vmaxset(vmax);
                }
            }
