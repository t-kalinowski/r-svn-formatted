...
SEXP CreateAtVector(double *axp, double *usr, int nint, Rboolean logflag)
{
    ...
    { /* ------ log axis ----- */
        ...
        umax = usr[1];
#ifdef DEBUG_axis
        REprintf("CreateAtVector(axp=(%g,%g,%g), usr=(%g,%g), _log_):", axp[0], axp[1], axp[2], usr[0], usr[1]);
#endif
        if (umin > umax)
        {
        ...
        case 1: /* large range:	1	 * 10^k */
        {
...
#ifdef DEBUG_axis
            REprintf(" .. case 1: umin,umax= %g,%g;\n  (nint=%d, ne=%d); ", umin, umax, nint, ne);
            if (ne < 1)
            {
                REprintf("ne = %d <= 0 !!\n\t axp[0:1]=(%g,%g) ==> i = %d, nint = %d; ", ne, axp[0], axp[1], i, nint);
            }
#endif
            double l10_max = log10(umax), d0 = l10_max - log10(dn);
#ifdef DEBUG_axis
            ...
            if (k > 1)
            {                      // i.e. ne > 308: 10^ne overflows; must split the multiplication
