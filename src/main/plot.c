...
attribute_hidden SEXP FixupPch(SEXP pch, int dflt)
{
    int i, n;
    ...
    else if (isString(pch))
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            else
            {
                ...
                {
                    wchar_t wc;
                    if (mbrtowc(&wc, translateChar(STRING_ELT(pch, i)), MB_CUR_MAX, NULL) > 0)
                        INTEGER(ans)[i] = wc;
                    else
...
attribute_hidden SEXP FixupFont(SEXP font, int dflt)
{
    int i, k, n;
    SEXP ans = R_NilValue; /* -Wall*/
    n = length(font);
...
attribute_hidden SEXP FixupCol(SEXP col, unsigned int dflt)
{
    int i, n;
    SEXP ans;
    n = length(col);
...
attribute_hidden SEXP FixupCex(SEXP cex, double dflt)
{
    SEXP ans;
    int i, n;
    n = length(cex);
 ...
 */
static void GetTextArg(SEXP call, SEXP spec, SEXP *ptxt, int *pcol, double *pcex, int *pfont)
{
...
SEXP attribute_hidden do_plot_window(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if ((Rf_dpptr(dd)->xlog && (xmin < 0 || xmax < 0)) || (Rf_dpptr(dd)->ylog && (ymin < 0 || ymax < 0)))
...
SEXP attribute_hidden do_axis(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    npadj = length(padj);
    if (npadj <= 0)
        errorcall(call, _("zero length 'padj' specified"));
    ...
    if (dolabels)
    {
        ...
        if (length(at) != length(lab))
            errorcall(call, _("'at' and 'labels' lengths differ, %d != %d"), length(at), length(lab));
    }
    PROTECT(lab);
    ...
    case 3:
        ...
        if (side == 1)
        {
            axis_lab = -axis_base + GConvertYUnits(Rf_gpptr(dd)->mgp[1] - lineoff, LINES, NFC, dd) +
        ...
        for (i = istart; i != iend; i += incr)
        {
            ...
            if (dolabels)
            {
                ...
                if (x > low && x < high)
                {
                    if (isExpression(lab))
                    {
                        GMMathText(VECTOR_ELT(lab, ind[i]), side, axis_lab, 0, x, Rf_gpptr(dd)->las, padjval, dd);
                    }
                    else
                    {
                        ...
                        if (label != NA_STRING)
                        {
                            ...
                            if (Rf_gpptr(dd)->las == 2 || Rf_gpptr(dd)->las == 3 || tnew - tlast >= gap)
                            {
                                GMtext(ss, side, axis_lab, 0, x, Rf_gpptr(dd)->las, padjval, dd);
                                tlast = temp + 0.5 * labw;
                            }
                        }
                    }
    ...
    case 4:
        ...
        if (side == 2)
        {
            axis_lab = -axis_base + GConvertXUnits(Rf_gpptr(dd)->mgp[1] - lineoff, LINES, NFC, dd) +
        ...
        for (i = istart; i != iend; i += incr)
        {
            ...
            if (dolabels)
            {
                ...
                if (y > low && y < high)
                {
                    if (isExpression(lab))
                    {
                        GMMathText(VECTOR_ELT(lab, ind[i]), side, axis_lab, 0, y, Rf_gpptr(dd)->las, padjval, dd);
                    }
                    else
                    {
                        label = STRING_ELT(lab, ind[i]);
                        if (label != NA_STRING)
                        {
                            ...
                            tnew = temp - 0.5 * labw;
                            /* Check room for perpendicular labels. */
                            if (Rf_gpptr(dd)->las == 1 || Rf_gpptr(dd)->las == 2 || tnew - tlast >= gap)
                            {
                                GMtext(ss, side, axis_lab, 0, y, Rf_gpptr(dd)->las, padjval, dd);
                                tlast = temp + 0.5 * labw;
                            }
                        }
                    }
...
SEXP attribute_hidden do_plot_xy(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    int i, n, npch, ncex, ncol, nbg, /*nlty,*/ nlwd, type = 0, start = 0, thispch, thiscol;
    char *vmax = NULL /* -Wall */;
...
#define PLOT_XY_DEALING(subname)                                                                                       \
    ...
    else
    {
        ...
        {
            if (LENGTH(pch) > 1)
                warningcall(call, _("plot type '%s' will be truncated to first character"), CHAR(pch));
            type = CHAR(pch)[0];
        }
        else
    ...
    PROTECT(bg = FixupCol(CAR(args), R_TRANWHITE));
    args = CDR(args);
    nbg = LENGTH(bg);
    ...
    case 'h': /* h[istogram] (bar plot) */
        ...
        for (i = 0; i < n; i++)
        {
            ...
            yy = y[i];
            GConvert(&xx, &yy, USER, DEVICE, dd);
            if (R_FINITE(xx) && R_FINITE(yy) && !R_TRANSPARENT(thiscol = INTEGER(col)[i % ncol]))
            {
    ...
    for (i = 0; i < imax2(n, ntxt); i++)
    {
        ...
        if (R_FINITE(xx) && R_FINITE(yy))
        {
            ...
            if (vectorFonts)
            {
                ...
                if (string != NA_STRING)
                    GVText(xx, yy, INCHES, translateChar(string), INTEGER(vfont)[0], INTEGER(vfont)[1], adjx, adjy,
                           Rf_gpptr(dd)->srt, dd);
            }
            else if (isExpression(txt))
            {
            ...
            else
            {
                string = STRING_ELT(txt, i % ntxt);
                if (string != NA_STRING)
                    GText(xx, yy, INCHES, translateChar(string), adjx, adjy, Rf_gpptr(dd)->srt, dd);
            }
        }
    }
    GMode(0, dd);
...
SEXP attribute_hidden do_mtext(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    nside = length(side);
    if (nside <= 0)
        errorcall(call, _("zero length 'side' specified"));
    if (n < nside)
        n = nside;
    args = CDR(args);
    ...
    nline = length(line);
    if (nline <= 0)
        errorcall(call, _("zero length 'line' specified"));
    if (n < nline)
        n = nline;
    args = CDR(args);
    ...
    nouter = length(outer);
    if (nouter <= 0)
        errorcall(call, _("zero length 'outer' specified"));
    if (n < nouter)
        n = nouter;
    args = CDR(args);
    ...
    nat = length(at);
    if (nat <= 0)
        errorcall(call, _("zero length 'at' specified"));
    if (n < nat)
        n = nat;
    args = CDR(args);
    ...
    nadj = length(adj);
    if (nadj <= 0)
        errorcall(call, _("zero length 'adj' specified"));
    if (n < nadj)
        n = nadj;
    args = CDR(args);
    ...
    ncex = length(cex);
    if (ncex <= 0)
        errorcall(call, _("zero length 'cex' specified"));
    if (n < ncex)
        n = ncex;
    args = CDR(args);
    ...
    ncol = length(col);
    if (ncol <= 0)
        errorcall(call, _("zero length 'col' specified"));
    if (n < ncol)
        n = ncol;
    args = CDR(args);
    ...
    nfont = length(font);
    if (nfont <= 0)
        errorcall(call, _("zero length 'font' specified"));
    if (n < nfont)
        n = nfont;
    args = CDR(args);
    ...
    for (i = 0; i < n; i++)
    {
        ...
        else
        {
            ...
            if (string != NA_STRING)
                GMtext(translateChar(string), sideval, lineval, outerval, atval, Rf_gpptr(dd)->las, padjval, dd);
        }
...
SEXP attribute_hidden do_title(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (Main != R_NilValue)
    {
        ...
        else
        {
            ...
            for (i = 0; i < n; i++)
            {
                ...
                if (string != NA_STRING)
                    GText(hpos, offset - i, where, translateChar(string), adj, adjy, 0.0, dd);
            }
        }
        UNPROTECT(1);
    }
    if (sub != R_NilValue)
    {
        ...
        if (isExpression(sub))
            GMMathText(VECTOR_ELT(sub, 0), 1, vpos, where, hpos, 0, 0.0, dd);
        else
        {
            n = length(sub);
            for (i = 0; i < n; i++)
            {
                ...
                if (string != NA_STRING)
                    GMtext(translateChar(string), 1, vpos, where, hpos, 0, 0.0, dd);
            }
        }
        UNPROTECT(1);
    }
    if (xlab != R_NilValue)
    {
        ...
        if (isExpression(xlab))
            GMMathText(VECTOR_ELT(xlab, 0), 1, vpos, where, hpos, 0, 0.0, dd);
        else
        {
            n = length(xlab);
            for (i = 0; i < n; i++)
            {
                ...
                if (string != NA_STRING)
                    GMtext(translateChar(string), 1, vpos + i, where, hpos, 0, 0.0, dd);
            }
        }
        UNPROTECT(1);
    }
    if (ylab != R_NilValue)
    {
        ...
        if (isExpression(ylab))
            GMMathText(VECTOR_ELT(ylab, 0), 2, vpos, where, hpos, 0, 0.0, dd);
        else
        {
            n = length(ylab);
            for (i = 0; i < n; i++)
            {
                ...
                if (string != NA_STRING)
                    GMtext(translateChar(string), 2, vpos - i, where, hpos, 0, 0.0, dd);
            }
        }
        UNPROTECT(1);
    }
...
static void drawLabel(double xi, double yi, int pos, double offset, char *l, DevDesc *dd)
{
    ...
    case 4:
        xi = xi + offset;
        GText(xi, yi, INCHES, l, 0.0, Rf_gpptr(dd)->yCharOffset, 0.0, dd);
        break;
    case 2:
        xi = xi - offset;
        GText(xi, yi, INCHES, l, 1.0, Rf_gpptr(dd)->yCharOffset, 0.0, dd);
        break;
    case 3:
        yi = yi + offset;
    ...
    case 1:
        yi = yi - offset;
        GText(xi, yi, INCHES, l, 0.5, 1 - (0.5 - Rf_gpptr(dd)->yCharOffset), 0.0, dd);
        break;
    case 0:
...
#define DO_STR_DIM(KIND)                                                                                               \
    {                                                                                                                  \
        ...
        for (i = 0; i < n; i++)                                                                                        \
            if (isExpression(str))                                                                                     \
                REAL(ans)[i] = GExpression##KIND(VECTOR_ELT(str, i), GMapUnits(units), dd);                            \
            else                                                                                                       \
            {                                                                                                          \
                ch = STRING_ELT(str, i);                                                                               \
                REAL(ans)[i] = (ch == NA_STRING) ? 0.0 : GStr##KIND(CHAR(ch), GMapUnits(units), dd);                   \
            }                                                                                                          \
    ...
    *y = dnd_hght[node - 1];
    ...
    else
    {
        ...
        if (STRING_ELT(dnd_llabels, -k - 1) != NA_STRING)
            GText(xl, yl - dnd_offset, USER, translateChar(STRING_ELT(dnd_llabels, -k - 1)), 1.0, 0.3, 90.0, dd);
    }
    /* right part */
    k = dnd_rptr[node - 1];
    if (k > 0)
    ...
    else
    {
        ...
        if (STRING_ELT(dnd_llabels, -k - 1) != NA_STRING)
            GText(xr, yr - dnd_offset, USER, translateChar(STRING_ELT(dnd_llabels, -k - 1)), 1.0, 0.3, 90.0, dd);
    }
    xx[0] = xl;
    yy[0] = yl;
    xx[1] = xl;
...
SEXP attribute_hidden do_dendwindow(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        ll[i] = (str == NA_STRING) ? 0.0 : GStrWidth(translateChar(str), INCHES, dd) + dnd_offset;
    }
...
SEXP attribute_hidden do_symbols(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    PROTECT(fg = FixupCol(CAR(args), R_TRANWHITE));
    args = CDR(args);
    nfg = LENGTH(fg);
    ...
    case 5: /* thermometers */
        ...
        if (pmax < pmin)
            errorcall(call, _("invalid thermometers[,%s]"), (nc == 4) ? "3:4" : "3");
        if (pmin < 0. || pmax > 1.) /* S-PLUS has an error here */
            warningcall(call, _("thermometers[,%s] not in [0,1] -- may look funny"), (nc == 4) ? "3:4" : "3");
        if (!SymbolRange(REAL(p), 2 * nr, &pmax, &pmin))
    ...
    case 6: /* boxplots (wid, hei, loWhsk, upWhsk, medProp) */
        ...
        if (pmin < 0. || pmax > 1.) /* S-PLUS has an error here */
            warningcall(call, _("boxplots[,5] outside [0,1] -- may look funny"));
...
SEXP attribute_hidden do_xspline(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    args = CDR(args);
    nx = LENGTH(sx);
    ss = SETCAR(args, coerceVector(CAR(args), REALSXP));
    args = CDR(args);
    open = asLogical(CAR(args));
    args = CDR(args);
    repEnds = asLogical(CAR(args));
    args = CDR(args);
    draw = asLogical(CAR(args));
    args = CDR(args);
