...
SEXP do_plot_xy(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (type == 'l' || type == 'o')
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            if ((FINITE(xx) && FINITE(yy)) && !(FINITE(xold) && FINITE(yold)))
                start = i;
            else if ((FINITE(xold) && FINITE(yold)) && !(FINITE(xx) && FINITE(yy)))
            {
                if (i - start > 1)
                    GPolyline(i - start, x + start, y + start, USER, dd);
            }
            else if ((FINITE(xold) && FINITE(yold)) && (i == n - 1))
...
SEXP do_polygon(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (INTEGER(lty)[0] == NA_INTEGER)
        ...
        dd->gp.lty = INTEGER(lty)[0];
    GPolygon(nx, REAL(sx), REAL(sy), USER, INTEGER(col)[0], INTEGER(border)[0], dd);
...
SEXP do_title(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (main != R_NilValue)
    {
        ...
        if (isExpression(main))
            GMathText(xNPCtoUsr(adj, dd), 0.5 * dd->gp.mar[2], MAR3, VECTOR(main)[0], 0.5, 0.5, 0.0, dd);
        else
            GText(xNPCtoUsr(adj, dd), 0.5 * dd->gp.mar[2], MAR3, CHAR(STRING(main)[0]), adj, 0.5, 0.0, dd);
    }
    if (sub != R_NilValue)
    {
        ...
        if (isExpression(sub))
            ...
            GMtext(CHAR(STRING(sub)[0]), 1, dd->gp.mgp[0] + 1.0, 0, xNPCtoUsr(adj, dd), 0, dd);
    }
    if (xlab != R_NilValue)
    {
        ...
        if (isExpression(xlab))
            ...
            GMtext(CHAR(STRING(xlab)[0]), 1, dd->gp.mgp[0], 0, xNPCtoUsr(adj, dd), 0, dd);
    }
    if (ylab != R_NilValue)
    {
        dd->gp.cex = dd->gp.cexbase * dd->gp.cexlab;
        ...
        if (isExpression(ylab))
            ...
            GMtext(CHAR(STRING(ylab)[0]), 2, dd->gp.mgp[0], 0, yNPCtoUsr(adj, dd), 0, dd);
    }
    GMode(dd, 0);
    GRestorePars(dd);
    /* NOTE: only record operation if no "error"  */
