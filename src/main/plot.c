        ...
        for (i = istart; i != iend; i += incr)
        {
            ...
            if (dolabels)
            {
                ...
                if (x > low && x < high)
                {
                    if (isExpression(lab))
                    {
                        GMMathText(VECTOR_ELT(lab, ind[i]), side, axis_lab, 0, x, Rf_gpptr(dd)->las, dd);
                    }
                    else
                    {
                        ...
                        {
                            GMtext(CHAR(STRING_ELT(lab, ind[i])), side, axis_lab, 0, x, Rf_gpptr(dd)->las, dd);
                            tlast = temp + 0.5 * labw;
                        }
                    }
                }
        ...
        for (i = istart; i != iend; i += incr)
        {
            ...
            if (dolabels)
            {
                ...
                if (y > low && y < high)
                {
                    if (isExpression(lab))
                    {
                        GMMathText(VECTOR_ELT(lab, ind[i]), side, axis_lab, 0, y, Rf_gpptr(dd)->las, dd);
                    }
                    else
                    {
                        ...
                        {
                            GMtext(CHAR(STRING_ELT(lab, ind[i])), side, axis_lab, 0, y, Rf_gpptr(dd)->las, dd);
                            tlast = temp + 0.5 * labw;
                        }
                    }
                }
        ...
        if (R_FINITE(xx) && R_FINITE(yy))
        {
            ...
            if (vectorFonts)
                GVText(xx, yy, INCHES, CHAR(STRING_ELT(txt, i % ntxt)), INTEGER(vfont)[0], INTEGER(vfont)[1], adjx,
                       adjy, Rf_gpptr(dd)->srt, dd);
            else if (isExpression(txt))
                GMathText(xx, yy, INCHES, VECTOR_ELT(txt, i % ntxt), adjx, adjy, Rf_gpptr(dd)->srt, dd);
            else
                GText(xx, yy, INCHES, CHAR(STRING_ELT(txt, i % ntxt)), adjx, adjy, Rf_gpptr(dd)->srt, dd);
        }
    }
    GMode(0, dd);
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (vectorFonts)
        {
            ...
            GMVText(CHAR(STRING_ELT(text, i % ntext)), INTEGER(vfont)[0], INTEGER(vfont)[1], sideval, lineval, outerval,
                    atval, Rf_gpptr(dd)->las, dd);
#else
            warningcall(call, "Hershey fonts not yet implemented for mtext()");
            GMtext(CHAR(STRING_ELT(text, i % ntext)), sideval, lineval, outerval, atval, Rf_gpptr(dd)->las, dd);
#endif
        }
        else if (isExpression(text))
            GMMathText(VECTOR_ELT(text, i % ntext), sideval, lineval, outerval, atval, Rf_gpptr(dd)->las, dd);
        else
            GMtext(CHAR(STRING_ELT(text, i % ntext)), sideval, lineval, outerval, atval, Rf_gpptr(dd)->las, dd);
