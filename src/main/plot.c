...
SEXP do_plot_window(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if ((dpptr(dd)->xlog && (xmin < 0 || xmax < 0)) || (dpptr(dd)->ylog && (ymin < 0 || ymax < 0)))
...
SEXP CreateAtVector(double *axp, double *usr, int nint, Rboolean logflag)
{
    ...
    { /* ------ log axis ----- */
        ...
        case 3: /* small range:	 1,2,5,10 * 10^k */
            ...
            break;
        default:
            error("log - axis(), 'at' creation: ILLEGAL {xy}axp[3] = %g", axp[2]);
        }
    }
    return at;
}
...
SEXP do_axis(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    case 1:
    case 3:
        axp[0] = dpptr(dd)->xaxp[0];
        axp[1] = dpptr(dd)->xaxp[1];
        axp[2] = dpptr(dd)->xaxp[2];
    ...
    case 2:
    case 4:
        axp[0] = dpptr(dd)->yaxp[0];
        axp[1] = dpptr(dd)->yaxp[1];
        axp[2] = dpptr(dd)->yaxp[2];
    ...
    case 3:
        ...
        if (side == 1)
        {
            if (R_FINITE(pos))
                ...
                axis_base = GConvertY(0.0, outer, NFC, dd) - GConvertYUnits(line, LINES, NFC, dd);
            if (R_FINITE(gpptr(dd)->tck))
                axis_tick = axis_base + GConvertYUnits(gpptr(dd)->tck, NPC, NFC, dd);
            else
                axis_tick = axis_base + GConvertYUnits(gpptr(dd)->tcl, LINES, NFC, dd);
        }
        else
        {
            if (R_FINITE(pos))
                ...
                axis_base = GConvertY(1.0, outer, NFC, dd) + GConvertYUnits(line, LINES, NFC, dd);
            if (R_FINITE(gpptr(dd)->tck))
                axis_tick = axis_base - GConvertYUnits(gpptr(dd)->tck, NPC, NFC, dd);
            else
                axis_tick = axis_base - GConvertYUnits(gpptr(dd)->tcl, LINES, NFC, dd);
        ...
        if (side == 1)
        {
            axis_lab = -axis_base + GConvertYUnits(gpptr(dd)->mgp[1], LINES, NFC, dd) + GConvertY(0.0, NPC, NFC, dd);
        }
        else
        {
            axis_lab = axis_base + GConvertYUnits(gpptr(dd)->mgp[1], LINES, NFC, dd) - GConvertY(1.0, NPC, NFC, dd);
        }
        ...
        for (i = istart; i != iend; i += incr)
        {
            ...
            if (dolabels)
            {
                ...
                if (x > low && x < high)
                {
                    if (isExpression(lab))
                    {
                        GMMathText(VECTOR_ELT(lab, ind[i]), side, axis_lab, 0, x, gpptr(dd)->las, dd);
                    }
                    else
                    {
                        ...
                        {
                            GMtext(CHAR(STRING_ELT(lab, ind[i])), side, axis_lab, 0, x, gpptr(dd)->las, dd);
                            tlast = temp + 0.5 * labw;
                        }
                    }
                }
    ...
    case 4:
        ...
        if (side == 2)
        {
            if (R_FINITE(pos))
                ...
                axis_base = GConvertX(0.0, outer, NFC, dd) - GConvertXUnits(line, LINES, NFC, dd);
            if (R_FINITE(gpptr(dd)->tck))
                axis_tick = axis_base + GConvertXUnits(gpptr(dd)->tck, NPC, NFC, dd);
            else
                axis_tick = axis_base + GConvertXUnits(gpptr(dd)->tcl, LINES, NFC, dd);
        }
        else
        {
            if (R_FINITE(pos))
                ...
                axis_base = GConvertX(1.0, outer, NFC, dd) + GConvertXUnits(line, LINES, NFC, dd);
            if (R_FINITE(gpptr(dd)->tck))
                axis_tick = axis_base - GConvertXUnits(gpptr(dd)->tck, NPC, NFC, dd);
            else
                axis_tick = axis_base - GConvertXUnits(gpptr(dd)->tcl, LINES, NFC, dd);
        ...
        if (side == 2)
        {
            axis_lab = -axis_base + GConvertXUnits(gpptr(dd)->mgp[1], LINES, NFC, dd) + GConvertX(0.0, NPC, NFC, dd);
        }
        else
        {
            axis_lab = axis_base + GConvertXUnits(gpptr(dd)->mgp[1], LINES, NFC, dd) - GConvertX(1.0, NPC, NFC, dd);
        }
        ...
        for (i = istart; i != iend; i += incr)
        {
            ...
            if (dolabels)
            {
                ...
                if (y > low && y < high)
                {
                    if (isExpression(lab))
                    {
                        GMMathText(VECTOR_ELT(lab, ind[i]), side, axis_lab, 0, y, gpptr(dd)->las, dd);
                    }
                    else
                    {
                        ...
                        {
                            GMtext(CHAR(STRING_ELT(lab, ind[i])), side, axis_lab, 0, y, gpptr(dd)->las, dd);
                            tlast = temp + 0.5 * labw;
                        }
                    }
                }
...
#define PLOT_XY_DEALING(subname)                                                                                       \
    ...
    {
        if (isString(CAR(args)) && LENGTH(CAR(args)) == 1 && LENGTH(pch = STRING_ELT(CAR(args), 0)) >= 1)
        {
            if (LENGTH(pch) > 1)
                warningcall(call, "plot type '%s' truncated to first character", CHAR(pch));
            type = CHAR(pch)[0];
        }
        else
    ...
    PROTECT(col = FixupCol(CAR(args), 0));
    args = CDR(args);
    ncol = LENGTH(col);
    ...
    case 'h': /* h[istogram] (bar plot) */
        ...
        for (i = 0; i < n; i++)
        {
            ...
            yy = y[i];
            GConvert(&xx, &yy, USER, DEVICE, dd);
            if (R_FINITE(xx) && R_FINITE(yy) && (thiscol = INTEGER(col)[i % ncol]) != NA_INTEGER)
            {
...
SEXP do_polygon(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < nx; i++)
    {
        ...
        else if ((R_FINITE(xold) && R_FINITE(yold)) && !(R_FINITE(xx) && R_FINITE(yy)))
        {
            if (i - start > 1)
            {
                drawPolygon(i - start, x + start, y + start, INTEGER(lty)[num % nlty], INTEGER(col)[num % ncol],
        ...
        { /* last */
            drawPolygon(nx - start, x + start, y + start, INTEGER(lty)[num % nlty], INTEGER(col)[num % ncol],
                        INTEGER(border)[num % nborder], dd);
            num++;
        }
        xold = xx;
...
SEXP do_text(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    gpptr(dd)->xpd = (xpd == NA_INTEGER) ? 2 : xpd;
        ...
        if (R_FINITE(xx) && R_FINITE(yy))
        {
            ...
            if (npos > 0)
            {
                ...
                case 1:
                    yy = yy - offset;
                    adjx = 0.5;
                    adjy = 1 - (0.5 - gpptr(dd)->yCharOffset);
                    break;
                case 2:
                    xx = xx - offset;
                    adjx = 1;
                    adjy = gpptr(dd)->yCharOffset;
                    break;
                case 3:
                    yy = yy + offset;
                    adjx = 0.5;
            ...
            if (vectorFonts)
                GVText(xx, yy, INCHES, CHAR(STRING_ELT(txt, i % ntxt)), INTEGER(vfont)[0], INTEGER(vfont)[1], adjx,
                       adjy, gpptr(dd)->srt, dd);
            else if (isExpression(txt))
                GMathText(xx, yy, INCHES, VECTOR_ELT(txt, i % ntxt), adjx, adjy, gpptr(dd)->srt, dd);
            else
                GText(xx, yy, INCHES, CHAR(STRING_ELT(txt, i % ntxt)), adjx, adjy, gpptr(dd)->srt, dd);
        }
    }
    GMode(0, dd);
...
SEXP do_mtext(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        gpptr(dd)->col = (colval == NA_INTEGER) ? colsave : colval;
        gpptr(dd)->adj = ComputeAdjValue(adjval, sideval, gpptr(dd)->las);
        atval = ComputeAtValue(atval, gpptr(dd)->adj, sideval, gpptr(dd)->las, outerval, dd);

        if (vectorFonts)
        {
#ifdef GMV_implemented
            GMVText(CHAR(STRING_ELT(text, i % ntext)), INTEGER(vfont)[0], INTEGER(vfont)[1], sideval, lineval, outerval,
                    atval, gpptr(dd)->las, dd);
#else
            warningcall(call, "Hershey fonts not yet implemented for mtext()");
            GMtext(CHAR(STRING_ELT(text, i % ntext)), sideval, lineval, outerval, atval, gpptr(dd)->las, dd);
#endif
        }
        else if (isExpression(text))
            GMMathText(VECTOR_ELT(text, i % ntext), sideval, lineval, outerval, atval, gpptr(dd)->las, dd);
        else
            GMtext(CHAR(STRING_ELT(text, i % ntext)), sideval, lineval, outerval, atval, gpptr(dd)->las, dd);
...
SEXP do_title(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (Main != R_NilValue)
    {
        ...
        else
        {
            ...
            offset = 0.5 * (n - 1) + vpos;
            for (i = 0; i < n; i++)
                GText(hpos, offset - i, where, CHAR(STRING_ELT(Main, i)), adj, adjy, 0.0, dd);
        }
    }
    if (sub != R_NilValue)
    {
        ...
        if (isExpression(sub))
            GMMathText(VECTOR_ELT(sub, 0), 1, vpos, where, hpos, 0, dd);
        else
        {
            n = length(sub);
            for (i = 0; i < n; i++)
                GMtext(CHAR(STRING_ELT(sub, i)), 1, vpos, where, hpos, 0, dd);
        }
    }
    if (xlab != R_NilValue)
    {
        ...
        if (isExpression(xlab))
            GMMathText(VECTOR_ELT(xlab, 0), 1, vpos, where, hpos, 0, dd);
        else
        {
            n = length(xlab);
            for (i = 0; i < n; i++)
                GMtext(CHAR(STRING_ELT(xlab, i)), 1, vpos + i, where, hpos, 0, dd);
        }
    }
    if (ylab != R_NilValue)
    {
        ...
        if (isExpression(ylab))
            GMMathText(VECTOR_ELT(ylab, 0), 2, vpos, where, hpos, 0, dd);
        else
        {
            n = length(ylab);
            for (i = 0; i < n; i++)
                GMtext(CHAR(STRING_ELT(ylab, i)), 2, vpos - i, where, hpos, 0, dd);
        }
    }
    GMode(0, dd);
    GRestorePars(dd);
...
SEXP do_box(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    */
    ...
    col = gpptr(dd)->col;
    gpptr(dd)->col = NA_INTEGER;
    fg = gpptr(dd)->col;
    gpptr(dd)->fg = NA_INTEGER;
    RecordGraphicsCall(call);
    ProcessInlinePars(args, dd);
    if (gpptr(dd)->col == NA_INTEGER)
    {
        if (gpptr(dd)->fg == NA_INTEGER)
            gpptr(dd)->col = col;
        else
...
static void drawLabel(double xi, double yi, int pos, double offset, char *l, DevDesc *dd)
{
    switch (pos)
    {
    case 4:
        xi = xi + offset;
        GText(xi, yi, INCHES, l, 0.0, gpptr(dd)->yCharOffset, 0.0, dd);
        break;
    case 2:
        xi = xi - offset;
        GText(xi, yi, INCHES, l, 1.0, gpptr(dd)->yCharOffset, 0.0, dd);
        break;
    case 3:
        yi = yi + offset;
        GText(xi, yi, INCHES, l, 0.5, 0.0, 0.0, dd);
        break;
    case 1:
        yi = yi - offset;
        GText(xi, yi, INCHES, l, 0.5, 1 - (0.5 - gpptr(dd)->yCharOffset), 0.0, dd);
    }
}
    ...
    else
    {
        ...
        while (k < npts)
        {
            ...
            else
            {
                ...
                if (plot)
                    drawLabel(xi, yi, INTEGER(pos)[imin], offset, CHAR(STRING_ELT(l, imin)), dd);
            }
        }
        GMode(0, dd);
...
SEXP do_dotplot(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    save_mUnits = gpptr(dd)->mUnits;
    save_defaultPlot = dpptr(dd)->defaultPlot;
    gpptr(dd)->mar[1] = gpptr(dd)->mar[3] + gw;
    dpptr(dd)->mUnits = gpptr(dd)->mUnits = LINES;
    dpptr(dd)->defaultPlot = gpptr(dd)->defaultPlot = 1;
    GReset(dd);
...
SEXP do_strheight(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
        if (isExpression(str))
            REAL(ans)[i] = GExpressionHeight(VECTOR_ELT(str, i), GMapUnits(units), dd);
        else
            REAL(ans)[i] = GStrHeight(CHAR(STRING_ELT(str, i)), GMapUnits(units), dd);
...
SEXP do_strwidth(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < n; i++)
        if (isExpression(str))
            REAL(ans)[i] = GExpressionWidth(VECTOR_ELT(str, i), GMapUnits(units), dd);
        else
            REAL(ans)[i] = GStrWidth(CHAR(STRING_ELT(str, i)), GMapUnits(units), dd);
    gpptr(dd)->cex = cexsave;
    UNPROTECT(1);
    return ans;
}
...
SEXP do_dendwindow(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    pin = gpptr(dd)->pin[1];
    for (i = 0; i < n; i++)
        ll[i] = GStrWidth(CHAR(STRING_ELT(llabels, i)), INCHES, dd) + dnd_offset;
    if (dnd_hang >= 0)
    {
...
SEXP do_symbols(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    PROTECT(fg = FixupCol(CAR(args), NA_INTEGER));
    args = CDR(args);
    nfg = LENGTH(fg);
    ...
    case 1: /* circles */
        ...
        for (i = 0; i < nr; i++)
        {
            if (R_FINITE(REAL(x)[i]) && R_FINITE(REAL(y)[i]) && R_FINITE(REAL(p)[i]))
            {
                ...
                GCircle(REAL(x)[i], REAL(y)[i], USER, rx, INTEGER(bg)[i % nbg], INTEGER(fg)[i % nfg], dd);
            }
        }
        break;
    case 2: /* squares */
    ...
    case 3: /* rectangles */
        ...
        for (i = 0; i < nr; i++)
        {
            if (R_FINITE(REAL(x)[i]) && R_FINITE(REAL(y)[i]) && R_FINITE(REAL(p)[i]) && R_FINITE(REAL(p)[i + nr]))
            {
                xx = REAL(x)[i];
                yy = REAL(y)[i];
                GConvert(&xx, &yy, USER, DEVICE, dd);
        ...
        for (i = 0; i < nr; i++)
        {
            ...
            if (R_FINITE(xx) && R_FINITE(yy))
            {
                ...
                for (j = 0; j < nc; j++)
                {
                    ...
                    yp[j] = GConvertYUnits(pp[j] * sin(j * p1), INCHES, NDC, dd) + yy;
                }
                GPolygon(nc, xp, yp, NDC, INTEGER(bg)[i % nbg], INTEGER(fg)[i % nfg], dd);
            }
        }
        vmaxset(vmax);
        break;
    case 5: /* thermometers */
        ...
        if (pmin < 0. || pmax > 1.) /* S-PLUS has an error here */
            warningcall(call, "thermometers[,%s] not in [0,1] -- may look funny", (nc == 4) ? "3:4" : "3");
        if (!SymbolRange(REAL(p), 2 * nr, &pmax, &pmin))
        ...
        for (i = 0; i < nr; i++)
        {
            ...
            if (R_FINITE(xx) && R_FINITE(yy))
            {
                ...
                if (R_FINITE(p0) && R_FINITE(p1) && R_FINITE(p2) && R_FINITE(p3))
                {
                    if (p2 < 0)
                        p2 = 0;
                    else if (p2 > 1)
                    ...
                    GRect(xx - rx, yy - ry, xx + rx, yy + ry, NDC, INTEGER(bg)[i % nbg], INTEGER(fg)[i % nfg], dd);
                    GRect(xx - rx, yy - (1 - 2 * p2) * ry, xx + rx, yy - (1 - 2 * p3) * ry, NDC, INTEGER(fg)[i % nfg],
                          INTEGER(fg)[i % nfg], dd);
    ...
    case 6: /* boxplots (wid, hei, loWhsk, upWhsk, medProp) */
        ...
        for (i = 0; i < nr; i++)
        {
            ...
            if (R_FINITE(xx) && R_FINITE(yy))
            {
                ...
                p4 = REAL(p)[i + 4 * nr]; /* median proport. in [0,1] */
                if (R_FINITE(p0) && R_FINITE(p1) && R_FINITE(p2) && R_FINITE(p3) && R_FINITE(p4))
                {
                    GConvert(&xx, &yy, USER, NDC, dd);
                    if (inches > 0)
                    {
