...
static struct
{
    ...
    "Alpha",
    65, /* Upper Case Greek Characters */
    "Beta",
    66,
    "Chi",
    67,
    ...
    "alpha",
    97, /* Lower Case Greek Characters */
    "beta",
    98,
    "chi",
    99,
    ...
    "universal",
    34, /* Miscellaneous Special Characters */
    "existential",
    36,
    "therefore",
    92,
...
static struct
{
    char *name;
    int code;
} BinopTable[] = {"*", 42, /* Binary Relationships */
                  "+", 43, "-", 45, "/", 47, ":", 58, NULL, 0};
...
static int atomFontFace(SEXP expr)
{
    ...
    if (symbolAtom(expr) && (symbolSymbol(expr) || binAtom(expr) || relAtom(expr) || groupAtom(expr) ||
                             operatorAtom(expr) || radicalAtom(expr)))
        fontFace = 5;
...
static BBOX relBBox(SEXP expr)
{
    ...
    return combineBBoxes(
        ...
        combineBBoxes(gapBBox(relGap()),
                      combineBBoxes(asciiBBox(relAscii(op)), combineBBoxes(gapBBox(relGap()), elementBBox(arg2)))));
}
