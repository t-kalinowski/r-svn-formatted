...
static struct
{
    ...
    "bracketleft",
    91, /* Miscellaneous Special Characters */
    "therefore",
    92,
    "bracketright",
    93,
    ...
    "braceleft",
    123, /* Miscellaneous Special Characters */
    "bar",
    124,
    "braceright",
    125,
...
{
    return (MathDevice->gp.font == ItalicFont || MathDevice->gp.font == BoldItalicFont);
}
    ...
    if (str)
    {
        ...
        while (*s)
        {
            ...
            if (draw)
            {
                ...
                GText(ConvertedX(), ConvertedY(), INCHES, chr, 0.0, 0.0, CurrentAngle, MathDevice);
...
static SymTab BinTable[] = {
    ...
    "+",  053,       "-",  055,  "/", 057, ":", 072, "%+-%", 0261, "%*%", 0264, "%/%", 0270, "%intersection%",
    0307, "%union%", 0310, NULL, 0};
...
static struct
{
    ...
    "hat", 94, "ring", 176, "tilde", 126, NULL, 0,
};
...
static SymTab OpTable[] = {"prod", S_PRODUCT, "sum",    S_SUM,    "union",  S_UNION,  "intersect", S_INTERSECTION,
...
SymTab RelTable[] = {
    ...
    "%<->%",      171, /* Arrows */
    "%<-%",       172, "%up%",        173, "%->%",      174, "%down%",     175, "%<=>%", 219, "%<=%",    220,
    "%dblup%",    221, "%=>%",        222, "%dbldown%", 223,

    "%supset%",   201, /* Sets (TeX Names) */
    "%supseteq%", 202, "%notsubset%", 203, "%subset%",  204, "%subseteq%", 205, "%in%",  206, "%notin%", 207,
