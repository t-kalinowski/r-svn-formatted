...
static double FontHeight()
{
     ...
     */
    GEMetricInfo(0, MathFont, MathCex, MathPs, &height, &depth, &width, MathDevice);
...
{
    double height, depth, width;
    GEMetricInfo('x', MathFont, MathCex, MathPs, &height, &depth, &width, MathDevice);
...
{
    double height, depth, width;
    GEMetricInfo('+', MathFont, MathCex, MathPs, &height, &depth, &width, MathDevice);
...
{
    double height, depth, width;
    GEMetricInfo('M', MathFont, MathCex, MathPs, &height, &depth, &width, MathDevice);
...
{
    double height, depth, width;
    GEMetricInfo('g', MathFont, MathCex, MathPs, &height, &depth, &width, MathDevice);
...
static double AscHeight()
{
    ...
    GEMetricInfo('d', MathFont, MathCex, MathPs, &height, &depth, &width, MathDevice);
    save = height;
    GEMetricInfo('a', MathFont, MathCex, MathPs, &height, &depth, &width, MathDevice);
...
static double TeX(TEXPAR which)
{
    ...
    case sigma8:                                                        /* num1 */
        return AxisHeight() + 3.51 * RuleThickness() + 0.15 * XHeight() /* 54/36 * 0.1 */
               + SUBS * DescDepth();
    case sigma9:                                                                                     /* num2 */
        return AxisHeight() + 1.51 * RuleThickness() + 0.08333333 * XHeight();                       /* 30/36 * 0.1 */
    case sigma10:                                                                                    /* num3 */
        return AxisHeight() + 1.51 * RuleThickness() + 0.1333333 * XHeight();                        /* 48/36 * 0.1 */
    case sigma11:                                                                                    /* denom1 */
        return -AxisHeight() + 3.51 * RuleThickness() + SUBS * FigHeight() + 0.344444 * XHeight();   /* 124/36 * 0.1 */
    case sigma12:                                                                                    /* denom2 */
        return -AxisHeight() + 1.51 * RuleThickness() + SUBS * FigHeight() + 0.08333333 * XHeight(); /* 30/36 * 0.1 */
...
{
    return (MathFont == ItalicFont || MathFont == BoldItalicFont);
}
...
static BBOX GlyphBBox(int chr)
{
    BBOX bbox;
    double height, depth, width;
    GEMetricInfo(chr, MathFont, MathCex, MathPs, &height, &depth, &width, MathDevice);
...
static BBOX RenderSymbolChar(int ascii, int draw)
{
    ...
    if (draw)
    {
        asciiStr[0] = ascii;
        asciiStr[1] = '\0';
        GEText(ConvertedX(), ConvertedY(), asciiStr, 0.0, 0.0, CurrentAngle, TextColor, MathGamma, MathFontFamily,
               MathFont, MathLineHeight, MathCex, MathPs, MathDevice);
        PMoveAcross(bboxWidth(bbox));
    }
    SetFont(prev);
    return bbox;
    ...
    if (str)
    {
        ...
        while (*s)
        {
            ...
            if (draw)
            {
                ...
                GEText(ConvertedX(), ConvertedY(), chr, 0.0, 0.0, CurrentAngle, TextColor, MathGamma, MathFontFamily,
                       MathFont, MathLineHeight, MathCex, MathPs, MathDevice);
...
static BBOX RenderStr(char *str, int draw)
{
    ...
    if (str)
    {
        ...
        if (draw)
        {
            GEText(ConvertedX(), ConvertedY(), str, 0.0, 0.0, CurrentAngle, TextColor, MathGamma, MathFontFamily,
                   MathFont, MathLineHeight, MathCex, MathPs, MathDevice);
...
static BBOX RenderSlash(int draw)
{
    ...
    if (draw)
    {
        ...
        y[1] = ConvertedY();
        PMoveUp(-height);
        GEPolyline(2, x, y, TextColor, MathGamma, LTY_SOLID, 1, MathDevice);
        PMoveAcross(0.5 * width);
    }
...
static BBOX RenderWideTilde(SEXP expr, int draw)
{
    ...
    if (draw)
    {
        ...
        GEPolyline(NTILDE + 3, x, y, TextColor, MathGamma, LTY_SOLID, 1, MathDevice);
...
static BBOX RenderWideHat(SEXP expr, int draw)
{
    ...
    if (draw)
    {
        ...
        x[2] = ConvertedX();
        y[2] = ConvertedY();
        GEPolyline(3, x, y, TextColor, MathGamma, LTY_SOLID, 1, MathDevice);
...
static void NumDenomVShift(BBOX numBBox, BBOX denomBBox, double *u, double *v)
{
...
static BBOX RenderOpSymbol(SEXP op, int draw)
{
    ...
    if (opId == S_SUM || opId == S_PRODUCT || opId == S_UNION || opId == S_INTERSECTION)
    {
        if (display)
        {
            ...
            return RenderSymbolChar(opId, draw);
    }
    else
    {
        FontType prevfont = SetFont(PlainFont);
...
static BBOX RenderRadical(SEXP expr, int draw)
{
    ...
    if (draw)
    {
        ...
        x[4] = ConvertedX();
        y[4] = ConvertedY();
        GEPolyline(5, x, y, TextColor, MathGamma, LTY_SOLID, 1, MathDevice);
        PMoveTo(savedX, savedY);
    }
    orderBBox = CombineAlignedBBoxes(orderBBox, RenderGap(leadWidth + radSpace, draw));
    SetPrimeStyle(style);
...
double GEExpressionWidth(SEXP expr, int font, double cex, double ps, GEDevDesc *dd)
{
    BBOX bbox;
    double width;
    MathDevice = dd;
...
double GEExpressionHeight(SEXP expr, int font, double cex, double ps, GEDevDesc *dd)
{
    BBOX bbox;
    double height;
    MathDevice = dd;
...
void GEMathText(double x, double y, SEXP expr, double xc, double yc, double rot, int col, double gamma, int font,
                double cex, double ps, GEDevDesc *dd)
{
    ...
    GEMetricInfo(0, font, cex, ps, &ascent, &descent, &width, dd);
...
{
    double width = GEExpressionWidth(expr, Rf_gpptr(dd)->font, Rf_gpptr(dd)->cex, Rf_gpptr(dd)->ps, (GEDevDesc *)dd);
    if (units == DEVICE)
        return width;
    else
...
{
    double height = GEExpressionHeight(expr, Rf_gpptr(dd)->font, Rf_gpptr(dd)->cex, Rf_gpptr(dd)->ps, (GEDevDesc *)dd);
    if (units == DEVICE)
        return height;
    else
...
void GMathText(double x, double y, int coords, SEXP expr, double xc, double yc, double rot, DevDesc *dd)
{
    ...
    GClip(dd);
    GEMathText(x, y, expr, xc, yc, rot, Rf_gpptr(dd)->col, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->font, Rf_gpptr(dd)->cex,
...
void GMMathText(SEXP str, int side, double line, int outer, double at, int las, DevDesc *dd)
{
     ...
     */
    GClip(dd);
    GEMathText(at, line, str, xadj, yadj, angle, Rf_gpptr(dd)->col, Rf_gpptr(dd)->gamma, Rf_gpptr(dd)->font,
