...
static struct
{
    ...
    "Alpha",   65,  "Beta",  66,  "Chi",   67,  "Delta", 68,  "Epsilon", 69,  "Phi", 70,  "Gamma",   71,
    "Eta",     72,  "Iota",  73,  "Phi1",  74,  "Kappa", 75,  "Lambda",  76,  "Mu",  77,  "Nu",      78,
    "Omicron", 79,  "Pi",    80,  "Theta", 81,  "Rho",   82,  "Sigma",   83,  "Tau", 84,  "Upsilon", 85,
    "sigma1",  86,  "Omega", 87,  "Xi",    88,  "Psi",   89,  "Zeta",    90,

    "alpha",   97,  "beta",  98,  "chi",   99,  "delta", 100, "epsilon", 101, "phi", 102, "gamma",   103,
    "eta",     104, "iota",  105, "phi1",  106, "kappa", 107, "lambda",  108, "mu",  109, "nu",      110,
    "omicron", 111, "pi",    112, "theta", 113, "rho",   114, "sigma",   115, "tau", 116, "upsilon", 117,
    "omega1",  118, "omega", 119, "xi",    120, "psi",   121, "zeta",    122,
...
static int binAtom(SEXP expr)
{
    int result = symbolAtom(expr) &&
                 ((expr == plusSymbol) || (expr == minusSymbol) || (expr == timesSymbol) || (expr == divideSymbol));
    return result;
}

static int relAtom(SEXP expr)
{
    return symbolAtom(expr) && (expr == equalSymbol);
}

static int multiplicationOperator(SEXP expr)
{
    return binAtom(expr) && (expr == timesSymbol);
}

static int superAtom(SEXP expr)
{
    return symbolAtom(expr) && (expr == superSymbol);
}

static int subAtom(SEXP expr)
{
    return symbolAtom(expr) && (expr == subSymbol);
}
...
{
    return symbolAtom(expr) && (hatAtom(expr) || barAtom(expr) || symbolMatch(expr, "tilde"));
}

static int fractionAtom(SEXP expr)
{
    return symbolAtom(expr) && (symbolMatch(expr, "over") || symbolMatch(expr, "frac"));
}
...
static int operatorAtom(SEXP expr)
{
    return symbolAtom(expr) &&
           (symbolMatch(expr, "sum") || symbolMatch(expr, "integral") || symbolMatch(expr, "product"));
}
...
{
    return symbolAtom(expr) && (symbolMatch(expr, "root") || symbolMatch(expr, "sqrt"));
}
...
{
    return formulaExpression(expr) && (italicAtom(CAR(expr)) || boldItalicAtom(CAR(expr)));
}

static int nonItalicExpression(SEXP expr)
{
    return formulaExpression(expr) && (boldAtom(CAR(expr)) || plainAtom(CAR(expr)));
}
...
static void boldFont()
{
    setFont(2);
}
static void italicFont()
{
    setFont(3);
}
static void plainFont()
{
    setFont(1);
}
static void boldItalicFont()
{
    setFont(4);
}
...
static int atomFontFace(SEXP expr)
{
    ...
    if (symbolAtom(expr))
    {
        if (greekSymbol(expr) || binAtom(expr) || relAtom(expr) || groupAtom(expr) || operatorAtom(expr) ||
            radicalAtom(expr))
            fontFace = 5;
        else
#ifdef OLD
...
static void supsubShift(SEXP body, SEXP sup, SEXP sub, double *supShift, double *subShift)
{
    ...
    *subShift = subscriptShift(body, sub, 0);
    ...
    temp1 = (*supShift - bboxDepth(superscriptBBox)) - (bboxHeight(subscriptBBox) - *subShift);
    if (temp1 < (4 * lineWidth()))
        *subShift = bboxHeight(subscriptBBox) - *supShift + bboxDepth(superscriptBBox) + (4 * lineWidth());

    temp2 = (4 * xHeight() / 5) - (*supShift - bboxDepth(superscriptBBox));
    if (temp2 > 0)
    {
...
{
    return (bboxWidth(elementBBox(body)) - bboxWidth(elementBBox(accent))) / 2;
}
...
static double numeratorVShift(SEXP num)
{
    ...
    theClearance = numeratorShift() - bboxDepth(numBBox) - (axisHeight() + 0.5 * lineWidth());
...
static double denominatorVShift(SEXP denom)
{
    ...
    theClearance = axisHeight() - 0.5 * lineWidth() - (bboxHeight(denomBBox) - denominatorShift());
    minClearance = 3 * lineWidth();
...
/* RATIO */
static void numdenomHShift(SEXP num, SEXP denom, double *numShift, double *denomShift)
{
    BBOX numBBox;
    BBOX denomBBox;
    float cexSaved = mathDevice->gp.cex;
...
static int normalRadical(SEXP body)
{
    ...
    return ((bboxHeight(bodyBBox) + radicalDrop()) <= bboxHeight(radicalBBox)) &&
...
{
    return makeBBox(bboxHeight(bbox) + shiftV, bboxDepth(bbox) - shiftV, bboxWidth(bbox));
}

static BBOX combineBBoxes(BBOX bbox1, BBOX bbox2)
{
    return makeBBox(max(bboxHeight(bbox1), bboxHeight(bbox2)), max(bboxDepth(bbox1), bboxDepth(bbox2)),
...
{
    double rotatedY = referenceY + (currentY - referenceY) * cosAngle + (currentX - referenceX) * sinAngle;
    return rotatedY;
}
...
static void drawAscii(int ascii)
{
    ...
    asciiStr[0] = ascii;
    asciiStr[1] = '\0';
    GText(convertedX(), convertedY(), INCHES, asciiStr, 0.0, 0.0, currentAngle, mathDevice);
...
{
    mathDevice->gp.font = atomFontFace(expr);
    GText(convertedX(), convertedY(), INCHES, str, 0.0, 0.0, currentAngle, mathDevice);
...
{
    if (((isItalic() && !nonItalicExpression(expr1)) || italicExpression(expr1)) &&
        ((!isItalic() && !italicExpression(expr2)) || nonItalicExpression(expr2)))
...
static BBOX binBBox(SEXP expr)
{
    ...
    if (length(expr) == 3)
    {
        ...
        if (multiplicationOperator(operator))
            middleBBox = correctionBetweenBBox(operand1, operand2);
        else
            middleBBox = combineBBoxes(gapBBox(binGapBefore(operand1)),
                                       combineBBoxes(atomBBox(operator), gapBBox(binGapAfter(operand2))));

        return combineBBoxes(elementBBox(operand1), combineBBoxes(middleBBox, elementBBox(operand2)));
    }
    else if (length(expr) == 2)
    {
        middleBBox = combineBBoxes(atomBBox(operator), smallgapBBox(binGapAfter(operand1)));
...
static BBOX supBBox(SEXP expr)
{
    ...
    if (supsub)
        ...
        return combineBBoxes(elementBBox(body),
                             combineBBoxes(correctionWithinBBox(body), shiftBBox(superscriptBBox(superscript),
...
static BBOX subBBox(SEXP expr)
{
    ...
    return combineBBoxes(elementBBox(body), shiftBBox(subscriptBBox(subscript), subscriptShift(body, subscript, 1)));
}
...
static BBOX supsubBBox(SEXP body, SEXP superscript, SEXP subscript)
{
    ...
    return combineBBoxes(elementBBox(body),
                         combineAlignedBBoxes(combineBBoxes(correctionWithinBBox(body),
...
{
    BBOX bodyBBox = elementBBox(body);
    return combineAlignedBBoxes(bodyBBox, makeBBox(bboxHeight(bodyBBox) + customAccentGap() + customHatHeight(), 0, 0));
}

static BBOX barBBox(SEXP body)
{
    BBOX bodyBBox = elementBBox(body);
    return combineAlignedBBoxes(bodyBBox, makeBBox(bboxHeight(bodyBBox) + customAccentGap(), 0, 0));
}
    ...
    if (hatAtom(accent))
        ...
        return combineAlignedBBoxes(elementBBox(body),
                                    combineBBoxes(makeBBox(accentHShift(body, accent), 0, 0),
                                                  shiftBBox(asciiBBox(accentAscii(accent)), accentVShift(body))));
}
...
static BBOX fractionBBox(SEXP expr)
{
    ...
    return combineAlignedBBoxes(
        shiftBBox(combineBBoxes(makeBBox(numHShift, 0, 0), numBBox), numeratorVShift(numerator)),
        shiftBBox(combineBBoxes(makeBBox(denomHShift, 0, 0), denomBBox), -denominatorVShift(denominator)));
}
...
static BBOX groupBBox(SEXP expr)
{
    return combineBBoxes(asciiBBox(groupOpenAscii()),
                         combineBBoxes(elementBBox(CADR(expr)),
                                       combineBBoxes(correctionWithinBBox(CADR(expr)), asciiBBox(groupCloseAscii()))));
}
...
static double integralTopShift()
{
    return 0.5 * fontHeight();
}

static double integralBottomShift()
{
    return -0.5 * fontHeight();
}
    ...
    if (integralOperator(operator))
        return combineAlignedBBoxes(
            ...
            combineAlignedBBoxes(asciiBBox(integralAscii(2)),
                                 shiftBBox(asciiBBox(integralAscii(3)), integralBottomShift())));
    else
...
static BBOX operatorBBox(SEXP expr)
{
    ...
    {
        body = CADR(expr);
        bodyBBox = combineBBoxes(opBBox, combineBBoxes(gapBBox(operatorGap(body)), elementBBox(body)));
        ...
        if (length(expr) > 2)
        {
            ...
            if (length(expr) > 3)
            {
                ...
                return combineAlignedBBoxes(
                    combineBBoxes(makeBBox(operatorHShiftAll(operator, lower, upper), 0, 0), bodyBBox),
                    combineAlignedBBoxes(
                        shiftBBox(
                            combineBBoxes(makeBBox(operatorUpperHShiftAll(operator, lower, upper), 0, 0), upperBBox),
                            operatorUpperShift(operator, upper)),
                        shiftBBox(
                            combineBBoxes(makeBBox(operatorLowerHShiftAll(operator, lower, upper), 0, 0), lowerBBox),
                            operatorLowerShift(operator, lower))));
            }
            else
                return combineAlignedBBoxes(
                    combineBBoxes(makeBBox(operatorHShift(operator, lower), 0, 0), bodyBBox),
                    shiftBBox(combineBBoxes(makeBBox(operatorLowerHShift(operator, lower), 0, 0), lowerBBox),
                              operatorLowerShift(operator, lower)));
        }
        else
            return bodyBBox;
...
{
    BBOX bodyBBox = elementBBox(body);
    return combineBBoxes(makeBBox(bboxHeight(bodyBBox) + customRadicalGap(), 0, customRadicalWidth()),
                         combineBBoxes(makeBBox(0, 0, customRadicalSpace()), bodyBBox));
}
...
{
    SEXP body = CADR(expr);
    return combineBBoxes(makeBBox(0, 0, absSpace()), combineBBoxes(elementBBox(body), makeBBox(0, 0, absSpace())));
}
...
static BBOX expressionBBox(SEXP expr)
{
    ...
    for (i = 0; i < numParams; i++)
    {
        ...
        expr = CDR(expr);
        if (i < numParams - 1)
            resultBBox = combineBBoxes(resultBBox, combineBBoxes(asciiBBox(commaAscii()), asciiBBox(spaceAscii())));
    }
    return combineBBoxes(resultBBox, combineBBoxes(correctionWithinBBox(lastTerm), asciiBBox(groupCloseAscii())));
}
...
static BBOX relBBox(SEXP expr)
{
    ...
    return combineBBoxes(
        elementBBox(arg1),
        combineBBoxes(gapBBox(relGap()),
                      combineBBoxes(asciiBBox(relAscii()), combineBBoxes(gapBBox(relGap()), elementBBox(arg2)))));
}
...
static BBOX concatenateBBox(SEXP expr)
{
    ...
    {
        result =
            combineBBoxes(result, combineBBoxes(correctionBetweenBBox(lastArg, CAR(args)), elementBBox(CAR(args))));
        lastArg = CAR(args);
        args = CDR(args);
    }
