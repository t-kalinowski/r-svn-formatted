        ...
        for (i = 0; i < ns; i++)
        {
            ...
            case REALSXP:
                if (LENGTH(tmp) == 1)
                {
                    ...
                    snprintf(pbuf, 115, "%s", EncodeReal(REAL(tmp)[0], w, d, e, OutDec));
                }
                else
    ...
    case RAWSXP:
        ...
        if (TYPEOF(t) == INTSXP)
        {
            ...
            else if (LENGTH(t) == 2)
            {
                ...
                char *rn, *cn;
                GetMatrixDimnames(s, &rl, &cl, &rn, &cn);
                printMatrix(s, 0, t, R_print.quote, R_print.right, rl, cl, rn, cn);
            }
            else
            {
                SEXP dimnames;
    ...
    if (a != R_NilValue)
    {
        ...
        while (a != R_NilValue)
        {
            ...
            if (isObject(CAR(a)))
            {
                ...
                */
                SEXP s, t, na_string = R_print.na_string, na_string_noquote = R_print.na_string_noquote;
                int quote = R_print.quote, digits = R_print.digits, gap = R_print.gap, na_width = R_print.na_width,
                    na_width_noquote = R_print.na_width_noquote;
                Rprt_adj right = R_print.right;

                PROTECT(t = s = allocList(3));
                SET_TYPEOF(s, LANGSXP);
                SETCAR(t, install("print"));
                t = CDR(t);
                SETCAR(t, CAR(a));
                t = CDR(t);
...
attribute_hidden int F77_NAME(dblep0)(char *label, int *nchar, double *data, int *ndata)
{
    int k, nc = *nchar;
...
attribute_hidden int F77_NAME(intpr0)(char *label, int *nchar, int *data, int *ndata)
{
    int k, nc = *nchar;
...
attribute_hidden int F77_NAME(realp0)(char *label, int *nchar, float *data, int *ndata)
{
