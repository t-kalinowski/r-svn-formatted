...
static void printFactorMatrix(SEXP sx, int offset, int r, int c, SEXP rl, SEXP cl, SEXP levels, int nlev)
{
    ...
    while (jmin < c)
    {
        ...
        for (i = 0; i < r; i++)
        {
            ...
            for (j = jmin; j < jmax; j++)
            {
                Rprintf("%*s%s", PRINT_GAP, " ", EncodeFactor(x[i + j * r], nlev, w[j], levels));
            }
        }
        Rprintf("\n");
        jmin = jmax;
...
static void printStringMatrix(SEXP sx, int offset, int r, int c, int quote, SEXP rl, SEXP cl)
{
    ...
    while (jmin < c)
    {
        ...
        for (i = 0; i < r; i++)
        {
            ...
            for (j = jmin; j < jmax; j++)
            {
                Rprintf("%*s%s", PRINT_GAP, "", EncodeString(CHAR(x[i + j * r]), w[j], quote, adj_left));
            }
        }
        Rprintf("\n");
        jmin = jmax;
