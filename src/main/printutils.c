...
attribute_hidden const char *EncodeEnvironment(SEXP x)
{
    ...
    if (x == R_GlobalEnv)
        ...
        snprintf(ch, 1000, "<environment: namespace:%s>", translateChar(STRING_ELT(R_NamespaceEnvSpec(x), 0)));
    else
...
attribute_hidden int Rstrwid(const char *str, int slen, cetype_t ienc, int quote)
{
    ...
    if (ienc == CE_BYTES)
    { // not currently used for that encoding
        for (i = 0; i < slen; i++)
        {
    ...
    if (mbcslocale || ienc == CE_UTF8)
    {
        ...
        for (i = 0; i < slen; i++)
        {
            ...
            wchar_t wc;
            int res = useUTF8 ? (int)utf8toucs(&wc, p) : (int)mbrtowc(&wc, p, R_MB_CUR_MAX, &mb_st);
            if (res >= 0)
            {
...
attribute_hidden const char *EncodeString(SEXP s, int w, int quote, Rprt_adj justify)
{
    ...
    if (s == NA_STRING)
    {
        ...
        cnt = (int)(quote ? strlen(CHAR(R_print.na_string)) : strlen(CHAR(R_print.na_string_noquote)));
        i = (quote ? Rstrlen(R_print.na_string, 0) : Rstrlen(R_print.na_string_noquote, 0));
        quote = 0;
    }
    else
    {
    ...
    if (mbcslocale || ienc == CE_UTF8)
    {
        ...
        {
            wchar_t wc;
            int res = (int)(useUTF8 ? utf8toucs(&wc, p) : mbrtowc(&wc, p, R_MB_CUR_MAX, &mb_st));
            /* res = 0 is a terminator
             * some mbrtowc implementations return wc past end of UCS */
            if (res >= 0 && ((0 <= wc && wc <= 0x10FFFF) || !wchar_is_ucs_or_utf16))
            {
                unsigned int k; /* not wint_t as it might be signed */
                if (useUTF8 && IS_HIGH_SURROGATE(wc))
                    k = utf8toucs32(wc, p);
