...
#define CHARVECTOR_TIGHTLOOP(ENCCALL)                                                                                  \
    ...
    {                                                                                                                  \
        ...
        Rprintf("%*s%s", R_print.gap, "", ENCCALL);                                                                    \
        width += w + R_print.gap;                                                                                      \
    } while (0)
...
attribute_hidden void printIntegerVectorS(SEXP x, R_xlen_t n, int indx)
{
    int w, labwidth = 0, width;
    R_xlen_t i;
    DO_first_lab;
...
attribute_hidden void printRealVectorS(SEXP x, R_xlen_t n, int indx)
{
    int w, d, e, labwidth = 0, width;
    R_xlen_t i;
    DO_first_lab;
...
attribute_hidden void printComplexVector(const Rcomplex *x, R_xlen_t n, int indx)
{
    ...
    for (R_xlen_t i = 0; i < n; i++)
    {
        NUMVECTOR_TIGHTLOOP(CMPLX_ISNA(x[i]) ? EncodeReal0(NA_REAL, w, 0, 0, OutDec)
                                             : EncodeComplex(x[i], wr + R_print.gap, dr, er, wi, di, ei, OutDec));
    }
    Rprintf("\n");
}

attribute_hidden void printComplexVectorS(SEXP x, R_xlen_t n, int indx)
{
    int w, wr, dr, er, wi, di, ei, labwidth = 0, width;
    R_xlen_t i;
    DO_first_lab;
...
static void printStringVector(const SEXP *x, R_xlen_t n, int quote, int indx)
{
    ...
    for (R_xlen_t i = 0; i < n; i++)
    {
        ...
        Rprintf("%*s%s", R_print.gap, "", EncodeString(x[i], w, quote, R_print.right));
        width += w + R_print.gap;
    }
    Rprintf("\n");
}
...
static void printStringVectorS(SEXP x, R_xlen_t n, int quote, int indx)
{
    ...
    for (R_xlen_t i = 0; i < n; i++)
    {
        CHARVECTOR_TIGHTLOOP(EncodeString(STRING_ELT(x, i), w, quote, R_print.right));
    }
    Rprintf("\n");
}

attribute_hidden void printRawVector(const Rbyte *x, R_xlen_t n, int indx)
{
    int w, labwidth = 0, width;
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            for (j = 0; j < nperline && (k = i * nperline + j) < n; j++)                                               \
                Rprintf("%s%*s", EncodeString(STRING_ELT(names, k), w, 0, Rprt_adj_right), R_print.gap, "");           \
            Rprintf("\n");                                                                                             \
            for (j = 0; j < nperline && (k = i * nperline + j) < n; j++)                                               \
                PRINT_1;                                                                                               \
        }                                                                                                              \
