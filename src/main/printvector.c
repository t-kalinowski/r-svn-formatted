...
attribute_hidden void printComplexVector(const Rcomplex *x, R_xlen_t n, int indx)
{
    ...
    for (R_xlen_t i = 0; i < n; i++)
    {
        ...
        if (ISNA(x[i].r) || ISNA(x[i].i))
            ...
            Rprintf("%s", EncodeComplex(x[i], wr + R_print.gap, dr, er, wi, di, ei, OutDec));
        width += w;
    }
    Rprintf("\n");
}
...
#define P_IMAG_NA                                                                                                      \
    if (ISNAN(x[k].i))                                                                                                 \
        Rprintf("+%si", "NaN");                                                                                        \
                        ...
                        static void printNamedComplexVector(const Rcomplex *x, int n, const SEXP *names)
                            PRINT_N_VECTOR(INI_F_CPLX,
                                ...
                                static void printNamedStringVector(const SEXP *x, int n, int quote, const SEXP *names)
    ...
    void printNamedVector(SEXP x, SEXP names, int quote, const char *title)
{
    ...
    if ((n = LENGTH(x)) != 0)
    {
        ...
        case STRSXP:
            if (quote)
                quote = '"';
            printNamedStringVector(STRING_PTR_RO(x), n_pr, quote, STRING_PTR_RO(names));
            break;
        case RAWSXP:
