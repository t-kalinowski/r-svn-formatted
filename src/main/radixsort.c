...
SEXP attribute_hidden do_radixsort2(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (narg > 1 && gsngrp[flip] < n)
    {
        ...
        if (xsub == NULL)
            Error("Couldn't allocate xsub in forder, requested %d * %d bytes.", maxgrpn, sizeof(double));
        // global variable, used by isort, dsort, sort and cgroup
        newo = (int *)malloc(maxgrpn * sizeof(int));
        if (newo == NULL)
            Error("Couldn't allocate newo in forder, requested %d * %d bytes.", maxgrpn, sizeof(int));
    }
    ...
    if (!sortStr && ustr_n != 0)
        Error("Internal error: at the end of forder sortStr = =FALSE but ustr_n!=0 [%d]", ustr_n);
    for (int i = 0; i < ustr_n; i++)
        SET_TRUELENGTH(ustr[i], 0);
