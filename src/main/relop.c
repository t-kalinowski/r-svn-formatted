...
SEXP attribute_hidden do_relop_dflt(SEXP call, SEXP op, SEXP x, SEXP y)
{
     ...
     */
    if (ATTRIB(x) == R_NilValue && ATTRIB(y) == R_NilValue && (typex == REALSXP || typex == INTSXP) &&
        (typey == REALSXP || typey == INTSXP) && nx > 0 && ny > 0)
    {
        ...
        if (nx == 1 && ny == 1)
        {
            ...
            {
                double dx = typex == REALSXP ? REAL(x)[0] : INTEGER(x)[0] != NA_INTEGER ? INTEGER(x)[0] : NA_REAL;
                double dy = typey == REALSXP ? REAL(y)[0] : INTEGER(y)[0] != NA_INTEGER ? INTEGER(y)[0] : NA_REAL;
                if (ISNAN(dx) || ISNAN(dy))
                    return ScalarLogical(NA_LOGICAL);
    ...
    if ((iS = isSymbol(x)) || TYPEOF(x) == LANGSXP)
    {
        ...
        PROTECT(tmp);
        SET_STRING_ELT(tmp, 0, (iS) ? PRINTNAME(x) : STRING_ELT(deparse1(x, 0, DEFAULTDEPARSE), 0));
        REPROTECT(x = tmp, xpi);
        UNPROTECT(1);
    }
    if ((iS = isSymbol(y)) || TYPEOF(y) == LANGSXP)
    {
        SEXP tmp = allocVector(STRSXP, 1);
        PROTECT(tmp);
        SET_STRING_ELT(tmp, 0, (iS) ? PRINTNAME(y) : STRING_ELT(deparse1(y, 0, DEFAULTDEPARSE), 0));
        REPROTECT(y = tmp, ypi);
        UNPROTECT(1);
    }

    if (isNull(x))
        REPROTECT(x = allocVector(INTSXP, 0), xpi);
    if (isNull(y))
        REPROTECT(y = allocVector(INTSXP, 0), ypi);
...
SEXP attribute_hidden do_bitwise(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    case 4:
        ...
        break;
    case 5:
        ans = bitwiseShiftL(CAR(args), CADR(args));
        break;
    case 6:
        ans = bitwiseShiftR(CAR(args), CADR(args));
        break;
    }
    return ans;
}
