...
int Ri18n_wcwidth(R_wchar_t c)
{
    ...
    if (0 != strcmp(setlocale(LC_CTYPE, NULL), lc_cache))
    {
        ...
        {
            if (0 == strncmp(cjk_locale_name[i].name, lc_str, strlen(cjk_locale_name[i].name)))
            {
    ...
    int wd = wcwidthsearch(c, table_wcwidth, (sizeof(table_wcwidth) / sizeof(struct interval_wcwidth)), lc);
    if (wd >= 0)
...
*/
attribute_hidden int Ri18n_wcswidth(const wchar_t *wc, size_t n)
{
    int rs = 0;
    while ((n-- > 0) && (*wc != L'\0'))
    {
        ...
        {
            R_wchar_t val = ((*wc & 0x3FF) << 10) + (*(wc + 1) & 0x3FF) + 0x010000;
...
#define ISWFUNC(ISWNAME)                                                                                               \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            _mb_buf = (char *)mb_buf;                                                                                  \
            rc = Riconv(cd, (const char **)&_mb_buf, (size_t *)&mb_len, (char **)&_wc_buf, (size_t *)&wc_len);         \
            Riconv_close(cd);                                                                                          \
            wc = ucs4_buf[0];                                                                                          \
            return wcsearch(wc, table_w##ISWNAME, table_w##ISWNAME##_count);                                           \
        }                                                                                                              \
...
static const Ri18n_wctype_func_l Ri18n_wctype_func[] = {{"upper", 1 << 0, Ri18n_iswupper},
...
wctype_t Ri18n_wctype(const char *name)
{
    ...
    for (i = 0; Ri18n_wctype_func[i].name != NULL && 0 != strcmp(Ri18n_wctype_func[i].name, name); i++)
        ;
...
int Ri18n_iswctype(wint_t wc, wctype_t desc)
{
    ...
    for (i = 0; Ri18n_wctype_func[i].wctype != 0 && Ri18n_wctype_func[i].wctype != desc; i++)
        ;
...
{
    int res = tlsearch(wc, table_toupper, sizeof(table_toupper) / sizeof(struct pair));
    return (res >= 0 ? res : wc);
}

R_wchar_t Ri18n_towlower(R_wchar_t wc)
{
    int res = tlsearch(wc, table_tolower, sizeof(table_tolower) / sizeof(struct pair));
    return (res >= 0 ? res : wc);
}
#endif
