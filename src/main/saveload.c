...
static SEXP OffsetToNode(int offset)
{
    ...
    if (offset == OldOffset[m])
        return VECTOR_ELT(NewAddress, m);
...
static void RemakeNextSEXP(FILE *fp)
{
    ...
    idx = InInteger(fp), type = FixupType(InInteger(fp));
    ...
    case EXPRSXP:
        ...
        break;
    default:
        error("bad SEXP type in data file");
    }
...
static void NewWriteItem(SEXP s, SEXP sym_list, SEXP env_list, FILE *fp)
{
    ...
    else
    {
        ...
        OutSpace(fp, 1);
        OutInteger(fp, LEVELS(s));
        OutSpace(fp, 1);
        OutInteger(fp, OBJECT(s));
        OutNewline(fp);
        switch (TYPEOF(s))
        {
...
static void NewDataSave(SEXP s, FILE *fp)
{
    ...
    OutInit(fp);
    OutInteger(fp, sym_count = HASH_TABLE_COUNT(sym_table));
    OutSpace(fp, 1);
    OutInteger(fp, env_count = HASH_TABLE_COUNT(env_table));
    OutNewline(fp);
