...
static void AsciiOutReal(FILE *fp, double x)
{
    if (!R_FINITE(x))
    {
        ...
        fprintf(fp, "%.16g", x); /* 16: full precision; 17 gives 999, 000 &c */
}
...
static void MarkSave(SEXP s)
{
    ...
    if (s == R_NilValue || s == R_GlobalEnv || s == R_UnboundValue || s == R_MissingArg)
        return;
...
static void permute(struct permute_info *pinfo, unsigned dimnum)
{
    hssize_t i;
    ...
    else
    {
        ...
        if (H5Sselect_elements(pinfo->space, H5S_SELECT_SET, 1, (const hssize_t **)pinfo->coord) < 0)
...

        {
            ...
            if (pinfo->writeflag)
            {
                if (H5Dwrite(pinfo->dataset, pinfo->memtid, pinfo->mspace, pinfo->space, H5P_DEFAULT, pointaddr) < 0)
                    errorcall(pinfo->call, "Unable to write dataset");
            }
            else
            {
                if (H5Dread(pinfo->dataset, pinfo->memtid, pinfo->mspace, pinfo->space, H5P_DEFAULT, pointaddr) < 0)
...
static hid_t make_sexp_ref_type(SEXP call)
{
    hid_t memtid;
...
static hid_t get_string_type(SEXP call, SEXP vec)
{
    hid_t stid;
    unsigned vecpos;
    size_t maxstrlen = 0;
...
static void vector_io(SEXP call, int writeflag, hid_t dataset, hid_t space, SEXP obj)
{
...
static void hdf5_save_attributes(SEXP call, hid_t loc_id, SEXP val)
{
    SEXP l;
    ...
    for (l = ATTRIB(val); l != R_NilValue; l = CDR(l))
    {
            ...
            continue;
        {
            ...
            if ((sid = H5Screate_simple(1, dims, NULL)) < 0)
                errorcall(call, "unable to create vector space for attribute `%s'", name);
        }
        ...
        {
            if (H5Tclose(memtid) < 0)
                errorcall(call, "unable to close string reference type `%s'", name);
        }
...
static void hdf5_write_vector(SEXP call, hid_t id, const char *symname, SEXP val)
{
    unsigned i, rank;
    SEXP dimvec;
    hid_t space, dataset;
...
static void hdf5_write_string(SEXP call, hid_t fid, const char *symname, const char *str)
{
    hid_t stringtype;
    hid_t dataset;
    hid_t dataspace;
    ...
    if ((dataset = H5Dcreate(fid, symname, stringtype, dataspace, H5P_DEFAULT)) < 0)
        errorcall(call, "Unable to create dataset");

    if (H5Dwrite(dataset, stringtype, H5S_ALL, H5S_ALL, H5P_DEFAULT, str) < 0)
...
static unsigned align(unsigned offset, unsigned alignto)
{
#if 0
...
static void create_rownames_dataset_attribute(SEXP call, hid_t dataset, SEXP rownames)
{
    ...
    if ((rnattrib = H5Acreate(dataset, ROWNAMES, stringtid, rndataspace, H5P_DEFAULT)) < 0)
...
static void hdf5_save_object(SEXP call, hid_t fid, const char *symname, SEXP val)
{
    if (isFrame(val))
    {
        ...
        for (pos = 0; pos < colcount; pos++)
        {
            ...
            case LGLSXP:
                ...
                break;
            default:
                errorcall(call, "No support for converting R type: %d to HDF5", type);
                break;
            }
        ...
        size = align(size, H5Tget_size(hdftypes[0]));
        {
...

            {
                ...
                for (pos = 0; pos < colcount; pos++)
                    if (H5Tinsert(ctid, CHAR(STRING(colnames)[pos]), offsets[pos], hdftypes[pos]) < 0)
            ...
            if ((dataset = H5Dcreate(fid, symname, ctid, dataspace, H5P_DEFAULT)) < 0)
        ...
        case VECSXP: {
            ...
            if (type == LISTSXP)
            {
                ...
                for (l = val, pos = 0; l != R_NilValue; l = CDR(l), pos++)
                {
                    ...
                    if (!isNull(TAG(l)))
                        hdf5_save_object(call, gid, CHAR(PRINTNAME(TAG(l))), s);
                    else
                    {
                        sprintf(buf, "%u", pos);
            ...
            else
            {
                for (pos = 0; pos < len; pos++)
                {
                    ...
                    if (!isNull(names))
                        hdf5_save_object(call, gid, CHAR(STRING(names)[pos]), s);
                    else
                    {
                        sprintf(buf, "%u", pos);
                        hdf5_save_object(call, gid, buf, s);
...
static void add_to_list(struct hdf5_iterate_info *iinfo, const char *name, SEXP obj)
{
    PROTECT(iinfo->ret);
...
static SEXP collect(SEXP call, hid_t id, H5G_iterate_t iterate_func, SEXP env)
{
...
static void load_rownames_dataset_attribute(SEXP call, hid_t dataset, SEXP vec)
{
...
static herr_t hdf5_process_attribute(hid_t loc_id, const char *attrName, void *data)
{
    ...
    if ((sid = H5Aget_space(aid)) < 0)
        errorcall(ainfo->call, "could not open space of attribute `%s'", attrName);

    if ((tid = H5Aget_type(aid)) < 0)
        errorcall(ainfo->call, "could not get type of attribute `%s'", attrName);

    if ((tid_size = H5Tget_size(tid)) < 0)
        errorcall(ainfo->call, "could not get size of attribute `%s' tid", attrName);

    if ((class = H5Tget_class(tid)) < 0)
        errorcall(ainfo->call, "could not get type class of attribute `%s'", attrName);

    {
        int rank;

        if ((rank = H5Sget_simple_extent_ndims(sid)) < 0)
            errorcall(ainfo->call, "could not get rank of attribute space `%s'", attrName);
        {
            hsize_t dims[rank];

            if (H5Sget_simple_extent_dims(sid, dims, NULL) < 0)
                errorcall(ainfo->call, "could not get extent of attribute space `%s'", attrName);
            ...
            if (rank == 1)
            {
                ...
                case H5T_STRING:
                    ...
                    break;
                default:
                    warningcall(ainfo->call, "skipping attribute `%s' due to type", attrName);
                    goto done;
                }
                ...
                if (H5Tclose(memtid) < 0)
                    errorcall(ainfo->call, "unable to close reference type in attribute `%s'", attrName);
            }
            else
                warningcall(ainfo->call, "skipping attribute `%s' due to rank", attrName);
        }
    }
done:
    if (H5Sclose(sid) < 0)
    ...
    else if (statbuf.type == H5G_DATASET)
    {
...

        {
            ...
            if (type == VECSXP && rank == 1)
            {
                ...
                if (H5Dread(dataset, tid, space, space, H5P_DEFAULT, buf) < 0)
...
#define VECLOOP(vectype, vecref, dtid)                                                                                 \
    {                                                                                                                  \
            ...
            memcpy(&vecref(*rowptr)[ri], itembuf, dsize);                                                              \
                    ...
                    case H5T_STRING:
                        *rowptr = allocVector(STRSXP, rowcount);
                        ...
                        break;
                    default:
                        errorcall(iinfo->call, "can't handle hdf class %d", class);
                    }
                }
        ...
        errorcall(iinfo->call, "no support for HDF object type: %d", statbuf.type);
    return 0;
}

static void add_to_symbol_table(struct hdf5_iterate_info *iinfo, const char *name, SEXP obj)
{
    setVar(install((char *)name), obj, iinfo->env);
}

static void add_to_return_list(struct hdf5_iterate_info *iinfo, const char *name, SEXP obj)
{
    PROTECT(iinfo->ret);
    iinfo->ret = CONS(obj, iinfo->ret);
    TAG(iinfo->ret) = install((char *)name);
