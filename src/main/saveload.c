...
static void permute(struct permute_info *pinfo, unsigned dimnum)
{
    ...
    else
    {
        ...
        if (H5Sselect_elements(pinfo->space, H5S_SELECT_SET, 1, (const hssize_t **)pinfo->coord) == -1)
...

        {
            ...
            if (pinfo->writeflag)
            {
                if (H5Dwrite(pinfo->dataset, pinfo->memtid, pinfo->mspace, pinfo->space, H5P_DEFAULT, pointaddr) == -1)
                    errorcall(pinfo->call, "Unable to write dataset");
            }
            else
            {
                if (H5Dread(pinfo->dataset, pinfo->memtid, pinfo->mspace, pinfo->space, H5P_DEFAULT, pointaddr) == -1)
...
static void vector_io(SEXP call, int writeflag, hid_t dataset, hid_t space, SEXP obj)
{
    ...
    if (type == STRSXP)
    {
        ...
        if (writeflag)
        {
            if (H5Tregister_soft(REF2STRING_CONV, H5T_REFERENCE, H5T_STRING, ref_string) == -1)
                errorcall(call, "Unable to register ref->string converter");
        }
        else
        {
            if (H5Tregister_soft(STRING2REF_CONV, H5T_STRING, H5T_REFERENCE, string_ref) == -1)
...
static void hdf_write_vector(SEXP call, hid_t id, const char *symname, SEXP val)
{
    unsigned i, rank;
    SEXP dimvec;
    hid_t space, dataset;
...

    {
        ...
        if ((dataset = H5Dcreate(id, symname, tid, space, H5P_DEFAULT)) == -1)
...
static void hdf_write_string(SEXP call, hid_t fid, const char *symname, const char *str)
{
    hid_t stringtype;
    hid_t dataset;
    hid_t dataspace;
    ...
    if ((dataset = H5Dcreate(fid, symname, stringtype, dataspace, H5P_DEFAULT)) == -1)
        errorcall(call, "Unable to create dataset");

    if (H5Dwrite(dataset, stringtype, H5S_ALL, H5S_ALL, H5P_DEFAULT, str) == -1)
...
static void hdf_save_object(SEXP call, hid_t fid, const char *symname, SEXP val)
{
    int type = TYPEOF(val);
...
static void hdf_save_symbol(SEXP call, hid_t fid, SEXP sym, SEXP env)
{
    SEXP val;
    int type;
...
static void add_to_list(struct iterate_info *iinfo, const char *name, SEXP obj)
{
    PROTECT(iinfo->ret);
...
static herr_t hdf_process_object(hid_t id, const char *name, void *client_data)
{
    ...
    else if (statbuf.type == H5G_DATASET)
    {
...

        {
            ...
            obj = ((rank == 1) ? allocVector(type, dims[0]) : allocMatrix(type, dims[0], dims[1]));
...
static void add_to_symbol_table(struct iterate_info *iinfo, const char *name, SEXP obj)
{
    setVar(install((char *)name), obj, iinfo->env);
}

static void add_to_return_list(struct iterate_info *iinfo, const char *name, SEXP obj)
{
    PROTECT(iinfo->ret);
    iinfo->ret = CONS(obj, iinfo->ret);
    UNPROTECT(1);
