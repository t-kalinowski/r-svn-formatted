...
static void FixHashEntries(SEXP ht)
{
    SEXP cell;
    int count;
    for (cell = HASH_TABLE_KEYS_LIST(ht), count = 1; cell != R_NilValue; cell = CDR(cell), count++)
...
static void NewWriteItem(SEXP s, SEXP sym_list, SEXP env_list, FILE *fp, OutputRoutines *m, SaveLoadData *d)
{
    ...
    else
    {
        ...
        m->OutSpace(fp, 1, d);
        m->OutInteger(fp, LEVELS(s), d);
        m->OutSpace(fp, 1, d);
        m->OutInteger(fp, OBJECT(s), d);
        m->OutNewline(fp, d);
        switch (TYPEOF(s))
        {
...
static void NewDataSave(SEXP s, FILE *fp, OutputRoutines *m, SaveLoadData *d)
{
    ...
    m->OutInteger(fp, sym_count = HASH_TABLE_COUNT(sym_table), d);
    m->OutSpace(fp, 1, d);
    m->OutInteger(fp, env_count = HASH_TABLE_COUNT(env_table), d);
    m->OutNewline(fp, d);
    for (iterator = HASH_TABLE_KEYS_LIST(sym_table); sym_count--; iterator = CDR(iterator))
    {
        R_assert(TYPEOF(CAR(iterator)) == SYMSXP);
        m->OutString(fp, CHAR(PRINTNAME(CAR(iterator))), d);
        m->OutNewline(fp, d);
    }
    for (iterator = HASH_TABLE_KEYS_LIST(env_table); env_count--; iterator = CDR(iterator))
    {
    ...
    if (TYPEOF(ans) == VECSXP)
    {
        ...
        for (i = 0; i < LENGTH(ans); i++)
        {
            ...
            if (R_seemsOldStyleS4Object(obj))
                warningcall(R_NilValue, _("'%s' looks like a pre-2.4.0 S4 object: please recreate it"),
...
SEXP attribute_hidden do_saveToConn(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    {
        magic[2] = 'A';
        type = (ascii == NA_LOGICAL) ? R_pstream_asciihex_format : R_pstream_ascii_format;
    }
    else
    {
        if (con->text)
...
SEXP attribute_hidden do_loadFromConn2(SEXP call, SEXP op, SEXP args, SEXP env)
{
        ...
        strncmp((char *)buf, "RDB3\n", 5) == 0 || strncmp((char *)buf, "RDX3\n", 5) == 0)
    {
        ...
        if (PRIMVAL(op) == 0)
        {
            int old_InitReadItemDepth = R_InitReadItemDepth, old_ReadItemDepth = R_ReadItemDepth;
