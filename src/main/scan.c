    ...
    for (;;)
    {
        ...
        else if (bch == '\n')
        {
            ...
            if (linesread == maxlines)
                break;
            if (d->ttyflag)
                snprintf(ConsolePrompt, CONSOLE_PROMPT_SIZE, "%lld: ", (long long)(n + 1));
            nprev = n;
        }
        if (n == blocksize)
        {
...
attribute_hidden SEXP do_scan(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    begincontext(&cntxt, CTXT_CCODE, R_GlobalContext->call, R_BaseEnv, R_BaseEnv, R_NilValue, R_NilValue);
