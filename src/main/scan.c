...
static SEXP scanVector(SEXPTYPE type, int maxitems, int maxlines, int flush, SEXP stripwhite, int blskip, LocalData *d)
{
    ...
    if (type == STRSXP)
    {
        ...
        if (maxitems > 0)
            maxstring = imin2(maxitems, maxstring);
                      ...
                      int multiline, LocalData *d)
{
    ...
    if (nstring > 0)
    {
        ...
        if (maxitems > 0)
            maxstring = imin2(maxitems * nstring / nc, maxstring);
        d->hash = HashTableSetup(maxstring);
    }
...
SEXP do_scan(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (isString(sep) || isNull(sep))
    {
        ...
        errorcall(call, _("invalid '%s' value"), "sep");
    ...
    case STRSXP:
    case RAWSXP:
        ans = scanVector(TYPEOF(what), nmax, nlines, flush, stripwhite, blskip, &data);
        break;
...
SEXP do_typecvt(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    { /* not all entries are NA */
        ...
        if (res == NA_INTEGER)
            isinteger = FALSE;
        Strtod(tmp, &endp, TRUE, &data);
        if (!isBlankString(endp))
            isreal = FALSE;
        strtoc(tmp, &endp, TRUE, &data);
        if (!isBlankString(endp))
            iscomplex = FALSE;
    }
...
SEXP do_writetable(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (nr == NA_INTEGER)
        errorcall(call, _("invalid '%s' value"), "nr");
    if (nc == NA_INTEGER)
        errorcall(call, _("invalid '%s' value"), "nc");
    if (!isNull(rnames) && !isString(rnames))
        errorcall(call, _("invalid '%s' value"), "rnames");
    if (!isString(sep))
        errorcall(call, _("invalid '%s' value"), "sep");
    if (!isString(eol))
        errorcall(call, _("invalid '%s' value"), "eol");
    if (!isString(na))
        errorcall(call, _("invalid '%s' value"), "na");
    if (!isString(dec))
