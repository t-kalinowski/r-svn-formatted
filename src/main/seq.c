    ...
    if (n1 > 1)
        warningcall(call,
                    ngettext("numerical expression has %d element: only the first used",
                             "numerical expression has %d elements: only the first used", (int)n1),
                    (int)n1);
    if (n2 > 1)
        warningcall(call,
                    ngettext("numerical expression has %d element: only the first used",
                             "numerical expression has %d elements: only the first used", (int)n2),
                    (int)n2);
    n1 = asReal(s1);
    n2 = asReal(s2);
...
#define R2_SWITCH_LOOP(it)                                                                                             \
    ...
    if (TYPEOF(t) == REALSXP)
        ...
        {
            //	if ((i+1) % NINTERRUPT == 0) R_CheckUserInterrupt();
            if (ISNAN(REAL(t)[i]) || REAL(t)[i] <= -1 || REAL(t)[i] >= R_XLEN_T_MAX + 1.0)
...
SEXP attribute_hidden do_rep_int(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (!isVector(ncopy))
        error(_("invalid type (%s) for '%s' (must be a vector)"), type2char(TYPEOF(ncopy)), "times");
...
SEXP attribute_hidden do_rep_len(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    else if ((na = asInteger(len)) == NA_INTEGER || na < 0) /* na = 0 ok */
        error(_("invalid '%s' value"), "length.out");
    ...
    */
    ...
    else
    {
        ...
        if (CADR(args) == R_MissingArg)
            ...
            PROTECT(times = coerceVector(CADR(args), INTSXP));
        nprotect++;
        nt = XLENGTH(times);
        if (nt == 1)
        {
...
SEXP attribute_hidden do_seq(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    */
    ...
    args = CDR(args);
    along = CAR(args);
    Rboolean miss_from = (from == R_MissingArg), miss_to = (to == R_MissingArg);
    if (One && !miss_from)
    {
        int lf = length(from);
    ...
    if (lout == NA_INTEGER)
    {
        ...
        { // 'by' specified
            ...
            { // overflow in  (to - from)  when both are finite
                n = rto / rby - rfrom / rby;
            }
            if (!R_FINITE(n))
            {
            ...
            R_xlen_t nn;
            if ((miss_from || TYPEOF(from) == INTSXP) && (miss_to || TYPEOF(to) == INTSXP) && TYPEOF(by) == INTSXP)
            {
                int *ia, ifrom = miss_from ? (int)rfrom : asInteger(from), iby = asInteger(by);
            ...
            else
            {
                ...
                { // |from - to| is infinite, but n = (from-to)/by is not
                    rfrom /= 4.;
                    rby /= 4.;
    ...
    {                                                           // and  len := length.out  specified, >= 1
        ...
        else
        {
            ...
            if (lout > 2)
            {
                ...
                {                // del:=(from - to) is infinite, but n = del/by is not
    ...
    else if (miss_to)
    {
        ...
        if (rfrom <= INT_MAX && rfrom >= INT_MIN && rto <= INT_MAX && rto >= INT_MIN && rby == (int)rby &&
            rfrom == (int)rfrom)
        {
            ans = allocVector(INTSXP, lout);
            for (i = 0; i < lout; i++)
...
SEXP attribute_hidden do_seq_along(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (isObject(CAR(args)) && DispatchOrEval(call, length_op, "length", args, rho, &ans, 0, 1))
    {
        len = asInteger(ans);
    }
    else
