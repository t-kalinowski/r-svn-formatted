...
SEXP attribute_hidden do_rep(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    */
    PROTECT(ap = CONS(R_NilValue, list4(R_NilValue, R_NilValue, R_NilValue, R_NilValue)));
...
SEXP attribute_hidden do_seq(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    */
    PROTECT(ap = CONS(R_NilValue, CONS(R_NilValue, list4(R_NilValue, R_NilValue, R_NilValue, R_NilValue))));
    tmp = ap;
    SET_TAG(tmp, install("from"));
    tmp = CDR(tmp);
    SET_TAG(tmp, install("to"));
    tmp = CDR(tmp);
    SET_TAG(tmp, install("by"));
    tmp = CDR(tmp);
    SET_TAG(tmp, install("length.out"));
    tmp = CDR(tmp);
    SET_TAG(tmp, install("along.with"));
    tmp = CDR(tmp);
    ...
    if (lout == NA_INTEGER)
    {
        ...
        else
        {
            ...
            if (TYPEOF(from) == INTSXP && TYPEOF(to) == INTSXP && TYPEOF(by) == INTSXP)
            {
