...
OutVec(Integer, INTEGER, Integer, int) OutVec(Real, REAL, Double, double) OutVec(Complex, COMPLEX, Complex, Rcomplex)
...
InVec(Integer, INTEGER, Integer, int) InVec(Real, REAL, Double, double) InVec(Complex, COMPLEX, Complex, Rcomplex)
...
static void InitMemInPStream(R_inpstream_t stream, membuf_t mb, void *buf, int length, SEXP (*phook)(SEXP, SEXP),
                             SEXP pdata)
{
    mb->count = 0;
    mb->size = length;
...
static R_INLINE Rboolean R_EncodeComplexVector(Rcomplex *c, void *buf, int len)
{
    ...
    for (int cnt = 0; cnt < len && success; cnt++)
    {
        ...
        if (success)
            success = xdr_double(&xdrs, &(c[cnt].i));
    }
    xdr_destroy(&xdrs);
    return success;
}
...
static R_INLINE Rboolean R_DecodeComplexVector(void *input, Rcomplex *output, int len)
{
    ...
    for (int cnt = 0; cnt < len && success; cnt++)
    {
        ...
        if (success)
            success = xdr_double(&xdrs, &(output[cnt].i));
    }
    xdr_destroy(&xdrs);
    return success;
}
