...
static void UnpackFlags(int flags, SEXPTYPE *ptype, int *plevs, int *pisobj, int *phasattr, int *phastag)
{
...
static void OutRefIndex(R_outpstream_t stream, int i)
{
    if (i > MAX_PACKED_INDEX)
    {
        ...
        OutInteger(stream, PACK_REF_INDEX(i));
}
...
static R_INLINE void OutIntegerVec(R_outpstream_t stream, SEXP s, R_xlen_t length)
{
    ...
    case R_pstream_binary_format: {
        ...
        {
            this = min2(CHUNK_SIZE, length - done);
            stream->OutBytes(stream, INTEGER(s) + done, (int)(sizeof(int) * this));
        }
        break;
    }
    default:
...
static R_INLINE void OutRealVec(R_outpstream_t stream, SEXP s, R_xlen_t length)
{
    ...
    case R_pstream_binary_format: {
        ...
        {
            this = min2(CHUNK_SIZE, length - done);
            stream->OutBytes(stream, REAL(s) + done, (int)(sizeof(double) * this));
        }
        break;
    }
    default:
...
static R_INLINE void OutComplexVec(R_outpstream_t stream, SEXP s, R_xlen_t length)
{
    ...
    case R_pstream_xdr_format: {
        ...
        for (done = 0; done < length; done += this)
        {
            ...
            for (int cnt = 0; cnt < this; cnt++)
            {
                if (!xdr_double(&xdrs, &(c[done + cnt].r)) || !xdr_double(&xdrs, &(c[done + cnt].i)))
                    error(_("XDR write failed"));
            }
    ...
    case R_pstream_binary_format: {
        ...
        {
            this = min2(CHUNK_SIZE, length - done);
            stream->OutBytes(stream, COMPLEX(s) + done, (int)(sizeof(Rcomplex) * this));
        }
        break;
    }
    default:
...
tailcall:
    ...
    else if (TYPEOF(s) == ENVSXP)
    {
        ...
        if (R_IsPackageEnv(s))
        {
            ...
            warning(_("'%s' may not be available when loading"), translateChar(STRING_ELT(name, 0)));
            vmaxset(vmax);
...
static void WriteBCLang(SEXP s, SEXP ref_table, SEXP reps, R_outpstream_t stream)
{
    int type = TYPEOF(s);
...
static R_INLINE void InIntegerVec(R_inpstream_t stream, SEXP obj, R_xlen_t length)
{
    ...
    case R_pstream_binary_format: {
        ...
        {
            this = min2(CHUNK_SIZE, length - done);
            stream->InBytes(stream, INTEGER(obj) + done, (int)(sizeof(int) * this));
        }
        break;
    }
    default:
...
static R_INLINE void InRealVec(R_inpstream_t stream, SEXP obj, R_xlen_t length)
{
    ...
    case R_pstream_binary_format: {
        ...
        {
            this = min2(CHUNK_SIZE, length - done);
            stream->InBytes(stream, REAL(obj) + done, (int)(sizeof(double) * this));
        }
        break;
    }
    default:
...
static R_INLINE void InComplexVec(R_inpstream_t stream, SEXP obj, R_xlen_t length)
{
    ...
    case R_pstream_xdr_format: {
        ...
        for (done = 0; done < length; done += this)
        {
            ...
            for (R_xlen_t cnt = 0; cnt < this; cnt++)
            {
                if (!xdr_double(&xdrs, &(output[done + cnt].r)) || !xdr_double(&xdrs, &(output[done + cnt].i)))
                    error(_("XDR read failed"));
            }
            xdr_destroy(&xdrs);
        }
    ...
    case R_pstream_binary_format: {
        ...
        {
            this = min2(CHUNK_SIZE, length - done);
            stream->InBytes(stream, COMPLEX(obj) + done, (int)(sizeof(Rcomplex) * this));
        }
        break;
    }
    default:
...
static int TryConvertString(void *obj, const char *inp, size_t inplen, char *buf, size_t *bufleft)
{
...
static SEXP ReadChar(R_inpstream_t stream, char *buf, int length, int levs)
{
    ...
    if (!stream->nat2nat_obj && !strcmp(stream->native_encoding, R_nativeEncoding()))
    {
    ...
    if (!stream->nat2utf8_obj)
    {
        ...
        {
            /* very unlikely */
            warning(_("unsupported conversion from '%s' to '%s'"), from, "UTF-8");
...
static SEXP ReadItem(SEXP ref_table, R_inpstream_t stream)
{
    ...
    case ENVSXP: {
        ...
        R_ReadItemDepth--;
        if (ATTRIB(s) != R_NilValue && getAttrib(s, R_ClassSymbol) != R_NilValue)
...
static SEXP ReadBCLang(int type, SEXP ref_table, SEXP reps, R_inpstream_t stream)
{
    switch (type)
    {
    case BCREPREF:
    ...
    case ATTRLISTSXP: {
        ...
        R_ReadItemDepth--;
        SETCAR(ans, ReadBCLang(InInteger(stream), ref_table, reps, stream));
        SETCDR(ans, ReadBCLang(InInteger(stream), ref_table, reps, stream));
        UNPROTECT(1);
        return ans;
    }
    default: {
...
void R_InitConnOutPStream(R_outpstream_t stream, Rconnection con, R_pstream_format_t type, int version,
                          SEXP (*phook)(SEXP, SEXP), SEXP pdata)
{
    CheckOutConn(con);
...
void R_InitConnInPStream(R_inpstream_t stream, Rconnection con, R_pstream_format_t type, SEXP (*phook)(SEXP, SEXP),
                         SEXP pdata)
{
    CheckInConn(con);
    if (con->text)
    ...
    R_InitInPStream(stream, (R_pstream_data_t)con, type, InCharConn, InBytesConn, phook, pdata);
}
...
static void InitBConOutPStream(R_outpstream_t stream, bconbuf_t bb, Rconnection con, R_pstream_format_t type,
                               int version, SEXP (*phook)(SEXP, SEXP), SEXP pdata)
{
    bb->count = 0;
    bb->con = con;
...
static SEXP R_serializeb(SEXP object, SEXP icon, SEXP xdr, SEXP Sversion, SEXP fun)
{
    ...
    InitBConOutPStream(&out, &bbs, con, asLogical(xdr) ? R_pstream_xdr_format : R_pstream_binary_format, version, hook,
                       fun);
    R_Serialize(object, &out);
...
static void InitMemInPStream(R_inpstream_t stream, membuf_t mb, void *buf, R_size_t length, SEXP (*phook)(SEXP, SEXP),
                             SEXP pdata)
{
    mb->count = 0;
    mb->size = length;
                              ...
                              SEXP (*phook)(SEXP, SEXP), SEXP pdata)
{
    ...
    mb->size = 0;
    mb->buf = NULL;
    R_InitOutPStream(stream, (R_pstream_data_t)mb, type, version, OutCharMem, OutBytesMem, phook, pdata);
}
...
SEXP attribute_hidden do_lazyLoadDBfetch(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (err)
        error("lazy-load database '%s' is corrupt", translateChar(STRING_ELT(file, 0)));
