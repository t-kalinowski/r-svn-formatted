...
tailcall:
    ...
    else if (TYPEOF(s) == ENVSXP)
    {
        ...
        if (R_IsPackageEnv(s))
        {
            ...
            warning("%s may not be available when loading", CHAR(STRING_ELT(name, 0)));
    ...
    {
        int flags = PackFlags(TYPEOF(s), LEVELS(s), OBJECT(s), ATTRIB(s) != R_NilValue, TAG(s) != R_NilValue);
...
static SEXP appendStringToFile(SEXP file, SEXP string)
{
    ...
    if (out != len)
        error("write failed");
    if (pos == -1)
        error("could not determine file position");
...
SEXP R_lazyLoadDBinsertValue(SEXP value, SEXP file, SEXP ascii, SEXP compsxp, SEXP hook)
{
    PROTECT_INDEX vpi;
