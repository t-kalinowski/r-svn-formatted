...
tailcall:
    ...
    else if (TYPEOF(s) == ENVSXP)
    {
        ...
        if (R_IsPackageEnv(s))
        {
            ...
            warning(_("'%s' may not be available when loading"), CHAR(STRING_ELT(name, 0)));
...
SEXP attribute_hidden do_serializeInfoFromConn(SEXP call, SEXP op, SEXP args, SEXP env)
{
