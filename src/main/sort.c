 ...
 */
void attribute_hidden orderVector1(int *indx, int n, SEXP key, Rboolean nalast, Rboolean decreasing)
{
...
SEXP attribute_hidden do_radixsort(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (tmp == NA_INTEGER)
            continue;
        if (tmp < 0)
            errorcall(call, _("negative value in 'x'"));
    ...
    xmax -= xmin;
    if (xmax > 100000)
        errorcall(call, _("too large a range of values in 'x'"));
    napos = off ? 0 : xmax + 1;
    off -= xmin;
