...
Rboolean isUnsorted(SEXP x, Rboolean strictly)
{
    ...
    if (n >= 2)
        ...
        case STRSXP:
            ...
            {
                for (i = 0; i + 1 < n; i++)
                    if (scmp(STRING_ELT(x, i), STRING_ELT(x, i + 1), TRUE) >= 0)
                        return TRUE;
            }
            else
            {
...
SEXP attribute_hidden do_isunsorted(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    {
        UNPROTECT(1);
        return (xlength(x) < 2) ? ScalarLogical(FALSE) : ScalarLogical(isUnsorted(x, strictly));
    }
    if (isObject(x))
    {
...
static int greater(R_xlen_t i, R_xlen_t j, SEXP x, Rboolean nalast, Rboolean decreasing, SEXP rho)
{
    int c = -1;
...
static int listgreater(int i, int j, SEXP key, Rboolean nalast, Rboolean decreasing)
{
    SEXP x;
    int c = -1;
...
#define GREATER_2_SUB_DEF(FNAME, TYPE_1, TYPE_2, CMP_FN_1, CMP_FN_2)                                                   \
    static int FNAME(int i, int j, TYPE_1 *x, TYPE_2 *y, Rboolean nalast, Rboolean decreasing)                         \
    {                                                                                                                  \
...
#ifdef LONG_VECTOR_SUPPORT
static int listgreaterl(R_xlen_t i, R_xlen_t j, SEXP key, Rboolean nalast, Rboolean decreasing)
{
    SEXP x;
    int c = -1;
...
static void orderVectorl(R_xlen_t *indx, R_xlen_t n, SEXP key, Rboolean nalast, Rboolean decreasing,
...
#define ORD_2_BODY(FNAME, TYPE_1, TYPE_2, GREATER_2_SUB)                                                               \
    ...
    {                                                                                                                  \
        ...
        for (int h = sincs[t]; t < 16; h = sincs[++t])                                                                 \
            ...
            {                                                                                                          \
                int itmp = indx[i], j = i;                                                                             \
                while (j >= h && GREATER_2_SUB(indx[j - h], itmp, x, y, nalast ^ decreasing, decreasing))              \
                {                                                                                                      \
                    indx[j] = indx[j - h];                                                                             \
                    j -= h;                                                                                            \
                }                                                                                                      \
...
#ifdef LONG_VECTOR_SUPPORT
static void orderVector1l(R_xlen_t *indx, R_xlen_t n, SEXP key, Rboolean nalast, Rboolean decreasing, SEXP rho)
{
    ...
    if (n != 0)
    {
        if (narg == 1)
        {
            ...
            if (n > INT_MAX)
            {
                ...
                for (R_xlen_t i = 0; i < n; i++)
                    in[i] = i;
                orderVector1l(in, n, CAR(args), nalast, decreasing, R_NilValue);
                for (R_xlen_t i = 0; i < n; i++)
                    REAL(ans)[i] = in[i] + 1;
            }
            else
#endif
            {
                ...
                for (R_xlen_t i = 0; i < n; i++)
                    INTEGER(ans)[i] = (int)i;
                orderVector1(INTEGER(ans), (int)n, CAR(args), nalast, decreasing, R_NilValue);
                for (R_xlen_t i = 0; i < n; i++)
                    INTEGER(ans)[i]++;
            }
        }
        else
        {
            ...
            if (n > INT_MAX)
            {
                ...
                for (R_xlen_t i = 0; i < n; i++)
                    in[i] = i;
                orderVectorl(in, n, CAR(args), nalast, decreasing, listgreaterl);
                for (R_xlen_t i = 0; i < n; i++)
                    REAL(ans)[i] = in[i] + 1;
            }
            else
#endif
            {
                ...
                for (R_xlen_t i = 0; i < n; i++)
                    INTEGER(ans)[i] = (int)i;
                orderVector(INTEGER(ans), (int)n, args, nalast, decreasing, listgreater);
                for (R_xlen_t i = 0; i < n; i++)
                    INTEGER(ans)[i]++;
            }
        }
...
SEXP attribute_hidden do_rank(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (n > 0)
    {
        ...
        if (isLong)
        {
            ...
            {
                j = i;
                while ((j < n - 1) && equal(in[j], in[j + 1], x, TRUE, rho))
                    j++;
                switch (ties_kind)
                {
                case AVERAGE:
...
SEXP attribute_hidden do_radixsort(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (isLong)
    {
        if (decreasing)
            ...
            {
                tmp = INTEGER(x)[i];
                REAL(ans)[n - (cnts[(tmp == NA_INTEGER) ? napos : off + tmp]--)] = (double)(i + 1);
            }
        else
            for (i = n - 1; i >= 0; i--)
            {
                tmp = INTEGER(x)[i];
                REAL(ans)[--cnts[(tmp == NA_INTEGER) ? napos : off + tmp]] = (double)(i + 1);
            }
    }
    else
#endif
    {
        if (decreasing)
            ...
            {
                tmp = INTEGER(x)[i];
                INTEGER(ans)[n - (cnts[(tmp == NA_INTEGER) ? napos : off + tmp]--)] = (int)(i + 1);
            }
        else
            for (i = n - 1; i >= 0; i--)
            {
                tmp = INTEGER(x)[i];
                INTEGER(ans)[--cnts[(tmp == NA_INTEGER) ? napos : off + tmp]] = (int)(i + 1);
            }
    }
    UNPROTECT(1);
    return ans;
...
SEXP attribute_hidden do_xtfrm(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (DispatchOrEval(call, op, "xtfrm", args, rho, &ans, 0, 1))
        return ans;
