...
void attribute_hidden parseError(SEXP call, int linenum)
{
    ...
    if (linenum)
    {
        ...
        {
        case 0:
            error(_("%s%d:%d: %s"), filename, linenum, R_ParseErrorCol, R_ParseErrorMsg);
            break;
        case 1:
            error(_("%s%d:%d: %s\n%d: %s"), filename, linenum, R_ParseErrorCol, R_ParseErrorMsg, R_ParseContextLine,
                  CHAR(STRING_ELT(context, 0)));
            break;
        default:
            error(_("%s%d:%d: %s\n%d: %s\n%d: %s"), filename, linenum, R_ParseErrorCol, R_ParseErrorMsg,
...
SEXP attribute_hidden do_parse(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    Rconnection con;
    Rboolean wasopen, old_latin1 = known_to_be_latin1, old_utf8 = known_to_be_utf8, allKnown = TRUE;
    int ifile, num, i;
    const char *encoding;
    ParseStatus status;
    ...
    if (length(text) > 0)
    {
        ...
        */
        for (i = 0; i < length(text); i++)
            if (!ENC_KNOWN(STRING_ELT(text, i)) && !strIsASCII(CHAR(STRING_ELT(text, i))))
            {
                allKnown = FALSE;
                break;
            }
