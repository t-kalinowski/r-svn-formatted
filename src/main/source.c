...
void attribute_hidden parseError(SEXP call, int linenum)
{
    ...
    if (linenum)
    {
        ...
        {
        case 0:
            errorcall(call, _("syntax error on line %d"), linenum);
            break;
        case 1:
            errorcall(call, _("syntax error at\n%d: %s"), linenum, CHAR(STRING_ELT(context, 0)));
            break;
        default:
            errorcall(call, _("syntax error at\n%d: %s\n%d: %s"), linenum - 1, CHAR(STRING_ELT(context, len - 2)),
    ...
    else
    {
        ...
        {
        case 0:
            errorcall(call, _("syntax error"), R_ParseError);
            break;
        case 1:
            errorcall(call, _("syntax error in \"%s\""), CHAR(STRING_ELT(context, 0)));
            break;
        default:
            errorcall(call, _("syntax error in:\n\"%s\n%s\""), CHAR(STRING_ELT(context, len - 2)),
