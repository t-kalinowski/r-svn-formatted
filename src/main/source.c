...
void NORET parseError(SEXP call, int linenum)
{
    ...
    if (linenum)
    {
        ...
        {
        case 0:
            error("%s%d:%d: %s", filename, linenum, R_ParseErrorCol, R_ParseErrorMsg);
            break;
...
SEXP attribute_hidden do_parse(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    args = CDR(args);
    SEXP prompt = CAR(args);
    args = CDR(args);
    SEXP source = CAR(args);
    args = CDR(args);
