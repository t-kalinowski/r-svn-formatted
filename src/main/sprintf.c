                    ...
                    */
                    ...
                    case INTSXP: {
                        ...
                        if (strcspn(fmt, "dix") >= strlen(fmt))
                            error("%s", _("use format %d, %i or %x for integer objects"));
                        if (x == NA_INTEGER)
                        {
                            fmt[strlen(fmt) - 1] = 's';
                            sprintf(bit, fmt, "NA");
                    ...
                    case STRSXP:
                        ...
                        if (strlen(CHAR(STRING_ELT(this, ns % lens[nthis]))) > MAXLINE)
                            warning(_("Likely truncation of character string"));
                        snprintf(bit, MAXLINE, fmt, CHAR(STRING_ELT(this, ns % lens[nthis])));
                        bit[MAXLINE] = '\0';
                        break;
                    default:
