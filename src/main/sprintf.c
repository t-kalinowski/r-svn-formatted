...
SEXP attribute_hidden do_sprintf(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    char fmt[MAXLINE + 1], fmt2[MAXLINE + 1], *fmtp, bit[MAXLINE + 1], outputString[MAXLINE + 1];
    size_t n, cur, chunk;
    ...
    for (ns = 0; ns < maxlen; ns++)
    {
        ...
        if (n > MAXLINE)
            errorcall(call, _("'fmt' length exceeds maximal buffer length %d"), MAXLINE);
            ...
            { /* handle special format command */
                ...
                else
                {
                    ...
                    { /* handle * format if present */
                        ...
                        if (strchr(starc + 1, '*'))
                            errorcall(call,
                                      _("at most one asterisk `*' is supported in each conversion specification"));
                    ...
                    else
                    {
                        ...
                        case 's':
                            ...
                            {
                                PROTECT(tmp = lang2(install("as.character"), this));
                                this = eval(tmp, env);
                                UNPROTECT(1);
                            }
                            break;
                        ...
                        case STRSXP:
                            ...
                            if (strlen(CHAR(STRING_ELT(this, ns % thislen))) > MAXLINE)
                                warning(_("Likely truncation of character string"));
                            snprintf(bit, MAXLINE, fmtp, CHAR(STRING_ELT(this, ns % thislen)));
                            bit[MAXLINE] = '\0';
                            break;
