...
static SEXP EnlargeVector(SEXP x, R_xlen_t newlen)
{
    ...
    if (LOGICAL(GetOption1(R_CheckBoundsSymbol))[0])
        warning(_("assignment outside vector/list limits (extending from %d to %d)"), len, newlen);
...
static int SubassignTypeFix(SEXP *x, SEXP *y, R_xlen_t stretch, int level, SEXP call, SEXP rho)
{
    ...
    default:
        error(_("incompatible types (from %s to %s) in subassignment type fix"), type2char(which % 100),
              type2char(which / 100));
    }
...
static SEXP VectorAssign(SEXP call, SEXP rho, SEXP x, SEXP s, SEXP y)
{
    ...
    { /* pretest to speed up simple case */
        ...
        if (isMatrix(s) && isArray(x) && ncols(s) == length(dim))
        {
            ...
            { // not using call: when coming from string case
    ...
    R_xlen_t ny = xlength(y), nx = xlength(x), iny;
...
static SEXP MatrixAssign(SEXP call, SEXP rho, SEXP x, SEXP s, SEXP y)
{
    ...
    default:
        error(_("incompatible types (from %s to %s) in matrix subset assignment"), type2char(which % 100),
              type2char(which / 100));
    }
    UNPROTECT(2);
    return x;
...
static SEXP ArrayAssign(SEXP call, SEXP rho, SEXP x, SEXP s, SEXP y)
{
    ...
    default:
        error(_("incompatible types (from %s to %s) in array subset assignment"), type2char(which % 100),
              type2char(which / 100));
    }
...
static SEXP SimpleListAssign(SEXP call, SEXP x, SEXP s, SEXP y, int ind, Rboolean check_cycles)
{
    SEXP indx, sub = CAR(s);
    int ii, n, nx;
    R_xlen_t stretch = 1;
...
SEXP attribute_hidden do_subassign2_dflt(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (isVector(x))
    {
        ...
        else
        {
            ...
            for (i = 0; i < ndims; i++)
            {
                ...
                pindx[i] = (int)get1index(thesub, isNull(names) ? R_NilValue : VECTOR_ELT(names, i), pdims[i],
        ...
        default:
            error(_("incompatible types (from %s to %s) in [[ assignment"), type2char(which % 100),
                  type2char(which / 100));
        }
    ...
    else if (isPairList(x))
    {
        ...
        else
        {
            ...
            {
                SEXP thesub = CAR(subs);
                pindx[i] = (int)get1index(thesub, VECTOR_ELT(names, i), pdims[i],
                                          /*partial ok*/ FALSE, -1, call);
                subs = CDR(subs);
                if (pindx[i] < 0 || pindx[i] >= pdims[i])
                    errorOutOfBoundsSEXP(x, i, thesub);
