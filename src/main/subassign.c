...
static int SubassignTypeFix(SEXP *x, SEXP *y, int stretch, int level, SEXP call)
{
    ...
    default:
        errorcall(call, _("incompatible types (from %s to %s) in subassignment type fix"), CHAR(type2str(which % 100)),
                  CHAR(type2str(which / 100)));
    }
...
static SEXP MatrixAssign(SEXP call, SEXP x, SEXP s, SEXP y)
{
    ...
    default:
        error(_("incompatible types (from %s to %s) in matrix subset assignment"), CHAR(type2str(which % 100)),
              CHAR(type2str(which / 100)));
    }
    UNPROTECT(2);
    return x;
...
static SEXP ArrayAssign(SEXP call, SEXP x, SEXP s, SEXP y)
{
    ...
    for (i = 0; i < k; i++)
    {
        SETCAR(tmp, arraySubscript(i, CAR(tmp), dims, getAttrib, (STRING_ELT), x));
        tmp = CDR(tmp);
    }
    ...
    for (i = 0; i < n; i++)
    {
        ...
        default:
            error(_("incompatible types (from %s to %s) in array subset assignment"), CHAR(type2str(which % 100)),
                  CHAR(type2str(which / 100)));
        }
    next_i:;
        if (n > 1)
...
SEXP attribute_hidden do_subassign2_dflt(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (isVector(x))
    {
        ...
        default:
            error(_("incompatible types (from %s to %s) in [[ assignment"), CHAR(type2str(which % 100)),
                  CHAR(type2str(which / 100)));
        }
