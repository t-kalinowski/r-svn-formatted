...
static int SubassignTypeFix(SEXP *x, SEXP *y, int stretch, int level, SEXP call)
{
    ...
    default:
        errorcall(call, _("incompatible types (%d) in subassignment type fix"), which);
    }
...
static SEXP MatrixAssign(SEXP call, SEXP x, SEXP s, SEXP y)
{
    ...
    case 1919: /* vector <- vector */
        ...
        break;
    default:
        error(_("incompatible types (case %d) in matrix subset assignment"), which);
    }
    UNPROTECT(2);
    return x;
}
...
static SEXP ArrayAssign(SEXP call, SEXP x, SEXP s, SEXP y)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        default:
            error(_("incompatible types (%d) in array subset assignment"), which);
        }
    next_i:;
        if (n > 1)
        {
