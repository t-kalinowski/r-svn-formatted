...
R_xlen_t attribute_hidden OneIndex(SEXP x, SEXP s, R_xlen_t len, int partial, SEXP *newname, int pos, SEXP call)
{
    SEXP names;
    R_xlen_t i, indx, nx;
    const void *vmax;
...
SEXP attribute_hidden vectorIndex(SEXP x, SEXP thesub, int start, int stop, int pok, SEXP call, Rboolean dup)
{
    ...
    for (i = start; i < stop; i++)
    {
        ...
        offset = get1index(thesub, getAttrib(x, R_NamesSymbol), xlength(x), pok, i, call);
...
static SEXP logicalSubscript(SEXP s, R_xlen_t ns, R_xlen_t nx, R_xlen_t *stretch, SEXP call)
{
    ...
    *stretch = (ns > nx) ? ns : 0;
    ...
    if (nmax > R_SHORT_LEN_MAX)
    {
        ...
        if (i1 > 0)
        { /* last recycling chunk is incomple -
     we have to get the truncated count as well */
        ...
        { /* no recycling - use fast single-index code */
            R_ITERATE_CHECK(
                NINTERRUPT, nmax, i, if (LOGICAL(s)[i]) {
            ...
            MOD_ITERATE_CHECK(
                NINTERRUPT, nmax, ns, nmax, i, i1, i2, if (LOGICAL(s)[i1]) {
