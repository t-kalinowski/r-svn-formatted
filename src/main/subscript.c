...
R_xlen_t attribute_hidden OneIndex(SEXP x, SEXP s, R_xlen_t nx, int partial, SEXP *newname, int pos, SEXP call)
{
    SEXP names;
    R_xlen_t i, indx;
    const void *vmax;
    ...
    *newname = R_NilValue;
    ...
    case SYMSXP:
        ...
        if (names != R_NilValue)
        {
            PROTECT(names);
            for (i = 0; i < nx; i++)
                if (streql(translateChar(STRING_ELT(names, i)), translateChar(PRINTNAME(s))))
                {
                    indx = i;
                    break;
                }
...
R_xlen_t attribute_hidden get1index(SEXP s, SEXP names, R_xlen_t len, int pok, int pos, SEXP call)
{
                ...
                if (STRING_ELT(names, i) != NA_STRING)
                {
                    ...
                    if (!strncmp(cur_name, ss, len))
                    {
                        ...
                        { /* first one */
                            ...
                            {
                                if (call == R_NilValue)
                                    warning(_("partial match of '%s' to '%s'"), ss, cur_name);
                                else
                                    warningcall(call, _("partial match of '%s' to '%s'"), ss, cur_name);
                            }
                        }
                        else
                        {
                            ...
                            if (warn_pok) /* already given context */
                                warningcall(R_NilValue, _("further partial match of '%s' to '%s'"), ss, cur_name);
                            break;
                        }
                    }
                }
 ...
 */
attribute_hidden SEXP vectorIndex(SEXP x, SEXP thesub, int start, int stop, int pok, SEXP call, Rboolean dup)
{
    int i;
    R_xlen_t offset;
    SEXP cx;
...
#else
        ECALL4(call, _("%d x %d character matrix subscript illegal for NULL dimnames"), nr, nc);
#endif
    }
...
static SEXP logicalSubscript(SEXP s, R_xlen_t ns, R_xlen_t nx, R_xlen_t *stretch, SEXP call)
{
    ...
    *stretch = (ns > nx) ? ns : 0;
    ...
    {
        if (ns == nmax)
        { /* no recycling - use fast single-index code */
            const void *vmax = vmaxget();
            double *buf = (double *)R_alloc(nmax, sizeof(double));
            count = 0;
            R_ITERATE_CHECK(
                NINTERRUPT, nmax, i, if (ps[i]) {
                    if (ps[i] == NA_LOGICAL)
                        buf[count++] = NA_REAL;
                    else
        ...
        if (i1 > 0)
        { /* last recycling chunk is incomplete -
     we have to get the truncated count as well */
        ...
        { /* nested recycling, total is sufficient */
            for (i = 0; i < ns; i++)
                if (ps[i])
                    count++;
            count *= nmax / ns;
        ...
        count = 0;
        MOD_ITERATE_CHECK(
            NINTERRUPT, nmax, ns, nmax, i, i1, i2, if (ps[i1]) {
                if (ps[i1] == NA_LOGICAL)
                    pindx[count++] = NA_REAL;
                else
...
attribute_hidden SEXP makeSubscript(SEXP x, SEXP s, R_xlen_t *stretch, SEXP call)
{
