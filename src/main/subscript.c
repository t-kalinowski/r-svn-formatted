    ...
    *newname = R_NilValue;
    ...
    case STRSXP:
        ...
        if (names != R_NilValue)
        {
            /* Try for exact match */
            for (i = 0; i < nx; i++)
                if (streql(CHAR(STRING_ELT(names, i)), CHAR(STRING_ELT(s, pos))))
                {
                    indx = i;
                    break;
                }
            ...
            if (partial && indx < 0)
            {
                ...
                for (i = 0; i < nx; i++)
                {
                    if (!strncmp(CHAR(STRING_ELT(names, i)), CHAR(STRING_ELT(s, pos)), len))
                    {
                        if (indx == -1)
                            indx = i;
                        else
    ...
    */
    ...
    case STRSXP:
        ...
        for (i = 0; i < length(names); i++)
            if (STRING_ELT(names, i) == NA_STRING || STRING_ELT(s, pos) == NA_STRING)
            {
                /* NA matches nothing */
            }
            else
    ...
    case SYMSXP:
        for (i = 0; i < length(names); i++)
            if (STRING_ELT(names, i) != NA_STRING && streql(CHAR(STRING_ELT(names, i)), CHAR(PRINTNAME(s))))
            {
                indx = i;
                break;
            }
...
SEXP attribute_hidden mat2indsub(SEXP dims, SEXP s)
{
    ...
    for (i = 0; i < nrs; i++)
    {
        ...
        for (j = 0; j < LENGTH(dims); j++)
        {
            ...
            if (k < 0)
                error(_("negative values are not allowed in a matrix subscript"));
            if (k == 0)
            {
                INTEGER(rvec)[i] = -1;
                break;
    ...
    *stretch = 0;
    ...
    if (min < 0)
    {
        if (max == 0 && !isna)
            ...
            error(_("only 0's may be mixed with negative subscripts"));
    }
    else
...
SEXP makeSubscript(SEXP x, SEXP s, int *stretch)
{
    ...
    if (isVector(x) || isList(x) || isLanguage(x))
    {
        ...
        error(_("subscripting on non-vector"));
    return ans;
}
...
static SEXP int_vectorSubscript(int nx, SEXP s, int *stretch, AttrGetter dng, StringEltGetter strg, SEXP x, Rboolean in)
{
    int ns;
