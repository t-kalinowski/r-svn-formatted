...
SEXP attribute_hidden ExtractSubset(SEXP x, SEXP indx, SEXP call)
{
    ...
    {
    case LGLSXP:
        EXTRACT_SUBSET_LOOP(LOGICAL0(result)[i] = LOGICAL_ELT(x, ii), LOGICAL0(result)[i] = NA_INTEGER);
        break;
    case INTSXP:
        EXTRACT_SUBSET_LOOP(INTEGER0(result)[i] = INTEGER_ELT(x, ii), INTEGER0(result)[i] = NA_INTEGER);
        break;
    case REALSXP:
        EXTRACT_SUBSET_LOOP(REAL0(result)[i] = REAL_ELT(x, ii), REAL0(result)[i] = NA_REAL);
        break;
    case CPLXSXP: {
        Rcomplex NA_CPLX = {NA_REAL, NA_REAL};
        EXTRACT_SUBSET_LOOP(COMPLEX0(result)[i] = COMPLEX_ELT(x, ii), COMPLEX0(result)[i] = NA_CPLX);
    }
    break;
    case STRSXP:
        EXTRACT_SUBSET_LOOP(SET_STRING_ELT(result, i, STRING_ELT(x, ii)), SET_STRING_ELT(result, i, NA_STRING));
        break;
    case VECSXP:
    case EXPRSXP:
        EXTRACT_SUBSET_LOOP(SET_VECTOR_ELT(result, i, VECTOR_ELT_FIX_NAMED(x, ii)),
                            SET_VECTOR_ELT(result, i, R_NilValue));
        break;
    case RAWSXP:
        EXTRACT_SUBSET_LOOP(RAW0(result)[i] = RAW_ELT(x, ii), RAW0(result)[i] = (Rbyte)0);
        break;
    case LISTSXP:
...
static SEXP VectorSubset(SEXP x, SEXP s, SEXP call)
{
    ...
    { /* pretest to speed up simple case */
...
static SEXP MatrixSubset(SEXP x, SEXP s, SEXP call, int drop)
{
    ...
    {
    case LGLSXP:
        MATRIX_SUBSET_LOOP(LOGICAL0(result)[ij] = LOGICAL_ELT(x, iijj), LOGICAL0(result)[ij] = NA_LOGICAL);
        break;
    case INTSXP:
        MATRIX_SUBSET_LOOP(INTEGER0(result)[ij] = INTEGER_ELT(x, iijj), INTEGER0(result)[ij] = NA_INTEGER);
        break;
    case REALSXP:
        MATRIX_SUBSET_LOOP(REAL0(result)[ij] = REAL_ELT(x, iijj), REAL0(result)[ij] = NA_REAL);
        break;
    case CPLXSXP: {
        Rcomplex NA_CPLX = {NA_REAL, NA_REAL};
        MATRIX_SUBSET_LOOP(COMPLEX0(result)[ij] = COMPLEX_ELT(x, iijj), COMPLEX0(result)[ij] = NA_CPLX);
    }
    break;
    case STRSXP:
        MATRIX_SUBSET_LOOP(SET_STRING_ELT(result, ij, STRING_ELT(x, iijj)), SET_STRING_ELT(result, ij, NA_STRING));
        break;
    case VECSXP:
    case EXPRSXP:
        MATRIX_SUBSET_LOOP(SET_VECTOR_ELT(result, ij, VECTOR_ELT_FIX_NAMED(x, iijj)),
                           SET_VECTOR_ELT(result, ij, R_NilValue));
        break;
    case RAWSXP:
        MATRIX_SUBSET_LOOP(RAW0(result)[ij] = RAW_ELT(x, iijj), RAW0(result)[ij] = (Rbyte)0);
        break;
    default:
    ...
    if (nrs >= 0 && ncs >= 0)
    {
        ...
        if (!isNull(dimnames))
        {
            ...
            {
                SET_VECTOR_ELT(newdimnames, 0, ExtractSubset(VECTOR_ELT(dimnames, 0), sr, call));
                SET_VECTOR_ELT(newdimnames, 1, ExtractSubset(VECTOR_ELT(dimnames, 1), sc, call));
            }
            else
            {
...
#define ARRAY_SUBSET_LOOP(STDCODE, NACODE)                                                                             \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            R_xlen_t ii = findASubIndex(k, subs, indx, pxdims, offset, call);                                          \
            if (ii != NA_INTEGER)                                                                                      \
                STDCODE;                                                                                               \
            else                                                                                                       \
                NACODE;                                                                                                \
...
static SEXP ArraySubset(SEXP x, SEXP s, SEXP call, int drop)
{
    ...
    {
    case LGLSXP:
        ARRAY_SUBSET_LOOP(LOGICAL0(result)[i] = LOGICAL_ELT(x, ii), LOGICAL0(result)[i] = NA_LOGICAL);
        break;
    case INTSXP:
        ARRAY_SUBSET_LOOP(INTEGER0(result)[i] = INTEGER_ELT(x, ii), INTEGER0(result)[i] = NA_INTEGER);
        break;
    case REALSXP:
        ARRAY_SUBSET_LOOP(REAL0(result)[i] = REAL_ELT(x, ii), REAL0(result)[i] = NA_REAL);
        break;
    case CPLXSXP: {
        Rcomplex NA_CPLX = {NA_REAL, NA_REAL};
        ARRAY_SUBSET_LOOP(COMPLEX0(result)[i] = COMPLEX_ELT(x, ii), COMPLEX0(result)[i] = NA_CPLX);
    }
    break;
    case STRSXP:
        ARRAY_SUBSET_LOOP(SET_STRING_ELT(result, i, STRING_ELT(x, ii)), SET_STRING_ELT(result, i, NA_STRING));
        break;
    case VECSXP:
    case EXPRSXP:
        ARRAY_SUBSET_LOOP(SET_VECTOR_ELT(result, i, VECTOR_ELT_FIX_NAMED(x, ii)),
                          SET_VECTOR_ELT(result, i, R_NilValue));
        break;
    case RAWSXP:
        ARRAY_SUBSET_LOOP(RAW0(result)[i] = RAW_ELT(x, ii), RAW0(result)[i] = (Rbyte)0);
        break;
    default:
    ...
    if (dimnames != R_NilValue)
    {
        ...
        if (TYPEOF(dimnames) == VECSXP)
        {
            ...
            for (int i = 0; i < k; i++)
            {
                if (bound[i] > 0)
                {
                    SET_VECTOR_ELT(xdims, j++, ExtractSubset(VECTOR_ELT(dimnames, i), CAR(r), call));
                }
                else
...
static R_INLINE int R_DispatchOrEvalSP(SEXP call, SEXP op, const char *generic, SEXP args, SEXP rho, SEXP *ans)
{
    SEXP prom = NULL;
...
SEXP attribute_hidden do_subset_dflt(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    else if (cddrArgs != R_NilValue && CDR(cddrArgs) == R_NilValue && TAG(cdrArgs) == R_NilValue &&
             TAG(cddrArgs) == R_NilValue)
    {
    ...
    { /* remove probably erroneous attr's */
...
SEXP attribute_hidden do_subset2_dflt(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    { /* vector indexing */
        ...
        offset = get1index(thesub, xnames, xlength(x), pok, len > 1 ? len - 1 : -1, call);
        UNPROTECT(1); /* xnames */
...
SEXP attribute_hidden fixSubset3Args(SEXP call, SEXP args, SEXP env, SEXP *syminp)
{
    SEXP input, nlist;
    ...
    {
        errorcall(call, _("invalid subscript type '%s'"), type2char(TYPEOF(nlist)));
