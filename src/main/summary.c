    ...
    ITERATE_BY_REGION(sx, x, i, nbatch, double, REAL, {
        for (R_xlen_t k = 0; k < nbatch; k++)
        {
            ...
            { /* Na(N) */
                ...
                {
                    if (!ISNA(s))
                        s = x[k]; /* so any NA trumps all NaNs */
                    if (!updated)
                        updated = TRUE;
                }
            }
            else if (!updated || x[k] < s)
...
static Rboolean smin(SEXP x, SEXP *value, Rboolean narm)
{
    ...
    for (R_xlen_t i = 0; i < XLENGTH(x); i++)
    {
        ...
        {
            if (!updated || (s != STRING_ELT(x, i) && Scollate(s, STRING_ELT(x, i)) > 0))
            {
                s = STRING_ELT(x, i);
                if (!updated)
                    updated = TRUE;
...
attribute_hidden SEXP fixup_NaRm(SEXP args)
{
    SEXP t, na_value;
...
static R_INLINE SEXP real_mean(SEXP x)
{
    ...
    { // infinite s, maybe just overflowed; try to use smaller terms:
...
SEXP attribute_hidden do_summary(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    { /* mean */
        ...
        {
        case LGLSXP:
            return logical_mean(x);
        case INTSXP:
            return integer_mean(x);
        case REALSXP:
            return real_mean(x);
        case CPLXSXP:
            return complex_mean(x);
        default:
    ...
    if (ALTREP(CAR(args)) && CDDR(args) == R_NilValue && (CDR(args) == R_NilValue || TAG(CDR(args)) == R_NaRmSymbol))
    {
        SEXP toret = NULL;
        SEXP vec = CAR(args);
        switch (PRIMVAL(op))
    ...
    default:
        errorcall(call, _("internal error ('op = %d' in do_summary).\t Call a Guru"), iop);
        return R_NilValue; /*-Wall */
    }

    SEXP stmp = NA_STRING, scum = PROTECT(NA_STRING);
        ...
        if (xlength(a) > 0)
        {
            ...
            case 3: /* max */
                ...
                case INTSXP:
                    int_a = TRUE;
                    if (iop == 2)
                        updated = imin(a, &itmp, narm);
                    else
                        updated = imax(a, &itmp, narm);
                    break;
                case REALSXP:
                    real_a = TRUE;
                    if (ans_type == INTSXP)
                    ...
                    if (iop == 2)
                        updated = rmin(a, &tmp, narm);
                    else
                        updated = rmax(a, &tmp, narm);
                    break;
                case STRSXP:
                ...
                { /* 'a' had non-NA elements; --> "add" tmp or itmp*/
                    ...
                    if (ans_type == INTSXP)
                    {
                        ...
                        if (icum == NA_INTEGER)
                            ;                                                       /* NA trumps anything */
                        else if (itmp == NA_INTEGER || (iop == 2 && itmp < icum) || /* min */
                                 (iop == 3 && itmp > icum))                         /* max */
                            icum = itmp;
                    }
            ...
            case 0: /* sum */
                ...
                case INTSXP:
#ifdef LONG_INT
                    updated = (use_isum ? isum(a, &iLtmp, narm, call) : risum(a, &tmp, narm));
                    ...
                    else if (use_isum && updated == 42)
                    {
                        ...
                        DbgP3(" .. switching type to REAL, tmp=%g, zcum.r=%g", tmp, zcum.r);
                    }
                    else if (updated)
                    {
                        ...
                        if (ans_type == INTSXP)
                        {
                            ...
                            if (s > INT_MAX || s < R_INT_MIN || iLtmp < -LONG_INT_MAX || LONG_INT_MAX < iLtmp)
                            {
                                ans_type = REALSXP;
                                zcum.r = s;
                        ...
                        { // dealt with NA_INTEGER already above
    ...
    {
    case INTSXP:
        INTEGER(ans)[0] = (iop == 0) ? (int)iLcum : icum;
        break;
    case REALSXP:
        REAL(ans)[0] = zcum.r;
        break;
...
SEXP attribute_hidden do_first_min(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (i)
    {
        ...
        { /* preserve names */
            SEXP ansnam;
            PROTECT(ansnam = ScalarString(STRING_ELT(getAttrib(sx, R_NamesSymbol), indx)));
...
SEXP attribute_hidden do_which(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (len > R_SHORT_LEN_MAX)
    {
        ...
        if (len)
            memcpy(REAL(ans), buf, sizeof(double) * len);
    }
    else
#endif
    {
    ...
    if (v_nms != R_NilValue)
    {
        ...
        if (TYPEOF(ans) == REALSXP)
            for (i = 0; i < len; i++)
            {
                SET_STRING_ELT(ans_nms, i, STRING_ELT(v_nms, (R_xlen_t)REAL(ans)[i] - 1));
            }
        else
#endif
            for (i = 0; i < len; i++)
            {
                SET_STRING_ELT(ans_nms, i, STRING_ELT(v_nms, (R_xlen_t)INTEGER(ans)[i] - 1));
            }
    ...
    case INTSXP: {
        ...
        for (a = CDR(args); a != R_NilValue; a = CDR(a))
        {
            ...
            MOD_ITERATE1(len, n, i, i1, {
                ...
                if (PRIMVAL(op) == 1)
                {
                    if ((narm && ra[i] == NA_INTEGER) || (ra[i] != NA_INTEGER && tmp != NA_INTEGER && tmp > ra[i]) ||
                        (!narm && tmp == NA_INTEGER))
                        ra[i] = tmp;
                }
                else
    ...
    case REALSXP: {
        ...
        for (a = CDR(args); a != R_NilValue; a = CDR(a))
        {
            ...
            MOD_ITERATE1(len, n, i, i1, {
                ...
                if (PRIMVAL(op) == 1)
                {
                    if ((narm && ISNAN(ra[i])) || (!ISNAN(ra[i]) && !ISNAN(tmp) && tmp > ra[i]) ||
                        (!narm && ISNAN(tmp)))
                        ra[i] = tmp;
                }
                else
