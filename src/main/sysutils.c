        ...
        for (j = 0; j < i; j++)
        {
            ...
            else
            {
                ...
                if (known_to_be_latin1)
                    tmp = mkCharCE(s, CE_LATIN1);
                else if (known_to_be_utf8)
                    tmp = mkCharCE(s, CE_UTF8);
                else
                    tmp = mkChar(s);
...
attribute_hidden SEXP do_iconv(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    else
    {
        ...
        if (STRING_ELT(CAR(args), 0) == NA_STRING)
            ...
            sub = translateChar(STRING_ELT(CAR(args), 0));
        args = CDR(args);
        ...
        if (streql(to, "UTF-8"))
            isUTF8 = TRUE;
        if (streql(to, "latin1") || streql(to, "ISO_8859-1") || streql(to, "CP1252"))
            isLatin1 = TRUE;
        ...
        for (R_xlen_t i = 0; i < XLENGTH(x); i++)
        {
            ...
            if (!isRawlist && IS_UTF8(si) && streql(from, ""))
            {
                if (utf8_obj == (iconv_t)-1)
                {
                    ...
                    if (utf8_obj == (iconv_t)(-1))
#ifdef Win32
                        error(_("unsupported conversion from '%s' to '%s' in codepage %d"), "UTF-8", to, localeCP);
#else
                        error(_("unsupported conversion from '%s' to '%s'"), "UTF-8", to);
#endif
                }
                obj = utf8_obj;
                fromUTF8 = TRUE;
            ...
            else if (!isRawlist && IS_LATIN1(si) && streql(from, ""))
            {
                if (latin1_obj == (iconv_t)-1)
                {
                    ...
                    if (latin1_obj == (iconv_t)(-1))
#ifdef Win32
                        error(_("unsupported conversion from '%s' to '%s' in codepage %d"), "latin1", to, localeCP);
#else
                        error(_("unsupported conversion from '%s' to '%s'"), "latin1", to);
#endif
                }
                obj = latin1_obj;
            }
            else
            {
                ...
                obj = arg_obj;
                fromUTF8 = streql(from, "UTF-8") || (streql(from, "") && known_to_be_utf8);
...
#define CHECK_CHARSXP(x)                                                                                               \
    do                                                                                                                 \
    {                                                                                                                  \
        SEXP __x__ = (x);                                                                                              \
        if (TYPEOF(__x__) != CHARSXP)                                                                                  \
            error(_("'%s' must be called on a CHARSXP, but got '%s'"), __func__, type2char(TYPEOF(__x__)));            \
    } while (0);
...
static R_INLINE nttype_t needsTranslation(SEXP x)
{
    ...
    if (IS_UTF8(x))
    {
        if (utf8locale || x == NA_STRING)
            return NT_NONE;
        return NT_FROM_UTF8;
    }
    if (IS_LATIN1(x))
    {
        if (x == NA_STRING || latin1locale)
            return NT_NONE;
...
/* Translates string in "ans" to native encoding returning it in string
   buffer "cbuff". */
static int translateToNative(const char *ans, R_StringBuffer *cbuff, nttype_t ttype, int mustWork)
{
    if (ttype == NT_NONE)
...
next_char:
    ...
    *outbuf = '\0';
    if (mustWork && failed)
    {
        ...
        if (mustWork == 2)
        {
            warning(_("unable to translate '%s' to native encoding"), err_buff);
            return 1;
        }
        else
        {
...
{
    if (IS_UTF8(x) || IS_ASCII(x) || x == NA_STRING)
        return NT_NONE;
    if (IS_BYTES(x))
        error(_("translating strings with \"bytes\" encoding is not allowed"));
    if (IS_LATIN1(x) || latin1locale)
        return NT_FROM_LATIN1;
    if (utf8locale)
        return NT_NONE;
...
/* Translates string in "ans" to UTF-8 returning it in string
   buffer "cbuff". */
static int translateToUTF8(const char *ans, R_StringBuffer *cbuff, nttype_t ttype, int mustWork)
{
    if (ttype == NT_NONE)
        error(_("internal error: no translation needed"));
...
next_char:
    ...
    *outbuf = '\0';
    ...
    if (mustWork && failed)
    {
        ...
        if (mustWork == 2)
        {
            warning(_("unable to translate '%s' to UTF-8"), err_buff);
            vmaxset(vmax);
            return 1;
        }
        else
...
/* This may return a R_alloc-ed result, so the caller has to manage the
   R_alloc stack */
static int translateToWchar(const char *ans, R_StringBuffer *cbuff, nttype_t ttype, int mustWork)
{
    void *obj;
    const char *inbuf, *from;
    char *outbuf;
    ...
    { /* t == NT_FROM_NATIVE */
        ...
        if (obj == (void *)(-1))
#ifdef Win32
            error(_("unsupported conversion to '%s' from codepage %d"), TO_WCHAR, localeCP);
#else
...
next_char:
    ...
    *((wchar_t *)outbuf) = L'\0'; /* terminate wide string */
    ...
    if (mustWork && failed)
    {
        ...
        if (mustWork == 2)
        {
            warning(_("unable to translate '%s' to a wide string"), err_buff);
            vmaxset(vmax);
            return 1;
        }
        else
        {
            R_FreeStringBuffer(cbuff);
            error(_("unable to translate '%s' to a wide string"), err_buff);
        }
        vmaxset(vmax);
    }
    return 0;
...
static int reEncodeIconv(const char *x, R_StringBuffer *cbuff, const char *fromcode, const char *tocode, int subst)
{
    void *obj;
    const char *inbuf;
    char *outbuf;
...
static int reEncode(const char *x, R_StringBuffer *cbuff, cetype_t ce_in, cetype_t ce_out, int subst)
{
...
{
    R_StringBuffer cbuff = {NULL, 0, MAXELTSIZE};
    if (reEncodeIconv(x, &cbuff, fromcode, tocode, subst))
        return x;
    size_t res = strlen(cbuff.data) + 1;
    char *p = R_alloc(res, 1);
    memcpy(p, cbuff.data, res);
...
size_t ucstoutf8(char *s, const unsigned int wc)
{
    ...
    if (ucsutf8_obj == NULL)
    {
        ...
        {
            error(_("unsupported conversion from '%s' to '%s'"), UNICODE, "UTF-8");
            return (size_t)(-1);
        }
        ucsutf8_obj = cd;
    }
...
int attribute_hidden R_is_redirection_tty(int fd)
{
    ...
    if (gfibh(h, FileNameInfo, fnInfo, size))
        ...
        res = ((wcsstr(fnInfo->FileName, L"msys-") || wcsstr(fnInfo->FileName, L"cygwin-")) &&
