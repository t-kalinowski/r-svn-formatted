...
static void iconv_Init(void)
{
    ...
    snprintf(dllpath, PATH_MAX, "%s%smodules%s%s%s", getenv("R_HOME"), FILESEP, FILESEP, "iconv", SHLIB_EXT);
    if (!initialized)
    {
...
const char *translateChar(SEXP x)
{
    ...
    if (x == NA_STRING || !(IS_LATIN1(x) || IS_UTF8(x)))
        return ans;
...
size_t Riconv(void *cd, const char *domain = "", const char **inbuf, size_t *inbytesleft, char **outbuf,
              size_t *outbytesleft)
{
    error(_("'iconv' is not available on this system"));
    return 0;
