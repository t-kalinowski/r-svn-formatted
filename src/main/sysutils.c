...
static Rboolean isASCIIAlias(const char *code)
{
    ...
    static char *names[] = {"ASCII",   "US-ASCII", "ANSI_X3.4-1968", "ANSI_X3.4-1986",   "CP367",
                            "IBM367",  "ISO-IR-6", "ISO646-US",      "ISO_646.IRV:1991", "US",
                            "CSASCII", NULL};
...
#endif
    if (!*tocode && !*fromcode)
        return iconv_open_trans(cp, cp);
    if (!*tocode)
        return iconv_open_trans(cp, fromcode);
    else if (!*fromcode)
        return iconv_open_trans(tocode, cp);
    else
        return iconv_open_trans(tocode, fromcode);
#else
...
/* Translates string in "ans" to native encoding returning it as string
   buffer "cbuff" */
static void translateToNative(const char *ans, R_StringBuffer *cbuff, nttype_t ttype)
{
