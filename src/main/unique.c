...
SEXP match5(SEXP itable, SEXP ix, int nmatch, SEXP incomp, SEXP env)
{
    ...
    if (LENGTH(x) == 1 && !incomp)
    {
        ...
        case LGLSXP:
        case INTSXP: {
            int x_val = INTEGER(x)[0], *table_p = INTEGER(table);
            for (int i = 0; i < LENGTH(itable); i++)
                if (table_p[i] == x_val)
                {
                    INTEGER(ans)[0] = i + 1;
        ...
        case REALSXP: {
            double x_val = REAL(x)[0], *table_p = REAL(table);
            for (int i = 0; i < LENGTH(itable); i++)
                if (table_p[i] == x_val)
                {
                    INTEGER(ans)[0] = i + 1;
        ...
        case CPLXSXP: {
            Rcomplex x_val = COMPLEX(x)[0], *table_p = COMPLEX(table);
            for (int i = 0; i < LENGTH(itable); i++)
        ...
        case RAWSXP: {
            Rbyte x_val = RAW(x)[0], *table_p = RAW(table);
            for (int i = 0; i < LENGTH(itable); i++)
                if (table_p[i] == x_val)
                {
                    INTEGER(ans)[0] = i + 1;
