...
{
    if (!NAN(COMPLEX(x)[i].r) && !NAN(COMPLEX(x)[i].i) && !NAN(COMPLEX(y)[j].r) && !NAN(COMPLEX(y)[j].i))
    {
        return COMPLEX(x)[i].r == COMPLEX(y)[j].r && COMPLEX(x)[i].i == COMPLEX(y)[j].i;
    }
    else if ((NAN(COMPLEX(x)[i].r) || NAN(COMPLEX(x)[i].i)) && (NAN(COMPLEX(y)[j].r) || NAN(COMPLEX(y)[j].i)))
    {
        return 1;
    }
    return 0;
