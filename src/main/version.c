...
void PrintVersionString(char *s)
{
    ...
    {
        sprintf(s, "Version %s.%s %s (%s-%s-%s)", R_MAJOR, R_MINOR, R_STATUS, R_YEAR, R_MONTH, R_DAY);
    }
    else
    {
        if (strlen(R_STATUS) == 0)
        {
            sprintf(s, "Version %s.%s (%s-%s-%s)", R_MAJOR, R_MINOR, R_YEAR, R_MONTH, R_DAY);
        }
        else
        {
            sprintf(s, "Version %s.%s %s (%s-%s-%s r%s)", R_MAJOR, R_MINOR, R_STATUS, R_YEAR, R_MONTH, R_DAY,
                    R_SVN_REVISION);
        }
    }
}
