...
static void vfonts_Init(void)
{
    ...
    if (!(ptr_GVStrWidth = (dblDL_FUNC)R_FindSymbol("Rvf_GVStrWidth", "vfonts", NULL)))
        return;
    if (!(ptr_GVStrHeight = (dblDL_FUNC)R_FindSymbol("Rvf_GVStrHeight", "vfonts", NULL)))
        return;
    if (!(ptr_GVText = R_FindSymbol("Rvf_GVText", "vfonts", NULL)))
        return;
    initialized = 1;
    return;
}
            ...
            double rotation, DevDesc *dd)
{
    if (!initialized)
        vfonts_Init();
    if (initialized > 0)
        (*ptr->GVText)(x, y, unit, s, typeface, fontindex, x_justify, y_justify, rotation, dd);
    else
        error("Hershey fonts cannot be loaded");
}
