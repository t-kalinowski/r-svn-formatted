...
static void vfonts_Init(void)
{
    ...
    if (!(ptr_GVStrWidth = (dblDL_FUNC)R_FindSymbol("Rf_GVStrWidth", "vfonts")))
        return;
    if (!(ptr_GVStrHeight = (dblDL_FUNC)R_FindSymbol("Rf_GVStrHeight", "vfonts")))
        return;
    if (!(ptr_GVText = R_FindSymbol("Rf_GVText", "vfonts")))
        return;
    initialized = 1;
    return;
}
