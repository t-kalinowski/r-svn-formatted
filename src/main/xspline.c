...
static Rboolean add_point(double x, double y, GEDevDesc *dd)
{
    if (npoints >= max_points)
    {
        ...
        {
            tmp_px = (double *)S_realloc((char *)xpoints, tmp_n, max_points, sizeof(double));
            tmp_py = (double *)S_realloc((char *)ypoints, tmp_n, max_points, sizeof(double));
        }
...
#define Q(s) (-(s))
#define EQN_NUMERATOR(dim) (A_blend[0] * dim[0] + A_blend[1] * dim[1] + A_blend[2] * dim[2] + A_blend[3] * dim[3])
...
static void point_adding(double *A_blend, double *px, double *py, GEDevDesc *dd)
{
    ...
    add_point(EQN_NUMERATOR(px) / (weights_sum), EQN_NUMERATOR(py) / (weights_sum), dd);
}

static void point_computing(double *A_blend, double *px, double *py, int *x, int *y)
{
    double weights_sum;
...
static float step_computing(int k, double *px, double *py, double s1, double s2, float precision)
{
    double A_blend[4];
...
static Rboolean compute_open_spline(int n, double *x, double *y, double *s, float precision, GEDevDesc *dd)
{
    int k;
    float step;
    double px[4];
