...
static void Cairo_MetricInfo(int c, const pGEcontext gc, double *ascent, double *descent, double *width, pDevDesc dd);
...
static void Cairo_Text(double x, double y, const char *str, double rot, double hadj, const pGEcontext gc, pDevDesc dd);
...
static PangoLayout *layoutText(PangoFontDescription *desc, cairo_t *cc, const char *str)
{
    PangoLayout *layout;
...
static void text_extents(PangoFontDescription *desc, cairo_t *cc, const pGEcontext gc, const gchar *str, gint *lbearing,
                         gint *rbearing, gint *width, gint *ascent, gint *descent, int ink)
{
    PangoLayout *layout;
...
static void Cairo_MetricInfo(int c, const pGEcontext gc, double *ascent, double *descent, double *width, pDevDesc dd)
{
    ...
    text_extents(desc, xd->cc, gc, str, NULL, NULL, &iwidth, &iascent, &idescent, 1);
    *ascent = iascent;
    *descent = idescent;
    *width = iwidth;
#if 0
...
static void Cairo_Text(double x, double y, const char *str, double rot, double hadj, const pGEcontext gc, pDevDesc dd)
{
    ...
    if (R_ALPHA(gc->col) > 0)
    {
        PangoFontDescription *desc = getFont(gc);
        cairo_save(xd->cc);
        text_extents(desc, xd->cc, gc, str, &lbearing, NULL, &width, &ascent, NULL, 0);
