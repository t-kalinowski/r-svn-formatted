...
{
    cairo_status_t res;
    if (xd->type == PNG || xd->type == JPEG || xd->type == PNGdirect || xd->type == TIFF || xd->type == BMP)
...
static Rboolean in_R_X11readclp(Rclpconn this, char *type)
{
    ...
    */
    ...
    XGetWindowProperty(display, clpwin, pty, 0, 0, False, AnyPropertyType, &pty_type, &pty_format, &pty_items,
                       &pty_size, &buffer);
    XFree(buffer);
    if (pty_format != 8)
    { /* bytes */
    ...
    { /* read the property */
        XGetWindowProperty(display, clpwin, pty, 0, (long)pty_size, False, AnyPropertyType, &pty_type, &pty_format,
