...
static double X11_StrWidth(const char *str, const pGEcontext gc, pDevDesc dd)
{
    ...
    else
    {
        ...
        if (utf8locale)
            return (double)Xutf8TextEscapement(xd->font->fontset, str, strlen(str));
        else
#endif
...
static void X11_NewPage(const pGEcontext gc, pDevDesc dd)
{
    ...
    if (xd->type > WINDOW)
    {
        ...
        SetColor(xd->fill, xd);
        xd->clip.x = 0;
        xd->clip.width = xd->windowWidth;
        xd->clip.y = 0;
        xd->clip.height = xd->windowHeight;
        XSetClipRectangles(display, xd->wgc, 0, 0, &(xd->clip), 1, Unsorted);
...
extern int R_SaveAsTIFF(void *d, int width, int height, unsigned int (*gp)(void *, int, int), int bgr,
                        const char *outfile, int res, int compression);

extern int R_SaveAsBmp(void *d, int width, int height, unsigned int (*gp)(void *, int, int), int bgr, FILE *fp,
                       int res);
                       ...
                       Rboolean interpolate, const pGEcontext gc, pDevDesc dd)
{
    ...
    for (i = 0; i < imageHeight; i++)
    {
        for (j = 0; j < imageWidth; j++)
        {
            ...
            XPutPixel(image, j, i,
                      GetX11Pixel(R_RED(rasterImage[pixel]), R_GREEN(rasterImage[pixel]), R_BLUE(rasterImage[pixel])));
        }
    }
...
static void X11_eventHelper(pDevDesc dd, int code)
{
    ...
    else if (code == 2)
    {
        ...
        else if (event.type == KeyPress)
        {
            ...
            XLookupString(&event.xkey, keystart, sizeof(keybuffer) - (keystart - keybuffer), &keysym, &compose);
            /* Rprintf("keysym=%x\n", keysym); */
...
static void Rf_addX11Device(const char *display, double width, double height, double ps, double gamma, int colormodel,
                            ...
                            int xpos, int ypos, const char *title, int useCairo, int antialias)
{
    pDevDesc dev = NULL;
    pGEDevDesc dd;
...
static void BM_Close_bitmap(pX11Desc xd)
{
    ...
    if (xd->type == PNG)
        ...
        R_SaveAsBmp(xi, xd->windowWidth, xd->windowHeight, Cbitgp, 0, xd->fp, xd->res_dpi);
    else
    {
        char buf[PATH_MAX];
        snprintf(buf, PATH_MAX, xd->filename, xd->npages);
