...
static void X11_Circle(double x, double y, double r, const pGEcontext gc, pDevDesc dd);
...
static void X11_Line(double x1, double y1, double x2, double y2, const pGEcontext gc, pDevDesc dd);
static void X11_MetricInfo(int c, const pGEcontext gc, double *ascent, double *descent, double *width, pDevDesc dd);
static void X11_Mode(int mode, pDevDesc dd);
static void X11_NewPage(const pGEcontext gc, pDevDesc dd);
static void X11_Polygon(int n, double *x, double *y, const pGEcontext gc, pDevDesc dd);
static void X11_Polyline(int n, double *x, double *y, const pGEcontext gc, pDevDesc dd);
static void X11_Rect(double x0, double y0, double x1, double y1, const pGEcontext gc, pDevDesc dd);
static void X11_Raster(unsigned int *raster, int w, int h, double x, double y, double width, double height, double rot,
                       Rboolean interpolate, const pGEcontext gc, pDevDesc dd);
static SEXP X11_Cap(pDevDesc dd);
static void X11_Size(double *left, double *right, double *bottom, double *top, pDevDesc dd);
static double X11_StrWidth(const char *str, const pGEcontext gc, pDevDesc dd);
static void X11_Text(double x, double y, const char *str, double rot, double hadj, const pGEcontext gc, pDevDesc dd);
...
static void CairoHandler(void)
{
    ...
    if (!buffer_lock && xdl->next)
    {
        ...
        {
            pX11Desc xd = z->this;
            if (xd->last > xd->last_activity)
                continue;
            if ((current - xd->last) < xd->update_interval)
                continue;
            Cairo_update(xd);
        }
        buffer_lock = 0;
...
static void addBuffering(pX11Desc xd)
{
    ...
    if (!xdln)
        error("allocation failed in addBuffering");
    xdln->this = xd;
    xdln->next = xdl->next;
    xdl->next = xdln;
    if (timingInstalled)
...
static int RGBlevels[][3] = {/* PseudoColor Palettes */
                             {8, 8, 4}, {6, 7, 6}, {6, 6, 6}, {6, 6, 5}, {6, 6, 4}, {5, 5, 5},
                             {5, 5, 4}, {4, 4, 4}, {4, 4, 3}, {3, 3, 3}, {2, 2, 2}};
    ...
    for (r = 0; r < nr; r++)
    {
        for (g = 0; g < ng; g++)
        {
            for (b = 0; b < nb; b++)
            {
                ...
                XPalette[i].red = (unsigned short)(pow(r / (nr - 1.0), RedGamma) * 0xffff);
                XPalette[i].green = (unsigned short)(pow(g / (ng - 1.0), GreenGamma) * 0xffff);
                XPalette[i].blue = (unsigned short)(pow(b / (nb - 1.0), BlueGamma) * 0xffff);
...
static void SetupPseudoColor(void)
{
    ...
    if (model == PSEUDOCOLOR1)
    {
        for (i = 0; i < NRGBlevels; i++)
        {
            ...
            if (size < maxcubesize &&
                GetColorPalette(display, colormap, RGBlevels[i][0], RGBlevels[i][1], RGBlevels[i][2]))
                break;
        }
        if (PaletteSize == 0)
        {
...
static unsigned int GetPseudoColor2Pixel(int r, int g, int b)
{
    ...
    for (i = 0; i < PaletteSize; i++)
    {
        if (r == RPalette[i].red && g == RPalette[i].green && b == RPalette[i].blue)
            return (unsigned int)XPalette[i].pixel;
    }
    /* Attempt to allocate a new color */
    XPalette[PaletteSize].red = (unsigned short)(pow(r / 255.0, RedGamma) * 0xffff);
    XPalette[PaletteSize].green = (unsigned short)(pow(g / 255.0, GreenGamma) * 0xffff);
    XPalette[PaletteSize].blue = (unsigned short)(pow(b / 255.0, BlueGamma) * 0xffff);
        ...
        if (xd->windowWidth != event.xconfigure.width || xd->windowHeight != event.xconfigure.height)
        {
            ...
            if (xd->useCairo)
            {
                if (xd->buffered)
                {
                    ...
                    xd->cs = cairo_image_surface_create(CAIRO_FORMAT_RGB24, xd->windowWidth, xd->windowHeight);
                ...
                { /* not buffered */
                    cairo_xlib_surface_set_size(xd->cs, xd->windowWidth, xd->windowHeight);
                    cairo_reset_clip(xd->cc);
                }
            }
#endif
...
#endif
    fontset = XCreateFontSet(display, fontset_name, &missing_charset_list, &missing_charset_count, &def_string);
    if (!fontset)
    {
        free(tmp);
        return NULL;
...
static void SetFont(const pGEcontext gc, pX11Desc xd)
{
    ...
    if (size != xd->fontsize || face != xd->fontface || strcmp(family, xd->fontfamily) != 0)
    {
...
static void SetLinetype(const pGEcontext gc, pX11Desc xd)
{
    ...
    if (newlwd < 1) /* not less than 1 pixel */
        newlwd = 1;
    if (newlty != xd->lty || newlwd != xd->lwd || gc->lend != xd->lend || gc->ljoin != xd->ljoin)
    {
        xd->lty = newlty;
        xd->lwd = newlwd;
        xd->lend = gc->lend;
        ...
        if (newlty == 0 || newlty == NA_INTEGER)
        { /* special hack for lty = 0 -- only for X11 */
            XSetLineAttributes(display, xd->wgc, (int)(newlwd * xd->lwdscale + 0.5), LineSolid, newlend, newljoin);
        }
        else
        {
            ...
            XSetLineAttributes(display, xd->wgc, (int)(newlwd * xd->lwdscale + 0.5), LineOnOffDash, newlend, newljoin);
        }
    }
}
...
Rboolean X11_Open(pDevDesc dd, pX11Desc xd, const char *dsp, double w, double h, double gamma_fac,
    ...
    if (!displayOpen)
    {
        ...
        */
        ...
        if (xd->handleOwnEvents == FALSE)
            addInputHandler(R_InputHandlers, ConnectionNumber(display), R_ProcessX11Events, XActivity);
    }
    ...
    if (type == WINDOW)
    {
        ...
        if (alreadyCreated == 0)
        {
            ...
            if (xpos == NA_INTEGER)
                ...
                hint->x = (xpos >= 0) ? xpos : DisplayWidth(display, screen) - iw + xpos;

            if (ypos == NA_INTEGER)
                hint->y = numX11Devices * 20 % (DisplayHeight(display, screen) + ih - 10);
            else
                hint->y = (ypos >= 0) ? ypos : DisplayHeight(display, screen) - iw - ypos;
            hint->width = iw;
            hint->height = ih;
...
#ifdef USE_Xt
            {
                ...
                xtdpy = XtOpenDisplay(app_con, dspname, "r_x11", "R_x11", NULL, 0, &zero, NULL);
                if (xtdpy)
                {
            ...
            /* set window icon */
            XChangeProperty(display, xd->window, XInternAtom(display, "_NET_WM_ICON", False),
            ...
            if (xd->useCairo)
            {
                ...
                if (xd->buffered)
                {
                    xd->xcs = cairo_xlib_surface_create(display, xd->window, visual, xd->windowWidth, xd->windowHeight);
                    ...
                    xd->cs = cairo_xlib_surface_create(display, xd->window, visual, xd->windowWidth, xd->windowHeight);
        ...
        if (alreadyCreated == 0)
        {
            ...
            while (XPeekEvent(display, &event), !XCheckTypedEvent(display, MapNotify, &event))
                ;
            /* XNextEvent(display, &event);
               if (event.xany.type == Expose) {
...
static double X11_StrWidth(const char *str, const pGEcontext gc, pDevDesc dd)
{
    ...
    else
    {
        ...
        if (utf8locale)
            return (double)Xutf8TextEscapement(xd->font->fontset, str, (int)strlen(str));
        else
#endif
            return (double)XmbTextEscapement(xd->font->fontset, str, (int)strlen(str));
    }
}
...
static void X11_NewPage(const pGEcontext gc, pDevDesc dd)
{
    ...
    if (xd->type > WINDOW)
    {
        ...
        SetColor(xd->fill, xd);
        xd->clip.x = 0;
        xd->clip.width = (unsigned short)xd->windowWidth;
        xd->clip.y = 0;
        xd->clip.height = (unsigned short)xd->windowHeight;
        XSetClipRectangles(display, xd->wgc, 0, 0, &(xd->clip), 1, Unsorted);
...
static void X11_Close_bitmap(pX11Desc xd)
{
    ...
    if (xd->type == PNG)
    {
        ...
        if (model == TRUECOLOR)
        {
            ...
            i = GetX11Pixel(R_RED(PNG_TRANS), R_GREEN(PNG_TRANS), R_BLUE(PNG_TRANS));
            r = ((i >> RShift) & RMask) * 255 / (RMask);
            g = ((i >> GShift) & GMask) * 255 / (GMask);
            b = ((i >> BShift) & BMask) * 255 / (BMask);
            pngtrans = (r << 16) | (g << 8) | b | 0xFF000000;
        }
        R_SaveAsPng(xi, xd->windowWidth, xd->windowHeight, bitgp, 0, xd->fp, (xd->fill != PNG_TRANS) ? 0 : pngtrans,
                    xd->res_dpi);
    }
    else if (xd->type == JPEG)
        R_SaveAsJpeg(xi, xd->windowWidth, xd->windowHeight, bitgp, 0, xd->quality, xd->fp, xd->res_dpi);
    else if (xd->type == BMP)
        R_SaveAsBmp(xi, xd->windowWidth, xd->windowHeight, bitgp, 0, xd->fp, xd->res_dpi);
    else if (xd->type == TIFF)
    {
        char buf[PATH_MAX];
        snprintf(buf, PATH_MAX, xd->filename, xd->npages);
        R_SaveAsTIFF(xi, xd->windowWidth, xd->windowHeight, bitgp, 0, R_ExpandFileName(buf), xd->res_dpi, xd->quality);
    }
...
static void X11_Path(double *x, double *y, int npoly, int *nper, Rboolean winding, const pGEcontext gc, pDevDesc dd)
{
...
{
    return GetX11Pixel(R_RED(rasterImage[pixel]), R_GREEN(rasterImage[pixel]), R_BLUE(rasterImage[pixel]));
}
                       ...
                       Rboolean interpolate, const pGEcontext gc, pDevDesc dd)
{
    ...
    if (rot != 0)
    {
        ...
        R_GE_rasterRotatedOffset(imageWidth, imageHeight, angle, 0, &xoff, &yoff);

        resizedRaster = (unsigned int *)R_alloc(newW * newH, sizeof(unsigned int));
    ...
    XPutImage(display, xd->window, xd->wgc, image, 0, 0, (int)x, (int)y, imageWidth, imageHeight);
...
{
    pX11Desc xd = (pX11Desc)dd->deviceSpecific;
    XImage *image = XGetImage(display, xd->window, 0, 0, xd->windowWidth, xd->windowHeight, AllPlanes, ZPixmap);
...
static void X11_Text(double x, double y, const char *str, double rot, double hadj, const pGEcontext gc, pDevDesc dd)
{
    ...
    {
        SetColor(gc->col, xd);
        XRfRotDrawString(display, xd->font, rot, xd->window, xd->wgc, (int)x, (int)y, str);
    }
}
...
static void X11_eventHelper(pDevDesc dd, int code)
{
    ...
    else if (code == 2)
    {
        ...
        else if (event.type == KeyPress)
        {
            ...
            XLookupString(&event.xkey, keystart, sizeof(keybuffer) - (int)(keystart - keybuffer), &keysym, &compose);
            /* Rprintf("keysym=%x\n", keysym); */
...
Rboolean X11DeviceDriver(pDevDesc dd, const char *disp_name, double width, double height, double pointsize,
                         double gamma_fac, X_COLORTYPE colormodel, int maxcube, int bgcolor, int canvascolor,
                         SEXP sfonts, int res, int xpos, int ypos, const char *title, int useCairo, int antialias,
                         const char *family, const char *symbolfamily, Rboolean usePUA)
{
    pX11Desc xd;
    const char *fn;
    ...
    if (useCairo)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            xd->antialias = CAIRO_ANTIALIAS_NONE;
            break;
        case 3:
            xd->antialias = CAIRO_ANTIALIAS_GRAY;
            break;
        case 4:
...
int Rf_setX11DeviceData(pDevDesc dd, double gamma_fac, pX11Desc xd)
{
    ...
    if (xd->type == PNG || xd->type == JPEG || xd->type == BMP || xd->type == TIFF)
    {
...
typedef Rboolean (*X11DeviceDriverRoutine)(pDevDesc, char *, double, double, double, double, X_COLORTYPE, int, int);
...
static void Rf_addX11Device(const char *display, double width, double height, double ps, double gamma, int colormodel,
                            ...
                            int xpos, int ypos, const char *title, int useCairo, int antialias, const char *family,
                            const char *symbolfamily, Rboolean usePUA, SEXP call)
{
    ...
    BEGIN_SUSPEND_INTERRUPTS
    {
        ...
        if (!X11DeviceDriver(dev, display, width, height, ps, gamma, colormodel, maxcubesize, bgcolor, canvascolor,
                             sfonts, res, xpos, ypos, title, useCairo, antialias, family, symbolfamily, usePUA))
        {
            free(dev);
...
static SEXP in_do_X11(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!strncmp(display, "png::", 5))
        ...
        devname = "JPEG";
    else if (!strncmp(display, "tiff::", 6))
        devname = "TIFF";
    else if (!strncmp(display, "bmp::", 5))
        devname = "BMP";
    ...
    Rf_addX11Device(display, width, height, ps, gamma, colormodel, maxcubesize, bgcolor, canvascolor, devname, sfonts,
                    res, xpos, ypos, title, useCairo, antialias, family, symbolfamily, usePUA, call);
    vmaxset(vmax);
    return R_NilValue;
}
...
static SEXP in_do_saveplot(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    else if (streql(type, "jpeg"))
    {
        ...
        R_SaveAsJpeg(xi, xd->windowWidth, xd->windowHeight, Sbitgp, 0, 75, fp, 0);
        fclose(fp);
    }
...
static Rboolean in_R_X11readclp(Rclpconn this, char *type)
{
    ...
    if (strcmp(type, "X11_secondary") == 0)
        sel = XA_SECONDARY;
    ...
    clpwin = XCreateSimpleWindow(display, DefaultRootWindow(display), 0, 0, 1, 1, 0, 0, 0);
    ...
    */
    ...
    ret = XGetWindowProperty(display, clpwin, pty, 0, 0, False, AnyPropertyType, &pty_type, &pty_format, &pty_items,
                             &pty_size, &buffer);
    if (ret)
    {
    ...
    else
    {
        ...
        { /* read the property */
            ret = XGetWindowProperty(display, clpwin, pty, 0, (long)pty_size, False, AnyPropertyType, &pty_type,
