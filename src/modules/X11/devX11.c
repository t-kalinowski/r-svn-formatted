 ...
 */
#ifdef USE_FONTSET
static XFontSet XLoadQueryFontSet(Display *display, const char *fontset_name)
{
    XFontSet fontset;
...
static void *RLoadFont(newX11Desc *xd, char *family, int face, int size)
{
    ...
    if (!tmp || (force_nonscalable && !ADOBE_SIZE(size)))
    {
        ...
        {
#ifdef USE_FONTSET
            tmp = (void *)XLoadQueryFontSet(display, "-*-fixed-medium-r-*--13-*-*-*-*-*-*-*");
#else
...
static int SetBaseFont(newX11Desc *xd)
{
    ...
    if (!xd->font)
    {
        xd->usefixed = 1;
#ifdef USE_FONTSET
        xd->font = xd->fixedfont = (void *)XLoadQueryFontSet(display, "-*-fixed-medium-r-*--13-*-*-*-*-*-*-*");
#else
