...
static SEXP in_do_jpeg(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    BEGIN_SUSPEND_INTERRUPTS
    {
        ...
        if (!(dev = (pDevDesc)calloc(1, sizeof(NewDevDesc))))
            return 0;
        if (!BMDeviceDriver(dev, JPEG, filename, quality, width, height, pointsize, bgcolor, res, antialias))
        {
            free(dev);
...
static SEXP in_do_png(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    BEGIN_SUSPEND_INTERRUPTS
    {
        ...
        if (!(dev = (pDevDesc)calloc(1, sizeof(NewDevDesc))))
            return 0;
        if (!BMDeviceDriver(dev, PNG, filename, 0, width, height, pointsize, bgcolor, res, (Rboolean)antialias))
        {
            free(dev);
            error(_("unable to start device jpeg"));
        }
