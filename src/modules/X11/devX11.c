...
static void X11_Close_bitmap(pX11Desc xd)
{
    ...
    else if (xd->type == TIFF)
    {
        char buf[PATH_MAX];
        snprintf(buf, PATH_MAX, xd->filename, xd->npages);
        R_SaveAsTIFF(xi, xd->windowWidth, xd->windowHeight, bitgp, 0, R_ExpandFileName(buf), xd->res_dpi);
    }
...
static void BM_Close_bitmap(pX11Desc xd)
{
    ...
    else
    {
        char buf[PATH_MAX];
        snprintf(buf, PATH_MAX, xd->filename, xd->npages);
        R_SaveAsTIFF(xi, xd->windowWidth, xd->windowHeight, Cbitgp, 0, R_ExpandFileName(buf), xd->res_dpi);
    }
}
...
const static struct
{
    ...
    X_GTYPE gtype;
                ...
                {"png", PNGdirect}, {"cairo_pdf", PDF}, {"cairo_ps", PS}, {"tiff", TIFF}};
