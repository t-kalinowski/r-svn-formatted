 ...
 */
#ifdef USE_FONTSET
static XFontSet RXLoadQueryFontSet(Display *display, const char *fontset_name)
{
    XFontSet fontset;
...
static void *RLoadFont(newX11Desc *xd, char *family, int face, int size)
{
    ...
    if (!tmp || (force_nonscalable && !ADOBE_SIZE(size)))
    {
        ...
        {
#ifdef USE_FONTSET
            tmp = (void *)RXLoadQueryFontSet(display, "-*-fixed-medium-r-*--13-*-*-*-*-*-*-*");
#else
    ...
    if (tmp)
    {
        ...
        if (fabs((pixelsize - size) / (double)size) > 0.1)
            warning("X11 used font size %d when %d was requested", pixelsize, size);
    }
...
static int SetBaseFont(newX11Desc *xd)
{
    ...
    if (!xd->font)
    {
        xd->usefixed = 1;
#ifdef USE_FONTSET
        xd->font = xd->fixedfont = (void *)RXLoadQueryFontSet(display, "-*-fixed-medium-r-*--13-*-*-*-*-*-*-*");
#else
...
Rboolean newX11_Open(NewDevDesc *dd, newX11Desc *xd, char *dsp, double w, double h, double gamma_fac,
                     X_COLORTYPE colormodel, int maxcube, int bgcolor, int canvascolor, int res)
{
...
static double newX11_StrWidth(char *str, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    if (xd->fontface == SYMBOL_FONTFACE)
        ...
        if (utf8locale)
        return (double)Xutf8TextEscapement((XFontSet)(xd->font), str, strlen(str));
    else
#endif
        return (double)XmbTextEscapement((XFontSet)(xd->font), str, strlen(str));
#else
        return (double)XTextWidth((XFontStruct *)xd->font, str, strlen(str));
#endif
}
...
#ifdef HAVE_XUTF8TEXTESCAPEMENT
            if (utf8locale)
                *width = Xutf8TextEscapement((XFontSet)(xd->font), buf, strlen(buf));
            else
#endif
                *width = XmbTextEscapement((XFontSet)(xd->font), buf, strlen(buf));
        }
        else
        {
...
static void newX11_Text(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc, NewDevDesc *dd)
{
    ...
    if (R_OPAQUE(gc->col))
    {
        ...
        len = strlen(str);
        if (xd->fontface == SYMBOL_FONTFACE)
            XRotDrawString(display, (XFontStruct *)(xd->font), rot, xd->window, xd->wgc, (int)x, (int)y, str);
        else
#ifdef USE_FONTSET
            XmbRotDrawString(display, (XFontSet)(xd->font), rot, xd->window, xd->wgc, (int)x, (int)y, str);
#else
            XRotDrawString(display, (XFontStruct *)xd->font, rot, xd->window, xd->wgc, (int)x, (int)y, str);
#endif
#ifdef XSYNC
