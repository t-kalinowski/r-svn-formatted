...
void OpenDataBrowser(void)
{
    ...
    if (!isEventHandlerOn)
    {
        InstallWindowEventHandler(BrowserWindow, NewEventHandlerUPP(BrowserEventHandler),
...
static pascal void MyItemNotification(ControlRef browser, DataBrowserItemID itemID, DataBrowserItemNotification message)
{
    ...
    case kDataBrowserContainerOpened: {
        AddDataBrowserItems(browser, itemID, NumberOfItems[itemID - 1], (DataBrowserItemID *)SubItemsID[itemID - 1],
                            kObjectColumn);
        {
            ...
            if (variableHeightRows)
                for (j = 0; j < NumberOfItems[itemID - 1]; j++)
                    SetDataBrowserTableViewItemRowHeight(browser, SubItemsID[itemID - 1][j],
                                                         20 + (SubItemsID[itemID - 1][j] - 1) % 10 * 3);
        }
    }
