...
enum
{
    kApplyPrefsButton = 5000,
    kCancelPrefsButton = 5001,
    kDefaultPrefsButton = 5002,
    kSavePrefsButton = 5003
};
...
void Raqua_StartConsole(void)
{
    ...
    if (ConsoleWindow != NULL)
    {
        ...
        if (err == noErr)
        {
            ...
            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kApplyPrefsButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);
            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kCancelPrefsButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);
            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kDefaultPrefsButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);
            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kSavePrefsButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);

            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kConsoleFontButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);

            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kOutputColorButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);

            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kInputColorButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);

            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kOutputBackButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);

            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kInputBackButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);

            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kWorkingDirButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);

            InstallControlEventHandler(GrabCRef(RPrefsWindow, kPrefControlsSig, kDeviceFontButton),
                                       GenContEventHandlerProc, GetEventTypeCount(okcanxControlEvents),
                                       okcanxControlEvents, RPrefsWindow, NULL);
...
static pascal OSStatus RWinHandler(EventHandlerCallRef inCallRef, EventRef inEvent, void *inUserData)
{
    ...
    case kEventClassWindow:
        ...
        case kEventWindowFocusRelinquish:
            SetFontInfoForSelection(kFontSelectionATSUIType, 0, NULL, NULL);
            break;
...
void SetDefaultPrefs(void)
{
    ...
    ATSUFindFontFromName("Monaco", strlen("Monaco"), kFontFullName, kFontNoPlatform, kFontNoScript, kFontNoLanguage,
                         &theFontID);
    ...
    ATSUFindFontFromName("Helvetica", strlen("Helvetica"), kFontFullName, kFontNoPlatform, kFontNoScript,
                         kFontNoLanguage, &theFontID);
...
void GetRPrefs(void)
{
    ...
    if (value)
    {
        if (!CFNumberGetValue(value, kCFNumberIntType, &fontsize))
            fontsize = DefaultPrefs.ConsoleFontSize;
        CFRelease(value);
    }
    else
    ...
    if (value)
    {
        if (!CFStringGetCString(value, consolefont, 255, kCFStringEncodingMacRoman))
            strcpy(consolefont, DefaultPrefs.ConsoleFontName);
        CFRelease(value);
    }
    else
    ...
    if (value)
    {
        if (!CFStringGetCString(value, workingdir, 500, kCFStringEncodingMacRoman))
            strcpy(workingdir, DefaultPrefs.WorkingDir);
        CFRelease(value);
    }
    else
    ...
    if (value)
    {
        if (!CFStringGetCString(value, devicefont, 255, kCFStringEncodingMacRoman))
...
void SetUpPrefsWindow(RAquaPrefsPointer Settings)
{
    ...
    text = CFStringCreateWithFormat(NULL, NULL, CFSTR("%s %d pt"), CurrentPrefs.ConsoleFontName,
                                    CurrentPrefs.ConsoleFontSize);
...
OSStatus MySetFontSelection(WindowRef thisWindow)
{
    ...
    verify_noerr(ATSUFindFontFromName("Times Roman", strlen("Times Roman"), kFontFullName, kFontNoPlatform,
    ...
    verify_noerr(ATSUSetAttributes(myStyle, 2, myTags, mySizes, myValues)); // 7
    myHIObjectTarget = (HIObjectRef)GetWindowEventTarget(thisWindow);       // 8
    SetFontInfoForSelection(kFontSelectionATSUIType, 1, &myStyle,
                            myHIObjectTarget); // 9
...

{
    ...
    status = GetEventParameter(event, kEventParamFMFontFamily, typeFMFontFamily, NULL, sizeof(instance.fontFamily),
                               NULL, &(instance.fontFamily)); // 2
    check_noerr(status);                                      // 3

    status = GetEventParameter(event, kEventParamFMFontStyle, typeFMFontStyle, NULL, sizeof(instance.fontStyle), NULL,
                               &(instance.fontStyle)); // 4
    check_noerr(status);

    status =
        GetEventParameter(event, kEventParamFMFontSize, typeFMFontSize, NULL, sizeof(fontSize), NULL, &fontSize); // 5
...
void GetDialogPrefs(void)
{
    ...
    fprintf(stderr, "\n tab=%d, psize=%d, w=%f, h=%f, auto=%d, anti=%d", CurrentPrefs.TabSize,
            CurrentPrefs.DevicePointSize, CurrentPrefs.DeviceWidth, CurrentPrefs.DeviceHeight, CurrentPrefs.AutoRefresh,
            CurrentPrefs.AntiAlias);
}
