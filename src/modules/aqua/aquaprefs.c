...
void GetRPrefs(void)
{
    ...
    if (text)
    {
        if (!CFStringGetCString(text, consolefont, 255, kCFStringEncodingMacRoman))
    ...
    if (value)
    {
        if (!CFNumberGetValue(value, kCFNumberIntType, &buffering))
            buffering = DefaultPrefs.Buffering;
        CFRelease(value);
    }
    else
    ...
    if (value)
    {
        if (!CFNumberGetValue(value, kCFNumberIntType, &buffersize))
            buffersize = DefaultPrefs.BufferSize;
        CFRelease(value);
    }
    else
...

{
    ...
    if ((status = GetEventParameter(event, kEventParamFMFontFamily, typeFMFontFamily, NULL, sizeof(instance.fontFamily),
...
void RSetPackagePrefs(void)
{
    ...
    /* actual work */
    if (strcmp(oldCRAN, CurrentPrefs.CRANmirror) | strcmp(oldBIOC, CurrentPrefs.BIOCmirror))
    {
