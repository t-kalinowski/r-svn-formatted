...
static pascal OSStatus bpGetSetItemData(ControlRef browser, DataBrowserItemID itemID, DataBrowserPropertyID property,
                                        DataBrowserItemDataRef itemData, Boolean changeValue);

static pascal void bpItemNotification(ControlRef browser, DataBrowserItemID itemID,
...
Boolean OpenBrowsePkg(void)
{
    ...
    InstallWindowEventHandler(BrowsePkgWindow, NewEventHandlerUPP(bpEventHandler),
...
void EmptyBrowsePkg(void)
{
    RemoveDataBrowserItems(BrowsePkgControl, kDataBrowserNoItem, 0, NULL, kDataBrowserItemNoProperty);
}
...
static void ConfigureBrowsePkg(ControlRef browser)
{
    ...
    case kDataBrowserListView: {
        ...
        columnDesc.headerBtnDesc.titleString =
            CFStringCreateWithCString(CFAllocatorGetDefault(), bpNames[0], kCFStringEncodingMacRoman);
        ...
        for (i = 1; i <= 3; i++)
        {
            ...
            columnDesc.headerBtnDesc.titleString =
                CFStringCreateWithCString(CFAllocatorGetDefault(), bpNames[i], kCFStringEncodingMacRoman);
...
static ControlRef GetDataBrowserFromWindow(WindowRef window)
{
    ...
    if (window != NULL)
        GetWindowProperty(window, 'RMAC', 'PKGB', sizeof(browser), NULL, &browser);
...
void InstallBrowsePkgCallbacks(ControlRef browser)
{
    ...
    myCallbacks.u.v1.itemDataCallback = NewDataBrowserItemDataUPP(bpGetSetItemData);

    myCallbacks.u.v1.itemNotificationCallback = NewDataBrowserItemNotificationUPP(bpItemNotification);
...
static pascal void bpItemNotification(ControlRef browser, DataBrowserItemID itemID, DataBrowserItemNotification message)
{
    UInt32 i;
...
pascal OSStatus bpEventHandler(EventHandlerCallRef a, EventRef inEvent, void *b)
{
