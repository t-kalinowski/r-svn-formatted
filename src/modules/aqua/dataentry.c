...
static pascal OSStatus deGetSetItemData(ControlRef browser, DataBrowserItemID itemID, DataBrowserPropertyID property,
                                        DataBrowserItemDataRef itemData, Boolean changeValue);

static pascal void deItemNotification(ControlRef browser, DataBrowserItemID itemID,
                                      DataBrowserItemNotification message);
static pascal Boolean deItemComparison(ControlRef browser, DataBrowserItemID itemOneID, DataBrowserItemID itemTwoID,
...
Boolean OpenDataEntry(void)
{
    ...
    InstallWindowEventHandler(DataEntryWindow, NewEventHandlerUPP(deEventHandler),
                              sizeof(deEvents) / sizeof(EventTypeSpec), deEvents, NULL, NULL);

    InstallWindowEventHandler(DataEntryWindow, NewEventHandlerUPP(DoCloseHandler), 1, RCloseWinEvent,
                              (void *)DataEntryWindow, NULL);
...
void EmptyDataEntry(void)
{
    RemoveDataBrowserItems(DataEntryControl, kDataBrowserNoItem, 0, NULL, kDataBrowserItemNoProperty);
}
        ...
        for (i = 0; i <= xmaxused; i++)
        {
            ...
            if (i == 0)
                columnDesc.headerBtnDesc.titleString =
                    CFStringCreateWithCString(CFAllocatorGetDefault(), "Row Labels", kCFStringEncodingMacRoman);
            else
                columnDesc.headerBtnDesc.titleString =
                    CFStringCreateWithCString(CFAllocatorGetDefault(), get_col_name(i), kCFStringEncodingMacRoman);
...
static ControlRef GetDataBrowserFromWindow(WindowRef window)
{
    ...
    if (window != NULL)
        GetWindowProperty(window, 'RMAC', 'RDEY', sizeof(browser), NULL, &browser);
...
void InstallDataEntryCallbacks(ControlRef browser)
{
    ...
    myCallbacks.u.v1.itemDataCallback = NewDataBrowserItemDataUPP(deGetSetItemData);

    myCallbacks.u.v1.itemNotificationCallback = NewDataBrowserItemNotificationUPP(deItemNotification);

    myCallbacks.u.v1.itemCompareCallback = NewDataBrowserItemCompareUPP(deItemComparison);
...
static pascal void deItemNotification(ControlRef browser, DataBrowserItemID itemID, DataBrowserItemNotification message)
{
    UInt32 i;
