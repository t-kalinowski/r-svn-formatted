...
static pascal OSStatus dmGetSetItemData(ControlRef browser, DataBrowserItemID itemID, DataBrowserPropertyID property,
                                        DataBrowserItemDataRef itemData, Boolean changeValue);

static pascal void dmItemNotification(ControlRef browser, DataBrowserItemID itemID,
...
Boolean OpenDataManager(void)
{
    ...
    CreateNewWindow(kDocumentWindowClass, kWindowStandardHandlerAttribute | kWindowStandardDocumentAttributes,
                    &dmBounds, &DataManagerWindow);

    if (DataManagerWindow == NULL)
        return (FALSE);

    InstallWindowEventHandler(DataManagerWindow, NewEventHandlerUPP(dmEventHandler),
    ...
    AddDataBrowserItems(DataManagerControl, kDataBrowserNoItem, NumOfDSets, DSetID, kDataBrowserItemNoProperty);
...
void EmptyDataManager(void)
{
    RemoveDataBrowserItems(DataManagerControl, kDataBrowserNoItem, 0, NULL, kDataBrowserItemNoProperty);
}
...
static void ConfigureDataManager(ControlRef browser)
{
    ...
    case kDataBrowserListView: {
        ...
        columnDesc.headerBtnDesc.titleString =
            CFStringCreateWithCString(CFAllocatorGetDefault(), dmNames[0], kCFStringEncodingMacRoman);
        ...
        for (i = 1; i <= 3; i++)
        {
            ...
            columnDesc.headerBtnDesc.titleString =
                CFStringCreateWithCString(CFAllocatorGetDefault(), dmNames[i], kCFStringEncodingMacRoman);
...
static ControlRef GetDataBrowserFromWindow(WindowRef window)
{
    ...
    if (window != NULL)
        GetWindowProperty(window, 'RMAC', 'DMAN', sizeof(browser), NULL, &browser);
...
void InstallDataManagerCallbacks(ControlRef browser)
{
    ...
    myCallbacks.u.v1.itemDataCallback = NewDataBrowserItemDataUPP(dmGetSetItemData);

    myCallbacks.u.v1.itemNotificationCallback = NewDataBrowserItemNotificationUPP(dmItemNotification);
...
static pascal void dmItemNotification(ControlRef browser, DataBrowserItemID itemID, DataBrowserItemNotification message)
{
    UInt32 i;
...
pascal OSStatus dmEventHandler(EventHandlerCallRef a, EventRef inEvent, void *b)
{
