...
static void GNOME_NewPage(DevDesc *dd)
{
    ...
    gd->plotarea = gnome_canvas_item_new(gd->group, gnome_canvas_rect_get_type(), "x1", (double)0.0, "y1", (double)0.0,
                                         ...
                                         "fill_color_rgba", Color_RGBA(dd->dp.bg), NULL);
}

/* create window etc */
static Rboolean GNOME_Open(DevDesc *dd, gnomeDesc *gd, char *dsp, double w, double h)
{
    GnomeCanvasItem *group;
...
static void GNOME_Rect(double x0, double y0, double x1, double y1, int coords, int bg, int fg, DevDesc *dd)
{
    double tmp;
    gnomeDesc *gd = (gnomeDesc *)dd->deviceSpecific;
    ...
    item = gnome_canvas_item_new(gd->group, gnome_canvas_rect_get_type(), "x1", x0, "y1", y0, "x2", x1, "y2", y1,
                                 "width_units", (double)dd->gp.lwd, NULL);

    if (bg != NA_INTEGER)
    {
        gnome_canvas_item_set(item, "fill_color_rgba", Color_RGBA(bg), NULL);
    }
    if (fg != NA_INTEGER)
    {
        gnome_canvas_item_set(item, "outline_color_rgba", Color_RGBA(fg), NULL);
    }
}

static void GNOME_Circle(double x, double y, int coords, double r, int col, int border, DevDesc *dd)
{
    GdkColor gcol_fill, gcol_outline;
    double x1, y1, x2, y2;
    gnomeDesc *gd = (gnomeDesc *)dd->deviceSpecific;
    ...
    {
        gnome_canvas_item_set(item, "outline_color_rgba", Color_RGBA(border), NULL);
    }
}

static void GNOME_Line(double x1, double y1, double x2, double y2, int coords, DevDesc *dd)
{
    ...
    if (dd->gp.col != NA_INTEGER)
    {
        ...
        gnome_canvas_item_set(item, "fill_color_rgba", Color_RGBA(dd->gp.col), NULL);
    }
...
static void GNOME_Polygon(int n, double *x, double *y, int coords, int bg, int fg, DevDesc *dd)
{
    double devx, devy;
    int i;
    GnomeCanvasPoints *points;
...
static void GNOME_Text(double x, double y, int coords, char *str, double rot, double hadj, DevDesc *dd)
{
    ...
    item = gnome_canvas_item_new(gd->group, gnome_canvas_text_get_type(), "text", str, "anchor", GTK_ANCHOR_SOUTH_WEST,
                                 "x", x, "y", y, "font_gdk", gd->font, "fill_color_rgba", Color_RGBA(dd->gp.col), NULL);
