...
/* routines from here */
Rboolean GTKDeviceDriver(DevDesc *dd, char *display, double width, double height, double pointsize);
...
static GnomeUIInfo graphics_toolbar[] = {
    {GNOME_APP_UI_ITEM, "Activate", "Make this window the current device", (gpointer)tb_activate_cb, NULL, NULL,
     GNOME_APP_PIXMAP_STOCK, GNOME_STOCK_PIXMAP_JUMP_TO, 0, (GdkModifierType)0, NULL},
    GNOMEUIINFO_SEPARATOR,
    {GNOME_APP_UI_ITEM, "Close", "Close this graphics device", (gpointer)tb_close_cb, NULL, NULL,
...
static Rboolean GTK_Open(NewDevDesc *dd, gtkDesc *gtkd, char *dsp, double w, double h)
{
    ...
    gtk_widget_set_events(gtkd->drawing, GDK_EXPOSURE_MASK | GDK_BUTTON_PRESS_MASK);
                           ...
                           NewDevDesc *dd)
{
    ...
    {
        g_snprintf(tmp, 2, "%c", (gchar)c);
        gdk_string_extents(gtkd->font, tmp, &lbearing, &rbearing, &iwidth, &iascent, &idescent);
...
static void locator_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
...
Rboolean GTKDeviceDriver(DevDesc *odd, char *display, double width, double height, double pointsize)
{
    NewDevDesc *dd;
    int ps;
    gchar tmp[2];
    ...
    {
        g_snprintf(tmp, 2, "%c", (gchar)c);
        gdk_string_extents(gtkd->font, tmp, &lbearing, &rbearing, NULL, NULL, NULL);
