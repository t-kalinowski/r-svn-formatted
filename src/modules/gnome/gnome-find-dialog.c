...
guint gnome_find_dialog_get_type(void)
{
    ...
    if (!find_dialog_type)
    {
        ...
        find_dialog_type = gtk_type_unique(gnome_dialog_get_type(), &find_dialog_info);
    }
...
static void gnome_find_dialog_class_init(GnomeFindDialogClass *klass)
{
    ...
    find_dialog_signals[FIND] =
        gtk_signal_new("find", GTK_RUN_LAST, object_class->type, GTK_SIGNAL_OFFSET(GnomeFindDialogClass, find),
                       gtk_marshal_NONE__NONE, GTK_TYPE_NONE, 0);
    find_dialog_signals[FIND_AGAIN] =
        gtk_signal_new("find_again", GTK_RUN_LAST, object_class->type,
                       GTK_SIGNAL_OFFSET(GnomeFindDialogClass, find_again), gtk_marshal_NONE__NONE, GTK_TYPE_NONE, 0);

    gtk_object_class_add_signals(object_class, find_dialog_signals, LAST_SIGNAL);
...
static void gnome_find_dialog_init(GnomeFindDialog *find_dialog)
{
    ...
    gnome_dialog_append_button_with_pixmap(GNOME_DIALOG(find_dialog), "Find", GNOME_STOCK_PIXMAP_SEARCH);
    gnome_dialog_append_button_with_pixmap(GNOME_DIALOG(find_dialog), "Find Again", GNOME_STOCK_PIXMAP_SEARCH);
    gnome_dialog_append_buttons(GNOME_DIALOG(find_dialog), GNOME_STOCK_BUTTON_CLOSE, NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(find_dialog), "clicked", GTK_SIGNAL_FUNC(dialog_clicked_cb), NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(find_dialog->find_entry), "changed", GTK_SIGNAL_FUNC(entry_changed_cb),
    ...
    find_dialog->backwards_radio =
        gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(find_dialog->forwards_radio), "Backwards");
    ...
    gtk_signal_connect(GTK_OBJECT(find_dialog->forwards_radio), "clicked", GTK_SIGNAL_FUNC(forwards_clicked_cb),
                       (gpointer)find_dialog);
    gtk_signal_connect(GTK_OBJECT(find_dialog->backwards_radio), "clicked", GTK_SIGNAL_FUNC(backwards_clicked_cb),
                       (gpointer)find_dialog);
    ...
    find_dialog->cursor_radio =
        gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(find_dialog->top_radio), "Cursor position");
    find_dialog->bottom_radio =
        gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(find_dialog->top_radio), "Bottom");
    ...
    gtk_signal_connect(GTK_OBJECT(find_dialog->top_radio), "clicked", GTK_SIGNAL_FUNC(top_clicked_cb),
                       (gpointer)find_dialog);
    gtk_signal_connect(GTK_OBJECT(find_dialog->cursor_radio), "clicked", GTK_SIGNAL_FUNC(cursor_clicked_cb),
                       (gpointer)find_dialog);
    gtk_signal_connect(GTK_OBJECT(find_dialog->bottom_radio), "clicked", GTK_SIGNAL_FUNC(bottom_clicked_cb),
                       (gpointer)find_dialog);
...
GtkWidget *gnome_find_dialog_new(const gchar *title, const GnomeFindDialogParams *find_params,
                                 gboolean show_case_sensitive, gboolean show_wrap_search, gboolean show_regex)
{
    GnomeFindDialog *find_dialog;
    ...
    if (find_params)
    {
        ...
        {
            gtk_entry_set_text(GTK_ENTRY(find_dialog->find_entry), find_params->find_text);
            gtk_editable_select_region(GTK_EDITABLE(find_dialog->find_entry), 0, -1);
        }
    }
    else
    {
    ...
    case GNOME_FIND_FORWARDS:
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(find_dialog->forwards_radio), TRUE);
        break;

    case GNOME_FIND_BACKWARDS:
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(find_dialog->backwards_radio), TRUE);
        break;
    }
    ...
    {
    case GNOME_FIND_TOP:
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(find_dialog->top_radio), TRUE);
        break;

    case GNOME_FIND_CURSOR:
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(find_dialog->cursor_radio), TRUE);
        break;

    case GNOME_FIND_BOTTOM:
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(find_dialog->bottom_radio), TRUE);
        break;
    }
    ...
    if (show_case_sensitive || show_wrap_search || show_regex)
    {
        ...
        if (show_case_sensitive)
        {
            ...
            gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(case_check), find_dialog->params.case_sensitive);
            gtk_box_pack_start(GTK_BOX(options_hbox), case_check, TRUE, TRUE, 0);
            gtk_signal_connect(GTK_OBJECT(case_check), "toggled", GTK_SIGNAL_FUNC(case_toggled_cb),
                               (gpointer)find_dialog);
        }
        if (show_wrap_search)
        {
            wrap_check = gtk_check_button_new_with_label("Wrap search");
            gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(wrap_check), find_dialog->params.wrap_search);
            gtk_box_pack_start(GTK_BOX(options_hbox), wrap_check, TRUE, TRUE, 0);
            gtk_signal_connect(GTK_OBJECT(wrap_check), "toggled", GTK_SIGNAL_FUNC(wrap_toggled_cb),
                               (gpointer)find_dialog);
        }
        if (show_regex)
        {
            regex_check = gtk_check_button_new_with_label("Regular expression");
            gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(regex_check), find_dialog->params.regex);
            gtk_box_pack_start(GTK_BOX(options_hbox), regex_check, TRUE, TRUE, 0);
            gtk_signal_connect(GTK_OBJECT(regex_check), "toggled", GTK_SIGNAL_FUNC(regex_toggled_cb),
                               (gpointer)find_dialog);
        }
