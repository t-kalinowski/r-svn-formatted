...
static void process_request(httpd_conn_t *c)
{
    ...
    {                   /* construct try(httpd(url, query), silent=TRUE) */
        ...
        SEXP y,
            x = PROTECT(lang3(install("try"),
                              LCONS(install("httpd"),
                                    CONS(mkString(c->url), CONS(query ? parse_query(query) : R_NilValue, R_NilValue))),
                              sTrue));
        SET_TAG(CDR(CDR(x)), install("silent"));
        DBG(Rprintf("eval(try(httpd('%s'),silent=TRUE))\n", c->url));
        x = PROTECT(eval(x, R_GlobalEnv)); /* FIXME: for now we evaluate in the global environemnt for testing purposes,
                                              but later it should be base or something like that ... */
         ...
         *    headers: must be a character vector - the elements will have CRLF appended and neither Content-type nor
         * Content-length may be used status code: must be an integer if present (default is 200) */

        if (TYPEOF(x) == STRSXP && LENGTH(x) > 0)
        { /* string means there was an error */
            const char *s = CHAR(STRING_ELT(x, 0));
            send_response(c->sock,
                          "HTTP/1.1 500 Evaluation error\r\nConnection: close\r\nContent-type: text/plain\r\n\r\n", 78);
    ...
    send_response(c->sock,
                  "HTTP/1.1 500 Invalid response from R\r\nConnection: close\r\nContent-type: text/plain\r\n\r\nServer "
