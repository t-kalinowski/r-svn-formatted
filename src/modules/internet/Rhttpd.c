...
static void uri_decode(char *s)
{
    ...
    while (*s)
    {
        ...
        else if (*s == '%')
        {
            ...
            s++;
            if (*s >= '0' && *s <= '9')
                ec |= ((unsigned char)(*s - '0')) << 4;
            else if (*s >= 'a' && *s <= 'f')
                ec |= ((unsigned char)(*s - 'a' + 10)) << 4;
            else if (*s >= 'A' && *s <= 'F')
                ec |= ((unsigned char)(*s - 'A' + 10)) << 4;
            if (*s)
                s++;
            if (*s >= '0' && *s <= '9')
                ec |= (unsigned char)(*s - '0');
            else if (*s >= 'a' && *s <= 'f')
                ec |= (unsigned char)(*s - 'a' + 10);
            else if (*s >= 'A' && *s <= 'F')
                ec |= (unsigned char)(*s - 'A' + 10);
            if (*s)
                s++;
...
static SEXP parse_query(char *query)
{
    ...
    while (1)
    {
        ...
        { /* we cannot use uri_decode becasue we need &/= *before* decoding */
            unsigned char ec = 0;
            s++;
            if (*s >= '0' && *s <= '9')
                ec |= ((unsigned char)(*s - '0')) << 4;
...
static SEXP handler_for_path(const char *path)
{
    ...
    { /* starts with /custom/ ? */
        ...
        while (*c && *c != '/')
            c++; /* find out the name */
        if (c - e > 0 && c - e < 64)
        { /* if it's 1..63 chars long, proceed */
            char fn[64];
            ...
            {
                if (!R_HandlersName)
                    R_HandlersName = install(".httpd.handlers.env");
...
static void process_request_(void *ptr)
{
    ...
    if (!c || !c->url)
        return; /* if there is not enough to process, bail out */
    s = c->url;
    while (*s && *s != '?')
        s++; /* find the query part */
    if (*s)
    {
        *(s++) = 0;
        query = s;
    ...
    {                   /* construct "try(httpd(url, query, body), silent=TRUE)" */
        ...
        SEXP y, x = PROTECT(lang3(sTry, LCONS(handler_for_path(c->url), sArgs), sTrue));
        ...
        { /* a list (generic vector) can be a real payload */
            ...
            if (LENGTH(x) > 1)
            {
                ...
                if (LENGTH(x) > 2)
                { /* third element is headers vector */
            ...
            if (TYPEOF(y) == STRSXP && LENGTH(y) > 0)
            {
                ...
                if (TYPEOF(xNames) == STRSXP && LENGTH(xNames) > 0 && !strcmp(CHAR(STRING_ELT(xNames, 0)), "file"))
                    fn = cs;
...
static char *remove_dot_segments(char *p)
{
    ...
    *out = '\0';
    ...
    while (*in)
    {
        ...
        */
        if ((in[0] == '.' && in[1] == '\0') || (in[0] == '.' && in[1] == '.' && in[2] == '\0'))
        {
            /* remove input */
            in[0] = '\0';
            continue;
...
static void worker_input_handler(void *data)
{
...
#ifndef _WIN32
    if (in_process)
        return; /* we don't allow recursive entrance */
#endif
    ...
    if (c->part < PART_BODY)
    {
        ...
        while (*s)
        {
            ...
            { /* single, empty line - end of headers */
                ...
                if (!(c->attr & HTTP_1_0) && !(c->attr & HOST_HEADER))
                ...
                if (c->body_pos)
                {
                    ...
                    c->line_pos -= c->body_pos; /* NOTE: we are NOT moving the buffer since non-zero left-over causes
                                                   connection close */
                }
            ...
            }
            {
                ...
                { /* complete line, great! */
                    ...
                    if (c->part == PART_REQUEST)
                    {
                        ...
                        if (!strncmp(bol + rll - 3, "1.0", 3))
                            c->attr |= HTTP_1_0;
                        if (!strncmp(bol, "GET ", 4))
                            c->method = METHOD_GET;
                        if (!strncmp(bol, "POST ", 5))
    ...
    { /* BODY  - this branch always returns */
        ...
        if (c->body_pos == c->content_length)
        { /* yay! we got the whole body */
            process_request(c);
            if (c->attr & CONNECTION_CLOSE || c->line_pos)
            { /* we have to close the connection if there was a double-hit */
                remove_worker(c);
                return;
            }
            /* keep-alive - reset the worker so it can process a new request */
...
static void srv_input_handler(void *data)
{
    ...
    if (c == NULL)
        error("allocation error in srv_input_handler");
    c->sock = cl_sock;
