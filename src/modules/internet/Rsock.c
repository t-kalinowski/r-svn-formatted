...
#ifdef Unix
        if ((!write && !FD_ISSET(sockfd, &rfd)) || (write && !FD_ISSET(sockfd, &wfd)) || howmany > 1)
        {
...
int R_SocketWaitMultiple(int nsock, int *insockfd, int *ready, int *write, double mytimeout)
{
    fd_set rfd, wfd;
    struct timeval tv;
    double used = 0.0;
    ...
    while (1)
    {
        ...
        { /* always poll occationally--not really necessary */
            tv.tv_sec = 60;
            tv.tv_usec = 0;
        }
#elif defined(Win32)
        ...
        {
            if (write[i])
                FD_SET(insockfd[i], &wfd);
            else
                FD_SET(insockfd[i], &rfd);
            if (maxfd < insockfd[i])
                maxfd = insockfd[i];
        }
        ...
        for (i = 0; i < nsock; i++)
            if ((!write[i] && FD_ISSET(insockfd[i], &rfd)) || (write[i] && FD_ISSET(insockfd[i], &wfd)))
            {
                ready[i] = 1; /* TRUE */
                nready++;
            }
...
int in_Rsockselect(int nsock, int *insockfd, int *ready, int *write, double timeout)
{
...
ssize_t R_SockRead(int sockp, void *buf, size_t len, int blocking, int timeout)
{
    ...
    if (blocking && R_SocketWait(sockp, 0, timeout) != 0)
        return 0;
...
ssize_t R_SockWrite(int sockp, const void *buf, size_t len, int timeout)
{
    ...
    do
    {
        if (R_SocketWait(sockp, 1, timeout) != 0)
            return out;
