...
static void *in_R_HTTPOpen(const char *url, const char *agent, const char *headers, int cacheOK);
...
#define Ri_HTTPRead(ctx, dest, len) (meth ? in_R_HTTPRead2(ctx, dest, len) : in_R_HTTPRead(ctx, dest, len))
...
#define Ri_FTPOpen(url) (meth ? in_R_FTPOpen2(url) : in_R_FTPOpen(url));

#define Ri_FTPRead(ctx, dest, len) (meth ? in_R_HTTPRead2(ctx, dest, len) : in_R_FTPRead(ctx, dest, len))
...
static size_t url_read2(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    UrlScheme type = ((Rurlconn)(con->private))->type;
    void *ctxt = ((Rurlconn)(con->private))->ctxt;
    size_t n = 0; /* -Wall */
...
static SEXP in_do_download(SEXP args)
{
    ...
    IDquiet = quiet = asLogical(CAR(args));
    args = CDR(args);
    if (quiet == NA_LOGICAL)
    ...
    )
    {
        ...
        UNPROTECT(1); /* utilsNS */
        PROTECT(sagent);
        const char *cagent = (TYPEOF(sagent) == NILSXP) ? NULL : CHAR(STRING_ELT(sagent, 0));
        /* TODO: flatten headers */
        const char *cheaders = (TYPEOF(sheaders) == NILSXP) ? NULL : CHAR(STRING_ELT(sheaders, 0));
                ...
                if (!quiet)
                {
...
#endif
                    {
                        if (guess <= 0)
                            ...
                            putdashes(&ndashes, (int)(50 * nbytes / guess));
                    }
                }
            }
            Ri_HTTPClose(ctxt);
        ...
        fclose(out);
        if (status == 1 && strchr(mode, 'w'))
            unlink(R_ExpandFileName(file));
        R_Busy(0);
        if (status == 1)
...
void *in_R_HTTPOpen(const char *url, const char *agent, const char *headers, int cacheOK)
{
    ...
    if (agent || headers)
    {
        fullheaders = malloc((agent ? strlen(agent) : 0) + (headers ? strlen(headers) : 0) + 1);
        if (!fullheaders)
            error(_("could not allocate memory for http headers"));
        fullheaders[0] = '\0';
        if (agent)
            strcat(fullheaders, agent);
        if (headers)
            strcat(fullheaders, headers);
    }
    ...
    if (ctxt != NULL)
    {
        ...
        if (rc != 200)
        {
            ...
            warning(_("cannot open URL '%s': %s status was '%d %s'"), url, "HTTP", rc, RxmlNanoHTTPStatusMsg(ctxt));
...
static void *in_R_HTTPOpen2(const char *url, const char *agent, const char *headers, int cacheOK)
{
    ...
    wictxt->length = -1;
    wictxt->type = NULL;
    wictxt->hand = InternetOpen(agent, INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
    if (!wictxt->hand)
    {
        free(wictxt);
    ...
    DWORD flags = INTERNET_FLAG_KEEP_CONNECTION | INTERNET_FLAG_NO_CACHE_WRITE | INTERNET_FLAG_IGNORE_REDIRECT_TO_HTTPS;
    if (!cacheOK)
    ...
    if (status != 200)
    {
        ...
        InternetCloseHandle(wictxt->hand);
        free(wictxt);
        warning(_("cannot open URL '%s': %s status was '%d %s'"), url, "HTTP", status, buf);
        return NULL;
    }
