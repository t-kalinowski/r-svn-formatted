...
#define Ri_HTTPOpen(url, headers, cacheOK)                                                                             \
    (meth ? in_R_HTTPOpen2(url, headers, cacheOK) : in_R_HTTPOpen(url, headers, cacheOK));
...
static SEXP in_do_download(SEXP args)
{
    ...
    )
    {
        ...
        UNPROTECT(1); /* utilsNS */
        PROTECT(sheaders);
        const char *headers = (TYPEOF(sheaders) == NILSXP) ? NULL : CHAR(STRING_ELT(sheaders, 0));
...
static void *in_R_HTTPOpen2(const char *url, const char *headers, const int cacheOK)
{
    ...
    wictxt->length = -1;
    wictxt->type = NULL;
    wictxt->hand = InternetOpen(headers, INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
    if (!wictxt->hand)
    {
        free(wictxt);
