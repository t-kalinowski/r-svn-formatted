...
static CURLMcode R_curl_multi_wait(CURLM *multi_handle,
                                   /* IGNORED */ void *unused,
                                   /* IGNORED */ unsigned int extra, int timeout_ms, int *ret)
{
    fd_set fdread;
    fd_set fdwrite;
    fd_set fdexcep;
...
static const char *ftp_errstr(const long status)
{
    ...
    {
    case 421:
        str = "Service not available, closing control connection";
        break;
    case 425:
        str = "Cannot open data connection";
        break;
    case 426:
        str = "Connection closed; transfer aborted";
        break;
    case 430:
        str = "Invalid username or password";
        break;
    case 434:
        str = "Requested host unavailable";
        break;
    case 450:
        str = "Requested file action not taken";
        break;
    case 451:
        str = "Requested action aborted; local error in processing";
        break;
    case 452:
        str = "Requested action not taken; insufficient storage space in system";
        break;
    case 501:
        str = "Syntax error in parameters or arguments";
        break;
    case 502:
        str = "Command not implemented";
        break;
    case 503:
        str = "Bad sequence of commands";
        break;
    case 504:
        str = "Command not implemented for that parameter";
        break;
    case 530:
        str = "Not logged in";
        break;
    case 532:
        str = "Need account for storing files";
        break;
    case 550:
        str = "Requested action not taken; file unavailable";
        break;
    case 551:
        str = "Requested action aborted; page type unknown";
        break;
    case 552:
        str = "Requested file action aborted; exceeded storage allocation";
        break;
    case 553:
        str = "Requested action not taken; file name not allowed";
        break;
    default:
        str = "Unknown Error";
        break;
    ...
    for (int n = 1; n > 0;)
    {
        ...
        if (msg && (msg->data.result != CURLE_OK))
        {
            ...
            if (status >= 400)
            {
                ...
                warning(_("cannot open URL '%s': %s status was '%d %s'"), url, type, status, strerr);
            }
            else
            {
                strerr = curl_easy_strerror(msg->data.result);
                if (streql(strerr, "Timeout was reached"))
                    warning(_("URL '%s': Timeout of %d seconds was reached"), url, current_timeout);
                else
...
static void curlCommon(CURL *hnd, int redirect, int verify)
{
    ...
    if (verify)
    {
        ...
        CURLcode ret = curl_easy_getinfo(hnd, CURLINFO_TLS_SSL_PTR, &tls_backend_info);
...
SEXP attribute_hidden in_do_curlGetHeaders(SEXP call, SEXP op, SEXP args, SEXP rho)
{
...
#else
        error("TLS argument is unsupported in this libcurl version %d.%d", LIBCURL_VERSION_MAJOR,
    ...
    if (ret != CURLE_OK)
    {
        if (errbuf[0])
            ...
            error(_("libcurl error code %d:\n\t%s\n"), ret, "unable to access SSL/TLS CA certificates");
...
SEXP attribute_hidden in_do_curlDownload(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
#ifndef HAVE_LIBCURL
    ...
    */
    ...
    for (int i = 0; i < nurls; i++)
    {
        ...
        {
            n_err += 1;
            warning(_("URL %s: cannot open destfile '%s', reason '%s'"), url, file, strerror(errno));
            if (nurls == 1)
                break;
            else
                continue;
    ...
    do
    {
        ...
        if (!numfds)
        {
            ...
            */
            if (repeats++ > 0)
                Rsleep(0.1); // do not block R process
        }
        else
            repeats = 0;
        R_ProcessEvents();
    ...
    {
        if (n_err == nurls)
            error(_("cannot download any files"));
        else if (n_err)
            warning(_("some files were not downloaded"));
    }
    else if (n_err)
    {
        if (status != 200)
...
static size_t rcvData(void *ptr, size_t size, size_t nitems, void *ctx)
{
    ...
    if (ctxt->filled)
        memmove(ctxt->buf, ctxt->current, ctxt->filled);
    ...
    if (add)
    {
        ...
        if (ctxt->filled + add > ctxt->bufsize)
        {
            ...
            if (!newbuf)
                error("Failure in re-allocation in rcvData");
            ctxt->buf = newbuf;
...
static size_t Curl_read(void *ptr, size_t size, size_t nitems, Rconnection con)
{
    RCurlconn ctxt = (RCurlconn)(con->private);
