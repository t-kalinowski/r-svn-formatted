...
static SEXP La_svd(SEXP jobu, SEXP x, SEXP s, SEXP u, SEXP vt)
{
    ...
    int lwork = -1;
    F77_CALL(dgesdd)
    (CHAR(STRING_ELT(jobu, 0)), &n, &p, xvals, &n, REAL(s), REAL(u), &ldu, REAL(vt), &ldvt, &tmp, &lwork, iwork, &info);
    if (info != 0)
        error(_("error code %d from Lapack routine '%s'"), info, "dgesdd");
    lwork = (int)tmp;
    double *work = (double *)R_alloc(lwork, sizeof(double));
    F77_CALL(dgesdd)
    (CHAR(STRING_ELT(jobu, 0)), &n, &p, xvals, &n, REAL(s), REAL(u), &ldu, REAL(vt), &ldvt, work, &lwork, iwork, &info);
    if (info != 0)
        error(_("error code %d from Lapack routine '%s'"), info, "dgesdd");
...
static SEXP La_zgecon(SEXP A, SEXP norm)
{
    ...
    (typNorm, &n, avals, &n, &anorm,
     /* rcond = */ REAL(val),
     /* work : */ (Rcomplex *)R_alloc(4 * (size_t)n, sizeof(Rcomplex)), rwork, &info);
    UNPROTECT(1);
    if (info)
...
static SEXP La_ztrcon(SEXP A, SEXP norm)
{
    ...
    (typNorm, "U", "N", &n, COMPLEX(A), &n, REAL(val),
     ...
     /* rwork: */ (double *)R_alloc(n, sizeof(double)), &info);
    UNPROTECT(1);
    if (info)
...
static SEXP qr_coef_cmplx(SEXP Q, SEXP Bin)
{
    ...
    if (info != 0)
        error(_("error code %d from Lapack routine '%s'"), info, "zunmqr");
    F77_CALL(ztrtrs)("U", "N", "N", &k, &nrhs, COMPLEX(qr), &n, COMPLEX(B), &n, &info);
    if (info != 0)
...
static SEXP La_svd_cmplx(SEXP jobu, SEXP jobv, SEXP x, SEXP s, SEXP u, SEXP v)
{
    ...
    double *rwork = (double *)R_alloc((f1 > f2 ? f1 : f2) * (size_t)mn0, sizeof(double));
    /* ask for optimal size of work array */
    int lwork = -1, info;
    Rcomplex tmp;
    int ldu, ldv;
    ...
    ldv = INTEGER(dims)[0];
    F77_CALL(zgesdd)
    (CHAR(STRING_ELT(jobu, 0)), &n, &p, xvals, &n, REAL(s), COMPLEX(u), &ldu, COMPLEX(v), &ldv, &tmp, &lwork, rwork,
     iwork, &info);
    if (info != 0)
    ...
    F77_CALL(zgesdd)
    (CHAR(STRING_ELT(jobu, 0)), &n, &p, xvals, &n, REAL(s), COMPLEX(u), &ldu, COMPLEX(v), &ldv, work, &lwork, rwork,
     iwork, &info);
    if (info != 0)
        error(_("error code %d from Lapack routine '%s'"), info, "zgesdd");
#else
    ...
    ldv = INTEGER(dims)[0];
    F77_CALL(zgesvd)
    (CHAR(STRING_ELT(jobu, 0)), CHAR(STRING_ELT(jobv, 0)), &n, &p, xvals, &n, REAL(s), COMPLEX(u), &ldu, COMPLEX(v),
     &ldv, &tmp, &lwork, rwork, &info);
    if (info != 0)
...
static SEXP qr_coef_real(SEXP Q, SEXP Bin)
{
    ...
    nrhs = Bdims[1];
    int lwork = -1, info;
    F77_CALL(dormqr)("L", "T", &n, &nrhs, &k, REAL(qr), &n, REAL(tau), REAL(B), &n, &tmp, &lwork, &info);
    if (info != 0)
        error(_("error code %d from Lapack routine '%s'"), info, "dormqr");
    lwork = (int)tmp;
    work = (double *)R_alloc(lwork, sizeof(double));
    F77_CALL(dormqr)("L", "T", &n, &nrhs, &k, REAL(qr), &n, REAL(tau), REAL(B), &n, work, &lwork, &info);
    if (info != 0)
        error(_("error code %d from Lapack routine '%s'"), info, "dormqr");
    F77_CALL(dtrtrs)("U", "N", "N", &k, &nrhs, REAL(qr), &n, REAL(B), &n, &info);
    if (info != 0)
...
static SEXP qr_qy_real(SEXP Q, SEXP Bin, SEXP trans)
{
    ...
    nrhs = Bdims[1];
    int lwork = -1, info;
    F77_CALL(dormqr)("L", tr ? "T" : "N", &n, &nrhs, &k, REAL(qr), &n, REAL(tau), REAL(B), &n, &tmp, &lwork, &info);
    if (info != 0)
        error(_("error code %d from Lapack routine '%s'"), info, "dormqr");
    lwork = (int)tmp;
    work = (double *)R_alloc(lwork, sizeof(double));
    F77_CALL(dormqr)("L", tr ? "T" : "N", &n, &nrhs, &k, REAL(qr), &n, REAL(tau), REAL(B), &n, work, &lwork, &info);
    if (info != 0)
        error(_("error code %d from Lapack routine '%s'"), info, "dormqr");
    UNPROTECT(1);
    return B;
