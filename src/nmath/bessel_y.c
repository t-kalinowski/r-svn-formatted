...
static void Y_bessel(double *x, double *alpha, int *nb, double *by, int *ncalc);
...
double bessel_y(double x, double alpha)
{
    ...
    if (!by)
        MATHLIB_ERROR("%s", _("bessel_y allocation error"));
#else
    vmax = vmaxget();
...
static void Y_bessel(double *x, double *alpha, int *nb, double *by, int *ncalc)
{
    ...
    /*----------------------------------------------------------------------
      ...
      1/gamma(1-x), abs(x) <= .5
      ----------------------------------------------------------------------*/
    const static double ch[21] = {-6.7735241822398840964e-24, -6.1455180116049879894e-23, 2.9017595056104745456e-21,
                                  1.3639417919073099464e-19,  2.3826220476859635824e-18,  -9.0642907957550702534e-18,
                                  -1.4943667065169001769e-15, -3.3919078305362211264e-14, -1.7023776642512729175e-13,
                                  9.1609750938768647911e-12,  2.4230957900482704055e-10,  1.7451364971382984243e-9,
                                  -3.3126119768180852711e-8,  -8.6592079961391259661e-7,  -4.9717367041957398581e-6,
                                  7.6309597585908126618e-5,   .0012719271366545622927,    .0017063050710955562222,
                                  -.07685284084478667369,     -.28387654227602353814,     .92187029365045265648};
    ...
    if (*nb > 0 && 0. <= nu && nu < 1.)
    {
        ...
        else if (ex < 3.)
        {
            ...
            while (fabs(g / (1. + fabs(ya))) + fabs(h / (1. + fabs(ya1))) > DBL_EPSILON)
            {
                f = (f * en + p + q) / (en * en - e);
                c *= (d / en);
                p /= en - nu;
