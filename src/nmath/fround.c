...
#define MAX_DIGITS (DBL_MAX_10_EXP + DBL_DIG)
    ...
    else if (dig > 0)
    {
        ...
        else if (dig <= DBL_MAX_10_EXP)
        { // both pow10 := 10^d and (x * pow10) do *not* overflow
            LDOUBLE pow10 = R_pow_di(10., dig);
            return sgn * (double)(nearbyint((double)(x * pow10)) / pow10);
        }
        else
        {                           // DBL_MAX_10_EXP < dig <= DBL_DIG - log10(x) : case of |x| << 1; ~ 10^-305
            int e10 = dig - max10e; // > 0
            LDOUBLE p10 = R_pow_di(10., e10), pow10 = R_pow_di(10., max10e);
