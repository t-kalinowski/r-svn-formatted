...
long double attribute_hidden pnbeta_raw(double x, double o_x, double a, double b, double ncp)
{
    ...
    gx = exp(a0 * log(x) + b * (x < .5 ? log1p(-x) : log(o_x)) - lbeta - log(a0));
    if (a0 > a)
...
double attribute_hidden pnbeta2(double x, double o_x, double a, double b, double ncp,
                                /* o_x  == 1 - x  but maybe more accurate */
...
double pnbeta(double x, double a, double b, double ncp, int lower_tail, int log_p)
{
#ifdef IEEE_754
