...
(cd ~/R/D/r-devel/debian-64-inst/src/nmath; gcc -I. -I../../src/include -I../../../R/src/include -I/usr/local/include
-DHAVE_CONFIG_H -DDEBUG_pnch -fopenmp -g -pedantic -Wall --std=gnu99 -DDEBUG_q -Wcast-align -Wclobbered  -c
...
double attribute_hidden pnchisq_raw(double x, double f, double theta /* = ncp */, double errmax, double reltol,
                                    int itrmax, Rboolean lower_tail, Rboolean log_p)
{
    ...
    { /* use 110 for Inf, as ppois(110, 80/2, lower.tail=FALSE) is 2e-20 */
        ...
        if (lower_tail && f > 0. && log(x) < M_LN2 + 2 / f * (lgamma(f / 2. + 1) + _dbl_min_exp))
        {
