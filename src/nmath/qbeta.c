...
double qbeta(double alpha, double p, double q, int lower_tail, int log_p)
{
    ...
    qbeta_raw(alpha, p, q, lower_tail, log_p, NA_LOGICAL, USE_LOG_X_CUTOFF, n_NEWTON_FREE, qbet);
    return qbet[0];
}
...
{
    Rboolean swap_choose = (swap_01 == NA_LOGICAL), swap_tail, log_, give_log_q = (log_q_cut == ML_POSINF),
...
L_Newton:
    ...
    { // find  log(xinbta) -- work in  u := log(x) scale
        ...
        for (i_pb = 0; i_pb < 1000; i_pb++)
        {
            ...
            R_ifDEBUG_printf("N(i=%2d): u=%#20.16g, pb(e^u)=%#12.6g, w=%#15.9g, %s prev=%11g,", i_pb, u, y, w,
                             (w * wprev <= 0.) ? "new" : "old", prev);
            g = 1;
        ...
        for (i_pb = 0; i_pb < 1000; i_pb++)
        {
            ...
            if (i_pb >= n_N && w * wprev <= 0.)
                prev = fmax2(fabs(adj), fpu);
            R_ifDEBUG_printf("N(i=%2d): x0=%#17.15g, pb(x0)=%#17.15g, w=%#17.15g, %s prev=%g,", i_pb, xinbta, y, w,
                             (w * wprev <= 0.) ? "new" : "old", prev);
            g = 1;
            for (i_inn = 0; i_inn < 1000; i_inn++)
            {
