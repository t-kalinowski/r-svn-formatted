...
double qchisq_appr(double p, double nu, double g /* = log Gamma(nu/2) */, int lower_tail, int log_p,
                   double tol /* EPS1 */)
{
#define C7 4.67
#define C8 6.66
...
#define pMAX (1 - 1e-14) /* was (1-1e-12) and 0.999998 = 1 - 2e-6 */
    ...
    const double i420 = 1. / 420., i2520 = 1. / 2520., i5040 = 1. / 5040;
...
#ifdef DEBUG_qgamma
    REprintf("qgamma(p=%7g, alpha=%7g, scale=%7g, l.t.=%2d, log_p=%2d): ", p, alpha, scale, lower_tail, log_p);
#endif
...
#ifdef DEBUG_qgamma
        if (i == 1)
            Rprintf(" Ph.II iter; ch=%g, p2=%g\n", ch, p2);
        if (i >= 2)
            Rprintf("     it=%d,  ch=%g, p2=%g\n", i, p2);
#endif
#ifdef IEEE_754
        if (!R_FINITE(p2))
#else
...
#ifdef DEBUG_qgamma
        if (i == 2)
            Rprintf(" p=%g, N{it=%d}, d{p}=%g\n", p, i, p_ - p);
        if (i >= 3)
            Rprintf("         it=%d,  d{p}=%g\n", i, p_ - p);
#endif
        if (fabs(p1) < fabs(EPS_N * p) || (g = dgamma(x, alpha, scale, FALSE)) == 0)
        {
            break;
        }
        else
