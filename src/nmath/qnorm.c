...
double qnorm(double p, double mu, double sigma, int lower_tail, int log_p)
{
    ...
    if (fabs(q) <= 0.42)
    {
        ...
        r = q * q;
        val = q * (((-25.44106049637 * r + 41.39119773534) * r - 18.61500062529) * r + 2.50662823884) /
    ...
    else
    {
        ...
        if (r > DBL_EPSILON)
        {
            r = sqrt(-(log_p ? (q > 0. ? R_D_Cval(p) : R_D_Lval(p)) : /*normal*/ log(r)));
            val = (((2.32121276858 * r + 4.85014127135) * r - 2.29796479134) * r - 2.78718931138) /
...
#define LOG (0)
    ...
    /* Final Newton step: */
    val = val - (pnorm(val, 0., 1., LOWER, LOG) - p_) / dnorm(val, 0., 1., LOG);
    return mu + sigma * val;
}
