...
RNGTAB RNG_Table[] = {
    ...
    {0, 0, "Wichmann-Hill", 0, 3, 123, dummy}, {1, 0, "Marsaglia-MultiCarry", 0, 2, 123, dummy},
    {2, 0, "Super-Duper", 0, 2, 123, dummy},   {3, 0, "Mersenne-Twister", 0, 1 + 624, 123, dummy},
...
double sunif(void)
{
    ...
    case MARSAGLIA_MULTICARRY: /* 0177777(octal) == 65535(decimal)*/
        ...
        return (do32bits(I1 << 16) ^ (I2 & 0177777)) * i2_32m1; /* in [0,1) */
...
void FixupSeeds(RNGtype kind)
{
    ...
    for (j = 0; j <= RNG_Table[kind].n_seed - 2; j++)
        if (!RNG_Table[kind].i_seed[j])
            RNG_Table[kind].i_seed[j]++;

    switch (kind)
    {
    case WICHMANN_HILL:
        if (RNG_Table[kind].i1_seed >= 30269 || RNG_Table[kind].i2_seed >= 30307 || RNG_Table[kind].i3_seed >= 30323)
...
{
    if (!RNG_Table[kind].is_seeded)
    { /* allocate ! */
#ifdef DEBUG
        MATHLIB_WARNING2("Allocating seed (length %d) for RNG kind '%d'\n", RNG_Table[kind].n_seed - 1, kind);
#endif
        RNG_Table[kind].i_seed = (Int32 *)calloc((size_t)RNG_Table[kind].n_seed - 1, sizeof(Int32));
