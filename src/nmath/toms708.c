...
void attribute_hidden bratio(double a, double b, double x, double y, double *w, double *w1, int *ierr, int log_p)
{
    /* -----------------------------------------------------------------------
    ...
    *w1 = R_D__0;
    ...
    R_ifDEBUG_printf("bratio(a=%g, b=%g, x=%9g, y=%9g, .., log_p=%d): ", a, b, x, y, log_p);
    *ierr = 0;
    if (x == 0.0)
        goto L200;
    if (y == 0.0)
...
static double bpser(double a, double b, double x, double eps, int log_p)
{
    ...
    { // the series did not converge (in time)
        ...
        if ((log_p && !(a * sum > -1. && fabs(log1p(a * sum)) < eps * fabs(ans))) ||
            ...
            MATHLIB_WARNING5(" bpser(a=%g, b=%g, x=%g,...) did not converge (n=1e7, |w|/tol=%g > 1; A=%g)", a, b, x,
                             fabs(w) / tol, ans);
    }
    R_ifDEBUG_printf("  -> n=%.0f iterations, |w|=%g %s %g=tol:=eps/a ==> a*sum=%g\n", n, fabs(w),
                     (fabs(w) > tol) ? ">!!>" : "<=", tol, a * sum);
    if (log_p)
    {
        if (a * sum > -1.0)
            ans += log1p(a * sum);
...
static double brcomp(double a, double b, double x, double y, int log_p)
{
    ...
    if (a0 < 8.0)
    {
        ...
        { /*		algorithm for max(a,b) = b0 <= 1 */
            ...
            return (log_p ? e_z + log(a0 * c) - log1p(a0 / b0) : e_z * (a0 * c) / (a0 / b0 + 1.0));
        }
        ...
        return (log_p ? log(a0) + z + log1p(gam1(b0)) - log(t) : a0 * exp(z) * (gam1(b0) + 1.0) / t);
    }
    else
    {
        /* ----------------------------------------------------------------------- */
...
static double brcmp1(int mu, double a, double b, double x, double y, int give_log)
{
    ...
    if (a0 < 8.0)
    {
        ...
        if (b0 >= 8.0)
        {
            ...
            return give_log ? log(a0) + esum(mu, z - u, TRUE) : a0 * esum(mu, z - u, FALSE);
        }
        else if (b0 <= 1.0)
        {
            //                   a0 < 1, b0 <= 1
            ...
            // L50:
            c = give_log ? log1p(gam1(a)) + log1p(gam1(b)) - log(z) : (gam1(a) + 1.0) * (gam1(b) + 1.0) / z;
            R_ifDEBUG_printf(" brcmp1(mu,a,b,*): a0 < 1, b0 <= 1;  c=%.15g\n", c);
            return give_log ? ans + log(a0) + c - log1p(a0 / b0) : ans * (a0 * c) / (a0 / b0 + 1.0);
        }
        ...
        // L72:
        return give_log ? log(a0) + esum(mu, z, TRUE) + log1p(gam1(b0)) - log(t) // TODO? log(t) = log1p(..)
...
static void bgrat(double a, double b, double x, double y, double *w, double eps, int *ierr, Rboolean log_w)
{
    /* -----------------------------------------------------------------------
...
#define n_terms_bgrat 30
    ...
    double bm1 = b - 0.5 - 0.5, nu = a + bm1 * 0.5, /* nu = a + (b-1)/2 =: T, in (9.1) of
    ...
    { // should not happen, but does, e.g.,
        ...
        MATHLIB_WARNING5("bgrat(a=%g, b=%g, x=%g, y=%g): z=%g, b*z == 0 underflow, hence inaccurate pbeta()", a, b, x,
                         y, z);
    ...
    for (int n = 1; n <= n_terms_bgrat; ++n)
    {
        ...
        if (fabs(dj) <= eps * (sum + l))
        {
            ...
            MATHLIB_WARNING5("bgrat(a=%g, b=%g, x=%g,..): did *not* converge; dj=%g, rel.err=%g\n", a, b, x, dj,
                             fabs(dj) / (sum + l));
    }
...
static double grat_r(double a, double x, double log_r, double eps)
{
    ...
    { /* L10:  Taylor series for  P(a,x)/x^a */
        ...
        double an = 3., c = x, sum = x / (a + 3.0), tol = eps * 0.1 / (a + 1.0), t;
        do
        {
            an += 1.;
            c *= -(x / an);
        ...
        double j = a * x * ((sum / 6. - 0.5 / (a + 2.)) * x + 1. / (a + 1.)), z = a * log(x), h = gam1(a), g = h + 1.0;
    ...
    else
    {
        ...
        double a2n_1 = 1.0, a2n = 1.0, b2n_1 = x, b2n = x + (1.0 - a), c = 1., am0, an0;
...
#define num_IT 20
    ...
    double s = 1.0, h2 = h * h, hn = 1.0, w = w0, znm1 = z, zn = z2;
...
static double alnrel(double a)
{
    ...
    if (fabs(a) > 0.375)
        return log(1. + a);
    // else : |a| <= 0.375
    static double p1 = -1.29418923021993, p2 = .405303492862024, p3 = -.0178874546012214, q1 = -1.62752256355323,
                  q2 = .747811014037616, q3 = -.0845104217945565;
    double t = a / (a + 2.0), t2 = t * t,
...
static double erf__(double x)
{
    ...
    static double a[5] = {7.7105849500132e-5, -.00133733772997339, .0323076579225834, .0479137145607681,
                          .128379167095513};
    static double b[3] = {.00301048631703895, .0538971687740286, .375795757275549};
    static double p[8] = {-1.36864857382717e-7, .564195517478974, 7.21175825088309, 43.1622272220567,
    ...
    static double q[8] = {1.,
                          ...
                          790.950925327898,
                          300.459260956983};
    static double r[5] = {2.10144126479064, 26.2370141675169, 21.3688200555087, 4.6580782871847, .282094791773523};
    static double s[4] = {94.153775055546, 187.11481179959, 99.0191814623914, 18.0124575948747};
...
static double erfc1(int ind, double x)
{
    ...
    if (ax <= 0.5)
    {
        double t = x * x, top = (((a[0] * t + a[1]) * t + a[2]) * t + a[3]) * t + a[4] + 1.0,
               bot = ((b[0] * t + b[1]) * t + b[2]) * t + 1.0;
...
static double gam1(double a)
{
    ...
    { /* t > 0;  L20: */
        static double p[7] = {.577215664901533,  -.409078193005776,   -.230975380857675,  .0597275330452234,
                              .0076696818164949, -.00514889771323592, 5.89597428611429e-4},
                      q[5] = {1., .427569613095214, .158451672430138, .0261132021441447, .00423244297896961};
...
static double psi(double x)
{
    ...
    static double p1[7] = {.0089538502298197, 4.77762828042627, 142.441585084029, 1186.45200713425,
                           3633.51846806499,  4138.10161269013, 1305.60269827897};
    static double q1[6] = {44.8452573429826, 520.752771467162, 2210.0079924783,
    ...
    static double p2[4] = {-2.12940445131011, -7.01677227766759, -4.48616543918019, -.648157123766197};
    static double q2[4] = {32.2703493791143, 89.2920700481861, 54.6117738103215, 7.77788548522962};
    /* --------------------------------------------------------------------- */
...
static double betaln(double a0, double b0)
{
    ...
    else
    {
        ...
        double w = bcorr(a, b), h = a / b, u = -(a - 0.5) * log(h / (h + 1.0)), v = b * alnrel(h);
        if (u > v)
