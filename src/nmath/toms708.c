...
#define SET_0_swap                                                                                                     \
    ...
    { /* L30: -------------------- both  a, b > 1  {a0 > 1  &  b0 > 1} ---*/
        ...
        lambda = R_FINITE(a + b) ? ((a > b) ? (a + b) * y - b : a - (a + b) * x) : a * y - b * x;
        do_swap = (lambda < 0.);
        if (do_swap)
        {
            lambda = -lambda;
...
static double bfrac(double a, double b, double x, double y, double lambda, double eps, int log_p)
{
    ...
    if (!R_FINITE(lambda))
        return ML_NAN; // TODO: can return 0 or 1 (?)
...
static double brcomp(double a, double b, double x, double y, int log_p)
{
    ...
    if (a0 < 8.)
    {
        ...
        { /*		algorithm for max(a,b) = b0 <= 1 */
            ...
            return (log_p ? e_z + log(a0 * c) - log1p(a0 / b0) : e_z * (a0 * c) / (a0 / b0 + 1.));
        }
        ...
        return (log_p ? log(a0) + z + log1p(gam1(b0)) - log(t) : a0 * exp(z) * (gam1(b0) + 1.) / t);
    }
    else
    {
        /* ----------------------------------------------------------------------- */
...
static double brcmp1(int mu, double a, double b, double x, double y, int give_log)
{
    ...
    if (a0 < 8.)
    {
        ...
        else if (b0 <= 1.)
        {
            ...
            // L50:
            c = give_log ? log1p(gam1(a)) + log1p(gam1(b)) - log(z) : (gam1(a) + 1.) * (gam1(b) + 1.) / z;
...
#define n_terms_bgrat 30
    ...
    double q_r = grat_r(b, z, log_r, eps), // = q/r of former grat1(b,z, r, &p, &q)
        v = 0.25 / (nu * nu), t2 = lnx * 0.25 * lnx, j = q_r, sum = j, t = 1., cn = 1., n2 = 0.;
    for (int n = 1; n <= n_terms_bgrat; ++n)
    {
        ...
        { // never? ; please notify R-core if seen:
            *ierr = 4;
            MATHLIB_WARNING5("bgrat(a=%g, b=%g, x=%g) *no* convergence: NOTIFY R-core!\n dj=%g, rel.err=%g\n", a, b, x,
                             dj, fabs(dj) / (sum + l));
        }
    } // for(n .. n_terms..)
...
static double grat_r(double a, double x, double log_r, double eps)
{
    ...
    { /* L10:  Taylor series for  P(a,x)/x^a */
        ...
        double j = a * x * ((sum / 6. - 0.5 / (a + 2.)) * x + 1. / (a + 1.)), z = a * log(x), h = gam1(a), g = h + 1.;
    ...
    else
    {
        ...
        double a2n_1 = 1., a2n = 1., b2n_1 = x, b2n = x + (1. - a), c = 1., am0, an0;
...
static double erfc1(int ind, double x)
{
    ...
    if (ax <= 0.5)
    {
        double t = x * x, top = (((a[0] * t + a[1]) * t + a[2]) * t + a[3]) * t + a[4] + 1.,
               bot = ((b[0] * t + b[1]) * t + b[2]) * t + 1.;
...
static double betaln(double a0, double b0)
{
    ...
    else
    {
        ...
        double w = bcorr(a, b), h = a / b, u = -(a - 0.5) * log(h / (h + 1.)), v = b * alnrel(h);
        if (u > v)
