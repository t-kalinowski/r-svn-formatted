...
static void doSpreadActive(DEEvent *myevent, WindowPtr whichWindow, short windowCode)
{
...
{
    setlineattribs(lwd);
    drawrectangle(ccol * box_w + lwd - 1, hwidth + crow * box_h + lwd - 1, box_w - lwd + 1, box_h - lwd + 1);
    Rsync();
}
...
void printstring(char *ibuf, int buflen, int row, int col)
{
    ...
    cleararea(col * box_w + 2, hwidth + row * box_h + 2, box_w - 3, box_h - 3);
...
static int initwin(void)
{
    ...
    SetRect(&dedragRect, screenBits.bounds.left + 4, screenBits.bounds.top + 24, screenBits.bounds.right - 4,
            screenBits.bounds.bottom - 4);

    FlushEvents(everyEvent, 0);

    dataentryWindow = NewWindow(0L, &dewindowBounds, "\pR Data Entry", true, documentProc, (WindowPtr)-1L, false, 0);
...
static void copyarea(int src_x, int src_y, int dest_x, int dest_y)
{
    XCopyArea(iodisplay, iowindow, iowindow, iogc, src_x, src_y, windowWidth - src_x, windowHeight - src_y, dest_x,
              dest_y);
    Rsync();
}
