...
static void PicTeX_ClipLine(double x0, double y0, double x1, double y1)
{
    ...
    if ((clippedx0 < clipleft && clippedx1 < clipleft) || (clippedx0 > clipright && clippedx1 > clipright) ||
        (clippedy0 < clipbottom && clippedy1 < clipbottom) || (clippedy0 > cliptop && clippedy1 > cliptop))
    {
        clippedx0 = clippedx1;
        clippedy0 = clippedy1;
        return;
    ...
    {
        clippedy0 = ((clippedy1 - clippedy0) / (clippedx1 - clippedx0) * (clipleft - clippedx0)) + clippedy0;
        clippedx0 = clipleft;
    }
    if (clippedx1 <= clipleft && clippedx0 > clipleft)
    {
        clippedy1 = ((clippedy1 - clippedy0) / (clippedx1 - clippedx0) * (clipleft - clippedx0)) + clippedy0;
        clippedx1 = clipleft;
    }
    /* Clipping Right */
    if (clippedx1 >= clipright && clippedx0 < clipright)
    {
        clippedy1 = ((clippedy1 - clippedy0) / (clippedx1 - clippedx0) * (clipright - clippedx0)) + clippedy0;
        clippedx1 = clipright;
    }
    if (clippedx1 <= clipright && clippedx0 > clipright)
    {
        clippedy0 = ((clippedy1 - clippedy0) / (clippedx1 - clippedx0) * (clipright - clippedx0)) + clippedy0;
        clippedx0 = clipright;
    }
    /*Clipping Bottom */
    if (clippedy1 >= clipbottom && clippedy0 < clipbottom)
    {
        clippedx0 = ((clippedx1 - clippedx0) / (clippedy1 - clippedy0) * (clipbottom - clippedy0)) + clippedx0;
        clippedy0 = clipbottom;
    }
    if (clippedy1 <= clipbottom && clippedy0 > clipbottom)
    {
        clippedx1 = ((clippedx1 - clippedx0) / (clippedy1 - clippedy0) * (clipbottom - clippedy0)) + clippedx0;
        clippedy1 = clipbottom;
    }
    /*Clipping Top */
    if (clippedy1 >= cliptop && clippedy0 < cliptop)
    {
        clippedx1 = ((clippedx1 - clippedx0) / (clippedy1 - clippedy0) * (cliptop - clippedy0)) + clippedx0;
        clippedy1 = cliptop;
    }
    if (clippedy1 <= cliptop && clippedy0 > cliptop)
    {
        clippedx0 = ((clippedx1 - clippedx0) / (clippedy1 - clippedy0) * (cliptop - clippedy0)) + clippedx0;
        clippedy0 = cliptop;
    }
}
...
static void PicTeX_Circle(double x, double y, double r, int col, int border)
{
    fprintf(texfp, "\\circulararc 360 degrees from %.2f %.2f center at %.2f %.2f\n", x, (y + r), x, y);
}
...
static void PicTeX_Polygon(int n, double *x, double *y, int bg, int fg)
{
    int i;
    if (debug)
        fprintf(texfp, "%% Drawing polygon with %d vertices.\n", n);
...
static void PicTeX_Text(double x, double y, char *str, double xc, double yc, double rot)
{
    int size;
    ...
    SetFont(GP->font, size);
    if (debug)
        fprintf(texfp, "%% Writing string of length %.2f, at %.2f %.2f, xc = %.2f yc = %.2f\n",
    ...
    if (rot == 90)
        ...
        fprintf(texfp, " [lB] <%.2fpt,%.2fpt>", xoff, yoff);
...
int PicTeXDeviceDriver(char **cpars, int ncpars, double *npars, int nnpars)
{
    double xoff, yoff;
    char *pagetype;
    DevInit = 0;
