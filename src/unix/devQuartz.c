...
unsigned char NotSymbols[] = {32, 33, 37, 38, 40, 41, 42, 43, 44, 58, 60, 61, 62, 91, 93, 95, 123, 125};
...
unsigned char Lat2Uni[] = {
    ...
    77,  /* Zeta */
    33,
    /* bracketleft */ /* Miscellaneous Special Characters */
    92,               /* therefore */
    35,               /* bracketright */
    140,              /* perpendicular */
    32,               /* underscore */
...
extern Rboolean CocoaInnerQuartzDevice(NewDevDesc *dd, char *display, double width, double height, double pointsize,
                                       char *family, Rboolean antialias, Rboolean autorefresh, int quartzpos, int bg);

extern void CocoaGetQuartzParameters(double *width, double *height, double *ps, char *family, Rboolean *antialias,
...
static void Quartz_Text(double x, double y, char *str, double rot, double hadj, R_GE_gcontext *gc, NewDevDesc *dd);
static void Quartz_Rect(double x0, double y0, double x1, double y1, R_GE_gcontext *gc, NewDevDesc *dd);
static void Quartz_Circle(double x, double y, double r, R_GE_gcontext *gc, NewDevDesc *dd);
static void Quartz_Line(double x1, double y1, double x2, double y2, R_GE_gcontext *gc, NewDevDesc *dd);
static void Quartz_Polyline(int n, double *x, double *y, R_GE_gcontext *gc, NewDevDesc *dd);
static void Quartz_Polygon(int n, double *x, double *y, R_GE_gcontext *gc, NewDevDesc *dd);
...
static void Quartz_MetricInfo(int c, R_GE_gcontext *gc, double *ascent, double *descent, double *width, NewDevDesc *dd);
...
{
    if (useaqua)
        return CocoaInnerQuartzDevice((NewDevDesc *)dd, display, width, height, pointsize, family, antialias,
                                      autorefresh, quartzpos, bg);
    else
        return innerQuartzDeviceDriver((NewDevDesc *)dd, display, width, height, pointsize, family, antialias,
                                       autorefresh, quartzpos, bg);
}
...
static Rboolean Quartz_Open(NewDevDesc *dd, QuartzDesc *xd, char *dsp, double wid, double hgt, int bg)
{
...
char *Quartz_TranslateFontFamily(char *family, int face, char *devfamily)
{
    ...
    PROTECT_WITH_INDEX(quartzenv = findVar(install(".Quartzenv"), graphicsNS), &xpi);
...
static void Quartz_SetStroke(int color, double gamma, NewDevDesc *dd)
{
    ...
    CGContextSetRGBStrokeColor(GetContext(xd), (float)R_RED(color) / 255.0, (float)R_GREEN(color) / 255.0,
                               (float)R_BLUE(color) / 255.0, (float)R_ALPHA(color) / 255.0);
}
...
static void Quartz_SetFill(int fill, double gamma, NewDevDesc *dd)
{
    ...
    CGContextSetRGBFillColor(GetContext(xd), (float)R_RED(fill) / 255.0, (float)R_GREEN(fill) / 255.0,
                             (float)R_BLUE(fill) / 255.0, (float)R_ALPHA(fill) / 255.0);
}
