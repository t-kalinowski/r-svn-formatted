...
static XFontStruct *RLoadFont(int face, int size)
{
    ...
    {
        static int near[] = {20, 24, 24, 25, 25, 25, 34, 34, 34};
        /* 15 16 17 18 19 20 21 22 23 */
        if (ADOBE_SIZE(size))
            return NULL; /* tried it */
...
#define SETGAMMA
    ...
    XChangeProperty(display, xd->window, XA_WM_NAME, XA_STRING, 8, PropModeReplace, (unsigned char *)"R Graphics", 13);
                    ...
                    int colormodel, int maxcube)
{
    ...
    dd->dp.cra[0] = xd->font->max_bounds.rbearing - xd->font->min_bounds.lbearing;
    dd->dp.cra[1] = xd->font->max_bounds.ascent + xd->font->max_bounds.descent;
