...
static void SetColor(int color, DevDesc *dd)
{
    ...
    if (color != xd->col)
    {
        ...
        if (NColors == 255 || XAllocColor(display, cmap, &(xd->fgcolor)) == 0)
...
static int X11_Open(DevDesc *dd, x11Desc *xd, char *dsp, double w, double h)
{
    ...
    if ((xd->window = XCreateWindow(display, rootWindow, DisplayWidth(display, screen) - iw - 10, 10, iw, ih, 1,
...
int X11DeviceDriver(DevDesc *dd, SEXP rcpars, int ncpars, SEXP rnpars, int nnpars)
{
