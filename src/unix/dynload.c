...
static void add_ptr(void *p, char *name)
{
...
static void HashInstall(char *name, DL_FUNC func, void *libhandle)
{
    int key;
    HashTabElem *newptr;
...
static DL_FUNC HashLookup(char *symbol)
{
    int key;
    HashTabElem *ptr;
    key = HashCode(symbol);
...
static void RemoveFromHashTable(void *handle)
{
    int key;
