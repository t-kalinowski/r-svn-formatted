...
extern DL_FUNC ptr_X11DeviceDriver, ptr_dataentry, ptr_R_GetX11Image, ptr_R_loadhistory, ptr_R_savehistory;
...
/* cannot use computeDLOpenFlag as warnings will crash R at this stage */
    ...
    if (!ptr_X11DeviceDriver)
        R_Suicide("Cannot load X11DeviceDriver");
    ptr_dataentry = R_dlsym(&dll, "RX11_dataentry");
    if (!ptr_dataentry)
        R_Suicide("Cannot load do_dataentry");
    ptr_R_GetX11Image = R_dlsym(&dll, "R_GetX11Image");
    if (!ptr_R_GetX11Image)
        R_Suicide("Cannot load R_GetX11Image");
}
...
/* cannot use computeDLOpenFlag as warnings will crash R at this stage */
    ...
    if (!ptr_R_Suicide)
        Rstd_Suicide("Cannot load R_Suicide");
    ptr_R_ShowMessage = R_dlsym(&dll, "Rgnome_ShowMessage");
    if (!ptr_R_ShowMessage)
        R_Suicide("Cannot load R_ShowMessage");
    ...
    if (!ptr_R_savehistory)
        R_Suicide("Cannot load Rgnome_savehsitoryr");
