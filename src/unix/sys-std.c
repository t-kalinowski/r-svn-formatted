...
fd_set *R_checkActivityEx(int usec, int ignore_stdin, void (*intr)(void))
{
    ...
    if (R_SelectEx(maxfd + 1, &readMask, NULL, NULL, (usec >= 0) ? &tv : NULL, intr))
        return (&readMask);
    else
        return (NULL);
}
 ...
 */
void pushReadline(char *prompt, rl_vcpfunc_t f)
{
...
static void readline_handler(char *line)
{
    ...
    if (line[0])
    {
        ...
        if (strlen(line) && rl_top->readline_addtohistory)
            add_history(line);
#endif
        l = (((rl_top->readline_len - 2) > strlen(line)) ? strlen(line) : (rl_top->readline_len - 2));
...
int Rstd_ReadConsole(char *prompt, unsigned char *buf, int len, int addtohistory)
{
    if (!R_Interactive)
    {
        ...
        /* according to system.txt, should be terminated in \n, so check this
           at eof and error */
        if ((err || feof(stdin)) && (ll == 0 || buf[ll - 1] != '\n') && ll < len)
        {
            buf[ll++] = '\n';
            buf[ll] = '\0';
        }
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            what = R_checkActivityEx(R_wait_usec ? R_wait_usec : -1, 0, handleInterrupt);
