...
static struct
{
    ...
    sigset_t oldset;
    struct sigaction oldalrm, oldint, oldquit, oldhup, oldterm, oldttin, oldttou, oldchld;
...
static FILE *R_popen_timeout(const char *cmd, const char *type, int timeout)
{
    ...
    begincontext(&tost.cntxt, CTXT_CCODE, R_NilValue, R_BaseEnv, R_BaseEnv, R_NilValue, R_NilValue);
...
static void warn_status(const char *cmd, int res)
{
    ...
    if (errno)
        ...
        warning(_("running command '%s' had status %d and error message '%s'"), cmd, res, strerror(errno));
    else
...
static void NORET cmdError(const char *cmd, const char *format, ...)
{
    ...
    va_list(ap);
    va_start(ap, format);
    SEXP cond = R_vmakeErrorCondition(call, "cmdError", NULL, nextra, format, ap);
    va_end(ap);
...
attribute_hidden SEXP do_system(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    { /* intern = TRUE */
        ...
        size_t buf_len = 0;
#else
             buf[INTERN_BUFSIZE + MB_LEN_MAX], buf2[MB_LEN_MAX + 1]; /* overflow bytes from MBCS truncation */
#endif
        int i, j, res;
        SEXP tchar, rval;
        ...
        if (!fp)
            cmdError(cmd, _("cannot popen '%s', probable reason '%s'"), cmd, strerror(errno));
#ifdef HAVE_GETLINE
        size_t read;
        ...
        { /* 127, aka -1 */
            if (errno)
                cmdError(cmd, _("error in running command: '%s'"), strerror(errno));
            else
...
void R_ProcessEvents(void)
{
    ...
    if (ptr_R_ProcessEvents && !R_isForkedChild)
        ptr_R_ProcessEvents();
#else
