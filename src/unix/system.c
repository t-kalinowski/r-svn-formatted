...
#if defined(HAVE_SYS_RESOURCE_H) && defined(HAVE_GETRLIMIT)
    {
        ...
        */
...
#if defined(HAVE_LIBC_STACK_END)
        {
            ...
            if (f)
            {
                for (;;)
                {
                    ...
                    if (fscanf(f, "%lx-%lx", &start, &end) == 2 && R_CStackStart >= (uintptr_t)start &&
        ...
        if (R_CStackStart == (uintptr_t)(-1))
            R_CStackLimit = (uintptr_t)(-1); /* never set */
...
#define R_INIT_TREAT_F(_AV_)                                                                                           \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            snprintf(msg, 1024, _("path given in -f/--file is too long"));                                             \
            R_Suicide(msg);                                                                                            \
        }                                                                                                              \
    ...
    { /* had at least one -e option */
        ...
        if (res != 1)
            error("fwrite error in initialize_R");
        fflush(ifp);
        rewind(ifp);
    }
