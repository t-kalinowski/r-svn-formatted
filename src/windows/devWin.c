...
static int Win_Locator(double *, double *);
static void Win_Mode(int);
static void Win_NewPage(DevDesc *);
static int Win_Open(DevDesc *, winDesc *, char *, double, double);
...
static void Win_Text(double, double, int, char *, double, double, double, DevDesc *);
...
static void SetLineType(int newlty, double nlwd, DevDesc *dd)
{
    ...
    {
        RCurrentPen = CreatePen(newlty, nlwd, RPenCol);
        if (RCurrentPen == NULL)
        ...
        else
        {
            ...
            wd->lwd = nlwd;
            Ghdc = GetDC(wd->window);
            DeleteObject(SelectObject(Ghdc, RCurrentPen));
            ReleaseDC(wd->window, Ghdc);
        }
    }
}
...
static int Win_Open(DevDesc *dd, winDesc *wd, double width, double height)
{
    ...
    if ((int)width < (r.right - r.left) && (int)height < (r.bottom - r.top))
    {
        iw = (int)width;
        ih = (int)height;
    }
    ...
    wd->window = (HWND)(UINT)SendMessage(RClient, WM_MDICREATE, 0, (LONG)(LPMDICREATESTRUCT)&mdicreate);
    if (RGraphWnd == NULL)
        return 0;

    SetWindowWord(wd->window, 0, dd);
...
static void Win_MetricInfo(int c, double *ascent, double *descent, double *width, DevDesc *dd)
{
    int first, last;
...
{
    HDC Ghdc;
    double Clipxl, Clipxr, Clipyb, Clipyt;
    winDesc *wd = (winDesc *)dd->deviceSpecific;
    ...
    IntersectClipRect(Ghdc, wd->clip.left, wd->clip.top, wd->clip.right, wd->clip.bottom);
...
static void Win_NewPage(DevDesc *dd)
{
    ...
    Ghdc = GetDC(wd->window);
    SelectClipRgn(Ghdc, NULL);
    FillRect(Ghdc, wd->graphics, GetStockObject(WHITE_BRUSH));
    ReleaseDC(wd->window, Ghdc);
}
...
static void Win_Rect(double x0, double y0, double x1, double y1, int coords, int bg, int fg, DevDesc *dd)
{
    double tmp;
    HDC Ghdc;
    winDesc *wd = (winDesc *)dd->deviceSpecific;
...
static void Win_Circle(double x, double y, int coords, double r, int col, int border, DevDesc *dd)
{
    int ir, ix, iy;
    HDC Ghdc;
    winDesc *wd = (winDesc *)dd->deviceSpecific;
    ...
    Ghdc = GetDC(wd->window);
    RCirclehelper(Ghdc, ix, iy, ir, bg, fg);
    ReleaseDC(wd->window, Ghdc);
}
...
static void Win_Polygon(int n, double *x, double *y, int bg, int fg)
{
    ...
    pt = (POINT *)C_alloc(n + 1, sizeof(POINT));
...
static void Win_Text(double x, double y, int coords, char *str, double xc, double yc, double rot, DevDesc *dd)
{
    HDC Ghdc;
    int fsize, nstr;
    double rotrad, xl, yl;
...
int WinDeviceDriver(DevDesc *dd, double width, double height, double pointsize)
{
    ...
    dd->dp.left = 0;                  /* left */
    dd->dp.right = wd->grap.right;    /* right */
    ...
    dd->dp.ipr[0] = 1 / (double)GetDeviceCaps(DDhdc, LOGPIXELSX);
    dd->dp.ipr[1] = 1 / (double)GetDeviceCaps(DDhdc, LOGPIXELSY);
 ...
 */
LRESULT FAR PASCAL GraphWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    PAINTSTRUCT ps;
    HDC thdc;
    DevDesc *dd;
    ...
    case WM_SIZE:
        ...
        {
            dd = GetWindowWord(hWnd, 0);
            wd = (winDesc *)dd->deviceSpecific;
            if (HIWORD(lParam) != wd->graph.bottom || LOWORD(lParam) != wd->graph.right)
            {
    ...
    case WM_PAINT:
        ...
        if (RGBhdc != NULL)
        {
            ...
            BitBlt(thdc, 0, 0, graphicsRect.right, graphicsRect.bottom, RGBhdc, 0, 0, SRCCOPY);
    ...
    case WM_MDIACTIVATE:
        if ((HWND)lParam == hWnd)
        {
            SendMessage(RClient, WM_MDISETMENU, (WPARAM)RMenuGraph, (LPARAM)RMenuGraphWin);
            DrawMenuBar(RFrame);
...
void doGraphicsMenu(HWND GWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    case RRR_PRINT:
        ...
        { /* one of the reasons I really hate Windows */
            ...
            hDIB = GlobalAlloc(GHND, sizeof(BITMAPINFOHEADER) + 16 * sizeof(RGBQUAD));
            lpbi = (BITMAPINFOHEADER *)GlobalLock(hDIB);
            *lpbi = bi;
            hdc = GetDC(RGraphWnd);
            i = GetDIBits(hdc, RGBitMap, 0, bmp.bmHeight, lpBits, (LPBITMAPINFO)lpbi, DIB_RGB_COLORS);
            ReleaseDC(RGraphWnd, hdc);
