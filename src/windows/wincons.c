...
int EventLoop()
{
    ...
    else
    {
        ...
        {
            if (!TranslateMDISysAccel(FrameClient, &msg) && !TranslateAccelerator(RFrame, hAccel, &msg))
            {
...
void WinConfig(void)
{
    ...
    SendMessage((HWND)-1, WM_DDE_INITIATE, (WPARAM)RFrame, MAKELONG(atomApp, atomApp));
    ...
    if (!PostMessage(hWndServerDDE, WM_DDE_EXECUTE, (WPARAM)RFrame, PackDDElParam(WM_DDE_EXECUTE, 0, (UINT)hCommand)))
    {
        GlobalFree(hCommand);
        goto errcd;
    }

    PostMessage(hWndServerDDE, WM_DDE_TERMINATE, PackDDElParam(WM_DDE_TERMINATE, (UINT)RFrame, 0), 0);

    /* GlobalFree(hCommand);  */
    return;

errcd:
    MessageBox(RFrame, "Cannot configure for this machine.", "R Application", MB_ICONEXCLAMATION | MB_OK);
    return;
}
/*
...
BOOL InitInstance(HINSTANCE hinstCurrent, int nCmdShow)
{
    ...
    RFrame = CreateWindow(RFrameClass, "R", WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_MAXIMIZE | WS_CLIPSIBLINGS,
                          CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, RMenuConsole, hinstCurrent,
                          NULL);
    ...
    RConsoleFrame = (HWND)(UINT)SendMessage(RClient, WM_MDICREATE, 0, (LONG)(LPMDICREATESTRUCT)&mdicreate);
...
HWND CreateConsoleWind(HWND hWndParent, HMENU hMenu, HANDLE hInstance)
{
    ...
    SendMessage(hWnd, WM_SETFONT, (WPARAM)GetStockObject(OEM_FIXED_FONT), MAKELPARAM(TRUE, 0L));
    return (hWnd);
}
/* win = 1 ; console
   win = 2 ; editor
...
LRESULT CALLBACK ConsoleWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CLOSE: /* disallow it if we aren't coming through quit */
        if (!RClosing)
        {
            MessageBox(hWnd, "You can't close the console", "R Console", MB_OK | MB_ICONEXCLAMATION);
            return 0;
        }
        break;
    case WM_COMMAND:
        ...
        case RRR_UNDO:
            SysBeep();
            MessageBox(hWnd, "Command Not Implemented", "R Console", MB_OK | MB_ICONEXCLAMATION);
            break;
        case RRR_QBROW:
            if (R_BrowseLevel > 0)
                InFlag = 2;
    ...
    case WM_MDIACTIVATE:
        if ((HWND)lParam == hWnd)
        {
            SendMessage(RClient, WM_MDISETMENU, (WPARAM)RMenuConsole, (LPARAM)RMenuConsWin);
            DrawMenuBar(RFrame);
        }
        return (0);
    }
...
LRESULT FAR PASCAL MainWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        RClosing = 0;
        RClient = CreateWindow("MDICLIENT", NULL, WS_CHILD | WS_CLIPCHILDREN | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL, 0,
                               0, 0, 0, hWnd, (HMENU)1, RInst, (LPSTR)&cltcr);
        return 0;
    case WM_DDE_ACK:
        if (hWndServerDDE == NULL)
    ...
    case WM_COMMAND:
        ...
        {
        case RRR_ABOUT:
            DialogBox(RInst, "AboutBox", hWnd, (DLGPROC)About);
            return (0);
        case RRR_TILE:
...
extern BOOL CALLBACK Menu_SetMem(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    char memval[20], *p;
    int nsize, vsize;
