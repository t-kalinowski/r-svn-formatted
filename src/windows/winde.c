...
int findsquare()
{
    ...
    if (yw < box_h)
        ...
        {
            if (ccol != wcol)
            { /* if it is a new col set the focus to it */
                closerect();
                ccol = wcol;
                crow = 1;
                activaterect(1);
...
static void setattribsfromwindow()
{
    ...
    if ((nwide * box_w != windowWidth) || (nhigh * box_h != windowHeight))
    {
        ...
        MoveWindow(RDEWnd, /*r.left, r.top,*/
                   20, 20, (r.right - r.left) - windowWidth + i, (r.bottom - r.top) - windowHeight + j, TRUE);
        GetClientRect(RDEWnd, &DERect);
        windowWidth = DERect.right - DERect.left;
        windowHeight = DERect.bottom - DERect.top;
    }
...
static int DEInit(void)
{
    ...
    RDEWnd = (HWND)(UINT)SendMessage(RClient, WM_MDICREATE, 0, (LONG)(LPMDICREATESTRUCT)&mdicreate);
    if (RDEWnd == NULL)
        return 0;
...
LRESULT FAR PASCAL RDEWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    HDC hdc;
    PAINTSTRUCT ps;
    int i;
    ...
    case WM_SETFOCUS:
        SetFocus(hWnd);
        if (IsWindow(RDEWnd))
            SendMessage(RDEWnd, WM_MDIACTIVATE, (WPARAM)NULL, (LPARAM)RDEWnd);
        break;
    case WM_MDIACTIVATE:
        if ((HWND)lParam == hWnd)
        {
        ...
        {
            MessageBox(hWnd, "You must quit the data entry window before doing anything else", "R Data Entry",
    ...
    case WM_SYSCOMMAND: /* Disable minimizing */
        ...
        {
            MessageBox(hWnd, "You can't iconify the Data Entry Window", "R Data Entry", MB_OK | MB_ICONEXCLAMATION);
            return 1l;
        }
        break;
    case WM_CLOSE:
...
extern BOOL CALLBACK DEVar(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    char name[20];
    static SEXP tvec;
    int type, levs;
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            if (!validName(name))
            {
                MessageBox(RDEWnd, "The variable name is not valid", "R Application", MB_ICONEXCLAMATION | MB_OK);
                name[0] = '\0';
                SetDlgItemText(hDlg, RDD_NAME, name);
                return 1;
            }
