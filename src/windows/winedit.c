...
int InitEditor(void)
{
    ...
    REditFrame = (HWND)(UINT)SendMessage(RClient, WM_MDICREATE, 0, (LONG)(LPMDICREATESTRUCT)&mdicreate);
    if (REditFrame == NULL)
        return 0;

    GetClientRect(REditFrame, &r);
    REditWnd = CreateWindow(
        "Edit", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_AUTOVSCROLL | ES_AUTOHSCROLL,
        0, 0, (r.right - r.left), (r.bottom - r.top), REditFrame, (HMENU)RRR_TEDIT, RInst, NULL);
...
LRESULT FAR PASCAL REditWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_MDIACTIVATE:
        if ((HWND)lParam == hWnd)
        {
            SendMessage(RClient, WM_MDISETMENU, (WPARAM)RMenuEdit, (LPARAM)RMenuEditWin);
            DrawMenuBar(GetParent(RClient));
            SetFocus(REditWnd);
            return 0;
        }
        else if (inFlag)
        { /* don't let them switch to another window */
            MessageBox(hWnd, "You must quit the editor before doing something else.", "R Editor",
    ...
    case WM_SYSCOMMAND: /* Disable minimizing */
        ...
        {
            MessageBox(hWnd, "You can't iconify the R Editor", "R Data Entry", MB_OK | MB_ICONEXCLAMATION);
            return 1l;
        }
        break;
    case WM_DROPFILES:
    ...
    case WM_COMMAND:
        ...
        case RRR_TEDIT:
            ...
            case EN_ERRSPACE:
            case EN_MAXTEXT:
                MessageBox(hWnd, "Text is too large.", "R Editor", MB_ICONEXCLAMATION | MB_OK);
                EmptyEd();
                return 0;
            }
            break;
...
SEXP do_edit(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    else if (x != R_NilValue || !strlen(REdfilename))
    { /* if x is R_NilValue then we are restoring and want to use the last file */
