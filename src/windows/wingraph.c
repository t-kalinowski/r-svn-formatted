...
int CALLBACK PaintMetafile(HDC hdc, HANDLETABLE *lpHTable, ENHMETARECORD *lpEMFR, int nObj, LPVOID lpData)
{
    double x0, x1, y0, y1;
    char *x;
...
static int Win_Open()
{
    ...
    RGraphWnd = (HWND)(UINT)SendMessage(RClient, WM_MDICREATE, 0, (LONG)(LPMDICREATESTRUCT)&mdicreate);
    if (RGraphWnd == NULL)
        return 0;
    ...
    RGBitMap =
        CreateCompatibleBitmap(Nhdc, graphicsRect.right - graphicsRect.left, graphicsRect.bottom - graphicsRect.top);
    if (RGBitMap == NULL)
    {
        MessageBox(RFrame, "couldn't alloc bitmap", "R Graphics", MB_ICONEXCLAMATION | MB_OK);
        return 0;
    }
