...
int CALLBACK PrintMetafile(HDC hdc, HANDLETABLE *lpHTable, ENHMETARECORD *lpEMFR, int nObj, LPVOID lpData)
{
    double x0, x1, y0, y1;
    char *x;
    HRGN hrgn;
...
BOOL RPrintGraph(HWND hwnd, HENHMETAFILE RMhmf)
{
    ...
    hdlgCancel = CreateDialog(RInst, (LPSTR) "PAbortDlg", hwnd, (DLGPROC)AbortPrintJob);
...
BOOL RPrintBitMap(LPBITMAPINFO lpbi, LPBYTE lpbm)
{
    ...
    hdlgCancel = CreateDialog(RInst, (LPSTR) "PAbortDlg", RConsoleFrame, (DLGPROC)AbortPrintJob);
    ...
    {
        MessageBox(RConsoleFrame, "Printer cannot display bitmaps", "Device Error", MB_OK);
        goto Error;
    }
    ...
    rcaps = StretchDIBits(pd.hDC, 20, 20, lpbi->bmiHeader.biWidth * xmult, lpbi->bmiHeader.biHeight * xmult, 0, 0,
                          lpbi->bmiHeader.biWidth, lpbi->bmiHeader.biHeight, lpbm, lpbi, DIB_RGB_COLORS, SRCCOPY);
...
void RPrintText(HWND hwnd, HWND TextWin)
{
    ...
    for (nColCopy = 0; nColCopy < (pd.Flags & PD_COLLATE ? pd.nCopies : 1); nColCopy++)
    {
        for (nPage = 0; nPage < nTotalPages; nPage++)
        {
            for (nNonColCopy = 0; nNonColCopy < (pd.Flags & PD_COLLATE ? 1 : pd.nCopies); nNonColCopy++)
            {
                nError = StartPage(pd.hDC);
                if (nError <= 0)
                    goto Error;
